...
| Enemy melee windup start via `ActorActionMessage::Melee` plus hitbox-entity lifecycle | Available | `docs/systems/brain-driver.md`, `crates/ambition_sandbox/src/features/ecs/brain_effects.rs` |
...
| Possession — take over a non-boss actor (Down+Interact hold) | Available (landed 2026-06-03); drive any enemy **or NPC** from player input through its own `ActorControlFrame` (movement scaled by `POSSESSED_MOVE_SPEED`); it flips to your faction (melee+ranged redirect at its former allies), the camera follows it, and on release you **vacate** — step out where it stood. Boss case + a possessed NPC's authored verbs + body-hide visual are handoffs | `crates/ambition_sandbox/src/possession.rs` |
...
| Sentry gauntlet (deployable auto-firing turret) | Available (landed 2026-06-03); `Attack` deploys a stationary turret that auto-fires player-faction bolts at the nearest enemy on a cadence for 5s — the first thing the player deploys that *attacks* (the puppy-slug summon is passive). Pairs with the vortex | `crates/ambition_sandbox/src/sentry.rs` |
| Dive gauntlet (offensive lunge-strike) | Available (landed 2026-06-03); `Attack` lunges the player along the aim (wall-stopped via `raycast_solids`, like blink) and cuts a one-shot damage corridor from start to landing — the kit's only *offensive-mobility* attack (blink is a defensive reposition). Overflow boss's signature drop | `crates/ambition_sandbox/src/dive.rs` |
| Meteor gauntlet (overhead AOE rain) | Available (landed 2026-06-03); `Attack` rains 5 falling player-faction projectiles onto a zone ahead — the only wielded attack that hits a band *from above* (every other strikes forward/centered). GNU-ton's signature apple-rain, wielded | `crates/ambition_sandbox/src/meteor.rs` |
...
| Gradient Sentinel boss fight | Available in current sandbox code; multi-special boss using focused `ActorActionMessage::Special` consumers | `crates/ambition_sandbox/src/features/bosses.rs`, `crates/ambition_sandbox/src/features/ecs/brain_effects.rs` |
...
| Cartography route durable state | Available; 10+ route flags and auto-quest hooks in the reviewed archive | `crates/ambition_sandbox/src/intro/route_state.rs`, `crates/ambition_content/src/quest.rs::default_quest_specs` |
