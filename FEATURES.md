...
| Blink ability (short collision-clamped teleport, the gameplay verb) | Available (landed 2026-06-03); wired ability — `Attack` teleports the player up to a fixed distance along aim, `raycast_solids`-clamped to stop short of walls; equippable from the OoT menu; sold by the merchant (45g). Distinct from the `BlinkWall`/`blink.md` lore | `crates/ambition_sandbox/src/blink.rs` |
