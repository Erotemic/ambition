...
| Connected gridvania content (authored end-to-end via the toolkit) | Available (landed 2026-06-03); a `gradient_ascent` staircase room → summit door → `gradient_summit_chamber` (25-coin reward), and a `mode_collapse_arena` off the hall of bosses — proving the area-create → IntGrid → entity/door pipeline. Difficulty/feel is a playtest pass | `tools/ambition_ldtk_tools/specs/gradient_ascent_area.ron`, `crates/ambition_sandbox/assets/ambition/worlds/sandbox.ldtk` |
