...
- **`SfxMessage` / `VfxMessage` / `DebrisBurstMessage`** `[stable]` — Vec-collector → MessageWriter pattern documented in `docs/archive/superseded-migrations/events-refactor-plan.md`. [audio.rs:53](crates/ambition_sandbox/src/audio.rs#L53), [fx.rs](crates/ambition_sandbox/src/fx.rs).
...
- **Shared `KinematicBody` sweep for enemies + NPCs** `[stable]` — `ambition_engine::kinematic::step_kinematic` is the single primitive that drives gravity + axis-separated collision for `EnemyRuntime`, `NpcRuntime`, and (eventually) RL agents / multiplayer characters. Same OneWay landing-from-above rule as `sweep_player_y`, plus a `drop_through` input that hostile / chasing enemies set when the player is meaningfully below them, so a goblin can follow the player through a one-way platform instead of getting pinned on top. Closes the long-standing TODO that "enemies and NPCs are not respecting collisions, gravity, and the environment" the same way the player is. [kinematic.rs](crates/ambition_engine/src/kinematic.rs); 6 unit tests cover Solid, OneWay-from-above, drop-through-OneWay, drop-through-stops-at-Solid, walks-off-ledge-falls, rising-through-OneWay-doesn't-stick.
...
- **Camera stability across body resizes** `[stable]` — `camera_follow` tracks a "standing-pose center" derived as `pos.y - (base_size.y - size.y) * 0.5` instead of raw `pos.y`. Body-mode transitions (Standing → Crouching/MorphBall/Sliding) keep feet planted by snapping `pos.y` by half the height delta; without compensation that snap propagated 1:1 into the camera, producing a 10.35px down-pop on slide entry that was visible only mid-room (clamped at room edges). Regression test: [crates/ambition_sandbox/tests/dash_stability.rs](crates/ambition_sandbox/tests/dash_stability.rs) asserts feet_y stable across grounded dash + slide.
...
- **Per-controller-profile filter defaults** `[stable]` — `ControllerProfileId::filter_defaults()` returns calibrated deadzone + trigger-threshold values per pad. Xbox 360 widens deadzones (0.27/0.30 vs 0.18/0.20) and the trigger hysteresis band (0.20–0.65 vs 0.30–0.55) to compensate for known stick / trigger drift. PlayStation tightens deadzones for its sharper sticks. `ControlSettings::apply_profile_defaults` stomps the active settings with the profile's calibrated values. [settings/controls.rs](crates/ambition_sandbox/src/settings/controls.rs).
  ...
  - **Touch Controls toggle** `[playable]` — `ControlSettings::touch_controls_visible` (default true) shows up as a "Touch Controls: on/off" row on the Controls page. Mirrors into `mobile_input::TouchControlsVisible` each Update via `sync_touch_visibility_from_settings`, so the on-screen joystick + buttons hide on the same frame the user toggles. Persisted via serde with `UserSettings`.
...
## Headless / rl_sim adapter

- **`rl_sim` feature flag** `[stable]` — default-enabled; gates the `crate::rl_sim` module (SandboxSim, AgentAction, AgentObservation, SandboxSimOptions, TimestepMode) plus the four RL/replay binaries (`headless`, `rl_random_walker`, `rl_smoke`, `trace_replay`) all of which `required-features = ["rl_sim"]`. Disable for stripped builds via `--no-default-features --features visible`.
