...
| Smash brain verb-selection by range (ranged at mid-range, melee up close) | Available (landed 2026-06-03); the enemy/player unification flex — a ranged-capable Smash actor fires its ranged verb at mid-range on a brain-side cadence (`RANGED_COOLDOWN_S`), then closes for the melee finish; goblins (`MediumStriker`) now throw rocks. No throw animation / balance pass yet | `crates/ambition_sandbox/src/brain/smash/mod.rs` (`maybe_substitute_ranged`) |
