...
- ✅ **Warning cleanup pass** — `cargo check -p ambition_sandbox --lib` went from 102 warnings at session start to ~12 at session end. Reductions came from removing genuinely-dead facade re-exports, removing the unused `clear_combat_slots_on_room_change` system, and targeted `#[cfg_attr(not(test/feature), allow(dead_code))]` on items the lib build can't see but tests/features do — plus module-wide `#![allow(dead_code)]` on the un-wired `dev::mechanics` HUD catalog, the `presentation::parallax` orphan stage, the Android-conditional `host::platform::android` module, the power-policy scaffold, and the `ui_nav::list` reserved menu helpers.
