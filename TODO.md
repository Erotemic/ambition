...
- [~] **Universal "brain" interface — every controllable entity shares one input shape** `[V4/D5]` (2026-05-24: Chunks 1–4f shipped; remaining is daytime work) - Design doc: [`docs/planning/universal-brain-interface.md`](docs/planning/universal-brain-interface.md). Overnight session landed: extended ActorControlFrame for player verbs (Chunk 1); scaffolded `crates/ambition_sandbox/src/brain/` with Brain enum + 7 reusable templates (StandStill/Patrol/Wanderer/MeleeBrute/Skirmisher/Sniper/BossPattern) + ActionSet per-entity capability + player brain (Chunk 2); migrated NpcRuntime onto Brain::StateMachine — peaceful NPCs tick through the brain seam, hostile-flip still swaps to EnemyRuntime (Chunk 3); attached Brain components to the player entity and wired tick_player_brains to fill ActorControl each tick (Chunks 4b/4c); expanded PlayerBody to cover most readers and migrated audio/env + ECS actor tick to read from it (Chunks 4d–f). 688 sandbox tests + 261 engine tests green. **Daytime work that remains:** (a) Enemies + bosses migrate onto Brain — the templates exist (MeleeBrute, Skirmisher, BossPattern) but EnemyRuntime / BossRuntime still drive themselves. (b) ActionSet effect-resolve path wired into the EFFECTS stage so per-entity attack specs actually spawn hitboxes via the resolver. (c) `ae::Player` decomposition completes — `update_player` consumes ActorControl, ae::Player deleted. Track in [`TODO-controllable-entity.md`](TODO-controllable-entity.md). Below is the original design context for posterity:
