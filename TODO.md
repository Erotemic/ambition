...
- **Universal "brain" interface — every controllable entity (player, NPC, enemy, boss, RL agent) shares one input shape** `[V4/D5]` - Design doc: [`docs/planning/universal-brain-interface.md`](docs/planning/universal-brain-interface.md). Continuation of the controllable-entity unification arc that already merged enemies + bosses behind `ActorControlFrame`. The full design:
