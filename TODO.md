...
- [~] I should be able to swap my starting character for PCA or a pirate or any other character instead of using player-robot and nothing in the game should change except my abilities. We could imagine the game has a choose your character list. This shouldn't be swapping a skin onto a bespoke player box. This should just be spawning the character and making its brain the keyboard input.
  - **Landed (2026-07-03):** `StartingCharacter` resource picks which `character_catalog.ron` id the local player spawns AS — the player box keeps `Brain::Player`, the home-body integration, and its traversal kit, but wears the chosen character's SPRITE + combat moveset + name. Any catalog id works; `overlay_character_moveset` overlays the character's authored melee/ranged/special onto the player kit (peaceful characters stay playable). Selection surface is LAUNCH-TIME: `AMBITION_START_CHARACTER=<id> cargo run -p ambition_app`, and `capture_scene --character <id>`. Curated `player::PLAYABLE_ROSTER`. See `player/starting_character.rs`.
  - **Remaining:** (1) an IN-GAME "choose your character" menu/hotkey — deferred because a live swap needs risky despawn/respawn of the player entity (162 `With<PlayerEntity>` sites); (2) drive the player's TRAVERSAL abilities (fly/blink/dash `AbilitySet`) from the character too — today only the combat `ActionSet` is per-character, the traversal kit stays the player's (god-mode), so every character can navigate every room; unifying `AbilitySet` ⇔ the character `CombatCapabilities` is the follow-up; (3) `body_kind: Floating` characters (PCA, parrot) still fall under player gravity — the player box does not adopt the character's gravity mode yet.
