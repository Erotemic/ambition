...
### Mechanics & combat
- [~] **Ladder + climbable-zone primitive** `[V4/D3]` — engine primitive + traversal landed 2026-05-07. End-to-end functional in code: drop a `ClimbableRegion` into a `World`, the engine populates `Player.climbable_contact` each tick, the body-mode driver enters/exits Climbing on Up/Down/Jump, `integrate_climb` drives motion. Six engine tests + four sandbox driver tests pin the contract. Remaining:
  - LDtk authoring: IntGrid value + Entity option ("Ladder", "Vine", "ClimbableWall") + repair/round-trip pass via `tools/validate_ambition_ldtk.py`
