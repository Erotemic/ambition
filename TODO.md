...
- **Cryptography crew batch 2 (7 NPCs)** `[V3/D3]` - Batch 1 (Bob, Alice, Eve, Mallory, Trent, Judy) landed 2026-05-21; see `docs/concepts/cryptography-crew.md` for the full roster and the silhouette-intent sketches for the remaining seven. Each gets a unique silhouette via a new combination of body plan + new outfit/hair/prop in `toon_side.py` (~150 lines per character), a review YAML in `configs/review/`, an entry in `RUNTIME_REVIEW_NPCS` in `cli.py`, and the canonical PNG installed via `draw-runtime-npcs`. Anti-stereotype rule applies: pick a prop that SUBVERTS the cryptographic role, not one that literalizes it.
  - **Trudy** — intruder, parkour silhouette + lockpick, NOT a ninja
  - **Craig** — cracker, wide-brim hat + stethoscope (safe-cracker, not script-kiddie)
  - **Sybil** — pseudonymous attacker, many-braids hair + mask-stack prop, NOT a hooded shadow
  - **Victor** — verifier, square-fringe haircut + magnifier, sharp/precise
  - **Peggy** — prover, athletic + long pointer + streamer cape, dynamic
  - **Walter** — warden, tricorn hat + lantern, dignified watchman
  - **Olivia** — oracle, veiled face + many-layered draped robe, still
