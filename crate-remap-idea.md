...
The current repo has LDtk as the sandbox world source, but it also has active-area composition, transition repair, room graph validity, loading-zone semantics, and hot reload behavior. Those should not be duplicated in `ambition_sandbox` and `ambition_game`.
...
This is the shared Bevy frontend layer used by both `ambition_sandbox` and `ambition_game`.
...
```text
ambition_sandbox = lab content + debug UX
