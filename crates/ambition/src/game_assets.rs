...
//! [`SandboxAssetCatalog`](ambition_actors::assets::sandbox_assets::SandboxAssetCatalog)
...
//! are `ambition_actors`, the `Startup` ordering anchor
//! ([`PlatformerPresentationSetupSet`](crate::presentation::PlatformerPresentationSetupSet))
//! is `ambition_render`, and `ambition_host` — the obvious home — is forbidden
//! from naming `ambition_actors` at all (its own module docs say so, and
...
//! app.add_plugins(ambition::game_assets::PlatformerAssetsPlugin::for_experience(
...
use ambition_actors::assets::sandbox_assets::SandboxAssetCatalog;
use ambition_actors::boss_encounter::BossCatalog;
use ambition_actors::ldtk_world::WorldManifest;
...
impl Plugin for PlatformerAssetsPlugin {
    fn build(&self, app: &mut App) {
        ...
        let catalog = ambition_actors::assets::sandbox_assets::build_sandbox_catalog(
            &config,
            &character_catalog,
            &boss_catalog,
            &music,
