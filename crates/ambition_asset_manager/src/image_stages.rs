...
//! never rollback state.
//!
//! ⛔⛔ **AND IT IS NO LONGER "A DIAGNOSTIC NOTHING AUTHORITATIVE READS".** That
//! is what this paragraph said, and it stopped being true when
//! `inspect_room_asset_manifest` began reading [`ImageStageLedger::is_awaiting_gpu`]
//! to decide whether a room or the startup cover may lift. A reveal barrier is
//! not a report. Recorded here rather than quietly deleted, because the
//! sentence was the reason nobody scrutinised the ledger's identity model, and
//! the model has real limits:
//!
//! * rows are keyed by `UntypedAssetId`, which is a PER-ARENA
//!   `{generation, index}` and collides between Apps in one process — measured,
//!   and why `common::resident_character_pages` answers residency from the
//!   App's own `Assets<Image>` and uses the ledger only as a classifier;
//! * the ledger's lifetime is the PROCESS, not an App's.
//!
//! ⭐ The render-world FACT has already been moved off it: [`RenderWorldPresent`]
//! is an App resource, and `is_awaiting_gpu` takes it as an argument rather than
//! reading a global. What remains global is the per-id prepared stamp. For the
//! shipped single-App game that is sound; for two Apps in one process it is the
//! same collision the residency guard had to solve, and moving the stamp to an
//! App-local store is the open half.
//!
