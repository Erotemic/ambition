...
///
/// ⭐ THIS EXISTS BECAUSE THE UNION WAS DERIVED IN FOUR PLACES AND ONE OF THEM
/// GOT IT WRONG. Per-attempt state — a broken brick, a spent power block, a
/// tripped monitor, a queued player hit — must be retracted both when the room
...
///
/// ```ignore
/// let crossed = replays.read().count() > 0 || loads.read().count() > 0; // ⛔ WRONG
/// ```
///
/// — and a `MessageReader` whose cursor does not advance re-reads that message
/// on a later frame, retracting state a second time for a boundary that was
/// already handled. Three of the four call sites carried a comment warning about
/// exactly this; the fourth (`void_pending_player_hits_at_lifecycle_boundaries`)
/// was written with the short-circuit and the comments could not stop it.
...
impl FreshAttempt<'_, '_> {
    /// Did a fresh attempt begin this frame, in ANY room?
    ///
    /// The right question for state that is per-attempt but not per-room: you
    /// can only stand in one room, so any boundary re-arms everything.
    pub fn began(&mut self) -> bool {
        let loaded = self.loads.read().count() > 0;
        let replayed = self.replays.read().count() > 0;
        loaded || replayed
    }

    /// Did a fresh attempt begin this frame for the NAMED room?
    ///
    /// A replay is always in the room you are in, so it counts regardless; only
    /// the load leg is filtered.
    pub fn began_in(&mut self, room_id: &str) -> bool {
        // ⛔⛔ `fold`, NOT `any`. `any` STOPS AT THE FIRST MATCH, leaving later
        // messages unread -- which is precisely the cursor defect this type was
        // built to make unrepresentable, committed inside the type itself. A
        // GPT review found it 2026-09-05. `fold` visits every message, so the
        // cursor is fully drained whatever the answer turns out to be.
        let loaded = self
            .loads
            .read()
            .fold(false, |seen, message| seen || message.room_id == room_id);
        let replayed = self.replays.read().count() > 0;
        loaded || replayed
    }
}
...
#[cfg(test)]
mod fresh_attempt_cursor_tests {
    use super::*;
    use bevy::prelude::*;

    /// ⛔⛔ TWO MATCHING LOADS IN ONE FRAME MUST BOTH BE CONSUMED.
    ///
    /// `began_in` used `.any(..)`, which stops at the first match and leaves the
    /// rest of the frame's traffic in the reader. The second message would then
    /// be delivered on a LATER frame and read as another fresh attempt --
    /// retracting per-attempt state for a boundary that already happened. That
    /// is the exact cursor defect `FreshAttempt` exists to make unrepresentable,
    /// committed inside `FreshAttempt`. Found by a GPT review 2026-09-05.
    ///
    /// ⭐⭐ THE WITNESS IS TWO CALLS IN ONE FRAME, and the first version of this
    /// test could not see the defect at all. Writing two messages and reading
    /// across two `update()`s proved nothing: `add_message` registers the
    /// double-buffer sweep, so the message a short-circuit left behind is
    /// DROPPED before the next frame reads it, and the poisoned code returned
    /// the same answers as the fixed code. Asking twice in ONE frame is immune
    /// to that -- a fully drained reader must answer `false` the second time.
    #[test]
    fn began_in_drains_every_matching_load_not_just_the_first() {
        #[derive(Resource, Default)]
        struct Answers(Vec<bool>);
        ...
        for _ in 0..2 {
            app.world_mut()
                .resource_mut::<Messages<ambition_platformer2d_world::rooms::RoomLoaded>>()
                .write(ambition_platformer2d_world::rooms::RoomLoaded {
                    room_id: "cove".to_string(),
                });
        }

        fn ask_twice(mut attempt: FreshAttempt, mut seen: ResMut<Answers>) {
            seen.0.push(attempt.began_in("cove"));
            seen.0.push(attempt.began_in("cove"));
        }

        app.add_systems(Update, ask_twice);
        app.update();

        assert_eq!(
            app.world().resource::<Answers>().0.as_slice(),
            &[true, false],
            "the second ask in the SAME frame still saw a fresh attempt, so the \
             first ask stopped at the first matching message and left the rest of \
             the frame's traffic in the reader"
        );
    }
}
