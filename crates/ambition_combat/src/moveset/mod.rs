...
impl MovePlayback {
    ...
    /// and mean nothing.
    ///
    /// Returns the REACH ITSELF, body-local — the leading edge of the grab box,
    /// not the box. The caller resolves facing and the acceleration frame,
    /// because it has them and this does not.
    ///
    /// ⛔⛔ IT USED TO RETURN `(offset, half_extent)` AND LET EACH CALLER ADD
    /// THEM, which put `offset.x + half.x` in the two view sites while the
    /// BRAIN computed the same sum for `AttackCandidate::reach`. Three copies of
    /// one formula answering two questions about one move: change one and the
    /// line a player reads parts company with the distance the AI aims at, and
    /// nothing fails, because both stay self-consistent. ⇒ The sum lives once,
    /// on `CaptureAttemptParams::reach_x`, and this hands out its answer.
    pub fn live_capture_reach(&self) -> Option<ae::Vec2> {
        self.spec
            .windows
            .iter()
...
/// named for when it is refused.
///
/// ⛔⛔ A REFUSED PRESS WITH NOTHING BEHIND IT IS A DEAD BUTTON, and a player
/// cannot tell one from a dropped input. The gates are right to REFUSE rather
/// than no-op — `MoveGates`' own doc calls a rule enforced after acceptance "a
/// silent failure with a comment" — but refusing is only half an answer. This is
/// the other half, and it is AUTHORED: a move that costs something may say what
/// happens when the body cannot pay.
///
/// ⭐ ONE PREDICATE, BOTH ROADS, WHICH IS THE POINT OF THE FUNCTION EXISTING.
/// The trigger road and the cancel road each ask the same four affordances, and
/// a gate enforced on one of two entry paths is a gate somebody walks around
/// without meaning to — the cancel road's own comments say so twice. Passing the
/// predicate in means the fallback cannot drift between the two the way the
/// checks themselves once did.
///
/// ⚠ ONE HOP. The variant's own `when_refused` is not followed: that bounds a
/// cycle without a visited-set and keeps the authored meaning legible. The
/// variant faces the SAME affordances, so a fallback is not a back door around
/// the gates — if it is refused too, the press is refused, which is the dead
/// button the author asked for.
fn accepted_or_variant(
    spec: ambition_entity_catalog::MoveSpec,
    contract: &ambition_entity_catalog::MovesetContract,
    affordable: impl Fn(&ambition_entity_catalog::MoveSpec) -> bool,
) -> Option<ambition_entity_catalog::MoveSpec> {
    if affordable(&spec) {
        return Some(spec);
    }
    spec.gates
        .when_refused
        .as_deref()
        .and_then(|id| contract.move_by_id(id))
        .filter(|variant| affordable(variant))
        .cloned()
}
...
pub fn trigger_moveset_moves(
    ...
    ) in &mut bodies
    {
        ...
        if let Some(mut pb) = playback {
            ...
            // ⛔⛔ THE CANCEL WINDOW IS ASKED ABOUT THE PRESS, NOT ABOUT THE
            // VARIANT — and this is BEFORE the affordances for that reason.
            // An author writes `cancelable into ["limit_dive"]` about the move
            // the player is reaching for; whether the body can afford it, and
            // therefore which of the two specs actually starts, is the gates'
            // business and not the window's. Asked the other way round, a
            // fighter whose meter was short would find the cancel itself refused
            // because the window had never heard of the fallback's id.
            //
            // ⚠ SCOPED, so the borrow of `spec.id` ends before `spec` moves into
            ...
            // gate enforced on one of two entry paths is a gate somebody walks
            ...
            // forward smash.
            //
            // ⛔⛔ AND "THIS MOVE" IS THE ONE THAT ACTUALLY STARTS, which is why
            // this now reads AFTER the variant is chosen. Read before it, a
            // charge banked against the priced move would be handed to its
            // cheaper stand-in — a fighter who could not afford the Limit
            // cashing the Limit's charge into the ordinary move.
