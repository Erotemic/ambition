...
/// ⛔⛔ A REFUSED PRICED MOVE RUNS ITS AUTHORED VARIANT INSTEAD OF NOTHING —
/// AND ON BOTH ROADS.
///
/// The meter gate above is right to REFUSE (`MoveGates`' own doc: a rule
/// enforced after acceptance "is not a rule, it is a silent failure with a
/// comment"), but a refusal with nothing behind it is a DEAD BUTTON, and a
/// player cannot tell that from a dropped input. `MoveGates::when_refused` is
/// the authored answer: the press always produces a move.
///
/// ⭐ THREE ARMS, BECAUSE TWO WOULD PASS ON A BROKEN IMPLEMENTATION. "Rich plays
/// the priced move" and "poor plays the variant" are both satisfied by a system
/// that ignores the price and plays whatever it resolved — so the third arm
/// removes the fallback and requires the DEAD BUTTON back. A fallback that
/// cannot be switched off is not a fallback, it is the gate not working.
///
/// ⛔ AND THE CANCEL ROAD, for the reason the meter test beside this one gives
/// in its own words: deleting that road's copy of the meter check once left
/// every test in this crate green. A fallback on one of two entry paths is the
/// same defect wearing the opposite sign — a fighter who gets the cheap dive by
/// pressing and NOTHING by cancelling.
#[test]
fn a_refused_priced_move_falls_through_to_its_authored_variant() {
    use ambition_entity_catalog::{MoveGates, MoveSpec, MovesetContract};

    // The cheap stand-in is a DIFFERENT move with its own id, bound to no verb:
    // it is reachable only through the fallback, which is what makes "the
    // variant ran" an unambiguous observation.
    let cheap = || MoveSpec {
        id: "swat_cheap".to_string(),
        ..swat()
    };
    let contract = |fallback: Option<&str>| MovesetContract {
        verbs: [("special".to_string(), "swat".to_string())]
            .into_iter()
            .collect(),
        moves: vec![
            ...
            cheap(),
            // A fallback priced ABOVE anything the fixture holds. It exists so
            // the guard can watch the variant being REFUSED in its turn — see
            // the fourth arm.
            MoveSpec {
    ...
    let world = |meter: f32, fallback: Option<&str>, cancelling: bool| -> (Option<String>, f32) {
        let mut app = App::new();
        app.insert_resource(
            ambition_characters::actor::character_catalog::CharacterCatalog::empty(),
        );
        ...
        let mut body = app.world_mut().spawn((
            ...
            ActorFaction::Enemy,
            ActorMoveset(contract(fallback)),
            ActorControl(frame),
            mana,
        ));
        if cancelling {
            // ⚠ The window names the PRESS (`swat`) and has never heard of
            // `swat_cheap` — which is the point: the cancel window is asked
    ...
    for cancelling in [false, true] {
        let road = if cancelling { "cancel" } else { "trigger" };

        // ── PAID: the priced move itself, and the meter actually debited.
        let (played, left) = world(40.0, Some("swat_cheap"), cancelling);
        assert_eq!(
            played.as_deref(),
            Some("swat"),
            "{road} road: a fighter who COULD pay got `{played:?}` instead of the \
             priced move — the fallback is firing when the price is met, which \
             makes the expensive move unreachable"
        );
        assert_eq!(
            left, 10.0,
            "{road} road: the priced move started and the meter was not debited"
        );

        // ── REFUSED: the variant, and NOT a single point spent on it.
        let (played, left) = world(29.0, Some("swat_cheap"), cancelling);
        assert_eq!(
            played.as_deref(),
            Some("swat_cheap"),
            "{road} road: a fighter short of the price got `{played:?}`. The \
             authored variant is what a refused press is supposed to produce; \
             `None` here is the dead button this field exists to end"
        );
        assert_eq!(
            left, 29.0,
            "{road} road: the FALLBACK charged the priced move's cost — the \
             variant must be free unless it prices itself"
        );

        // ── NO FALLBACK AUTHORED: the dead button is preserved on purpose.
        let (played, left) = world(29.0, None, cancelling);
        assert_eq!(
            played, None,
            "{road} road: a move with NO `when_refused` still produced \
             `{played:?}`. Every assertion above would pass on a system that \
             plays some other move whenever it refuses one, and that is not a \
             gate — a move without an authored variant must still refuse."
        );
        assert_eq!(
            left, 29.0,
            "{road} road: a refused move with no variant still charged the meter"
        );

        // ── THE VARIANT IS GATED TOO, and this arm is here because the other
        // three do not need it to pass. Deleting the fallback's own affordance
        // check left all of them green: a back door that only opens when the
        // front one is shut still opens. ⇒ A fallback nobody can afford must be
        // refused exactly as a directly-pressed move is.
        let (played, left) = world(29.0, Some("swat_dear"), cancelling);
        assert_eq!(
            played, None,
            "{road} road: the fallback `swat_dear` costs 100 and the body holds              29, and it played anyway (`{played:?}`). A variant that skips the              gates is a back door around every price in the moveset — author a              cheap move, name it as the fallback of an expensive one, and the              expensive one is free."
        );
        assert_eq!(
            left, 29.0,
            "{road} road: an unaffordable fallback was refused and still              charged the meter"
        );
    }
}
...
mod technique_flow {
    ...
    /// ⛔⛔ A FLOW GETS **ONE** CONTACT DECISION PER OCCURRENCE, AND A SECOND
    /// `Wait` ON CONTACT PASSES WITHOUT A SECOND HIT.
    ///
    /// `MovePlayback`'s contact flags are per-OCCURRENCE latches: `landed_hit`,
    /// `connected_hit` and `blocked_hit` are set true by the strike seam and by
    /// `mark_move_playback_resolved_hits`, and nothing clears them until the move
    /// ends. `contact()` derives `overlapped` from all three. ⇒ Once anything has
    /// been touched, EVERY later `Wait { on: Overlapped }` in the same flow is
    /// already satisfied and falls through on the tick it is reached.
    ///
    /// ⭐ THE DESIGN CONSEQUENCE, which is why this is pinned rather than left to
    /// be discovered: **a multi-beat combo confirm is not authorable.** "Hit,
    /// then hit AGAIN, then finish" needs per-BEAT contact, and the signal
    /// vocabulary is per-occurrence. Both flows the roster ships are
    /// single-decision, and that is a consequence of this rather than a
    /// coincidence of taste.
    ///
    /// ⚠ THIS TEST PINS BEHAVIOUR IT DOES NOT ENDORSE. If per-beat contact ever
    /// arrives, this test SHOULD fail — and its failure is the reminder that the
    /// flow docs, `TechniqueFlow::problems()` and this note all describe the old
    /// rule. It is a boundary marker, not a guarantee.
    ///
    /// ⛔ AND THE WHIFF ARM IS WHAT MAKES IT A MEASUREMENT. Without it, "both
    /// effects fired" is equally true of a flow that ignores its waits entirely,
    /// which is a different and much worse defect.
    #[test]
    fn a_second_wait_on_contact_passes_on_the_first_hit() {
        let two_beats = || TechniqueFlow {
            nodes: vec![
                FlowNode::Wait {
                ...
                effect("smash.first_beat", 2),
                // The second confirm an author would write for a combo: "and if
                // THAT one touches too…". It cannot ask that question.
                FlowNode::Wait {
                    on: FlowSignal::Overlapped,
                    timeout_s: 0.05,
                    then: 3,
                ...
                effect("smash.second_beat", 4),
                FlowNode::Finish,
            ],
        };

        let (hit, _) = run(two_beats(), 1, true);
        assert_eq!(
            hit,
            vec![
                "smash.first_beat".to_string(),
                "smash.second_beat".to_string()
            ],
            "ONE contact should satisfy both waits, because the signal is a \
             per-occurrence latch. If this now fails with only the first beat, \
             contact has become per-beat and a combo confirm just became \
             authorable — update the flow docs and this note."
        );

        // ⛔ THE CONTROL: with no contact at all the first wait times out, so
        // neither beat fires. Without this, the assertion above is satisfied by
        // a flow that never waits for anything.
        let (whiff, _) = run(two_beats(), 3, false);
        assert!(
            whiff.is_empty(),
            "a flow that touched nothing still emitted {whiff:?}, so its waits \
             are not waiting and the latch above proves nothing"
        );
    }
