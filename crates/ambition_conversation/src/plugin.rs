...
//!
//! ⛔ **this module used to have zero registrations of its own, and that was the
//! carve blocker no import graph could see.** Every measurement said
//! `conversation` was the strongest extraction candidate in the actor monolith —
//! ~1,800 lines, zero `crate::` edges out, zero in — and its own header claimed
//! *"the carve is a `Cargo.toml`"*. It was not:
//! `features::FeatureInteractionSchedulePlugin` held [`ActiveConversation`],
//! [`ConversationCutBark`], seven `NarrativeInputPlugin` installs, and three of
//! this module's systems wedged into ONE anonymous `.chain()` between
//! `interact_ecs_actors_and_switches` and the chest systems.
//!
...
//!
//! [`ConversationPlugin`] is the answer to the first half — the state, the port
//! channel, the payload this module itself defines, and its own systems now
//! belong to it. The second half is
//! [`FeatureInteractionSet`](ambition_platformer2d_shared_tangle::schedule::FeatureInteractionSet),
//! the named ordering vocabulary that replaced the anonymous chain: this plugin
//! states WHICH PHASE each of its systems runs in and says nothing about the
//! interaction, cast or encounter systems it interleaves with. The total order
//! is declared once, by the owner of the phase.
//!
...
//! `FeatureInteractionSchedulePlugin`, and that is the correct seam rather than
//! a leftover. The ledger's payload list IS the classification of which Yarn
//! commands are gameplay-bearing (see [`super::ledger`]), and each payload
//! belongs to whoever CONSUMES it: `ChallengeRequested`, `BrainCommand` and
//! `ReleaseProvocation` are `features` types this module cannot even name after
//! a carve, and `SetFlagRequested`, `ItemGrantRequested` and
//! `ShopTransactionRequested` are applied by `features::bus` and
//! `crate::items::narrative`. Conversation provides the ledger MECHANISM; it
//! does not decide another domain's vocabulary.
//!
