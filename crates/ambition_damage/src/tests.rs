...
// ⭐⭐ **THE CM1 KNOCKBACK-SCALING ARMS MOVED TO THE LAW THEY ARE ABOUT.**
// `scaled_knockback` was `ambition_combat::util`'s and the fighter brain kept
// its own simpler copy; both are gone and
// `ambition_entity_catalog::launch::launch_speed` is the one line. Its tests
// went with it — parity at zero growth, fixed knockback under every scale, the
// percent term against the base, and the crossover the brain used to get
// wrong. What stays here is the one arm below, which is about GRAVITY
// CONJUGATION and merely needed a launch speed to rotate.
...
fn scaled_launch_speed_conjugates_under_rotated_gravity() {
    ...
    let feel = Platformer2dFeelTuningMonolith::default();
    // `launch_speed(100, 2, {damage 30, weight 2, scale 1})` — the law's own
    // arithmetic, stated as its result so this file needs no dependency on the
    // catalog to assert something about gravity.
    let launch_speed = 130.0_f32;
