...
impl BossEncounterState {
    ...
    /// Pick the right `BossPatternSchedule` for the current phase.
    /// Returns `None` for non-attacking phases (Dormant / Intro /
    /// Transition / Stagger / Death).
    ///
    /// Today this only knows about the gradient_sentinel phases; real
    /// authored content can either supply schedules per-spec (future
    /// `BossEncounterSpec.schedules: HashMap<phase, BossPatternSchedule>`
    /// extension) or rely on the spec id matching one of the
    /// hard-coded entries here.
    pub fn current_pattern_schedule(&self) -> Option<crate::BossPatternSchedule> {
        if !self.phase.is_attacking() {
            return None;
        }
        match (self.spec.id.as_str(), self.phase) {
            ("gradient_sentinel", BossEncounterPhase::Phase1) => {
                Some(crate::BossPatternSchedule::gradient_sentinel_phase1())
            }
            ("gradient_sentinel", BossEncounterPhase::Phase2) => {
                Some(crate::BossPatternSchedule::gradient_sentinel_phase2())
            }
    ...
    /// Convenience: combine `current_pattern_schedule` with
    /// `BossPatternSchedule::evaluate`. Returns the active beat (step
    /// + sub-phase + progress) at the current `phase_elapsed`. Bevy
    /// boss controllers call this each tick to drive vfx / hitboxes /
    /// dash motion.
    ///
    /// Owns the schedule rather than borrowing because
    /// `current_pattern_schedule` returns by-value today; if/when
    /// schedules move onto `BossEncounterSpec` as a `HashMap`, this
    /// can switch to a borrowed return type.
    pub fn evaluate_pattern(
        &self,
    ) -> Option<(
        crate::BossPatternSchedule,
        crate::BossBeatPhase,
        usize,
        f32,
        crate::BossPatternStep,
    )> {
        let schedule = self.current_pattern_schedule()?;
        let beat = schedule.evaluate(self.phase_elapsed, true)?;
        let step = *beat.step;
        let phase = beat.phase;
        let index = beat.step_index;
        let progress = beat.phase_progress;
        Some((schedule, phase, index, progress, step))
    }
...
mod tests {
    ...
    #[test]
    fn current_pattern_schedule_returns_none_in_non_attacking_phases() {
        let s = BossEncounterState::new(BossEncounterSpec::gradient_sentinel());
        assert!(matches!(s.phase, BossEncounterPhase::Dormant));
        assert!(s.current_pattern_schedule().is_none());
    }

    #[test]
    fn current_pattern_schedule_returns_phase1_when_in_phase1() {
        let mut s = BossEncounterState::new(BossEncounterSpec::gradient_sentinel());
        let _ = s.set_phase(BossEncounterPhase::Phase1);
        let sched = s
            .current_pattern_schedule()
            .expect("phase 1 should have a schedule");
        assert_eq!(sched.phase, 1);
        assert_eq!(sched.boss_id, "gradient_sentinel");
    }

    #[test]
    fn current_pattern_schedule_returns_traversal_pattern_in_enrage() {
        let mut s = BossEncounterState::new(BossEncounterSpec::gradient_sentinel());
        let _ = s.set_phase(BossEncounterPhase::Enrage);
        let sched = s
            .current_pattern_schedule()
            .expect("enrage should have a schedule");
        assert_eq!(sched.phase, 3, "enrage should map to the traversal pattern");
    }

    #[test]
    fn evaluate_pattern_returns_active_beat_with_phase_elapsed() {
        let mut s = BossEncounterState::new(BossEncounterSpec::gradient_sentinel());
        let _ = s.set_phase(BossEncounterPhase::Phase1);
        s.phase_elapsed = 0.0;
        let (sched, phase, idx, progress, step) =
            s.evaluate_pattern().expect("phase 1 should evaluate");
        assert_eq!(sched.phase, 1);
        assert_eq!(idx, 0);
        assert_eq!(phase, crate::BossBeatPhase::Telegraph);
        assert!(progress < 0.05);
        let _ = step; // shape check
    }

    #[test]
    fn evaluate_pattern_advances_with_phase_elapsed() {
        let mut s = BossEncounterState::new(BossEncounterSpec::gradient_sentinel());
        let _ = s.set_phase(BossEncounterPhase::Phase1);
        let sched = crate::BossPatternSchedule::gradient_sentinel_phase1();
        // Move to half-way through step 0's active phase.
        let s0 = sched.steps[0];
        s.phase_elapsed = s0.telegraph + s0.active * 0.5;
        let (_sched, phase, idx, progress, _step) =
            s.evaluate_pattern().expect("phase 1 should evaluate");
        assert_eq!(idx, 0);
        assert_eq!(phase, crate::BossBeatPhase::Active);
        assert!(
            (progress - 0.5).abs() < 0.05,
            "active progress should be ~0.5, got {progress}"
        );
    }
}
