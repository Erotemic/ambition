...
//!
//! ⚠ Lives in the FLOOR — `ambition_engine_core` depends on no workspace crate,
//! which is what lets every domain implement [`SnapshotState`] for its OWN
//! types. It was `ambition_runtime::rollback::codec` until 2026-07-30, and
//! `ambition_runtime` depends on twenty domain crates while none depends on it,
...
// the call site — not `ambition_runtime`, where they used to live.
