...
fn drive_shell_pause_menu(
    ...
    // ⛔⛔ THE LAUNCHER IS A MENU, AND THERE IS NOTHING TO PAUSE BEHIND IT.
    // Reported by Jon 2026-09-05: Escape on the title screen opened this menu
    // OVER the game picker, both stayed live, and which one answered a press
    // looked random.
    //
    // `ShellPauseMenuSuppressed` did not cover it and could not: the host sets
    // it from `in_base_mode`, which asks whether AMBITION'S OWN SESSION is live.
    // This module's doc says the two menus "partition every LIVE SESSION with no
    // overlap" -- true, and beside the point, because the title screen is the
    // state with NO session at all. A scope phrase doing load-bearing work.
    //
    // ⇒ Asked here rather than folded into that flag, because the flag has ONE
    // writer (the host's bridge) and a second contributor to one boolean is the
    // shape this fix exists to remove, not to add. The launcher lives in this
    // crate, so asking it costs no export and no new seam.
    ...
    // The launcher owns the screen and brings its own chrome, so this menu must
    // not open over it -- and folds shut if the launcher came up while it was
    // open.
...
mod tests {
    ...
    /// ⛔⛔ JON'S BUG, 2026-09-05: Escape on the TITLE SCREEN opened this menu
    /// over the game picker, both menus stayed live, and which one answered a
    /// press looked random.
    ///
    /// `ShellPauseMenuSuppressed` could not have caught it: the host sets that
    /// from `in_base_mode`, which asks whether AMBITION'S OWN SESSION is live,
    /// and the title screen is the state with NO session. The module doc's
    /// "partition every LIVE SESSION with no overlap" is true and covers nothing
    /// here.
    #[test]
    ...
    /// And it lets go if the launcher arrives while it is already open — a
    /// quit-to-home from a live session, which is exactly how a player reaches
    /// the title screen with the menu up.
    #[test]
    ...
    /// ⚠ THE CONTROL, and it is the one that matters: a standalone demo app
    /// composes this shell with NO launcher, and there this menu IS the pause
    /// menu. A fix that yielded unconditionally would take the only system menu
    /// away from Sanic, Mary-O and the pocket demo.
    #[test]
