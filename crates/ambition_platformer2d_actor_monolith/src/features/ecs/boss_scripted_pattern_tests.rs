...
fn gradient_sentinel_every_special_strike_is_a_moveset_effect() {
    ...
    let moveset = boss_attack_moveset(
        ...
        behavior
            .combat_size
            .unwrap_or(ambition_platformer2d_core::Vec2::new(100.0, 100.0)),
        &telegraph_windows,
    )
...
fn boss_motion_respects_world_collision_against_a_wall() {
    ...
    seed.body =
        ambition_body_seed::ActorBody::from_abilities(ambition_platformer2d_core::AbilitySet::classic_actor(), true, seed.kin.size);
    let behavior = BossBehaviorProfile::clockwork_warden();
