...
mod authored_enemy_reads_its_character {
    ...
    /// The same no-blueprint road, for a character that AUTHORS its abilities
    /// (as the Hall's `mary_o`/`sanic` do): "no blueprint" means no locomotion,
    /// not "nobody described this body", so the live `AbilityBase` and
    /// `BodyAbilities` carry what it authored from the construction frame.
    #[test]
    fn a_prepared_npc_without_a_blueprint_spawns_with_its_authored_abilities() {
        use ambition_entity_catalog::placements::{InteractableSpec, InteractionKindSpec};
        let authored = ambition_platformer2d_world::rooms::Authored::new(
            "NpcSpawn-beaver",
            "NpcSpawn",
        ...
        let (worn, base, effective) = q.iter(world).next().expect("the NPC was built");
        assert_eq!(worn.id(), "npc_busy_beaver");
        assert_eq!(
            (base.abilities, effective.abilities),
            (run_jump, run_jump),
            "a prepared character with no body blueprint spawned without the \
             abilities it authored; possessed, it could not run or jump"
        );
    }
    ...
    fn spawn(name: &'static str, character_id: &'static str) -> (i32, Option<String>) {
        ...
        let (health, worn) = q.iter(world).next().expect("one enemy body");
        (health.health.max, worn.map(|worn| worn.id().to_string()))
    }
