...
/// D199, the other tunnelling half: a fast shot must still HIT a victim it
/// crosses, with no wall involved.
///
/// The victim test is `reached_by`, an ENDPOINT overlap — so a shot whose tick
/// carries it from in front of a body to behind it can pass through without ever
/// overlapping. The row names this as the twin of the wall case: *"fast shots
/// tunnel thin walls and thin hurt volumes"*.
///
/// ⛔⛔ IGNORED BECAUSE IT IS THE UNFIXED HALF, NOT BECAUSE IT IS FLAKY. Confirmed
/// failing 2026-08-29: a 6000 px/s shot crosses the body in one tick and it takes
/// NO damage. The wall half of D199 is fixed (a raycast over the leg answers
/// "did geometry stop this first"), but that technique does not transfer here:
/// the victim test is `reached_by`, which resolves against the victim's PUBLISHED
/// damage volumes, and there is no swept-versus-volume test to call.
///
/// ⛔ The tempting shortcut — sweep the projectile's AABB and test the UNION of
/// its start and end boxes — is refused deliberately. It is exact for an
/// axis-aligned shot and an OVER-approximation for a diagonal one, so it would
/// buy the tunnelling fix by inventing false hits on bodies the shot visually
/// missed. That is precisely the trade this row warns against: "swapping trades
/// one wrong answer for the opposite".
///
/// ⇒ un-ignore this with the swept transaction the row schedules, which is where
/// a swept-versus-volume test belongs.
#[ignore = "D199: documents the unfixed swept-versus-hurt-volume half; un-ignore with the swept transaction"]
#[test]
fn a_fast_shot_still_hits_a_victim_it_crosses() {
    // No blocks at all: this is purely the body-overlap question.
    let world = ae::World::new(
        "no_walls",
        ae::Vec2::new(2000.0, 2000.0),
        ae::Vec2::new(200.0, 200.0),
    ...
    app.update();
    {
        ...
        let mut body = ambition_projectiles::ProjectileBody::from_spec(spec);
        body.kin.pos = ae::Vec2::new(360.0, 300.0);
        // One tick carries it from well in front of the body to well behind it.
        body.kin.vel = ae::Vec2::new(6000.0, 0.0);
        crate::projectile::tests::spawn_player_projectile(&mut app, body);
    }
    advance_time(&mut app, 0.016);
    app.update();
    ...
    let (health, max) = {
        ...
        let (_, health) = query
            .iter(world)
            .find(|(identity, _)| identity.id() == "crossed_enemy")
            .expect("the crossed enemy should be spawned as an ECS actor");
        (health.health.current, health.health.max)
    };
    assert!(
        health < max,
        "a shot that crosses a body in one tick must still hit it (was {max}, now {health})"
    );
}
