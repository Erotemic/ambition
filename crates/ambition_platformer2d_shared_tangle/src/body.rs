...
/// ⭐ IT LIVES HERE BECAUSE ITS TWENTY FIELDS DO. Nineteen are
/// `ambition_platformer2d_core`'s body clusters and one is this crate's
/// `ResolvedMotionFrame`, so `shared_tangle` is the lowest crate that can name
/// them all. It sat in the actor monolith's `actor` module, which
/// `actor-monolith-decomposition.md` measured as *"138 lines that OWN
