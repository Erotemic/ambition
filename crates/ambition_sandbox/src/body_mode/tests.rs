...
/// by `input_timer_system` because the player tick consumes a local copy
...
/// Repro for the ControlFrame-not-flowing-through-the-player-tick
...
fn morph_ball_does_not_fire_from_control_frame_alone() {
    ...
    assert_eq!(
        ...
        "the body-mode driver must read PlayerInteractionState::double_tap_down_pending, \
         not controls.fast_fall_pressed (which the player tick consumes \
