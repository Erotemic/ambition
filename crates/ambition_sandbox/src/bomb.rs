...
/// Spawn one resting bomb ground item near the player on the first frame a
/// player exists (debug convenience until authored placement lands).
pub fn spawn_debug_bomb_once(
    mut commands: Commands,
    mut done: Local<bool>,
    ...
    let Some(spec) = crate::brain::held_item_by_id(BOMB_ID) else {
        return;
    };
    *done = true;
    commands.spawn((
...
mod tests {
    ...
    fn bomb_ground(vel: ae::Vec2) -> GroundItem {
        GroundItem {
            spec: crate::brain::held_item_by_id(BOMB_ID).unwrap(),
