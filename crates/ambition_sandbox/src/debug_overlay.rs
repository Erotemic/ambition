...
fn draw_ldtk_runtime_spine(
    ...
    for entity in &spine_index.entities {
        let color = match entity.role {
            ...
            // Surface runtime rects are drawn by the dedicated Surface index
            // pass so they can be color-keyed against the JSON-derived
            // collision blocks during the raw-vs-runtime overlay work.
            crate::ldtk_world::LdtkRuntimeRole::Surface => continue,
