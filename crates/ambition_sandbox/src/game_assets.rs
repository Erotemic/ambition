...
/// Camera-relative foreground parallax PNGs keyed by room biome.
///
/// These are generated by `python -m ambition_sprite2d_renderer draw-backgrounds`
/// into `assets/<sprite_folder>/backgrounds/`. The PNG files are intentionally
/// ignored by git; this enum only wires the optional runtime handles.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
pub enum ForegroundParallaxSprite {
    Hub,
    Lab,
    Basement,
    Cove,
...
impl ForegroundParallaxSprite {
    pub const fn relative_path(self) -> &'static str {
        match self {
            Self::Hub => "backgrounds/hub_foreground.png",
            Self::Lab => "backgrounds/lab_foreground.png",
            Self::Basement => "backgrounds/basement_foreground.png",
            Self::Cove => "backgrounds/cove_foreground.png",
            Self::Skybridge => "backgrounds/skybridge_foreground.png",
            Self::Boss => "backgrounds/boss_foreground.png",
            Self::Water => "backgrounds/water_foreground.png",
            Self::Cave => "backgrounds/cave_foreground.png",
        }
    }
...
/// Map biome-ish strings to the closest foreground treatment. Unknown
/// values deliberately fall back to Hub/default instead of rendering no
/// foreground; missing PNGs still produce no spawned layer.
pub fn foreground_parallax_sprite_for_biome(biome: Option<&str>) -> ForegroundParallaxSprite {
    let Some(raw) = biome else {
        return ForegroundParallaxSprite::Hub;
    };
    let normalized = raw.trim().to_ascii_lowercase();
    match normalized.as_str() {
        "lab" | "mob_arena" | "laboratory" => ForegroundParallaxSprite::Lab,
        "basement" | "ruins" | "cantina" | "tower" => ForegroundParallaxSprite::Basement,
        "cove" | "coast" | "beach" | "pirate" => ForegroundParallaxSprite::Cove,
        "skybridge" | "sky" | "bridge" | "outpost" => ForegroundParallaxSprite::Skybridge,
        "boss" | "boss_arena" | "arena" => ForegroundParallaxSprite::Boss,
        "water" | "underwater" | "aquatic" => ForegroundParallaxSprite::Water,
        "cave" | "cavern" => ForegroundParallaxSprite::Cave,
        _ => ForegroundParallaxSprite::Hub,
    }
}

/// Conservative near-camera drift per generated foreground theme. A value
/// above 1.0 means the layer drifts slightly faster than the gameplay world
/// as the camera pans, while the sync system keeps it near the screen edges.
pub fn foreground_parallax_factor(key: ForegroundParallaxSprite) -> f32 {
    match key {
        ForegroundParallaxSprite::Hub => 1.08,
        ForegroundParallaxSprite::Lab => 1.10,
        ForegroundParallaxSprite::Basement => 1.12,
        ForegroundParallaxSprite::Cove => 1.06,
        ForegroundParallaxSprite::Skybridge => 1.04,
        ForegroundParallaxSprite::Boss => 1.05,
        ForegroundParallaxSprite::Water => 1.08,
        ForegroundParallaxSprite::Cave => 1.10,
    }
}

#[derive(Default, Clone)]
pub struct ForegroundParallaxSet {
    handles: HashMap<ForegroundParallaxSprite, Handle<Image>>,
}

impl ForegroundParallaxSet {
    pub fn get(&self, key: ForegroundParallaxSprite) -> Option<&Handle<Image>> {
...
pub struct GameAssets {
    ...
    pub foregrounds: ForegroundParallaxSet,
...
pub fn load_game_assets(
    ...
    let foregrounds = load_foreground_parallax_sprites(asset_server, &config.sprite_folder);
    ...
    let foreground_missing = ForegroundParallaxSprite::ALL.len() - foregrounds.len();
    if foreground_missing > 0 {
        eprintln!(
            "[game_assets] {foreground_missing}/{} foreground parallax layers missing under assets/{}/backgrounds/ — run ./regen_sprites.sh to generate them.",
            ForegroundParallaxSprite::ALL.len(),
            config.sprite_folder,
        );
    }
...
fn load_foreground_parallax_sprites(
    asset_server: &AssetServer,
    sprite_folder: &str,
) -> ForegroundParallaxSet {
    let mut handles = HashMap::with_capacity(ForegroundParallaxSprite::ALL.len());
    for &key in ForegroundParallaxSprite::ALL {
        let rel = format!("{sprite_folder}/{}", key.relative_path());
        if asset_exists(&rel) {
            handles.insert(key, asset_server.load(rel));
        }
    }
    ForegroundParallaxSet { handles }
}
