...
//! [`ray_aabb`]) AND the `impl SolidWorldQuery for engine_core::World` adapter
//! now live in the content-free `ambition_platformer_runtime::world_query`
//! module (Stage 16 / S1). Both the trait and `engine_core::World` are
...
pub use ambition_platformer_runtime::world_query::{ray_aabb, raycast_solids, SolidWorldQuery};
