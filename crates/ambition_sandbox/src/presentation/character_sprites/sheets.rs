...
/// Process-wide index of every `SheetRecord`, populated lazily on first
/// `load_spec` call from the compile-time `BAKED_SHEET_RONS` table.
///
/// The table itself is generated by `build.rs`, which walks
/// `assets/sprites/` + one level of subdirs and emits a
/// `&[(filename_root, ron_text)]` slice via `include_str!`. The runtime
/// scan was retired so Android / wasm builds — where the desktop
/// `CARGO_MANIFEST_DIR` path doesn't exist on the device — get the same
/// data as desktop without panicking when a `LazyLock<CharacterSheetSpec>`
/// is first forced.
///
...
///
/// Shared with the runtime `SheetRegistry` resource only by construction
/// (both parse the same baked table); the index here is what backs the
/// `LazyLock<CharacterSheetSpec>` statics, which need their data before
/// any Bevy `Startup` system runs.
...
fn spec_from_record(record: &SheetRecord, tuning: &SheetTuning) -> CharacterSheetSpec {
    // Per the V3/D4 migration plan (`TODO-character-catalog-and-hall.md`
    // → SheetRegistry-driven sprite specs): when the manifest carries
    // an embedded `tuning:` block, prefer it over the passed-in
    // `SheetTuning` const. The Rust const stays as a fresh-clone
    // fallback for sheets that haven't been migrated to manifest-
    // authored tuning yet.
...
pub static ROBOT_SHEET: LazyLock<CharacterSheetSpec> =
    LazyLock::new(|| load_spec("robot", &ROBOT_TUNING));
...
pub static PLAYER_ROBOT_SHEET: LazyLock<CharacterSheetSpec> =
    LazyLock::new(|| load_spec("player_robot", &PLAYER_ROBOT_TUNING));

const GOBLIN_TUNING: SheetTuning = SheetTuning::new(2.1, 1);
pub static GOBLIN_SHEET: LazyLock<CharacterSheetSpec> =
    LazyLock::new(|| load_spec("goblin", &GOBLIN_TUNING));
...
pub static ABSURD_GENERAL_SHEET: LazyLock<CharacterSheetSpec> =
    LazyLock::new(|| load_spec("absurd_general", &ABSURD_GENERAL_TUNING));
...
pub static PUPPY_SLUG_SHEET: LazyLock<CharacterSheetSpec> =
    LazyLock::new(|| load_spec("puppy_slug", &PUPPY_SLUG_TUNING));
...
pub static ARCHITECT_SHEET: LazyLock<CharacterSheetSpec> =
    LazyLock::new(|| load_spec("architect", &ARCHITECT_TUNING));

/// Kernel Guide — onboarding NPC at the hub spawn area.
const KERNEL_GUIDE_TUNING: SheetTuning = SheetTuning::new(1.10, 2);
pub static KERNEL_GUIDE_SHEET: LazyLock<CharacterSheetSpec> =
    LazyLock::new(|| load_spec("kernel_guide", &KERNEL_GUIDE_TUNING));

/// Vault Keeper — persistence / save-seed NPC in the basement.
const VAULT_KEEPER_TUNING: SheetTuning = SheetTuning::new(1.10, 2);
pub static VAULT_KEEPER_SHEET: LazyLock<CharacterSheetSpec> =
    LazyLock::new(|| load_spec("vault_keeper", &VAULT_KEEPER_TUNING));
...
pub static GATE_RING_SHEET: LazyLock<CharacterSheetSpec> =
    LazyLock::new(|| load_spec("interdimensional_gate_ring", &GATE_RING_TUNING));
...
pub static GATE_PORTAL_SHEET: LazyLock<CharacterSheetSpec> =
    LazyLock::new(|| load_spec("interdimensional_gate_portal", &GATE_PORTAL_TUNING));
...
pub static LAB_PROP_GENESIS_VAT: LazyLock<CharacterSheetSpec> =
    LazyLock::new(|| load_spec("genesis_vat", &LAB_PROP_TUNING));
#[allow(dead_code)]
pub static LAB_PROP_SPECIMEN_JAR: LazyLock<CharacterSheetSpec> =
    LazyLock::new(|| load_spec("specimen_jar", &LAB_PROP_TUNING));
pub static LAB_PROP_NEURAL_CONSOLE: LazyLock<CharacterSheetSpec> =
    LazyLock::new(|| load_spec("neural_console", &LAB_PROP_TUNING));
pub static LAB_PROP_RESONANCE_COIL: LazyLock<CharacterSheetSpec> =
    LazyLock::new(|| load_spec("resonance_coil", &LAB_PROP_TUNING));
pub static LAB_PROP_POWER_CORE: LazyLock<CharacterSheetSpec> =
    LazyLock::new(|| load_spec("power_core", &LAB_PROP_TUNING));
pub static LAB_PROP_REPAIR_CRADLE: LazyLock<CharacterSheetSpec> =
    LazyLock::new(|| load_spec("repair_cradle", &LAB_PROP_TUNING));
#[allow(dead_code)]
pub static LAB_PROP_DRONE_CRADLE: LazyLock<CharacterSheetSpec> =
    LazyLock::new(|| load_spec("drone_cradle", &LAB_PROP_TUNING));
#[allow(dead_code)]
pub static LAB_PROP_PORTAL_CALIBRATOR: LazyLock<CharacterSheetSpec> =
    LazyLock::new(|| load_spec("portal_calibrator", &LAB_PROP_TUNING));
...
pub static CART_SHEET: LazyLock<CharacterSheetSpec> =
    LazyLock::new(|| load_spec("intro_cart", &CART_TUNING));
...
pub static OILER_SHEET: LazyLock<CharacterSheetSpec> =
    LazyLock::new(|| load_spec("oiler", &OILER_TUNING));
...
pub static ERDISH_SHEET: LazyLock<CharacterSheetSpec> =
    LazyLock::new(|| load_spec("erdish", &ERDISH_TUNING));
...
pub static ALICE_SHEET: LazyLock<CharacterSheetSpec> =
    LazyLock::new(|| load_spec("alice", &ALICE_TUNING));
...
pub static BOB_SHEET: LazyLock<CharacterSheetSpec> =
    LazyLock::new(|| load_spec("bob", &BOB_TUNING));

/// Merchant Prototype — placeholder shopkeeper NPC.
const MERCHANT_PROTOTYPE_TUNING: SheetTuning = SheetTuning::new(1.10, 2);
pub static MERCHANT_PROTOTYPE_SHEET: LazyLock<CharacterSheetSpec> =
    LazyLock::new(|| load_spec("merchant_prototype", &MERCHANT_PROTOTYPE_TUNING));
...
pub static GOBLIN_CANTINA_CHIEFTAIN_SHEET: LazyLock<CharacterSheetSpec> =
    LazyLock::new(|| load_spec("goblin_cantina_chieftain", &GOBLIN_CANTINA_CHIEFTAIN_TUNING));

/// Captain Pulse — Pulse Voyagers faction leader.
const PULSE_VOYAGER_CAPTAIN_TUNING: SheetTuning = SheetTuning::new(1.20, 1);
pub static PULSE_VOYAGER_CAPTAIN_SHEET: LazyLock<CharacterSheetSpec> =
    LazyLock::new(|| load_spec("pulse_voyager_captain", &PULSE_VOYAGER_CAPTAIN_TUNING));
...
pub static TECH_BRO_DISRUPTOR_SHEET: LazyLock<CharacterSheetSpec> =
    LazyLock::new(|| load_spec("tech_bro_disruptor", &TECH_BRO_DISRUPTOR_TUNING));

const SANDBAG_TUNING: SheetTuning = SheetTuning::new(1.38, 1);
pub static SANDBAG_SHEET: LazyLock<CharacterSheetSpec> =
    LazyLock::new(|| load_spec("sandbag", &SANDBAG_TUNING));
...
pub static NINJA_SHEET: LazyLock<CharacterSheetSpec> =
    LazyLock::new(|| load_spec("ninja_shadow_duelist", &NINJA_TUNING));
