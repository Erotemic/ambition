...
//!
//! Pogo-able features now publish [`ambition_combat::PogoTargetVolumes`].
//! The default feature rule derives those volumes from current
//! [`ambition_platformer2d_actor_monolith::features::DamageableVolumes`], so peaceful NPCs, hostile actors,
//! and boss hurtboxes all feed the same affordance path. The legacy
//! [`ambition_combat::PogoTargetContributor`] marker remains as a fallback for
...
use ambition_combat::{PogoTargetContributor, PogoTargetVolumes};
...
pub fn update_pogo_target_below(
    ...
    let center = ambition_platformer2d_core::Vec2::new(kin.pos.x, feet_y + POGO_DETECTION_DEPTH * 0.5);
    let half_size =
        ambition_platformer2d_core::Vec2::new(POGO_DETECTION_HALF_WIDTH, POGO_DETECTION_DEPTH * 0.5);
    let scan = ambition_platformer2d_core::Aabb::new(center, half_size);
