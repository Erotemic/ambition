...
fn build_test_app() -> (App, Entity) {
    use ambition_platformer2d_core::{BodyEnvironmentContact, BodyGroundState, BodyModeState};
...
fn special_dispatches_on_aim_direction() {
    ...
    // Side-stick (forward relative to right-facing) → SideSpecial.
    {
        ...
        let mut kin = entity.get_mut::<ambition_platformer2d_core::BodyKinematics>().unwrap();
        kin.facing = 1.0;
    }
...
fn airborne_player_with_down_aim_reads_as_dair() {
    ...
    {
        let mut entity = app.world_mut().entity_mut(player_entity);
        let mut ground = entity.get_mut::<ambition_platformer2d_core::BodyGroundState>().unwrap();
...
fn affordances_follow_the_possessed_body_not_the_home_avatar() {
    use ambition_platformer2d_core::{BodyEnvironmentContact, BodyGroundState, BodyKinematics, BodyModeState};
...
fn b_air_fires_when_aim_opposes_facing_aerial() {
    let (mut app, player_entity) = build_test_app();
    {
        ...
        ent.get_mut::<ambition_platformer2d_core::BodyGroundState>()
            .unwrap()
            .on_ground = false;
        ent.get_mut::<ambition_platformer2d_core::BodyKinematics>()
            .unwrap()
            .facing = 1.0;
    }
