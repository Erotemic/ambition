...
## 2026-07-19 A `portal_render` host composition without `PortalPlugin` panics — and the gate cannot see it
- **Where:** `crates/ambition_host/tests/demo_shell_smoke.rs` (5 of 6 tests) run with `--features portal_render`: "Parameter `messages` failed validation: Message not initialized". `PlatformerHostPlugins` adds `PortalObservationPlugin` under `portal_render`, but every portal message (`TogglePortalGun`, `FirePortalGun`, `PortalFireIntent`, …) is registered by `ambition_portal::PortalPlugin`, which is added content-side and NOT by the host group.
- **Smell:** a feature flag on the host silently requires a plugin the host does not add. Two candidate resolutions (needs a decision, not a patch): have `PortalObservationPlugin` depend on / self-register the mechanic, or `run_if`-guard its systems on the messages existing.
- **Why it survived:** `scripts/run_tests.py` lists `ambition_host` in `SKIP_FEATURE_JOB`, whose comment asserts these features "gate NO test code (verified: … host …)". That assertion is now false — `portal.rs` carries `#[cfg(test)]` tests behind `portal_render` — so **the portal host-adapter tests do not run in `./run_tests.sh`**. Verified pre-existing: the same 5 failures reproduce at HEAD with the change stashed.
- **Suggested fix / size:** M — decide the composition rule, fix `demo_shell_smoke`, then drop `ambition_host` from `SKIP_FEATURE_JOB` so the tests actually gate. Do not drop it from the skip list first; the job is red today.
