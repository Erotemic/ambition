...
- **Where:** `scripts/regen/sprites.sh` — `all_outputs_present()`, whose *last* command (and therefore its return status) is `inspect_hall_portraits … --only-issues`.
- **Smell:** the fast path requires `all_outputs_present`, which ends up returning the coverage tool's exit code — `1` unless all 128 Hall rows have portraits. Twelve of them have no render path at all (see the entry below), so the condition is unsatisfiable and *every* regen re-renders every sprite (~10-21 min) on any checkout lacking those portraits, no matter how current the outputs are. Measured here: stored fingerprint present, all 288 expected files present, `inspect_hall_portraits` exit=1, cache misses. The script's own comment says the expected-files list is "a single source of truth" shared by the fast path and the postcondition — this extra condition silently breaks that, and gates a *performance* cache on a *content-completeness* question it was never meant to answer.
- **Noticed while:** verifying zero-to-runnable setup from a fresh clone (2026-07-26), when run 5 re-rendered sprites despite run 4 having just succeeded with nothing relevant changed.
- **Suggested fix / size:** S — drop the coverage call from `all_outputs_present` and let `expected_files` (which already includes the portrait PNGs that actually ship: `alice_portraits.png`, `oiler_portraits.png`, `pipi_tau_portraits.png`) be the sole condition, as the comment intends. Same root cause as the report-as-gate entry below: one tool's "findings exist" exit code doing duty as a boolean in two places that wanted different questions answered.

## 2026-07-26 Superproject scripts import renderer internals with nothing checking the contract
- **Where:** `scripts/generate_visual_quality_variants.py:41-48` — `sys.path.insert` on `tools/ambition_sprite2d_renderer`, then `from ambition_sprite2d_renderer.registry import …` and `from ambition_sprite2d_renderer.authoring.packer import …`.
- **Smell:** the game's scripts reach past the renderer's CLI into its Python internals, across a submodule pin, and nothing anywhere validates that the symbols still exist. The renderer's `eba4e21` (2026-07-02) unified `TackonTarget`/`AdapterTarget` into one `Target` with a `kind` field — a clean refactor on its side — and this consumer kept importing `AdapterTarget`. The variants phase then died at *import* in 1 second, and stayed dead for ~3½ weeks, because nothing runs it except a full `scripts/regen/assets.sh` and the phases before it take ~10 minutes to reach it.
- **Noticed while:** verifying zero-to-runnable setup from a fresh clone (2026-07-26); fixed by mapping to `Target` + `kind == "config"` per the refactor's own note.
- **Suggested fix / size:** M — either give the renderer a supported Python API surface (an `__all__`-backed façade the superproject may import, so its refactors know what's load-bearing), or move what this script needs behind a renderer CLI subcommand like every other phase uses. S as a stopgap: an import smoke-test over the scripts that cross the boundary, so a pin bump fails fast and loudly instead of ~10 minutes into a regen.

## 2026-07-26 Twelve Hall-of-Characters rows have no render path to their portraits
- **Where:** `game/…/character_catalog` Hall rows vs `scripts/regen/sprites.sh` render lists. The 12: `npc_carl_runga`, `npc_carl_stargan`, `npc_data_lovelace`, `npc_davy_hylbert`, `npc_genghis_can`, `npc_genghis_cant`, `npc_hunny_horror_boss`, `npc_mami_marzakhani`, `npc_martin_cutta`, `npc_neil_ongras_turfson`, `npc_paradox_barber`, `npc_yuclid` (128 catalog rows total; `no_png=12, ok=116`).
- **Smell:** each of these has a `targets/characters/<name>.py` in the sprite renderer that defines portrait clips, but none appears in any render list in `scripts/regen/sprites.sh` — so no regen path produces their `*_portraits.png`. On a machine where the PNGs were hydrated out of band this is invisible; on a fresh clone the Hall is 12 portraits short and nothing can generate them. Since binary payloads are git-ignored-but-present by policy, "it works here" cannot distinguish a rendered asset from a hand-placed one, which is exactly how this stayed hidden.
- **Noticed while:** verifying zero-to-runnable setup from a fresh clone (2026-07-26). The coverage report named all 12 — the report was right; it was only wired wrong (see next entry).
- **Suggested fix / size:** S if they just need adding to the tackon/publish lists and the targets render as-is; M if any target's portrait clips are incomplete. Worth pairing with a check that every Hall row is reachable from some render list, so a catalog row and its renderer can't drift apart again.

## 2026-07-26 A portrait *report* was wired as a hard gate and aborted all of asset regen
- **Where:** `scripts/regen/sprites.sh:1020-1023` (now fixed) calling `ambition_ldtk_tools.inspect_hall_portraits`.
- **Smell:** the tool returns `1` unless *every* catalog row is `ok` (`inspect_hall_portraits.py:144`) — right for a `--check` caller, wrong here. Called as `… --only-issues 2>&1 | sed 's/^/  /'` under `set -euo pipefail`, the exit status propagated and `set -e` killed the script — after the 21-minute sprite render had already succeeded and its real postcondition had passed, and before music, SFX, atlases, and the cargo check ever ran. Everything adjacent to it (the publish-boundary sweep, `portrait-gallery`) is written tolerantly; this one call site wasn't. A missing portrait for an unshipped Hall row is not a reason to abort the build.
- **Noticed while:** same fresh-clone verification — it was the fourth consecutive blocker, each only visible once the previous one was cleared.
- **Suggested fix / size:** DONE (S) — call site now branches on failure and prints a `note:` instead of aborting; the report stays as loud as it was. The general lesson is the reusable one: a tool whose exit code encodes "findings exist" needs `|| true` or an explicit branch at every non-gating call site, and `pipefail` makes that non-optional.

## 2026-07-26 `scripts/regen/sprites.sh` restates a sheet list the renderer already owns
- **Where:** `scripts/regen/sprites.sh:261-305` (`expected_files`) vs the `runtime_stems` set in `tools/ambition_sprite2d_renderer/ambition_sprite2d_renderer/cli/commands.py` (`draw_all`).
- **Smell:** two hand-maintained lists across a submodule boundary both answer "which sprite sheets does the game require," and nothing keeps them in sync. They silently disagreed: `ranged_skirmisher`, `exploding_mite`, and `dividing_mite` were listed here as runtime-required and had renderer configs, but `draw_all`'s allowlist filtered them out — so a fresh-clone regen rendered for ~21 minutes and *then* failed its own postcondition with 9 missing files. The postcondition did its job; the duplication is what made the failure possible and expensive to discover.
- **Noticed while:** verifying zero-to-runnable setup from a fresh clone (2026-07-26). Unblocked by adding the three stems to the renderer's allowlist, which leaves the duplication intact on both sides.
- **Suggested fix / size:** S–M — have the renderer *report* its runtime targets (a `list-runtime-targets` subcommand, or the manifest it already writes) and let `expected_files` be derived from that, so the game asserts "everything the renderer promised arrived" rather than restating the roster. Logged on the renderer side too, in that submodule's `docs/code_smells.md`.

## 2026-07-26 Music setup never fetches the SFZ corpus the scores ask for
- **Where:** `run_developer_setup.sh` (`install_system_packages` installs only GM soundfonts) and `scripts/regen/music.sh`; the unused `tools/ambition_music_renderer/download_ambition_audio_tools.sh`; `docs/tools/generated-audio-tools.md` (silent on both).
- **Smell:** 10 of 53 active scores reference SFZ sample libraries, 44 instrument backends in total. Nothing in setup or regen ever fetches them, and `/data/audio-tools` does not exist on a fresh machine. This is *not* a blocker — every one of those 44 backends is `optional: true`, so `render/group.py` warns and falls back rather than raising — but it means a fresh clone renders those cues with substitute instruments and reports success. "Setup completed" and "the music sounds as intended" are not the same state, and nothing distinguishes them.
- **Noticed while:** same fresh-clone verification.
- **Suggested fix / size:** M — a songs-only subset fetch invoked by setup (the needed libraries are computable from each score's `library_ref`s), with the full corpus opt-in for authoring; plus a regen summary line naming which libraries were missing so a degraded render says so out loud. Renderer-side half logged in that submodule's `docs/code_smells.md`.
...
- **RESOLUTION (2026-09-06) — THE FIX EXISTS AND HALF THE SMELL WAS NEVER TRUE.** The asked-for gate is built and is `ParticipantContexts`: a cutscene declares `ContextClaim::capturing(CUTSCENE_CONTEXT, context_priority::CUTSCENE)` exactly as dialogue declares `DIALOGUE_CONTEXT`, in one system (`actor_monolith/src/schedule/input_systems.rs:460`), prioritised rather than boolean. That IS "a shared gate consumed by the one input-folding seam", and it arrived without this row being closed.
  ⛔ **The ENCOUNTER half was a misreading of the cited line.** `ambition_encounter/src/lifecycle.rs` locks EXITS, not input — `locks_exits`, `EncounterEvent::LockChanged`. Two different facts wearing the word "lock"; encounter never carried an input mechanism to unify.
  ⭐ **What was left was one dead symbol, and it is what made the row look true.** `ActiveCutscene::freezes_player_input` had the body `self.is_playing()` and **zero callers anywhere in the tree** — measured with `git grep`, whose only two hits were the definition and this row. A POLICY ("playing ⇒ input frozen") spelled as a fact, beside the state question it aliased. Deleted; `is_playing` now carries a comment saying where input is actually decided, so the next reader does not re-file this.
...
- **RESOLUTION (2026-08-14):** it did not earn its keep, and the decision was overdue for a reason this entry could not see: the board's ANCHOR was `PrimaryPlayer`, or the lowest `PlayerSlot` when a build had none, which made a mechanism with no consumer the largest player-centric assumption inside generic actor simulation — the thing D117 exists to remove. Deleted `CombatSlot`/`CombatSlotBoard`/`SlotRequest`/`assign_slots`/`CombatSlotsRes`/`compute_holding_positions`, the `PrimaryPlayer` query and the position + `PlayerSlot` reads in `tick_actor_brains` (its `player_query` is now entity + `BodyHealth`, i.e. target liveness only), the rollback registration `resource.combat_slot_board` (schema v28, both baselines re-recorded), and the room-transition/room-reset paths that cleared it. `ambition_combat::slots` became `::crowd`, keeping only the ground/aerial `CrowdKind` the surviving crowding signal reads; `ActorTuning::slot_kind` became `crowd_kind`. Behaviour-preserving by construction — nothing read the board — so the evidence is the unchanged suites, not a new assertion. ⭐ **target-relative arbitration was the plan and deletion was the answer**: re-anchoring a mechanism no consumer observes is the abstraction, not the fix. If a crowd board is ever wanted again it should be built against a real reader, anchored on the body a crowd is contesting.
...
`WorldItemArt`; `scripts/regen/sprites.sh` publishes that flat canonical (tackon target +
