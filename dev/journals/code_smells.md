...
- **2026-06-26 Pre-existing failing test: `brain::player::tests::blink_precision_aim_is_screen_relative_by_default_quick_is_locomotion`** — fails on `cargo test -p ambition_characters --lib` and was ALREADY failing at session-start commit `649eb3f3` (verified via worktree), so it is NOT caused by the PCA-encounter work. The assertion (`player.rs:393`) expects quick-blink `blink_quick_dir == (-1, 0)` under sideways gravity (`control_down = (1,0)`); the brain produces something else. Unrelated to the smash/reaction-latency changes. Left for the player-blink/gravity-frame owner — flagged here so the red test isn't mistaken for a regression from this branch.
