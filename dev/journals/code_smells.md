...
- **Where:** `regen_sprites.sh` — `all_outputs_present()`, whose *last* command (and therefore its return status) is `inspect_hall_portraits … --only-issues`.
...
- **Where:** `game/…/character_catalog` Hall rows vs `regen_sprites.sh` render lists. The 12: `npc_carl_runga`, `npc_carl_stargan`, `npc_data_lovelace`, `npc_davy_hylbert`, `npc_genghis_can`, `npc_genghis_cant`, `npc_hunny_horror_boss`, `npc_mami_marzakhani`, `npc_martin_cutta`, `npc_neil_ongras_turfson`, `npc_paradox_barber`, `npc_yuclid` (128 catalog rows total; `no_png=12, ok=116`).
- **Smell:** each of these has a `targets/characters/<name>.py` in the sprite renderer that defines portrait clips, but none appears in any render list in `regen_sprites.sh` — so no regen path produces their `*_portraits.png`. On a machine where the PNGs were hydrated out of band this is invisible; on a fresh clone the Hall is 12 portraits short and nothing can generate them. Since binary payloads are git-ignored-but-present by policy, "it works here" cannot distinguish a rendered asset from a hand-placed one, which is exactly how this stayed hidden.
...
- **Where:** `regen_sprites.sh:1020-1023` (now fixed) calling `ambition_ldtk_tools.inspect_hall_portraits`.
...
## 2026-07-26 `regen_sprites.sh` restates a sheet list the renderer already owns
- **Where:** `regen_sprites.sh:261-305` (`expected_files`) vs the `runtime_stems` set in `tools/ambition_sprite2d_renderer/ambition_sprite2d_renderer/cli/commands.py` (`draw_all`).
...
- **Where:** `run_developer_setup.sh` (`install_system_packages` installs only GM soundfonts) and `regen_music.sh`; the unused `tools/ambition_music_renderer/download_ambition_audio_tools.sh`; `docs/tools/generated-audio-tools.md` (silent on both).
