...
- **Suggested fix / size:** L, incremental. Per façade module: expand the glob, see what leaks, convert the hub to an explicit curated `pub use`. `bosses/specials/mod.rs` DONE (2026-07-15). Next cleanly-separable candidates (submodules use few parent items, like specials did): `ambition_render/rendering/actors`, `ambition_runtime/snapshot`, `ambition_world/rooms`.
