...
live under `crates/ambition_platformer2d_actor_monolith/src/engine_core/` as an intra-
crate module of `ambition_platformer2d_actor_monolith`.
...
1. `actor.rs` (292 lines: `Actor`, `Health`, `KinematicPath`,
   `RespawnPolicy`, …) → `crates/ambition_platformer2d_actor_monolith/src/actor.rs`
   (commit `d30cb3d4`).
2. `combat.rs` (499 lines: damage volumes, hit semantics) →
   `crates/ambition_platformer2d_actor_monolith/src/combat.rs` (earlier in branch).
...
- All remaining engine modules (`abilities`, `geometry`,
  ...
  `player_state`, `world`) → `crates/ambition_platformer2d_actor_monolith/src/engine_core/`.
...
- Sandbox-wide, `use ambition_engine as ae;` → `use crate::engine_core as ae;`,
  ...
  files).
- `crates/ambition_platformer2d_actor_monolith/Cargo.toml` inherited the engine's
...
- **`src/bin/*.rs` are separate compilation units.** Inside a bin,
  ...
  bin had to use `ambition_platformer2d_actor_monolith::engine_core::*` while every
