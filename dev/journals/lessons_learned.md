...
⛔⛔ **SUPERSEDED 2026-08-15 (D126.2) — the instruction this entry ended with now names a deleted API, and following it would rebuild the fork it was written to prevent.** The last paragraph predicted the resolution and got the DIRECTION backwards: the player never migrated down to `KinematicBody`; the actor unification moved every body — player, enemy, NPC, boss, item — UP onto the rich `ambition_platformer2d_core::step_motion` kernel, and `EnemyRuntime` / `NpcRuntime` are themselves long gone. `step_kinematic` was then left with no caller but its own test module for months while eight comments across five crates still cited it as the live enemy path, which is what made it read as a second live sweep. The whole `ambition_platformer2d_shared_tangle::kinematic` module is deleted.

**What survives is the lesson, not the API:** do NOT roll a new `blocked_*` predicate for a new actor type. There is ONE sweep — drive a `BodyKinematics` through `step_motion` and let the shared `collision_semantics` vocabulary (which outlived both sweeps and is where the OneWay / landing-from-above / support rules actually live) decide contact. Drop-through is not a boolean you pass in; it is the derived gesture `descend + jump` that `movement::integration::wants_drop_through` owns.
...
Desktop still enters through `src/main.rs`; Android packages the library as `libambition_actors.so` and needs Bevy's `#[bevy_main]` macro to generate the Android boilerplate.
