...
1. The validation command used `cargo run -p ambition_actors`, but the package has multiple binaries, so the correct visible-game command is:

```bash
cargo run -p ambition_actors --bin ambition_game_bin
```
