...
1. The validation command used `cargo run -p ambition_platformer2d_actor_monolith`, but the package has multiple binaries, so the correct visible-game command is:

```bash
cargo run -p ambition_platformer2d_actor_monolith --bin ambition_game_bin
```
