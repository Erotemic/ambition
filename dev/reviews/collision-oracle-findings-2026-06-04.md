...
`crates/ambition_actors/tests/collision_invariant_oracle.rs` — a fuzz-driven
...
```bash
cargo test -p ambition_actors --test collision_invariant_oracle \
    -- --ignored --nocapture
```
...
`crates/ambition_actors/src/engine_core/movement/collision.rs`). The *stuck-
...
Reproduce one: `cargo run -p ambition_actors --bin rl_random_walker -- <STEPS>
