...
```bash
cargo test -p ambition_platformer2d_actor_monolith --test collision_invariant_oracle \
    -- --ignored --nocapture
```
...
`crates/ambition_platformer2d_actor_monolith/src/engine_core/movement/collision.rs`). The *stuck-
...
Reproduce one: `cargo run -p ambition_platformer2d_actor_monolith --bin rl_random_walker -- <STEPS>
