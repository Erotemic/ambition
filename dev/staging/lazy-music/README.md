...
2. Apply the surgical edits in `changes.md` to:
   ...
   - `crates/ambition_platformer2d_actor_monolith/src/music/director.rs`
   - `crates/ambition_platformer2d_actor_monolith/src/pause_menu.rs`
   (`audio.rs`'s re-exports list and `setup.rs` need NO changes.)
3. `cd crates/ambition_platformer2d_actor_monolith && cargo check --features audio,input`.
4. Run the audio tests: `cargo test -p ambition_platformer2d_actor_monolith --features audio
   --lib audio::`.
