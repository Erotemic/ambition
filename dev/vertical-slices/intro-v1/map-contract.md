...
`cargo fmt --check` / `cargo test -p ambition_sandbox --lib` / `cargo run
-p ambition_sandbox --bin headless` were not run as part of this task (no
...
- `area create --dry-run` is supported on individual area YAMLs; pass
  `--ldtk crates/ambition_sandbox/assets/ambition/worlds/intro.ldtk` so the
...
4. Fix the spec drift on `intro_escape_shaft_area.yaml`: either update
   ...
   to intro.ldtk with `--ldtk crates/ambition_sandbox/assets/ambition/worlds/intro.ldtk`.
5. After the shape lands, re-run the section-9 validate command and a quick
   `cargo run -p ambition_sandbox --bin headless` smoke check. Update this
