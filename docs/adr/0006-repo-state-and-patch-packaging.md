...
Patch zips may contain only modified files to save bandwidth, but they must preserve repo-relative paths exactly and include `patch` in the zip name. Crate-local files must stay under `crates/<workspace_member>/` (today: `ambition_platformer2d_actor_monolith`, `ambition_asset_manager`, `ambition_sfx`, `ambition_sfx_bank`); agents must not create duplicate top-level crate directories such as `ambition_platformer2d_actor_monolith/`. Documentation should normally live under `docs/`, except for the root `README.md`.
