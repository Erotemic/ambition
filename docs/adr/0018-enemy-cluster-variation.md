...
The seed is `crates/ambition_actors/src/combat/variation.rs::seed_from_id(&enemy.id)`,
...
f32, f32, f32)` in `crates/ambition_actors/src/combat/variation.rs`: an xorshift32
...
- Every new brain template must specify which jitter dimensions
  ...
  `crates/ambition_actors/src/features/ecs/brain_builders.rs` is the canonical reference: every
...
- Tests for "deterministic brain tick given fixed snapshot" still
  ...
  stable across runs.
- The `crates/ambition_actors/src/combat/variation.rs` helpers are the single jitter source. New
...
`crates/ambition_actors/src/combat/variation.rs`, and keep
...
- `crates/ambition_actors/src/combat/variation.rs`
  — the canonical seed + jitter helpers.
- `crates/ambition_actors/src/features/ecs/mount/mod.rs::enforce_mount_rider_link`
