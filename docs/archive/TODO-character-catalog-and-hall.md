...
| 9.F Bonus: drop bare-name rollups + drop-through basement floors + spawn-overlap | — | 0.30 h | ✅ done | Removed npc_pirate_heavy / npc_robot_heavy catalog entries per Jon's "don't shoehorn" feedback. Added drop-through gaps to each basement floor so player can navigate down between rows. Spawn-overlap validator covers NpcSpawn/EnemySpawn/BossSpawn (was DebugLabel-only). |
| 9.G Bonus: asset-resolution tests + sprite census log | — | 0.20 h | ✅ done | New `every_character_catalog_entry_resolves_a_load_path` test catches manifest-row presence + path resolution. Added one-line startup INFO log emitting catalog→sprite chain census (loaded / no-spec / no-path counts). Helps future "is this stale build?" debugging. |
...
| 9.O Bonus: boss_encounters/<id>.ron proof-of-concept | — | 0.20 h | ✅ done | Drained one item from the deferred-followups list. New `assets/data/boss_encounters/gnu_ton.ron` + `load_boss_specs_from_disk()` in `boss_encounter/specs.rs`. Behind `#[allow(dead_code)]` until a future PR flips `BossEncounterRegistry` to RON-driven. Pinned test verifies on-disk RON ≡ hardcoded constructor field-for-field. |
| 9.P Bonus: wire boss-encounters RON loader through profile assembly | — | 0.10 h | ✅ done | Removed `#[allow(dead_code)]`; `default_boss_profiles` now calls `load_boss_specs_from_disk` once and overrides each profile's encounter spec when a matching RON exists. `gnu_ton.ron` is now authoritative for the encounter's numeric fields. Pin test `gnu_ton_profile_encounter_matches_hardcoded_constructor` guards the override path. ADR 0017 + TODO.md deferred-list entry updated to reflect partial completion. |
| 9.Q Bonus: author RON for remaining bosses (mockingbird, clockwork_warden) | — | 0.10 h | ✅ done | Drained the rest of the numeric-fields half of the boss-encounters RON migration. All three authored bosses now ship `<id>.ron`. Added shared `assert_spec_matches_disk` / `assert_profile_matches` helpers + per-boss pin tests on both layers (specs + profile), 4 new tests total. 43 boss_encounter tests pass. |
| 9.R Bonus: publish-time Idle-row warning | — | 0.10 h | ✅ done | Closes follow-up #4 from `sprite-rendering-surface.md`. `tackon_sheet.diagnose_idle_coverage` prints a stderr warning at publish time when a sheet has ≥1 CharacterAnim row but no Idle alias (the runtime's placeholder-fallback case). 7 unit tests, including a galwah-pre-rename regression marker. |
| 9.S Bonus: BossEncounterSpec::clockwork_warden + sandbox warning cleanup | — | 0.10 h | ✅ done | Centralizes the gradient_sentinel-then-mutate pattern as `BossEncounterSpec::clockwork_warden()`; 3 callsites cleaned. Also cleared 3 lingering `#[cfg(test)]` warnings (unused `bevy::prelude::*` imports, one needless `mut`). Sandbox lib+tests now compile with zero warnings. 785 sandbox lib tests pass. |
| 9.T Bonus: Python schema pins for boss_encounters RON files | — | 0.10 h | ✅ done | 7 Python tests over `boss_encounters/*.ron` (parses, required fields present, id↔filename, fractions in [0,1], positive timings + max_hp). Earlier validation surface than the Rust pin — fires without compilation, catches typos that would silently fall back to the hardcoded constructor. |
| 9.U Bonus: fix pre-existing pytest-discovery failures in author-area tests | — | 0.10 h | ✅ done | `test_area_authoring_features.py` was written script-first and pytest auto-discovery tripped on the unresolved `td` parameter. Converted each test to take `tmp_path`. Two cases also picked world coordinates now claimed by `crawl_lab`; moved to (100000, 0/256). 5 tests pass; full Python suite at 59 passed (up from 54+5 errors). |
| 9.V Bonus: pin publish_catalog_sprites helpers | — | 0.05 h | ✅ done | 5 unit tests over `renderer_target_for_catalog_entry` (canonical case, subdir-bypass, underscore preservation, defensive rejections). Catches drift in the catalog-driven publish driver's name-derivation hop. |
| 9.W Bonus: orphan + duplicate guards on boss_encounters RON loader | — | 0.05 h | ✅ done | Two extra pins on `load_boss_specs_from_disk`: orphan RON files (no matching authored profile) and duplicate ids both trip focused-diff failures instead of silent fallback. Catches the typo'd-filename / leftover-from-rename / collision cases. 787 sandbox lib tests. |
| 9.X Bonus: doc-health green + doc/recipe updates | — | 0.15 h | ✅ done | Fixed two documentation-health violations: ADR 0017 needed a "Current implications for agents" section (added — covers new-character authoring, room creation, number-tuning workflow); `docs/concepts/cryptography-crew.md` referenced the old `targets/toon_side.py` path (moved into `targets/characters/`, 3 refs fixed). Closed the "Auto-spacer for overlapping DebugLabels" TODO item (shipped as `space_debug_labels` in 9.L). |
| 9.Y Bonus: startup census logs for boss + encounter registries | — | 0.10 h | ✅ done | One-line INFO census on first registration for boss-encounters (X RON-overridden, Y constructor-only) and encounter (N spec(s) loaded from LDtk). Mirrors the catalog sprite census pattern — "did the RON files actually load?" is now visible in the log without grepping a registry dump. Per Jon's "log messages... not a bad idea so long as they don't get spammy". |
| 9.Z Bonus: REAL fix for gnu_ton + mockingbird Hall placeholders | — | 0.30 h | ✅ done | Jon caught a real-world regression: gnu_ton + mockingbird still showed placeholder. The Python inspector reported 97/97 OK but the Rust runtime returned None. Root cause: `ron_parse.dumps({})` emitted `()` (RON unit type) instead of `{}` (empty Map); `synth_boss_manifest.py` wrote `anchors: ()` for every frame rect; Rust's strict `ron::from_str::<FrameRect>` rejected those manifests; both bosses fell back to placeholder. **Fixes:** dumps emits `{}` for empty dicts; regenerated both subdir manifests; new Rust pin test `boss_subdir_manifests_resolve_through_catalog`; two Python tests guarding the `{}`-not-`()` invariant. New memory `feedback-parser-drift-detection` captures the lesson that Python RON parsers are looser than Rust's and Python success ≠ Rust success — Rust-side parse tests are mandatory for any Python-authored Rust-consumed content. |
| 10.A Judgment-call: manifest-authored sheet tuning (V3/D4 step 1) | — | 0.50 h | ✅ done | Per Jon's "in the manifest" call for the SheetRegistry-driven specs follow-up. **Landed infrastructure:** SheetRecord carries `tuning: Option<SheetTuningSpec>` (collision_scale + frame_sample_inset); `spec_from_record` prefers manifest tuning over the hardcoded const; both renderer paths (tackon + adapter) emit `tuning:` block when the manifest dict has it; `CharacterJob.sheet_tuning` reads from `configs/<target>.yaml`; first character migrated (`robot`, `sheet_tuning: { collision_scale: 2.1, frame_sample_inset: 1 }`). 2 Rust pin tests (override + fallback) + 8 Python tests. 790 sandbox lib tests pass. Remaining: regen robot's RON (needs renderer run), then migrate the other ~29 chars one-by-one. |
| 10.B Judgment-call: mockingbird hybrid + multi-file target reference doc | — | 0.30 h | ✅ done | Per Jon's "use the mockingbird as an example of what the full API surface will likely need" call. The mockingbird boss generator stays a multi-file tack-on (scenegraph + part-editor authoring affordances intact) and `publish_catalog_sprites.py` now drives both top-level and subdir targets uniformly via the layout-aware coverage check. New concept doc `docs/concepts/multi-file-character-target.md` captures the architecture as the reference for future rich-rig (bone-based) characters with explicit open questions for the schema, part-editor integration, and per-part tuning. Closes follow-up #3 of sprite-rendering-surface.md. |
| **Grand total incl. bonus** | — | **~5.80 h** | | Remaining slack tracked against 8 h MISSION_DURATION. Final state: 100% Hall sprite coverage (verified through Rust runtime path), 0 sandbox.ldtk warnings, 3 intentional intro.ldtk authoring warnings, boss-encounters numeric-fields RON migration complete + orphan/dupe-guarded + census-logged, Idle-row miss is now a publish-time warning, sandbox builds with zero warnings, doc-health green, parser-drift bug fixed, SheetRegistry-driven tuning infrastructure landed (robot migrated as proof of concept; ~29 chars remain), mockingbird now uniform tack-on + reference doc for future bone-based rig system. |
...
Reads:
- `crates/ambition_platformer2d_actor_monolith/src/character_sprites/assets.rs::NPC_SPRITE_REGISTRY` (parsed from Rust)
- `crates/ambition_platformer2d_actor_monolith/src/features/ecs/spawn.rs::enemy_default_brain()` (parsed for archetype → preset mapping)
- `crates/ambition_platformer2d_actor_monolith/src/brain/state_machine/mod.rs::*_DEFAULT` consts (parsed for preset values)
- Renderer's `list-targets` output (cross-reference for missing sprites)

Emits:
- `crates/ambition_platformer2d_actor_monolith/assets/data/character_catalog.ron` (one entry per registered character + one entry per renderer target that wasn't registered, marked with `tags: ["needs_review"]`)
- `crates/ambition_platformer2d_actor_monolith/assets/data/boss_encounters/<id>.ron` placeholders
...
The plugin lives in `ambition_platformer2d_actor_monolith` today; a future PR can lift
...
**Files:**
- New: `crates/ambition_platformer2d_actor_monolith/assets/data/character_catalog.ron` (codegen'd from current registry)
...
**Exit criteria:**
- `cargo test -p ambition_platformer2d_actor_monolith --lib content::character_catalog` green
...
**Files:**
- Edit: `crates/ambition_platformer2d_actor_monolith/assets/ambition/worlds/sandbox.ldtk` (NpcSpawn defs + every instance)
- Edit: `crates/ambition_platformer2d_actor_monolith/assets/ambition/worlds/intro.ldtk` (same)
- Edit: `crates/ambition_platformer2d_actor_monolith/src/world/ldtk_world/` (parser reads `character_id`)
- Edit: `crates/ambition_platformer2d_actor_monolith/src/features/npcs.rs` (NpcRuntime construction)
- Edit: `crates/ambition_platformer2d_actor_monolith/src/features/ecs/spawn.rs::spawn_interactable` (catalog lookup)
...
**Files:**
- Edit: `crates/ambition_platformer2d_actor_monolith/assets/data/character_catalog.ron` (add ~25 entries)
- Edit (potential): `crates/ambition_platformer2d_actor_monolith/assets/<sprite_folder>/` (run `python -m ambition_sprite2d_renderer install <name>` for each)
...
- Edit: `crates/ambition_platformer2d_actor_monolith/assets/ambition/worlds/sandbox.ldtk` (new level + door from central_hub_main)
...
**Files (deletions):**
- Delete: `NPC_SPRITE_REGISTRY` array in `crates/ambition_platformer2d_actor_monolith/src/character_sprites/assets.rs`
- Delete: per-character `*_SHEET` `LazyLock<CharacterSheetSpec>` consts in `crates/ambition_platformer2d_actor_monolith/src/character_sprites/sheets.rs` (where the catalog now resolves the spec)
...
```bash
~/.cargo/bin/cargo check -p ambition_platformer2d_actor_monolith
~/.cargo/bin/cargo check -p ambition_platformer2d_actor_monolith
~/.cargo/bin/cargo test  -p ambition_platformer2d_actor_monolith  --lib
~/.cargo/bin/cargo test  -p ambition_platformer2d_actor_monolith --lib
~/.cargo/bin/cargo run   -p ambition_app --bin headless -- 100
python3 -m ambition_ldtk_tools validate crates/ambition_platformer2d_actor_monolith/assets/ambition/worlds/sandbox.ldtk
...
- [`crates/ambition_platformer2d_actor_monolith/src/character_sprites/assets.rs`](crates/ambition_platformer2d_actor_monolith/src/character_sprites/assets.rs) — current `NPC_SPRITE_REGISTRY` to be retired
- [`crates/ambition_platformer2d_actor_monolith/src/brain/`](crates/ambition_platformer2d_actor_monolith/src/brain/) — brain module that the catalog references
