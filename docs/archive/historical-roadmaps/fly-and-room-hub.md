...
in `crates/ambition_platformer2d_actor_monolith/src/ldtk_world.rs` keeps the engine
...
- ~~Add tests for room transitions after the room graph moves into the engine.~~ Covered by `crates/ambition_platformer2d_actor_monolith/tests/repro_walls.rs` and the LDtk runtime tests in `ldtk_world.rs`; deeper transition fuzz coverage is still future work.
