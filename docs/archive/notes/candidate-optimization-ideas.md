...
The current workspace has two crates, `crates/ambition_engine` and `crates/ambition_platformer2d_actor_monolith`, using Cargo resolver 2.  The sandbox crate currently depends on Bevy 0.18.1 plus a broad set of runtime, authoring, audio, physics, inspector, UI, LDtk, input, RON/Serde, and graph dependencies.  The uploaded `cargo tree -e features` also shows the resulting dependency graph is large, including Bevy proc-macro machinery and broad Bevy/render/devtool feature activation. 
...
The likely issue is not simply “too few crates.” The issue is that `ambition_platformer2d_actor_monolith` appears to mix several axes:
...
Would ambition_platformer2d_actor_monolith still be able to prototype quickly?
```
...
```text
PR 1: Add feature groups and optional dependencies in ambition_platformer2d_actor_monolith.
...
```text
cargo check -p ambition_platformer2d_actor_monolith                 # still works for visible/default path
cargo check -p ambition_platformer2d_actor_monolith --features dev_tools
cargo check -p ambition_platformer2d_actor_monolith --no-default-features --features headless
cargo test -p ambition_engine
cargo test -p ambition_platformer2d_actor_monolith
cargo build --timings -p ambition_platformer2d_actor_monolith       # compare before/after
```
