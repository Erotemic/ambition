...
- `cargo test -p ambition_platformer2d_actor_monolith --lib` → **1140 passing, 0 failing**
  (engine + sandbox merged; the former `ambition_engine` test suite
  now runs as part of the sandbox lib tests under
  `crates/ambition_platformer2d_actor_monolith/src/engine_core/`).
...
- `cargo test -p ambition_platformer2d_actor_monolith --lib` → 577 passing
...
`crates/ambition_platformer2d_actor_monolith/Cargo.toml` defines `headless = []` but disabling
...
* Add CI or local commands for:
  ...
  cargo check -p ambition_platformer2d_actor_monolith --no-default-features --features visible
  cargo check -p ambition_platformer2d_actor_monolith --no-default-features --features web
  ```
...
`crates/ambition_platformer2d_actor_monolith/src/app/update.rs` still has a large `sandbox_update`
...
The schedule is already strong: `crates/ambition_platformer2d_actor_monolith/src/app/schedule.rs`
...
`crates/ambition_platformer2d_actor_monolith/src/boss_encounter/systems.rs` comments say
...
`crates/ambition_platformer2d_actor_monolith/src/dialog/content.rs` (1016 lines) and intro /
