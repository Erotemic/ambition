...
**Status:** ✔ CLOSED 2026-08-14 — all eight phases landed, all twelve AC7 exit criteria verified against HEAD.  
**Date:** 2026-08-13  
...
### ✔ VERDICT, 2026-08-14 — every criterion checked against HEAD, not against this document

```text
 1 D73's definition of done          ✔ ONE character authority; the third is deleted
 2 dead actor mirrors still absent   ✔ 0 references
 3 no save→rebuild→restore           ✔ the sync is one string comparison
 4 one reaction-timer lifecycle      ✔ probe A: 5 maintenance lists → 1 owner
 5 scheduler perturbation identical  ✔ 2/2, falsifier included
 6 one construction authority        ✔ probe C: 5 production roads, 1 constructor
 7 adopt_character_intrinsics gone   ✔ 0 references
 8 roster/ArchetypeSpec gone         ✔ 0 non-comment references; the file is gone
 9 unresolved ids are errors         ✔ refused at PREPARATION, 3 typed variants
10 probes need fewer authorities     ✔ and probe B found a live fork while doing it
11 no consumerless compatibility     ✔ PreparedKit::Unauthored measured and justified
12 docs describe the final state     ✔ D73 37 marks → 14; AC7.1 naming done
```

⚠ **what is NOT claimed**: that nothing about characters remains to do. Five
items survive in D73 — the `EnemySpawnSpec::character_id` type, `WornCharacter`
→ `CharacterIdentity`, prepared completeness for death traits, and two
string-typed `PreparedSeat` ids. None is a second actor-definition system, which
is what D73 was written to delete, and this campaign's non-goals forbid starting
a successor to keep the row open.
