...
> of `ambition_platformer2d_actor_monolith`. This doc remains as historical context so future
...
- `crates/ambition_platformer2d_actor_monolith/src/audio/render.rs::render_lofi_theme`
- `crates/ambition_platformer2d_actor_monolith/src/audio/render.rs::render_sfx*`
- `crates/ambition_platformer2d_actor_monolith/src/audio/runtime.rs::TrackSource::Procedural`
- `game/ambition_app/src/bin/tune_preview.rs`
- `game/ambition_app/src/bin/music_track_report.rs`
- the `fundsp` Cargo dependency on `ambition_platformer2d_actor_monolith`.
...
The ECS [`AudioEnvironment`](../../../crates/ambition_platformer2d_actor_monolith/src/audio/environment.rs)
...
- `crates/ambition_platformer2d_actor_monolith/src/audio/mod.rs` — module root + re-exports
- `crates/ambition_platformer2d_actor_monolith/src/audio/runtime.rs` — `AudioLibrary`,
  music track table, `audio_play_sfx_messages` system, SFX cue handles
- `crates/ambition_platformer2d_actor_monolith/src/audio/render.rs` — bank-clip → Kira
  `AudioSource` adapter + `SfxBankHandleCache` for ad-hoc
  `SfxMessage::Play { id }` lookups
- `crates/ambition_platformer2d_actor_monolith/src/audio/bank_asset.rs` — Bevy `Asset` +
  `AssetLoader` for the packed `.sfxbank`; promotes to
  `SfxBankResource` once decoded
- `crates/ambition_platformer2d_actor_monolith/src/audio/web_unlock.rs` — telemetry around
  the browser AudioContext gesture unlock
- `crates/ambition_platformer2d_actor_monolith/src/music/mod.rs` — the music director, layer
  channels, adaptive cue catalog
- `crates/ambition_platformer2d_actor_monolith/src/setup.rs::try_load_sfx_bank_via_catalog`
