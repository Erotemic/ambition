...
[`vision.md`](../../../planning/vision.md) §7 (grades, the deviation rule);
[`decision-principles.md`](../../../planning/decision-principles.md) for autonomous calls.
...
[`engine/netcode.md`](../../../planning/engine/netcode.md) is the template), and continue
...
**Read [`engine/fable-final-audit-2026-07-07.md`](../../../planning/engine/fable-final-audit-2026-07-07.md)
...
[`../archive/reviews/static-audit-response-2026-07-10.md`](../static-audit-response-2026-07-10.md).
...
0. ~~**P0 architecture refactor — unified encounter orchestration (Jon's idea).**~~
   ...
   E7 audit in [`engine/encounter-orchestration.md`](../../../planning/engine/encounter-orchestration.md)):
   ...
   [`demos/sanic-recovery.md`](../../../planning/demos/sanic-recovery.md).
...
2. **The refactor chain — [engine/refactor-chain.md](../../../planning/engine/refactor-chain.md).**
   ...
   **`player/` NO LONGER EXISTS**: body vocab → `actor`, control seam →
   ...
   [refactor-chain.md](../../../planning/engine/refactor-chain.md) §R6e: measured, it is ~1560 sites
...
5. ~~**Bookkeeping:** … **D-B is REOPENED.** … add an executable line gate with a
   reviewed waiver list …~~ ✅ **D-B RE-CLOSED (2026-07-11).** The executable line gate
   landed (`engine.module-size` in `tests/ambition_workspace_policy`, config
   `policies/module_size.toml`: 1500-line limit + a bidirectional reasoned waiver list,
   poison-tested); the three over-limit modules were split; `MODULES.md` was regenerated
   and the member count corrected to 45 (44 crates + the `ambition_workspace_policy` test
   package, was mis-documented as 42). The adapter-floor ruling and playbook exit 5 remain
   valid; the sole remaining waiver is the declarative `kaleidoscope_app.rs`. See the D-B
   row in the Track index.
...
6. ~~**N0.1 — fixed-tick sim mode.**~~ **✅ DONE (opus, 2026-07-09).** The sim
   ...
   [engine/netcode.md](../../../planning/engine/netcode.md) N0.1**, along with three recorded
...
- ~~**The residual ledger is wrong.**~~ **RULED (Jon, 2026-07-10): the adapter
  ...
  **64.0k total src lines** (units matter: TOTAL, incl. tests) against a
  ...
  [engine/refactor-chain.md](../../../planning/engine/refactor-chain.md).
...
| **DECISION BRIEF — exit_3 act-timer 59 vs 60 ticks** | `ambition_demo_sanic_app::exit_3::the_demos_own_rules...` expects the act clock to accumulate exactly 60 ticks over 60 updates but gets 59 (`0.98333`). NOT the D-B splits or the InputPlugin fix — the `SimTick==119` test passes (tick counting intact), and it is a pre-existing residual the input panic had masked (all 3 tests panicked before `9519aba2`). **First hypothesis TESTED and REFUTED:** an explicit `ApplyDeferred` between `spawn_sanic_mode_owner` (deferred `spawn_mode_scoped`) and `tick_sanic_act` did NOT change the count — so it is not simply the mode-owner spawn landing a tick late. Real cause is a subtler first-sim-tick interaction: `refresh_world_time` sets `WorldTime::scaled_dt` from `Res<Time>`'s frame delta in `Update`, `advance_sim_tick` SKIPS its first increment (a priming step), and `build_demo_app` pins the frame dt via `TimeUpdateStrategy::ManualDuration` — so exactly one of the 60 sim runs contributes 0 (the first-frame `Time` delta / priming tick). OPTIONS: **(A)** make the priming tick carry a real dt (engine-timing change — audit every `advance_sim_tick` consumer); **(B)** accept 59 as correct over a window whose first tick is a zero-dt priming step and reconcile the test's expected value with a comment. A demo/engine-owner call on priming-step semantics; the `--features rl_sim` replay/canary suites already pass, so it does not block N3.x. Parked, not guessed. | [opus / Jon] |
...
- **opus** — **N0.1 FIXED-TICK LANDED.** The sim no longer names a schedule: it
  ...
  [engine/netcode.md](../../../planning/engine/netcode.md).
