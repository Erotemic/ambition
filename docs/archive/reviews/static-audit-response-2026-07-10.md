...
- ~~**Resolved-codec decode failure is indistinguishable from authored absence**~~ **RESOLVED 2026-07-11.** `SnapshotResolve::resolve` now returns `Result<Option<Self>, ResolveDecodeError>`: `Err` (a truncated / non-canonical blob) → `DecodeFailed`; `Ok(None)` (the authored half is gone) → a content-change `Unapplied`; `Ok(Some)` → `Applied` after the `finish()` check. `resolve` decodes the whole blob before it looks up content, so a truncated blob is `Err` even when the content is present. Pinned by `a_truncated_resolved_blob_is_a_decode_failure_not_a_content_change`. NARROWER RESIDUAL: cursor/resolved codecs decode into a live target, so they still cannot be PREFLIGHTED standalone — a decode failure surfaces mid-reconciliation, not before.
