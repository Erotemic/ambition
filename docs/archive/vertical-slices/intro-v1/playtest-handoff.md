...
```text
 1. Run `cargo run -p ambition_actors --bin headless -- --start-room=intro_wake_room`
...
cargo run -p ambition_actors --bin headless -- --start-room=intro_wake_room
...
-p ambition_actors --lib` (584/584), headless smoke for 120 ticks.
...
### A. Chained flag system (`crates/ambition_actors/src/intro/route_state.rs`)
