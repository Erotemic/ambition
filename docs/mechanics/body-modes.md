...
- The sandbox has body-mode modules under `crates/ambition_actors/src/body_mode/`.
- Player-authoritative runtime state lives on ECS player components under `crates/ambition_actors/src/player/` and the engine `Player` state.
...
```bash
cargo test -p ambition_actors --lib engine_core::player_state
cargo test -p ambition_actors --lib engine_core::movement
cargo test -p ambition_actors body_mode
