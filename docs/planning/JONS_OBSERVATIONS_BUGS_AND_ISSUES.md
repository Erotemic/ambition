...
* In the ambition game, when I move from one room to another that is separated in LDTK, the camera moved as if there is a pan that should be happening. The camera room transition pans are just wrong.
  ...
  * ▢ **but there IS a real gap next door, if you want it closed:** the camera snaps ONLY on a room change or a blink, never on "the subject teleported". A synthetic teleport inside one room panned it 440px over about 40 ticks. ⛔ **and "no authored transition is of that kind" was WRONG — `sandbox`'s `portal_lab` authors FOURTEEN portals in one room**, so a transit there is exactly that teleport. `PortalCameraTransitMode::Pop`'s own doc hands the case straight back: *"if its focus teleports, the camera pops/snaps/lerps exactly as the host camera system normally would"* — which is the lerp. Ambition defaults to `Continuous`, which fixes the ROLL and says nothing about the position, so the 440px pan is reachable today by walking through the lab in `Pop`. ⚠ the multi-fighter cast camera already has the missing term (`CastFraming::teleported` arms a settle allowance); it is the single-subject FOLLOW path that has none.
