...
* ~~**[agent-found] The couch-input smash test fails in a full-binary run.**~~ **FIXED, and MEASURED GREEN BOTH WAYS 2026-08-08 (`527a619f7`).** `a_keyboard_player_and_a_pad_player_drive_different_fighters` passes in isolation (`1 passed`) and in the whole binary (`320 passed; 0 failed`, 213s). The numbers, driven rather than read: the pad moves its own fighter **175.17px** and the keyboard player's **0.00px**; the keyboard moves its own **86.13px** and the pad player's **0.00px**. Two sibling tests measure the same property with two and with four pads — `versus_stage::two_controllers_make_versus_a_two_player_game` (83.54px / 0.00px each way) and `versus_stage::four_pads_each_move_their_own_fighter_and_nobody_else_s` (57.71px own, 0.00px on all three others, for each of the four). **Twelve cross-seat pairs, all exactly zero.**
  * ⭐ **the fix was `65d31c116`, and it is the one the note below predicted.** *"A mirror match was one fighter to every id-keyed index in the runtime"* — `realize_seat` used the CHARACTER id as the body's identity, so two seats wearing one character shared one entry in the anti-clump slot board (and in `spawn_dynamic_feature_visuals`, `entity_to_id`, `faction_by_id`, `target_entity_by_id`). Two fighters stood in one place and pushed each other apart. It landed on `main` in the `b6ee30a1d` merge at **08-07 12:09**, ten hours after the 02:14 re-measurement below — so the "unconditional failure" reading was correct about the tree it was taken on and stale by lunchtime. Spawn separation is 192px now.
  * ⚠ **the CONTACT reading below was right and the assertion's wording was wrong**, exactly as it says. Anyone re-probing this: measure the two bodies' separation and the SIGN, and settle for ~120 frames after releasing a key — measuring immediately reports 17.21px of "leak" that is only momentum decay.
  * *(kept for the method — arguing from the sign of the displacement is what solved it)* ⚠ RE-MEASURED 2026-08-07 and the diagnosis below is STALE. `app_it::smash_in_the_host::a_keyboard_player_and_a_pad_player_drive_different_fighters` passes in isolation and fails when the whole `app_it` binary runs (measured both ways, 2026-08-02, and confirmed identical on a clean worktree at the previous commit — it is not a regression from the Mary-O geometry or contact-harm work). So it is order- or parallelism-dependent: something earlier in the binary leaves seat/pad state that this test then reads. That is a real defect and not merely a flaky test — a keyboard participant and a pad participant driving the same fighter is exactly the couch bug class that has bitten repeatedly, and the isolated pass is what makes it invisible. Worth finding what the shared state is rather than adding a serial-test attribute over it.
