...
* Super sanics spikes are clipped by the sprite renderer. This might need a
  ...
  ⊙ **Re-swept 2026-08-31, and this time the POPULATION is complete: 38 of 209 targets, 405 frames.** Every discovered target rendered through its own road; all 100 config-driven targets are clean and every clipper is a `module` target. ⛔⛔ my first pass this session reported **42 of 209** and was wrong in a way worth naming — it reimplemented the render loop over `module.ROWS`, which 116 targets do not export, so it measured a SAMPLE and called it a census.
  ✔ `perfect_cellular_automaton` is CLEAN now (0 of 913): the "one that matters" line above is spent. ✔ `carl_stargan` (8 frames) was a REAL cut and is fixed — the shared scientist `roll` tucks 38-42px below the ground line and his was the one canvas of the three with no room for it, because he restated the rig frame as his published frame instead of adding the padding his two siblings already had.
  ⚠ **AND YOUR ORIGINAL POINT NOW CUTS THE OTHER WAY TOO.** *"We should not be able to clip sprite artwork so easily"* is right, and the guard built for it cannot yet tell a cut from art drawn deliberately flush to a fitted frame. Mary-O's six forms (41 frames) report `bottom` because a 24x32 logical sprite is seated flush on the last row and a flat pixel-art foot does not taper; re-rendering with headroom finds at most two raster rows of ink beyond the edge. The honest open work is that instrument, not 35 canvas edits — inflating those canvases would move every Mary-O sprite's ground contact to silence a false positive.
...
* The smash UI for character select looks good, but the controls don't feel good, they are very hard to use with a gamepad. 
  ...
  * ⊙ **TWO MORE EXPLANATIONS RULED OUT, measured 2026-09-04 — the complaint is narrower than it was.** After the snap fix above, two obvious remaining suspects were the shape of the snap GRAPH, and neither survives measurement:
    ⇒ **Every portrait is reachable from every other by the d-pad alone.** Walked the real desktop layout's real targets as a directed graph — four directions from each portrait, `cursor::snap` deciding every edge — and the portraits form one reachable set. A cell you can enter and never leave, or a column cut off from its neighbour, would pass every existing one-hop snap test and be unusable on a pad; nothing was checking the whole graph. Guarded now (`every_portrait_is_reachable_from_every_other_by_the_dpad`), and poison-verified: disabling vertical snapping reports *"17 of 23 portraits cannot be reached from portrait 0"* with the list.
    ⇒ **The grid is 8 presses across**, found by tightening a bound until it failed (≤6 fails, ≤8 passes) on the 23-cell desktop grid. Reachable is not the same as reasonable, but 8 is reasonable for a grid this shape — so *"it takes forever to get anywhere"* is not the complaint either.
    ⇒ ⚠ So whatever remains is not reachability and not distance. The untested suspects are now the free-pointer FEEL — the speed ramp, the deflection curve, the arming threshold, all of which have unit tests pinning their maths and none pinning whether the result feels good — and the token drag. **Both need a human on a pad; neither is measurable from here.**
...
* In smash it should be easy for 2 controllers to select their own characters, or turn other characters off or into cpus, any controller should be able to turn a slot into a player if there is a controller connected to it.
  ...
  * ✔ **THE "OFF OR INTO CPUS" HALF IS SHIPPED, MEASURED 2026-09-03 — this bullet's ▢ was covering a built feature.** `SlotOccupant` (`game/ambition_demo_smash/src/select.rs:179`) has exactly the three states the ask names — `Absent`, `Controller { device }`, `Cpu` — and `cycle_role` (:388) walks them in a ring: `Absent → Controller → Cpu → Absent`, skipping the controller rung when no free source is left so a lone player reaches CPU in ONE press rather than two.
    ⭐ **And it is reachable from a pad, which is the half worth checking given your gamepad complaint.** The road is `SelectTarget::RoleButton(slot)` → `layout.targets()` (`select_screen/layout.rs:367`) → `rects` → `snap_rects` → `cursor::snap` (`select_screen/cursor.rs:104`), so directional navigation lands on a role button exactly as it lands on a portrait. It is not mouse-only.
    ⭐ **Covered end-to-end through the real screen, not through the model**: `a_player_alone_can_add_a_cpu_and_start_the_match` (`the_screen_decides.rs:503`) clicks the role button, asserts the second card becomes `Cpu`, picks fighters, presses START, and asserts the resulting `MatchParticipantRoster` gives seat two a `brain_profile`. The `Cpu → Absent` rung — turning a character OFF — is pinned by `the_slot_button_cycles_absent_controller_cpu` (`select/tests.rs:95`).
    ⇒ ⚠ So of this report's three asks, TWO are shipped ("2 controllers select their own characters" was fixed in `5902930a7`; "off or into cpus" is the above) and the remaining `▢` is only the ⊙ question below — which of the two failure shapes you actually saw. It should not be read as "the CPU/off feature is missing".
...
⭐ **The three respawn branches already have a written 2026-08-25 verdict**: *"verdict 3:
