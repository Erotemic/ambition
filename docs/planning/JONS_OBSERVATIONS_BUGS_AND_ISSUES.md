...
* When I change the video quality in ambition, my sprite went from the robot v3 character to the robot v2 character. 
  ...
  * ▢ Where to look: quality variants are deliberately NOT resolved by the shared target road — `SheetRegistry` skips them (`is_quality_variant_file_root`) and keys them through a separate FILE-ROOT index (`from_baked_table_by_file_root` / `character::sheets::record_index`). That index is what a quality change re-resolves against, so a v3-to-v2 swap is a wrong file root there, not a target collision. (Target resolution itself is sorted last-wins, so it is deterministic and cannot flip on reload.)
  * ⭐ And the likely SITE is now named: `3bf154974` (2026-08-08) made a quality change RETIRE and RE-MATERIALIZE the bodies already on screen instead of only the next room — `CharacterSheetState::Declared` says so ("retired by a quality change"). Before that, an on-screen body kept its old handle and could not change at all. ⇒ the re-materialization path is where a v3→v2 swap can happen, and it is the newest thing on that road. Owner doc: `sprite-residency-and-live-quality.md`.
