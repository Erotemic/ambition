...
* ◐ Also listed, unreviewed in detail — THREE OF SIX ARE DONE: the held bomb now blasts at its holder's hand (`69a0918f5`), the Admiral's gun-sword has the discharge its comments promised (`216316bef`), and aim assist no longer bends toward an `OutOfPlay` fighter (same push). STILL OPEN: stored charge has no deliberate "put it away" action; the ponytail boomerang disappears the first time it hits anyone; bomb interaction is single-controlled-subject and hard-impact detection only understands world geometry. Tracked as D254 R8/R9/R10/R11 in `triage/gpt-review-2026-08-27-remaining.md`.
