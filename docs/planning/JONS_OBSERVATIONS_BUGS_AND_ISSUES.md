...
* JON (2026-08-08 STILL OPEN) The snake and AI slop are still way too big visually, and the sprite might not match the box for the snake.
  ...
  * ⇒ ⭐⭐ **so all three of your size reports reduce to ONE blocked question, and it is the six numbers.** Zero of 145 rows author a height, so **100% of characters take the broken branch** — the good one has never run in shipped content. The data it needs is already there: **203 of 206 spritesheets publish `body_metrics`**, all six pirates included. Nothing is left to build here.
