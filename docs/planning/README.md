...
⛔⛔ **AND TWO ROWS OF THIS TABLE DRIFTED AGAIN, which is the table's own point
landing on the table.** Re-measured 2026-09-05 against each figure's own
AUTHORITY, not by eye: `check_absence_contracts.py` prints *"51 crates linked, 23
a movement-only game never asked for"*, and `run_tests.py --list
--run-everything-…` prints *"Planned 53 job(s)"*. Both corrected columns were one
low.

⭐ **The fix for a figure like this is to CITE THE COMMAND, not to retype the
number** — every one of these has a tool that computes it, and the table is a
copy that has now gone stale twice.

⛔ **AND DELIBERATELY NOT A GUARD, which is the tempting third step.** A check
pinning "footprint == 51" would redden the moment a crate is added — legitimate
growth, no defect, and an absolute-count guard that can never go green again. The
thing worth guarding is already guarded: `capability-footprint-may-not-grow` is
an absence contract and it PASSES. These figures are documentation of a past
correction, so their staleness is cosmetic and a gate would cost more than it
catches.

⚠ **Two rows were NOT changed, and nearly were.** The monolith's
`[dependencies]` reads 52 to a naive line count and **33** to a parser asking for
`ambition_*` — 33 is what this row means and it is still right; my first count
measured a different population. The rollback tally's "409 names" is not the
baseline file's 480 lines either, and I did not establish what it counts, so I
left it alone rather than publish a drift I had not measured.

| figure | said | was | why it moved |
|---|---|---|---|
| capability footprint | 45 crates / 18 | **51 / 23** (was 50) | four carves landed; drifted again 2026-09-05 |
...
| exhaustive job count | 49 | **53** (was 52) | one job per new crate; drifted again 2026-09-05 |
