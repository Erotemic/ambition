...
| AC6 delete archetype/roster/tuning authority | ⏳ **UNGATED 2026-08-13** | 2026-08-13 | | ⭐ the gate dissolved on inspection: **all three possible answers to the `small_lurker` question delete the ontology identically** — the word only picks a body, and that is one string constant. When Jon said *"continue on the hard parts"* with this as the sole gate, the surfaced recommendation was applied provisionally: `GRADIENT_CASCADE_MINION_ARCHETYPE = "npc_ai_slop"` (the cascade's own *"slop minions"* design note; the sibling constant already cast; reversal = that constant). `OPEN_CASTING` is EMPTY, `what_still_needs_an_archetype_row` expects `[]`, every summon resolves a registered character. ⇒ now the deletion: ron file + schema + `ArchetypeSpec` + `CharacterRoster` + waiver machinery + fallback roads + `ActorTuning` |
