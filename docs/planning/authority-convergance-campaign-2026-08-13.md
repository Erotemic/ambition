...
| AC6 delete archetype/roster/tuning authority | ◐ **UNGATED + content half DONE; the machinery deletion is next** | 2026-08-13 | | ⭐ the gate dissolved on inspection: **all three answers to `small_lurker` delete the ontology identically** — the word only picks a body. Applied provisionally per the surfaced recommendation: `GRADIENT_CASCADE_MINION_ARCHETYPE = "npc_ai_slop"` (`5de883e79`; reversal = that one constant). ✔ **NOTHING LIVING RESOLVES A ROW ANYWHERE NOW**: `OPEN_CASTING` empty, `what_still_needs_an_archetype_row` expects `[]`, every summon resolves a registered character, and **Mary-O's fragment — the last LIVING rows (the two plane swarms) — is gone** (`1cae63776`): Mary-O owns them whole (catalog rows + definitions, `plane.rs`), the fork's two hidden divergences (patrol pace, respawn) surfaced and settled, 4 placements author `respawn` via LDtk tooling (map-assets `0464bd2`), two D102-refusal fixtures repaired to production shape. ⇒ **REMAINING**: (1) Sanic's badnik resolves via the HOST waiver to generic `combatant` — author `sanic_badnik` as a character (its own file documents the intent: 1-HP wanderer over the published `ai_slop` sheet; D96 6 is that question, same shape as the lurker); (2) delete the roots compiler-guided: Ambition's one-row `character_archetypes.ron` + `content_schema` + `ArchetypeSpec` (`ambition_combat`) + `CharacterRoster`/fragments/waiver machinery (`enemies/mod.rs`) + `spec_for_brain`/`has_brain_key`/`generic_body_for_*` + `ActorClusterSeed::new_in` fallback roads (`spawn_actors.rs` 1430/1828/2505) + `ActorClusterSeed.spec` field + `spawn_encounter_mob`'s roster param (then power_loop's empty-fragment fixture); (3) AC6.2 `ActorTuning` collapse by ownership — classify surviving fields, no `ActorTuning2` bag. ⚠ ledger D112 (Mary-O death road broken) fails at HEAD, predates this work, probe findings recorded there |
