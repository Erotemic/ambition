...
- `scripts/multi_writer_resource_census.py` is the WRITER-side shortlist: which `Resource`s are written from more than one production file. ⚠ **NOT a finding list** — the discriminator is on the reader's side, and the script's own docstring carries two measured cases that came out opposite ways. `scripts/check_multi_writer_resources_are_adjudicated.py` ratchets the population on every `--maintenance` run so a new one cannot land unnoticed; 4 of the 85 carry a written verdict, and it prints the other 81 as debt rather than implying they were read. ⭐ **AND A VERDICT COSTS A POISON, NOT A READING** — `ClockState` was adjudicated on 2026-09-17 by breaking the coupling its three writers depend on (`apply_suspended_time_scale_system` zeroes the REQUESTED scale as well as the live one, so the smoother cannot ramp back up) and watching `suspended_frame_zeros_world_time_scaled_dt` go red. An entry without that step is an opinion in a machine-readable list.
