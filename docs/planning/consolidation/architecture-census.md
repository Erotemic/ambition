...
5. **Dynamic entity spawn.** Projectiles/summons use entity-level identity and lifecycle; they are not world-replacement transactions. ⭐⛤ **ENUMERATED 2026-09-18, and `ROAD-DYNAMIC-SPAWN` was the one road row naming no entry point — which is what made it `SOURCE_INFERRED`.** There are THREE spawn-request seams in the tree: `ProjectileSpawnRequest` (`crates/ambition_projectiles/src/spawn_request.rs:57`), `SpawnActorRequest` (`crates/ambition_platformer2d_actor_spawn/src/actor_spawn/mod.rs:98`) and `SpawnPlayerCloneRequest`. ⇒ **The mechanism splits them, and the split predicts which one was broken.** The two domain seams are `Message`s raised INSIDE the simulation and read inside it, so a resimulation re-raises them and nothing crosses the rewind boundary. `SpawnPlayerCloneRequest` was the odd one out — a `Resource` flag raised OUTSIDE the simulation — and it was `Q136`'s first landed specimen, fixed on 2026-09-18 by routing it through the mechanical-edit road. So this row's *"domain request plus parent/source mechanical state"* is accurate for the two real domain roads, and the exception was an ingress defect rather than a fourth model.
