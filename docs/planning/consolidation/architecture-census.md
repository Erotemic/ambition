...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| ROAD-INITIAL-SESSION | `translate_shell_session_lifecycle` -> `activate_prepared_platformer_sessions` | game shell + platformer provider | prepared content, `SessionMechanics`, prepared room/session world | `ShellActivationId`, `SessionScopeId`, load/prepared transaction data | `PlatformerSessionWorld` materialization plus prepared room construction | canonical `SessionRoot` plus activated provider/session state | shell/load/gate refusal can stop activation before the session is live; this census does not prove every provider failure path at runtime | shell session retirement and session-scope cleanup | normal session root and room construction primitives | SOURCE_INFERRED |
...
| PUB-ROOM-LOADED | room transaction verifier | `verify_and_publish` writes `RoomLoaded` | yes for the message, no for already-written live resources | message can be withheld while earlier resource/root changes already happened | notification only; not a full authority switch | publication notification, not full authority switch | SOURCE_CONFIRMED |
