...
| PUB-CONTENT-GENERATION | `publication_boundary` through the shell gate | `commit_content_generation` and content/provider activation code | yes: pending candidate and gate verdict are separate from active selection | candidate can remain pending or be refused before active family publication; the shell/content A-supersedes-B race is HALF witnessed (MEASURED 2026-09-16, correcting an earlier same-day claim: the session half passes in the shipped visible composition via `a_candidate_session_replaced_while_pending_is_discarded`; the transaction half is unit-only — see `consolidation/README.md`) | content activation and scene/world replacement are still separate publication slices | candidate/admission publication road | SOURCE_CONFIRMED |
