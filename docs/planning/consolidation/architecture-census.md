...
| PUB-CONTENT-GENERATION | `publication_boundary` through the shell gate | `commit_content_generation` and content/provider activation code | yes: pending candidate and gate verdict are separate from active selection | candidate can remain pending or be refused before active family publication; the shell/content A-supersedes-B race still owes its named production witness (MEASURED 2026-09-16: three unit witnesses exist over a hand-built `ShellRouter`; the missing piece is an `app_it` arm in the composed host — `consolidation/README.md` names all four) | content activation and scene/world replacement are still separate publication slices | candidate/admission publication road | SOURCE_CONFIRMED |
