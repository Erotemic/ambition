...
⚠ **ROW 4's ASIDE, moved here 2026-09-06 because it sat BETWEEN rows 4 and 5
and broke the table** — a prose line inside a markdown table ends it, so rows
5–13 were rendering as a headerless block. Text unchanged:

⛔⛔ **AND I OVER-ESTIMATED HOW ASSEMBLED IT IS — correcting my own estimate from an hour earlier on this same page.** `acquire_captures` catches BODIES and a ledge is not one, so her 150px grab cannot become a ledge tether by reach alone. The probe exists (`probe_ledge_grab_in_frame`, which could be called at a VIRTUAL position — *"could a body standing there catch a ledge"* — a legitimate reuse) ⚠ **but it has NO production caller outside its own tests**, and it wants a `&World`, so wiring it into a move is engine work rather than composition. ⇒ The honest cost of this row is a technique plus a probe adapter plus the visual extension — comparable to the bolt, not to the mine. ⭐⭐ **RE-MEASURED 2026-09-05 and the blocker SURVIVES — the first of this week's five that did.** Precisely: `probe_ledge_grab_in_frame` still has no production caller; **`ae::World` is a `Resource` nowhere in the tree**; and the ONE place `ambition_combat` builds one is the fighter brain's recovery lens (`brain/fighter/recovery.rs:149`), which assembles a *"perceived stage"* out of the AI PERCEPTION layer — deprioritised by the goal, and the wrong geometry for a human-facing move besides. ⇒ The LDtk road does publish `LdtkRuntimeSolidIndex` as a `Res`, but a smash stage is built programmatically and never passes through it. ⛔⛔ **AND THAT BLOCKER IS STALE — RE-DERIVED 2026-09-06, AND IT WAS NEVER TRUE OF THE ENGINE, ONLY OF MY SEARCH.** The claim above is that the stage's solids are not readable from a system, so the row needs engine work in the movement crate. ⇒ `ambition_platformer2d_world::collision::CollisionWorld` is a **SystemParam** whose own comment calls it *"the composed collision read-API rather than its three ingredients"*, and `CollisionWorld::solids() -> Option<Cow<'_, ae::World>>` hands back exactly the assembled `ae::World` the probe wants — sandbox solids, portal carves and gate solids included. ⭐ **It already has a production caller inside `ambition_combat` itself**: `pogo_moveset_off_world_orbs` takes the param and passes `&assembled` to `pogo_target_for_attack_hitbox(world: &ae::World, ..)`. Both crates are already `ambition_combat` dependencies and `ledge_grab` is a `pub mod`, so `probe_ledge_grab_in_frame` is callable from a combat system TODAY. ⇒ **The measurement that produced the false blocker was `grep 'Res<..World>'`, and a composed read-API is invisible to it** — the world is ASSEMBLED from its ingredients per call, so it is a `Resource` nowhere and readable everywhere. Searching the spelling instead of the concept, for the fourth time this week. ⇒ The row's honest cost is a technique plus a visual extension: composition, not engine work. The one thing the probe genuinely needs and a move does not have for free is the `wall_normal_x` it fires off — it wants a body already resolved against a wall, so an AERIAL tether reaching for a ledge it is not touching must supply that side itself rather than read a contact. ⇒ **AND THAT IS EXACTLY WHAT IT DOES.** The line goes where she FACES and asks for the wall whose normal points back at her — `-facing.signum()`, the same reading the kernel uses for an airborne grab request. ✔ **LANDED: `smash_tether` (authoring) + `ambition_demo_smash::tether` (runtime), on `polygon_projectile_recoil_lift`.** ⭐⭐ THE ROW'S POINT IS WHAT THE MOVE DOES NOT OWN: the reel delivers her to `LedgeContact::anchor` — documented as *"world position the player should snap to"* — and then LETS GO. The kernel's own `try_start_ledge_grab_clusters_in_frame` decides whether she catches, keeping its release cooldown, its eligibility and its trumping rules in one place. A tether that set `LedgeHang` itself would have been a second implementation of ledge state. ⇒ THE RULE — coordinate authorities, never become one — is the whole implementation, not a caveat on it. ⛔ AND THE HANDOVER HAS A SHAPE, found by reading how the authority catches rather than by assuming it does: it wants a stick-requested wall normal or `FALL_SNAP_MIN_VY` (45px/s) of DESCENT, so a reel that ended while still carrying her upward would leave her hanging in the air beside the ledge. Arriving therefore commands ZERO and hands her to gravity, which crosses 45px/s in about two frames and a pixel of drop. ⚠ The two exits are deliberately different: GIVING UP leaves her momentum alone, because a reel that expired and also stopped her dead would delete the recovery she had left. Collapsing them was the first draft's bug and is now a test. ⭐ AUTHORING SURFACE MEASURED: `TetherPullParams` is **3 fields** against a median of 4 across 20 techniques (`scripts/authoring_surface.py`), and the authoring helper's own asserts catch the mistake the three fields make together — `speed * timeout_s >= reach`, because a line that bites at full reach and runs out of clock halfway drops her in the blast zone and reads as an engine bug. GUARDS: 5 tests, **6 poisons run red** (reach ignored, aerial gate removed, flat-speed overshoot, collapsed exits, arrival not zeroed, anchor latched to her own position) plus a 7th on the content side (her up-B retuned away from her grab's reach). ⚠ One poison had to be rewritten: ignoring the authored reach by changing only the sample DISTANCE left the step count derived from it, giving 3 samples over 4000px — a sampler that steps over every ledge, which failed the wrong tests and proved nothing about the reach gate. ROLLBACK: `TetherReel` is registered `component-clone-probed`, schema **v164**. The anchor is the load-bearing half: it is a point on the stage she committed to on ONE frame, so a peer that restored the reel without it would re-probe and could latch a DIFFERENT ledge — a divergence that grows instead of correcting. ⓘ Only TWO ledgers moved, not four: `component-clone` types appear in `registry.rs` and `rollback_schema_baseline.txt` and in neither the codec-shape file nor the JSON baseline, which was verified against `smash.homing_dash`'s own rows rather than assumed. ⭐⭐ AND THE SEAM ITSELF IS NOW ASKED OF THE AUTHORITY RATHER THAN ARGUED IN PROSE: `where_the_reel_releases_her_is_a_place_the_authority_would_catch` runs the reel to completion and then calls **`probe_ledge_grab_in_frame` — the same function the kernel calls every frame** — from where it let go. A reel that delivered her a few pixels wrong reddens there instead of in a playtest; poisoned twice (releasing 40px early, and reaching for the wrong side) and red both times. ⚠ THIS IS THE GEOMETRIC HALF ONLY. Catching also wants descent (`FALL_SNAP_MIN_VY`), which gravity supplies, so the remaining unproven claim is narrow and named: a LIVE match ending in `LedgeHang`. ⛔ Its cost was measured rather than guessed — `ambition_platformer2d_core::test_support` is `pub(crate)`, so the kernel cannot be driven from this crate, and the demo app's roster is three stand-ins that do not author the move (injecting the action message into a live match is the road, and it is a row of its own). ✔ **AND THAT ROW IS DONE, 2026-09-06 — `the_tether_catches_a_ledge` runs a REAL MATCH on a REAL STAGE and she ends up hanging.** It reads the main platform's lip out of the live `RoomGeometry` rather than hardcoding it, drops her off the edge facing in, injects the technique (the demo roster is three stand-ins who do not author a tether, and a response and a move event both arrive as an ordinary `ActorActionMessage`), and asserts a hang. ⭐ It is PAIRED with a miss — the same position with the line thrown the other way must NOT hang — because a ledge authority runs every frame and will catch a falling body on its own, so an unpaired hang is evidence of nothing this row built. ⛔⛔ **AND IT IMMEDIATELY FOUND A BUG THE WHOLE UNIT SUITE PASSED THROUGH.** `LedgeContact::anchor` is a HANG position, and a hanging body OVERLAPS the wall it hangs on: measured live, her body is 34.4px wide, the anchor sat at x=63.8 against a platform face at x=80, so the anchor puts her right edge 1px INSIDE the solid. The swept resolve correctly refuses to move a free body there, so she pinned ~1px short and the arrival test never fired. A fixture with no collision lets her reach the anchor exactly, which is why six unit tests were green. ⇒ The reel now ASKS THE AUTHORITY — `probe_ledge_grab_in_frame` from where she currently is — and releases the moment the answer is yes. No tolerance to tune, and it cannot disagree with the thing it is handing her to. ⚠ **AND MY FIRST DIAGNOSIS OF IT WAS WRONG, caught by a poison that PASSED.** I wrote that the pin was a livelock that prevented the catch. It is not: the reel rides out its timeout, releases on the clock, and the authority catches her anyway — tick 22 chasing the anchor against tick 6 asking. The cost is a quarter-second of a fighter stuck to a wall, which reads as the move failing and then working for no reason. ⇒ The test now asserts a PROMPT catch (12 ticks, chosen between the two measurements) because a 40-tick window is green either way — which is exactly what the poison exposed. ⭐ `ledge_assist` on a TELEPORT remains the cheap way to get *"a recovery that catches a ledge"*; what it cannot give you is a tether. ⚠ Marked ◐ rather than ✔ deliberately: B2's own row says "ground tether", and reading that as the whole row is how a half-built capability gets reported as finished |
...
| `PlacedMine`, the bomb, the ponytail | ⚠ **a placeholder quad** | `GroundItem` resolves through `HeldItemArt`, whose doc says *"absent / unmatched → the placeholder quad"*. ⛔⛔ **CORRECTED 2026-09-06 — THE REGISTRATIONS EXIST; THE ART DOES NOT.** I read "placeholder quad" as a missing `HeldItemArtEntry` and it is not: `polygon_bomb`, `polygon_mine` AND `polygon_ponytail` are all three declared in `items/held_visuals.rs`, which a new guard (`every_held_item_a_move_creates_has_art`) now holds. ⇒ What is missing is the FILE — no `polygon_*.png` exists anywhere in the asset tree. ⚠ My earlier claim came from grepping `HeldItemArtEntry::new("…")`, which matches only literal-string calls and missed how these are declared: the same truncated-search error as `head -12` and `somebody_elses_move`. ⭐ **AND NOTHING CATCHES THE GAP, though the pattern exists one shelf over**: `check_gauntlet_props_are_rendered.py` holds exactly this for gauntlet props — *"game declares an id the renderer does not draw → FAIL"* — and held-item art has no sibling. ⇒ The generator path is mapped: `ambition_sprite2d_renderer`'s `targets/icons/item_icons.py`, whose `ICON_SPECS` list feeds `SHEET_FILES` automatically and already carries a reusable `bomb` drawer, so the bomb needs a spec and the mine and ponytail need a spec plus a small drawer each. ⛔ THE GENERATOR PATH WAS MAPPED WRONG ONCE MORE ON THE WAY IN: I reported `write_gauntlet_props` had NO CALLER, having grepped `--include=*.py` — the caller is `scripts/regen/sprites.sh:1366`, a shell line invoking it through `python -c`. A language filter is a truncated search, the same shape as `head -12`. ✔ **DONE 2026-09-06.** All three now wear their own procedural art: `HELD_ITEM_ICON_SPECS` in `item_icons.py` (three new drawers, installed to `sprites/props/` by the same `write_gauntlet_props` the regen script already calls), and `held_visuals.rs` points at them — the javelin's `44×6` extent went square with the art, which is the bug a borrow leaves behind when it ends. The ponytail took three passes and the two failures are the design note: a constant-width stroke with a ball on one end reads as a **wand**, a monotonic taper reads as a **balloon**; a ponytail is narrow at the band, flares below it, then runs to a point, so the width profile is now explicit in `_tress_radius`. ⭐ And the check this row said would redden the lane is landed with it (`check_held_item_props_are_rendered.py` + 6 tests), because the art existing is exactly what removed that objection. Five poisons run RED: the game repointed at borrowed art, a spec deleted under a live registration, a renderer-side rename, an emptied spec list, and the Rust constructor renamed. ⚠ Its FIRST draft failed against the live tree on `axe` and `javelin` — self-named art drawn by a different renderer target — so the orphan rule is scoped to the families the spec list owns, and that false positive is pinned as a test rather than described in a comment. |
...
- ◐ **A1. `TechniqueFlow` minimum** — ⭐⭐ **THE ENGINE HALF WAS ALREADY BUILT,
  ...
  - ⛔ **Symbolic slots: GENUINELY ABSENT, AND DELIBERATELY NOT BUILT — measured
    2026-09-06.** Zero matches, and the reason to leave it that way is not taste.
    The plan defines a slot precisely: *"a move references a semantic occurrence
    (`"thunder"`) instead of holding an `Entity`, which is what keeps rewind,
    inspection and remote detonation clean."* ⇒ **So a slot needs a later node
    that names something an earlier node made, and no shipped move has one.**

    | measured | |
    |---|---|
    | **every flow in the game has exactly ONE `Emit`** | all three of them — the oni's `iaijutsu`, the goblin's `headlong_charge`, the demo's `read_and_seize`. Each waits for a verdict and fires one effect. **Nothing a later node could refer to is ever created** |
    | **`FlowNode::Emit` is caster-anchored** | it writes `world_offset: ae::Vec2::ZERO`, and the technique handlers compute their own position from `kin.pos` plus authored params rather than reading the event's offset. A slot has nothing to point AT without changing every handler |
    | **the three candidate customers each already have a domain-owned name** | the mine answers to `PlacedMine::owner_seat` and is strictly one-per-seat (a press with one out detonates or is ignored — a second is unreachable by construction); the portal pair links by `Indexed(n)`/`Indexed(n ^ 1)`; the goblin's grab-on-connect reads `MovePlayback::hit_targets` |
    | **sequencing is being authored, just not through flows** | 3 flows against **8 cancel windows** and 2 `on_hit` techniques. When nine specials were authored on 2026-09-06 and one wanted "land this, then that" (`shadow_answer` into `iaijutsu`), the thing it wanted was `cancelable` — the timeline's answer, already shipped |

    ⚠ **AND THE MINE — the plan's own "remote detonation" example — is the
    clearest refusal.** It deliberately OUTLIVES its move: you place it, the move
    ends, and a later press sets it off. `MovePlayback` is per-occurrence memory,
    so a move-scoped slot **cannot** hold it. The example that motivated slots is
    the example that rules them out at this scope.

    ⇒ **The rung is not refused, it is unpriced.** The campaign's acceptance rule
    is that a move earns an engine change only by introducing a new semantic
    primitive, and no authored move asks for this one. ⭐ The condition that would
    reopen it is stated so it can be checked rather than re-argued: **a move that
    creates TWO live things of the same kind within one occurrence** — where every
    seat-scan and link-group index becomes ambiguous and only a symbol can
    disambiguate. Nothing on the roster does, and `read_and_seize`'s constant
    `offset: (22.0, 0.0)` is tuned to its own hitbox reach rather than guessing at
    a victim it cannot name.
