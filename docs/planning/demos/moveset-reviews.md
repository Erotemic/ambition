...
|---|---|---|
| *"we have a lot of characters with boring specials"* / *"Many have boring specials"* | roster-wide | ⇒ the expressiveness census now stands at **18 expressive, 1 plain**; the one plain entry is `theorem_chain`, the duel arena's minimal two-hit demonstrator, which is plain deliberately. ⚠ **THAT COUNTS FIGHTERS, AND A SECOND CENSUS COUNTS SPECIALS — they disagree usefully.** `the_census_of_specials_that_carry_no_technique` (2026-09-06, `cargo test -p ambition_content --lib the_census_of_specials -- --nocapture`) walks every `special*` verb in `tables()` and asks what each move CARRIES: a technique key (an `Effect` on the timeline or a window's `sustain_effect`), and failing that any other authoring — extra hit windows, a charge, a start impulse, timeline events. ⇒ At the fighter level `pugnacious_polygon` counted as EXPRESSIVE while all five of its specials were single hitboxes, which is precisely the state Jon's sentence is about. ⛔ And the finer number needs its second half or it misleads the other way: **65 of 88 specials carry no technique, and only 4 carry no other authoring either** — the Perfect Cellular Automaton's down-B is a three-pulse multihit with autolink volumes and wants no technique at all. Started at 5 plain on 2026-09-06 and stands at **2** — the brawler's haymaker gained a charge, his ground slam gained a shock, and `officer_disperse` was never plain at all. ⛔ **THAT LAST ONE WAS MY INSTRUMENT LYING.** The per-special census did not count VOLUME REACTIONS, so it reported the roster's FIRST AUTHORED WINDBOX as carrying no authoring, and I went looking for a way to improve the move. The per-fighter census in the same file had already learned that exact lesson and written it in a comment — two censuses, two definitions, one subject. ⇒ They share one now (`expressive_reasons`), which is what the disagreement above should have been resolved into rather than celebrated. ✔ **AND AT ZERO SINCE 2026-09-06.** The last two were ONE MOVE SEEN TWICE — `director_moveset` borrows the Pointed Polygon's whole table and renames the prefix, so `director_point` IS `polygon_point` — and it now carries a TIPPER, which is one edit reaching two fighters. ⭐ The engine had ranked sweetspots all along (`StrikeRank` is the move's own reading order and the strike seam takes *"the FIRST-AUTHORED volume that reaches it and no other"*); what was missing was a way to SAY it, which is the same sentence the `gust` helper carries. ⚠ **Zero is not a target and should not become one.** A plain-strike special is a legitimate design, and this row measures whether a fighter's specials say anything about that fighter — not whether every one of them has a gadget |
