...
| **Swordies** | counter | *"Swordies will get a counter."* | shipped |
...
|---|---|---|
| *"we have a lot of characters with boring specials"* / *"Many have boring specials"* | roster-wide | ⇒ the expressiveness census now stands at **18 expressive, 1 plain**; the one plain entry is `theorem_chain`, the duel arena's minimal two-hit demonstrator, which is plain deliberately |
...
| three item icons — the **mine**, the **bomb**, the **ponytail** | Jon: *"Worth three icons"*. They draw the placeholder quad today. ⛔ **BLOCKED ON A SUBMODULE, measured 2026-09-05:** the seam is main-repo (`HeldItemArtManifest`, spec id → image + display size, registered in `items/held_visuals.rs`), but the icons themselves are GENERATED, and the generator lives at `ambition_sprite2d_renderer/targets/icons/hud_icons.py` **inside the `tools/ambition_sprite2d_renderer` submodule** — the same place `hud_stock_icon.png` comes from. Adding three icons means committing in that submodule and moving its pointer, and this tree's pointer is already diverged from work I did not do. ⇒ Owner: whoever holds the sprite pipeline. Everything on the Ambition side is ready to receive them |
