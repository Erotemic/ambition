...
- **Act 2, the highway (2026-09-24).** A second, 15,040px act
  (`sanic_highway`, generated by `tools/author_highway_ldtk.py`): four loops (A,
  a back-to-back B+C, and a radius-240 D), two pits crossed by springs, an
  upside-down tunnel where a `GravityZone` points gravity up and the momentum
  solver rides the ceiling chain, a spike gauntlet, and the `velocity` score
  (its level field `music_track`). Each act's `next_room` level field names the
  other, so a clear leads on instead of replaying (`cycle_act_after_clear`), and
  an act restarts on arrival (`begin_act_on_arrival`). The goal is every act's
  `width − GOAL_FROM_END`.
  - Engine pieces it needed: an **attached loop** is one call,
    `ae::World::attach_loop` (`ambition_platformer2d_core::surface_loop`),
    replacing the speedway's hand-built graft; the LDtk `SurfaceLoop` noun reaches
    it through `attach_to` (resolved after a room's chains are assembled), so
    Act 2's loops are data. The floor is split where a loop joins it, so an
    author places a loop by centre and radius only.
  - Authoring lesson, measured: a `ReboundPad` whose impulse is mostly ALONG a
    riding body's surface is a speed booster that only raises speed, so a pit
    launcher must be a spring (normal-dominant). The first cut's diagonal
    launchers did nothing to a runner already past their speed.
  - Witnesses: `oracle_every_highway_loop_is_ridden_exactly_once_holding_up`,
    `the_highway_is_act_two_and_every_loop_on_it_is_attached_data`,
    `each_act_leads_to_the_other_and_the_session_holds_both`, and the app-level
    `the_speedway_leads_to_the_highway_and_the_highway_back` (Right held through
    both acts, the tunnel ceiling ridden, back at Act 1).
  - Follow-ups worth engine infrastructure: the act/level departure (dwell,
    re-ask, give-up, transition intent) is now written twice, in Mary-O's
    `cycle_level_on_flag_tally` and Sanic's `cycle_act_after_clear`; a shared
    "leave for `next_room`" helper would collapse them. `area create` cannot
    author `next_room` (the tool's `OPTIONAL_LEVEL_FIELDS` omits it), so both
    Sanic scripts set it with `level set-field` afterwards.
