...
- **Act 2, the highway (2026-09-24, rebuilt 09-25).** A 16,000×1,600 act
  (`sanic_highway`, generated by `tools/author_highway_ldtk.py`, whose docstring
  is the course map) with the `velocity` score (its `music_track` level field).
  The ground climbs and drops across a room twice the screen's height. Speed
  picks the road: off the kicker lip a fast runner clears the chasm onto the sky
  bridge (loop B), a slow one drops into the valley (badniks on its slopes, a
  spike dip, a spring back up). Two secrets: a roll-breakable wall in the cliff
  ...
  - Engine pieces it added: **attached loops** (`ae::World::attach_loop`,
    ...
    chain, so a hill is drawn as a hill); **`sweep_ball`** (what a small ball
    first hits, chains included, so scattered rings bounce on slopes instead of
    falling through them); **`ground_ends_ahead`** + the `turns_at_ledges`
    brain policy, and wall facts from the surface solver, so a walker on the
    surface solver paces hills and turns at pit lips; **`spawn_mode_owner`**
    (a mode owner with its session scope, mode scope, `SimId` and
    `Departure`); **`Departure`** (a level asks to leave; the engine resolves
    `next_room`, re-asks the lifecycle commit until arrival, and replays with a
  ...
  - Authoring lessons, measured: a `ReboundPad` mostly ALONG a riding body's
    ...
    `area create` lowers `Solid`/`OneWayPlatform` into the IntGrid and drops
    their names, so a block a rule finds by name (a monitor, a breakable wall)
