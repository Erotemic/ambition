...
| Initial dash | ◐ | M | E1 | MECHANISM LANDED 2026-08-25, SMASH HAS NOT ADOPTED IT. `LocomotionTuning::initial_dash_time` + `initial_dash_speed`, `0.0` everywhere so every world is byte-identical. ⭐⭐ ONE EDGE IS THE WHOLE RULE: a steer direction that DIFFERS from last tick starts the phase, which is the dash, its free reversal AND the foxtrot's re-tap; a held direction never re-triggers. ⛔ THE DASH SETS THE SPEED, it does not `approach` it — poisoned both ways. ⛔⛔ TURNING IT ON COSTS A KO: `the_stage_kills::a_second_match_on_the_same_stage_counts_in_and_ends` fails on its PREMISE at 14 frames, at 4 frames, and with the dash respecting analog magnitude — the window length does not matter, so the change is to the SHAPE of CPU movement, not its amount. ⇒ the next step is an INSTRUMENT on fighter steering, not another value. See D217. |
| Foxtrot | ▢ | S/M | E1 | ⚠ BLOCKED ON D217's ADOPTION, not on its mechanism: the re-tap is the same edge and should be free, but the phase is declared `0.0` in Smash until the KO regression is explained. |
| Dash dance | ▢ | M | E1 | ⚠ BLOCKED ON D217's ADOPTION. The reversal half is MEASURED in the kernel (`an_initial_dash_is_at_speed_at_once_and_may_still_reverse` arm 4); what is missing is a Smash that declares the phase. |
