...
| On-block cancel windows | ~ | S/M | E1 | ⭐ 2026-08-31 the CHANNEL EXISTS: `ResolvedBodyHit` means CONNECT, now carries `attacker`, is published by BOTH roads, and a blocked strike never reaches it (the `Blocked` arm returns first) — pinned by `a_blocked_hit_publishes_no_resolved_connect_and_a_landed_one_names_its_attacker`. ▢ what is left is a FEEL ruling: `mark_move_playback_landed_hits` reads the OVERLAP channel, so pointing it at the resolved one means either delaying it past the damage road (losing the connect-frame cancel window) or retracting on block. See D-CANCEL-ONBLOCK. |
...
| Feature | Status | Effort | Engine | Elegant implementation |
|---|---:|---:|---:|---|
| Right-stick smash/tilt attack mode (C-stick) | ~ | S | E1 | ✔ THE SEAM SHIPPED 2026-08-31: `AttackStrengthHint` (`Auto`/`Tilt`/`Smash`) replaces the one-way `attack_strong_hint` bool from `ControlFrame` through `ActorControl` to `resolve_attack_gesture`, and an explicit hint now overrules the stick in BOTH directions — pinned by `an_explicit_hint_overrules_the_stick_in_both_directions`, whose two arms use the same full deflection and the same flick history so only the hint differs. Rollback schema 142→143 (the hint is a byte on the wire). ▢ WHAT REMAINS IS THE DEVICE HALF: no adapter emits `Tilt` yet — bind the right stick to an attack press plus a hint, which is `ambition_input` work and touches no sim code. ⚠ the harness `Action` and the trace replayer are still strong-or-not, so a recorded tilt-stick press would collapse to `Auto`; nothing records one yet. |
...
| `P05` | On-block cancel fact | E1 | ⭐ HALF DONE 2026-08-31 — the fact is `ResolvedBodyHit` (connect, attacker-bearing, block-free), not a new one beside the landed hit. What remains is the marker's channel and the feel ruling that goes with it; see D-CANCEL-ONBLOCK. |
