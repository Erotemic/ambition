...
| Attack rebound after clang | ✔ | S/M | E1 | ✔ SHIPPED 2026-08-24, in the same slice as the clank because without it a trade is not a mechanic: two attacks vanish and both fighters stand mid-animation with nothing having happened, which reads as dropped inputs. `clank::rebound_from_clanks` ends BOTH moves through the one teardown path and throws both bodies apart along the axis BETWEEN them — not either one's facing, which is what a spinning or mid-turn body is least reliable about. A hard `recoil_lock_timer`, the footstool's lock rather than hitstun: nobody was hit, and what makes the moment matter is the frames neither can act in. `clank_rebound_speed` (Smash declares 190). |
...
| Items on/off and spawn-rate rules | ✔ | S/M | E1 | ✔ SHIPPED — `MatchParticipantRoster::item_spawns: Option<MatchItemSpawns>`, `None` for every match that does not ask (versus declares it off by name). ⭐ FOUR ways to be off and `active()` is the one place that knows them: no declaration, a zero interval, no points, or every row's weight zero. A caller checking two and missing the third would drop nothing while believing items were on. |
