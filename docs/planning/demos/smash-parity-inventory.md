...
|---|---|---|---|
| `S0.1` | **Clash and projectile priority completeness** | `ambition_combat::clank::arbitrate_attack_clanks` is on the production schedule, but Smash declares `clank_damage_window: 0.0`, so melee clanking is disabled in the shipped rules. Projectile contact does not enter the strike-volume clank arbitration. | Ship and playtest a nonzero Smash melee-clank policy, then give projectile-vs-melee and projectile-vs-projectile contact one deterministic priority vocabulary. Clankability/transcendent exceptions are authored data, never move or projectile IDs. |
...
| Projectile clash / priority arbitration | ▢ | C | E2 | **S0.1 GATE.** Projectiles currently bypass `StrikeVolume` clank arbitration. Add deterministic projectile-vs-melee and projectile-vs-projectile outcomes through shared contact/priority semantics; preserve projectile-family flight/contact ownership rather than forcing every projectile through melee move runtime. |
