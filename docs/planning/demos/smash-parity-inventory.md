...
⚠ **`S0.1` IS TWO HALVES AND ONLY ONE IS ENGINEERING.** ✅ **THE ENGINEERING HALF
IS DONE 2026-09-11**: swings and shots now arbitrate in ONE contest
(`ambition_combat::clank::resolve_clashes`, supplied by
`ambition_platformer2d_actor_monolith::clash::arbitrate_attack_clashes`), and the
receipt is on `S0.1`'s own row below. ⛔ **WHAT REMAINS IS JON'S AND IS NOT
ENGINEERING**: every shipped ruleset declares `clank_damage_window: 0.0`, so none
of it runs in a shipped room, and turning it on re-tunes the ground game — a PLAY
SESSION. `clank_damage_window: 9.0` is the number to try first.
...
Only three things may preempt this gate inside the Smash lane:

1. a correctness regression that prevents a gate row from being implemented or
   measured;
2. instrumentation required to prove a gate row on the production road; or
3. an explicit maintainer decision.

This is a **Smash-local** priority rule. It does not reorder unrelated Ambition
engine programs. Scope is deliberately narrow: the raw movement/contact/reaction
interaction between fighters. Stage layout, stage hazards, special match rules,
UI/ceremony, content breadth, and presentation polish do not count toward closing
this gate.

Verified against `1988c7c` on 2026-09-09. The nine packets below are ordered; take
the first unblocked packet before later packets, and take all nine before returning
to ordinary inventory work. The first six are shared interaction foundations; the
last three close the remaining fighter-movement/reaction/effect gaps identified by
the same raw-combat audit.

| Order | packet | current HEAD evidence | closure contract |
|---|---|---|---|
| `S0.1` | **Clash and projectile priority completeness** | ✅ **ENGINEERING HALF CLOSED 2026-09-11.** One contest for every attack family: `resolve_clashes` (pure — ordering, pairing, opposition, overlap, verdict) with `arbitrate_attack_clashes` supplying melee volumes AND live shots, scheduled ahead of `step_projectiles` so it is before BOTH damage roads. ⛔ The old bypass was TWO facts, not one missing query: the melee arbiter ran later in the tick than the projectile stepper, and the stepper takes its victims `Without<LiveProjectile>`. ⭐ A beaten shot is SPENT, not despawned, so it expires through the stepper's own road (catalog FX, `Expired` trace) rather than through a copied one — which is what makes the schedule order load-bearing. **STILL OPEN AND IT IS JON'S:** every shipped ruleset declares `clank_damage_window: 0.0`, so none of this runs in a shipped room; ship and playtest a nonzero policy (`9.0` first). Clankability/transcendent exceptions remain unauthored — and Jon's 2026-09-11 ruling says AUTHORED PRIORITY is the answer when there is one, damage the proxy when there is not, same rule for both families, which is what is implemented. |
| `S0.2` | **Grab-contact arbitration** | ⭐⭐ **ONE HALF OF THIS ROW WAS A LIVE DEFECT AND IS FIXED 2026-09-12 — a hit could not break a grab in any host but the Smash demo.** Measured: of the capture mechanic's systems, the engine compositions (`CombatSchedulePlugin`, `PlayerSchedulePlugin`, the monolith's `features/mod.rs`) install every one EXCEPT `release_interrupted_captures`, whose sole production installer was `ambition_demo_smash`'s own `SmashRulesPlugin`. ⇒ Everywhere else a hold ended only on the escape timer, a throw, or a despawned captor — **no damage to either fighter freed the captive** — while **17 movesets under `game/ambition_content/assets/data/movesets/` author `smash.capture_attempt`**. Nothing in the tree claimed that was deliberate. The rule now installs in `CombatSchedulePlugin` under the published `ambition_combat::capture::GrabInterruptionApplied` set, in `CombatSet::Settle` (the demo's own reasoning, kept: `Resolve` writes the hitstun a release must read, so anything earlier reads last tick's answer), and the demo's install is retired so it runs once. Guard: `the_shipped_engine_installs_the_grab_interruption_exactly_once`, poison-verified in BOTH directions (0 → fail, 2 → fail). ⛔ **A SYSTEM-NAME GUARD WOULD HAVE BEEN A CHECK THAT CANNOT FAIL**: `System::name()` reports `"<Enable the debug feature to see the name>"` in this build, so the first version counted zero and would have done so forever — the published set is a node the graph can be asked about instead. ⚠ **THIS RE-TUNES THE GROUND GAME AND JON HAS NOT PLAYED IT**: grabs in the main game were effectively damage-proof and now are not. · `capture::acquire_captures` uses deterministic greedy matching, so mutual same-tick grabs grant one edge. Its source comment currently claims this is Ultimate behavior; it is not the parity target. Hitbox-vs-grab outcome is also not represented as one explicit arbitration rule. | Mutual grabs cancel through a tuned grab-parry/rebound result rather than selecting a winner. Ordinary hitbox-vs-grab contact is resolved through one explicit same-frame arbitration policy, with exceptional grab-priority/armor behavior represented as data rather than fighter IDs. |
| `S0.3` | **Hit-volume target and interaction semantics** | `HitVolume` carries geometry, damage, knockback/reaction, on-hit and presentation metadata, but no victim posture eligibility; several interaction flags remain separate backlog rows. `StrikeRank` chooses one authored winner for overlapping parts. | A hit volume can state victim posture eligibility (at minimum grounded/airborne/prone) plus the remaining core interaction policy: guard/blockability and extra shield damage, clankability/transcendent behavior, per-hit hitlag/SDI and no-tech override, weight-independent/set-weight reaction, and armor bypass. Multiple same-frame parts remain independently meaningful when target/policy differs. Defaults preserve every ordinary hit. |
| `S0.4` | **Airborne capture semantics** | ⭐⭐ **SCOPED BY MEASUREMENT 2026-09-13, AND THE TWO FACTS BELOW CHANGE WHAT THIS ROW IS.** (1) **THERE IS NO AERIAL GRAB AT HEAD AND NO AUTHORED DEMAND FOR ONE:** every one of the 27 authored captures in the tree goes through `author_standing_grab`, whose own NAME is the posture — `git grep 'author_standing_grab('` returns 27 callers and there is no second constructor. ⇒ Building authored posture eligibility today is a capability with no caller, which `RootScope::rebind` records the cost of: *"a capability with no caller is not narrower than one with a caller; it is the same capability, untested."* **This row needs a customer or a ruling, exactly like `A8`.** (2) **BUT THE FACT IS RECORDED TWICE AND THAT PART IS REAL:** *"a capture requires both bodies grounded"* lives in the constructor's name AND in two hard-coded `!ground.on_ground` checks (`capture/systems.rs:133` for the captor, `:179` for the victim). They agree by DISCIPLINE. ⇒ The day somebody writes `director_aerial_grab`, the engine silently refuses it — *"a field without a lowering"*, which is the trap `A5` deleted `ChestSpec.state` to remove. ⛔ **AND THE OBVIOUS FIX IS ALREADY RULED OUT BY THIS REPO:** a `postures` field on `CaptureAttemptParams` would make all 32 literal construction sites declare a value they do not care about, which `CAPTURE_CARRY`'s own doc rejects in as many words (*"`CaptureAttemptParams` is constructed literally at 29 sites and every one of them would have had to declare a `carry: false` it does not care about"*). ⇒ The posture must ride the CONSTRUCTOR (a sibling to `author_standing_grab`) or a wire type it builds, never the authored literal — and `#[serde(default)]` keeps the 17 `.ron` movesets hydrating unchanged, their params being a flat map that simply omits the key. · `capture::acquire_captures` rejects both a captor and a candidate victim when `ground.on_ground` is false. The capture relation itself is reusable; the eligibility gate is not. | Capture requests/specs carry explicit captor/victim posture eligibility. Existing standing grabs retain today's grounded defaults, while an authored aerial command grab can capture the postures it declares without a second capture relation or fighter-ID branch. |
| `S0.5` | **Threshold armor** | ⭐⭐ **THE DAMAGE HALF IS SHIPPED 2026-09-12; THE KNOCKBACK HALF IS MEASURED AND FILED RATHER THAN GUESSED.** `BodyCombat::armored: bool` is now `ArmorPolicy` — *"suppress the reaction"* and *"suppress it for hits under this size"* were the same bit in a boolean, so threshold armor was not a feature the type held badly, it was one the type could not hold at all. `ArmorPolicy::{None, Super, Damage{breaks_at}}`, authored as the new `WindowTag::ArmorUnder{damage}` via `authoring::armor_under`. ⇒ **THE COMPATIBILITY PATH THE CONTRACT ASKS FOR IS EXACT**: plain `WindowTag::Armor` still lowers to `Super`, so all THREE shipped armoured moves (`alice_moveset.rs` `n_b`, `patent_clerk_moveset.rs` `side_b`, `player_robot_moveset.rs` `down_b`) are unedited and unchanged — a threshold FIELD on `Armor` would have made each of them author a number `serde(default)` picked for them. `>=` breaks; nothing accumulates, so two small hits never add up. Overlapping armor windows resolve to the STRONGER rather than the first authored, because order-dependence would make one pair of windows behave two ways. Rollback schema 181 → 182 with authored variant codes (code 3 RESERVED). Guards: the reaction boundary (9 absorbed / 10 breaks, and a broken-through hit matches an UNARMOURED body exactly), the authored→runtime projection WITH its number, overlap resolution, and a hand-written-codec round-trip incl. a refusal on an unknown tag — all poison-verified. ⛔ **AND AN EXISTING ASSERTION WAS SILENTLY WEAKENED BY THE MIGRATION AND PUT BACK**: `.armored` → `.armor.is_armed()` reads as the obvious port and is strictly weaker, because `is_armed()` is true for a threshold too. ⛔⛤ **THE KNOCKBACK THRESHOLD IS NOT SHIPPED AND THE REASON IS A MEASUREMENT**: `HitKnockbackMagnitude` is `FeelScale` (a victim-relative multiplier) OR `LaunchSpeed` (px/s) and exists precisely so one is not read as the other, so a bare `breaks_at: f32` would reintroduce the untyped scalar it forbids; and the one quantity both resolve to — the launch velocity — is computed BELOW the armor gate, whose early return also serves the `knockback: None` case that D203 is about. ⇒ Two roads, both real, and choosing needs a ruling; see `ArmorPolicy`'s own doc. · Move armor reached runtime as the boolean `BodyCombat::armored`; `apply_body_hit_reaction` suppresses the launch/reaction wholesale. Damage-threshold and knockback-threshold armor are still absent. | One shared armor policy supports super armor plus damage- and knockback-threshold variants. Under-threshold hits still land/pay damage as authored but suppress the reaction; over-threshold hits break through. Existing boolean armor behavior remains the default compatibility path until migrated. | <!-- cite-ok: `BodyCombat::armored` is the DELETED boolean these rows record replacing (2026-09-12, ArmorPolicy); a resolvable citation here would mean the replacement did not happen. -->
| `S0.6` | **Localized hurtbox defense** | `HurtboxVolume` contains only `shape`; whole-body invulnerability is separate. A move can change geometry over time but cannot make one limb intangible while another region remains hittable. | Hurtbox volumes gain the smallest region/defense policy needed for localized intangibility/invulnerability. Hit eligibility consumes that policy from the same resolved hurt geometry. At least one production-path fixture proves an intangible region and a simultaneously vulnerable region on the same body. |
| `S0.7` | **Character-specific movement traits** | Core movement has wall-jump/wall-cling ability bits and a nonzero crouch-speed law, but `SMASH_FIGHTER_CEILING` admits neither wall trait and Smash owns one match-wide `crouch_speed_frac: 0.0`. The current rules therefore flatten roster-specific wall movement and crawling. | Preserve character identity through Smash admission: a fighter may author/retain wall-jump, wall-cling, and crawl capability/tuning without granting it to the whole roster. Non-crawlers keep the current stopped crouch default. No fighter-ID checks in movement. |
| `S0.8` | **Special victim-reaction families** | The production reaction vocabulary covers ordinary hitstun/launch/tumble/knockdown, jab lock, teching and sleep, but the raw-combat audit found no gameplay-state implementation for bury, freeze-status, paralysis, or crumple reactions. | Add explicit authored reaction/status semantics for bury, gameplay freeze (distinct from hitlag), paralysis, and crumple. Each owns its timer/escape/posture rules through shared reaction/status authority and composes with damage, launch, capture, invulnerability, rollback, and KO without move-ID branches. |
| `S0.9` | **Remaining target/effect interaction primitives** | Smash has a homing fighter dash and a player-steered bolt, but no projectile that deterministically follows a semantic target. `OnHitEffectMessage` can carry arbitrary effect identity, but no production lifesteal/heal-on-hit consumer turns a resolved victim hit into owner recovery. | Ship a target-seeking projectile through projectile-owned deterministic target/steering state and a generic owner-heal/lifesteal on-hit effect through the resolved-hit effect seam. Both require real production-path customers/tests and stable rollback identity. |

**Gate acceptance:** a packet does not close because a type or helper exists. It
closes only when production-path tests exercise the intended interaction, the
Smash rules/content actually select the new behavior where required, rollback
identity/order is deterministic, and no character-ID branch was introduced.
Detailed rows below remain the specification for individual subfeatures; an
`S0.*` label on one of them means this gate overrides that section's normal
ordering.
...
Combat-semantics priority audit: 2026-09-09, snapshot `1988c7c`; the `S0.*`
gate above was re-read from current source rather than inferred from stale row marks.
...
The `S0.*` combat-semantics gate at the top is the first selection surface: while
it is open, lower section numbers and small-effort rows do not outrank it.
...
| Jumping | Full hop, jump squat/short hop, double jump, fast fall; core wall-jump/cling exists but Smash does not currently admit it per fighter (`S0.7`) | `ambition_platformer2d_core::movement` |
...
| Jab 1 → 2 → 3 chains | ✔ | — | E1 | jab2 and jab3 are their own moves in BOTH shipped tables (`smash_duelist_a.ron` and George's `smash_moveset.ron`, kept equal by `the_jab_string_is_one_string_in_both_tables` until AP54 lets a file import moves); a held Attack continues the string only to a successor the playing window NAMES, so a move with no chain does nothing — which is what makes it safe for a brain to hold every neutral basic. |
| Rapid jab + finisher | ✔ | — | E1 | `MoveSpec::repeat: Option<MoveLoop>` — an authored `from_s`/`to_s` jump with its own timeout, and the finisher starts where the loop's stretch ends (once `FLURRY_FROM_S` / `FLURRY_TO_S` in Rust; now the numbers in `jab3`'s windows and `repeat`). No fighter id anywhere in it. |
...
| Damage-threshold armor | ▢ | M | E1 | **S0.5 GATE.** Make armor policy carry a damage threshold now; this no longer waits for a future fighter. Keep the decision in shared hit reaction, not character code, and prove under/over-threshold controls. |
| Knockback-threshold armor | ▢ | M | E1 | **S0.5 GATE.** Same armor-policy seam; compare resolved launch/reaction, not attacker identity. Prove a launch below the threshold is absorbed and one above it breaks through. |
...
| Same-frame hitbox parts | ▢ | M | E1 | **S0.3 GATE.** Build on hitbox arbitration so several live regions can remain independently meaningful when target eligibility or interaction policy differs, rather than always collapsing to one author-order winner. |
| Victim-state hitbox eligibility | ▢ | M | E1 | **S0.3 GATE.** Add explicit per-volume target posture eligibility (at minimum grounded / airborne / prone). The filter belongs to hit eligibility, not move selection, so simultaneous boxes can legitimately target different victim states. |
...
| Weight-independent knockback | ▢ | S/M | E1 | **S0.3 GATE.** Add the compact set-weight/weight-independent reaction policy to the hit payload; keep ordinary hits unchanged. |
| Armor-bypass hit property | ▢ | S/M | E1 | **S0.3 GATE.** A hit can explicitly bypass reaction armor when the authored interaction requires it. Resolve against armor policy in hit reaction; do not identify bypass moves by character or move id. |
...
| Extra shield damage | ▢ | S/M | E1 | **S0.3 GATE.** Author shield-resource damage separately from body percent; resolve it in shield contact, not by inflating normal damage. |
| Unblockable strike flag | ▢ | S/M | E1 | **S0.3 GATE.** Add explicit runtime guard-interaction policy to the hit payload. Planner metadata such as `ignores_guard` is not the gameplay authority. Do not infer unblockable behavior from move IDs. |
| Hitbox clanking | ~ | C | E2 | **S0.1 GATE.** ◐ THE MECHANISM IS FINISHED AND PROVEN ON THE PRODUCTION ROAD; the STAGE declares it OFF pending a play session. Reopened once already for testing a surrogate — it filtered `With<HitboxLifetime>` and authored volumes carry none, so no Smash attack reached it. Now: queries `StrikeVolume`, orders by `SimId::strike_volume` (⛔ an `Entity` is an allocator identity and two peers index differently), resolves once per ATTACK PAIR (⛔ per-volume announced a 2×2 trade four times and rebounded four times), ends the losing MOVE rather than its rectangle, and only GROUNDED attacks clank — the genre's rule. ⛔⛔ TURNED ON IT RE-TUNES THE WHOLE GROUND GAME: at 9 damage two CPUs traded so constantly that `every_live_fighter_stays_inside_the_frame` measured ZERO body-frames outside the stage in a full match. `clank_damage_window: 9.0` is the number to try first. |
| Projectile clash / priority arbitration | ✅ | C | E2 | **S0.1, CLOSED 2026-09-11.** Projectile-vs-melee and projectile-vs-projectile resolve in the same contest as melee-vs-melee, ordered by a key the simulation minted (`SimId::strike_volume` for a swing, the rollback-registered `ProjectileSeq` for a shot) rather than by `Entity`, which is an allocator identity. ⭐ Projectile flight/contact ownership is PRESERVED exactly as this row asked: nothing goes through melee move runtime — the contest names a [`ClashAttack`] and each family maps it back onto its own ending. |
| Attack rebound after clang | ~ | S/M | E1 | ◐ Built and unit-tested, and it fires exactly when clanking is declared on — which the stage currently does not. `arbitrate_attack_clashes` owns ending the losing ATTACK (so the stronger-wins case, which announces nothing, ends its loser by the same road) and `rebound_from_clanks` owns the push and the hard lock. ⚠ The rebound is a BODY event and a shot has no body, so a swing that trades with a bolt rebounds nobody — the trade still ends both attacks. |
| Cannot-clank/transcendent hit | ▢ | S | E1 | **S0.3 GATE.** ⭐ UNBLOCKED and DELIBERATELY UNBUILT — it wants a customer. The arbitration exists, so this is one authored field consulted before the damage comparison; what is missing is a move that needs it. ⛔ the genre's transcendent hits are mostly PROJECTILES, and projectiles do not use `Hitbox` at all — they never reach the arbitration — so the obvious customer is already answered by the architecture. Build it the day a melee move wants to pass through a swing, the same rule D127's `when … then` form is waiting on. |
| Per-hit hitlag multiplier | ▢ | S/M | E1 | **S0.3 GATE.** Optional scalar on `HitVolume`/resolved strike. Useful for multihits and heavy impacts without changing global hitlag tuning. |
| Per-hit hitstun multiplier | ▢ | S/M | E1 | Same payload; add only when a move needs reaction distinct from knockback. |
| Per-hit SDI multiplier | ▢ | S/M | E1 | **S0.3 GATE.** Same payload; useful for multihit escape tuning. |
| Cannot-tech hit property | ◐ | S/M | E1 | **S0.3 GATE for the authored override only.** ⭐ THE ARCHITECTURE THIS ROW ASKS FOR IS ALREADY BUILT, and by a DERIVED answer rather than an authored one. `AxisManeuverState::tumble_untechable` is stamped at `launch_into_tumble` — the one place the launch speed exists — and `tick_knockdown` reads it to refuse the tech press, which is exactly "stored in the resolved launch/reaction state so the tech system owns eligibility". The source is `untechable_launch_speed` (1400 for Smash): hard hits commit, which is the genre rule and needs no per-move authoring. ⛔ WHAT IS ACTUALLY MISSING is an OVERRIDE road — a move that is untechable at any speed — and that wants a customer, like the transcendent-hit row above. Build the field the day a move needs it; do not add an authoring surface whose only value would be to restate the derived answer. |
...
| Partial-body intangibility | ▢ | C | E2 | **S0.6 GATE.** Requires hurtbox-region identity/policy. This no longer waits for a future fighter: establish the reusable per-volume policy and a production-path customer/fixture. Do not special-case sprite bones in damage code. |

## 3. Damage, launch, and impact presentation

| Feature | Status | Effort | Engine | Elegant implementation |
|---|---:|---:|---:|---|
| Bury reaction | ▢ | M/C | E1 | **S0.8 GATE.** Model bury as an authored victim reaction/status with explicit grounded/posture eligibility, escape/expiry and release behavior. It must not be faked as generic hitstun or collision embedding. |
| Gameplay freeze reaction | ▢ | M | E1 | **S0.8 GATE.** Add a victim status distinct from hitlag/match freeze, with explicit duration/break behavior and presentation facts derived from the gameplay state. |
| Paralysis reaction | ▢ | M | E1 | **S0.8 GATE.** Add a timed victim reaction/status through the same hit-effect authority; preserve deterministic input/action suppression and rollback state. |
| Crumple reaction | ▢ | M | E1 | **S0.8 GATE.** Add an authored grounded collapse/stagger reaction that transitions through the shared knockdown/recovery vocabulary rather than a move-specific timer. |
...
| Character-specific crawl / crouch walk | ~ | M | E1 | **S0.7 GATE.** The generic locomotion law already honors `crouch_speed_frac`, but Smash currently supplies one match-wide `0.0`, so every fighter is forced to stop. Preserve that as the default for non-crawlers while giving character authoring the smallest capability/tuning seam needed for a subset of fighters to crawl. The speed cap remains the movement owner; do not branch on fighter ID. |
...
| Feature | Status | Effort | Engine | Elegant implementation |
|---|---:|---:|---:|---|
| Character-specific wall jump / wall cling | ▢ | M | E1 | **S0.7 GATE.** Core `AbilitySet` already has both verbs, but `SMASH_FIGHTER_CEILING` does not permit them, so no Smash fighter can retain those authored traits. Widen admission through character-owned capability composition; do not grant either verb universally just to make one fighter work. |
...
| Aerial command grab | ▢ | M | E1 | **S0.4 GATE.** Generalize capture eligibility/posture policy for both captor and victim; do not duplicate `CapturedBy`. Existing grounded grabs keep their current default. |
...
| Grab-vs-grab cancellation | ~ | S/M | E1 | **S0.2 GATE.** The matching is deterministic, but a mutual same-tick attempt currently grants one edge. Ultimate-style parity cancels the mutual pair and gives both fighters the rebound/grab-release result with tuned minimal damage. NOTE: the current source comment saying Ultimate resolves this by port is wrong; do not preserve that comment as design evidence. Keep the no-body-is-both-captor-and-captive invariant for larger attempt graphs. |
| Hitbox-vs-grab arbitration | ▢ | M | E1 | **S0.2 GATE.** Encode same-frame strike-vs-grab outcome explicitly instead of relying on schedule accident plus later capture release. Ordinary Ultimate-style hits beat ordinary grabs; exceptional grab-priority/armor cases must be authored interaction policy, not fighter IDs. |
...
| Homing projectile | ▢ | M | E1 | **S0.9 GATE.** Projectile owns deterministic steering toward a semantic target; AI/perception may choose/resolve the target but does not move the projectile. Do not confuse the shipped homing fighter dash or player-steered bolt with a target-seeking projectile. |
...
| Heal/lifesteal on hit | ▢ | S/M | E1 | **S0.9 GATE.** A resolved victim hit emits an owner-side recovery effect through the existing on-hit message/effect seam. Prove the production consumer, cap behavior, blocked/whiff controls, and rollback determinism. |
...
| Feature | Status | Effort | Engine | Elegant implementation |
|---|---:|---:|---:|---|
| Stage-select screen | ◐ | C | — | ◐ **THE CAPABILITY SHIPPED 2026-09-04; THE SCREEN DID NOT, and the row asked for a screen.** A player can now choose a stage before a match — `SmashStageChoice` (`Flat` / `Platforms`, each naming its own room), a `SelectTarget::Stage` button on the CHARACTER select screen, and `smash_prepared_session_world` building whichever room the choice names. ⚠ So stage selection is a CONTROL on an existing screen, not a screen of its own, and this row's Class `C` was scoped to the latter. ⇒ Whether a separate screen is wanted at two stages is a design question, not a plumbing one: the seam it named (shell/experience preflight) is the one the choice already flows through. ⭐ Guarded by `a_phone_leaves_no_two_touch_targets_fighting_over_the_same_pixels`, which found the new button sits exactly at the grid line with **zero** margin — `START_H` is 34px against a 44px touch floor, so the strip's buttons are expanded and the expansion reaches the grid. |
...
| `P11` hit-grab | ◐ **RECLASSIFIED 2026-09-04 — not a missing seam, a missing SECOND CONSUMER of a working one.** The writer census above is still exactly right: `translate_authored_capture_effects` is the only thing that raises `CaptureAttemptRequested`, and a landed hit cannot reach it. ⛔ But *"plumbing"* implied the road does not exist, and it does: `LandedBodyHit` → `dispatch_landed_hit_effects` → `OnHitEffectMessage` → **`apply_pogo_bounce`**, all three scheduled in `combat_schedule.rs` (`:328`, `:329`) and the message rollback-registered. ⇒ **The landed-hit-to-authored-technique seam is built, live, and already carrying one production technique.** A hit-grab is a second reader of `OnHitEffectMessage`, which is the same shape as `P14`'s finish zoom — and the same size. ⚠ **The design fact whoever takes it must state**: this road fires from a hit that CONNECTED, so a shield blocks it — the opposite of the overlap grab, which ignores shields on purpose to keep *"attack beats grab, grab beats shield, shield beats attack"* (`a_shielding_body_is_captured_anyway`, `crates/ambition_combat/src/capture/systems.rs:871`) <!-- cite-test: names the test BY FUNCTION NAME as evidence of a deliberate design choice; the row is about what that test PINS, not about a production writer -->. Two capture roads with deliberately opposite shield behaviour is a genre-correct answer and it has to be chosen out loud. ⛔⛔ **AND THERE IS A DESIGN WRINKLE IN THE PACKET THAT "second consumer" HIDES — found by reading `acquire_captures` before writing one.** `CaptureAttemptRequested` carries an offset/half-extents BOX and no victim; acquisition catches whoever overlaps it, ordered by distance then `SimId`. **A hit-grab already knows its victim** (`OnHitEffectMessage::victim`), so routing it through the box would re-search and can catch a DIFFERENT body than the one the hit landed on — in a 4-seat match, routinely. ⇒ The honest packet therefore includes *"give the attempt an optional target"*, which is a small change to the most carefully-reasoned system in this module: `acquire_captures` resolves in two phases specifically because two same-tick grabs once made each fighter captor AND captive of the other and **deadlocked for 28% of a match** (40 captures, zero pummels, zero throws, measured in a rung-9 mirror). ⚠ **NOT taken today**, deliberately: no authored move wants a hit-grab yet, and modifying capture arbitration while the volume is at 97% and the full lane refuses to start is the wrong order to do it in. ⇒ The reclassification stands — the seam is built — and the packet is "second consumer **plus a targeted attempt**", which is bigger than the finish zoom was and still far short of new plumbing. ⓘ **Nearly published as *"nothing consumes `OnHitEffectMessage`"***, which is false: the grep that said so excluded `on_hit.rs`, the file the consumer lives in. And `capture_effects` in that module's tests is a test spy, not a capture mechanic — a false friend in the one search a reader of this row would run. |
...
| genuinely open, all needing a seam | **0** — ⚠ all three resolved 2026-09-04 by re-deriving the absence rather than building anything; the residues are authoring, a design call and a fourth customer of an existing shape. Was **1** an hour earlier and **3** that morning — ⚠ was 3 this morning. `hit-grab` moved out when its seam turned out built and already carrying a production technique; **`tech result` moved out when its fact turned out published AND SPENT** — `MovementOp::Tech` drives a sound and a particle burst today, and the row's search had covered three crates that do not contain it. ⇒ Both were found the same way and neither was a build: **re-derive the ABSENCE, because a search space is a denominator** |
...
| `P06` | Ground locomotion phase | E1 | ✔ **SHIPPED — measured 2026-09-03.** `LocomotionTuning` carries `initial_dash_time`, `turnaround_time` and `teeter_margin` (`ambition_platformer2d_core/src/abilities.rs:889-897`), and the movement authority runs the clock: `state.turnaround_timer` against `tuning.locomotion.turnaround_time` (`movement/abilities.rs:265-300`). ⚠ **The derived-move half is thin but NOT empty, and the row's own list hid that.** The three moves it names — foxtrot, dash-dance, pivot grab/smash — are absent. But a locomotion-stance-derived move DOES ship: the **running grab**, selected by `move_for_flat_verb(GRAB_VERB, grounded, running)` (`ambition_combat/src/moveset/mod.rs:3041`) and derived through `dash_stance_verb` rather than authored twice. ⇒ The seam from a locomotion fact to a derived move is proven end-to-end by one customer. ⛔ **But "the missing three are authoring, not plumbing" — which this row said until 2026-09-04 — is WRONG for the PIVOT ones, and I generalised it from the dash precedent without checking.** `move_for_flat_verb(base, grounded, running)` hardcodes exactly ONE derived stance: `if grounded && running`, try `dash_stance_verb(base)` — which is just `format!("{base}_dash")`. There is no third stance and no way to ask for one. ⇒ A **foxtrot** or **dash-dance** is authoring (both are dash-stance moves the existing branch already selects). A **pivot grab or pivot smash** needs the selector to learn a stance it cannot express: a `pivot_stance_verb`, a third argument, and the caller passing the turnaround fact. ⚠ Cheap, though — **one non-test call site** (`ambition_combat/src/moveset/mod.rs:3041`), so the signature change is contained. The point is that it IS a change, and a row that called it authoring would have sent someone to write a moveset entry that nothing could ever select. |
...
| `P09` | Shield/OOS arbitration | E1 | ✔ **SHIPPED — one owner, which is what the row asked for.** `OutOfShield` (`ambition_platformer2d_core/src/movement/abilities.rs:142`) enumerates the legal actions — `Jump`, `Burst`, `Grab`, `UpAttack`, `UpSpecial` — and move acceptance takes it as `oos_policy` (`ambition_combat/src/moveset/mod.rs:2421`) beside `rises_out_of_shield` (:2111). The vocabulary and its single consumer both exist. |
| `P10` | Complete tech surface/result vocabulary | E1 | ◐ **PARTIAL — the SURFACES shipped, the RESULT VOCABULARY did not.** All three surfaces plus the wall-tech jump have arms in `movement/tests/combat_actions.rs`: floor (`a_tech_on_the_landing_skips_the_knockdown_entirely`), wall (`a_tumbling_body_can_tech_off_a_wall`), ceiling (`a_tumbling_body_can_tech_off_a_ceiling`), the jump (`a_wall_tech_asked_to_jump_leaves_the_wall_higher_than_one_that_is_not`), plus lockout and untechable-threshold arms — eight in all, and the state lives in `movement/model.rs` (`tech_press_timer`, `tech_lockout_timer`, `tumble_untechable`). ⛔ **What is missing is the second half of the row's own title — and it splits, measured 2026-09-04.** ⇒ **The AI half is DELIBERATELY absent and must stay that way.** `SelfView` publishes `tumbling: bool` and stops there, and its own doc gives the reason: *"a body cannot know this about itself more precisely than a watcher knows it."* The sim's `tumble_untechable` is exactly the thing a watcher cannot see, and a tech press into an untechable launch **costs the option** (`movement/knockdown.rs:240`) — which is what makes teching a READ rather than a reflex. Publishing it to the brain would be a cheat, not a fix. The brain already presses correctly from what it CAN see (`reeling.rs:169` — tumbling, airborne, a floor below, closing on it). ⛔⛔ **THE "PRESENTATION HALF IS A REAL GAP" CLAIM IS WRONG — corrected 2026-09-04, and the cause is the SEARCH SPACE.** The sentence below searched `ambition_sim_view`, `ambition_render` and `ambition_vfx`. **The fact lives in neither**: `MovementOp::Tech` is declared in `ambition_platformer2d_core` and consumed in `ambition_platformer2d_actor_monolith`. ⇒ A tech publishes a fact, and that fact is **already spent on presentation** — `movement_fx.rs:345` plays `ambition_sfx::ids::PLAYER_TECH` and writes a bright `VfxMessage::Burst` (10 particles at 430 speed, near-white). It even carries a trace glyph, `"TC"` (`ambition_platformer2d_core/src/movement/ops.rs:86`), and has a test. ⇒ **Same denominator error as `P14`'s and `P11`'s: the population searched excluded where the answer lives.**
| `P11` | Capture acquisition policy | E2 | ◐ **PARTIAL, and the open half is now measured too (2026-09-03).** ✔ The arbitration shipped: `CapturedBy` (`capture/mod.rs:20`) is a real relationship with `MapEntities`, and `acquire_captures` (`capture/systems.rs:70`) is the single acquisition system. ⭐ **There is exactly ONE production writer of `CaptureAttemptRequested`** — `translate_authored_capture_effects` (`crates/ambition_combat/src/capture/systems.rs:2818`), which turns an authored `Special` carrying the `smash.capture_attempt` key into the typed request. So the row's *"six roads"* are not six code paths to audit; they are six AUTHORED MOVES that would all share the one key, and the audit is a count of what is authored. ⇒ **Authored today: 2 of 6.** `GRAB_VERB` (standing) and `GRAB_DASH_VERB` (running) — George answers both — and **neither id is AUTHORED in that file**, which the row used to obscure by citing a line inside its test module. `grab` is a VERB that resolves through the contract's verb map (`george_booul_moveset.rs:127`), and the running form is derived from the standing one by the capture kit. The evidence that both ids exist is the fighter's own test (`george_booul_moveset.rs:908` <!-- cite-test: a `#[cfg(test)]` line cited ON PURPOSE — it is the assertion that PINS the verb map's output, and this file authors neither id -->), and the running form is DERIVED by `dash_stance_verb` rather than spelled twice, with a contract that authors no running grab left untouched. **Aerial is answered by fallback, not authoring** (*"an ungated grab must still answer an airborne press"*, `ambition_entity_catalog/src/tests.rs:462`). **Absent: pivot, command grab, tether, hit-grab.** ⛔ **"The seam is finished and cheap to extend — a new road is an authored move plus the existing key, not new plumbing" was this row's own conclusion until 2026-09-04, and it holds for ONE of the four.** Measured per road: **command grab — authoring only** ✔ (an authored `Special` carrying `smash.capture_attempt`; the seam really is finished for this one). **pivot grab — PLUMBING**: `move_for_flat_verb` hardcodes one derived stance (`if grounded && running` → `{base}_dash`) and cannot express a pivot; see `P06`. **tether — ◐ RECLASSIFIED 2026-09-04, and the framing was the problem.** The grapple observation is true and it is about the wrong mechanic: `grapple_system` casts at a SOLID SURFACE and yanks the caster toward it (`crates/ambition_abilities/src/traversal/grapple.rs`) — a traversal ability, not a body-catcher. Connecting it to capture would mean changing what it does. ⭐ **But a tether grab does not need it.** `CaptureAttemptRequested` already carries `offset` and `half_extents` — a reach box — and **nothing anywhere bounds them**: not the params schema, not `acquire_captures`, not the content pass. The three authored grabs use `(10,12)`, `(18,15)` and `(12,14)` because that is what a hand reaches, not because anything says so. ⇒ **The capture half of a tether is AUTHORING**, exactly like the command grab, and acquisition's distance-then-`SimId` ordering is already the right arbitration for a long reach. ⭐ **And the LINE half has a documented precedent with three customers**: `ambition_render/src/rendering/flyline.rs` — *"a rope from a point in the flies down to a body on it"* — a procedural per-body visual with a lifecycle, deliberately kept in the same shape as `submerged.rs` and `morph_ball.rs`, whose own note says a generalisation *"is better with three examples than with one"*. A tether line is the fourth. ⇒ So the tether is **authoring plus a fourth customer of an established shape**, not a missing seam. ⛔⛔ **AND THE UNBOUNDED REACH IS A REAL HAZARD THIS ROW SHOULD CARRY**: a typo in authored `half_extents` is a grab that catches across the stage, and there is no schema bound, no clamp and no test that would say so. Whoever authors the tether should add the bound in the same pass, because the tether is the first move that makes a large number LOOK correct. ✔ **BOUND ADDED THE SAME DAY** — `no_authored_grab_reaches_further_than_the_stage_allows` censuses every authored `smash.capture_attempt` across both movesets and refuses a reach past **96px**, a fifth of the shipped 480px platform, against a longest-authored 62. ⭐ **A CEILING, NOT A CLAMP**: clamping in the engine would silently truncate a deliberate long reach, which is worse than refusing it — a real tether RAISES the number in the commit that authors it, which is the ratchet working. ⓘ It carries a population floor (≥3 captures) because its healthy answer is "no offender" and a census that walked nothing answers identically. Poison-verified both ways: a lost decimal (`28` → `280`) reports *"reaches 314px, past the 96px ceiling"*, and moving the effect key reports *"measuring nothing rather than passing"*. **hit-grab — ◐ RECLASSIFIED 2026-09-04**: nothing raises `CaptureAttemptRequested` from a landed hit, and that is true — but the landed-hit-to-technique SEAM exists, is scheduled, and already carries `apply_pogo_bounce`. It needs a second consumer of `OnHitEffectMessage`, not new plumbing; see this row's own hit-grab line above for the shield consequence that comes with it. ⇒ So the cheap slice off this row is the command grab, and the other three each need a seam before an authored move could reach them. |
| `P12` | Recovery-use budget | E1 | ✔ **SHIPPED — and the row's hedge (*"only if real play needs it"*) was already spent; play needed it.** `AxisLocomotion::recovery_charges: u8` (`ambition_platformer2d_core/src/body_clusters.rs:348`), spent at `moveset/mod.rs:2484` (`saturating_sub(1)`) and refreshed to `ae::DEFAULT_RECOVERY_CHARGES` on the landing road (`hit_reaction.rs:280`). ⭐ It is **an integer, not a flag** — the field's own comment: *"The genre's default is one, and a winged fighter authoring two is an ordinary tuning statement rather than a second mechanic"*, which is precisely the 0/1/N the row asked for. ⭐⭐ It also carries a derived-vs-stored ruling worth reading before touching it: **helplessness is a stored EPISODE, not `recovery_charges == 0` derived** — because a refresh is landing-shaped, and an accepted hit that hands the dodge back must not also end the helpless episode. |
| `P13` | Participant-generic item action path | E2 | ✔ **SHIPPED — `DrivenBodies` is exactly this primitive.** (`ambition_held_items/src/lib.rs:1041`, a `SystemParam` used by every wielded ability: `volley.rs:91`, `beam.rs:85`, `meteor.rs:88`.) Its own comment states the row's premise — *"EVERY DRIVEN BODY, not the one the primary seat happens to hold. `ControlledSubject` is singular by construction, so a possessed body or a second seat holding the same gauntlet simply never fired."* It answers from the seat half when no possession road is registered, so a composition without one is an ordinary answer rather than a panic. |
| `P14` | Resolved presentation facts/events | E1 | ✔ **SHIPPED for four of its five named facts, plus a fifth it did not name — measured 2026-09-03 against `ambition_sim_view`, the read-model presentation actually consults, searching by CONCEPT rather than by the row's spelling.** **charge** → `smash_charge_fraction`, `charge_tier`; **unhittable** → `unhittable`; **launch beat** → `launch_beat_secs`, and `the_front_of_a_launch_is_published_apart_from_the_tumble` names the split as a test; **shield-break phase** → `rebuild_guard_breaks_view` (`crates/ambition_sim_view/src/pose_view.rs`) reading `BodyShieldState::break_phase`. ⛔ **Unnamed by the row but published anyway — and I cited a GHOST for it.** The knockout beat with its place IS published, as `KnockoutBeatRequested` (`ambition_vfx/src/vfx.rs:365`), an effect INTENT that rides the confirmed-effect quarantine with the sfx and camera shake beside it. I originally cited `crates/ambition_sim_view/src/pose_view.rs`, which is a doc comment and nothing else: `KnockoutsView` was a rebuilt read-model **deleted** at `1250209d3` precisely because a resource refilled per simulation advance and sampled per render frame runs on two clocks — a rollback host advances several times per frame, so an intermediate knockout was erased before presentation saw it. The struct went; its doc block stayed, and drifted on top of `LaunchedBodyFact`, where it read as that type's documentation. ⇒ **The claim survived and its evidence did not**, which is only visible if you try to build on the citation. Orphan removed. ⛔⛔ **AND THAT "one genuine gap" WAS WRONG TOO — corrected 2026-09-04, by the same rule that produced the two corrections above it.** The row said *"no fact anywhere says **this blow is the finishing one**"*. Two do. **`FighterStockSpent::eliminated`** (`crates/ambition_combat/src/stocks.rs:119`) is documented in place as *"`true` when that was the last: this fighter is out"*, and it is **not** a presentation-side inference — the beat message repeats the point: *"The simulation's own answer, never a comparison of remaining against zero on the presentation side."* It is published as **`KnockoutBeatRequested::eliminated`**, riding the confirmed-effect quarantine with the sfx and the shake. And the match-level half exists too: **`MatchVerdict::Winner(side)`** (`crates/ambition_combat/src/stocks.rs:550`). ⭐ **The fact is not merely present, it is already SPENT on presentation**: `knockout_beat(eliminated, flight_speed)` (`crates/ambition_render/src/rendering/knockout.rs:134`) branches on it today, drawing `ELIMINATION_SPARKS` / `ELIMINATION_RGBA` / `ELIMINATION_RING_SCALE` instead of the ordinary stock beat. ⇒ **So the missing thing is not a FACT, it is a WIRE**: nothing routes an established, rollback-correct, already-consumed gameplay fact to the camera, whose zoom is still driven only by authored `CameraZoneSpec`. ⚠ **With one correction to "just a wire", found after that sentence was first written:** the camera scale is **zoom-OUT-only** — `effective_zoom` is `.max(1.0)` (`crates/ambition_platformer2d_world/src/rooms/camera.rs:110`) and `camera_snapshot.rs` floors `target_scale` at `1.0` again — because *"the view is a FLOOR, so authored zoom still wins whenever it is already wider"*, a readability guarantee. ⭐ The architecture already answers it: the shake this fact rides beside is **presentation-only, applied downstream of the snapshot**, and a finish zoom is the same kind of thing. It belongs there, not in `zoom_multiplier`. Full packet in `queue.md`'s **D72**. That is a much smaller and much better-defined piece of work than "invent the gameplay fact", and it was hidden behind a sentence that described the harder job. ⚠ **One precision to carry into that work:** `eliminated` is per-FIGHTER — their last stock — which in a two-fighter match is the finishing blow and in a larger one is not. A finish zoom keyed to it alone would fire on every elimination in a 4-player match; the match-level question is `MatchVerdict`'s. ⓘ This is the third correction on this row, all one shape and all recorded below: the row's own spelling (`finish_zoom`) is not the codebase's, and searching for the ABSENCE of a fact is exactly where that failure is invisible. ✔✔ **AND THE WIRE IS SHIPPED, 2026-09-04 — `P14` now has every named fact and both it did not name.** `FinishZoomRequest` / `FinishZoomState` / `FinishZoomTuning` sit beside the shake in `ambition_platformer2d_shared_tangle::camera_ease`; `ambition_combat::finish_zoom::zoom_camera_on_decided_match` produces the intent; `camera_follow` multiplies the PRESENTED orthographic scale by it, never `zoom_multiplier`. ⭐ **Keyed to the match VERDICT rather than to `eliminated`** — `MatchVerdict::winner()`, so a `Draw` and a `NoContest` get no victory zoom and the trigger is player-count-agnostic where the per-fighter fact is not, which is the precision the paragraph above asked to be carried into the work. ⛔ Rollback-registered as `message.finish_zoom_request` and quarantined, because a finishing zoom on a speculative frame tells the player the match is over while it is still running; the arm is `a_match_decided_on_an_abandoned_branch_never_zooms`. ⚠ **The camera's new inputs are REQUIRED `Res`, and the first version made them `Option` — reversed the same day.** Taking them as required turned an `ambition_render` fixture red on the first run, and `Option` was the wrong lesson to draw from that: the select screen's stage button had already ruled on this exact question when its `ResMut` reddened 28 tests — *"an Option would have kept the fixtures green and silently no-opped the stage button in production if the real registration were ever dropped."* ⭐ And the usual counter-argument is dead here specifically: `camera_follow` already takes `CameraShakeState` as a required `Res`, so a host that would panic on a missing `FinishZoomState` is one that already panics on the shake. The Option bought nothing in production and cost the protection against a silent no-op. The fixture registers both, exactly as it registers the shake. |

⚠ **`P10`'s TWO ASIDES, moved here 2026-09-06 because they sat BETWEEN rows P10
and P11 and broke the table** — a prose line inside a markdown table ends it, so
`P11`–`P14` were rendering as a headerless block. Text unchanged:

⭐ **What IS true, and is the precise residue the row's own title asks for — *"result VOCABULARY"*: all three tech surfaces emit ONE variant.** Wall (`knockdown.rs:278`), ceiling (`:298`) and floor/roll (`:343`) each write `MovementOp::Tech`, so presentation cannot tell them apart: one sound and one burst for three different maneuvers. ⚠ **Not asserted as a defect.** The FX module's own comment reads *"a tech is the crisp recovery that refused it"* — one beat for the concept — which is a legible design choice, not an oversight. ⇒ Whether a wall tech should sound different from a ceiling tech is a FEEL decision; what this row can say is that the vocabulary is **undifferentiated by construction**, and no amount of authoring changes that without a wider `MovementOp`.
ⓘ **The superseded sentence follows, kept because its search is the lesson.** Nothing publishes *"a tech happened here"*: a workspace-wide search of `ambition_sim_view`, `ambition_render` and `ambition_vfx` for a tech-shaped fact returns one hit, and it is a doc comment listing `tech_roll` among pose names. So the pose exists and no fact drives it — the same shape as `P14`'s finish-zoom eligibility — ⚠ **except that `P14`'s half of that comparison was itself corrected 2026-09-04**: the finishing-blow fact DOES exist (`FighterStockSpent::eliminated`) and is already spent on the knockout beat, so `P14` is a missing WIRE and this row is a genuinely missing FACT. ⇒ The two are no longer the same shape, and `P10` is now the harder of the pair. |
...
>
> ⛔⛔ **A THIRD SHAPE, AND `check_planning_citations.py` CANNOT SEE IT: a citation
> that RESOLVES and is still a ghost.** The `P14` row cited **pose_view.rs, line
> 609** (spelled out here rather than as a citation, so the checker does not chase
> the very thing this note is about) for a published knockout fact. That line existed,
> the checker passed, and there was no such type — `KnockoutsView` was **deleted**
> at `1250209d3` and its doc block left behind, drifting on top of
> `LaunchedBodyFact` where it read as that struct's documentation. I cited prose
> describing a type removed for good reasons (a resource refilled per simulation
> advance and sampled per render frame runs on two clocks, so a rollback host
> erased intermediate knockouts before presentation saw them). The CLAIM happened
> to be true — the beat is published as `KnockoutBeatRequested` in
> `ambition_vfx` — but its evidence was a headstone.
>
> ⇒ **The checker proves a path and a line number exist. It cannot prove the line
> says what the prose claims.** A citation landing in a doc COMMENT deserves more
> suspicion than one landing on a declaration: a deleted item's documentation
> outlives it silently and nothing in the build objects.
>
> ⛔⛔ **A FOURTH SHAPE, AND IT IS THE ONE I REPEATED MOST: generalising from the
> one example that happened to be easy.** Four rows on this page carried a claim
> about a whole set that I had verified on a single member:
>
> - `P01` — "one authored move uses charge", checked on the rare STORED charge
>   while every smash attack charges by default.
> - `P06` — "the missing three are authoring, not plumbing", checked on the dash
>   grab; the pivot moves need a stance the selector cannot express.
> - `P11` — "a new road is an authored move plus the existing key", true of the
>   command grab and of none of the other three.
> - `P14` — the census that was right about `Dodge` and stale about `Shield`.
>
> ⇒ Each was a true sentence about one member offered as a property of the set.
> **Before writing "the rest are X", check the member that looks LEAST like the
> one you checked** — the pivot grab, not the second dash move. The easy example
> is the one that volunteers itself, which is exactly why it is unrepresentative.
...
The `S0.*` gate at the top of this file overrides every ordinary Smash ordering
while it is open. The current order is:

1. `S0.1` clash and projectile priority completeness;
2. `S0.2` grab-contact arbitration;
3. `S0.3` hit-volume target and interaction semantics;
4. `S0.4` airborne capture semantics;
5. `S0.5` threshold armor;
6. `S0.6` localized hurtbox defense;
7. `S0.7` character-specific movement traits;
8. `S0.8` special victim-reaction families;
9. `S0.9` remaining target/effect interaction primitives.

Do not skip to stage breadth, match completeness, roster expansion, character-
specific primitive breadth, presentation polish, items, training tooling, or CPU
adoption while an earlier unblocked `S0.*` packet remains. A small lower row is
not higher priority because it is cheap. A shipped-but-unreached mechanic is not
higher priority because it is frustrating. Finish the raw fighter-interaction
vocabulary first.

After all nine packets close, re-measure the remaining inventory against HEAD and
write a new post-gate order from the rows that are still actually open. Do not
restore the historical order that used to live here; several of its first items
have already shipped, and Git retains that chronology.
...
to George's table's (`smash_moveset.ron`) **26**.
