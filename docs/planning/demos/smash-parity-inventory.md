...
| `P11` | Capture acquisition policy | E1 | ◐ **THE RELATIONSHIP AND ITS ONE OWNER SHIPPED; THE SIX ROADS ARE THE OPEN HALF.** `CapturedBy` (`ambition_combat/src/capture/mod.rs:20`) is a real relationship with `MapEntities`, and `acquire_captures` (`capture/systems.rs:70`) is the single acquisition system — every capture arrives through one `CaptureAttemptRequested` channel, which is the arbitration the row asked for. ⚠ What is unmeasured is whether all six roads the row lists (standing/running/pivot, command, aerial, tether, hit-grab) actually raise that attempt; the mechanism cannot tell you how many callers it has. |
