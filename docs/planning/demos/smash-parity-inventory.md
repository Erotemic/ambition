...
| customer | cost |
|---|---|
| `P11` command grab | ✔ **DONE 2026-09-04** — authored, no engine change, guard poison-verified five ways, ⚠ **and its in-play verification is WEAKER than I first wrote it.** `capture-probe` recorded 3 `lunge_grab` starts in 90s — **on the ENGINE FLOOR**, because I ran it before the probe learned `--ladder`. On the SHIPPED rows it started **zero times in 120s**. ⇒ So "measured in play" was measured on a configuration no player meets, which is the exact error this page documents five times over, committed by me while verifying my own move. A longer shipped-ladder sample is running; until it lands, the honest status is *reachable on the floor, unconfirmed on the shipped ladder.* |
