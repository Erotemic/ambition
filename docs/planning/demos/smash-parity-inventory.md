...
| Hitbox clanking | ▢ | C | E2 | ⛔⛔ REOPENED 2026-08-24 — IT WAS NEVER REACHING AUTHORED MOVES. `arbitrate_attack_clanks` queries `With<HitboxLifetime>`, and `advance_move_playback` spawns authored volumes with a comment saying *"NO `HitboxLifetime` on purpose"* — so every Smash jab/tilt/smash/aerial was invisible to it, and the tests that passed spawned synthetic boxes carrying the component production refuses. ⛔ do NOT fix by swapping the query filter: arbitration must order by `SimId::strike_volume` (not allocator `Entity`), resolve once per ATTACK PAIR (not per volume, or a 2-volume attack rebounds twice), and a losing attack must not continue through its sibling and later volumes. See `triage/gpt-review-2026-08-24-correction-pass.md`. |
| Attack rebound after clang | ▢ | S/M | E1 | ⛔ REOPENED with the row above — it consumes a clank that authored moves never produce. The rebound itself (end both moves, throw both apart, hard lock) is written and unit-tested; what is missing is that it never fires in a real match. |
