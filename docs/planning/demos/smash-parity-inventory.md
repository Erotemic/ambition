...
| Pivot grab | ▢ | S | — | ⭐ UNBLOCKED 2026-08-25: `BodyMotionFacts::turning_around` now exists, which is the fact this row was waiting on. `grab_dash` already ships; what remains is selecting the pivot grab from that fact. |
...
| Reverse aerial rush | ▢ | S/M | — | ⭐ UNBLOCKED 2026-08-25: turnaround timing exists and is published. ⚠ MEASURE BEFORE BUILDING — the row says it should EMERGE from turnaround → jump → back-air, so the first step is driving that sequence and seeing what comes out, not adding an RAR state. |
