...
| `S0.4` | **Airborne capture semantics** | `capture::acquire_captures` rejects both a captor and a candidate victim when `ground.on_ground` is false. The capture relation itself is reusable; the eligibility gate is not. | Capture requests/specs carry explicit captor/victim posture eligibility. Existing standing grabs retain today's grounded defaults, while an authored aerial command grab can capture the postures it declares without a second capture relation or fighter-ID branch. |
