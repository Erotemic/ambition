...
| Foxtrot | ▢ | S/M | E1 | ⭐ UNBLOCKED 2026-08-25 and LIKELY ALREADY TRUE: the re-tap is the same edge that starts the phase. ⚠ MEASURE BEFORE BUILDING — the only open question is whether a re-tap through neutral registers at 60Hz. |
| Dash dance | ▢ | M | E1 | ⭐ UNBLOCKED 2026-08-25 and the reversal half is MEASURED (`an_initial_dash_is_at_speed_at_once_and_may_still_reverse` arm 4: reversing inside the window is instant). ⚠ MEASURE BEFORE BUILDING — what may remain is only the presentation/animation phase. |
