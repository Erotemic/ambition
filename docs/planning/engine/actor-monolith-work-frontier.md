...
|---|---|---|
| **A11a** | ✔ landed | `TechniqueSupport` is declared by whoever installs the handler (*"A KEY PRESENT HERE MEANS A CAPABILITY SAID 'I install the thing that answers this'"*); `declare` *"refuses a second claim on one key instead of replacing it"*, which is the duplicate policy this packet demands instead of last-write-wins. The composition's table is `InstalledTechniques`. **The refusal kinds this packet lists, checked one at a time against `admit_at`:** duplicate → `declare`; unknown → `TechniqueRefusal::Unknown`; unsupported-site → `WrongSite`; invalid parameters → `BadParams` (including non-finite, *"NaN and infinity are valid RON and hydrate cleanly"*); paramless-and-canonical-empty → `TechniqueParams::None` + `UnexpectedParams`. ⚠ **`disabled` has no variant**, and the reading is that it cannot: a key is in the table IFF something installed its handler, so a disabled capability is `Unknown` by construction. That is a reading of the shape, not a refusal anyone demonstrated |
