...
| `measure_foreign_system_ordering.py` | 0 capability/ruleset private orderings; 73 composition; 174 foreign installs | Locator classification, not phase visibility or ownership acceptance. ⚠ **Re-run 2026-09-17 and unchanged when re-run again 2026-09-18: 10 / 68 / 217 — and the comparison is NOT the obvious one.** The classifier was rewritten since the baseline, so part of that change is the instrument widening rather than the code moving. The baseline script was checked out and run against today's tree as the control; both readings and the reconciliation live in [capability-and-runtime-composition.md](capability-and-runtime-composition.md), which owns them. ⇒ Do not quote 68 against 73 |
| `measure_carveable_installations.py` | 3 reducible / 38 irreducible | Current crate-identity heuristic, not permission to move every reducible block. ⚠ **Re-run 2026-09-18: 5 reducible / 35 irreducible.** Two blocks moved from irreducible to reducible and one appeared. ⛤ Nothing ran this between the baseline and that date, and this row and [capability-and-runtime-composition.md](capability-and-runtime-composition.md) both stated the stale pair — the instrument is unchanged, so unlike the row above this IS a change in the code |
