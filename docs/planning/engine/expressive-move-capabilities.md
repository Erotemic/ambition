...
⛔⛔ **MEASURED BOUNDARY (2026-09-05): A FLOW GETS EXACTLY ONE CONTACT DECISION
PER OCCURRENCE, SO A MULTI-BEAT COMBO CONFIRM IS NOT AUTHORABLE.**
`MovePlayback`'s `landed_hit` / `connected_hit` / `blocked_hit` are per-OCCURRENCE
LATCHES — set true by the strike seam and by `mark_move_playback_resolved_hits`,
and cleared by nothing until the move ends — and `contact()` derives `overlapped`
from all three. ⇒ Once anything has been touched, **every later
`Wait { on: Overlapped }` in the same flow is already satisfied** and falls
through on the tick it is reached.

⭐ **THE SHAPE THIS FORBIDS IS A REAL ONE**: *"hit, then hit AGAIN, then finish"*
— the homing-attack chain, the rekka, the confirm-into-confirm. Authoring it needs
per-BEAT contact and the signal vocabulary is per-occurrence. ⓘ Both flows the
roster ships (the oni's side-B, the goblin's) are single-decision, and that is a
CONSEQUENCE of this rather than a coincidence of taste — which is the useful part,
because it means the two shipped customers do not demonstrate the limit.

⚠ **PINNED, NOT ENDORSED**, by `a_second_wait_on_contact_passes_on_the_first_hit`
in `ambition_combat::moveset::tests::technique_flow`, whose whiff arm is what
makes it a measurement rather than a description. ⇒ If per-beat contact ever
lands, that test SHOULD fail, and its failure is the reminder that this note,
`FlowSignal`'s doc and `TechniqueFlow::problems()` all describe the old rule.

⛔ **AND IT IS NOT OBVIOUSLY THE NEXT RUNG.** Per-beat contact means either
clearing the latches at some boundary — which makes `MovePlayback` the authority
for a thing other systems read, the exact inversion this page's own rule forbids —
or a second, beat-scoped signal beside the occurrence-scoped one, which grows the
vocabulary O3's gate deliberately kept small. ⇒ It wants a real customer first:
the shape is only worth its cost if a fighter the roster actually wants cannot be
authored without it.

✔✔ **AND THAT QUESTION IS NOW ANSWERED: NO. THE COMBO CONFIRM ALREADY HAS A
ROAD, AND IT IS NOT THE FLOW.** `CancelCondition::OnHit`'s own doc names this
exact shape — *"combo confirm — jab chains into jab2 on hit"* — beside `OnWhiff`
(the whiff escape) and `OnBlock` (the safe-on-block follow-up). ⛔ Measured
2026-09-05: **all three had ZERO customers across the whole roster**, and only one
fighter authored a `Cancelable` window at all. So the capability was shipped,
tested at the engine seam (`a_follow_up_press_walks_the_authored_jab_chain`), and
never asked for by any content.

⭐ **THE DIVISION OF LABOUR THAT FALLS OUT OF THIS IS THE USEFUL PART, and it
retires the rung rather than deferring it:**

| the shape | its road | why |
|---|---|---|
| an unconditional multi-hit | WINDOWS on one timeline | `theorem_chain`'s doc: a combo that needed two moves and a cancel *"would prove the runtime can chain presses, not that a move can be a combo"* |
| a confirmed follow-up the PLAYER presses | `Cancelable` + `CancelCondition` | it is a read, and a read needs an input — the rekka's second press IS the mechanic |
| an AUTOMATIC consequence nobody presses | `TechniqueFlow` | the one decision neither of the above can make |

⇒ Per-beat flow contact would duplicate the middle row while removing the input
that makes it a read. **The flow's one-decision limit is a boundary, not a gap.**

ⓘ The roster now exercises the middle row: Pugnacious Polygon's jab confirms into
`polygon_brawler_jab2` on hit and owes its recovery when blocked, held by
`every_cancel_target_resolves_and_a_confirm_is_authored`.
...
| **Persistent traps** ✔ | C4, mine, Lloid, planted bomb | bomb/item/projectile lifetimes | ⭐ **SHIPPED 2026-09-05 AND THE "MISSING" COLUMN WAS WRONG.** This row asked for *stable owner/source/tag identity and lookup of owned occurrences*, and the mine needed none of it: `MatchSeat` is already rollback-registered and is already how this codebase names a fighter durably, so "my mine" is a `usize` comparison. ⇒ What a trap actually needs is an owner it can NAME, and for a 1v1 match that owner is a SEAT. A general occurrence-identity scheme is still owed to the steerable projectile, but not to this | Owning entity domain |
...
| **Reactive defense** ✔ | counters, Revenge, Witch Time | parry already supplies "qualifying contact is denied" | ⭐⭐ **SHIPPED (`ParriedBodyHit` + `smash.counter`), and it is more general than this row asked.** The consequence is an ARBITRARY TECHNIQUE, not a reaction type: `answer_a_parry_with_the_authored_counter` dispatches `Special(stance.response)`. Three fighters, three unlike moves — a grab, an ambush teleport, a sleep pulse. ⇒ "Counter" is a TRIGGER | Combat |
...
| **Area force / field** ◐ | windbox, vacuum, gravity pull, poison | ⭐ **MORE THAN "partly": `VolumeReaction::Windbox` is complete down to a `WindboxWithDamage` validation error, and `hit_reaction` sets `flinchless` from it. It had ZERO authored customers until the Officer's neutral on 2026-09-05** | ▢ The move-scoped gust is shipped (`ambition_entity_catalog::authoring::gust`); what is still open is a persistent field emitter, once a move needs lifetime/periodic application | Combat/world |
| **Input delegation** ◐ | PK Thunder, Nikita, menu specials | ⭐ **STRONGER THAN THIS ROW SAID, read 2026-09-05.** `DrivingParticipant` is the authoritative per-body driver identity and rollback state; `ActorControl` is the per-tick control frame, kept off `Brain` *"precisely so a brain swap cannot disturb the frame"*; `TemporaryControl` masks a body's autonomous brain and names the controller by stable `SimId`, with `Player` and `Mounted` variants shipped. Combat's `causal::seat_of` already attributes a driven body's hits to its seat | ⛔ NOT the lease itself — **a per-seat driver of it that a move can operate**. The only existing driver is `PossessionState`: one seat, hard-wired to `PlayerSlot::PRIMARY`, and by its own comment inert in a versus match. ⚠ A move must FEED `project_driving_participant`, never write `DrivingParticipant` — that system is the documented sole writer and its comment names the two-writer failure it exists to prevent | Input/control authority |
...
| **Self-interaction** ✔ | PK Thunder hitting its owner, recoil shots | ownership/friendly-hit filtering | ⭐ **SHIPPED with the bolt, and the row's caution was right**: the eligibility is the MOVE's, never a global weakening — the bolt owns a `clear_of_caster` latch and no owner-exclusion rule was touched. ⛔ The latch is not decoration: without it the bolt came home on the tick it spawned, because **a spawn point is inside the spawner** | Projectile/combat |
