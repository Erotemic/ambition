...
**Status:** SR-8 is built. The 3D 2+1D minimap is on by default. TwinTrack
recenters its teaching exhibits around the laboratory, authors no perimeter
collision, and uses an unclamped follow-camera zone plus a deep zero-gravity
blast margin, so a body that leaves the authored rectangle is not pinned or
reset. This is game-authored and adds no relativity cost to other experiences.
Open: visible-feel validation, which no headless gate can answer.
...
- `ambition_relativity` owns dimension-independent Minkowski interval,
  ...
  The policy row `engine.ambition_relativity-stays-engine-free` forbids `bevy`
  and `ambition_` in this crate's source. This keeps the mathematics
  dimension-independent, so `ambition_relativity2d` is the only 2D-aware layer
  and a later GR provider can sit beside it.
...
Only `ambition_demo_twintrack_app` links the relativity crates. Re-derive per
app with `cargo tree -p <app> | grep -c "ambition_relativity v"`; every other
