...
- provider authoring still duplicates activation/host boilerplate across the
  three customers (P0);
- input acceptance covers raw keyboard AND controller launcher nav/confirm
  (unit-tested against `ButtonInput<KeyCode>` and a spawned `Gamepad`) unified
  through one neutral `(up, down, confirm)` edge mapping, plus the F10/Start
  Quit-to-Home binding; launcher row counts in the test derive from registered
  entries + host actions, not a literal;
...
- startup vanity sequence (B0) is DONE (opt-in "Powered by Ambition" card →
  launcher); loading activity (B1) remains OPEN;
...
| W0 | PARTIAL→ (2026-07-14: `ActiveGameplaySession` now REFERENCES the active world — `GameplaySessionInstance.world: Option<SessionWorldRef>` (a type-erased `Arc<dyn Any>` the engine holds without naming any provider's world type); all three providers attach their prepared `RoomSet` on activation, and `active_world()`/`active_world_as::<T>()` expose it. This makes "no active gameplay-world authority at a frontend route" STRUCTURAL, not merely gated: the session is the world reference-holder and there is no session at home. Proven by `assert_home` asserting `active_world().is_none()`, `assert_in_game` asserting the session's `active_world_as::<RoomSet>()` names THIS session's active room (not a prior provider's resident room), and game_shell unit tests `session_world_ref_downcasts_to_the_attached_type_only`, `active_world_is_absent_without_a_session_and_present_after_attach`, `a_fresh_session_world_is_a_distinct_reference_from_the_prior`. HONEST CAVEAT: the ~30 `Res<RoomGeometry>`/~13 `Res<RoomSet>` consumers still read the resident projection (the session-held `Arc` is the authority anchor, the resident resources are its published projection); migrating consumers behind a `SessionWorld` SystemParam — starting with the `WorldCollision` param that already centralizes geometry — remains OPEN, as does folding geometry/metadata/character into the referenced bundle. Identity + load-authorization + world-REFERENCE authority is canonical; world-CONSUMPTION migration is not. | One canonical App-local active gameplay-session representation owns current world/provider/session/load authority. |
...
| B0 | DONE (2026-07-14: `compose_ambition_startup_sequence` composes a "Powered by Ambition" vanity card via the EXISTING shell sequence mechanism — a `ShellSequenceCatalog` entry keyed by a plain startup experience, a route whose `on_complete` is `GoTo(launcher)`, and the startup route as the initial one; one text card, auto-advancing (2s) and skippable via confirm. The production windowed binary opts in (`cli::run`); `--direct` and the headless ownership tests boot straight to the launcher. Proven by `shell_host_startup::startup_card_plays_then_hands_off_to_the_launcher` (boot on the startup route with the sequence running and NO gameplay session; skip → exactly one launcher authority, still no gameplay session, sequence cleaned up) and `without_the_startup_composition_boot_bypasses_straight_to_the_launcher`) | Startup sequence hands off to launcher while direct route entry remains available. |
