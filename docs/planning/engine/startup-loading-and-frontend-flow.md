...
The candidate intentionally does not attempt the remaining large refactors
without compiler feedback:

- `ActiveGameplaySession` still references an activation-time world snapshot;
  gameplay consumers continue to read resident `RoomSet`, `RoomGeometry`, and
  metadata projections. A live session-owned world bundle and consumer
  migration remain `OPEN`.
- `AmbitionExperiencePlugin` still lives in `ambition_app`; extraction into a
  reusable provider crate remains `OPEN`.
- provider activation boilerplate remains duplicated and no fourth-provider
  proof exists.
- routes still lack real provider load plans, fresh preparation transactions,
  immutable prepared sessions, retry/cancellation/supersession, and one-shot
  activation consumption.
- loading presentation/activity and the complete frontend zero-state ownership
  proof remain `OPEN`.
...
| C1-audio-session | OPEN (exact frontend/gameplay audio-context implementation candidate awaiting Rust verification) | Every shell activation owns an explicit audio context; frontend and gameplay music/SFX resolve through the exact current owner; stale retirement or queued work cannot affect a newer activation. |
| MUSIC-SILENCE | OPEN (explicit governed-empty playback behavior is implemented but must be reverified after the unified context rewrite) | A provider or frontend profile that deliberately authors no music stops prior playback rather than retaining it. |
| AUDIO-REQUIRED | OPEN (required registry plus explicit empty fragments retained in the candidate; focused tests must pass) | Missing audio composition is an error; deliberate silence is a valid explicit profile. |
...
| C1-adaptive-music | OPEN (complete provider-composable cue catalogs and provider-qualified loaded layers are implemented as a Rust-verification candidate) | Actual adaptive cue definitions, encounter bindings, loaded layers, and playback authority are provider-relative and exact-owner scoped. |
| C1-audio-reset | OPEN (focused context-change reset bundle implemented; same-provider and stale-retirement tests must compile and pass) | Room, radio, encounter, adaptive, director, playback, and SFX request state is activation-local; relaunch starts fresh without damaging a newer owner. |
| C1-sfx-session | OPEN (actual provider procedural/bank source resolution, exact-owner messages, late-bank refresh, frontend SFX, and playback records are implemented as a Rust-verification candidate) | Frontend and gameplay SFX resolve from the exact active profile and provider source; delayed work, silent providers, late banks, and same-provider relaunch are identity-safe. |
...
| W0 | OPEN | One canonical live session-owned world representation contains room set, geometry, metadata, starting character, provider selections, load authorization, and request state; all gameplay consumers read it rather than resident projections. |
...
| RAW-INPUT | DONE for keyboard AND controller (2026-07-14: `basic_shell_keyboard` unifies raw `ButtonInput<KeyCode>` and `Query<&Gamepad>` through one neutral `menu_nav_edges` → launcher nav/confirm; D-pad mirrors arrows, South mirrors Enter/Space — `arrow_keys_move_the_launcher_cursor`, `enter_and_space_confirm_the_selection`, `keyboard_is_inert_when_launcher_is_not_active`, `controller_dpad_and_south_drive_the_same_launcher_commands_as_the_keyboard` (spawns a real `Gamepad`, presses via `digital_mut`), `controller_is_inert_when_launcher_is_not_active`. Quit-to-Home binding `quit_to_home_on_key` accepts F10 OR the controller Start button. X0's launcher walk derives its row count from `ShellLaunchCatalog.entries` + the Exit action, no literal. | The title launcher responds to actual keyboard AND controller input resources, not only injected commands. |
...
| W2 | OPEN | Camera, HUD, dialog, map, cutscene UI, input, music, adaptive layers, SFX queues/loops, and developer chrome have explicit host/session ownership and an exact frontend zero-state proof. |
| L0 | OPEN | Every provider contributes real preparation work through a fresh `ambition_load` transaction and publishes an immutable prepared session behind a required activation barrier. |
| L1 | BLOCKED on L0 | Retry, cancellation, supersession, streaming, promotion, stale-completion rejection, and relaunch consume fresh one-shot transaction authority. |
| P0 | OPEN | Provider authoring is a compact Bevy plugin surface with shared registration/preparation/activation/teardown defaults and a fourth-provider proof. |
| A0 | OPEN | Ambition is independently composable from a reusable provider crate; the host no longer performs hidden Ambition gameplay/content initialization. |
...
| X0 | OPEN (baseline lifecycle proof exists; exact-owner SFX/adaptive/reset and shared-host headless changes require revalidation) | Headless cross-experience cycle proves fresh identities, actual provider audio sources, live world replacement, and exact zero-state ownership. |
| X1 | OPEN (baseline no-window proof exists; expanded actual-source and exact frontend ownership assertions require revalidation) | No-window shipping composition proves exact camera/UI/input/music/adaptive/SFX/sprite ownership through Ambition, Sanic, Mary-O, relaunch, and Exit. |
| B0 | OPEN (baseline startup sequence exists; exact frontend audio context and actual input skip path require revalidation) | Powered-by startup naturally advances or is skipped through real input, owns frontend audio/presentation exactly, cleans up, and hands off to one launcher authority. |
| B1 | OPEN | One deterministic optional loading activity proves engagement, ready-hold, Continue, cleanup, and destination isolation over a real load transaction. |
