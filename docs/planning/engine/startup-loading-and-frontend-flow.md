...
- SFX authority is still resident-Ambition: no current provider authors SFX and
  SFX playback does not yet consult `ActiveAudioSelection.sfx()`, so
  cross-provider SFX composition (combined bank indexing, provider-filtered
  lookup, per-session ownership) is UNIMPLEMENTED — only music is provider-
  relative. OPEN (Issue 6).
