...
- the live session world remains canonical, while the explicitly owned compatibility projection still requires gradual consumer removal before `W0` can become `DONE`.
...
|---|---|---|
| W0-live-world | OPEN | Candidate owns one exact mutable session-world entity and adds room-change, stale-publication, stale-retirement, same-provider freshness, and frontend-unavailability evidence. Remaining resident compatibility consumers and Cargo verification keep this open. |
