...
| alter world mechanisms, return to the changed state | ◐ the save is a switch's one authority since `561055516` (`SwitchOn`, its per-tick mirror and that mirror's restore test are deleted), so a rebuilt room reads the switch where the press wrote it; witnessed at the persistence layer by `save_then_load_preserves_encounter_and_switch`. ⚠ No test presses a switch, leaves the room and comes back, and the reconstitution census no longer flips switches (it only ever wrote the mirror). |
