...
| P5.42 | Measure deletion payoff | ◐ **MEASURED, and it is a SCRIPT rather than a number** — `scripts/measure_archetype_legacy.py`, so the next reading is taken off the tree instead of remembered. ⇥ ⛔⛔ **STATE THE UNITS: two of them give OPPOSITE answers.** Against the brief's 3,755-line acceptance list, the six components now stand at **3,431 production lines (comments included, −324)** and **1,959 CODE lines (comments excluded)**. This campaign writes very long explanatory comments — `archetype_spec.rs` is 320 lines of which 119 are code and 191 are commentary — so a line count overstates the surviving MODEL by roughly two-fold, in both directions: it flatters a deletion that removed prose and panics about a fix that added a paragraph. ⇥ ⛔⛔ **AND THE FIRST VERSION OF THIS MEASUREMENT SAID THE LEGACY HAD GROWN BY 388 LINES.** It had shrunk. Two silent errors, both in the test/production split: whole `*_tests.rs` files counted as production because they carry no inline `#[cfg(test)]` (719 lines in one directory), and `#[cfg(test)]` on a `fn` rather than a `mod` went undetected (a 257-line fixture roster). ⇒ the script carries both bugs in its own docstring, because the next person to write this measurement will write the same two. ⇥ ⭐ **where the mass actually is, by CODE lines**: `features/enemies` 1181 (flat against its 1198 baseline — the row that has barely moved), `autonomous_reconcile` 506 (halved from 1045), `ArchetypeSpec` 119, `ActorTuning` 87, `enemy_roster` 21, and `character_archetypes.ron` **45 data lines against 843** — the one component that is nearly gone. ⇒ the remaining work is concentrated in ONE file, and it is not the one the acceptance signal names. ▢ re-run at the end of each phase; the row is ✔ when the trend is recorded against a phase rather than against a session. |
