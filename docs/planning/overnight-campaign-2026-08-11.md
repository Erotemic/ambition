...
| P4.29 | Wire shields/parry for appropriate fighters | ✔ **authored, reaching the seat, and it immediately exposed a CPU defect worth the whole exercise.** Giving the fighters `shield` turned the stage into two statues: `Disadvantage` covers CORNERED as well as hitstun, Shield outscored Retreat, and guarding does not un-corner anybody — an absorbing state reached in the opening second, per fighter, forever. Fixed where the genre says: a shield is a reaction to a SWING (gated on a hostile mid-attack), and a cornered fighter with nothing incoming retreats. ⇥ AS WRITTEN: still unverified in play: does the bubble block, does the parry window read ⇥ ✔ **VERIFIED IN PLAY 2026-08-13** — `a_seated_fighter_carries_the_verbs_its_character_authored_and_not_the_engines` boots the real smash stage through `build_demo_app`, decides a two-fighter match, runs 120 ticks and reads the LIVE `BodyAbilities` off both seated bodies. ⭐ **the gap these three boxes named was never a capability, it was a MEASUREMENT**: the verbs were authored and the engine machinery has been there all along, but nothing checked the DISTANCE between them — definition → preparation → seating → the match's ability MASK, each step of which had its own test while the chain did not. ⇥ ⛔⛔ **and falsifying it corrected the claim.** Dropping `shield` from the character turns it red (the character is NECESSARY). Adding `fly` to the character alone does NOT — the match mask intersects it away — so the poison proves the MASK, not "the character decides"; adding `fly` to BOTH turns it red. ⇒ what the test actually measures, end to end through a live stage, is `AbilitySet` INTERSECTION: a character states what its body can do, a ruleset states what the mode permits, and a verb needs both. The middle row is the one worth keeping — authoring a capability onto a character is not enough to smuggle it into a mode, which is what makes P3.25's mask a real restriction rather than decoration. ⇒ ◐ **the CAPABILITY half is closed and the BEHAVIOUR half is not**, which is why this stays open while P4.30 does not: *does the bubble block, does the parry window read* are questions about what the shield DOES on the stage, and a test proving the verb reaches the body cannot answer them. ⇥ ✔ **THE BLOCK'S CONSEQUENCE IS PINNED (2026-08-13) — and the gap was exactly where a tested rule stops.** `shield_blocks_hit` had SIX unit tests and `resolve_body_hit` had NONE, so *the bubble blocks* was proven as GEOMETRY (is this hit on the faced side?) and never as CONSEQUENCE (does the body keep its health?). A resolver that computed the geometry and then fell through to the damage path would have passed every test in that file. ⭐ `a_raised_shield_blocks_the_hit_and_a_lowered_one_does_not` asserts four OUTCOMES of one hit: shield up + front ⇒ `Blocked` and HP unchanged; shield DOWN + same hit ⇒ damaging (the poison, without which the first clause is satisfied by a body nothing can hurt); shield up + from BEHIND ⇒ damaging, which is what stops it passing on a resolver that blocks everything while a shield is held. ⚠ falsified by returning the damage path after reporting the block. ⇥ ✔ **P4.29 IS DONE** — the ✔✔ below says so outright: the parry window reads through `body_vulnerable`, the single gate every damage boundary consults. ⇥ AS WRITTEN: **what remains of this row is the PARRY WINDOW**, which is a different gate: a parrying body is invulnerable through `body_vulnerable`'s `!shield.parrying()`, so a parried hit is `Ignored` rather than `Blocked` — never registered at all. That distinction is untested and is the last of *"does the bubble block, does the parry window read"*. ⇥ ✔✔ **AND THE PARRY WINDOW READS — P4.29 IS DONE (2026-08-13).** `body_vulnerable` is the single gate every damage boundary consults (hazards, empowerment, the actor update, the damage resolver, projectiles, the avatar's body integration) and the ONE thing that makes a parry a different defence from a block: a parried hit is `Ignored` and never registers, where a blocked one registers and arms a guard i-frame. ⛔ **it had SIX production callers and no test.** `vulnerability_gate_tests` toggles each of its four terms ALONE against an otherwise-vulnerable body — which is what makes it a test of the GATE rather than of one scenario, since a gate that had lost any single term would still pass a test that raised two. ⚠ **the clause that earns its keep is the POISON**: a shield held PAST its parry window leaves the body vulnerable here, so a parry cannot be satisfied by merely holding a guard, and the two defences stay two. ⚠ falsified by deleting `!shield.parrying()` from the conjunction — `parrying_alone_makes_a_body_untouchable` goes red with its own message and the other four stay green, which is the isolation working. ⇒ **both halves of *"does the bubble block, does the parry window read"* are now pinned at the seam that decides them.** |
| P4.30 | Wire grounded dodge | ✔ **CLOSED 2026-08-14** — this row's own text says *"ONE test closes all three"*, and that test landed on 2026-08-13 for its two siblings: `a_seated_fighter_carries_the_verbs_its_character_authored_and_not_the_engines` (`the_stage_kills.rs`) boots the real smash stage, decides a two-fighter match, runs 120 ticks and reads the LIVE `BodyAbilities` off both seated bodies — the COMPOSITION half this row was waiting on. ⇥ AS WRITTEN: capability authored on the smash fighters; unverified in play. ⇥ ⭐ **THE ENGINE HALF IS COVERED — measured 2026-08-12, so that box was narrower than it read.** `available_dodge` gates on `abilities.dodge`, and `movement/abilities.rs` carries a dedicated test module for the resolver: which maneuver a body performs from one buffered press, with a body owning BOTH verbs as the interesting case. What is unverified is the COMPOSITION half — that a fighter's authored `dodge: true` is what its seated body carries — and that is the same claim as P4.29's shield and P4.32's ledge grab, because all three arrived in one `with_abilities` call for one reason. ONE test closes all three. ⇥ ⛔⛔ **AND THE ATTEMPT MEASURED WHY IT IS STILL OPEN (2026-08-12), so the next run does not rediscover it.** A seating test in `ambition_demo_smash` composing `MinimalShellPlugins + AmbitionLoadPlugin + SmashExperiencePlugin` — the composition the demo's own routing tests use — **PANICS on the first update after a roster exists**: `sync_live_player_dev_edits_system` takes `EditableAbilitySet` as a plain `Res`, and no shell-only composition publishes it. Adding `add_headless_foundation` + `PlatformerEnginePlugins::fixed_tick()` fixes that and then seats **nothing**, because this demo enters through CHARACTER SELECT: a roster inserted by hand never reaches a stage, so there is no seated body to read. ⇒ **the test needs a live stage, not a composed app** — the route has to be driven, which is what `smash_in_the_host.rs` does behind the `input` feature. That is the work, and it is fixture engineering rather than a missing capability. ⚠ recorded rather than left as a red test: a failing test in the tree is a worse instrument than an honest open box. ⇥ ✔ **VERIFIED IN PLAY 2026-08-13** — `a_seated_fighter_carries_the_verbs_its_character_authored_and_not_the_engines` boots the real smash stage through `build_demo_app`, decides a two-fighter match, runs 120 ticks and reads the LIVE `BodyAbilities` off both seated bodies. ⭐ **the gap these three boxes named was never a capability, it was a MEASUREMENT**: the verbs were authored and the engine machinery has been there all along, but nothing checked the DISTANCE between them — definition → preparation → seating → the match's ability MASK, each step of which had its own test while the chain did not. ⇥ ⛔⛔ **and falsifying it corrected the claim.** Dropping `shield` from the character turns it red (the character is NECESSARY). Adding `fly` to the character alone does NOT — the match mask intersects it away — so the poison proves the MASK, not "the character decides"; adding `fly` to BOTH turns it red. ⇒ what the test actually measures, end to end through a live stage, is `AbilitySet` INTERSECTION: a character states what its body can do, a ruleset states what the mode permits, and a verb needs both. The middle row is the one worth keeping — authoring a capability onto a character is not enough to smuggle it into a mode, which is what makes P3.25's mask a real restriction rather than decoration. ⇒ ✔ **BOTH HALVES ARE NOW CLOSED, so this row is done.** The ENGINE half was already covered (`available_dodge`'s resolver module, which maneuver a body performs from one buffered press, with a body owning BOTH verbs as the interesting case); the COMPOSITION half — that a fighter's authored `dodge: true` is what its seated body carries — is what `a_seated_fighter_carries_the_verbs_its_character_authored_and_not_the_engines` closed on 2026-08-13, on the live stage, exactly as this row predicted ONE test would. ⚠ **P4.29 and P4.32 do NOT close with it**, and the difference is real rather than bookkeeping: their remaining clauses are about BEHAVIOUR on the stage — *does the bubble block, does the parry window read*, and *verify grab/hang/climb/roll/getup-attack/jump/drop* — which the capability test cannot answer. Dodge had no such clause because its behaviour was already pinned in the engine. |
...
| P4.32 | Enable and tune existing ledge mechanics in Smash | ◐ `ledge_grab` authored on the smash fighters — Jon's diagnosis was exactly right, *"the generic fighter capability set did not grant ledge_grab"*. ⇥ AS WRITTEN: verify grab/hang/climb/roll/getup-attack/jump/drop on the real stage, and fix what the first real adopter exposes ⇥ ✔ **VERIFIED IN PLAY 2026-08-13** — `a_seated_fighter_carries_the_verbs_its_character_authored_and_not_the_engines` boots the real smash stage through `build_demo_app`, decides a two-fighter match, runs 120 ticks and reads the LIVE `BodyAbilities` off both seated bodies. ⭐ **the gap these three boxes named was never a capability, it was a MEASUREMENT**: the verbs were authored and the engine machinery has been there all along, but nothing checked the DISTANCE between them — definition → preparation → seating → the match's ability MASK, each step of which had its own test while the chain did not. ⇥ ⛔⛔ **and falsifying it corrected the claim.** Dropping `shield` from the character turns it red (the character is NECESSARY). Adding `fly` to the character alone does NOT — the match mask intersects it away — so the poison proves the MASK, not "the character decides"; adding `fly` to BOTH turns it red. ⇒ what the test actually measures, end to end through a live stage, is `AbilitySet` INTERSECTION: a character states what its body can do, a ruleset states what the mode permits, and a verb needs both. The middle row is the one worth keeping — authoring a capability onto a character is not enough to smuggle it into a mode, which is what makes P3.25's mask a real restriction rather than decoration. ⇒ ◐ **same split as P4.29**: the capability reaches a live seated body (2026-08-13) and the ledge's seven behaviours — grab, hang, climb, roll, getup-attack, jump, drop — are still unverified in play. The capability test is a floor for that work, not a substitute. ⇥ ⭐⭐ **AND THE ENGINE HALF IS NOT UNVERIFIED — it is the best-covered feel system in the crate** (measured 2026-08-13, before writing anything). The row's box listed seven behaviours *"to verify on the real stage"*; **60 tests already run them**, 52 in `ledge_grab/tests.rs` and 4 in `movement/tests/ledge_grab.rs`, and they cover every one by name: hang (`make_hanging_player`, and a rejection when the hang space has a wall in front), jump (away / toward the platform / with no horizontal input), climb (`up_alone_still_starts_a_climb`, `climb_path_curves_up_before_going_over`), roll (`shield_held_starts_a_ledge_roll`, `ledge_roll_lands_further_inboard_than_climb`, `ledge_roll_uses_a_curved_arc_not_a_diagonal`), getup-attack (`attack_press_from_hang_starts_getup_attack_and_fires_slash`), drop (`voluntary_drop_arms_a_regrab_cooldown`), and grab on two surface kinds. ⚠ **and my first census of this was WRONG in the alarming direction**: grepping `movement/*.rs` for `MovementOp::LedgeRoll` / `LedgeDrop` found no producer and I nearly wrote *"two declared verbs nothing emits"* into this row. They are emitted from `ledge_grab/runtime.rs`, a directory the grep did not reach. ⇒ **checked before recording, which is the only reason it is not a fifth wrong mechanism in a planning file.** ⇥ ▢ **so what is genuinely left is not engineering**: the verbs are implemented, tested, and reach a live seated fighter (P4.29/30/32's capability test, 2026-08-13). *"Fix what the first real adopter exposes"* is a PLAY question — how the ledge feels on the smash stage at its tuning — and that is Jon's call, not a measurement. |
| P4.33 | Author landing lag/autocancel on real aerials | ◐ **all five aerials author BOTH halves** (n/f/b/u/d-air, lag 0.10–0.28s, autocancel windows inside each move's duration) — `MoveSpec` has carried the pair for a while with no adopter, and the guard asserts both are present because an autocancel with no lag is silently inert. ✔ **ANSWERED IN THIS ROW'S OWN NEXT SENTENCE** — the chain below traces every link end to end, so *does landing mid-aerial actually lock control* is **yes**. ⇥ AS WRITTEN: unverified in play: does landing mid-aerial actually lock control for the authored time ⇥ ⭐⭐ **THE CHAIN IS COMPLETE AND TRACED END TO END** (2026-08-13) — so *"does landing mid-aerial actually lock control"* is **yes**, and here is every link: authored `MoveSpec::landing_lag_s` → `moveset/mod.rs:917` writes `combat.landing_lag_timer = timer.max(lag)` on landing → `body_integration.rs:170` passes `combat.recoil_lock_timer.max(combat.landing_lag_timer)` as *"the HARD lock this frame"* → `apply_post_hit_input_gates` does `if hard_lock_timer > 0.0 { input.axes = LocalAxes::ZERO }` → `input_systems.rs:63` decrements it by `frame_dt`. It is also rollback state, registered and cleared on room transition and lifecycle commit. ⇥ ✔ **THE WIRING IS GUARDED, and the row's own prescription landed** (verified 2026-08-14): `landing_lag_alone_still_locks_control` pins `BodyCombat::hard_lock_timer()` — reduce it to `recoil_lock` alone and that test fails. ⭐ **and the expression is no longer at a call site to reduce**: `engine_input_from_actor_control` takes `&BodyCombat` rather than two loose `f32`s, so the reduced form the ACTOR road used to pass is unrepresentable. ⛔ **what the recheck DID find is one layer over**: `causal.rs` traced `recoil_lock` as *"a HARD lock … no input authority at all this tick"* while publishing half the gate, so a body locked by landing lag alone read as fully in control in the one artifact whose job is explaining a divergence. It publishes `hard_lock` now. ⇥ AS WRITTEN: **what is still unguarded is the WIRING, and it is one `.max()`.** `landing_out_of_an_aerial_costs_its_authored_lag_unless_it_autocancelled` proves the TIMER is set, and nothing proves the timer LOCKS: if line 170 were reduced to `recoil_lock_timer` alone, landing lag would stop locking anything and every existing test would stay green. ⇒ that is the test to write, and it wants a player-tick fixture rather than a unit call — the integration function takes eighteen parameters. ⚠ recorded as TRACED rather than tested, deliberately: reading a chain is weaker evidence than running it. ⇥ ⛔⛔ **AND TRACING IT FOUND A DEFECT — the chain above is the PLAYER's, and there is no actor equivalent (ledger D108).** `sync_actor_components_from_cluster` rebuilds `BodyCombat` every frame for a non-player body and carries five timers across; `landing_lag_timer` is not one, so a CPU's landing lag is erased the next frame. And the actor road's stagger is `(hitstun, recoil_lock)` — no landing lag in it at all. ⇒ **a CPU lands clean out of the aerial that costs a human 0.10–0.28s**, which is *"changing the controller changes the body"* — the thing this campaign exists to prevent — sitting inside the feel row that authored the content. ⇥ ⭐⭐ **BOTH REMAINING HALVES CLOSED 2026-08-13.** (1) the WIRING is guarded: `a_staggered_body_loses_input_authority_like_the_player` now drives a body in LANDING LAG ALONE down the actor road and requires zero steering — demonstrated red under the old form, where it steered 14.1px. (2) ⛔ **D108's third half was still live and this found it.** AC3 unified the decay and the reset, so a CPU's landing lag was set, decayed and rolled back correctly — and locked NOTHING, because the actor road's gate took `(hitstun, recoil_lock)` as two loose `f32`s and filled the hard-lock slot with `recoil_lock_timer` alone while the player road filled it with `recoil_lock.max(landing_lag)`. ⇒ the gate takes `&BodyCombat` now, so the reduced form is UNREPRESENTABLE — there is no `f32` parameter left for a caller to fill with the wrong field. ⚠ **and my AC3 commit message overstated this**: it said CPUs "now pay the landing lag", which was true of the timer and not of the lock. They pay it now. |
...
| P5.38 | CPU AI chooses from actual character movesets/capabilities | ◐ **MOST OF THIS WAS ALREADY TRUE and the row did not say so** (grepped 2026-08-12 before touching anything). The Smash CPU seats run `template: Fighter`, and that brain does ask: `attack_kit_of` enumerates the body's REAL moveset by asking `move_for_directional_verb` for basic/smash/special × five directions, so a CPU throws the character's own `smash_forward` — `press_the_chosen_attack` sets `melee_strong_hint` for a Smash binding. Movement verbs are capability-gated on the body (`can_blink`/`can_shield`/`can_dash`, jump on the real air-jump budget). ⛔ **the ONE thing it got wrong was the evade, and it got it wrong in three places at once**: `apply_dodge` claims the dash buffer BEFORE `apply_dash` can see it, so a body owning `dodge` never dashes — and the Smash fighters author `dash: true` and `dodge: true` together (P4.30). Every burst a CPU chose on that stage was named `Dash` by the brain, modelled as `ShadowIntent::Dash` by the rollout, and performed as a ROLL by the body; and a body authoring only `dodge` was offered no burst at all, because the question asked was `can_dash`. ⇒ `SelfView::can_dodge`, one `evade` selection that names the maneuver the press will really produce, a direction rule (roll AWAY from a swing, INTO everything else — perceivable, so a human could make the same read), and `Dodge` added to the rollout's unmodelled list, which REMOVES a lie rather than adding a gap. ⇥ ▢ remaining: no ledge verb (`ledge_grab` is authored but `Recovery` still walks toward centre and jumps — that is P4.32's). ⇥ ⛔⛔ **AND THE SECOND CLAUSE WAS WRONG — re-measured 2026-08-12.** It said the `template: Smash` brain *"builds an EMPTY `attack_kit` by design, so Ambition's own enemies do not choose from their movesets"*. The premise is true and the conclusion does not follow: `attack_kit_of` does return `Vec::new()` for a non-`Fighter` brain, deliberately, because a scored kit is a per-actor per-tick `Vec` no other brain reads. But choosing from a moveset does not require a scored kit. The Smash brain AIMS: `smash::action` picks up / down / forward from the target's real offset, `emit` turns that into `ActorControlFrame::attack_axis`, and `resolve_attack_gestures` resolves the axis against the body's OWN `MovesetContract` — so a goblin that authors an up-tilt throws it at a target overhead, and one that authors none falls back to its base attack. ⇒ what Ambition's enemies genuinely do NOT choose is the VERB (basic vs smash vs special) and they never weigh frame data; that is the real gap, and it is a smaller and more specific one than the row claimed. ⇥ ✔ **AND THE HALF THAT IS TRUE IS PINNED NOW, because it was not**: `choose_action`'s direction pick — the entire mechanism — had ONE test asserting a melee comes out at all. `an_engaged_swing_aims_at_where_the_target_actually_is` asserts all four reads: level → sideways toward the foe, overhead → up, airborne-above-foe → down, and ⭐ **GROUNDED-above-foe → NOT down**, because a body standing on a platform over its foe is not throwing a down-air and a rule reading the vertical offset alone would answer identically. ⚠ FALSIFIED: dropping `&& !obs.self_on_ground` turns it red on exactly that clause. ⚠ gravity-framed throughout — `down` is the observation's, so a rotated-gravity room reads the same (I10). ⇥ ⛔⛔ **THE THIRD CLAUSE IS WRONG TOO, AND I ALMOST SHIPPED A DUPLICATE AUTHORITY PROVING IT (2026-08-13).** The row says Ambition's enemies *"never choose the VERB (basic vs smash vs special)"*. Grepping `SpecificAction::` inside `action.rs` returns five emitted verbs — `Idle`, `Walk`, `Jump`, `DoubleJump`, `MeleeAttack` — and no ranged, so I added a ranged branch to `choose_action`'s `Approach` arm. ⛔ **it turned `ranged_shot_suppressed_when_line_of_fire_blocked` RED**, and that test is the whole story: the brain ALREADY chooses ranged, in a separate stage in `smash/mod.rs` that upgrades a CLOSING action to a `RangedAttack` when the target is in the band between `attack_range` and `aggro_radius` — with a real `WorldView::line_of_fire` occlusion check my branch bypassed. ⇒ **reverted.** A second producer for one decision, defeating the first one's gate, is exactly the defect this campaign exists to remove; the test caught it before it landed. ⇥ ⭐ **so the measured verb inventory is**: `Walk`/`Idle`/`Jump`/`DoubleJump`/`MeleeAttack`/`Dash` produced in `action.rs`, `RangedAttack` produced by the band upgrade in `mod.rs`, and **`Special`, `Shield` and `Dodge` with ZERO producers**. ⇥ ▢ **and `Special`'s absence is a DECISION already recorded at the site**, not a gap: *"wiring the autonomous brain to choose `SpecificAction::Special` here is a one-line change. It's LEFT OFF deliberately: a naive 'fire special whenever melee recharges' cadence spams the move and crowds out the damage-triggered regroup kit (it broke the duel-arena regroup-dash). The autonomous special CADENCE is a feel/AI tuning pass for Jon."* ⇒ what remains of this row is a FEEL call with a known one-line seam, plus `Shield`/`Dodge`, which are the defensive half P4.29/P4.30 just proved reach a body. ⇥ ⭐⭐ **`Shield`'s ZERO PRODUCERS WERE A THREE-LAYER STORY, AND THE BOTTOM LAYER WAS A STALE COMMENT** (2026-08-13). `emit_inputs` dropped `SpecificAction::Shield` to Idle saying *"Reserved — no engine-side input bit yet"*, and `shield_held` is a field on **the very frame that function writes to** — the live path a player's guard already takes (`shield_held` → `resolve_shield`). ⇒ wired, and pinned by `a_chosen_shield_presses_the_guard_and_stops_moving`. ⇥ ⛔⛔ **THE WIRE IS DELIBERATELY ABSENT, AND THE CODE SAYS SO** (measured 2026-08-14, before building it). This row reads as plumbing waiting to be laid — *the vocabulary exists; the wire from it to the brain does not* — and the smash brain states the opposite intent at the decision itself: reactive defense *"reacts to a **perceivable** lunge — the opponent closing fast — not a privileged read of its attack flag, so a human could make the same read"*. `perceived_threat` takes the target's position out of `obs_history.delayed(now, reaction_delay_s)` and differentiates it, so the read is both LAGGED and derivable by a player watching the screen. ⇒ **adding `target_phase` to `BrainSnapshot` would install exactly the privileged read that comment refuses**, and no brain would be entitled to it. ⭐ what is genuinely open is therefore a DESIGN question for Jon and not an engine gap: should a CPU guard because it saw a swing (frame-perfect, unhuman) or because somebody rushed it (lagged, human-fair)? The engine currently answers the second, on purpose. ⇥ AS WRITTEN: **what actually remains is ONE layer, and it is now named**: the brain cannot SEE an incoming swing. `ObservationFrame` has no channel for what the TARGET is doing and neither does `BrainSnapshot` — while `PerceivedActor` already carries `phase` + `phase_remaining` and `BodyPhase` already distinguishes `AttackStartup` (*"the punish window"*) from `AttackActive` (*"do not walk into it"*). The vocabulary exists; the wire from it to the brain does not. ⇥ ⚠ **`Dodge` is NOT the same job and no longer shares an arm with shield.** There is no dodge bit at all; a dodge reaches a body through the DASH buffer, which is the coupling this row already records, so emitting a dash for it would ride the defect rather than route around it. ⇥ ⛔ **the CHOICE is deliberately not attempted**: a CPU that guards changes how every CPU fight plays, and this file's own precedent defers that class — the note beside `Special` says the autonomous cadence is a feel pass for Jon. ⭐ but a shield trigger is REACTIVE (an attack is coming) rather than a CADENCE (fire every N seconds), so it does not carry the spam failure mode that argument was made about, which is worth knowing before the feel pass. |
...
| P5.40 | Rerun PCA as an unconditionally registered character | ✔ **BLOCKED ON D74, WHICH JON'S OWN ROW ALLOWS SEPARATING** (*"keep D74 separate if the timing bug remains"*). Re-verified 2026-08-12: it remains. ⭐ the state is already LOCATED and written at the exclusion itself in `character_catalog.rs` — registering the PCA reds `possession_end_to_end::attack_while_possessing_…`, the per-step trail is identical through step 3 and parts at step 4 on `vel.x` (baseline zeroes it on a 4-step cadence, the registered build accumulates at −10.83/step, both falling), with the same hp, same `Brain::Player(0)`, same collision size and same gravity — so the fault is UPSTREAM of combat, in a movement or contact decision on a falling body, and the possessed body ends 580 px away and airborne. ⛔ **four wrong mechanisms have been written down for this already; the standing rule at that comment is DO NOT ADD A FIFTH WITHOUT OUTPUT**, so this row stays OPEN rather than accumulating another guess. The next step is the one the comment names: bisect the movement kernel across the deterministic step-4 divergence. ⚠ the cost of the exclusion is stated and unchanged — the PCA is on `SMASH_ROSTER`, so the grid is one portrait shorter. ⇥ ✔ **DONE 2026-08-13 — D74 IS CLOSED AND THIS LANDED WITH IT.** `perfect_cellular_automaton` is on `PLAYABLE_ROSTER`, registered like every other character, and the smash grid is no longer one portrait short of the roster it advertises. ⭐ **what unblocked it was a coupling this campaign had already deleted.** D74's last surviving hypothesis — five others refuted — was *"one more registered character is one more sheet demanded at load"*. Running the probe that row asked for (in-flight loads per step, both builds) reports `staged=3 ready=0` at every step of the divergence window in BOTH builds, with the possession trail identical to the last decimal and the test passing: registration is DECLARATIVE now, so there is no extra sheet and no timing to differ. Nobody had to fix anything; the premise expired. ⚠ verified before landing — app 337+179+1, content 192+32, smash 67, gate green. |
| P5.41 | Clean architecture docs and stale comments | ◐ **THE EIGHT THIS SESSION BROKE ARE REPAIRED**, and repairing them found the real shape of the row. Deleting `project_provoked_archetype`, `HostileArchetypeId`, `AutonomousSource::Provoked`, `reconstruct_provoked` and `BUILDABLE_ONLY_CAST` left doc links pointing at items that no longer exist — and two of them were worse than broken, they were WRONG: `autonomous_reconcile`'s module doc still described a rewind "rerunning the roster archetype construction … tuning / capabilities from the archetype id the binding retained" (all three nouns gone), and `reconstruct_provoked_profile`'s note still called its counterpart the one that "rebuilds a whole body because an archetype IS the creature" — which was the DEFECT, fixed the same day. Each repair keeps a ⚠ line saying what the sentence used to claim, because a reader who knew the old text needs to know it was retired rather than lost. ⇥ ⛔⛔ **AND `cargo check` CANNOT SEE ANY OF THIS.** The gate is `check --all-targets`; broken intra-doc links are a RUSTDOC lint, so this whole class rots silently between the two. Measured 2026-08-12 with `cargo doc --no-deps`: **199 unresolved-link / private-item-link warnings** across four crates — monolith 122, `ambition_characters` 39, `ambition_platformer2d_core` 21, `ambition_combat` 17 — and only eight of them were mine. ⇒ ✔ **the RATCHET exists and CI runs it** — `scripts/check_doc_link_ratchet.py --check`, baseline in `dev/doc_link_ratchet_baseline.json`, in the `rustfmt + clippy` job. Counts may fall and must not rise; it also fails when a crate emits NO rustdoc output, because zero warnings from a build that did not happen is not a score. Poisoned with one bogus link: red. ▢ the 199 itself is untouched on purpose — the ratchet is what makes lowering it stick, and a sweep with nothing behind it is 199 links again by September. Ledger **D103**. ⇥ ✔ **A SWEEP KEYED ON WHAT THIS SESSION DELETED (2026-08-13), which is the only census for this row that can be complete.** Every deletion leaves comments describing it, and the useful question is not *"is any comment stale"* — unanswerable — but *"what did I just remove, and what still describes it in the PRESENT TENSE"*. Grepping the seven names deleted today (`sandbags_are_passive`, `brain_profile_for`, `hostile_spec_for_actor`, `hostile_brain_id_for_actor`, `fallback_brain_id`, `provider_fallbacks`, `IDENTIFIERS_AWAITING_A_CASTING_DECISION`) found 19 mentions, of which 16 are correct HISTORY (*"it was X"*) and three were false: `seat_brain_profile`'s doc still said *"`SMASH_ROSTER_RON` exists solely to hold brain-only rows for this lookup"* about a file deleted in D87; `unsatisfiable_seats` still said *"the archetype arm still answers for it"*, which would now make it approve a seat that seating refuses; and the smash demo said a `brain_profile` *"was an ARCHETYPE key"* as if the alternative were still available. ⚠ **history is not staleness** — a ⛔ block explaining why something was deleted is the most valuable comment in the file, and a sweep that deleted those would be the real damage. The test is TENSE, not mention. |
| P5.42 | Measure deletion payoff | ◐ **THE FIRST ACTUAL MEASUREMENT, 2026-08-12 — and it reframes the acceptance signal.** Jon's baseline is *~3,755 lines of standing legacy*. Measured today, the same six items are **3,059 lines — of which 1,600 are CODE and 1,459 (48%) are PROSE.** ⛔ **STATE THE UNITS: a line count over this repository is not a code count.** This campaign writes long ⛔/⭐ notes recording what left and why, and those notes live in the files they are about, so the legacy shrinks in behaviour faster than it shrinks in bytes. A deletion ledger read in raw lines will understate the payoff and — worse — can read a file as GROWING while its code halves. ⇥ **per item, total / code:** `features/enemies/mod.rs` 1,786 / **1,107** · `autonomous_reconcile` 416 / 204 · `ArchetypeSpec` 320 / 119 · `ActorTuning` 197 / 80 · `character_archetypes.ron` 264 / **45** (two rows) · `enemy_roster.rs` 76 / 45. ⭐⭐ **`features/enemies/mod.rs` IS 69% OF ALL REMAINING LEGACY CODE**, and everything else together is under 500 lines. That is the lever, and it is one file. ⚠ **its raw line count is ABOVE the baseline's 1,198 and that is not necessarily growth** — the baseline read *"features/enemies + CharacterRoster"* and it is not recorded which files that spanned, so the two numbers are not comparable and this row will not pretend they are. What IS comparable is the code figure going forward, which is why it is recorded here. ⇥ **the three items that have visibly moved**: `character_archetypes.ron` 843 → 264 (45 code, two rows), `autonomous_reconcile` 1,045 → 416, `ActorTuning` 275 → 197. ⇥ **the two that have not moved at all**: `ArchetypeSpec` (319 → 320) and `enemy_roster.rs` (75 → 76). Both are downstream of the same content decisions D96 holds, which is the honest reason and not an excuse — an item that cannot move until Jon answers should say so rather than sit unmarked. ⇥ ⭐⭐ **RE-MEASURED 2026-08-13, AND THE THREE-WAY SPLIT IS THE NUMBER THAT MATTERS.** Jon's baseline is *~3,755 lines of standing legacy*. The six items are **3,414 lines today — but only 1,175 of them are PRODUCT code**; 566 are test code and 1,673 (49%) are notes and blanks, most of them the ⛔ blocks this campaign writes as it deletes. ⇥ ```text\n  ArchetypeSpec             333 lines    119 product      0 test\n  features/enemies         1887 lines    806 product    322 test\n  enemy_roster.rs           165 lines     36 product     53 test\n  ActorTuning               197 lines     80 product      0 test\n  autonomous_reconcile      568 lines     89 product    191 test\n  character_archetypes.ron  264 lines     45 product\n  TOTAL                    3414 lines   1175 product    566 test\n``` ⇥ ⛔ **`features/enemies/mod.rs` GREW in raw lines (1,785 → 1,887) and that reads like the failure mode Jon named**, so it is worth saying exactly what moved: its PRODUCT code is 806, down from ~1,107, and the growth is notes plus `OpenCastingDecision` (a ~40-line type that replaced a 3-name `const`). The const was ENGINE-OWNED CONTENT POLICY; the type moved that authority out to the provider that owns the decision. ⇒ +40 engine lines that delete an engine authority is not the *"+4000 new / −2400 old"* wrap — but the raw line count cannot tell those apart, which is why this row measures product code separately from here on. ⇥ ⭐ **and `autonomous_reconcile` is 89 product lines**, not the 1,045 the baseline sizes it at: the post-GGRS reconstruction pass was deleted (D104) and what remains is two projections and their tests. ⇒ the acceptance signal is now dominated by ONE file's 806 product lines, and every road into it is body construction — blocked on the three casting decisions, not on engineering. ⇥ ⭐ **AND THERE ARE NO FREE DELETIONS LEFT — asked of the COMPILER, 2026-08-13.** `probe_dead_public_fns.py` (D105) marked every public `fn` in the four legacy files and built the workspace: `features/enemies/mod.rs` **23 of 23**, `archetype_spec.rs` 4 of 4, `actor_tuning.rs` 2 of 2, `autonomous_reconcile.rs` 2 of 2 — **31 of 31 have a call site the compiler can see**, through re-exports, macros, trait impls and cfgs alike. ⇒ this row's *"no free deletion left in it"* is now compiler-backed rather than grep-backed, and the remaining 1,175 product lines go when their CALLERS go — which is the casting decisions, not a cleanup pass. ⚠ recorded so the probe is not re-run: it costs a full workspace build. |
