...
| NAV | navigability pass on the worst complex-logic god-files (#1 north-star goal) | ☑ | XL | ~6 cycles | (6 commits) | — | proven split pattern (regex super::-doubling + dual-glob + pub(crate) for test-helpers + manual import-trim, NOT cargo-fix which over-trims the test build). Split: `combat/components.rs` 1185, `features/enemies.rs` 1302, `features/ecs/actors/mod.rs` 1111, `assets/game_assets/mod.rs` 1002, `dev/dev_tools/mod.rs` 1064, `features/ecs/bosses/mod.rs` 719. Each replay bit-identical, 0 warnings. Remaining big files are data-tables (boss sprites), mostly-tests (features/bosses), a prototype (falling_sand), or complex-imports (ecs/damage) — lower ROI. |
