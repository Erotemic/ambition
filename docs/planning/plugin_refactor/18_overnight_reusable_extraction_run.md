...
| T5 portal Q | 50m | ~25m | `5181fe15` (landed) + doc/verify this run | DONE — **BOTH couplings inverted (clean, zero divergence).** Verified portal core (`portal/{transit,plugin,presentation,messages,pickup}.rs`, non-test) imports **neither** `crate::input::ControlFrame` **nor** `crate::items::pickup::GroundItem` — only doc comments name them. **ControlFrame → `PlayerMovementIntent`** (portal-owned content-agnostic movement-intent resource, `portal/transit.rs:40`): `warp_portal_input` reads/writes `PlayerMovementIntent`; the content adapter `crate::ambition_content::portal::transit_adapter::{sync_movement_intent_from_control, apply_movement_intent_to_control}` brackets it (`.before`/`.after` `warp_portal_input`) to mirror `ControlFrame` axes in→intent and intent→`ControlFrame` out, byte-identical to mutating `ControlFrame` directly. **GroundItem → `PortalTransitable`** (portal-owned generic transitable body component, `portal/transit.rs:380`): `portal_item_transit` operates on `PortalTransitable`; the content adapter `{sync_ground_items_to_transitable, sync_transitable_to_ground_items}` attaches the marker to `GroundItem` entities and mirrors the body back after transit. Both names are now **hard-forbidden** in portal-core non-test code by the guard `architecture_boundaries_portal_core_does_not_import_ambition_content_roster` (tests.rs allowlisted — it drives the full content+core chain on the `ControlFrame`/`GroundItem` surface on purpose). The decoupling commit `5181fe15` predates this run; this run **verified** the gate (lib 1428, architecture_boundaries 15, scripted_gameplay 3, replay_fixture_regression **ZERO divergence**, portal_bridge_reachability 1, portal_lab_usable 1 — all green) and recorded the result. Nothing deferred. **→ Portal core is now free of BOTH couplings; T6 (crate extraction) is unblocked by T4+T5.** |
| T6 portal crate | 30m | — | — | conditional on T4+T5 — **now UNBLOCKED** (T4 render-split + T5 ControlFrame/GroundItem decouple both landed clean). Portal core is render-independent and content-roster-free; the crate extraction is mechanically possible. Remaining pre-crate item: the `crate::items::pickup::ItemPickupSet` schedule *ordering label* (allowlisted, not a content concept) would become a small neutral ordering label the crate owns. Not attempted this slice. |
...
| portal→`ControlFrame` (T5) | transit/input warp read the general player input frame | **Incidental** (portal needs a movement axis, not the Ambition input frame) | **LOW** — invert to a portal-owned `PlayerMovementIntent` + a content-side translator bracketing the warp | **DONE** — inverted (commit `5181fe15`); portal core reads `PlayerMovementIntent`, content adapter mirrors `ControlFrame`↔intent; zero replay divergence |
| portal→`GroundItem` (T5) | item transit teleported the sandbox held-item-on-ground body | **Incidental** (portal needs a transitable body, not the item roster type) | **LOW** — invert to a portal-owned `PortalTransitable` marker + a content-side attach/mirror translator | **DONE** — inverted (commit `5181fe15`); portal core operates on `PortalTransitable`, content adapter attaches it to `GroundItem` + mirrors back; zero replay divergence |
