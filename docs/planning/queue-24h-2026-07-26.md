...
- ▢ **D2b** the two findings D2's proof produced, both engine-side:
  (a) starting a rollback session before construction finishes diverges on every
  frame and the only diagnostic is a raw checksum mismatch — the engine's own
  harness dodges this by construction, so no in-repo test can see it;
  (b) a consumer must know TWO input seams (`ControlFrame` fixed-tick,
  `PendingLocalInput` GGRS) and writing the wrong one is silently ignored —
  Phase-6 leak #4.
