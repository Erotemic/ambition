...
- ✔ **V2 a behavioural guard, not a grep.** `every_manifest_family_carries_a_
  ...
  ▢ **what it does NOT cover:** families that do not join a folder at all (audio
  tracks carry `track.asset_path` verbatim; world/data/font entries are engine
  paths). If a consumer ever authors audio or a world through a folder
  convention, this test grows a row — it is a list, and a list is only as
  complete as the last person to add to it.
