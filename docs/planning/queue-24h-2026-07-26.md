...
- ◐ **D7** (Task 6) `player_schedule.rs` named `ambition_actors::` leaf systems
  **30 times** in one chain, the literal negation of "runtime orders semantic
  ...
  ▢ REMAINDER: four sets left — `WorldPrep`, `PlayerSimulation`, `RoomTransition`,
  `PresentationSync`. `PlayerSimulation` next: it holds the movement kernel and the
  post-sim damage/respawn resolver, which is the next place a consumer has to
  point at something.
