...
- ◐ **D7** (Task 6) `player_schedule.rs` named `ambition_actors::` leaf systems
  **30 times** in one chain, the literal negation of "runtime orders semantic
  ...
  ▢ REMAINDER: `RoomTransition` and `PresentationSync`. Both are small, and the
  same audit should run first — if neither has a cross-crate leaf reference, the
  honest answer is a documented ⊘ rather than two more enums.
