...
▢ **the honest remainder of Milestone E:** the fifth clause ("evidence exists to
design a public recipe/prefab API") is evidence, not an API. Designing that API
is real work with a real customer now that a consumer can author content, own its
art, and read its own error messages — and it wants its own campaign row rather
than a line here.
