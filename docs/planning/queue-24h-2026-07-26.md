...
- ▢ **L6 nothing gates a character against the geometry it will be played in.**
  ...
  broken together, and discovered by a human running the binary. A cheap
  instrument exists — seat every registered character in a plain walled box,
  hold a direction for a few seconds, and assert the body is still inside it.
  That is one test that would have caught L1 the day the arena was authored, and
  it generalises to every character a provider adds later.
- ▢ **L7 the versus fighters cannot hit each other.** Both seats derive their
  persona now (L3), and neither `mary_o` nor `sanic` authors a `MovesetContract`
  — Sanic by explicit design. So couch versus is two people walking into each
  other. This is the customer C3's `with_moveset` row has been waiting for
  ("waiting on its first real fighter"): versus needs ONE character with a
  hand-authored move list, and that character should be a new one authored for
  fighting rather than a swing bolted onto a speedster.
- ▢ **L8 the versus stage has no rules.** No win condition, no rounds, no HUD,
  no respawn. Deliberately deferred through slices 1–6 because a stage has to be
  right before rules can be written, and that is now true. The smallest honest
  version is: KO at zero HP, best of three, a two-bar health HUD. It needs L7
  first — rounds nobody can win are not rules.
