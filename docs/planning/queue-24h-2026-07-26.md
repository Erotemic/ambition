...
- ◐ **D12** (Tasks 7 + 12) "supported hosts" means desktop. No android or wasm
  ...
  → the cheap next step is a `--target wasm32-unknown-unknown` CHECK job in
  `run_tests.py` (not a test run — there is no wasm runner here), so the build
  that just came back cannot rot again unnoticed. That is a smaller commitment
  than an android crate and it is the thing that failed silently for four days.
