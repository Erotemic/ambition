...
- ✔ **U1 sprite-sheet METADATA was compile-time engine data, so a third party
  ...
  **Two candidate seams, and the choice is a real architecture decision:**
  ...
  ▢ **what is still not proved: pixels on screen.** The fixture's windowed face
  compiles and the whole chain resolves, but no test in this repo renders the
  consumer's character and looks at it. That wants the `RenderMode::Headless`
  treatment the in-repo demos get (`ov1_draws_the_world`), from outside the
  workspace — a real row, and a different one from "the art exists".
