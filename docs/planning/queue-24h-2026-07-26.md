...
- ◐ **X9** migrate match/secondary-fighter seating. ⚠ **not a rename.**
  ...
  ▢ **still open:** seating should read the resolved identity DIRECTLY rather
  than write a placeholder and be corrected. Two passes that now agree is better
  than two passes that did not, and it is still two passes.
- ◐ **X10** NPC/enemy construction where the same authority applies.
  ...
  kit alone.
  ▢ **what is left is the question, not the plumbing:** should an enemy's
  ARCHETYPE kit or its CHARACTER's authored kit win when both exist? Today the
  projection wins, because it runs after the spawn. That is defensible and it has
  never been decided — and it is the same "who owns the kit" question C3 asked
  about players, arriving from the enemy side.
