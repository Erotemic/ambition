...
- ▢ **L19 no STAGE seats more than two.** L17 proved a 2v2 works through
  seating; nothing a player can pick offers one. The versus arena hardcodes two
  participants and `SlotControls` slots 2 and 3 have never carried a real
  device. A 2v2 route would exercise both, and `seat_for` already spreads four.
...
- ▢ **L23 a latent despawn/insert race in `bind_worn_character_presentation`.**
  Surfaced by adding a system to the render chain: a system that spawns nothing
  in the failing scenario was enough, so the hazard is pre-existing and only
  needs a flush boundary to land wrong. `insert(Sprite, Anchor, CharacterAnimator,
  PlayerSpriteBaseline, PlayerSpriteCharacter)` targets an entity that is
  despawned in the same frame. The fix is for the binder to queue handled
  (`queue_silenced`) or to not target entities scheduled for despawn — but it
  should be diagnosed properly first: WHICH despawn wins the race matters more
  than silencing the symptom.
  ⛔ do not "fix" it by re-ordering the chain until that is known; the ordering
  is what hides it, not what causes it.
