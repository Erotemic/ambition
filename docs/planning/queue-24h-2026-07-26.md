...
- ▢ **F0e** `DeathPolicy::Unbounded` is now *reachable* and still has no
  ...
  actually launches far enough to reach a blast zone, and that is untested. Do
  not ship a versus mode nobody can win: tune the launch first, with a test that
  measures how far a clean smash sends a body, THEN flip the policy.
...
- ▢ **F0g** LDtk cannot author a blast margin. `RoomEmission` (the LDtk
  conversion's own struct) has no such field, so every LDtk-authored room takes
  the 200px default. Per Jon's rule — missing a concept means ADD IT TO LDTK —
  this is the authoring half of F0b and it is not done.
