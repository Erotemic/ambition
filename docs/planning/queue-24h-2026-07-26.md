...
- ▢ **D8** (Task 2) the CONTACT family never rewinds.
  `collision_invariant_oracle` is the contact instrument and every sim in it is
  plain fixed-tick — no `with_sync_test_rollback_settings` anywhere in the file.
  Damage, transition and action all have rewound scenarios; contact does not.
