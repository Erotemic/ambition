...
- ▢ **I3** `write_global` delegates to `SfxWriter::write`, which stamps the HOST
  ...
  milestones, act clear, monitor pop; Mary-O's brick smash) is attributed to
  whoever is hosting rather than to the course that authored it. Needs a third
  explicit operation beside body-owned and session-global: `write_from(provider,
  message)`.
- ▢ **I4** moveset strike volumes are spawned with no `SimId`
  (`crates/ambition_combat/src/moveset/mod.rs`), so every anonymous carrier
  hashes to the same fallback identity and two simultaneous hitboxes with SWAPPED
  owners are indistinguishable — for exactly the `Hitbox`/`StrikeVolume`/
  `HitboxHits` carriers H4 added the pair probe for.
