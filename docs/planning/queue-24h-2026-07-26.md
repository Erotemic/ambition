...
- ▢ **D18** ⚠ a hole in the instrument, found by the above and MORE valuable than
  the defect: invariant 1 tests the body's CENTRE only, so a body embedded up to
  half its height in a solid is invisible to it. `stagger_steps` had one standing
  15px inside a step across whole episodes and the oracle said nothing. A widened
  test (AABB overlap with a strict `<`, since a resting body's bottom equals the
  floor's top exactly) would see it — but it must be MEASURED across all 72 rooms
  first, because if it surfaces a pile the promotion above becomes the bug hunt
  §6.1 says it should not be.
