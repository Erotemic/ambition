...
- ▢ **D11** (Task 4) CC3's illegal-state coverage is still a diagnostic rig.
  Approach item 2 says to promote it to enforcement "when its documented
  behavioral preconditions are met", and nobody has re-checked whether they now
  are — so it is neither executable nor explicitly deferred, just unexamined.
