...
- ◐ **L24 sweep the other deferred reconciles that target session-scoped
  ...
  ▢ **remaining candidates, each needing a per-site decision:**
  `actors/overlays.rs` (`SpriteOriginalState` insert/remove on render visuals
  that `despawn_dead_dynamic_feature_visuals` can retire), `actors/boss.rs`
  (boss visual insert), `gate_portal_visuals.rs` (two sites).
  ▢ **the real want is a REPRODUCTION harness**, not more reasoning: a fixture
  that runs a pass and then despawns its targets before the flush would turn
  every one of these from an argument into a test. That is the thing worth
  building next in this area.   ⚠ do not blanket-convert: `try_insert` on a target that should ALWAYS exist
