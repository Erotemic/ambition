...
- ▢ **F0f** side and ceiling blast zones. The gate measures distance past the
  world AABB along `frame.down()` only, so walking off the SIDE does not kill —
  a body knocked sideways dies only once its arc brings it below the world.
  Correct for a platformer pit, incomplete for a platform fighter, where the
  side blast zone is where most stocks are actually lost.
