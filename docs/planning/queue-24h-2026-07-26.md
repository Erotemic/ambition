...
- ✔ **P6 the evidence parser counted QUALIFIED verdicts as complete.** Substring
  ...
  ▢ **NOT fixed, recorded:** a row with several citations still passes when ONE
  survives, so evidence for a completed clause can vanish while an unrelated
  citation keeps the row green. Binding a citation to the clause it supports
  needs clause-level parsing, and it wants its own row rather than a claim.
...
- ✔ **U1 sprite-sheet METADATA was compile-time engine data, so a third party
  ...
  **Two candidate seams, and the choice is a real architecture decision:**
  ...
  ▢ **stage B, deliberately not done here:** the COLLISION path
  (`sprite_body_collision_for_character_id_in` and the three
  `actor_clusters`/`spawn_actors` sites behind it) still reads only the baked
  index, so a consumer's character renders from its own sheet and takes the
  DEFAULT collision box. That is the same fallback any character without a baked
  sheet has always had — not a new inconsistency — but it means "authored sheet
  drives the hitbox" is still engine-only, and the row should not be read as
  claiming otherwise.

  ▢ **and the art itself is still not proved.** Nothing here decodes a PNG: the
  fixture ships no image, because this repo does not commit binary art. What is
  closed is that a consumer can NAME and DESCRIBE its art; what is open is a
  test that renders it (T2's remaining half, which wants generated art).
