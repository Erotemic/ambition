...
- ▢ **F0e** `DeathPolicy::Unbounded` is now *reachable* and still has no
  ...
  **Tried, measured, and NOT landed.** Setting `carried_run` to the launch's
  ...
  ⛔ **AND THE FIX IS NOT LANDED, because it has a second-order defect.**
  Reverted after the suite, with everything measured. Conserving the whole
  launch until input/landing keeps a struck body IN CONTACT WITH ITS ATTACKER:
  it no longer gets shoved clear and stopped, it slides along, and each time the
  i-frames lapse it is hit again. Each of those hits arms a hitstop, and the
  player-involved hitstop beat is a GLOBAL clock rule, so the simulation clock
  starves. The equipment oracle measured it exactly: the route still completes
  (all four events by frame 164, same 5.7s wall clock as clean), but GGRS
  advanced **300 times in 600 harness steps** where a clean tree advances more
  than 600 — half the sim ticks per step, from hitstop alone. Isolated to the
  single `*carried_run = …` line by disabling just that line and watching the
  oracle go green.
