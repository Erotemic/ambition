...
- ◐ **Y1 ROLLBACK COVERS ONE SEAT, and the game now seats four.**
  ...
  **This is not a bug in couch play.** Local play predicts nothing, so a seat
  ...
  ▢ **still owed — the ACCEPTANCE half, which is the point:** nothing yet DRIVES
  two streams through a rewind and compares. `RollbackMode::SyncTest` in the
  harness carries `check_distance` and `max_prediction_window` and no player
  count, and `AgentAction` is one stream, so the harness needs a second before
  the two-seat claim is proved rather than plumbed. That is netcode.md's step 2
  proper, and it is the Campaign 2 precondition the Campaign 1 closeout named.
