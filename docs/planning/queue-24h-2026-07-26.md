...
- ▢ **D10** (Task 9) there is no `CameraPolicy` and no `AnimationPolicy` type. A
  provider declares presentation PROFILES but cannot declare either policy the
  task is named for; camera is one host-owned plugin. Name what a camera policy
  IS before declaring it — the no-central-branches half is already satisfied.
...
- ▢ **K6** `SimId` concatenates unescaped segments, so `placement("giant/0")` and
  `spawned(placement("giant"), 0)` both produce `placement:giant/0`. Slash-bearing
  authored placement ids are real repository content. A collision merges distinct
  entities on restore, misattributes reference probes, and can despawn the wrong
  thing. The same applies to `strike_volume` when a move id contains a delimiter.
  Needs escaped or length-prefixed segments — and, per `sim_id.rs`'s own doc, a
  test that the encoding is INJECTIVE rather than merely readable.
