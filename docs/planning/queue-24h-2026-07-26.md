...
- ▢ **D6** (Task 11) **the save file has NO VERSION FIELD.** `save.rs` contains
  no `version`, no migration, no `migrate`. The first time the shape changes,
  every existing player file is a parse error or — worse — silently deserializes
  into the wrong meaning. Cheapest catastrophic-failure class on the roadmap, and
  it must close BEFORE anything ships, because afterwards the untagged files
  already exist. A version tag plus one exercised migration step is enough; the
  migration does not have to DO anything yet.
