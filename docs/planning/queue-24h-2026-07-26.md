...
- ◐ **L24 sweep the other deferred reconciles that target session-scoped
  ...
  ▢ **still unswept, and cheap now** (the witness harness and the read-model
  `from_rows` constructors were the expensive parts): the actor overlay /
  nameplate families where the write is a SPAWN of a child rather than an insert
  on a queried entity — a different hazard shape (an orphaned child rather than a
  panic), so it wants its own probe rather than this one pointed at it.
...
- ✔ **M4 the roadmap's per-task Status lines drift, and nothing catches it.**
  ...
  row is skipped.
  ▢ **the ADRs remain unswept.** A third shape, and worth a look before a third
  parser — ADR status lines may already be uniform enough for the task parser.
