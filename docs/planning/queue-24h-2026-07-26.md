...
- ▢ **F0p** `RoomMetadata` now carries five optional numbers that all mean
  "distance in pixels" and are all validated the same way (`>= 0`, else None).
  That is three copies of the same `filter` in the reader and three fields the
  composer has to remember to forward. One more and it wants to be a struct —
  note it now, before the fourth arrives and the copy becomes the pattern.
...
- ▢ **F0h** a body with active i-frames that crosses the blast margin is
  IGNORED by the damage pass and survives that frame. It self-heals, because the
  gate is a position test and re-fires every tick until the i-frames lapse, but
  "you died a few frames later than you left" is a real behaviour and nothing
  pins it.
