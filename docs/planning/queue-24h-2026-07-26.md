...
- ▢ **F0m** no stage in the repo authors a side or ceiling blast zone except
  the versus arena, and the versus arena is a Rust room. The LDtk half is wired
  and DEFINED in all five projects, and zero levels set either field — so the
  authoring path is proven by unit test and not by a single shipped room. The
  Sanic speedway is the obvious first customer (a body flung off the course at
  speed currently sails until its arc drops it), and it IS an LDtk course, so
  it would exercise the level-field path end to end.
- ▢ **F0n** the blast margins are invisible. A stage author sets a number and
  has no way to see where the line is short of throwing a body at it. The debug
  overlay already draws world geometry; the three margins are four line
  segments in the body's frame, and drawing them is what turns "authored" into
  "authorable".

  Scouted: `draw_room_bounds` in `ambition_render::rendering::debug_viz` is the
  home — the room's outer edge and its kill line are the same idea one step
  apart, and `show_room_bounds` is already the toggle. The one thing that must
  not be fudged is the FRAME: the gate measures along `frame.down()`, so an
  overlay that draws `y = size.y + margin` is right only under down-gravity and
  quietly lies in the Noether Chamber. `gravity_dir_or_default(Option<&Gravity
  Field>)` is the existing accessor, and the system does not currently take it.
