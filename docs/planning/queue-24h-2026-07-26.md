...
- ◐ **F6** = **C3**. Marking A1 / slice 7.6 complete overstated the case; the
  ...
  inherited.
  ▢ **what is genuinely left of C3, stated precisely so the next person does not
  re-derive it:** the ACTION SET, the identity baseline and movement tuning are
  still `CharacterCatalog`'s. The moveset and hurtbox doc are the registry's. The
  remaining work is not "wire the registry in" — it is deciding whether an action
  set authored on a `CharacterDefinition` should outrank the catalog row the way
  the moveset does, and that is a question about who owns a character's KIT, not
  a plumbing task.
...
- ▢ **W1 the M4 guard cannot check a claim whose content is that nothing
  ...
  **The mechanism that would work already exists in this repo and is not
  ...
  ▢ **remainder, and GPT's fourth review asks for it explicitly:** dependency-edge
  contracts from `cargo metadata` (crate A must not depend on crate B). Grep
  cannot express a dependency edge, and it is the strongest enforcement
  available short of the compiler. Tracked as **G4** in
