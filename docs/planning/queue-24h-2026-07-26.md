...
- ◐ **C4** no versus mode in a running game; the fight is proven only in a test.
  ...
  ▢ **slice 2: a ROUTE.** Seating is reachable only from code; a stranger cannot
  watch two CPUs fight. Needs a shell route that inserts a roster and activates a
  stage — which is what turns C4 from "the engine can" into "you can see it".
  ▢ **slice 3: player vs cpu.** A `Human` binding needs a slot-to-body assignment
  (`Brain::Player(slot)`); seating deliberately skips those today, and
  `a_human_participant_is_left_for_the_couch_slice` is the test that will turn red
  when it stops skipping.
