...
- ✔ **M4 the roadmap's per-task Status lines drift, and nothing catches it.**
  ...
  ▢ **the same treatment for `docs/planning/status.md` and the ADRs**, which
  carry the same kind of claim and have the same failure mode. The script takes
  a path argument for exactly this.
