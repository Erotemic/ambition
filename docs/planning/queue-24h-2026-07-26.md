...
- ▢ **F0h** **i-frames delay a blast-zone KO, and they should not delay it at
  all.** `resolve_body_hit` opens with
  `if !combat.vulnerable() { return Ignored; }`, and that early-return sits
  BEFORE the `killed` / `left_the_world` logic in `apply_actor_hit` — so a body
  that crosses the blast line while invulnerable keeps falling and nothing
  happens.

  The window depends on which path armed it, and the two differ by almost 4×:
  an ACTOR arms `ACTOR_DAMAGE_IFRAME_S` = **0.2s** (12 frames), while the
  PLAYER path arms `feel.knockback_invulnerability_time` = **0.75s** (45
  frames) — and it is the launch itself that arms it, so being knocked off the
  stage is precisely the case that buys the longest reprieve. It self-heals,
  because the gate is a position test that re-fires every tick, which is
  exactly why nobody noticed: the body dies eventually, far below where it
  should have.

  This is the SAME argument, for the third time. `DeathPolicy::Unbounded` says
  "the meter never kills me, the world does", and filtering the world's kill
  through the meter's policy left such a body immortal. Filtering it through
  i-frames leaves it immortal for 45 frames. The blast zone kills whatever it
  catches: you cannot be invulnerable to the edge of the world, and shielding
  should not save you either.

  The shape: an `unstoppable` argument on `resolve_body_hit` that bypasses the
  vulnerability gate and the shield block — and ONLY those. The `!alive()`
  early-return must stay, because the blast gate re-fires on corpses and the
  alive-gate on the producer side depends on the resolver refusing them. THREE
  production callers, not two — `damage_apply` (the player), `damage/actor_hit`
  and `damage/boss_hit` — each passing
  `matches!(event.source, HitSource::LeftTheWorld)`. A boss is a body and the
  edge of the world does not care what it is.
