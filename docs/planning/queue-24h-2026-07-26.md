...
- ▢ **Y1 ROLLBACK COVERS ONE SEAT, and the game now seats four.**
  ...
  **This is not a bug in couch play.** Local play predicts nothing, so a seat
  ...
  → the slice: `build_sync_test_session` takes a player count, `publish_ggrs_input`
  publishes per-handle instead of `first()`, and a test drives TWO input streams
  through a rewind and compares. That is also the fingerprint-comparison
  precondition the Campaign 1 closeout named for Campaign 2.
