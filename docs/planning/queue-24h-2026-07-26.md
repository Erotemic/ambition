...
- ▢ **U1 sprite-sheet METADATA is compile-time engine data, so a third party
  cannot author a character sheet at all.** This is the real blocker behind T2,
  ...
  **Two candidate seams, and the choice is a real architecture decision:**
  ...
  ⛔ **do not start this by picking (2) because it is easier to type.** The
  question is which one lets `fixtures/external_consumer` author a character
  END TO END without the engine crate learning its name, and that is answerable
  by trying (1) first and recording where it breaks.
