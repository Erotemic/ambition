...
- ✔ **G2b** (2026-07-27) — **and most of the row was wrong.** It named eleven
  ...
  at every reader.
  ▢ **the honest residue:** 114 presence-only probes remain, 36 of them derived.
  The ones left are not entity-handle cases — they are authored payloads, small
  values, and republished caches — so closing them is a different kind of work
  (value projections, one at a time, where the value is worth folding) and does
  not belong under this row.
...
- ◐ **N9 nothing would catch a NEW render/sim crossing.** N4's fix removes the
  ...
  ▢ **the residue: app-shell rollback state is unswept.** Two candidate shapes,
  neither obviously right — host the shell app on `Fixed60Hz` in a test
  composition (does the shell survive it?), or walk the `Update` schedule's
  per-system access (Bevy 0.18 keeps `FilteredAccessSet` crate-private, which is
  why the component sweep chose entity composition instead). Decide which before
  writing either.
