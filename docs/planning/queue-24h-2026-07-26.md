...
- ◐ **D7** (Task 6) `player_schedule.rs` named `ambition_actors::` leaf systems
  **30 times** in one chain, the literal negation of "runtime orders semantic
  ...
  ▢ REMAINDER: `PlayerInput` is one set of six; `WorldPrep`, `PlayerSimulation`,
  `RoomTransition`, `Combat` and `PresentationSync` still take leaf names. Same
  treatment, one set at a time — `Combat` next, since that is where the move
  clock, the damage path and the hitbox publisher all order against each other.
