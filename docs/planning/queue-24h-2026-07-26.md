...
- ▢ **L24 sweep the other deferred reconciles that target session-scoped
  bodies.** L23 was one instance of a general shape: a presentation or reconcile
  pass that `commands.entity(body).insert(...)` where `body` can be despawned by
  session or room teardown in the same frame. `action_scheme.rs` and
  `posed_body.rs` already use `try_insert` and say why; nobody has checked
  whether the rest do. A grep for `commands.entity(` followed by `.insert(` in
  render and character-sprite passes would list the candidates in a minute, and
  each is either safe (target is not session-scoped) or one word from safe.
