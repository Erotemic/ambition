...
- ◐ **D12** (Tasks 7 + 12) "supported hosts" means desktop. No android or wasm
  ...
  ▢ **still owed:** android, an actual browser run, and the decision about
  whether `web`'s reduced feature set (no `basic_shell_presentation`) is the
  product or a stopgap.
...
- ◐ **F6** = **C3**. Marking A1 / slice 7.6 complete overstated the case; the
  ...
  *"an explicitly authored `ActionSet` on `CharacterDefinition` should outrank the
  ...
  ▢ **still `CharacterCatalog`'s:** the remaining movement TUNING (as distinct
  from the model), and the
  identity baseline's remaining fields. Deliberately NOT folded into this slice —
  see [architecture-campaign-2026-07-28.md](architecture-campaign-2026-07-28.md)
  revision R-a: motion tuning is read by the solver on a different cadence, and
  including it makes the first commit un-reviewable in one sitting. Tracked as
  X2/X8–X13 there.
...
- ⊘ **X13** confine the legacy catalog to preparation, or delete it. **REFRAMED,
  ...
  ▢ **the honest residue, named so nobody re-derives it:** the remaining movement
  TUNING (as distinct from the motion MODEL, which moved). Everything else on
  that list is a different question about who owns NAMES and ART, and it wants
  its own row and its own argument rather than inheriting C3's.
