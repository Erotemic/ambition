...
- ✔ **L15 Task 11 is PARTIAL: startup does not open the checkpoint's room.**
  ...
  **A guard phrased as an outcome can still be satisfied by the narrowest case
  ...
  the other end.
  ▢ the guard check itself should be tightened to cross a room boundary, or it
  will keep passing for the narrow case. The
...
- ✔ **L18 the versus HUD is text where it should be bars, and there is no
  ...
  ▢ **still open (L20):** the gauge is the first non-string thing the declared
  HUD can draw, and it is drawn by ONE hardcoded shape — a 6px bar in the slot's
  own colour. A portrait, an icon row, a stock counter and a timer all want the
  same seam and none of them is a fraction. Before adding a second one-off,
  decide whether `HudReadout` grows a `kind` or whether presentation gets a
  per-slot widget declaration.
...
- ◐ **L24 sweep the other deferred reconciles that target session-scoped
  ...
  ▢ **remaining candidate:** `actors/boss.rs` (boss visual insert). Deliberately
  NOT converted on the strength of "same shape as the others", which is
  reasoning, and this row exists because reasoning was not enough.
  ⚠ **the probe is expensive and here is why**, so nobody rediscovers it: the
  pass early-outs unless `GameAssets` resolves a boss sheet AND
  `Assets<Image>` already holds that sheet's page-0 texture. A fixture therefore
  has to build a `CharacterSpriteAsset` with pages and register a real `Image` —
  everything before that point is vacuous, exactly like the portal probe was.
  ▢ **also worth doing:** point the harness at the character-sprite passes
  (`posed_body.rs` already uses `try_insert`; the rest are unaudited) and at the
  actor overlay/nameplate families.   ⚠ do not blanket-convert: `try_insert` on a target that should ALWAYS exist
...
- ▢ **M2 the honest residue of transactionality: can `spawn_contents`' COMMANDS
  fail after the infallible boundary?** `apply_to_world` promises no fallible
  LOOKUP; it promises nothing about the commands it queues, and the L23/L24
  despawn-then-insert class lives exactly there. `deferred_write_safety` now
  exists to answer this — point it at the construction path. If commands can
  fail mid-commit, THAT is the real partial-state vector and it is much narrower
  than a staging world.
- ▢ **M3 audit the other three lifecycle paths' failure handling.** Five paths
  share the transaction; two were checked (`lifecycle_commit`, `reset`) and
  `setup` was judged. The transition and reconstruction paths have not been
  looked at, and "shares one transaction" says nothing about what each does when
  the transaction refuses.
- ▢ **M4 the roadmap's per-task Status lines drift, and nothing catches it.**
  Task 4's said "the CC3 GATE is diagnostic, not enforcing" for a day after the
  gate started enforcing; Task 5's described a boundary that exists; Task 11's
  said MET while the feature only worked in one room. Three stale claims in one
  document, all found by reading the source instead of the doc. A status line
  that cites a TEST NAME can be checked mechanically — a line that describes a
  situation cannot, and that is the difference worth designing for.
