...
- ◐ **C4** no versus mode in a running game; the fight is proven only in a test.
  ...
  **two Mary-Os**. The test passed because it asserted both fighters were PRESENT,
  ...
  ▢ **slice 4: local couch PvP.** Two human seats need two device slots bound to
  two bodies; adoption handles exactly one today (`player.single_mut()`). ⛔ still
  no matchbox — Jon's order ends "and only then netcode".
