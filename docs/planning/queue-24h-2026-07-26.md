...
- ◐ **L24 sweep the other deferred reconciles that target session-scoped
  ...
  ▢ **remaining candidate:** `actors/boss.rs` (boss visual insert). Same
  treatment — write the probe, watch it go red, then fix — and check the probe
  is not vacuous before believing it.
