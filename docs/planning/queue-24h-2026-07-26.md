...
- ◐ **L24 sweep the other deferred reconciles that target session-scoped
  ...
  ▢ **still unswept:** the actor overlay / nameplate families, and the
  `ambition_render` passes outside `rendering/actors`. Cheap now — the witness
  harness and the read-model `from_rows` constructors are the expensive parts and
  they exist.
