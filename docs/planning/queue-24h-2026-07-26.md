...
- ▢ **Z′12 — every capture I took shows a DEV build, so it overstates the noise.**
  ...
  → give it `--dev-overlays` (OFF by default): a verification screenshot should
  show the PRODUCT, and the debugging use that wants nameplates can ask. The
  switches already exist — `UserSettings.gameplay.debug_hud_visible` and
  `DeveloperTools.show_hud`.
