...
- ◐ **C3** the prepared authority coexists with the six old seams instead of
  ...
  ▢ **What remains is CONTENT.** Every production `register_character` call today
  passes only `.with_sheet(...)` — Mary-O's three tiers, Sanic's two forms — so the
  projection is a live seam with no production caller, which is the exact shape
  this queue keeps criticising. Moving one real character's moveset out of its
  catalog fragment and into its `CharacterDefinition` is the next row, and it is
  what turns C3 from "the engine can" into "the game does".
...
- ◐ **D18** ⚠ a hole in the instrument, found by D11 and more valuable than the
  defect: invariant 1 tests the body's CENTRE only, so a body embedded up to half
  its height is invisible to it. `stagger_steps` had one standing 15px inside a
  step across whole episodes and the oracle said nothing.
  ✔ **MEASURED** (`measure_how_many_frames_end_with_a_body_overlapping_solid`,
  `#[ignore]`d): **116 of 21,600 frames — 0.54%** — end with the body overlapping
  solid material by more than 1px, across 12 of 72 rooms, deepest 24.2px.
  ⊘ **Verdict: do NOT widen invariant 1 into the gate yet.** 116 frames is neither
  "a handful of authoring mistakes" nor noise; adopting it now is precisely the bug
  hunt §6.1 says the promotion must not become. The measurement stays as the
  artifact, and the widening waits on D19.
- ▢ **D19** the lead the measurement produced, and the reason D18 is deferred
  rather than dropped: the overlap depth is **exactly 8.6px in seven unrelated
  rooms** (`central_hub_complex`, `hall_of_bosses`, `overfit_annex_climb`,
  `scroll_lab`, `stagger_steps`, `under_town_pipes`, `volatile_cache`), 10 frames
  each. Seven rooms agreeing to one decimal is ONE shared cause, not seven
  authoring mistakes — most likely a body resolving into a block for a single tick
  and being pushed out on the next, the same one-frame-late shape the
  `stagger_steps` dash showed. Find the cause, and invariant 1 can probably be
  widened for free.
  ⚠ `portal_lab`'s 24.2px is almost certainly legitimate — a straddling body
  mid-transit, which invariant 2 already handles as its own case — so exclude it
  from the hunt rather than counting it as evidence.
