...
- ▢ **D16** the load-plugin composition hazard, found while writing D9: whether the
  engine group or the host owes `AmbitionLoadPlugin` is an undocumented rule, and
  adding it twice is a hard Bevy panic. Both in-repo demos were edited when it
  moved; the external fixture — invisible to a repo grep — sat red until someone
  read the panic. An engine a stranger composes cannot have unwritten composition
  rules whose violation is a crash.
