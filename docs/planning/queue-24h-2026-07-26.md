...
- ◐ **F6** = **C3**. Marking A1 / slice 7.6 complete overstated the case; the
  ...
  *"an explicitly authored `ActionSet` on `CharacterDefinition` should outrank the
  ...
  ▢ **still `CharacterCatalog`'s:** movement tuning / motion model, and the
...
- ✔ **S2 the round was decided and then went on being fought.** P1 fixed the
  ...
  → the fighters are marked `ScriptedControl` on the deciding tick and released
  in `begin_round`. Motion and playback still decelerate into the card, which is
  the beat P1 kept; what stops is DECIDING.
...
- ✔ **V2 a behavioural guard, not a grep.** `every_manifest_family_carries_a_
  ...
  ▢ **still outside it:** world / data / font entries, which are ENGINE paths no
...
- ▢ **X12** delete runtime arbitration. ⚠ amended by R-h: the ability-derived
