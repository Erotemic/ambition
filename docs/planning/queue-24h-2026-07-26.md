...
- ✔ **Z′3 — CLOSED, and it was not a HUD problem at all.** Jon looked at the
  ...
  default `desktop_dev` feature set and a keyboard session publishes no touch
  footprint, so EVERY desktop run drew a virtual d-pad in the corner. It landed on
  the versus scoreboard because that is where the scoreboard is.
