...
- ✔ **L14 the versus rules run on the RENDER clock, outside the sim schedule,
  ...
  **A comment asserting a system is order-free is a claim about every line
  ...
  they can disagree.
- ▢ **L17 nothing tests a 2v2, which is the case teams exist for.** L9 landed
  `MatchTeam` and unit-tested the relation, but every stage that seats a roster
  is 1v1, so the interesting property — two fighters who CANNOT hit each other
  standing next to two who can — is proven only in a pure function. The versus
  arena seats `roster.participants.len()` and `seat_for` already spreads four
  fighters two per side, so a 2v2 roster is a handful of lines away. It would
  also be the first thing to exercise `SlotControls` slots 2 and 3, and the
  first real test of whether the seating latch's per-seat retry holds up beyond
  two.
- ▢ **L18 the versus HUD is text where it should be bars, and there is no
  round-start beat.** `publish_versus_hud` writes "Long Guard 47/60" because the
  declared HUD seam has no bar widget. Both are presentation and neither touches
  the rules — which is exactly why they were deferred — but a health BAR is most
  of what makes a fighting game readable at a glance, and a "ROUND 2 — FIGHT"
  beat is how a player knows the freeze ended. The seam needs a slot kind that
  is not a string.
