...
- ✔ **L8 the versus stage has no rules.**
  ...
  ▢ **still open:** a round-start COUNTDOWN. Today the round opens on a
  "ROUND n — FIGHT" card that fades while play is already live. A numeric 3-2-1
  is the genre convention and it implies a FREEZE, which contradicts the standing
  decision that a round must not begin with a stutter — so this is a feel call
  with two defensible answers, not a missing feature. Decide it with a controller
  in hand, not from a queue row.
...
- ◐ **T2 consumer-owned art still cannot reach a CHARACTER.** The overlay reader
  ...
  agreed to.
  ▢ **and it stops there — but NOT for the reason I first wrote down.** I closed
  the addressing and recorded "the rest needs a PNG this repo will not commit".
  Then I traced the rest of the pipeline, and the missing art is the smaller
  half. See U1.
