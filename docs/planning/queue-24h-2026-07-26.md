...
- ◐ **D2b** the two findings D2's proof produced, both engine-side.
  ...
  every frame, and the only diagnostic is a raw checksum mismatch. The engine's
  own harness dodges it by construction (`with_start_room` builds first), so no
  in-repo test can reach it. A consumer gets frames 2–4 mismatching forever and no
  hint that construction inside the rollback window is the reason.
