...
- ▢ **D7** (Task 6) `player_schedule.rs` names `ambition_actors::` leaf systems
  **30 times** in one chain, which is the literal negation of "runtime orders
  semantic sets rather than naming leaf systems". Not cosmetic: J1's
  `GgrsSchedule` cycle happened because a system's set membership is discoverable
  only by reading which `add_systems` block it sits in.
