...
- ✔ **M4 the roadmap's per-task Status lines drift, and nothing catches it.**
  ...
  ▢ **the same treatment for `docs/planning/status.md` and the ADRs** — but it
  is NOT free reuse, checked 2026-07-27: `status.md` has no `### Task N`
  headings and no `**Status**` lines at all. It is a narrative page whose claims
  live in bullets, so the parser does not apply and pointing the script at it
  would report "checked 0 rows" and look clean. Either give `status.md` the
  roadmap's row shape, or give the script a second parser — and decide which
  before writing either, because "0 rows checked, no problems" is the most
  misleading output a guard can produce.
...
▢ **Still open from this review's own reading:** N4's fix removes the render/sim
crossing by construction, and there is no test that would catch a NEW one. The
resource sweep names unrewound resources and the message oracle names unrewound
channels; neither notices a rollback-registered resource being written from
`Update`. That is a real gap and the instrument for it is not obvious — a
change-detection probe over rollback-registered resources during a
sim-schedule-free frame is the shape, and it wants its own row before it is
attempted.
