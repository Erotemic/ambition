...
- ◐ **D7** (Task 6) `player_schedule.rs` named `ambition_actors::` leaf systems
  **30 times** in one chain, the literal negation of "runtime orders semantic
  ...
  ▢ REMAINDER: three sets left — `WorldPrep`, `RoomTransition`,
  `PresentationSync`. `WorldPrep` next (movement kernel + feature ticks); it is the
  biggest and the one an external consumer is most likely to need a slot in, since
  Outlander's ridge gate already lives next door in `PlayerSimulation`.
