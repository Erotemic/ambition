...
- ▢ **Y″4 — `Vitals.mass` is authored and unconsumed.** Zero runtime reads
  (grep, 2026-07-29). Marked in the source so nobody authors against it believing
  it works. Whether mass should drive knockback, mounting, momentum or collision
  response is a product decision, not a cleanup.
...
- ▢ **Z′4 — `Interact (F)` is offered on a fighting stage** where there is nothing
  to interact with. The prompt derives from the body's action scheme and Interact
  is granted to every controllable subject unconditionally; a match has no use for
  it. Either the scheme learns that a seated fighter is not an explorer, or the
  versus route declares which prompt family it wants.
