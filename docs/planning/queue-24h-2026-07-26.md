...
- ◐ **D12** (Tasks 7 + 12) "supported hosts" means desktop. No android or wasm
  ...
  ▢ **DECISION STILL OWED, and now it can be an informed one.** Compiling is not
  running: nobody has opened the web build in a browser, no wasm target is in the
  suite, and `web` omits `basic_shell_presentation` — so the universal
  pause/settings menu, including the title-screen audio rows landed the same day,
  is ABSENT on web. That last one is the interesting part of the decision: it
  means "supported host" currently implies a different feature set, not just a
  different target, and per-host budgets cannot mean anything until somebody says
  which set is the product.
