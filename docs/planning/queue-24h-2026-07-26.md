...
- ▢ **Y′9 — the topology has no generation tie to the roster.** Y′2 gave it one
  owner and one lifetime; the review also asks that topology, roster, GGRS
  settings and latches all be stamped with the SAME generation so a disagreement
  is detectable rather than merely unlikely. That is the same shape as H6's
  `CharacterCatalogGeneration` with no consumer, and should be done with it.
