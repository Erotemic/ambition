...
- ▢ **F0e** `DeathPolicy::Unbounded` is now *reachable* and still has no
  ...
  **Found, and it is the fourth instance today of the same shape.** The per-frame
  ...
  ✔ **Wired.** `apply_body_hit_reaction` now sets `carried_run` to the launch's
  side component, the way the portal fling does. Same smash, same fighter, same
  lip: `|dx|` 15.5px → **111.8px**, and they leave the stage. The per-frame
  trace is the whole story — `vel.x` used to fall 62.5/frame to zero by t+7, and
  now holds flat at 329.7 while gravity does its work on `vel.y` alone.

  This is a CORE FEEL change and it touches every struck body, not just
  fighters, so it needs the full suite and not just the versus tests.
