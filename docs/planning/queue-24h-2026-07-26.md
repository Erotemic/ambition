...
- ◐ **C4** no versus mode in a running game; the fight is proven only in a test.
  ...
  **two Mary-Os**. The test passed because it asserted both fighters were PRESENT,
  ...
  ◐ **slice 4: local couch PvP — the ENGINE half is done; the DEVICE half is the
  whole remaining gap, and it is smaller than it sounds.**
  The engine was already ready and nobody had asked it: `SlotControls` holds four
  slots and `tick_player_brains` drives any body whose `Brain::Player(slot)` names
  one. What was missing was a seat that produces the second body. Seat 1+ now
  spawns its own body carrying `Brain::Player(slot)`, `LocalPlayer` and a
  `PlayerInputFrame` — the marker matters, because `sync_local_player_input_frame`
  skips a body without it and the symptom is a fighter that stands still, which is
  indistinguishable from an unplugged controller.
  ▢ **what is left: a WRITER for slot 1.** `populate_slot_controls` fills only
  `PlayerSlot::PRIMARY` from the one global `ControlFrame`, and its own docs
  already name the fix — *"co-op / netcode add their own writers for higher slots
  without touching this one"*. The cost is in `ambition_host`: the leafwing bridge
  produces ONE frame from one merged input map, and couch play needs one per
  gamepad. Anything that can write `SlotControls` directly — a test, an RL policy,
  a replay — already gets couch versus today.
