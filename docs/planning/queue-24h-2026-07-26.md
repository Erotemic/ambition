...
- ✔ **U1 sprite-sheet METADATA was compile-time engine data, so a third party
  ...
  **Two candidate seams, and the choice is a real architecture decision:**
  ...
  ▢ **stage C, still open:** attack VOLUMES. `authored_attack_volume_resolver`
  reaches gameplay through a fn-pointer resource in `ambition_combat`, which has
  no business learning a sprite-sheet type, and the polygons come from
  `file_root_registry()` — a SECOND baked index with the same gap as the first.
  A consumer's melee box is still engine-only, and closing it means deciding how
  that seam carries authored content, not adding another parameter.
