...
- ✔/▢ **Campaign 2 — the before/after HARNESS exists** (2026-07-31). Its gate
  ...
  * ▢ **next: migrate ONE domain** (R3). `ambition_projectiles` is the smallest
    at ~15 registrations and has no reverse dependency on the runtime. Move its
    registrations to a `ProjectileRollbackSchemaPlugin`, keep the dump
    byte-identical, delete the central lines. Do NOT strengthen probes in the
    same commit.
