...
**It found three more the moment it existed**, and they are budgeted rather than
...
no sprite), ability visuals (`sync_bubble_shield_visual`,
`sync_morph_ball_visual`, for abilities any body can be granted), the cutscene UI
(`sync_cutscene_ui`), and `sync_resolved_visual_quality` (without it
`ResolvedVisualQuality` never leaves its default, so a demo's quality settings
are inert). ▢ **`UNCLAIMED_BUDGET`, and the ratchet fails BOTH ways** — over is a
new one, under is a fix that did not tighten the budget in the same commit.

✔ **The projectile pair is FIXED and the budget is down to 4** (2026-07-31), and
