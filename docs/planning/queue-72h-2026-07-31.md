...
→ **new row: a CPU seat that names an unknown brain profile should be REFUSED,
not silently given a standing-still fallback.** `resolve_initial_brain` already
gets this right for placements — an override that does not resolve is a loud
`UnknownPreset`, *"never a silent fall back to the default"* — and the seating
path has the opposite policy for the same class of mistake.
