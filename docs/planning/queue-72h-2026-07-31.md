...
- ✔/▢ **Campaign 2 — the before/after HARNESS exists** (2026-07-31). Its gate
  ...
  * ▢ **a tidy the relocations deliberately did NOT do:** bespoke checksum
    projections (`room_set_checksum`, `ldtk_runtime_index_checksum`,
    `pending_player_hits_checksum`) still live beside the central function and
    are now `pub(super)` so their domain can reach them. A projection used by
    exactly one domain should follow it. R3 forbids doing that in a relocation
    commit, which is why it is a row and not a diff.
