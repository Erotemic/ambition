...
- ✔/▢ **Campaign 2 — the before/after HARNESS exists** (2026-07-31). Its gate
  ...
  * ▢ **remaining: actors (~101) and characters/primitives (~56).** Same script.
    ⚠ actors is the one to do carefully: it is the largest and its registrations
    are interleaved with body-cluster ones that must NOT move (a `bc::` type
    belongs to `ambition_engine_core`, not to the actors domain).
