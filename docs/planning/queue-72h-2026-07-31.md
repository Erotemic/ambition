...
- ✔/▢ **Campaign 2 — the before/after HARNESS exists** (2026-07-31). Its gate
  ...
  * ▢ **next: R1, extract the schema vocabulary.** The adapter lives in
    `ambition_runtime` because `AmbitionRollbackApp` does and the domain crates
    must not gain a runtime dependency. A genuinely domain-OWNED adapter needs
    the vocabulary in a crate the domains can depend on. Until then the owner
    label stays `ambition_runtime`, which is honest — the module is there.
