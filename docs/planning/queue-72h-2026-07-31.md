...
- ✔/▢ **Campaign 2 — the before/after HARNESS exists** (2026-07-31). Its gate
  ...
  * ✔ **R4 IS MET.** vfx (11) and items (1) migrated; `rollback/mod.rs` is **690
    ...
    all nine moves.
  * ▢ **R5: a dependency guard, so the central function cannot reacquire this.**
    An absence contract in the shape of the existing ones — `rollback/mod.rs` may
    not name `ambition_actors`, `_combat`, `_items`, `_portal`, `_projectiles`,
    `_characters`, `_encounter`, `_vfx` or `_platformer_primitives`. Without it
    the next registration lands wherever its author is looking, and the campaign
    un-does itself one convenient line at a time.
