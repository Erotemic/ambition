...
- ✔/▢ **Campaign 3B — the "one helper" half is BUILT, and better than the doc's
  ...
  * ▢ **still open: three respawn paths still use `transit_body`.** The boss
    revive (`features/ecs/reset.rs`) and two enemy respawns
    (`features/enemies/integration.rs`) each call it under a comment saying
    "respawn is a discrete transit (ADR 0024 authority)" — right about the POSE
    and silent about the maneuver state, which is exactly the shape the smash
    respawn had. They are restarts by their own words. Converting them is now
    SAFE (the tuning footgun is gone) but is a behaviour change to enemy state,
    so it wants its own slice and its own before/after.
