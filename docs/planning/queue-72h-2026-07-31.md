...
→ ▢ **new row: the fighter still loses every stock to itself inside ~16 s.** The
veto now fires, and a body that is still walking off after being told not to is
either (a) being vetoed too late — the veto reruns only on decision ticks, and by
then the momentum is committed — or (b) getting only one movement option, so
vetoing it leaves nothing and `apply_movement` is skipped entirely rather than
falling through to "stand still". (b) is checkable in ten minutes and is the
first thing to look at: `find(|o| !vetoed.contains(&o.verb))` returns `None` when
every offered verb is suicidal, and doing NOTHING is not among the offers.
