...
**⚠ ONE HALF IS OUTSTANDING AND IS NOT A ROUNDING ERROR: the attack kit has no
producer.** §13.2 wants `BrainSnapshot.attack_kit` filled from the body's real
`ActorMoveset`. The field exists and the rig CONSUMES it; both snapshot builders
fill it EMPTY, because `ActorMut` does not carry the moveset and the producer
needs it threaded into the actor query. A fighter with an empty kit plays
MOVEMENT ONLY — `generate_options` returns no attacks — which is honest
degradation rather than a wrong answer, but it means the ladder cannot yet
measure L3 against L2, which is the whole reason FB4b exists.

→ **next row on this thread:** thread `ActorMoveset` into `build_enemy_brain_snapshot`
and fill the kit from `spec.frame_data()` per attack row. Everything downstream
