...
- ✔/▢ **Campaign 2 — the before/after HARNESS exists** (2026-07-31). Its gate
  ...
  * ▢ **R1 is a DESIGN question, not a mechanical move — do not start it blind.**
    `registry.rs` imports `bevy_ggrs::{ComponentSnapshotPlugin, RollbackApp,
    LoadWorld, LoadWorldSystems, ResourceSnapshotPlugin}` and
    `crate::SimulationHost`. Extracting the vocabulary into a crate the domains
    can depend on therefore means **`ambition_projectiles` (a primitives-level
    crate) would gain a transitive dependency on `bevy_ggrs`.** That may be the
    right trade — or it may be the thing to avoid, given this engine's
    primitives are deliberately light and rollback is one of several session
    modes. The campaign says only "the schema layer must not depend on GGRS
    session HOSTING", which `SimulationHost` plainly is and the snapshot plugins
    arguably are not.
    → **the question to answer first:** can the registration vocabulary be
    expressed without naming a `bevy_ggrs` type — descriptors + a strategy enum
    the runtime interprets — so the domains declare and the runtime installs? If
    yes, R1 is a small crate and every domain owns its schema for real. If no,
    the adapters stay in `ambition_runtime` and "domain-owned" means
    module-owned, which is worth saying out loud rather than discovering later.
