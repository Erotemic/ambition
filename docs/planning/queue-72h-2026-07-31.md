...
- ✔/▢ **Campaign 3B — the "one helper" half is BUILT, and better than the doc's
  ...
  * ▢ **still open:** the `reason` and `generation` half of 3B's sketch
    ...
    a field that drifts.
  * ▢ **and the survey is not finished.** `transit_body` vs `reset_body_clusters`
    is a judgement call at every call site, and the smash respawn proves it can
    be got wrong silently. Worth an instrument: which call sites move a body
    without restarting it, and is each one a genuine TELEPORT?
