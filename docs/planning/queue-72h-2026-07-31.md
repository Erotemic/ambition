...
→ **new row: L2 movement options must score the EDGE.** An `Approach` that ends
past the platform is a self-KO, and on a stocks stage it is the cheapest way to
lose. This is the first thing the ladder will measure and the first thing that
makes level 1 vs level 9 mean anything — a brain that kills itself has no
difficulty curve.
