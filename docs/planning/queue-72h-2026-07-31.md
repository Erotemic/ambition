...
→ ▢ **new row: the veto has no lever in the situation that kills.** Every
remaining self-KO happens AIRBORNE, where L2 offers exactly one verb, so
"choose a different verb" cannot help however good the model gets. The deaths are
recovery FAILURES, not bad choices: the body leaves the stage with too little
height and the real recovery cannot bring it back. That is an L2 question (do not
go offstage) and a body question (is the recovery strong enough), and it wants
`distance_to_edge` pricing airborne position — not more rollout. Do not spend
more depth on it before that is tried.
