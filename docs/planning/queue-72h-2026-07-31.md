...
- ▢ **the select screen has no VISUAL.** `SMASH_SELECT_ROUTE` is passed as the
  composition's launcher route, so it renders the launcher's experience list.
  The screen works and cannot be seen, which is the same defect one layer up.
