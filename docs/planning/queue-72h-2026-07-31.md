...
- ▢ **3A residue** — strike volumes and summons are not round-scoped yet. They
  are retired by their own authorities today (`retire_orphaned_strike_volumes`
  derives from playback), so nothing leaks; the row is to decide whether those
  authorities should DEFER to the round scope or stay independent. Do not attach
  a second owner to a volume's lifetime without answering that — two authorities
  over one lifetime is how they come to disagree.
