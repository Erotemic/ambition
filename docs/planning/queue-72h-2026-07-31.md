...
whatever is happening is between the emitted frame and the body. Candidates, in
the order worth checking: a dash clock still running (the engine's dash SETS
velocity and ignores input for `DASH_TIME`), an airborne input gate, or a
locomotion frame convention. The next instrument is a KERNEL-side trace of the
