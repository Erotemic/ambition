...
- ✔/▢ **Campaign 2 — the before/after HARNESS exists** (2026-07-31). Its gate
  ...
  * ▢ **R4's last mile: `ambition_vfx` (11) and `ambition_items`.** Both are
    domains by the campaign's own list ("actor, combat, item, portal, boss or
    provider-specific"). Same procedure. `ambition_engine_core`, `_time`,
    `_persistence`, `_sim_view`, `_world` and `_sfx` are runtime-adjacent and
    belong where they are — that is the aggregation R4 asks the function to
    contain.
