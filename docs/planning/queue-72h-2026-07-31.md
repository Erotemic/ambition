...
→ ▢ **new row: every rung still loses all three stocks.** The remaining
self-KOs are no longer walk-offs the veto can see — a body already airborne has
no `ground_level` in the shadow at all, so every verb comes back fatal and the
halt fires, which does nothing for a body that is already falling. **The shadow
model has no recovery**: no double jump, no air drift, no up-B. `MovementVerb::
Recover` exists and `movement_intent` returns `None` for it, so it is the one
verb never judged — and it is the only verb that could save an airborne body.
That is the next real modelling row, and it is bigger than the last one.
