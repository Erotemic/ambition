...
→ **new row: the shadow model needs a floor, and a KO gate that distinguishes
"offstage and reeling" from "past the point of return".** Both are real
modelling questions, not tuning. Until then `suicidal_movement` fires only for a
line ending in HITSTUN offstage — a real launch the brain walked into, which is a
true veto and not the one the demo needs.

→ ⛔ **and the spec-level question stands: FB6e's `l3_earns_its_depth` cannot be
measured on SURVIVAL.** §8's ladder metrics are
...
measured on a metric attack choice actually dominates. Authoring nonzero
`rollout_depth` on a ladder row is blocked on that decision, which is exactly
where FB6e said it was — for a different reason than anyone expected.
