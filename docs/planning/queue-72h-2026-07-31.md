...
→ **still open: the ledge needs to be in the SITUATION, not only the score.** L1
classifies `Recovery` for a body already offstage; there is no situation for
"about to be". That is the shape the fix wants, and it is a bigger change than a
score adjustment — which is why this row stays open rather than being called
done at the first improvement.
