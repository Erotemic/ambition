...
- ▢ **Campaign 3B** — `BodyLifecycleTransition`: every semantic restart path
  (death respawn, round restart, sandbox reset, checkpoint restore, room
  reconstruction) emits from ONE helper. `ae::BodyRestarted` already exists and
  `reset_body_clusters` raises it; the row is whether every path routes through
  it or only the ones somebody remembered.
