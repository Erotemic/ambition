...
- ✔/▢ **Campaign 3B — the "one helper" half is BUILT, and better than the doc's
  ...
  * ▢ **the trap is only closed at the two respawn sites.** `ActorBody::from_kit`
    builds every enemy/boss body from `BodyClusterScratch::new_with_abilities`,
    which defaults `base_size`, and the real size arrives later on
    `BodyKinematics`. The general fix is for the spawn path to record the identity
    size where the reset authority reads it — same "ask at the call site"
    principle as the air-jump parameter. Until then any NEW reset of an enemy
    body has the same trap waiting.
  * ▢ **the boss revive (`features/ecs/reset.rs`) is NOT converted.** Same shape,
    but a boss carries phase state and a scripted pattern, and `base_size` is
    genuinely identity-derived there (a phase can resize it). It wants the
    general `base_size` fix first, not a third local workaround.
