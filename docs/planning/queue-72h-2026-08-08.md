...
⛔⛔ **AND ITS OWN NEW TEST FOUND A DEFECT — open, being worked:** a carried object
**SURVIVES a reset** when it must not
(`an_untouched_placement_is_authored_on_every_entry_and_a_reset_rebuilds_it`).
⭐ **the instructive part: this is the FIRST test that actually EXECUTES a
reset.** Its sibling asserts `RoomScopedEntity` is still present and *infers* in
prose that *"every sweep that culls on the scope (the sandbox reset) still sees
it"* — an inference execution has now contradicted. ⛔ **same class as a test that
pins a FUNCTION while claiming to pin the WIRING.** ⇒ stakes are concrete: the
reset road states no dispositions on purpose, so the room re-authors in full, and
a surviving carried item beside a re-authored one is **two live occurrences of one
placement `SimId`** — the exact duplication this leg closes.
