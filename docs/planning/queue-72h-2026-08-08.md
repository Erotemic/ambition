...
HEAD, four properties are complete, the observation/decision decomposition is
partial, and two remain open. ⛔ **read it before starting a slice** — deleting
the dead slot board was correct, but escaping Bevy's parameter ceiling is not the
same thing as finishing the semantic phase split. A row that looks more complete
than the code is the most expensive kind of stale.
