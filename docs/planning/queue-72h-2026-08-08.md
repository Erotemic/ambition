...
- ▢ **D33 THE DECOMPOSITION BRIEF (GPT 5.6, handed over by Jon 2026-08-08).**
  ...
  **▢ STILL OPEN — the next action is step 1**, and it is a mechanical repoint of
  ~28 call sites. ⚠ what remains genuinely undetermined: whether the hoisted
  subset clears the ratchet's `critical_path_crates` guard (only the simulator can
  say, and only after the hoist), and whether the runtime crate can own
  `SpritePosedBody` construction — a design question reading alone cannot settle.
