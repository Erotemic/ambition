...
- ▢ **D100 ⭐⭐ REGISTERING A CHARACTER TAKES FOUR EDITS IN THREE FILES, AND THAT
  ...
  campaign exists to refuse. ⚠ before starting, COUNT the two lists and write
  the numbers down — the count is what says whether the seam worked, and this
  run has published three wrong censuses from regexes already.
...
- ▢ **D101 GPT 5.6's REVIEW: WHAT LANDED AND WHAT DID NOT.** Recorded so the
  ...
  is corrected in the campaign file. ▢ §4 is D100 above. ▢ **Noether remains a
