...
- ▢ **D107 ⛔⛔ `BodyCombat::attacking` IS FALSE FOR ALMOST EVERY BODY THAT IS
  ...
  * what it mirrors is `BodyMelee::is_swinging()`, the FLAT swing. A body
  ...
  ⇒ the fix is one of: write `attacking` from the moveset path too, widen the
  query off `PlayerEntity`, or delete the field and have the instruments read
  `MovePlayback`. ⛔ **the third is probably right** — a mirror that has to be
