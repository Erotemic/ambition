...
- ▢ **D108 ⛔⛔ LANDING LAG IS A PLAYER-ONLY MECHANIC, IN TWO INDEPENDENT WAYS —
  ...
  **for exactly the reason this row is about** — one list is checked by the
  ...
  ⇒ **the fix is three parts and they must land together**, and each is now one
  line at a named place: add `landing_lag_timer` to the carry list at both
  rebuild sites, add it to `decay_reaction_timers`, and call `hard_lock_timer()`
  on the actor road instead of passing `recoil_lock` alone. ⛔ **carrying it alone
