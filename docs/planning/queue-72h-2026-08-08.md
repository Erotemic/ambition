...
| line | row | state |
|---|---|---|
| 669 | **D33** the decomposition brief | steps 1–3 ✔; **step 4 traced + re-verified, dispatch-ready** |
| 1187 | **D28** compile/test time in real work | recorder ✔ landed; the rhythm question answered |
| 1788 | **D44-ORIGINAL** | superseded — **D44 LANDED `afc36b390`**, kept for its measurements |
| 1886 | **D45** sixteen observations from Jon | INDEX only; ⛔ Jon said it does not change priorities |
| 1972 | **D42** the patent clerk's rig | ⚠ observe before changing art; Jon's read is that it is an EDITOR fix |
| 1998 | **D43** touch controls dead on some menus | **IN FLIGHT** — the fork is 20 lines wide |
| 2248 | **D40** Sanic's transform button reads "Fly" | **IN FLIGHT** — engine half already shipped |
| 2354 | **D51** `Auto` never decides what Attack does | Jon's held-item observation, diagnosed |
| 2414 | **D50** the dropped weapon is session-scoped | Jon's laser sword, diagnosed; predicted in writing a week ago |
| 2486 | **D49** the LDtk editor nulled every `EntityRef` | Jon's sharks; cause + commit + blast radius |
| 2555 | **D48** an enemy cannot say who it IS | Jon's Iron Mary; ⛔ **the design call is Jon's** |
| 2638 | **D47** the drawing-vs-body function has ONE caller | ⭐ where Jon's five sizing complaints actually live |
| 2731 | **D46** the unclaimed-body warning | the magenta flash; falsifier written in, not yet run |
| 2864 | **D39** the goblin encounter | **IN FLIGHT** |
| 3088 | **D37** is a crate name part of the wire format | **IN FLIGHT** — answer corrected to (b′), final segment only |
| 3180 | **D36** the `SheetRegistry` collision warning | LOW — log noise, same defect class as D46 |
| 3229 | **D26** the death-beat freeze instrument | re-verified still open 2026-08-08 |

⭐ **six of these (D46–D51) were filed 2026-08-09 and four close a Jon
observation diagnostically** — the flash, Iron Mary, the sharks, the laser sword,
the held-item jab. None of them needed a device or a repro; all five were read
out of the tree.
...
- ▢ **D57 ⛔⛔ THE SPIKE TESTS ARE GREEN ONLY IN THE CONFIGURATION WHERE INPUT
  ...
  * ⚠ **related, same shape, already known and still true**: `-p ambition_input`
    ...
    `bevy_plugin` is `mobile_touch`-gated. **Three instances now.** ⇒ ▢ **worth
    one pass that answers "which features does each crate's suite need to run all
    of itself", written down where a brief can copy it.** That makes every future
    verification honest at once, which is the cheapest thing on this list.
