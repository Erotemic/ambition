...
- ◐ **D71/D92 — Finish the real room-transition transaction path.** ⭐ **THE
  ...
  commit gated on the same transaction. What remains under this row is the
  prefetch/latency MEASUREMENT, which was blocked on exactly this.
...
- ▢ **D121 — The browser runs a DIFFERENT application, and the source already
  said this would happen.**

Use [`engine/web-platform-parity.md`](engine/web-platform-parity.md). Jon,
2026-08-14: the wasm links, the canvas loads, and nothing appears. ⭐ **measured
the same day:** `run_web()` never calls `AmbitionShellHosted` /
`compose_ambition_shell_host*` / `install_ambition_shell_visuals`, which
`build_visible_app_with` does — and that builder's own doc records the last time
a hand-spelled composition dropped `install_ambition_shell_visuals`: it cost
*"the entire room"* for two days. ⛔ the fix is ONE visible composition both
entry points call, not the three calls pasted into a third builder.
