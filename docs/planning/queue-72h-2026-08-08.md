...
**⇥ REGENERATED 2026-08-09 (FIFTH time today).** The fourth version said D52,
D62 and D63 were *"IN FLIGHT"* after all three had **landed**, told a reader to
*"sequence D65 after D62/D63"* when D65 was **already done**, and was missing D66
and D67. ⇒ **it rots in about two hours. Regenerate from
`grep -n '^- ▢ \*\*D'`; never patch.**

| row | what | state |
|---|---|---|
| **D33** | the actor-monolith decomposition | ✔✔ **CARVE LANDED `a64bf22f8`** — plugin shape, path held at **12**, +4.5 s · ▢ **measure the new crate** (needs an empty fleet) · ▢ `character_runtime` + `assets.rs` untouched |
| **D67** | one line in `level_1_2()` kills all block art | 🔴 **IN FLIGHT** — explains 3 of Jon's rows at once |
| **D66** | a ratchet test pinned a coincidence | ✔ **FIXED `e475acf54`** — carve size is derived now; ⭐ the same carve costs 35% more than when it was written |
| **D65** | no Mary-O room but 1-1 could be captured | ✔✔ **LANDED `00d8bbc59`** — and the capture **confirmed D61 on screen** |
| **D64** | four more of Jon's observations, unfiled | INDEX only, one fact each |
| **D59** | `ambition_render` lib test cannot link | ⚠ **theory WEAKENED** — it linked clean with 11 rust-analyzer processes live. Offer Jon the setting as hygiene, **not as the fix** |
| **D56** | the renderer binds a sheet by DISPLAY NAME | ⛔ **must land with D48**; the deadlock is proven symmetric |
| **D55** | the shield bubble | ✔✔ **SOLVED — painted into v3's `block` sheet row**; ▢ regen the art |
| **D54** | a slash with no owner | ✔ hazard fixed · ✔ art ruled out ⇒ ▢ **needs one reproduction from Jon** |
| **D53** | the Android suspend `.take()` | ✔ fixed blind · ⭐ **NEW: it CAN be unit-tested here** — the module is un-gated, only 7 items inside it are |
| **D52** | live quality Apply | ✔ prop stamp · ✔ **tier split `79289a3dc`** · ▢ provenance + bounded-memory Apply · ⚠ `resident_tiers()` has **0 production callers** |
| **D50** | the dropped weapon is session-scoped | diagnosed · ⭐ **`drop_ability_pickup` lacks `RoomScopedEntity`**, arrow audited both ends |
...
| **D46** | the unclaimed-body warning | ⛔ **my grace-period fix REFUTED** — the placeholder is the cover's settle signal ⇒ ⭐ **split the two roles**, scoped for dispatch |
...
| **D26** ×2 | the death-beat freeze instrument | measures nothing; **dispatch-ready** (second fixture, every piece exists) |
| ~~**D60**~~ | the portal gun has D51's bug | ⛔⛔ **DO NOT DISPATCH AS WRITTEN** — 187 refs; arbitrate the resolution, not the declaration |

⭐ **closed today and no longer in this table**: D58 (`727cf8458`), D61
(`e75d5bc74`, **verified on screen** by D65's capture), D62 (`2707d86d9`), D63
(`b24651aed`), D49 (`680568fe8`), D51, D40, D41, D38.
...
- ▢ **D53 ✔✔ THE DECISION NOW RUNS ON A MACHINE WITH NO ANDROID — `578d1b5c7`.**
  ...
  * ▢ ⚠ **the GLUE is still uncompiled, and the worker declined to close it — 
    correctly.** The cfg-flip typecheck (retarget the gates to the host,
    poison-check, restore) is what would turn *"uncompiled"* into
    *"typechecked"*, and it edits a file `app_it` compiles — **a transient flip
    would have handed another live agent a red that was not theirs.** ⇒ **~5
    minutes when the tree is free.**
