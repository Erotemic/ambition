...
⇒ **M0 census dispatched 2026-08-15** (read-only worker). Its output lands as the
inventory section of the plan, and it must name two materially different real
proof customers — one Rust-constructed encounter sequence, one ordinary world
mechanism — **with the deletions each migration would earn.** ⛔ a migration that
only adds is a wrapper, not a proof.
