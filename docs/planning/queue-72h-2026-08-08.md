...
- ▢ **D90 ⛔ `smash_it` WAS 14/18 AND THE GATE COULD NOT SEE IT — 16/18 now, two
  ...
  * `unsatisfiable_seats` consulted the archetype table ALONE, so four perfectly
  ...
  ⇥ ▢ **ONE LEFT: `losing_a_stock_announces_a_body_restart`.** A fighter is
  teleported to `y = 100_000`, past the blast zone, and `BodyLifetime::
  restart_pending` is never observed in the following 120 ticks. ⚠ this one warms
  up 240 ticks, so it is NOT the countdown, and it seats through `smash_roster`
  (seat 0 human) — check whether the fighter it grabs is the inert seat before
  assuming the reset is broken. ⇒ if the seat is fine, the suspect is D85: a
  respawning seat that no longer asks for `RecharacterizeBody` may be taking a
  reset path that never raises the flag.
