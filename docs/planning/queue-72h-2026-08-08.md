...
- ▢ **D81 ⭐ THE `BrainPreset` RETIREMENT, MEASURED — 125 ADOPTERS, 8 LIVE KEYS,
  ...
  * ⭐ **AND `parrot_lively` IS THE NEXT FREE ONE — with a live disagreement in
    ...
    ⇥ ⛔ **`wanderer_puppy_slug` is NOT next** — measured, not assumed. Its three
    adopters are `npc_puppy_slug`, `npc_burning_flying_shark` and
    `npc_puppy_slug_variant2`, and the VARIANT has no authored arm at all. Deleting
    the preset would leave it with no brain. A preset dies with its LAST namer, and
    that one is still holding on.
