...
⇒ what remains on this row: **K4** contact completeness (crush/one-way policy),
**K5** authoring polish, **K6** a second kinematic customer. ⚠ **K4 inherits one
residue**: the three roads disagree on TRIMMING — the enemy road trims at
conversion, the hazard road at lookup, the NPC road not at all. The sweep mirrors
each road exactly so it never lies, but a leading space in an authored NPC
`patrol_path_id` still silently disables that patrol.
...
1. ⛔ **`camera_follow` is not yet N-view, and this is the live seam.** It reads
   `query.iter().next()` to take **the first main camera's** `PresentsView`,
   resolves that one view, and then applies that snapshot to **every** main
   camera in its loop. Two cameras bound to two views would both receive the
   first view's transform. ⚠ `PresentedViewState::get()` already refuses to
   choose arbitrarily between several main cameras; `camera_follow` has no
   equivalent protection. The relationship must become camera-by-camera, proven
   by two cameras bound to two views receiving DISTINCT correct state.
2. **Per-view gameplay rectangles.** `CameraViewport` already exists as a
   component on the view with NO consumer, so wiring it deletes nothing and
   invents nothing. ⛔ the POLICY that chooses rectangles (shared / fixed split /
   BG3-like adaptive with hysteresis) is a product decision the maintainer has
   not made — wire the rectangle, do not invent the enum.
3. **The remaining `PresentedViewState` consumers name WHICH view.** Measured
   2026-08-15: only THREE draw systems are genuinely per-view — foreground,
   label_layout, nameplates — plus a dev overlay. A fifth apparent consumer,
   `rendering::actors`, affects NO pixel: its `orthographic_scale` feeds only the
   `[sprite-size]` diagnostic.
