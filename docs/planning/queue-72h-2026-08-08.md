...
- ▢ **D103 ⛔⛔ 199 BROKEN DOC LINKS, AND THE GATE CANNOT SEE ONE OF THEM.**
  ...
  ⇒ the work is a RATCHET rather than a sweep: record the number, add a check
  the gate actually runs, and let it fall. ⛔ do NOT open it by fixing 199
  links; a sweep with no ratchet behind it is 199 links again by September.
  ⚠ and check whether `cargo doc` is affordable in the gate before proposing it
  — `reference_compile_cost_measurement` says COLD and REBUILD want opposite
  fixes, and a doc build is a cold-ish pass over every crate.
