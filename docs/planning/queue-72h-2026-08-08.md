...
- ▢ **D88 ⚠ THE `ambition_platformer2d_host` TEST SUITE IS RED, AND WAS BEFORE
  ...
  * ✔ **`ambition_workspace_policy` (2)** — and both were REAL architectural
    ...
    reading the flag.
  * ▢ **`ambition_demo_smash_app` `smash_it` (4 of 18)**, including
    `losing_a_stock_announces_a_body_restart`. ⚠ SMASH is the proving ground, so
    this is the next row to work, and it is unmeasured: find out whether these
    predate the campaign the same way (stash, re-run) BEFORE assuming.
...
- ▢ **D81 ⭐ THE `BrainPreset` RETIREMENT, MEASURED — 125 ADOPTERS, 8 LIVE KEYS,
  ...
  **The census (2026-08-11), so no step begins with a survey.** Adopters counted
  ...
  |---|---|---|---|
  | `stand_still` | 43 + **every one of the 6 other providers** | none | none |
  | `patrol_peaceful` | 43 | `speed: 28.0` | `aggressiveness: 0.0` |
  | `melee_brute_striker` | 23 | `chase_speed: 110.0` | `aggressiveness: 1.0` |
  | `melee_brute_brute` | 7 | `chase_speed: 75.0` | `aggressiveness: 1.0` |
  | `wanderer_puppy_slug` | 3 | `speed: 36.0` | `aggressiveness: 0.0` |
  | `skirmisher_ranger` | 3 | `strafe_speed: 85.0` | `aggressiveness: 1.0` |
  | `cellular_automaton_raider` | 2 | `cruise 88 / dive 210` | `aggressiveness: 1.0` |
  | `parrot_lively` | 1 | `cruise 95 / dive 240` | `aggressiveness: 0.0` |
  | `sniper_default` | **0** | `fire_cooldown 1.5` | `aggressiveness: 1.0` |
  ⭐⭐ **START WITH THE TWO FREE ONES.**
  * `sniper_default` has **ZERO adopters**. It is authored, validated, and
    reachable by nobody. That is a deletion with no migration attached, and it
    should not survive another session.
...
- ▢ **D96 SIX CONTENT DECISIONS ARE STACKED UP, AND EVERY ONE OF THEM BLOCKS A
  ...
  ⇥ ⭐ **and a seventh that is different in kind**: P3.25's bridge cannot be
