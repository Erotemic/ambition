...
| **D74** | the PCA is held off the playable roster by the **`is_aerial` conflict** | ▢ **DIAGNOSED 2026-08-10** — registering it flips the body to Floating (catalog) against `is_aerial: Some(false)` (archetype), so the AERIAL attack plays. Waits on D73 phase 2's judgement call #1 |
...
- ▢ **D74 A FIGHT THAT STARTS BEFORE ITS SHEETS LAND NEVER RECOVERS.** Found
  ...
  * ⭐ `autonomous_reconcile::peaceful_config` (`is_aerial = matches!(catalog
  ...
  ⇒ **possession runs the second one.** `apply_catalog_mode` rebuilds the body's
  `tuning` wholesale — `max_health: 1`, NPC patrol speeds, default capabilities,
  the kit's action set, and `is_aerial` from the catalog row. Registering the PCA
  changes what `sprite_character_id` resolves to, so a row saying `Floating`
  becomes reachable where it was not, and the body floats.

  ⭐⭐ **that is Jon's own sentence about provocation, reproduced exactly**:
  *"becoming angry currently reconstructs the actor as another mechanical
  creature."* Here it is becoming POSSESSED, and the reconstruction includes a
  1-HP pool and the wrong gravity. ⇒ this row is evidence for **D73 phase 5**
  (provocation becomes a disposition + controller transition, no body rebuild)
  as much as for phase 2's `is_aerial` arbitration — and phase 5 is what makes
  it stop being reachable at all.
