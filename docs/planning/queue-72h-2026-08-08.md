...
- ▢ **D73 ⭐⭐ THE RUN'S TOP PRIORITY, SET BY JON 2026-08-10: A CHARACTER IS A
  ...
  **Read that file before touching character construction.** It answers D48 —
  ...
  ✔ **landed against the correction (2026-08-10):** the default autonomous
  profile is typed (`BrainPresetId`, `5f5cbd1bb`); the authored-enemy path
  stopped inferring gameplay identity from `sprite_character_id` and asks the
  placement instead (`e67468819`); and the phase-3 harness that was missing —
  an authored enemy constructed against a populated prepared registry — now
  exists with the inversion's poison in it (`a235947ac`).
