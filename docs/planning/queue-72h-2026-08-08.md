...
- ▢ **D59 `cargo test -p ambition_render` CANNOT LINK — a stale artifact in the
  ...
  * ⭐ **the suspects, from the symbol names**: `hashbrown` and
    `bevy_image::texture_atlas::TextureAtlas` point at a sheet/atlas crate.
    **`ambition_sprite_sheet` was rewritten heavily today by D44** and is the
    first thing to clean. ⚠ **do not guess past that** — guessing this class cost
    an hour once already; if cleaning it does not work, the honest next step is
    to remove the whole target dir and pay the cold build.
