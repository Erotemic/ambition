...
- ▢ **C4 the DIALOGUE domain carve** — `conversation` + `dialog` + the Yarn
  bindings as one unit, which is what actually removes `ambition_dialog` (and
  `ambition_ui_nav` behind it) from a movement-only graph. ⚠ **blocked on ONE
  thing**: the third `features` reference to `ambition_dialog`. Find it, decide
  whether it is integration that belongs above both domains (plan step 5), and
  move it there before designing the crate. Almost every other low-count dep has
  the same shape, so whatever this costs is likely reusable.
