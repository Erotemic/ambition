...
- ▢ **D108 ⛔⛔ LANDING LAG IS A PLAYER-ONLY MECHANIC, IN TWO INDEPENDENT WAYS —
  ...
  **for exactly the reason this row is about** — one list is checked by the
  ...
  ⇒ **the fix is three parts and they must land together**: carry the field
  across the rebuild, decrement it in the actor tick, and call
  `hard_lock_timer()` on the actor road too. ⛔ **carrying it alone
