...
- ▢ **C6 `ambition_platformer2d_ldtk` is the one available footprint win** —
  one root module (`world`), one file naming it, no other dependent in the
  consumer's graph. ⚠ measure `world/`'s inward edges first, the way
  `conversation` was measured; and re-run `cargo tree -i` after, because that is
  the only thing that proves the closure moved.
