...
- ◐ **D102 ⭐⭐⭐ THE LAST TWO ARCHETYPE ROWS — NOW ONE — 60 production references
  ...
  ⇥ ⭐ **`spec_for_brain` now has exactly TWO production callers**, both genuine
  whole-body construction: `ActorClusterSeed::new_enemy_in`
  (`actor_clusters.rs:493`) and `spawn_enemy_with_faction_into`
  (`spawn_actors.rs:1504`). Every other site asked it for ONE fact and was
  content with "nobody said". Those two are exactly where the unknown-key
  decision lands, and there is nothing else in the way of it.
