...
- ▢ **D88 ⚠ THE `ambition_platformer2d_host` TEST SUITE IS RED, AND WAS BEFORE
  ...
  ⇥ ⇒ two questions, in order: does the demo shell legitimately run WITHOUT an
  encounter system (then the system's params want `Option`/`If`, per this repo's
  own rule that a plain `Res` read of a released resource PANICS), or is the
  fixture missing setup it should have? ⚠ do not answer by adding resources until
  the first question is settled — a shell that needs a quest registry to boot is a
  claim about the engine's minimum, not a fixture detail.
