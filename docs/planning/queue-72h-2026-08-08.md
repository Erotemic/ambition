...
- ◐ **D102 — ✔ THE GLOBAL FALLBACK IS GONE (2026-08-12). What is left is three
  ...
  ```text
  the IDENTIFIER is open   IDENTIFIERS_AWAITING_A_CASTING_DECISION — a short,
                           shrinking list; empty closes D102
  ...
  by fixing what it NAMED, not by relaxing the rule. A unit poison
  (`an_open_casting_decision_gets_a_body_and_a_misspelling_does_not`) holds both
  halves against the same roster.
