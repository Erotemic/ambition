...
- ⛔⛔ **A WORKTREE AGENT IS NOT AN ISOLATED BUILD, AND IT DOES NOT START ON
  ...
  * `.cargo/config.toml` sets an **absolute** `target-dir`, so every worktree and
    ...
    concurrent job.**
  * and a worktree has **no art or audio** (`*.png` under `assets/sprites/` and
    the whole `audio/` tree are gitignored), so `app_it` fails there for reasons
    that are not the job's. Name an asset-free verification set and run the full
    suite in the main tree afterwards.
