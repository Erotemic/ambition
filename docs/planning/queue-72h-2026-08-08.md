...
- ▢ **D22 THREE ROWS THAT READ AS OPEN ARE SITTING IN THE ARCHIVE.** Surfaced by
  ...
  * same file `:1968` — *"milestone 5 is NOT reached, and here is the number"*.
  ...
  different from retired-ledger rows.** Two of the three are not orphaned work:
  ...
  * ▢ **milestone 5 — GENUINELY UNRESOLVED, and it is the one to carry.** *"With
    both fighters seated, the pad's DPadRight moved BOTH — 14.19px on the keyboard
    player's fighter against 11.44px on the pad player's."* **No test asserts that
    a pad moves only its own fighter** (the two near names are about unplugging and
    about the roster grid). The row's hypothesised cause no longer matches the
    tree — zero `PRIMARY`+global-`ControlFrame` hits in `ambition_input` — so the
    architecture moved under it and the defect may well be fixed.
    ⛔ **but it cannot be settled by reading**: couch input needs TWO sources and
    is invisible with one pad, which is exactly how five bugs hid here in one day.
    A two-source probe at the press is the only thing that answers it.
