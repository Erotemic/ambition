...
**⇥ REGENERATED 2026-08-09 (SIXTH time today).** ⇒ **it rots in about two hours.
Regenerate from `grep -n '^- ▢ \*\*D'`; never patch.** ⭐ **and the curation is
the point** — the row headers already carry landed/open, so what this table adds
is *what a reader should do next*, which is exactly the part that goes stale.

| row | what | state |
|---|---|---|
| **D33** | the actor-monolith decomposition | ✔✔ **CARVE LANDED `a64bf22f8`** — path held at **12**, +4.5 s · 🔴 **the crate is being PRICED right now** · ▢ `character_runtime` + `assets.rs` untouched |
| **D69** | a discovered hidden block loses its picture | ▢ **NEW** — promoted blocks have **no render pass**; payout lands, art is despawned |
| **D68** | Jon's death→restart is invisible to every test | ▢ **NEW** — chain verified end to end; **the test counts MESSAGES, not a restart** ⇒ one composed-app falsifier decides it |
| **D67** | one line killed all block art in 1-2 | ✔✔ **LANDED `4af02a1d0`** + `d5f1d4621` (the invisible brick really was dead) |
| **D66** | a ratchet test pinned a coincidence | ✔ **FIXED `af37438e1`** — third draft; **name both denominators** |
| **D64** | four more of Jon's observations, unfiled | INDEX only · ⚠ **one is now answered**: the 1-2 tile texture is a taste call, not a bug |
...
| **D53** | the Android suspend `.take()` | ✔✔ **decision TESTED here `578d1b5c7`** · ▢ the glue is still uncompiled (~5 min, free tree) |
| **D52** | live quality Apply | ✔ prop stamp · ✔ tier split · ▢ provenance + bounded-memory Apply |
| **D50** | the dropped weapon is session-scoped | ✔✔ **ability drop fixed `481072760`** · ▢ **Jon's laser-sword lifetime is still his** |
| **D48** | is an enemy a CHARACTER or an ARCHETYPE? | ⛔⛔ **ONE OF THREE THINGS BLOCKED ON JON** |
| **D47** | the drawing-vs-body function | ✔ falsifier run — a RIG project, not an engine one |
| **D46** | the unclaimed-body warning | ⛔ my grace-period fix **REFUTED** ⇒ ⭐ **split the two roles**, scoped for dispatch |
...
| **D26** ×2 | the death-beat freeze instrument | measures nothing — ⭐ **same blind spot as D68, same beat** |

⭐ **closed today**: D33's carve, D67, D66, D65, D63, D62, D61, D58, D55, D53's
decision half, D51, D50's sibling, D49, D41, D40, D38.
⛔ **blocked on Jon, none blocking work**: D48 · the measurement-submodule pointer ·
the rust-analyzer setting.
...
- ▢ **D70 ⛔ SOME MARY-O BLOCKS STAY SPENT ACROSS A RESTART — and it is NOT a
  ...
  * ⚠ **the falsifier, and it must name a SPECIFIC block**: bonk a `?`-block,
    ...
    per-attempt block state lives in one of those two sets. ⭐ **the third one is
    already known to exist**: a discovered Hidden block is promoted through the
    overlay (D69), which is neither a spent power block nor a broken brick.
    **Start there.**
...
- ✔✔ **D68 THE FALSIFIER CAME BACK GREEN — `5fddacd76` (2026-08-09).** The chain
  ...
  * ▢ **the one cheap thing left**: three routes write `ActorDiedMessage` —
    `death_respawn_player` (a hit) ✔ now tested, the level timeout via
    `spend_lives_on_death` ✔ already tested, and **`publish_kernel_reset_death`
    (a pit, a hazard, any kernel reset) ▢ NOT tested end to end.** ⇒ if Jon was
    dying to a pit or a spike rather than to a hit, that is the untested route.
    **One more fixture, same shape.**
...
- ▢ **D69 ⛔ A DISCOVERED HIDDEN BLOCK PAYS OUT AND THEN HAS ITS PICTURE DELETED.**
