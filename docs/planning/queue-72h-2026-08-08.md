...
- ▢ **C4c move `dialog/yarn_bindings.rs` to `game/ambition_content`** — an
  OWNERSHIP correction, not a decomposition win, and it should be committed as
  that. 609 lines of `<<give_item>>` / `<<buy_item>>` / `<<challenge>>` leaving
  an engine crate. ⚠ its inward edges (`items`, `features`, `shop`, `actor`,
  `save`) all become ordinary downward edges from the content crate.
