...
- ▢ **C4b lift the dialogue domain as a crate** — `conversation` + `dialog` +
  the Yarn bindings. NOW unblocked on the `features` side. ⚠ **re-measure
  first**: the remaining `ambition_dialog` namers are `dialog/` itself and the
  conversation module, and the question to answer before moving anything is
  where `dialog/yarn_bindings.rs` goes — it names game vocabulary (items, shop,
  brains, challenge) that the reusable dialogue runtime must not. The narrative
  ledger already separated the WRITER from the applier, so the bindings may
