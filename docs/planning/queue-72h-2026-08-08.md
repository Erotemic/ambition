...
`RoomAssetReadiness` computes `pending: Vec<String>` and `failed: Vec<String>`,
and `ActiveRoomTransitionLoad` keeps only `(settled, total)`. **A load coordinator
that knows it is blocked must be able to say what on.** ⭐ measured: the Hall
authors **129 `NpcSpawn` rows naming 129 DISTINCT characters, no repeats** — every
one its own layout and image, materialized in the frame that builds the manifest,
with the reveal barrier then waiting on all of them. A barrier correctly waiting
on the slowest of 129 fetches and one deadlocked on an asset that will never
arrive look identical today and want opposite fixes.
