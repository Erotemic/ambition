...
- ▢ **D45 SIXTEEN MORE OBSERVATIONS FROM JON (`7ace7b5e7`) — INDEX, not triage.**
  ...
  * ⭐⭐ **so the three "faces" are TWO mechanisms, and I can now say which**:
    ...
    * **transient** — this flash, and (I believe) the device log's `NpcSpawn-*`
    ...
    ⚠ **not yet checked**: whether the *"cover gave up waiting"* warning actually
...
- ▢ **D43 TOUCH CONTROLS UNUSABLE ON SOME MENUS ON ANDROID.** Jon, 2026-08-08,
  ...
  * ⛔ **needs a device and I have none** — see
...
- ▢ **D40 SANIC'S TRANSFORM BUTTON READS "FLY" BECAUSE A TOUCH LABEL IS AN
  ...
  **Two halves:**
  1. **engine** — the touch layout's string becomes a FALLBACK. The label for a
     button resolves through its `ControlSlot` to the active actor's
     `ActionSchemeContract`, using `ActionSpec::display()`; the hardcoded verb
     applies only when no action claims the slot. Fixes every game at once.
  2. **Sanic** — author an `ActionSpec` on `ControlSlot::Utility` for the
     transformation. ⚠ **it currently authors none**: `lib.rs:1407` consumes the
     raw Utility edge imperatively, which is exactly why no label can be found
     for it today. Even an id of `transform` yields "Transform" by title-casing,
     so `display_name` is optional.

  ⚠ half 1 alone changes nothing visible (no `ActionSpec` claims Utility, so the
  fallback still reads "Fly"), and half 2 alone changes nothing (nothing reads
  the label). **Neither half is testable without the other** — land them
  together or the change looks inert.
