...
- ▢ **C4e actually carve `ambition_conversation`** — 2,164 lines, two inward
  ...
  `npc_ambient_bark_line` and `PreparedCharacterRegistry`. ⚠ **and it is still
  not a footprint win** — `ambition_dialog` moves from the monolith to the new
  crate, and a movement-only game linking the monolith would stop pulling it
  only if nothing else in the monolith does. CHECK THAT FIRST: if `dialog.rs`'s
  generic wiring still names it, the edge does not leave and the carve is a
  compile-isolation win only. Say which it is before starting.
