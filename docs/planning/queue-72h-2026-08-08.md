...
- ▢ `EnemySpawnSpec::character_id` — ⭐ **the expensive half of this item is
  stale.** It was deferred because *"the field is genuinely absent on unmigrated
  entities, so making it required is a content migration rather than a signature
  change."* Measured 2026-08-14 over EVERY `.ldtk` in the repo — content, both
  demos, and the `ambition_map_assets` submodule: **184 `EnemySpawn` entities, 0
  missing `character_id`.** There is no migration left, so this is a
  compiler-driven signature change: `Option<CharacterId>` → `CharacterId`,
  `presentation_identity`'s display-name fallback deleted,
  `gameplay_character_id` returning a value rather than an `Option`. Sized:
  62 `EnemySpawnSpec` mentions, 12 `presentation_identity` callers, 5
  `gameplay_character_id`, 13 `with_character_id`. ⚠ the lowering must REFUSE an
  authored entity with no id rather than default one — the whole point is that
  absence stops being representable.
- ▢ `WornCharacter` → universal `CharacterIdentity` — blocked, and the blocker
  SHRANK rather than vanishing: `apply_worn_character_overlay` resolves registry
  → catalog → id, so the catalog is a fallback now, but the 14 characters that
  cannot build a body from their own definition still land on it. Enrolling
  everybody hands exactly that population the catalog's kit. ⚠ a second smaller
  thread: `catalog: Res<CharacterCatalog>` is still a REQUIRED resource on that
  system.
