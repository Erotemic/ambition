...
- ▢ **D86 ROBOT v3 OFF `HostCode` — ⭐ THE PRODUCT DECISION IS IN: THE ROBOT
  ...
  *count the ADOPTERS, not the capability*, applied to a fact rather than a type.
  ...
  ⇥ ▢ **the two `HostCode` variants SURVIVE, and the count is the work.**
  `PlayableKitSource::HostCode` is down to **1 adopter** (the host demo-shell
  smoke fixture, which exists to test the variant itself) — smash's three
  duelists already authored a `duelist` action set while ALSO saying `HostCode`,
  and pocket's runner is a provider-acceptance fixture that never fights and `PreparedKit::HostCode` is the fallback every character with no
