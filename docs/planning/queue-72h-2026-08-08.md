...
| D18 | do coins/score/lives survive a room transition? | free — **and D17 is why nobody could see** |
| D19 | promote one card from the orphaned param-architecture doc | free — `QueryData` still 5, `SystemParam` 35→40 |
...
- ▢ **D18 A1.5 — continuity across a room transition, orphaned since 2026-07-25.**
  Carried from `queue-24h-2026-07-25.md`, which has **zero inbound references**
  and one open mark, and which I nearly archived on that basis. The mark is real:
  *"continuity: score, coins, lives, and worn power across the transition. Assert
  against the BODY, never the emitter's bookkeeping."* Its own column called it
  *"the next thing to do in this queue"* — two weeks ago.
  ⭐ **grepped before queuing, as the rule demands, and nothing covers it.** The
  nearest guards are `authored_portals_survive_both_death_and_manual_resets` and
  `a_room_transition_survives_the_rollback_window` — a portal-identity claim and
  a rollback claim. **Neither asserts that the player's coins are still there on
  the other side of a door.**
  ⚠ **the row's own instruction is the design**: assert against the BODY, never
  the emitter's bookkeeping. A score counter that survives because the HUD
  resource was never cleared is not continuity; the wallet on the body is.
  ⭐⭐ **and D17 makes this urgent rather than tidy.** If a dropped coin is never
  claimed by a render family, nobody has been able to SEE whether coins carry —
  the two rows are the same seam from opposite ends, and A1.5 has been unwatched
  for two weeks partly because its subject was invisible.
  ⛔ **so `queue-24h-2026-07-25.md` is NOT archivable yet** — same trap as the
  08-06 ledger this morning, caught the same way. Second time today that a
  zero-inbound retired file held live work.
