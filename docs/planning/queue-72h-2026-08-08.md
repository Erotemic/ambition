...
**Regenerated 2026-08-09 after the D37/D39/D40/D43/D44 landings.** Line numbers
drift on every edit; the ROW IDS do not — grep for `**D47` rather than jumping to
a line.

| row | what | state |
|---|---|---|
| **D33** | the decomposition brief | steps 1–3 ✔; **step 4 IN FLIGHT** |
| **D28** | compile/test time in real work | recorder ✔; the rhythm question answered |
| **D45** | sixteen observations from Jon | INDEX only; ⛔ Jon said it does not change priorities |
| **D42** | the patent clerk's rig | ⚠ observe before changing art; Jon's read is that it is an EDITOR fix |
| **D58** | ⭐⭐ a finger cannot tap a smash portrait | **the real answer to Jon's Android menu bug**; confirmed by two routes |
| **D57** | ⛔⛔ the spike tests pass only where input is absent | + the 23-crate feature-gate table |
| **D56** | ⭐⭐ the renderer binds a sheet by DISPLAY NAME | ⛔ **must land with D48 or D48 makes it worse** |
| **D55** | the shield bubble ignores the feet anchor | Jon's bubble; cause verified, first hypothesis refuted by a sum |
| **D54** | a slash with no owner is drawn at the world origin | verified hazard; the link to Jon's VFX is NOT |
| **D53** | the Android suspend `.take()` | ✔ fixed, ⛔ **uncompiled — no NDK here** |
| **D52** | ⭐⭐ quality Apply converges one class of sprite | all 5 findings verified from the tree; ⭐ the prop stamp is a 2-line fix |
| **D51** | `Auto` never decides what Attack does | Jon's held-item jab |
| **D50** | the dropped weapon is session-scoped | Jon's laser sword; predicted in writing a week earlier |
| **D49** | ⭐⭐ the LDtk editor nulled every `EntityRef` | Jon's sharks; cause + commit + blast radius |
| **D48** | an enemy cannot say who it IS | Jon's Iron Mary; ⛔ **the design call is Jon's** |
| **D47** | ⭐⭐ the drawing-vs-body function has ONE caller | ⭐ where Jon's five sizing complaints actually live |
| **D46** | the unclaimed-body warning | the magenta flash; **falsifier written in, not yet run** |
| **D36** | the `SheetRegistry` collision warning | LOW — log noise, same class as D46 |
| **D26** | the death-beat freeze instrument | re-verified still open 2026-08-08 |

✔ **LANDED 2026-08-09**: D44 `afc36b390` · D39 `830d386ce` · D37 `3333a4b0f`
(**schema v20**) · D40 `7860e5c02` · D43 `adbf5f0ac`.

⭐ **thirteen rows (D46–D58) were filed this day, and six close a Jon observation
diagnostically** — the magenta flash, Iron Mary, the sharks, the laser sword, the
held-item jab, the shield bubble, and the dead smash menu. **None needed a device
or a repro; every one was read out of the tree.**

⛔⛔ **and four of the day's rows carried a WRONG mechanism that a worker's probe
killed** — D39 (three identity args, not two collapsed), D40 (`label_for`
returned `Some`, and my *correction* was wrong too), D43 (an absence grepped over
two guessed filenames), D55 (refuted by my own arithmetic before it shipped).
⇒ **a row's diagnosis is a hypothesis until something runs.** The ones that
survived were the ones with a falsifier written in *before* the work started.
...
- ▢ **D56 ⭐⭐ THE RENDERER BINDS A SHEET BY DISPLAY NAME WHILE EVERYTHING ELSE
  ...
  * ⚠ **FALSIFIER before touching it**: author `character_id` on ONE `EnemySpawn`
    whose display name differs from its catalog id, and confirm the body draws
    unclaimed. If it draws correctly, the index is already keyed on something
    else and this row is wrong. ⭐ cheap, and it also builds D48's first adopter.
...
- ▢ **D51 ⭐ `HeldUseBehavior::Auto` DECIDES THROW-OR-NOT AND NEVER DECIDES
...
- ▢ **D48 ⭐⭐ AN ENEMY SPAWN CAN SAY WHAT IT LOOKS LIKE AND WHAT IT DOES, BUT
  ...
  * **(b) an enemy is an ARCHETYPE wearing a character**, which is today's model
  ...
  the spawn must be able to declare which it is. **This belongs in
  `awaiting-maintainer-decision.md`** once someone scopes the work behind each
  arm; it is not scoped yet, so it is not filed there.
...
- ▢ **D47 ⭐⭐ THE FUNCTION THAT KNOWS "THE DRAWING" FROM "THE BODY" HAS EXACTLY
  ...
  * ⚠ **FALSIFIER, cheap and decisive, run it before touching anything**: of the
    characters Jon named, how many are in the 34 — and of those, how many are
    ALREADY tight? `player_robot_v3` is in the 34; the ICA is in the 34 and is
    still wrong. **Compute body-area ÷ frame-area for all 34 and sort it.** If
    the miss-sized cover pirates, `ai_slop`, `solid_snake` and the patent clerk
    are in the 156, half 1 fixes none of the complaints and the row is entirely a
    rig project. **Write the answer here before starting.**
