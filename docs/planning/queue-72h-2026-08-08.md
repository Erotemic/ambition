...
| **D74** | the PCA is held off the playable roster by a **fragile instrument** | ▢ **PROBED 2026-08-10** — the "zero melee" claim no longer reproduces and geometry is NOT sticky; what actually reds is `fly_frames > 0`, an assertion about a rare emergent behaviour. Re-scoped |
...
- ▢ **D74 A FIGHT THAT STARTS BEFORE ITS SHEETS LAND NEVER RECOVERS.** Found
  ...
  ⇒ **what remains for whoever unblocks the PCA**: swap `fly_frames > 0` in
  `duel_fighters_actually_enact_their_abilities_on_the_body` for the capability
  claim plus a citation of those two, exactly as the dash comment does, then add
  `perfect_cellular_automaton` to `PLAYABLE_ROSTER` and delete the workaround
  comment. ⛔ do NOT simply delete the assertion — the F0e precedent is a
  REPLACEMENT with a stricter direct test, and that is why it was allowed.
