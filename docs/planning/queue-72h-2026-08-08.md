...
- The rollback schema is at **v20** (`registry.rs:125`), **423 baseline rows** /
  ...
  **and this line**. The absence-contract ratchet catches the third and the
  ...
  ⚠ **`--test rollback_schema_baseline` does not resolve** — `ambition_app` sets
  `autotests = false` with one `[[test]] app_it`. Use
  `--test app_it -- rollback_schema_baseline`.
...
- ▢ **D59 `cargo test -p ambition_render` CANNOT LINK — a stale artifact in the
  ...
  **Ruled out, so nobody repeats them** — the failure is byte-identical after
  ...
  ⚠ **THE FALSIFIER IS RUNNING**: `CARGO_TARGET_DIR=<fresh> cargo test -p
  ambition_render`. **Cold and still failing ⇒ not a cache, and the hypothesis
  above is live.** Cold and passing ⇒ it WAS the shared dir, and the interesting
  question becomes what poisoned it that three targeted cleans could not reach.
  **Write the answer into this row before anyone acts on it.**
  * ⭐ if it is the CGU reading, the cheap probes are
    `CARGO_PROFILE_DEV_CODEGEN_UNITS=1` and `-C lto=off` on the test build; the
    fix is a profile setting, not a clean.
...
- ▢ **D57 ⛔⛔ THE SPIKE TESTS ARE GREEN ONLY IN THE CONFIGURATION WHERE INPUT
...
- ▢ **D51 ⭐ `HeldUseBehavior::Auto` DECIDES THROW-OR-NOT AND NEVER DECIDES
  ...
  * ⚠ **FALSIFIER, and it is one grep**: does anything already intercept a plain
    Attack for an `Auto` item with a ranged verb? `UseSystem` items are
    intercepted by name (`fire_puppy_slug_gun_system`, `fire_shockwave_system`);
    if an equivalent exists for `Auto`, this chain is wrong and the jab is
    surviving for a different reason. **Write the answer into this row before
    changing anything.**
