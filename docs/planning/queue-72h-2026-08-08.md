...
- ◐ **D95 THIRTY-TWO PLACEMENTS HAVE THE RIGHT RESPAWN POLICY BY COINCIDENCE**
  ...
  ⇥ ▢ what is left is the DEMOS' own: `mary_o_ai_slop` (14), `mary_o_snake` (6),
  `sanic_badnik` (4), the two plane-swarm keys (4). ⛔ those resolve against
  MARY-O's and SANIC's rosters, not Ambition's, so "resolves nothing" was measured
  against the wrong cast for them — each demo has to be asked separately, and that
  is a different measurement rather than more of this one.
