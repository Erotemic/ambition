...
⛔⛔ **M2 IS NOT CLOSED — one bounded COMPOSITION proof is still owed (maintainer,
2026-08-14), and the per-view work must not be overstated.** The plumbing is real:
per-camera `camera_follow`, per-camera viewports, `PresentedForView` projections
and render isolation derived from that relationship. What has NOT been shown is
**two explicit `LocalView`s with two explicitly associated cameras and DISTINCT
gameplay rectangles, assembled the way a host actually assembles a session** —
and under this project's acceptance doctrine a fixture that bypasses the assembly
path does not prove the composition supports two views. ⇒ **if the production
assembly cannot yet express two views, THAT IS THE FINDING**, not something to
fake.

⛔ **and the ambiguous "take the first view" authority must be removed or
rejected**: any surviving `views.iter().next()` in a multiview composition either
is the honest single-view fallback — in which case it must **REFUSE LOUDLY when
several views exist**, matching `PresentedViewState::get()` — or it silently picks
one, which is the defect. ⭐ **the standard already set in this seam: silently
drawing something is worse than declining to draw.**

⇒ **then STOP.** Close the bounded slice; do not expand into networking,
transport, participant slots, a layout-policy enum, `MainCameraEntity`, or the
parallax `.single()` gap.
