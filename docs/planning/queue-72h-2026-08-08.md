...
- ▢ **D81 ⭐ THE `BrainPreset` RETIREMENT, MEASURED — 125 ADOPTERS, 8 LIVE KEYS,
  ...
  **The census (2026-08-11), so no step begins with a survey.** Adopters counted
  ...
  ⇒ **what this changes is the ORDER, not the size.** `wanderer_puppy_slug` and
  `skirmisher_ranger` really are the cheapest keys to retire — one character
  each — and that window is open only while the sidecars are inert. When the
  actor-contract migration lands and they ARE consumed, the same two keys cost
  14 and 6 art files to re-author. ⭐ **retire them before the sidecar migration,
  not after.**
