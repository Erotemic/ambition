...
- ▢ **D73 ⭐⭐ THE RUN'S TOP PRIORITY, SET BY JON 2026-08-10: A CHARACTER IS A
  ...
  **Read that file before touching character construction.** It answers D48 —
  ...
  ⭐ **THE NEXT SLICE: route the NPC path through the plan**, which is what
...
- ✔ **D38 THE PROCEDURAL SFX PATH HAS A LOUDNESS TARGET. ⛔ AND JON'S PREMISE
  ...
  **+3.6 dB over the sfx median** (19 procedural sounds, loudest −17.8 RMS
  ...
  ⛔ **regenerate before citing the report.** `scripts/audio_levels.py` has been
  modified since `dev/audio_loudness_report.md` was written (script 2026-08-10,
  report 2026-08-08 21:42), so the checked-in numbers are not guaranteed to be
  what the current script produces. The report's own header already says
  *"regenerate after any audio edit; the numbers describe the tree at
  measurement time"* — this is the same rule applied to the MEASURER rather than
  the measured.
