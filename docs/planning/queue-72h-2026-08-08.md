...
| **D74** | registering the PCA moves the possessed body **580 px** and off the ground | ▢ **MEASURED 2026-08-10** — gravity and size are identical in both builds; the possession sequence diverges. Five mechanisms asserted, four refuted; next step is a per-step position log |
...
- ▢ **D74 A FIGHT THAT STARTS BEFORE ITS SHEETS LAND NEVER RECOVERS.** Found
  ...
  * ⭐ `autonomous_reconcile::peaceful_config` (`is_aerial = matches!(catalog
  ...
  rebuild, `is_aerial`). ⇒ do not add a sixth from reading code. The next step is
  the only one left that cannot be guessed wrong: log the body's position per
  step through the 900-step possession loop in both builds and find the frame
  they diverge.
