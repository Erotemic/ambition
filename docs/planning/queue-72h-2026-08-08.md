...
- ▢ **D86 ROBOT v3 OFF `HostCode` — ⭐ THE PRODUCT DECISION IS IN: THE ROBOT
  ...
  ⇥ ▢ what remains is §5: the robot authors its canonical `ActionSet`, a general
  helper filters it by effective abilities, and `PlayableKitSource::HostCode` /
  `PreparedKit::HostCode` are DELETED rather than left empty.
