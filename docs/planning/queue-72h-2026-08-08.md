...
- ▢ **D112 ⛔⛔ MARY-O DOES NOT DIE TO A SNAKE'S SIDE CONTACT — a shipped-game
  death road is broken, found failing at HEAD (2026-08-13), NOT caused by the
  AC6 content work (verified by stash: fails identically without it).**
  `mary_o_it::level_1_acceptance::a_small_mary_o_dies_to_one_hit_and_the_level_restarts`:
  she is placed overlapping a snake, side contact, no armor — and her HP stays
  1 for 600 frames; no death, no restart.

  ⭐ **WHAT PROBES ALREADY ESTABLISHED** (instrumentation since removed — do not
  re-derive): 1-1 stages FIVE snakes; the test's `some_snake` (id-sorted) picks
  `EnemySpawn-106877`, whose published `CenteredAabb` and kin AGREE (~292, 393)
  — no stale-hurtbox story. The teleport works: she sits at x≈291.5 on the
  ground, overlapping that snake, every sampled frame. She is VULNERABLE
  throughout (`Invulnerability(0)`, not evading, shield inactive,
  `damage_invuln_timer 0`). Every enemy holds `ActorTarget = her entity`,
  `body_contact_damage: true`, `damage_amount: 1`. A `Messages<HitEvent>` len
  probe over the first 10 frames after contact printed ZERO events.

  ⇒ the drop is inside the shared body-contact pass or upstream of it:
  `actors/update.rs` ~1395–1470 (pass 1 snapshots `contact_attack()` per
  attacker with a resolved `ActorTarget`; pass 2 checks `body_vulnerable` and
  `attack.hit_event(..)` overlap). Every gate probed from OUTSIDE passes, so
  instrument INSIDE: does pass 1 push a pending attack for 106877 at all, and
  if yes, which pass-2 gate eats it? ⚠ suspects with motive: AC3's `BodyCombat`
  12→7 (`body_vulnerable`'s inputs), AC1's mirror deletion (a per-frame sync
  road removed), D110's hitlag scoping. ⚠ the OTHER contact direction works —
  `landing_on_a_snake_stomps_it_instead_of_hurting_her` passes — so compare
  what the stomp road reads that the hurt road doesn't. Find the last commit
  where the test passed before modelling anything.
