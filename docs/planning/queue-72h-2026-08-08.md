...
- ▢ **D78 ⛔⛔ A HOSTILE CHARACTER-FIRST CRAWLER DESYNCS UNDER ROLLBACK.**
  ...
  * the change-tick recovery fix below was IMPLEMENTED AND TRIED — a
  ...
  ⇒ the reason, and it is the thing to design around: for a WORN body there is a
  SECOND writer, `apply_worn_character_gameplay`, which owns the same component
  and replaces it on its own schedule. So the projection's "is it already equal"
  test compares against a value that writer may have just changed, the skip does
  not fire, and the mid-run write happens anyway. ⇒ **the two writers have to be
  reconciled before this can be fixed**, which is campaign item P3.23/P3.26
  territory (`HostCode` vs authored data) rather than a rollback patch. Do not
  attempt a third variant of "grant it somewhere else" without first deciding
  which system owns a worn body's `ActionSet`.
