...
- ▢ **D104 ⛔⛔⛔ `reconcile_autonomous_actors` HAS NO PRODUCTION CALL SITE, AND
  ...
  ⇒ ▢ **the next step is GPT's, and it is the right one: prove it end to end.**
  A test through the real `LoadWorld` machinery that crosses peaceful → provoked
  and provoked → peaceful, WITHOUT calling `reconcile_autonomous_actors`
  manually. ⛔ every existing test of this file calls it directly, which proves
  the function works and says nothing about whether it runs.
