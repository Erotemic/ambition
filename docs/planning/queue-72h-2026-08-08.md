...
- ◐ **D102 ⭐⭐⭐ THE LAST TWO ARCHETYPE ROWS — NOW ONE — 60 production references
  ...
  ⇒ NOT repaired by restoring the row. What a small skitter IS is **D96 item 3**,
  open and Jon's, and the same shape as `small_lurker` and `large_brute` before
  it — bodies nobody has cast, spawning generic combatants until somebody does.
  Restoring an archetype row that no character needs would be the ontology
  growing back to cover a casting decision.
