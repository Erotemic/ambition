...
- ▢ **D73 ⭐⭐ THE RUN'S TOP PRIORITY, SET BY JON 2026-08-10: A CHARACTER IS A
  ...
  **Read that file before touching character construction.** It answers D48 —
  ...
  ⭐ **THE NEXT SLICE, and why it is the one that makes the next ten cheapest:**
  build `CharacterSpawnPlan` + `SpawnContext` (shape derived in the plan's
  APPENDIX E) and route the authored enemy through it. Everything still open in
  this row is waiting on it — phase 2's content migration is explicitly ordered
  after it, the encounter/programmatic/summon paths have nowhere to lower to
  without it, `PreparedMatch` cannot drop `CharacterRoster` without it, and
  `adopt_character_intrinsics` cannot be deleted until something replaces the
  precedence it performs. ⛔ **do not migrate more fields through the probe
  seam while it does not exist**; that is the specific failure appendix C names.
