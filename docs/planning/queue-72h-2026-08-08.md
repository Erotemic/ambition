...
- ◐ **D104 ⛔⛔⛔ `reconcile_autonomous_actors` HAS NO PRODUCTION CALL SITE, AND
  ...
  ⇥ ▢ **one path is still unproven**: `reconcile_temporary_control`'s insertion
  of a `MountSlot` that was ABSENT at save. A codec cannot insert what the
  snapshot never held, so if that case is reachable, possession/mount across a
  rewind is broken in production today and has been. Same harness, same shape of
  test; that is what decides whether the file goes entirely.
