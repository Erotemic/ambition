...
| **D74** | the PCA is held off the playable roster by **three fragile instruments** | ▢ **2026-08-10, one diagnosis CORRECTED** — the kit is NOT lost (7 attack verbs, melee `Some`); the swipe is retimed and a once-per-step sampler can miss an 0.08s window. Unproven, and the next thing to test |
...
- ▢ **D74 A FIGHT THAT STARTS BEFORE ITS SHEETS LAND NEVER RECOVERS.** Found
  ...
  The kit is not lost, it is RETIMED. The archetype swipe is `0.24 / 0.08 / 0.30`
  and `striker_swipe` is `0.28 / 0.08 / 0.32`. That test samples live `Hitbox`
  components once per `sim.step`, and a step under the fixed harness advances
  MANY sim frames — its own comment says so. An `0.08 s` active window is about
  five frames, so shifting it relative to the sample grid can hide it entirely.

  ⇒ **the likely fault is the sampling, not the swing** — the same family as the
  `fly_frames` assertion one test over, and the third instance of it in this
  area. ⚠ NOT PROVEN. Prove it by stepping one sim frame at a time, or by
  observing a durable record of the strike rather than a live component, BEFORE
  touching the assertion. ⛔ do not relax it on the strength of this paragraph.
