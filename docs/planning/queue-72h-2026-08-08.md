...
| D18 | do coins/score/lives survive a room transition? | free — **and D17, which hid it, is now fixed** |
...
- ▢ **D18 A1.5 — continuity across a room transition, orphaned since 2026-07-25.**
  ...
  *"the next thing to do in this queue"* — two weeks ago.
  ...
  ⭐⭐ **and D17 is why this was never watchable.** A dropped coin was claimed by
  no render family, so nobody could SEE whether coins carry — the two rows are
  the same seam from opposite ends, and A1.5 sat unwatched for two weeks partly
  because its subject was invisible. D17 landed in `0146789`, so the subject is
  visible now and this row is answerable by looking as well as by asserting.
...
- ▢ **D20 ⛔ A SHIPPED BOSS PROBABLY CANNOT SUMMON — found by the D17 agent,
  unfixed, and I verified it by a second route.** A construction-built body gets
  ...
  * the agent's live probe: `counter=None` on every authored enemy in
    `proving_grounds`.
  ⛔ **and the unit tests cannot catch it: they build the summoner BY HAND**, so
  they supply the counter the executor omits. A guard on the real construction
  path is the deliverable, not a fix alone.
  ⚠ **the one link still unverified**: whether a shipped boss is both built by
  the executor AND authored to summon. If no shipped boss summons, this is latent
  rather than live — **check that before calling it a player-facing bug**, and
  the `warn!` means a real occurrence would be in a log.
