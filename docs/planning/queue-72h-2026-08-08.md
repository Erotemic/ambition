...
EXISTS (censused 2026-08-14).** Every other room feature is drawn reactively by a
render family discovering published views, with a loud magenta stand-in for
anything unclaimed. Room construction spawns exactly two visuals directly: the
moving platform and physics debris. ⇒ the next slice is a DELETION — publish
platform rows like any other feature, let a family claim them, delete
`MovingPlatformVisual` and its spawn/sync pair — not a new generic message minted
for one customer. Detail in the focused plan's K3.
