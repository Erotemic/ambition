...
`prefetch_hit=FALSE` — and that last one is structural.**
`NEIGHBOR_PREFETCH_ROOM_BUDGET` is 4 ROOMS; `central_hub_main` has **21 loading
zones** and holds the Hall door (`central_hub_basement` 18, `hall_of_bosses` 10).
So **the prefetch is ANTI-CORRELATED with cost**: it always covers the cheap
corridors and structurally never covers the expensive destinations behind a hub,
which is where a player stands idle and could afford to pay. ⛔ recorded, NOT cut
— raising the budget without a WORK measure re-creates the launch stutter it was
added to prevent (2026-07-30). The redesign is "bound by work, order by expected
cost", and it wants the browser numbers first. **Still open: the burst itself,
