...
⇒ **so K5's real target is `LoadingZone` (302 instances), not paths** — and the
`mounted_on` precedent means the work is adoption, not design.
...
| **`equipment`, `menu`, `quest`** | **0** | **0** | ⭐ fully isolated in both directions |
| `affordances`, `dev`, `gravity` | **0** | 2 / 3 / 3 | ⭐ nothing here depends on them |
...
would begin exactly where change amplification is worst. ⭐ **the honest carve
candidates are the six modules with ZERO inbound edges**, and three of them
(`equipment` 303 lines, `menu` 812, `quest` 27) have zero edges in BOTH
directions — they are already separable and are sitting inside an *actor* monolith
for historical reasons rather than architectural ones. ⚠ **`menu` inside an actor
crate is itself the finding.**
