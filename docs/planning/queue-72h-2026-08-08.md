...
- ▢ **D74 A FIGHT THAT STARTS BEFORE ITS SHEETS LAND NEVER RECOVERS.** Found
  ...
  * ⭐ `autonomous_reconcile::peaceful_config` (`is_aerial = matches!(catalog
  ...
  ⇒ **the next probe, and it is cheap**: print `on_ground`, `surface
  .gravity_scale` and `kin.size` across the attack window, with and without the
  registration. Registration also changes the resolved sprite BODY SIZE
  (`sprite_body_collision_for_character_id_in` finds a row it did not before),
  which is a mundane candidate: a body resized under its own feet is a body in
  the air. ⛔ do not write another mechanism into this row without that output.
