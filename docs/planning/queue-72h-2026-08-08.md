...
- ▢ **D107 ⚠ `PreparedMatch::cast_generation` IS THREADED AND NEVER READ.** The
  ...
  ⇒ ▢ **so the question is narrower than the row first stated**, and it is a
  decision rather than a task: is frozen-plan provenance worth carrying when
  nothing reads it? Two defensible answers — a DIAGNOSTIC (activating a plan
  whose cast has since changed means characters registered after planning, which
  for a match is an anomaly worth one `warn!`), or deletion of accessor AND field
  together. ⛔ **not the accessor alone**: that leaves the field threaded through
  three constructors meaning nothing, which is the `source`-field shape from
  D105. ⚠ and not a rebuild, for the reason above — the comment that forbids it
  is fifteen lines from the field.
