...
- ▢ **D106 ⛔⛔ PORTRAIT TARGETS ARE NEVER CHECKED, IN ANY COMPOSITION.** Found
  ...
  ⚠ **the builder is KEPT.** Deleting it removes the only road to the check
  rather than the reason it is unused. `with_available_sheets` sits beside it and
  IS wired, so the pattern works and this one was simply never connected —
  ⇒ the work is to connect it where sheets are connected, and to prove it with a
  character naming a portrait that does not exist.
