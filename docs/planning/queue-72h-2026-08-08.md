...
- ✔ **D97 THE `default_brain` DELETIONS RAN AHEAD OF THE RUNTIME, AND TWO SHIPPED
  ...
  * **The other 21 spawn fine and are worse**, because nothing says so: they
  ...
  ⇥ ▢ **still open, and named so it is not lost**: `default_brain_profile` — the
  `BrainPresetRef` half of the fork — now has zero authors and one consumer. It
  goes when the preset vocabulary does (D81), not before, because the ~125
  catalog `brain_presets` adopters still resolve through it.
