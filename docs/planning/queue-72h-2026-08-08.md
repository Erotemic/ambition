...
- ▢ **D110 ⛔ HITLAG IS PRIMARY-PLAYER-SCOPED — a hit between two fighters that
  ...
  **Nothing on the actor road reads it.** Every reader is one of four:
  ...
  and consumed only on the player's road. ⇒ worth asking whether `hitstop` wants
  the treatment `hard_lock_timer()` just got — one named rule both roads call.
...
- ▢ **D107 ⛔⛔ `BodyCombat::attacking` IS FALSE FOR ALMOST EVERY BODY THAT IS
  ...
  * it is written in exactly ONE place — `write_player_ecs_components`, whose
    ...
    set.**
  * what it mirrors is `BodyMelee::is_swinging()`, the FLAT swing. A body
    attacking through the MOVESET runtime (`MovePlayback`) never touches it — and
    that is every Smash fighter and every character with an authored moveset.
  ...
  ⇥ ✔ **HALF FIXED THE SAME DAY — the moveset road now sets it.**
  `write_player_ecs_components` reads `attack.is_swinging() || playback.is_some()`,
  so a PLAYER mid-moveset-swing no longer publishes `attacking: false`. Pinned by
  `a_moveset_swing_reports_attacking_even_though_the_flat_swing_is_idle` and
  falsified by reverting the `||` — the test goes red on the exact sentence.
  ⚠ the fixture builds a minimal `MoveSpec` through `ambition_entity_catalog`,
  because `ambition_combat`'s `MoveSpec` is a PRIVATE re-import and the monolith
  cannot name it — which is also why the monolith never constructs a playback in
  production.

  ⇥ ▢ **the other half is untouched and is the design call.** The query is still
  ...
  field and have the instruments read `MovePlayback`. ⛔ **the third is probably right** — a mirror that has to be
  kept in sync with two attack roads is the kind of duplicate this campaign
  removes — but it is a real design call, not a cleanup.
