...
- ▢ **D114 ⛔ HITLAG IS PRIMARY-PLAYER-SCOPED — a hit between two fighters that
  ...
  **Nothing on the actor road reads it.** Every reader is one of four:
  ...
  ⇥ ⛔⛔ **THE THIRD CANDIDATE — THIS ROW'S OWN RECOMMENDATION — DEADLOCKS, and
  it was written and reverted before landing (2026-08-14).** *"Extend the global
  beat to any hit between seated fighters"* costs one run condition, exactly as
  the row says. It also hangs the game:

  ```text
  player bodies   input_systems.rs  decay_reaction_timers(frame_dt)   REAL time
  actor bodies    actors/update.rs  decay_reaction_timers(dt)         SIM time
  boss bodies     bosses/tick.rs    decay_reaction_timers(dt)         SIM time
  ```

  ⇒ the player-involved freeze terminates **because the player's timers decay on
  frame time**, so a sim clock at 0.0 cannot stop the thing that ends it. A
  seated CPU fighter is an ACTOR: freeze the sim for its hitlag and its
  `hitstop_timer` stops decaying, and the world never comes back — which is the
  precise shape of Jon's 2026-08-07 freeze (*"the characters are just stuck in
  air"*, menus still working) that the no-primary rung was written to fix.

  ⭐ **so the row's remaining work has a PREREQUISITE it did not name**: the
  reaction-timer decay clock has to stop depending on which road a body is on.
  Either the actor road decays hitstop on frame time as the player road does (one
  timer, not the set — hitstun and i-frames are sim facts and must stay sim), or
  the beat is asked for at a non-zero scale that still advances the timer. Until
  one of those exists, *any* implementation of candidate 3 is a hang.

  ⚠ **the asymmetry itself is still worth a look**: a decay clock that differs by
  road is the same family as D108 and this row's own finding — a per-body fact
  maintained differently depending on who owns the body.
