...
- ▢ **D79 ⭐ MARY-O'S TWO ROSTER FRAGMENTS ARE THE NEXT DELETION, AND THE RECIPE
  IS PROVEN.** Sanic's went on 2026-08-11 (`5e050e050`) and Mary-O's are the
  ...
  * ⚠ expect `app_local_catalog_composition` to need the same honest repair
    ...
    existence.
  ⇥ and the stale warning is already gone: those rows carried a doc comment
  saying they were RESTORED because "a multi-game shell host publishes an EMPTY
  prepared cast" (D75). Sanic's deletion disproved it — shell-host suite 20/20,
  `app_it` 327/327 — so do not let that comment stop the other two.
