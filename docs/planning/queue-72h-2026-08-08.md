...
- ✅ **D18 A1.5 — continuity across a room transition. CLOSED 2026-08-08, and
  ...
  The mark carried from `queue-24h-2026-07-25.md` was
  *"continuity: score, coins, lives, and worn power across the transition. Assert
  against the BODY, never the emitter's bookkeeping."*
  ⛔ **"grepped before queuing and nothing covers it" was WRONG, and the grep is
  why.** It searched `game/ambition_app/tests/` and landed on
  `authored_portals_survive_both_death_and_manual_resets` and
  `a_room_transition_survives_the_rollback_window`, then concluded absence. The
  row's own section is the correction: A1.5 sits under **"A1. A demo can have two
  rooms + Mary-O World 1-2"**, directly beneath A1.3 (*1-2 is a real second
  `RoomSpec`*) and A1.4 (*the ferry*). Score / coins / lives / worn power are not
  engine vocabulary — they are the four readouts of **Mary-O's HUD**. The
  transition the row means is 1-1 → 1-2, and its guard was always going to live
  in `game/ambition_demo_mary_o_app/`, which the grep never opened.
  ...
  *"NOT covered yet: crossing while GROWN … a set-up equip would prove the
  ...
  ⭐ **`queue-24h-2026-07-25.md` is archivable now**: A1.5 was its last open mark
  and is marked ✅ there.
