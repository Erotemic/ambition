...
- ▢ **D90 ⛔ `smash_it` WAS 14/18 AND THE GATE COULD NOT SEE IT — 16/18 now, two
  ...
  * `unsatisfiable_seats` consulted the archetype table ALONE, so four perfectly
    ...
    ladder test.
  ⇥ ▢ **the two that remain are BEHAVIOURAL and predate this batch** (measured by
  stashing and re-running at HEAD): `the_fighter_brain_engages_rather_than_
  standing_still` reports *"neither fighter moved in 120 ticks"* with both bodies
  frozen at their spawn x, and `losing_a_stock_announces_a_body_restart` fails
  beside it. ⚠ **do NOT assume they are the roster deletion's** — that is exactly
  what this row proved wrong about the other two; stash back to before the smash
  campaign and bisect.
