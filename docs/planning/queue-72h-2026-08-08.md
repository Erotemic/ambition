...
- ▢ **D96 SIX CONTENT DECISIONS ARE STACKED UP, AND EVERY ONE OF THEM BLOCKS A
  ...
  ```text
   ...
   3  what is under_town_skitter      `OPEN_CASTING`-adjacent: the ONLY namer   1 of 65 EnemySpawn
                                      of the `medium_striker` ROW               placements
   4  is dive_drill's Target a sandbag the other placement with no character_id  1 of 65
  ...
  ⇒ **(1)+(2)+(3) are the acceptance signal.** `character_archetypes.ron` is down
