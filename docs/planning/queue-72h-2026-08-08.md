...
- ▢ **D78 ⛔⛔ A HOSTILE CHARACTER-FIRST CRAWLER DESYNCS UNDER ROLLBACK.**
  ...
  ⇒ next probe: name the character on a placement in a room the oracle does NOT
  simulate and see whether the checksum still moves. If it does, the defect is
  ordering/global and not about that body at all. ⚠ read the three prior
