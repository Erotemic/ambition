...
⛔⛔ **AND IT OPENED ONE HAZARD, recorded on `ItemCustody`:** carrying an
**authored** placement out of its room and back now yields the carried object
**and a freshly authored copy with the same `SimId::placement(..)`**. It could not
arise while the boundary destroyed the object. ⇒ closing it needs the
construction plan to know a placement is **in custody** — durable **room** state,
and the exact twin of the still-open inventory leg.
