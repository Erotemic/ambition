...
- ▢ **D75 SEVERAL HOSTS REACH ENEMY CONSTRUCTION WITH AN EMPTY PREPARED CAST.**
  ...
  **Where to start, given that**: the cast is not LATE, it is absent from
  whatever world the spawn reads. `composes_through_the_sdk` asserts (and
  passes) that BOTH mounted games' characters are in the registry when the test
  looks — so the same App has a populated registry at one moment and an empty
  one at construction. Suspect a second `PreparedCharacterRegistry` (a session
  world, a sub-App, a scoped resource) rather than a timing race.
