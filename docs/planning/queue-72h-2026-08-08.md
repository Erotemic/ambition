...
- ▢ **D107 ⚠ `PreparedMatch::cast_generation` IS THREADED AND NEVER READ.** The
  ...
  ⇒ so a match plan carries a staleness token nobody checks. Either a consumer
  should check it — a seat built against a cast that has since changed is exactly
  the bug the token was invented for — or the token is ceremony and goes with its
  field. ⛔ **do not resolve this by deleting the accessor alone**: that leaves
  the field threaded through three constructors, which is the `source`-field
  shape from D105 (a value carried everywhere, meaning nothing, printing
  nothing).
