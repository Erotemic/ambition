...
- ▢ **D39 THE GOBLIN ENCOUNTER DRAWS MAGENTA BOXES BECAUSE ITS MOBS ARE NOT
  ...
  * **the fork, and it is a design choice rather than a bug fix**:
    ...
    third spawn path.
  * ⚠ **check the sibling cases before landing**: the same by-name resolution
    is what leaves `NpcSpawn-0017` and `hub_gravity_switch` unclaimed in the
    device log, so this may be one defect with three faces rather than three.
