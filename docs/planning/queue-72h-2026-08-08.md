...
- ▢ **D46 THE UNCLAIMED-BODY WARNING CANNOT SAY WHETHER ITS BOX IS STILL ON
  ...
  * **the fix is to the INSTRUMENT, and it is the same shape as D36.** A
    placeholder that gets despawned is a non-event; one that survives is the
    defect. So report on RETIREMENT, not on spawn: warn (or count) when a
    placeholder has stood for longer than some small number of frames, and
    downgrade the spawn-time line to debug. ⭐ the harmful and harmless cases
    become distinguishable in the log, which is the entire ask.
