...
| row | what | state |
|---|---|---|
| **D71** | ⭐⭐ rooms changed with **no transition transaction** | ✔ **ANSWERED, NOT STALE** — 24 room changes / **0** transactions under the ROLLBACK host the shipped binary composes; the bypass is `detect_room_transition_system`'s deferred branch. ▢ fix outstanding |
