...
| row | what | state |
|---|---|---|
| **D33** | the decomposition brief | steps 1–4 ✔; ⭐ **the carve is DECIDED — plugin shape or not at all** |
| **D28** | compile/test time in real work | recorder ✔; rhythm answered |
| **D45** | sixteen observations from Jon | INDEX only |
| **D42** | the patent clerk's rig | ✔ **answered: the anchor absorbs it, no code change** — an editor fix if anything |
| **D59** | `ambition_render` lib test cannot link | diagnosed; ⭐ likely `rust-analyzer` sharing the target dir — **Jon's one-line setting** |
| **D58** | ⭐⭐ a finger cannot tap a smash portrait | **the real Android menu bug**, confirmed two routes |
| **D56** | the renderer binds a sheet by DISPLAY NAME | ⛔ **must land with D48** |
| **D55** | the shield bubble | ⛔ **blocked on a picture** — I argued in a circle twice |
| **D54** | a slash with no owner | ✔ **fixed** — absent owner is now an absent slash + a warn |
| **D53** | the Android suspend `.take()` | ✔ fixed, ⛔ **uncompiled — no NDK here** |
| **D52** | quality Apply | ✔ the false prop stamp fixed; ▢ provenance + bounded-memory Apply remain |
| **D51** | the held item does not claim the Attack press | **IN FLIGHT** |
| **D50** | the dropped weapon is session-scoped | Jon's laser sword |
| **D49** | ⭐⭐ the LDtk editor nulled every `EntityRef` | ✔ **guard landed** (ratchet, 4 known); refs deliberately unrepaired |
| **D48** | is an enemy a CHARACTER or an ARCHETYPE? | ⛔ **SCOPED AND FILED FOR JON** — 41 of 65 already name a character |
