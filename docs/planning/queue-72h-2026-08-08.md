...
- ▢ **D22 THREE ROWS THAT READ AS OPEN ARE SITTING IN THE ARCHIVE.** Surfaced by
  ...
  * same file `:1968` — *"milestone 5 is NOT reached, and here is the number"*.
  ...
  ⭐ **and the base rate says check**: twice today a retired ledger looked
  archivable and held live rows — E1a's unguarded room-scope fix, and a
  continuity row open since the day it was filed. Both were real. **Two for two.**
  ⛔ **grep for what each says is missing before assuming either way** — an
  archived row is exactly as likely to be stale as a live one, and these are
  older.
