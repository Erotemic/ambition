...
1. **The substrate.** `authored definition + spawn provenance → runtime instance
   + lifetime policy + persistence policy → explicit terminal transition`. One
   model must hold a normally-unique named character, twenty mobs from one
   `CharacterDefinition`, a persistent quest object and an ephemeral projectile
   **without pretending they share persistence semantics**. ⛔ authored identity
   does NOT imply world uniqueness — "there is normally one Fia" is content
   policy, not the meaning of a definition id. ⛔ **do not invent a universal
   `EntityId`**; the separation is settled, the exact identity types deliberately
   are not.
