...
- ◐ **D105 ⭐⭐ "WHOSE ONLY CALLERS ARE ITS OWN TESTS?" IS THE QUESTION D104
  ...
  ⇒ so this row is NOT "write a script that greps". The tool that has been right
  every time is the COMPILER: `#[deprecated]` on a suspect item and read
  `cargo check`, which sees through re-exports, macros and test cfgs. A guard
  worth having automates THAT, or it does not exist.
