...
- `EnemySpawnSpec` — the census is done, the TYPE is what remains (line 673).
- `WornCharacter` → universal `CharacterIdentity` — blocked, and the blocker
  SHRANK rather than vanishing: the 14 characters that cannot build a body from
  their own definition still fall back to the catalog, so enrolling everybody
  hands them the catalog's kit.
- Prepared completeness (appendix C ruling 8) for death traits — ⭐ **its stated
  blocker is GONE**: `adopt_character_intrinsics` is deleted (AC5.3), the
  archetype is deleted (AC6.1), the mites author their own traits. Whether the
  ruling is now free to take is a measurement nobody has made.
- Still string-typed: `PreparedSeat::character_id` and several `&str` accessors.
