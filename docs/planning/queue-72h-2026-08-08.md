...
- ▢ **D75 SEVERAL HOSTS REACH ENEMY CONSTRUCTION WITH AN EMPTY PREPARED CAST.**
  ...
  **Where to start**: `ActorPlacementContext::prepared` defaults to an empty
  ...
  ⇥ ▢ **WHAT IS STILL UNVERIFIED, and the instrument is why.** Whether the two
  named hosts still reach that branch is NOT settled: `bevy::log::warn!` prints
  nothing without a `LogPlugin`, so `composes_through_the_sdk` running green and
  silent is not evidence either way — the same class of trap this row already
  documented once (libtest capturing stdout made three reverted changes look
  like a fix). ⛔ do not close this row on a quiet test run. The measurement that
  would settle it is the one that found the cause: a REQUIRED `Res<..>` at the
  spawn seam, or a probe that reads the registry out of the finished world.
