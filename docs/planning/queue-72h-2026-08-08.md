...
- ▢ **D81 ⭐ THE `BrainPreset` RETIREMENT, MEASURED — 125 ADOPTERS, 8 LIVE KEYS,
  ...
  * ⭐ **AND `parrot_lively` IS THE NEXT FREE ONE — with a live disagreement in
    ...
    ⛔ **blocked on a SCHEMA fact, not on content**: `default_brain` is a required
    `String`, so the row must name SOME preset, and `parrot_lively` has only this
    namer. Making that field optional is the unblock — after which every character
    that authors its own profile stops naming a vocabulary it does not use, and
    each preset dies with its last namer.
