...
| **D75** | ⭐ Smash has **no knockback GROWTH** — Jon reported it and was right | ▢ **NEW 2026-08-10** — the engine grows launch by percent, the duelists' prefab melee authors `kb_growth: 0.0`, and `scaled_knockback` short-circuits on zero. Authoring fix; do not widen the melee specs |
| **D74** | a fight that starts before its sheets land **never recovers** | ▢ **NEW 2026-08-10** — known and unexamined in `character_catalog.rs`; the PCA is held off the playable roster to keep one instrument green, and D73 phase 2 needs that workaround gone |
...
- ▢ **D75 SMASH HAS NO KNOCKBACK GROWTH, AND JON IS RIGHT ABOUT IT.** His
...
- ▢ **D74 A FIGHT THAT STARTS BEFORE ITS SHEETS LAND NEVER RECOVERS.** Found
  ...
  out while the underlying stickiness stands. ⭐ start by asking whether the
  geometry is re-derived when the sheet arrives or only sampled once at spawn —
  that is one grep and it decides whether this is a timing bug or a caching one.
