...
⚠ **three limits recorded in the plan, not hidden:** blast margins are probed at
**zero** while the stage authors 120px (conservative by exactly that) · ⛔ **the
ledge-transition case is the one NOT pinned**, and it is the common one — the
capture point tests the body's **centre** against `ground_span` while the kernel
uses the **footprint**, and at a ledge those disagree by a half-extent · cost is
unmeasured, and the existing budget bench rolls no movement lines so it does not
price the lens.
