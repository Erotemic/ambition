...
- ▢ **D52 ⭐⭐ LIVE QUALITY APPLY CONVERGES ONE CLASS OF SPRITE AND SILENTLY
  ...
  * **Apply is load-new-then-release-old**, i.e. the staged strategy, when the
    plan of record says Android should get `cover → detach → drop old → load new
    → rebind → reveal`. ⛔ **on the device that motivated this campaign, a
    participant lowering quality because memory is tight momentarily needs BOTH
    generations.**
  * **Apply re-demands every declared realization still in the table**, not the
    set the current room needs — so it changes the flavour of historically
    accumulated residency without bounding it. ⇒ **the `193 MB → 527 MB` problem
    is not solved yet**, and I should not say it is.

  ⚠ **falsifier for the whole "reported" block**: each is a claim about code I
  have not read. Verify by a different route before acting
  ([[feedback_ask_the_tool_dont_model_it]]) — and note the review itself declined
  to promote a `body_pixel_extent` concern to a finding because it could not name
  a shipped sheet that fails, which is the standard to hold these to as well.
