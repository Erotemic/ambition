...
- ▢ **D48 ✔✔ ANSWERED BY JON 2026-08-10 AND SUPERSEDED BY D73 — READ THAT ROW,
  NOT THIS ONE.** The fork this row posed (*is an enemy a CHARACTER, or an
...
- ▢ **D90 ⛔ `smash_it` WAS 14/18 AND THE GATE COULD NOT SEE IT — 16/18 now, two
  ...
  * `unsatisfiable_seats` consulted the archetype table ALONE, so four perfectly
  ...
  ⇥ ▢ **WHAT IS LEFT IS A REAL BUG, and the fixed window is what exposed it**:
  the CPU fighter **travels but never commits**. Seat 1 walks 416 → 133 → 513 past
  a stationary opponent at 224 and nobody is ever hit (`percent 0.0` on both after
  120 AND after 240 ticks). The test's own message is exactly right — *"the brain
  travels but never commits"*.
  * ⚠ **ruled out**: the ruleset's `fighter_abilities` DOES grant `attack` and
    `shield`, so the new `ActionSet::gated_by` mask is not stripping the melee;
    and `smash_duelist_b` resolves the `duelist` preset, which authors a real
    4-damage swipe.
  * ⇒ next probe: the fighter brain's TARGETING. Seat 0 is a human seat with no
    controller — a statue — and the question is whether the Smash driver treats a
    hostile-but-inert body as a target worth committing to at all.
  ⇥ ▢ `losing_a_stock_announces_a_body_restart` fails beside it and is very likely
  the same cause: no hits means no KOs means no stock lost.
...
- ▢ **D89 ⛔ THE PCA AUTHORS ITS WHOLE BODY AND THE DUEL STILL WILL NOT TAKE IT
  ...
  * ⇒ **the fix is to migrate that three-state onto the character**, not to flip
  ...
  1. **the body shrinks to 48px** where `duel_pca_body_is_sprite_authored...`
     wants >60. `body_kind: Floating` drives the COLLISION BOX as well as the
     locomotion, and separating those is the real remaining change. ⚠ measured:
     the size regresses with the three-state applied even with `body_kind`
     restored to `Floating` and the fly abilities restored — so the coupling is
     NOT simply `is_aerial`, and I did not isolate it. Stash, re-apply, bisect the
     four files.
