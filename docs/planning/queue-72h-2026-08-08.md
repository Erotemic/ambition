...
- ▢ **D55 ⭐⭐ SOLVED BY MEASUREMENT: THE MISPLACED BUBBLE IS PAINTED INTO THE
  ...
  * **(b) delete bubble AND emitter**, and let `BubbleShieldVisual` be the only
  ...
  the bubble is **not** a rig part to hide, so whoever takes this must find where
  the generator draws it before estimating. **I did not find it**, and I am
  stopping rather than guessing at a fourth mechanism on this row.
