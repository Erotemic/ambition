...
- ✔ **C4c the game's Yarn verbs moved to `game/ambition_content`**
  ...
  moved.
- ▢ **C4d `conversation` is now the ONLY thing pinning `ambition_dialog`** into
  the monolith (plus `dialog.rs`'s generic wiring and three test files). It is
  the reusable continuity authority and keeps that edge wherever it lives, so
  the question is whether it becomes `ambition_conversation` sitting BESIDE the
  monolith. ⚠ **re-measure the inward edges first** — they were two (both the
  bark) at 2026-08-08, and `opening.rs` was added since without changing that.
  A carve is a port for "give me a bark for this character in this situation"
  plus a `Cargo.toml`.
