...
- ▢ **D33 ✔✔ THE CARVE LANDED `a64bf22f8` (2026-08-09) — `ambition_character_sprites`
  ...
  *any* legitimate carve makes false before its `compile_collect.py` run. Now a
  ...
  ⇒ ▢ **the repair, running now**: a SECOND collection against the now-warm
  target dir, which is rebuild-class and does supply weights. ⭐ note it enumerates
  **56** lib roots where the first said **55** — the first run's perturbation list
  was transiently short while the lockfile settled, though its cold build
  measured all 56 anyway. **Then re-freeze on real numbers.**
