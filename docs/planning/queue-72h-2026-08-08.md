...
- ▢ **D19 promote ONE card from `triage/bevy-system-parameter-architecture.md`,
  ...
  **The recommended tool is untouched while the workaround grew by five.** That
  ...
  ⛔ **I could not re-derive its "27 functions with ≥13 parameters" and did not
  publish a replacement.** My counter matched `:` inside a paren span, so `::`
  paths and turbofish inflated it — it reported 197 parameters for one function.
  An implausible number is a broken instrument; counting Rust parameters needs a
  parser, not a regex. **That figure is unverified, not refuted.**
