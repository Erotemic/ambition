...
- ▢ **D92 THE THIRD 08-12 REPORT IS NOT REPRODUCED: *"I can't use F to go
  through doors anymore ... In fact going through loading zones doesn't work
  anymore."*** Everything I can drive says it works.
  ⇥ **what is already GREEN, so the next session does not re-measure it:** all
  four `door_entry` tests, including `a_door_in_the_shipped_host_opens_for_the_
  interact_key` (the SHELL composition, pressing the key read from the live
  `InputMap`) and `a_door_opens_under_a_rollback_host_and_not_only_a_fixed_tick_
  one`. `rollback_room_transition` covers the deferred `LifecycleIntent::
  Transition` commit path incl. an `EdgeExit`. `app_it` 330/330.
  ⇥ **content refuted too:** `sandbox.ldtk` has 123 `LoadingZone` entities and
  **zero** with a missing target; the 52 null EntityRefs in that world are all
  `mounted_on`, which is legitimately empty. So this is not
  [[reference_ldtk_editor_nulls_entity_refs]].
  ⇥ ⭐ **what is left is what only Jon's process has:** his SAVE and his
  SETTINGS. Two candidates, both cheap for him to answer and neither reachable
  from here — (a) the keyboard preset: `ArrowsZxc`/`ArrowsQwer` bind interact to
  **F**, `WasdJkl`/`WasdUipo` bind it to **E**, and a persisted preset index that
  shifted would read exactly as *"F stopped working"*; (b) a `GatePortalRegistry`
  switch state in the save leaving portal-backed zones refusing traversal
  (`allows_traversal`), which would look like "loading zones" while doors on
  non-portal zones still worked.
  ⇥ ⛔ **do not "fix" this by loosening a gate.** Every gate in
  `detect_room_transition_system` is currently proven to pass in-process; a
  change that makes an unreproduced report go away is a change nothing measured.
  Ask Jon which key his build shows in the controls screen first.
