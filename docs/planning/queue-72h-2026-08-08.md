...
- ✔ **D73 CLOSED 2026-08-14 — THE ENEMY-ARCHETYPE SYSTEM IS DELETED AND ONE
  CHARACTER AUTHORITY REMAINS.** The Authority Convergence campaign
  ([`authority-convergance-campaign-2026-08-13.md`](authority-convergance-campaign-2026-08-13.md))
  closed all eight phases with all twelve AC7 exit criteria verified against
  HEAD: `character_archetypes.ron`, `ArchetypeSpec`, `CharacterRoster` and the
  roster family are gone (68 files, ≈2,600 lines); five production roads — NPC,
  authored enemy, summoned minion, encounter wave, match seat — reach ONE
  constructor and there is no second one; an identifier that resolves to no body
  is REFUSED at preparation rather than becoming a generic, with three typed
  variants naming three distinct fixes; and `adopt_character_intrinsics`, the
  build-then-patch bridge, has zero references.

  ⭐ **the two failure modes Jon named are both answered.** (1) `ArchetypeSpec`
  was not migrated wholesale — its three authorities separated: intrinsic body
  facts to the character, controller policy to `BrainProfile`, placement policy
  to the placement, with `ActorTuning`'s own collapse deciding each surviving
  field by ownership. (2) The old model was not wrapped: the deletion is
  measured in files removed, and the phases that added machinery deleted more
  than they added.

  ⚠ **FIVE ITEMS SURVIVE IN THE BRIEF AND NONE IS A SECOND DEFINITION SYSTEM**:
  the `EnemySpawnSpec::character_id` type (a content migration, since the field
  is genuinely absent on unmigrated LDtk entities), `WornCharacter` →
  `CharacterIdentity`, prepared completeness for death traits (its blocker was
  deleted; it needs re-measuring), and two string-typed `PreparedSeat` ids.
  Those are successor work.

  ⇥ AS WRITTEN: **D73 ⭐⭐ THE RUN'S TOP PRIORITY, SET BY JON 2026-08-10: A
  CHARACTER IS A REUSABLE AUTHORED TEMPLATE, AND THE ENEMY-ARCHETYPE SYSTEM IS
  DELETED.** The
...
- The rollback schema is at **v27** (`registry.rs`). Baseline-row and encoded-type
...
- ✔ **D52 CLOSED 2026-08-13 — all three flagged defects are fixed, and the
  central one is now STRUCTURALLY unreachable.** Verified by reading the code
  rather than the row:

  * ✔ **the prop stamp**: all three `BoundSpriteQuality` sites now write
    `asset.resolved_tier` — including the prop path that used to write the
    REQUESTED `active_sprite_scale(quality)` and mark a Half-built prop `Full`
    forever.
  * ⭐⭐ **and it cannot come back by editing one line**:
    `refresh_prop_sprites_on_game_assets_change` no longer TAKES the quality
    profile (`assets`, `images`, `props` — that is the whole signature), so
    stamping the request would need a system parameter re-added, which is a
    visible change rather than a typo.
  * ✔ **the `assets.is_changed()` early-out is gone**, with the actor path's own
    reason recorded beside it: images decode asynchronously, so the frame
    `GameAssets` changes is not the frame the texture is usable.
  * ✔ **`resident_tiers()` reads `resolved_tier`**, so the API that answers *what
    is physically resident* no longer reports a request. That was the row's
    condition for trusting the Android memory numbers.
  * ✔ **the guard the row said it owed exists**:
    `a_prop_is_stamped_with_the_tier_it_was_actually_built_from`, whose fixture is
    the divergence itself — profile `High` (Full) while the prop table holds only
    `Half`. `ambition_render` 102 green.

  ⚠ **and my first falsifier was wrong, which is worth recording.** I swapped the
  prop stamp to `asset.requested_tier` and the guard stayed green, and I read that
  as *"the test cannot detect its own defect"*. It was the falsifier that was
  wrong: in that fixture the asset is BUILT at Half, so `requested_tier ==
  resolved_tier == Half` and the substitution changes nothing. The original defect
  read the QUALITY PROFILE, not the asset — a value that function can no longer
  see. ⇒ a falsifier has to reproduce the defect's SOURCE, not merely a
  neighbouring field.

- ✔ **D52-ORIGINAL (superseded; text kept) LIVE QUALITY APPLY CONVERGES ONE CLASS OF SPRITE AND SILENTLY
...
- ▢ **D70 ⛔ SOME MARY-O BLOCKS STAY SPENT ACROSS A RESTART — and it is NOT a
  ...
  * ⚠ **the falsifier, and it must name a SPECIFIC block**: bonk a `?`-block,
  ...
  ### ⛔⛔ A MECHANISM I PROPOSED AND THEN REFUTED, SAME HOUR (2026-08-14)

  ⚠ **recorded because it is the obvious theory and the next reader will have
  it too.** Both rearm systems are driven by messages — `RoomLoaded` and
  `RoomReplayRequested` — and both are `clear_message_on_rollback`, while
  `SpentPowerBlocks` and `BrokenBricks` are rollback RESOURCES. So: roll back
  across the frame that carried the request, the queue is wiped, the resource is
  restored to its pre-clear value, nothing re-arms the blocks, and the failure is
  intermittent and scoped to whichever blocks were spent before the window —
  which is *exactly* the shape of *"some blocks from the last run remain spent"*,
  and would explain why a fix that is present, registered, correctly ordered and
  provably reached still fails for Jon while every fixed-tick test passes.

  ⛔ **IT IS WRONG, and the reason is written at the writer's registration.**
  `close_death_interlude` — which writes `RoomReplayRequested` — is scheduled in
  the SIM schedule, in the same frame as its consumer, deliberately:

  > *"that channel is `clear_message_on_rollback`. Written here in `Outcome` it
  > would be consumed … in the NEXT frame's `PlayerInput` — so a rewind across
  > that boundary wipes the message, the resimulated branch never resets the
  > level, and the two runs diverge. Measured: a GGRS sync-test checksum mismatch
  > at the first death … Everything this system reads is SNAPSHOT state, so
  > running it in the same frame as the consumer makes the whole request
  > re-derivable during a resimulation."*

  ⇒ the defect I reconstructed was **already found, measured and fixed**, and
  Mary-O's rearm systems sit in `ContentRoomReplayResetSet`, which is configured
  `.before(apply_room_replay_request_system)` in that same sim schedule — so they
  read the request in the frame it is rewritten. ⭐ **the whole message path is
  rollback-re-derivable, which removes it from D70's suspect list rather than
  adding to it.**

  ⚠ **what the excursion DID establish, and it stands**: nothing escapes the two
  sets. `rearm_all` clears both the spent set and the per-block coin tally schema
  v24 added; neither `bricks.rs` nor `powerups.rs` keeps per-entity state
  (`MaryOSpark` is the only component in either). And the rollback test that
  looks like it already covers this —
  `a_manual_reset_restores_a_damaged_enemy_and_a_broken_brick_under_forced_rollback`
  — does not: its brick is `combat::components::BreakableFeature`, a per-ENTITY
  Ambition component restored by a MANUAL reset. Two mechanisms one word apart.

  ⇒ **so the row's own framing is still the live one: WHICH blocks.** The next
  probe is Jon's reproduction, not a theory — which `?`-block, which brick, and
  in which room.
...
- **D48 ✔✔ ANSWERED BY JON 2026-08-10 AND SUPERSEDED BY D73 — READ THAT ROW,
  ...
  * ⇒ **Jon's *"the pirates no longer ride their sharks"* is likely the same
  ...
  ### ⇥ ⛔⛔ EVERY NUMBER IN THAT CENSUS IS STALE — RE-MEASURED 2026-08-13

  The table above is from before D73's content migration and each of its three
  derived claims is now false. Walking the same four worlds' LDtk JSON:

  ```text
                                          this row    2026-08-13
    EnemySpawn instances, all 4 worlds          65            65
      carrying character_id                      0            63
      carrying mounted_on                        3             7
      rider-brained with NO mount                ?             0
  ```

  * ⛔ **"the art road is 100% display-name" is dead**: 63 of 65 name a character
    directly. The two that do not are `under_town_pipes` / `under_town_skitter`
    and `dive_drill` / "Target" — the two content decisions D96 items 3 and 4
    already track, and the same two the buildability guard reports.
  * ⛔ **"an engine capability that exists, works, and is unused" is dead with
    it**, and so is its consequence — *"renaming any enemy character's
    `display_name` silently un-arts every level that places it"*. A placement that
    names a `character_id` survives a rename; 63 of 65 do.
  * ⛔⛔ **AND JON'S OBSERVATION IS FIXED IN THE CONTENT.** This row reasons that
    *"the pirates no longer ride their sharks"* is an authoring gap because *"the
    pirate-sky raiders' `mounted_on` is `None` while their brain is
    `pirate_shark_rider`"*. **Zero rider-brained `EnemySpawn`s lack a mount
    today**, and `mounted_on` went 3 → 7. The hypothesis was right about the
    mechanism and the data has since changed underneath it.

  ⇒ **the falsifier this row proposes — `git log -p` on `sandbox.ldtk` inside the
  submodule — is no longer needed to decide whether it was a regression or a gap:
  either way it is authored now.** ⚠ what is NOT re-measured is whether the
  pirates ride correctly IN PLAY; this is a census of the authoring, and Jon's
  observation was about a fight.
...
- ✔ **D36 CLOSED 2026-08-14 — THE COLLISION WARNING NOW TELLS ITS HARMFUL CASE
  ...
  * ⭐ **and the chaining arithmetic is checkable**: N records claiming one target
  ...
  ### ⇥ ✔ CLOSED 2026-08-14 — the noise is gone AND the filter landed

  ⭐ **both halves, in the two places that can each see one fact.**
  `ambition_sprite_sheet` records the collision (`shadowed_targets()`) and
  reports the COUNT once — it is the only place both records pass through, and
  its whole claim is to be a content-free vocabulary. `ambition_app` — the caller
  that installs `SheetRegistryPlugin` and owns the catalog — runs
  `report_shadowed_character_sheets`, which warns only for a shadowed target the
  CATALOG knows. A rig name (`toon`, `robot`, `goblin`) is not a catalog id, so
  the 17/18/9 legitimate sharers say nothing; `pirate_heavy_broadside_bess` is,
  so her case still shouts.
  ⚠ `Option` on both resources deliberately: a composition may reach that system
  with neither, and saying nothing is right there — reporting every rig target as
  suspicious is the noise this row is about.

  ⇥ ◐ HALF LANDED 2026-08-14 — the NOISE is gone, the FILTER is a second slice

  ⭐ **the collision is now RECORDED where it is visible and REPORTED once.**
  `SheetRegistry` carries `shadowed_targets() -> &[ShadowedTarget]`, filled
  during `from_baked_table`, and the per-record `warn!` is replaced by a single
  summary line naming the count and the first entry. That kills the ~30-per-boot
  Android noise Jon reported without silencing anything: the day-costing case
  still announces itself on every boot, once, and the full list is a call away.

  ⇒ **what remains is exactly what this row prescribed and could not do inside
  that crate**: filter to targets a reader can actually resolve. The fact needed
  — *is `pirate_heavy_broadside_bess` a key something looks art up by, while
  `toon` is not* — belongs to a caller with a catalog, and `ambition_characters`
  cannot host the check because `ambition_sprite_sheet` depends on IT (the edge
  would be a cycle). ⚠ so the consumer belongs in a crate that sees both —
  `ambition_platformer2d_actor_monolith::character_sprites` is the natural home —
  and it is a startup system reading `SheetRegistry` + `CharacterCatalog` that
  warns per shadowed target whose name is a catalog id. **Small, and deliberately
  not rushed**: a half-wired startup system that reports nothing would silence the
  row while looking closed.
...
- ▢ **D81 ⭐ THE `BrainPreset` RETIREMENT, MEASURED — 125 ADOPTERS, 8 LIVE KEYS,
  ...
  **The census (2026-08-11), so no step begins with a survey.** Adopters counted
  ...
  ⭐ **RE-MEASURED 2026-08-14: every catalog count above is UNCHANGED** (37 / 43
  / 14 / 4 / 1 / 1, and 7 rows carrying `default_brain: ""`). The row's census is
  accurate at HEAD; nothing has moved since 08-13.

  ⛔⛔ **BUT THERE IS A SECOND AUTHORING SURFACE THIS CENSUS NEVER COUNTED, and a
  plain `grep -r` cannot see it.** The preset keys are also named by the sprite
  SIDECARS — `brain: Some((default_preset: Some("wanderer_puppy_slug")))` in
  `*_actor.ron` — which live under `assets/sprites/` and are GITIGNORED, so they
  are invisible to the recursive greps this row was sized with. Counted with
  `find … | xargs grep`, over 168 sidecars:

  | key | catalog `default_brain` | sidecar `brain.default_preset` |
  |---|---:|---:|
  | `stand_still` | 37 | 26 |
  | `patrol_peaceful` | 43 | **84** |
  | `melee_brute_striker` | 14 | 17 |
  | `melee_brute_brute` | 4 | 9 |
  | `wanderer_puppy_slug` | **1** | **14** |
  | `skirmisher_ranger` | **1** | **6** |

  ⚠⚠ **AND THE CONCLUSION THAT NUMBER INVITES IS WRONG — I checked before
  writing it.** *"So the two singletons actually have 15 and 7 adopters"* does
  not follow: `ArtifactClass::ActorSidecar` is documented as *"Transitional
  actor-contract sidecar (`*_actor.ron`). Installed today, **not yet consumed by
  the sandbox**"*, and the runtime `SheetRecord` parses geometry and images only
  — no `brain` block. **Nothing reads them, so nothing breaks if a key is
  deleted today.**

  ⇒ **what this changes is the ORDER, not the size.** The sidecar window is open
  only while they are inert: when the actor-contract migration lands and they ARE
  consumed, `wanderer_puppy_slug` and `skirmisher_ranger` cost 14 and 6 art files
  to re-author on top of everything else. ⭐ **retire keys before the sidecar
  migration, not after.**

  ### ⛔⛔ AND THE RETIREMENT IS GATED ON REGISTRATION — measured 2026-08-14

  I went to retire those two as the cheapest pair (**one adopter each**) and they
  are not cheap, because **neither adopter is a registered character**:
  `npc_puppy_slug_variant2` (`wanderer_puppy_slug`) and `npc_helpful_liar`
  (`skirmisher_ranger`) have **zero references anywhere in the workspace** — they
  exist as catalog ROWS and nothing else.

  ⇒ **a `BrainProfile` needs a `CharacterDefinition` to live on.** An unregistered
  character has none, so its only way to state a policy is the catalog's
  `default_brain` — the exact vocabulary this row retires. The correlation is
  exact and checkable both ways: **every one of the 8 rows that already left
  (`default_brain: ""` — the 7 pirates and Carl Stargan) is registered**, and the
  26 registered ids are where all the movement has come from.

  ⇒ **D81 is DOWNSTREAM of the registration gap, not independent of it** — the
  same population AC5.2 measured (109 of 163 NPC placements naming one of 94
  unregistered characters). That is why the counts have not moved since 08-13:
  the seven that left were the registered ones, and what remains is mostly rows
  for characters nobody has authored.

  ⚠ **so the next step is NOT "retire the two smallest keys"**, and AC4's rule
  says why: *"do not author new capabilities/health/moves solely to drive the
  incomplete count to zero."* Registering two characters to delete two preset
  keys is that move exactly. The honest sequencing is registration first, for
  characters that earn a definition on their own merits, with the preset key
  falling out as a consequence — which is how all eight of the migrated rows
  happened.
...
- ✔ **D113 CLOSED 2026-08-14 — SATISFIED IN CODE, AND NOW GUARDED.** The row
  existed for a gap in EVIDENCE rather than in behaviour: the cap was authored
  and enforced, and nothing said so, so a one-character edit to the constant
  would have restored the bug Jon reported with every test green. The guard
  landed the same day the row was written and is verified passing at HEAD
  (`movement::tests::she_may_have_max_live_sparks_out_at_once_and_not_one_more`,
  1 passed) — it fires with an empty screen, with one spark up, and at the cap,
  so it distinguishes the cap from a body that simply cannot fire.

  ⚠ **the cap is TWO and Jon's note said one**, which is not a contradiction to
  resolve by editing the constant: two-on-screen is the documented rule of the
  genre this demo models, and the standing instruction is to ship the researched
  standard rather than ask. The constant's own doc says so — *"the classic
  ...
  what he asked for, including the scoping. If he wants one, it is a one-line
  authored change with a test that will fail loudly, which is the state this row
  was opened to reach.

  ⇥ AS WRITTEN: **D113 JON'S "MARY-O CAN ONLY HAVE 1 FIREBALL OUT" IS SATISFIED
  IN CODE AND UNRECORDED ANYWHERE** — so the observation still reads open
  (verified 2026-08-13).
...
- ✔ **D108 CLOSED 2026-08-13 — and it was THREE independent ways, not two.**
  (1) the carry: `sync_actor_components_from_cluster` rebuilt `BodyCombat` every
  frame and carried five timers, not six, so a CPU's landing lag was erased the
  next frame — fixed by deleting the save→rebuild→restore entirely, so nothing is
  carried because nothing is disturbed. (2) the decay: the shared
  `decay_reaction_timers` forgot it while the player road spelled its own list —
  fixed by making the shared one THE decay and having the player call it, which
  also fixed a co-op body whose `hit_flash` nothing decayed. (3) ⛔ **the LOCK,
  which no census had counted**: `apply_post_hit_input_gates` took two loose
  `f32`s and the actor road filled the hard-lock slot with `recoil_lock_timer`
  alone, so the timer was set, decayed and rolled back correctly and locked
  nothing. Fixed by passing `&BodyCombat`, which makes the reduced form
  unrepresentable. Guarded by
  `a_staggered_body_loses_input_authority_like_the_player`, demonstrated red at
  14.1px of steering where it must be 0. ⚠ **the difficulty statement is now
  live**: CPU fighters and bosses pay the 0.10–0.28s a human pays.
  ⇥ AS WRITTEN: ▢ **LANDING LAG IS A PLAYER-ONLY MECHANIC, IN TWO INDEPENDENT WAYS —
  a CPU lands clean out of the same aerial that costs a human 0.10–0.28s** (found
  2026-08-13 while closing P4.33's *"unverified in play"*).

  All five Smash aerials author `landing_lag_s`, and `moveset/mod.rs:917` writes
  `combat.landing_lag_timer` for **any** body that lands mid-move — it is not
  player-scoped. Everything after that is:

  ```text
    the timer is ERASED       sync_actor_components_from_cluster REBUILDS
    for a non-player body     `*combat` every frame and carries across exactly
                              five fields: damage_invuln, hit_flash, hitstun,
                              recoil_lock, hitstop. `landing_lag_timer` is not
                              one, so the write is gone the next frame.
    and it is NEVER GATED     the actor road passes `(combat.hitstun_timer,
    even if it survived       combat.recoil_lock_timer)` as its stagger
                              (`update.rs:967`). The PLAYER road passes
                              `recoil_lock.max(landing_lag)`
                              (`body_integration.rs:170`). Landing lag is in one
                              expression and not the other.
    and only the player       `input_systems.rs:63` decrements it, under
    ticks it down             `With<PlayerEntity>`.
  ```

  ⛔ **this is the campaign's central proposition failing**: the same character,
  driven by a human or by a CPU, has different physics. *"Changing the controller
  does not change the body"* is exactly what does not hold here.

  ⚠ **I first filed the second half as "the timer accumulates forever on CPU
  bodies" and that was WRONG** — the read-model rebuild wipes it, so nothing
  accumulates. Checking the sync before writing it down is what caught it.

  ⛔⛔ **AND IT IS TWO SITES, NOT ONE — the boss road copied the miss.**
  `bosses/sync.rs::boss_component_snapshot` rebuilds `BodyCombat` the same way
  and carries the same five timers, and its own comment says it follows *"the
  same rule as `sync_actor_components_from_cluster`"*. **It does, including that
  function's omission** — the list was written by reading the other one. ⇒ *a
  citation is only as correct as the thing it cites*
  ([[reference_a_comment_describes_intent_not_the_code]]): the comment was
  accurate, the rule it named was wrong, and the error propagated intact.
  ⚠ bosses run the shared moveset runtime, so this is structurally live rather
  than academic. Both sites now carry the destructure guard.

  ⭐ **AND THE OTHER TWO LISTS WERE UPDATED — which is how this went unnoticed.**
  `BodyCombat`'s reaction timers are hand-listed in THREE places, and
  `landing_lag_timer` is in two of them: `lifecycle_commit.rs:402` and
  `room_transition/commit.rs:165` both clear it. Only the CARRY list at the actor
  rebuild was missed. ⇒ whoever added the field found two of the three lists,
  which is exactly the failure mode of a rule kept in prose: it is followed
  *mostly*, and the miss is invisible.

  ⭐⭐⭐ **AND THE PROJECT ALREADY USES THE REMEDY — ON THE SAME STRUCT.**
  `BodyCombat`'s ROLLBACK codec lists all twelve fields in `snapshot_pod!`, and
  that list **cannot** rot: the macro's `decode` builds `Self { $($field: …),+ }`
  as a struct literal, so omitting a field is `E0063` at compile time. ⇒ the
  codec has never lost a field and the carry list four files away lost one,
  **for exactly the reason this row is about** — one list is checked by the
  compiler and the other is checked by whoever remembers.

  ⭐⭐ **THE GENERALIZABLE PART: a hand-maintained CARRY LIST is the same shape as
  a hand-maintained field census, and today's remedy applies.** That function's
  own comment enumerates *"the SAME fields the player carries"* — five of them —
  and `landing_lag_timer` was added to `BodyCombat` later and never joined the
  list. An exhaustive destructure of `BodyCombat` at the rebuild would have made
  adding a field a COMPILE ERROR until somebody decided whether it survives the
  refresh. ⇒ same fix as `ActorTuning` / `ArchetypeSpec` / `CharacterDefinition`.

  ⇥ ⭐ **THE THIRD PART IS NOW A ONE-LINER, and the rule has one spelling.**
  `BodyCombat::hard_lock_timer()` names it — `recoil_lock.max(landing_lag)` —
  and the player road calls it instead of spelling it inline. Pinned by
  `landing_lag_alone_still_locks_control`, falsified by reducing the method to
  `recoil_lock` alone. ⛔ this does NOT fix the row: the actor road still passes
  its own reduced pair, which is now VISIBLE as one road calling the rule and the
  other spelling half of it. ⚠ the test also pins that `hitstun_timer` stays OUT
  of it — hitstun reduces authority, it does not remove it.

  ⇥ ⛔⛔ **AND A THIRD SITE, WHICH IS THE MOST TELLING OF ALL.**
  `BodyCombat::decay_reaction_timers` ticks five timers and its own doc says it
  exists *"retiring the two hand-copied five-line decay blocks (fable review
  §A1)"* — it was written BECAUSE a hand-kept list had been duplicated. **It is
  still a hand-kept list**, and `landing_lag_timer` never joined it. ⇒ **the
  consolidation solved the duplication and not the rot**, which is the whole
  argument for the destructure guard rather than another shared helper. Guarded.

  ⇥ ⛔ **AND A FOURTH: `BodyCombat::reset()`** clears six fields and not this one,
  so a body reset mid-landing-lag keeps the lock. Three production callers reach
  it (`sandbox_reset`, `features::ecs::reset`, `session::reset`), and for the
  PLAYER — whose road does read the timer — that is up to 0.28s of input lock
  carried through a reset. ⚠ the room-transition and lifecycle-commit paths clear
  it explicitly, which is why this has never been the visible symptom. Guarded.

  ⇒ **FOUR hand-kept lists, one field missing from every one of them.** The two
  that DO carry it are the two that were edited when it was added; every list
  written or consolidated since has been written from a sibling that lacked it.

  ⇒ **the fix is four one-line changes at named places**: add `landing_lag_timer`
  to the carry list at both rebuild sites, to `decay_reaction_timers`, to
  `reset()`, and call `hard_lock_timer()` on the actor road instead of passing
  `recoil_lock` alone. ⛔ **carrying it alone
  makes the bug I wrongly reported REAL** — a timer that persists and is never
  decremented.

  ⚠ **and the whole thing is a difficulty statement, so it is JON'S**: applying
  it makes every CPU fighter commit to its aerials the way a human must. That is
  what the content authored, and it is still a change to how every CPU fight
  plays — the same class as *"is `unarmed_melee` scaffolding"* and the provoked
  villager's HP pool.

- ✔ **D107 CLOSED 2026-08-13 BY DELETION — the mirror is gone, so there is no
  population left to be false for.** Its two production consumers were both
  INSTRUMENTS (`causal.rs`, `dev/trace`) and they read `BodyMelee` directly now;
  the brains never read it at all, deriving attack state from
  `perception::BodyPhase` and `attack_windup_remaining`. The row's own proposed
  repair — widen the mirror to CPU bodies — was answered by removing the thing
  that had a population. ⛔ and its rejected half stays rejected: widening the
  predicate to `|| playback.is_some()` reports a fighter as attacking while it
  fires a bolt, because `MovePlayback` also carries ranged moves and specials.
  ⇥ AS WRITTEN: ▢ **`BodyCombat::attacking` IS FALSE FOR ALMOST EVERY BODY THAT IS
  ATTACKING — and the CAUSAL TRACE reports it** (found 2026-08-13 while trying to
  reproduce Jon's *"in smash if you throw out an attack you hurt yourself"*).

  Two constraints, either of which alone would be enough:

  * it is written in exactly ONE place — `write_player_ecs_components`, whose
    query is `With<PlayerEntity>`. **No enemy, NPC or CPU fighter ever has it
    set.**
  * ⛔⛔ **THIS BULLET WAS WRONG AND ITS "FIX" WAS WORSE — both reverted
    2026-08-13 (GPT 5.6 review).** It read: *"what it mirrors is
    `BodyMelee::is_swinging()`, the FLAT swing. A body attacking through the
    MOVESET runtime never touches it."* **It does.**
    `project_moveset_melee_to_body_melee` projects melee playback onto
    `BodyMelee` in `CombatSet::Playback`, and `write_player_ecs_components` runs
    later in `PresentationSync` — so `is_swinging()` is already the canonical
    melee read-model for BOTH attack roads.
  * ⛔ **and the patch written on that premise was semantically wrong**:
    `attacking = is_swinging() || playback.is_some()`. **`MovePlayback` is not
    melee** — it carries ranged moves and content specials too, which is why the
    projection classifies with `is_melee_swing_move` instead of checking for a
    playback. That version reported a fighter as attacking while it fired a bolt,
    and the regression added with it PINNED that, using a probe move with an idle
    clip, no windows and no binding. ⇒ reverted, and the projection's own poison
    (`a_ranged_move_does_not_project_a_phantom_melee_swing`) gained the third arm
    it was missing: melee → swing, ranged → none, **special → none**.

  ⛔ **and it is read by the instruments**: `causal.rs:274` publishes
  `.field("attacking", …)` and `dev/trace/detect.rs:239` records it. So a causal
  artifact says *"attacking: false"* for a fighter mid-swing, which is
  [[reference_causal_instrument_gotchas]]'s exact shape — an instrument whose
  silence looks like evidence.

  ⭐ **MEASURED, not argued.** A host-harness probe seated a keyboard fighter
  against a CPU, pressed every preset's attack key, and sampled both fields for
  120 frames: `attacking` stayed `false` throughout **for both seats**, while a
  per-seat `MovePlayback` query showed the CPU playing moves the whole time.
  `BodyCombat::strike_count` behaved identically and is the same trap.

  ⚠ **what this does NOT say**: it is not evidence about Jon's self-damage
  report. That reproduction FAILED for a different reason — keyboard attack
  input never reached the seated body in the harness, though keyboard MOVEMENT
  demonstrably does (`the_puppy_slug_forced_onto_the_stage_keeps_the_body_it_authored`
  drives one 34.85px with `ArrowRight`). ⇒ **the self-damage observation remains
  unreproduced and unrefuted**, and the next attempt needs a way to make a
  human-seated fighter swing.

  ⇥ ⭐ **SO THE ROW REDUCES TO ONE TRUE DEFECT, and it is the query.**
  `write_player_ecs_components` is `With<PlayerEntity>`, so no enemy, NPC or CPU
  body ever has `attacking` written — while `causal.rs` and `dev/trace/detect.rs`
  both REPORT it. The instrument says "not attacking" about every non-player body
  in the world, always.

  ⚠ **and the lesson is worth more than the row.** The false half survived because
  *"the moveset road never touches `BodyMelee`"* sounded exactly like the
  player-scoped-consumer pattern I had just found four times, so I fitted the
  evidence to the shape instead of reading the projection.
  [[reference_the_sentence_after_the_measurement]] — except the error was a step
  earlier than usual, in the premise rather than the conclusion.

  ⇒ the remaining fix is one of: widen the query off `PlayerEntity`, or delete the
  field and have the instruments read the projected `BodyMelee`. ⛔ **the second is
  probably right** — a mirror kept in sync with a projection that is already a
  read-model is a duplicate of a duplicate — but it is a real design call.

- ◐ **D96 ⭐⭐ JON ANSWERED SIX OF THE NINE ON 2026-08-13, UNPROMPTED — ONE
  BLOCKS A DELETION NOW.** He sent the rulings as one handoff while the Authority
  Convergence campaign was running; every one is verbatim in
  `maintainer-decisions.md`:

  ```text
   1  what is a small_lurker          ▢ STILL OPEN — the last one, and now the SOLE
                                      blocker of deleting character_archetypes.ron
   2  what is a large_brute           ✔ a real authored Goblin Brute character
   3  what is under_town_skitter      ✔ npc_puppy_slug — "skitters are Puppy Slug"
   3b what a DividingMite splits into ✔ npc_puppy_slug, and the character says so
                                        itself now rather than the engine
   4  is dive_drill's Target a sandbag ✔ NEITHER — deleted. "AI-invented placeholder
                                        content Jon does not care about preserving"
   5  does Carl Stargan fight          ✔ yes, and the QUESTION was rejected: "there is
                                        no separate 'can fight' character property"
   7  a provoked villager's health     ✔ 4 — "these are ordinary tuning values"
   8  a pirate quartermaster's vitals  ✔ 4 / 6 heavy, and the Patent Clerk 6
  ```

  ⭐ **the pattern is worth more than the answers.** Three of them were blocked on
  product decisions he did not consider product decisions at all: health was
  tuning, Carl's fighting was a category error, and the skitters were AI-invented
  taxonomy he never cared about. ⇒ **when a row's cost is "one number", state the
  number you would pick and what it blocks** — that is the shape that got
  answered.

  ⇥ AS WRITTEN: ▢ **NINE CONTENT DECISIONS ARE STACKED UP, AND EVERY ONE OF THEM BLOCKS A
  DELETION** (collected 2026-08-12; **the header said SIX over a table of EIGHT,
  and a NINTH was missing entirely — corrected 2026-08-13**). Each was found by measuring rather than
  guessing, each is Jon's rather than a migration's, and each is recorded where
  the code that waits on it lives:
  ### ⇥ ⭐⭐ WHAT EACH ONE UNBLOCKS, MEASURED 2026-08-13 — the whole chain in one place

  Eighteen hours of measuring the campaign's rows converged here: **every
  remaining ▢ in the D73 checklist, the campaign table and this ledger that is not
  a FEEL call traces to one of the decisions below.** The engineering behind them
  is done. What each costs, exactly:

  ```text
   #  decision                        blocks                                    measured
   1  what is a small_lurker          `OPEN_CASTING` entry 1 of 3               enemy_roster.rs
   2  what is a large_brute           `OPEN_CASTING` entry 2 of 3               enemy_roster.rs
   3  what is under_town_skitter      `OPEN_CASTING`-adjacent: the ONLY namer   the LAST placement
                                      of the `medium_striker` ROW               needing a row
   3b what a DividingMite splits into `OPEN_CASTING` entry 3 of 3 — `SmallSkitter`, enemy_roster.rs
                                      the last CamelCase name in shipped content
   4  is dive_drill's Target a sandbag ⛔ NOT the roster: its brain is `Passive`  the last placement
                                      and never reaches one. It blocks the       with no character
                                      display-name fallback (checklist item 15)
   5  does Carl Stargan fight         one `REGISTERED_WITHOUT_A_BODY` entry     character_catalog.rs
   6  is a badnik the 1-HP wanderer   the sanic demo's own row                  —
   7  a provoked villager's HP pool   D81 ENTIRELY: 107 preset adopters, and    6 presets,
                                      ZERO of them author a BrainProfile        0 authored
   8  a pirate quartermaster's vitals six `REGISTERED_WITHOUT_A_BODY` entries   D98
  ```

  ⭐⭐ **ALL FOUR NOW HAVE OPTIONS WRITTEN UP** (2026-08-13): (2) as *"Who is the goblin lab's
  HEAVY?"* and (1)+(3)+(3b) as *"The three castings that delete `character_archetypes.ron`"*,
  both in `awaiting-maintainer-decision.md`. Each is a yes/no with the evidence the code already
  carries and a recommendation — `npc_ai_slop` for the lurker (the cascade's own note calls its
  minions *"slop"*), `npc_puppy_slug` for the skitters (the placement named `pg_skitter` is
  already cast that way). ⇒ **none of these needs Jon to derive anything; they need him to
  agree or name something else.**

  ⇒ **(1)+(2)+(3)+(3b) are the acceptance signal, and a TEST now counts them
  down** — `worlds::tests::what_still_needs_an_archetype_row` (2026-08-13) rebuilds
  every placement's resolution against an EMPTY roster and asserts the exact set
  that stops resolving. It reports those four and nothing else, which is the
  measurement this paragraph used to assert by argument. ⭐ **with no archetype
  table at all, every other enemy placement in all four worlds builds as a
  character.**

  ⛔ **(4) was in this list for the wrong reason and is NOT part of the
  acceptance signal.** `try_spec_for_brain` returns `None` for every non-`Custom`
  brain, so a `Passive` placement never consults the roster. Casting Target
  deletes the display-name fallback (checklist item 15), not a row.

  ⇒ as originally written, **(1)+(2)+(3) are the acceptance signal.** `character_archetypes.ron` is down
  to two rows: `combatant`, which exists ONLY as the row those three waivers
  borrow, and `medium_striker`, which exists ONLY because `under_town_pipes` names
  it. Cast those three identifiers and the file deletes — and `ArchetypeSpec`
  (119 product lines), `CharacterRoster` (806), the roster fragments and
  `ActorTuning`'s archetype projection go with it, because every surviving reader
  of each is on the road those two rows keep alive.

  ⇒ **(7) is the largest single unblock in the ledger.** D81's *"seven key rows
  left"* is not seven migrations: 107 characters name a brain preset and **not one
  of them authors its own `BrainProfile`**, so no preset can be deleted as
  duplication — each is its author's only policy statement. They cannot state one
  until they are registered with real bodies, and that needs their vitals. One
  decision retires six presets.

  ⚠ **and none of these is a migration waiting for an engineer.** Every road that
  consumes them refuses honestly today (`every_shipped_enemy_placement_can_be_built`
  proves the two unnamed placements are the only ones), the waivers are declared by
  the provider that owns them rather than by the engine, and each decision's cost is
  one line of content plus a deletion.
  ...
  1. **What IS a small lurker?** The Gradient Sentinel's cascade summons two per
     ...
     says so at runtime.
  2. **Which creature is a goblin fight's heavy?** ⭐ **the OPTIONS and the trap
     are written up in `awaiting-maintainer-decision.md`** ("Who is the goblin
     lab's HEAVY?"): a new anonymous brute row (an ART task), reuse of
     `npc_hand_saint` / `npc_ninja_heavy` (one data line, and a ninja in a goblin
     cave), or leave the placeholder. ⚠ that file also records why the obvious
     answer is wrong — the cast's one goblin heavy, `npc_goblin_cantina_chieftain`,
     is a NAMED hub individual, and wave 3 is a duo, so casting him spawns two
     Fretjaws and kills them. Three waves of
...
- ◐ **D101 GPT 5.6's REVIEW: WHAT LANDED AND WHAT DID NOT.** Recorded so the
  ...
  ⇥ ✔ **THE BODY-MUTATION RESIDUE IS GONE (2026-08-12).** Generic provocation projected
  ...
  ⇥ ✔ **BOTH SURVIVING BODY FACTS ARE DELETED (2026-08-12, `713345288`).**
  `ProvokedArchetype` carries four fields and every one is a mind or a kit; its
  test destructures the struct exhaustively so a fifth cannot be added quietly.

  ⇥ ⭐ `gravity_scale` — the write re-grounded a flying body so a "grounded"
  policy could drive it, and **the premise had been stale since 2026-08-11**.
  The engine's default provoked policy is `CharacterBrainTemplate::Smash`, and
  the Smash brain branches on `obs.self_aerial` with **no `can_fly` gate**: a
  flyer's grounded motor outputs are discarded and it steers a 2D
  `velocity_target`. `cfg.can_fly` gates only the hybrid take-off/landing
  toggle — correct, since a baseline flyer never toggles — and it is read off
  the body's own `AbilitySet`, which `ActorBody::from_kit` forces true for an
  aerial body. ⛔ **and the test that pinned the write could not have caught
  that**, because it built a body production never builds: `gravity_scale = 0.0`
  poked onto a grounded seed with `flight.fly_enabled` left FALSE. The brain read
  that half-body as aerial (`gravity <= 0.001 || fly_enabled`) and the integrator
  read it as grounded (`fly_enabled` alone), so it really did freeze — from the
  fixture's own disagreement, not from provocation. Replaced by
  `a_flying_npc_stays_flying_when_it_is_provoked`, whose three pre-stimulus
  assertions state that the body agrees with itself about flying before anybody
  hits it.

  ⇥ ⭐ `max_health` — the flip ran
  `*em.health = fresh_health_pool(DEFAULT_PROVOKED_HEALTH)`, which resized the
  body to 4 **and healed it**, because a peaceful placement spawned at
  `max_health: 1`. **The `1` was the defect.** An undescribed body is
  undescribed before anybody hits it, so the number moved up a level to
  `ambition_characters::actor::DEFAULT_UNAUTHORED_BODY_HEALTH`, shared by the two
  seeds that answer *how tough is a body nobody authored* (the character body
  blueprint and `new_peaceful_npc_in`), and provocation stopped writing health.
  ⚠ **the VALUE is unchanged at 4 and D96 item 7 still owns it** — what changed
  is the AUTHORITY. Not a rebalance either: a peaceful body takes no health
  damage at all (`actor_hit` says "No health damage" in as many words), so
  raising the peaceful default is inert until the body is hostile, at which point
  it has exactly the pool it used to be handed. `fresh_health_pool` and
  `DEFAULT_PROVOKED_HEALTH` are deleted with their one caller.

  ⇥ ⭐ **AND THE SEIZURE FIX HAD NO TEST.** Provoking a body under player control
  used to replace its `Brain::Player(slot)` in place and permanently (a human's
  fighter became a CPU 28 frames after its pad went quiet). The fix landed with
  nothing pinning it, and it was unreachable only until a seated fighter became
  provokable. `provoking_a_player_driven_body_changes_its_mood_and_not_its_driver`
  pins it, with an undriven twin as the poison.
...
- ✔ **D111 — THE ROBOT'S SOUND POLICY CAME BACK OUT OF THE CHARACTER DOMAIN
  (2026-08-12), AND A CONTRACT NOW HOLDS THE LINE.**
  P1.7's bulk move lowered `moveset/prefabs.rs` into `ambition_characters` so
  `prepare_character` could call `build_actor_moveset` from below. Most of that
  file had to move. `apply_player_robot_slash_sfx` and the three
  `PLAYER_ROBOT_*` cue constants did not — they moved because they were
  **ADJACENT IN THE FILE**.

  ⭐ **the test is "does character PREPARATION call it", not "was it next to
  something preparation calls".** Measured: the overlay has exactly ONE
  production caller in the workspace, `avatar/starting_character.rs` — the
  protagonist road — and `prepare_character` never reaches it. So canonical
  protagonist presentation policy sat in the crate whose entire claim is that a
  character is DATA. (GPT 5.6 review of `1579ab3`, finding 4.)

  ⇒ back in `ambition_combat::moveset::player_robot_slash`, beside the
  compile-time hash pins that correctly never left. The text and its proof are
  one crate again — the first move had them in two.

  ⚠ **the generic inputs STAYED down and that is the line**: `SWING_SFX_CUE`,
  `SLASH_ARC_VFX`, `SLASH_POKE_VFX` are what the builders themselves author into
  every moveset. The rule is not "anything with a cue in it".

  ⭐ **a 26th absence contract**,
  `the-character-domain-is-not-named-after-a-character`: no `fn` / `const` /
  `struct` in `ambition_characters` may be NAMED after one creature. Patterns run
  comment-stripped, so the crate may still EXPLAIN itself with a concrete example
  (`entry.rs` cites `player_robot_v3`) and TEST with one (`npc_puppy_slug`
  fixtures) — what it may not do is own one's policy. ⚠ FALSIFIED: re-adding a
  single `pub const PLAYER_ROBOT_SWING_SFX_CUE` turns it red.

  ⇥ ▢ **the further destination is CONTENT and is NOT done.** The canonical
  Player Robot is authored in `game/ambition_content` and its slash presentation
  ideally belongs there. What blocks it is the CALLER: the protagonist road lives
  in the actor monolith, which cannot see a game's content crate. Moving the
  overlay means moving the call, and that is a seam question, not a file move.

- ✔ **D109 — THE BINDING BOUNDARY HAS ITS OWN CRATE (2026-08-12).**
  `ambition_characters`, the canonical character domain, depended on
  `ambition_platformer2d_shared_tangle` — **~18k lines across 51 files** of
  platformer lifecycle, camera, transit, schedules, hotkeys and Bevy state/input
  — for exactly **one name**, `Namespace`, in exactly **one file**. Every edit to
  any of those 51 files invalidated it.

  ⛔⛔ **the edge was LEGAL and that was the wrong test.** No absence contract
  forbids it, and the note in `binding_namespaces.rs` argued its safety on
  precisely those grounds — "verified against `check_absence_contracts.py`, whose
  contract is TRANSITIVE". Legality is not the same question as FLOOR, and P1.7
  is partly about compile topology (GPT 5.6 review of `1579ab3`, finding 3).

  ⇒ `crates/ambition_binding` — `Resolver` / `Bound` / `Ref` / `UnresolvedRef` /
  `Namespace` / `BindingLedger` / `BindingReport`, 941 lines whose **entire
  dependency list is `tracing`**. `ambition_characters` no longer names the
  platformer at all.

  ⚠ `shared_tangle::binding` stays as a documented re-export: nine files across
  `ambition_render`, `ambition_sprite_sheet` and the actor monolith spell that
  path, and churning them would be a rename campaign rather than an architecture
  change. The edge that mattered is the one that is gone.

  ⚠ **the capability ratchet went RED and the baseline moved, with the reason
  written into it.** 41 → 42 crates. The counter asks whether a movement-only
  game gained a CAPABILITY it never asked for; it did not — the new node is a
  strict SUBSET of a crate already in the closure, so the sentinel links the same
  code under one more name, and `never_asked_for` is unchanged at 15. The
  baseline entry carries a ⛔ saying what it must not be copied for.

- ✔ **D110 — `fixtures/external_consumer` COMPILES AND PASSES AGAIN (2026-08-12), AND IT CAUGHT TWO PUBLIC-API REGRESSIONS ON THE WAY BACK.** Found while refreshing lockfiles for D109. It fails on
  `CharacterRosterFragment::from_ron_at` (deleted as an unused constructor) and a
  missing `character` field on `SpawnActorKind` (added by P1.12). Both are
  earlier campaign work, neither is recent, and the fixture is `exclude`d from
  the workspace in the root `Cargo.toml` — so `cargo check -p ambition_app
  --all-targets` cannot see it and never will.

  ⚠ **an EXCLUDED fixture is a consumer nobody compiles**, which is the exact
  thing it exists to be. It is the only in-repo consumer that links the engine
  from OUTSIDE the workspace, so it is the only check that the public surface
  survives a real dependent — and it has been silently answering yes by not
  being asked. Repair it, then decide how it gets built: a gate that never runs
  is the instrument-that-measures-nothing shape this ledger keeps naming.

  ⇥ ✔ **the two things it was hiding, and both are the same shape.**
  **(1) `CharacterRosterFragment::from_ron_at` WAS NOT DEAD.** It was deleted by
  the `#[deprecated]` + `cargo check` census (ledger D105) as *"compiler-verified
  to have no call site in the WORKSPACE"* — and the census's own words name its
  blind spot, because this fixture is `exclude`d from the workspace and is the
  only in-repo consumer that links the engine from OUTSIDE, which is the entire
  population a public-API census is about. RESTORED, with the note that deleted
  it corrected. ⚠ the technique itself is sound and has now been refined three
  times: `-p` could not see dependents, `--workspace` could not see an excluded
  consumer. ⚠ the `source` FIELD really was dead and stays deleted — the assembly
  ERROR carries its own, read from the build parameter, which is what makes a
  located diagnostic work. The two looked like one feature, which is how a
  working one left with a dead one.
  **(2) `PlayableKitSource::HostCode` was deleted 2026-08-11** and its only
  external consumer still authored it, so the fixture's catalog RON no longer
  parsed. The row says `Authored`; the test that rested on it keeps its
  assertion and states the smaller fall-through it now guards (the row's own
  `drifter` set) rather than the deleted variant.

  ⭐ **and the gate exists — it just never ran.** `run_tests.py` has had an
  "external consumer: outlander" job since Phase 6, correctly commented as *"the
  only gate that can see"* an umbrella API break. It is guarded by
  `not only and everything`, so a campaign of `cargo test -p <crate>` never
  fires it. A `cargo check --all-targets` for the fixture is UNCONDITIONAL now —
  seconds, and it catches exactly the class that got through — while the full
  suite stays whole-suite-only, because its value is the behaviour rather than
  the surface. ⚠ 24 tests, all green.

- ◐ **D102 — ✔ THE GLOBAL FALLBACK IS GONE (2026-08-12). What is left is three
  content decisions, not engineering.**

  ⭐ **an unknown identifier is a construction ERROR now**, the rule P0.1
  established for an absent `CharacterId`. `const FALLBACK_BRAIN_KEY` and the
  content-schema obligation that every roster carry a `combatant` row are
  DELETED; `CharacterRoster::fallback` — the field — is deleted; and the assembly
  step that promoted one provider's default to a process-wide answer (and quietly
  used an inert engine one when two providers disagreed, so the same misspelling
  built different bodies depending on how many games were linked) is deleted with
  it. `combatant` is an ordinary row.

  ⭐⭐ **THREE WAIVERS, EACH NAMED, AND THEY ARE NOT THE SAME WAIVER:**

  ```text
  the IDENTIFIER is open   the PROVIDER's own `with_open_casting_decision`
                           declarations; all of them retired closes D102
  the HOST published none  generic_body_for_a_composition_with_no_cast — a
                           registry with ZERO characters cannot be "missing" one
  the BRAIN names nothing  a non-`Custom` brain (`Passive`) states a POLICY, not
                           an identifier; there is nothing to have gotten wrong
  ```

  ⛔ folding any two together is how this defect survived: the old fallback WAS
  all three at once, so a misspelling, a row a migration deleted, and a genuinely
  open question produced the same body and the first two hid inside the third.

  ⭐ **the list is MEASURED.** Instrumenting the fallback and running the whole
  app suite produced five live keys — and the waivers in the code named none of
  them. `small_lurker` and `large_brute` were cited as "the live ones"; what
  actually reached the fallback was `cellular_automaton_fighter` (9),
  `puppy_slug` (6), `SmallSkitter` (4), `pirate_raider` (2) and
  `burning_flying_shark` (1). `under_town_skitter`, cited for two campaigns as
  "the one placement in the shipped worlds that reaches this", names
  `medium_striker` — a row that still EXISTS — and never came near it.

  ⇒ only `small_lurker`, `large_brute` and `SmallSkitter` (what a `DividingMite`
  splits into, the last CamelCase archetype name in shipped code) are real open
  questions and are on the list. The rest were TEST FIXTURES still naming rows
  D73 deleted, and they were repaired to name their characters
  (`perfect_cellular_automaton`, `npc_pirate_raider`,
  `npc_burning_flying_shark`) — nine fixtures across seven files, including the
  mount pair in `rollback_coverage` whose shark and pilot were BOTH generic
  `combatant` bodies, so a sweep about mount components was registering none of
  them, and `player_pilots_mount_end_to_end` where the mount named its character
  and the rider beside it did not.

  ⚠ **FALSIFIED BY THE CHANGE ITSELF**: turning the rule on failed 23 monolith
  tests and 13 app tests with the new message, and every one of them was repaired
  by fixing what it NAMED, not by relaxing the rule.

  ⛔⛔ **AND THE SHORT LIST WAS STILL A GLOBAL FALLBACK — just a shorter one**
  (GPT 5.6 review priority 3, landed 2026-08-12). `IDENTIFIERS_AWAITING_A_CASTING_DECISION`
  was a `const` in a REUSABLE PLATFORMER ENGINE holding three Ambition creature
  names. It moved the policy from *"every unknown key silently gets combatant"* to
  *"these three names silently get combatant"* — better, and the same shape: a
  second game linking this engine inherited Ambition's open content decisions for
  free, and a typo matching one of the three entered the temporary road as quietly
  as before.

  ⭐ **the waiver is CALLER INTENT now and travels on the roster fragment.** A
  provider declares which of ITS authored identifiers is still uncast, names the
  row of its OWN roster the body borrows meanwhile, and gives the reason that goes
  into every warning — `ambition_content::enemy_roster` holds all three. The
  engine knows none of the names. Assembly refuses a waiver onto a row nobody
  contributes (`MissingTemporaryRow`) and two providers claiming one identifier
  (`DuplicateOpenCasting`).

  ⚠ **the poison's third clause is the one an engine-owned list could not pass**,
  and it was GREEN FOR THE WRONG REASON first: written against a control roster
  with no `combatant` row, it passed against a deliberately broken engine, because
  the refusal came from the roster being small rather than the waiver being local.
  The control host owns exactly the row a leaked waiver would hand it now, and the
  falsifier fails. `a_declared_identifier_gets_a_body_and_nothing_else_does`.

  ⇥ ✔ **AND THE PROVIDER-FALLBACK MACHINERY IS DELETED WITH IT** (GPT priority 4).
  `fallback_brain_id` survived the process-wide deletion as a per-provider map —
  and the map was `#[cfg(test)]`. Production still ACCEPTED the ids, VALIDATED
  them, could REJECT a provider whose fallback named no row, and assembled the
  specs, then discarded them: one reader, one test. The single production caller
  passing one said `Some("combatant")`, whose only remaining effect was requiring
  a row the content schema had already stopped requiring. Field, validation,
  assembly, three error variants and the two asserts whose subject it was are
  gone; the test keeps the claim that always mattered — no default answers for an
  unknown key with two providers linked.
...
- ◐ **D102 ⭐⭐⭐ THE LAST TWO ARCHETYPE ROWS — NOW ONE — 60 production references
  ...
  ⇥ ✔ **THE CONSTRUCTOR CAN NO LONGER REACH THE FALLBACK (2026-08-12,
  `47986f8a7`).** `ActorClusterSeed::new_in` opened with
  `roster.spec_for_brain(&brain)` — a lookup that cannot fail — so EVERY spawn
  road that reached that constructor inherited the silent downgrade without
  choosing it. That is the mechanism behind all three of this campaign's silent
  body swaps, and it is deleted: `new_in` takes a RESOLVED `ArchetypeSpec`, the
  lookup is hoisted to the three production call sites, and `spec_for_brain` is
  replaced by `generic_body_for_unresolved_brain(brain, waiver)` — the fallback
  asked for BY NAME, with the caller's reason travelling into the warning. The
  old silent form survives `#[cfg(test)]`, where naming a shape rather than a
  creature is the point. **The compiler is the guard**: a new spawn road has no
  roster to ask.

  ⇥ ⭐ **and hoisting it found a second one.** Two callers in `construction/`
  asked for a spec only to answer *is this a limbed host* — `combatant` is not
  one, so `None` gives the same verdict. They wanted a row because
  `giant_hand_plans` demanded `&ArchetypeSpec`, and got the fallback purely to
  satisfy the signature: a CHARACTER-authored giant's hands were sized from an
  archetype's `default_size`. Neither surviving row authors one, so the geometry
  is unchanged — and the next row to author one can no longer resize somebody
  else's hands. `giant_hand_plans` takes `Option<&ArchetypeSpec>` now.

  ⇥ ▢ **WHAT IS LEFT IS TWO CONTENT DECISIONS, NOT ENGINEERING.** The census
  that sizes the rest, measured 2026-08-12 by reading the shipped bytes:

  ```text
    65  EnemySpawn placements in assets/worlds
    63  carry a character_id, and ALL 63 name a BUILDABLE character
     2  characterless — `under_town_skitter` (medium_striker, a row that
        exists) and `Target` (brain: Passive, which names no row at all)
     0  LDtk placements reach the unknown-key fallback today
  ```

  ⇒ so the fallback's ENTIRE live surface is two ids named from code rather
  than from levels: `small_lurker` (the Gradient Sentinel's cascade) and
  `large_brute` (three waves of `goblin_encounter.ron`). Both are waived by name
  in `every_summoned_minion_id_resolves_a_body`, both are D93/D96, and both are
  Jon's. ⛔ **`FALLBACK_BRAIN_KEY` and the schema's `combatant` requirement stay
  until they are answered** — deleting the landing pad while two valid paths
  still reach for it is how a warning becomes a crash mid-boss.
  ...
  ⇥ ⛔⛔⛔ **AND THE DELETION SHIPPED A LIVE REGRESSION, found 2026-08-12 by
  finally censusing the LEVELS instead of the code.** One placement —
  `under_town_pipes` / `under_town_skitter` — names `medium_striker` and carries
  NO `character_id`, so the brain key was the only thing deciding what its body
  was. It is built as the `combatant` fallback now: **4 HP and melee-only where
  it was 5 HP with a thrown rock**. `cargo check --all-targets` was clean and the
  whole app suite stayed green, because neither can see LDtk.

  ⇒ ✔ **THE ROW IS RESTORED, and the first answer here was wrong.** The commit
  that found this argued the row should stay deleted — *"restoring an archetype
  row that no character needs is the ontology growing back"* — and chose to
  record the regression while waiting on D96 item 3. That trades a surviving
  legacy row for a SHIPPED GAMEPLAY NERF nobody asked for, which is the worse of
  the two, and "the acceptance signal is a deletion" does not buy one. A legacy
  row surviving because a placement needs it is honest pressure; a body quietly
  losing its rock is not.

  ⇒ ⭐⭐ **AND ITS LIFE IS NOW TIED TO ITS REASON, which is the half a countdown
  cannot express.** The monolith's countdown asserts the row is still there;
  nothing asserted its REASON still was, so the day the skitter gets cast the row
  becomes dead weight every future census has to re-litigate — which is how 843
  lines of this file survived to August. `the_striker_row_lives_exactly_as_long_as_the_placement_that_needs_it`
  asserts the two agree in BOTH directions, and it lives in `ambition_content`
  because that is the only crate where the levels and the roster are both
  readable. The countdown test is now a list of survivors each carrying the
  sentence that keeps it alive, and an exact set rather than a floor.

  ⇒ ✔ **and the guard that would have caught it now exists**:
  `authored_spawns_resolve.rs` walks every `.ldtk` under `assets/worlds` and
  fails when a placement names neither a buildable character nor an archetype row
  that exists. **65 placements, all but TWO character-first** — those two are the
  entire surface where deleting a row can still change what a body is. Poisoned
  by removing the skitter's exemption: red, naming the level, the placement and
  the key. ⚠ its own count is deduplicated by `iid`, because a world file stores
  its levels more than once and a naive walk reports 186.

  ⇥ ⛔⛔ **the EARLIER census also missed four authored references — a guard
  caught those too.**
...
- ◐ **D103 ⛔⛔ 199 BROKEN DOC LINKS, AND THE GATE CANNOT SEE ONE OF THEM.**
  Measured 2026-08-12 with `cargo doc --no-deps`, per crate:

  ```text
    ambition_platformer2d_actor_monolith   122
    ambition_characters                     39
    ambition_platformer2d_core              21
    ambition_combat                         17
                                           ───
                                           199
  ```

  These are `[`Item`]` references to things that were renamed, moved or deleted,
  plus public docs linking private items. The project gate is
  `cargo check -p ambition_app --all-targets`; unresolved intra-doc links are a
  RUSTDOC lint, so the entire class rots in the space between the two commands
  and nothing has ever failed because of it.

  ⭐ **why it matters more here than in most repositories**: this codebase's
  doc comments are load-bearing. They carry the ⛔ notes that say why a thing is
  the way it is, and a campaign that deletes a type every few hours turns every
  reference to it into a sentence describing a world that no longer exists.
  Eight of the 199 were made THIS SESSION, and two of them did not merely fail
  to resolve — they described the exact defect that had just been fixed, as
  though it were still the design.

  ⇒ ✔ **THE RATCHET EXISTS** — `scripts/check_doc_link_ratchet.py`, baseline in
  `dev/doc_link_ratchet_baseline.json`, wired into the `rustfmt + clippy` CI job.
  Counts may FALL and must not RISE; `--update` banks an improvement in the
  commit that earns it. Poisoned with one bogus `[`link`]`: red, naming the
  crate and the old number.

  ⚠ **it also fails when it observes NOTHING**, which is the trap every
  measurement in this campaign has fallen into at least once: a `cargo doc` that
  errors, or a crate name that stops existing, emits zero warnings — and zero
  reads as a perfect score. A crate that produces no rustdoc output at all is a
  failure rather than a triumph.

  ⚠ **the cost was measured before proposing it, per the compile-cost memory**:
  warm, the four crates take ~4s wall (the three small ones together) and the
  monolith ~1.5s cached. It runs after `cargo clippy --workspace --all-targets`
  in the same job, so the target dir is already warm when it starts.

  ⇥ ▢ **the number itself is untouched at 199, and deliberately.** Fixing them is
  the work; the ratchet is what makes fixing them stick. ⛔ do NOT open this by
  sweeping all 199 — a sweep with nothing behind it is 199 links again by
  September, and the ratchet is now the thing behind it. Lower it a crate at a
  time, banking each with `--update`.

- ✔ **D104 ⛔⛔⛔ `reconcile_autonomous_actors` HAD NO PRODUCTION CALL SITE, AND
  EVERYTHING IT RECONSTRUCTS IS ALREADY ROLLBACK STATE.** Reported by GPT 5.6,
  2026-08-12; verified by grep the same hour, and the verification is worse than
  the report.

  ```text
    references to reconcile_autonomous_actors, whole workspace:
      1  pub use          (features/ecs/mod.rs)
      1  pub use          (features/mod.rs)
      4  doc comments describing what it does
      n  its own #[cfg(test)] module, calling it directly
      0  production call sites
  ```

  The ONLY system in `AmbitionLoadWorldSet::Reconcile` is
  `codecs::reconcile_brain_bindings`, and it filters on
  `binding.active_preset()?` — which returns `None` for `ProvokedDefault`,
  `ProvokedProfile`, `CharacterProfile` and `Boss`. ⇒ **every provoked and every
  character-first body is skipped by the only reconciler that runs.**

  ⇥ ⛔⛔ **AND WIRING IT IN WOULD DAMAGE THE RESTORE.** Every component it
  reconstructs is registered rollback state already: `Brain` (cursor),
  `BrainBinding`, `BodyHealth`, `ActorSurfaceState`, `TemporaryControl` and
  `CombatCapabilities` (canonical), `ActorConfig`, `ActionSet`, `Mounted`,
  `MountSlot`, `RidingOn`, `MountedBrainCache` (clone). So the codec has already
  put every one of them back correctly — and the reconstruction would then
  OVERWRITE two of them with derived values: `fresh_health_pool(max_health)`
  replaces a damaged provoked actor's snapshot HP with a full pool, and the
  policy-derived `gravity_scale` replaces the snapshot's.

  ⇥ ⭐⭐ **THE LIKELY ANSWER IS A DELETION, NOT A WIRE-UP** — and it is the
  SECOND-LARGEST component on D73's acceptance list: `autonomous_reconcile` is
  506 code lines (of 1,959 remaining). If the codecs genuinely restore
  everything, the file's whole reason for existing evaporated when the components
  became rollback state, and nobody re-read it.

  ⇥ ⚠ **what might still be load-bearing**, and the reason this is ▢ rather than
  a deletion already: `reconcile_temporary_control` INSERTS a `MountSlot` that
  was absent at save (`construction/mod.rs:1049` cites it), and a codec cannot
  insert what the snapshot never held. That path is either genuinely needed — in
  which case possession/mount across a rewind is BROKEN in production today and
  has been — or it is also redundant.

  ⇒ ✔ **PROVEN END TO END, and the codecs win.**
  `rollback_provoked_actor.rs` drives a SYNC-TEST session at prediction distance
  4 — so `SaveWorld`/`LoadWorld` genuinely run and every frame is resimulated —
  with a provoked, WOUNDED body folded into the rollback baseline. After 180
  frames the source is still `ProvokedDefault`, the mind is still the provoked
  one, and the damage is still the damage. **Nothing called the reconciler.**

  ⚠ two things keep it from being vacuous. Prediction distance is 4 rather than
  the shipped 0, which saves nothing (measured by `rollback_lifecycle_reset`'s
  cost probe) and would make the test exercise no rollback at all. And the
  provoked state is folded into the BASELINE rather than written mid-window,
  because a direct write inside a live prediction window is not reproduced during
  resimulation — the test would be measuring its own fixture. ⇒ poisoned by
  making the binding codec decode tag 2 as `CatalogDefault`: red, which is the
  proof it observes the round-trip rather than the absence of one.

  ⇥ ⛔ **AND THE HP ASSERTION IS THE ONE THAT WOULD BREAK IF IT WERE WIRED IN.**
  The provoked reconstruction calls `fresh_health_pool(max_health)`, so a damaged
  actor would heal to full on every load — which is exactly the divergence class
  the Track B campaign note recorded as *"a mid-brawl enemy full-heal"*. GPT's
  instruction offered two branches, wire-it-in or leave-it; the measurement says
  neither: **delete it.**

  ⇥ ✔ **AND SO DOES POSSESSION** — the other half, and the one that could have
  been a real production bug. `reconcile_temporary_control` rebuilds mount links
  from stable ids and INSERTS a `MountSlot` absent at save, which a codec cannot
  do. `possession_survives_the_real_rollback_window` possesses a body for real
  (the same Down+Interact hold the possession suite uses) and then simulates 120
  more frames inside a live prediction window: the possession and the
  player-brain both survive. The entity-mapping codecs run in
  `LoadWorldSystems::Mapping` before any reconciliation, which is what that
  hand-rolled `by_sim_id` rebuild was duplicating.

  ⇥ ⛔⛔⛔ **AND NEITHER TEST PROVED A ROLLBACK HAD HAPPENED — Jon caught it
  before the 506 lines went.** Both asserted that state SURVIVED a window they
  merely BELIEVED was a rollback window. State that is never disturbed always
  survives, so if the session had silently stopped rolling back — a changed
  harness default, a flag, a prediction distance quietly back to 0 — every
  assertion would still have passed and the deletion would have proceeded on
  nothing.

  ⇒ ✔ **the runtime already exposes the oracle**: `RollbackExecutionStats`,
  incremented by `count_load_run` inside `AmbitionLoadWorldSet::Reconcile` — the
  very set the absent reconciler would live in. Both tests now read it before and
  after the window and assert it GREW. ⚠ `lifetime_load_runs`, not `load_runs`:
  a rebase installs a new session and zeroes the per-session counters, and these
  tests rebase. That misreading is the one the type's own doc records, where the
  exit oracle looked like a session that had stopped being driven at frame 12
  while it had executed 2915 advances.

  ⇥ ⭐⭐ **THE POISON IS THE WHOLE POINT AND IT IS DAMNING.** Re-run at prediction
  distance **0 — the SHIPPED setting** — `LoadWorld` runs **zero** times, and the
  original tests passed anyway. With the oracle they fail loudly, naming the
  distance. ⇒ any future test claiming a rollback behaviour must assert against
  this counter; believing a harness setting is not evidence.

  ⇒ ✔✔ **DELETED, 2026-08-12.** 654 lines of rollback reconstruction and 391
  lines of tests that exercised it. `autonomous_reconcile.rs` went from 1,403
  lines to 333, and what remains is the LIVE half that always did the work:
  `provoked_projection` (a mind and a kit, never a body) and `peaceful_config`.

  ⇥ ⭐ **the cascade is the tell that it was genuinely one road, not two.**
  Deleting the entry point made `autonomous_brain_for_source`,
  `reconcile_temporary_control`, four `reconstruct_*`/`restore_*` helpers,
  `PROVOKED_DEFAULT_LABEL` and three `character_policy` world-shaped functions
  dead in turn — each found by the compiler rather than by a census.

  ⇥ ⭐⭐ **AND THE PAYOFF MEASUREMENT MOVED FOR THE FIRST TIME THIS CAMPAIGN.**
  `scripts/measure_archetype_legacy.py`, before and after:

  ```text
    production lines   3,431 → 2,787     (−644 in one commit; −968 vs baseline)
    CODE lines         1,959 → 1,537     (−422)
    autonomous_reconcile   506 →    ~90 code lines
  ```

  ⚠ the strictness the deleted `character_policy` functions carried is NOT lost:
  `apply_brain_selection` still rejects a `CharacterProfile` default it cannot
  resolve, pinned by
  `a_character_first_default_that_cannot_be_resolved_is_rejected`. GPT's second
  instruction survives its own subject being deleted.

  ⇥ ⛔ **AND THE DOCS I WROTE ABOUT IT ARE WRONG IN A NEW WAY.** This session
  carefully maintained prose about "the rollback road" and "the two twins must
  write the same set" for a function that does not execute. `61d46c086`
  strengthened it. Whatever the test decides, those comments are describing a
  mechanism rather than a behaviour, and they must say which.

- ◐ **D105 ⭐⭐ "WHOSE ONLY CALLERS ARE ITS OWN TESTS?" IS THE QUESTION D104
  ANSWERED, AND NOTHING ASKS IT.** `reconcile_autonomous_actors` survived months
  with zero production callers because nothing looks for that shape, and 654
  lines of it were being carefully maintained by an agent who believed the doc
  comments. The class is worth a guard.

  ⇥ ⛔⛔ **BUT THE OBVIOUS GUARD IS A CENSUS, AND CENSUSES ARE HOW THIS RUN KEEPS
  BEING WRONG — five times now.** Applying the question by hand to
  `features/enemies` the same day produced two "dead" items; one of them,
  `CharacterRoster::fallback_for_provider`, is the observation seam for
  `provider_defaults_coexist_without_becoming_a_cross_game_global`, a test three
  hundred lines further down the SAME FILE. The census had excluded that file to
  find external callers and then read *"no external callers"* as *"no callers"*.
  The compiler caught it; the grep had already said the opposite.

  ⇒ ✔ **THE TOOL EXISTS**: `scripts/probe_dead_public_fns.py`. It marks every
  public `fn` in the given files with `#[deprecated(note = "PROBE_<name>")]`,
  builds, reports the names no warning ever mentions, and ALWAYS restores the
  files — including when the build fails.

  ⇥ ⭐⭐ **ALL THREE OF ITS OWN BUGS ARE DESIGNED OUT, because I hit all three by
  hand first**: it refuses `-p` and always builds `--workspace --all-targets`
  (a `-p` run cannot see a crate's dependents); its `#[cfg(test)]` skip is
  brace-DEPTH scoped rather than a latch; and marking ZERO items is a hard
  failure rather than a clean bill.

  ⇥ ⭐ **it disagrees with the buggy run exactly where it should.** The
  hand-rolled latch version marked 0 functions in `features/enemies/mod.rs` and
  reported 21 as dead, `spec_for_brain` among them. The tool marks 21 and reports
  **21 of 21 have a call site**. That contradiction IS its poison test.

  ⚠ the docstring carries the "no call site is not delete it" warning and the
  `fallback_for_provider` near-miss, because the tool's output is the moment
  somebody is most likely to forget it.

  ⚠ **and the answer is often "keep it"**: test-only API that pins an
  architectural invariant is not dead code, it is the invariant's only witness.
  A guard that cannot tell those apart will get waived, and a waived guard is
  worse than none.

  ⇥ ✔ **THE TECHNIQUE WAS RUN PROPERLY AND IT WORKS** (2026-08-12): mark every
  public fn in a file `#[deprecated(note = "PROBE_<name>")]`, build, and read
  which probe names never appear — those have no call site the compiler can see,
  through re-exports, macros and cfgs alike.

  ⇥ ⛔⛔ **AND THE FIRST RUN OF THE TECHNIQUE WAS ALSO WRONG — sixth trap, caught
  before acting.** `cargo check -p <crate> --all-targets` compiles that package
  and its DEPENDENCIES, never its dependents, so every cross-crate caller is
  invisible. It named FIVE dead functions in `features/enemies`; `--workspace`
  named three. `sandbags_are_passive` and `from_prepared_specs` are the two it
  would have deleted, the second called from `ambition_content`. ⇒ the rule is
  `--workspace --all-targets`, and it belongs in the row because the wrong
  version is the one that runs faster and looks the same.

  ⇥ ⛔⛔ **EIGHTH TRAP, AND THE TOOL ITSELF FELL INTO IT (2026-08-12): `--workspace`
  CANNOT SEE A CONSUMER THE WORKSPACE EXCLUDES.** Run over
  `features/enemies/mod.rs`, `probe_dead_public_fns.py` reported exactly one dead
  function — `CharacterRosterFragment::from_ron_at`. That function had been
  deleted for this exact reason six days earlier and restored the same morning
  (D110); its only caller is `fixtures/external_consumer`, `exclude`d in the root
  `Cargo.toml` and the ONLY in-repo consumer that links the engine from outside a
  shared workspace — which is the entire population a public-API census is about.
  Trusting the output would have deleted it a SECOND time, on a green instrument.

  ⇒ ✔ **FIXED**: the probe builds every excluded consumer too, with the same
  markings, and unions the warnings. It reports **25 of 25** on that file now,
  and the case that produced the false positive is the case that validates the
  fix. `EXCLUDED_CONSUMERS` is a named constant so the next excluded crate is one
  line rather than a rediscovery.

  ⚠ **and a hand-rolled census still cannot substitute for it.** Re-deriving the
  same question by hand the same day marked 16 items and reported FIVE as dead:
  `sandbags_are_passive`, `archetype_for`, `contains_brain`,
  `fallback_for_provider`, `providers`. All five have callers, and the pattern is
  exact — every silent one is called only from a `#[cfg(test)]` module, and every
  one that warned has a production caller. A green hand-rolled census means "no
  PRODUCTION callers", never "no callers". The tool reports all five as live.

  ⇥ ✔ **`from_ron_at`, `provider_id`, `source` DELETED**, plus the `source`
  FIELD the compiler then reported dead. A provenance feature with neither
  producer nor consumer: `from_ron_at` was its only setter and had no callers, so
  every fragment ever built carried `None`, and `MalformedFragment`'s message has
  never printed a source. ⚠ the error still has a `source` — it reads the build
  PARAMETER, not the field, which is why the two read as one feature.

  ⇥ ✔ **AND THE PROBE FOUND A CONCEPT DYING, NOT A HELPER.**
  `ActorTuning::adopting_archetype` — "take on `archetype`'s combat tuning while
  keeping the fields this actor's PLACEMENT owns" — is deleted. It existed
  because a projected archetype tuning was assigned WHOLESALE onto a provoked
  body, which dragged a mob's `OnRoomReenter` onto a named NPC: the kill hook
  wrote no death flag, save-sync had nothing to read, and the NPC came back alive
  next room. A live bug, and this function was its fix.

  ⇒ provocation projects NO tuning at all now (D101), so the respawn policy
  survives for the same reason the run speed does and there is no wholesale
  assignment left to protect a field from. Its last caller went with
  `project_provoked_archetype` (D104). **Three ledger rows, one line of code,
  and the concept is what left.**

  ⇥ ⛔⛔ **THE PROBE SCRIPT ITSELF HAS A LATCH BUG — SEVENTH TRAP, caught by
  printing the COUNT.** A `#[cfg(test)]`-skipping flag that is set and never
  cleared latches at the FIRST such attribute in a file, so nothing after it is
  marked. On `features/enemies/mod.rs` — whose first `#[cfg(test)]` is at line
  134 — that meant **zero** functions were marked, and the run then reported 21
  functions as having "no call site", including `spec_for_brain`, which has
  callers in three crates. ⇒ a probe MUST print how many items it marked, and a
  count of zero is a failure rather than a clean bill. Every measurement in this
  campaign that went wrong went wrong by reporting an absence it had not looked
  for.

  ⇥ ✔ **from the run that DID mark things** (`integration.rs`, 12 fns):
  `ActorRef::rotation_rad` and `ActorRef::attack_telegraph_aabb`, both deleted.
  ⚠ the second was worse than dead — it returned `self.attack_aabb()` verbatim,
  a differently-NAMED accessor for the identical box. A reader reaching for a
  "telegraph" volume wants the windup's warning box, which is normally larger and
  earlier than the hitbox; this would have handed them the hitbox and looked
  correct. No caller ever did, which is the only reason it never mattered.

  ⚠ the probe's REACH is worth recording: `archetype_spec.rs` has four public
  fns and `actor_tuning.rs` three — those files are mostly struct FIELDS, which
  `#[deprecated]` does not usefully cover. A dead field is a different
  instrument (`cargo check` reports "never read" only once every reader is gone,
  which is how the `source` field surfaced).

  ⇥ ✔ one genuine deletion from the earlier grep pass also survives:
  `CharacterRosterFragment::fallback_brain_id()`, a public accessor with zero
  references anywhere including tests. Its FIELD is load-bearing — validation,
  fragment dedup and assembly all read it — and nothing ever asked a fragment
  for it.

- ✔ **D106 ⛔⛔ PORTRAIT TARGETS WERE NEVER CHECKED, IN ANY COMPOSITION.** Found
  2026-08-12 by pointing `scripts/probe_dead_public_fns.py` at the NEW
  character-template model rather than at the legacy — which is where the
  campaign's own named failure mode lives (*"+4000 new / −2400 old means the old
  model got WRAPPED"*).

  `CharacterBindings::with_available_portraits` has no caller anywhere, and it is
  the ONLY way to populate `self.portraits`. So the field is `None` in every
  composition, `PortraitTarget::NAME` never joins the prepared character's
  `checked` list, and preparation's report says *"we did not look"* about
  portraits — permanently, correctly, and therefore invisibly. A character naming
  a portrait nobody authored is a fault nothing can raise.

  ⚠ **the builder was KEPT rather than deleted**: it was the only road to the
  check, not the reason it was unused.

  ⇒ ✔ **CONNECTED, 2026-08-12.** `with_engine_portrait_vocabulary()` mirrors the
  sheet twin and is applied inside `try_register_character` — the ONE seam every
  registration passes through — rather than at the three call sites, so the next
  provider cannot forget one of them.

  ⇥ ⚠ **`OnceLock`, because `baked_portrait_registry()` PARSES.** Preparation
  runs per character, so calling the constructor there would re-parse every baked
  portrait manifest once per registered character — the startup decode storm §7.1
  deleted, rebuilt from the other end. `available_portrait_targets()` caches it
  process-globally, exactly as the sheet index does and for the reason that one's
  doc gives.

  ⇥ ⛔⛔ **AND THE TEST THAT SHOULD HAVE CAUGHT THIS IS NAMED
  `sheets_portraits_and_derived_vfx_are_resolved_at_preparation` — it only ever
  asserted SHEETS.** That is how the gap survived: the claim was in the title,
  the coverage was not, and anyone checking whether portraits were handled found
  a test saying yes. The new guard goes through the REGISTRATION SEAM rather than
  passing a resolver by hand, because a hand-passed resolver proves the check
  works while leaving it disconnected — which is precisely the state it replaces.
  Poisoned by removing the seam's call: red, quoting D106 back.

  ⇥ ⭐ **the same probe found the shape this campaign is most at risk of**: new
  API written for SYMMETRY with no caller. `checked_namespaces()` beside
  `was_checked()`, and `voice()` beside `voice_line()` — plural twins of singular
  accessors that consumers actually use. Both deleted. 50 of 54 public fns across
  `definition.rs`, `prepared_match.rs` and `character_spawn_plan.rs` do have
  callers, which is the reassuring half of the number.

- ✔ **D107 ⚠ `PreparedMatch::cast_generation` WAS THREADED AND NEVER READ.** The
  accessor has no caller (compiler-verified); the FIELD is stored, defaulted and
  copied from `registry.generation()` at plan time. Its purpose, per the
  registry's own doc, is that *"a consumer that derived something from the
  registry can hold this beside it and know, cheaply, whether its derivation is
  still about the cast that exists now"*. No consumer does.

  ⇥ ⛔ **AND THE OBVIOUS CONSUMER IS THE ONE THE MODULE EXISTS TO PREVENT.** I
  reached for the natural fix — make a cast-generation change re-prepare the
  plan, mirroring the SESSION staleness rule right beside it — and
  `prepare_the_match`'s own comment refuses it in advance: *"Re-preparing every
  tick would rebuild the seeds under a live match and re-resolve them against a
  possibly-republished registry, which is the authority-in-activation this module
  exists to remove."* A plan is deliberately FROZEN against the cast it was made
  from. ⇒ the generation is PROVENANCE, not a staleness trigger, and wiring a
  rebuild would have re-introduced the exact authority the module was built to
  delete.

  ⇒ ✔ **ANSWERED AS THE DIAGNOSTIC (2026-08-12).** Two answers were defensible —
  a `warn!` or deleting accessor AND field together — and the field's own doc
  picks one: it calls itself *"a staleness ASSERTION"*, so it should assert.
  Activation reads the live registry generation (for that ONE comparison and
  nothing else) and warns `_once` when the published cast has moved on since
  planning: a character published between preparation and activation is simply
  not in this match, which is correct and is also the only thing that would
  explain a missing fighter. The bare `cast_generation()` accessor is replaced by
  `cast_moved_on(live)` — the question rather than the value — so the field is
  no longer ceremonial and the accessor is no longer dead in the same edit. ⛔
  **not a rebuild**, for the reason above.

  ⇥ ⭐ `a_plan_can_tell_when_the_cast_moved_on_under_it` pins BOTH halves,
  because either alone is worthless: the condition is REACHABLE (publishing a
  character after planning moves the generation — via `insert_prepared`, since
  `register_character` panics after the barrier closes, on purpose) and it is NOT
  a bug (the match still seats exactly the fighters it was planned with). One
  assertion describes an alarm; the other leaves the alarm untested.

- ✔ **D108 ⛔⛔ THE RUN STALLED ON A FULL DISK, AND THE BUILD CACHE WAS 285 GB.**
  2026-08-12: every check failed at once with `No space left on device (os error
  28)` — not a compile error, a filesystem. `/dev/vda1` is 484 GB, ONE partition
  for `/`, `/tmp` and `/home`, and it was at 100% with **2.9 MB free**.

  ⛔ **the first symptom was not the build.** The harness could not capture the
  output of `echo hi`: its own temp filesystem is on that partition, so every
  diagnostic command returned "output was lost" before any of them could say
  why. A full disk breaks the instrument before it breaks the work, which is
  what makes it read as a mystery instead of a resource.

  ⇥ the measurement, once anything could print:

  ```text
    285 GB  /home/joncrall/ambition-target        the shared CARGO_TARGET_DIR
    267 GB    debug/
    204 GB      debug/deps
     58 GB      debug/incremental
     17 GB  release/
  ```

  ⇒ **freed 92 GB without touching a source file or the git state.**
  `debug/incremental` went first (58 GB): it is regenerated transparently and
  invalidates nothing. Then `debug/deps` files older than four days (34 GB) —
  the `cargo sweep --time` behaviour, and safe because a missing output is a
  rebuild rather than a wrong answer. ⚠ **age is the right axis here and the
  buckets prove it**: NOTHING in that directory was older than seven days, so
  the whole 204 GB is this campaign's own churn — every rebuild of a crate
  writes a new hashed artifact and the previous one just stays. Verified after:
  `cargo check --workspace --all-targets` clean, app 337, smash 67 + 17, and the
  cache survived (no full rebuild).

  ⚠ **it refills at roughly 5 GB per full test cycle**, so a multi-day run needs
  this periodically rather than once. The cheap habit: prune `debug/incremental`
  first and only touch `debug/deps` by age.

  ⛔ **and `du` on the target dir is itself slow enough to matter** — measure the
  top level, then one level down, rather than walking 285 GB of small files.

- ✔ **D112 CLOSED 2026-08-14 — THE DEATH ROAD WAS NEVER BROKEN; THE FIXTURE
  MODELLED A STOMP WHILE ASSERTING A SIDE CONTACT.**
  `mary_o_it::level_1_acceptance::a_small_mary_o_dies_to_one_hit_and_the_level_restarts`
  placed her at `snake.max.y - kin.size.y * 0.5` — her FEET exactly on the
  snake's top, which is the stomp position, and the same one its sibling
  `landing_on_a_snake_stomps_it_instead_of_hurting_her` uses on purpose. The
  comment one line above said *"a SIDE contact is the hit; from the top it would
  be a stomp"*; the arithmetic did the opposite of what the comment said.

  ⭐ **WHAT THE INSTRUMENTATION SHOWED** (added inside the contact pass as this
  row instructed, then removed): on the FIRST frame after the teleport the near
  snake was already `Retreating(0.35)`, `step_snake_shell` had cleared its
  `body_contact_damage` — a shell is harmless to touch — and it then fell away
  off the platform. By frame 200 there was no snake within 40px of her at all.
  Nothing could hurt her for the remaining 400 frames.

  ⇒ the fix is one line: she goes at the snake's own centre — level with it,
  overlapping it, feet BELOW its top — which is a side contact and cannot be
  read as landing on it. `mary_o_it` is 34/34 and the repository has no red
  test.

  ⚠⚠ **THE ROW'S OWN PROBE FINDINGS WERE ACCURATE AND ITS CONCLUSION WAS NOT**,
  which is the lesson worth keeping. Every fact it recorded was true — she is
  vulnerable, the boxes agree, every enemy holds `ActorTarget = her entity`,
  `body_contact_damage: true`, zero `HitEvent`s — and all of them were measured
  BEFORE the teleport, during the settle. The row then reasoned from "every gate
  probed from OUTSIDE passes" to "the drop is inside the shared body-contact
  pass", and named three innocent suspects (AC3's `BodyCombat` collapse, AC1's
  mirror deletion, D114's hitlag scoping). ⛔ **a measurement taken at the wrong
  MOMENT reads exactly like a measurement of the wrong THING.** The probe that
  settled it printed a frame ordering, not another gate.
