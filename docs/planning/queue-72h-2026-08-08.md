...
- ▢ **D81 ⭐ THE `BrainPreset` RETIREMENT, MEASURED — 125 ADOPTERS, 8 LIVE KEYS,
  ...
  * ⭐ **AND `parrot_lively` IS THE NEXT FREE ONE — with a live disagreement in
    ...
    schema change kept.
    ⇥ ⭐ **that is this row's own migration order, proven by walking into it**: the
    `aggressiveness` half must land on the placements' `SpawnDisposition` FIRST,
    and only then does a preset have nothing left to hold. The bullet above this
    one says exactly that, and I still had to find out by breaking it.
