...
- ✔ **D24 LANDED — `scripts/compile_report.py` reads all five ledgers into one
  ...
  * ⚠ the test's own weakness, stated: `deplines` saturates (most crates sit on
  ...
  ### ✔✔ ANSWERED — nightly `-Z time-passes` on `ambition_relativity2d`
  ...
  * ⚠ **and it retires my 68.1 s figure for this crate.** The ledger's release
    samples came from a build doing 541 units in 987 s wall — heavily contended.
    Uncontended, this crate is 12.77 s. ⛔ **so the absolute per-crate seconds in
    every table above are contention-inflated**; the RANKINGS are what survive,
    which is what the reader's per-build view was already saying.
  * ⚠ **not yet measured: whether this generalises.** One crate, one config. The
    same command on `ambition_platformer2d_runtime` and the monolith would say
    whether ThinLTO dominates everywhere or only for small dense crates — and
    that is the difference between a workspace-wide knob and a per-crate one.
