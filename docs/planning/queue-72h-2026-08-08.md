...
- ▢ **D70 ⛔ SOME MARY-O BLOCKS STAY SPENT ACROSS A RESTART — and it is NOT a
  ...
  * ⚠ **the falsifier, and it must name a SPECIFIC block**: bonk a `?`-block,
  ...
  ### ⭐⭐ A MECHANISM THAT FITS EVERY OBSERVED FACT (2026-08-14) — and it is WHEN, not WHICH

  ⛔ **this row asks "which blocks", and the evidence points at "which FRAME".**
  Both rearm systems are driven by MESSAGES — `RoomLoaded` and
  `RoomReplayRequested` — and **both of those messages are
  `clear_message_on_rollback`**:

  ```text
  clear_message_on_rollback::<…rooms::RoomLoaded>
  clear_message_on_rollback::<…session::reset::RoomReplayRequested>
  ```

  Meanwhile `SpentPowerBlocks` and `BrokenBricks` are rollback RESOURCES
  (`rollback_resource_clone_checksum`, `lib.rs:1525/1538`). ⇒ if the frame that
  carried the replay request is rolled back and resimulated, the message queue is
  wiped while the resource is RESTORED to its pre-clear value — so on the
  resimulated timeline nothing ever asks the blocks to re-arm, and they stay
  spent. **Intermittent by construction, and scoped to whichever blocks were
  spent before the rollback window**, which is exactly the shape of *"some blocks
  from the last run remain spent"*.

  ⭐ **it also explains why every test disagrees with Jon.** Mary-O's suite runs
  on fixed-tick fixtures where `Rollback` is absent, so the message is never
  wiped and the reset always lands. A fix that is *"present, registered,
  correctly ordered and provably reached"* and still observed failing is what a
  rollback-only defect looks like from inside a test suite.

  ⛔⛔ **AND THE EXISTING ROLLBACK-RESET PROOF DOES NOT COVER IT** — checked,
  because a green test named *"restores … a broken brick under forced rollback"*
  looks exactly like a refutation.
  `rollback_lifecycle_reset::a_manual_reset_restores_a_damaged_enemy_and_a_broken_brick_under_forced_rollback`
  runs (it is `rl_sim`-gated and `rl_sim` is a DEFAULT feature, so app_it does
  execute it) and it passes — but its brick is
  `combat::components::BreakableFeature`, a per-ENTITY Ambition component, and
  its reset is a MANUAL one. Mary-O's `BrokenBricks` / `SpentPowerBlocks` are
  RESOURCES cleared by systems that read a message, and no test drives that
  combination under rollback at all. **Two different mechanisms one word apart**:
  the proof covers the entity that gets restored, not the resource that gets
  cleared.

  ⚠ **STATED AS A HYPOTHESIS, because nobody has reproduced it under a rollback
  host.** ⛔ do not fix it blind: the falsifier is a rollback-host repro that
  spends a block, forces a rollback across the replay frame, and asserts the
  block is armed afterwards. If it reproduces, the repair is the one the ledger
  already knows — per-attempt CONTENT state must be cleared in the CONSUMER's
  frame rather than off a message that does not survive a rewind.
