...
- ▢ **D52 ⭐⭐ LIVE QUALITY APPLY CONVERGES ONE CLASS OF SPRITE AND SILENTLY
  ...
  * ✔✔ **Apply operates on historical residency — VERIFIED.**
  ...
  1. props + host-published sprites get rematerialization provenance; kill the
     false `BoundSpriteQuality` stamp (⭐ **the prop stamp is a two-line fix and
     it is verified — do it first, separately, so it is not stuck behind the
     model work**);
