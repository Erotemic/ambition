...
3. **The remaining `PresentedViewState` consumers name WHICH view.** ✔ census
   ...
   ✔ **DECIDED 2026-08-14 — DUPLICATE THE PRESENTATION PROJECTIONS PER VIEW, and
   do NOT pick a privileged view.** ⭐ **the authoritative/world source stays
   SINGULAR**; what becomes view-scoped is only the render entities that need a
   view-dependent transform — foreground/parallax, labels, nameplates. That
   distinction is the whole decision: one simulation, N projections of it, never N
   worlds. A privileged view re-centralises exactly what D116 exists to
   decentralise and would be undone by the next slice, and duplication is the one
   option that shared, fixed-split and BG3-style adaptive layouts all eventually
   need. ⛔ **the LAYOUT POLICY above it — shared / fixed split / adaptive with
   hysteresis — remains a product decision and no agent invents that enum.**
