...
| **D74** | the PCA is held off the playable roster — **by D73, it turns out** | ▢ **DIAGNOSED 2026-08-10** — the instrument was fixed and the roster change still reds: a bare registration retracts the archetype's kit and the PCA's swing spawns no strike. Waits on D73 phase 2 |
...
- ▢ **D74 A FIGHT THAT STARTS BEFORE ITS SHEETS LAND NEVER RECOVERS.** Found
  ...
  its swing spawns **no strike hitbox at all**. That is the ~100-NPC blanket-
  registration regression this file already documents — *"a bare registration
  says the character authors no body, and preparation correctly RETRACTS what an
  incoming persona does not author"* — in miniature, reproducible, and in one
  test rather than a bisect.

  ⇒ ⭐ **so the earlier conclusion on this row was half right and the half it
  got wrong is the important one.** The STATED reason (a sheet-timing
  stickiness) is indeed dead. The blocker is that **the archetype still owns
  the PCA's kit** — which is queue **D73 phase 2**, and which the plan already
  names as the reason `PLAYABLE_ROSTER` can only stop gating buildability at the
  END of that phase. This row is now a consumer of D73, not an independent bug.

  ⇒ what is left here: nothing until D73 phase 2 moves the kit. The roster line
  was reverted; the comment at `character_catalog.rs` now records the measured
  reason rather than the superseded one, and the fly assertion stays replaced
  because it was wrong on its own merits.
