...
3. ⛔ **crush is a REFUSED wire.** Every resolve site is gated on Jon's
   no-artificial-pushout rule, with a test asserting non-ejection. Do not "fix" it.
...
2. ◐ **Item custody — the first demanding consumer, and it found a FORK
   ...
   ⛔ **the honest gap was one level down: whether a body holds a weapon was
   stored TWICE** — the `HeldItem`/`PortalGun` component and `OwnedItems::equipped`
   — maintained by hand at six sites, **three of which ended the holding while
   writing only one end**. The duplication was literal: `equip_held_spec`'s own doc
   said *"Mirrors `pickup_held_item_system`"*, and `inventory_adapter.rs`
   re-exported `equip_portal_gun`/`unequip_portal_gun` and then inlined both
   anyway. ⇒ **live bug**: `drop_portal_gun_system` detached the component and left
   `equipped` naming the gun, so the inventory screen reported you were holding a
   portal gun lying on the floor — while `throw_held_item_system`, the same
   operation for the other weapon kind, did clear it. Five hand-written copies
   deleted; four functions now own BOTH ends. ⭐ `grant` deliberately stays at
...
1. **Three of the run's thirteen goal checks CANNOT FAIL.** Checks `[0]`, `[4]` and
   ...
   repair.
2. **`WornCharacter` → `CharacterIdentity` is a cosmetic rename, and the
   recommendation is to KEEP `WornCharacter`.** Both engineering blockers are gone
   (the incomplete-character population went 14 → 7 → 0, and nothing falls back to
   the catalog). ⭐ but the rename was proposed BEFORE D73 settled, and D73's
   conclusion argues against it: a character is a reusable authored template a body
   **wears**, which is what `RecharacterizeBody` presupposes — `CharacterIdentity`
   would quietly assert the thing that campaign refuted. ⛔ an agent should not
   rename *away* from a name that encodes a settled model on its own initiative.
