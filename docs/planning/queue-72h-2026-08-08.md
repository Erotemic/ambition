...
- ▢ **D35 REMOVE `bevy_material_ui`, AND FIND WHAT ACTUALLY MADE IT LOOK
  LOAD-BEARING.** Opened 2026-08-08 from Jon's *"dig into the material ui
  issue"*, sharpened by GPT 5.6. Journal: `dev/journals/material-ui-removal-2026-08-08.md`.

  **What the crate is worth.** One clean dev build, `03878f81b`, `dirty=false`,
  from `dev/compile_units.jsonl` — one profile, one commit, no pooling:
...
- ▢ **D33 THE DECOMPOSITION BRIEF (GPT 5.6, handed over by Jon 2026-08-08).**
  ...
  *"this is the payoff for recording them, so external agents can review and be a
  ...
  ⛔⛔ **BLOCKED ON D35 (material_ui removal), 2026-08-08 — and the reason is
  measurement validity, not priority.** GPT 5.6, reviewing the material_ui thread:
  *"I would do this before using new clean-build telemetry to prioritize the next
  monolith carve; otherwise Material is distorting the cost profile we are trying
  to optimize."*

  That is correct and it is the `measure_the_suspect_not_the_aggregate` lesson
  again, one level up. `bevy_material_ui` + its icons crate is **160.05 s** of the
  clean dev build at `03878f81b` — larger than any single carve D33 proposes, and
  it is a dependency **no Rust file in this repository names**. Ranking first-party
  carves against a total that includes a 160 s accidental dependency ranks them
  against noise. ⇒ **take the free 160 s out of the graph FIRST, re-measure, then
  rank the carves against a profile that reflects code we actually wrote.**

  ⚠ this is a BLOCK on the *prioritization*, not on the brief's reasoning, which
