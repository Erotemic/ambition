...
- ◐ **D102 ⭐⭐⭐ THE LAST TWO ARCHETYPE ROWS — NOW ONE — 60 production references
  ...
  ⇥ ✔ **the LIMB checks are honest** (four sites): a brain key with no row says
  NOTHING about whether a body is limbed, and `is_limbed_host` takes an
  `Option<&ArchetypeSpec>` to say so. They asked `spec_for_brain`, so a
  misspelled key inherited the reserved fallback's limb answer.
