...
- ▢ **D98 SEVEN CHARACTERS AUTHOR FACTS NOTHING EVER READS — and one of them is
  ...
  * **`special_patent_clerk`** — authors the eleven-move repertoire written this
  ...
  ⚠ what is NOT blocked: the six pirates' provoked-policy regression is real
  whatever their vitals turn out to be, and it can be closed independently by
  making the provocation road ask the CATALOG-derived character rather than only
  the prepared registry — the pirates' `provoked_profile_ref` is a fact about the
  character, and it does not need a body to be true.
