...
- ▢ **D90 ⛔ `smash_it` WAS 14/18 AND THE GATE COULD NOT SEE IT — 16/18 now, two
  ...
  * ⇒ **the question is what MOVED THE BODY BACK.** Two candidates worth
    separating before touching anything: an out-of-world RESCUE (the same
    `last_safe_pos` path `damage_apply` runs for a player leaving the world), or a
    position-authority change that makes a direct `BodyKinematics::pos` write
    no longer authoritative — the movement kernel re-deriving position would undo
    it silently every frame.
  * ⛔ **do NOT fix it by raising the flag.** A test that asserts a restart no
    knockout caused is worse than a red one.
