...
⇒ **NEXT ACTION, and it is the row's primary work:** authored item → pick up →
**a real room transition** → same occurrence survives → drop in the destination →
same `SimId` resident under the destination room's lifetime. ⛔ **do not fix that
by teaching room-transition code an inventory entourage** — custody transfer must
own/project the item's lifetime and residency. ⛔ keep the inventory/count-table
leg explicitly separate.
