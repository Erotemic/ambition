...
⭐ **M2 now has a concrete acceptance test with a live symptom** (Jon, 2026-08-14,
the F1 overlay). `rebuild_body_pose_views` is filtered `With<PlayerVisual>`, so
only player-bodied entities publish a presented pose — and the combat overlay
consequently draws a player's strike on the frame clock and everything else on
the tick clock, in the same picture. The slice is **one body-generic
presented-pose seam**, with `collision`, `hurtboxes` and body-anchored strikes
all reading the SAME delta in the same rendered frame:

```text
presentation delta = presented body origin − authoritative body origin
```

⛔ not another lookup in `debug_viz`, and ⛔ not a per-colour smoothing patch;
the seam must be reusable by any body-attached presentation consumer. Census
`draw_player_debug` while there — it already draws a PRESENTED cyan box beside
the generic overlay's authoritative one, which is two representations of one
body unless the distinction is deliberate and explained.
