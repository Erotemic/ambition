...
- ▢ **D13 two rows ORPHANED between runs, carried forward from `queue-72h-2026-08-06.md`.**
  ...
  * **E1a — no test that a drop dies with its room.** From Jon's 2026-08-05
    ...
    *"drops outliving a reset"*, and the presentation-binding-fails-silently
    ...
    leave a repeating unclaimed-view population.* Count stand-ins across two
    consecutive transitions; a number that does not return to its baseline is the
    defect, whatever entity caused it.
...
- ▢ **D16 CONFIRM (or refute) that `rollback/domains/` is the frontend cost —
  ...
  * **inferred**: that the generics *cause* the frontend cost. **Cargo's unit is
  ...
  ⚠ **a cheaper falsifier exists and should be tried FIRST**: the runtime's
  `room_transition/` is 1,723 lines with **ZERO** generic functions, against
  `rollback/`'s 8.55/kloc. If frontend seconds do not divide between those two
  areas roughly as the generic density predicts, the hypothesis is wrong and no
  nightly toolchain was needed to learn it.
