...
one with real content (3 authored `LockWall`, 14 authored `Switch`, two
independent contributors): it APPEARS, it does not SLIDE, so adopting the mover
...
- ⏸ **D116 — Ambition multiplayer/multi-view first slice. M2 DONE, RESTING at the maintainer's instruction.**
...
✔✔ **M2 IS DONE AND THIS ROW NOW RESTS (2026-08-15), by maintainer instruction —
*"do not overstate M2; one bounded composition proof remains, then STOP."*** That
proof landed and verified (app gate clean, `app_it` 361, host 120, Smash 18).
⛔ **do not open an M3 here.** The next multiview work is gated on a real product
need for a second view, not on tidying what M2 exposed.

⚠ **and M2 stopped ON a boundary rather than at completion, which is the honest
framing:** two bypasses were named and left. **No production seam spawns a second
camera**, and `publish_camera_viewport` writes ONE rectangle to every view **by
construction** — it projects the single `ResolvedGameplayPresentation`, which is a
fact about the physical screen. ⇒ **distinct gameplay rectangles cannot come out
of the shipped schedule today**, so the remaining composition gap belongs to a
future split host, not to this slice. ⭐ that is a composition gap, not a defect,
and the difference is why the row rests instead of carrying a ▢.
...
⭐⭐ **THE CUSTODY SEAM IS MEASURED (2026-08-15), and it is ONE CLASS WIDE.**
Instance-capability is decided by exactly one thing — whether `Item::held_item_id()`
resolves, because that is the only key `Item::from_held_item_id` matches a
`GroundItem`'s `HeldItemSpec.id` against. The 24 catalog slots are **not** uniform:

- **held weapons/abilities (9)** — Axe, Javelin, GunSword, PuppySlugGun, Fireball,
  Blink, Grapple, MarkRecall, Bomb — ⭐ **are BOTH an instance and a count**;
- **portal gun** — partial: a pickup body with **no** `ItemCustody`, still
  destroy/respawn;
- **unwired abilities (3) · consumables and currency (5) · key/quest items (5) ·
  reserved (1)** — ⭐ **counts forever**, and their readers legitimately want a
  quantity. `PickupKind::Currency|Health` credits a number and marks the dispenser
  `Collected`; no object is implied.
- `WorldItem` (touch-to-equip → `WornEquipment`, the object genuinely ends) is a
  correct third thing outside both models.

⇒ **so 5 of 6 classes stay counts forever and the whole problem is the nine.**
⛔ this retires the instinct to give the count table a row per object.

⚠ **the flattening line is `items/pickup/mod.rs:616`** — `owned.grant(item, 1)`
inside `pickup_held_item_system`, while the throw deliberately keeps the count.
⛔ **and that composes into an unbounded duplication loop**: pick up an authored
axe (count 1, `Held`) → throw (count 1, `InWorld`) → the menu's Equip checks only
`owned.has`, so the hand fills from the count with no object behind it → throw →
the mint arm at `mod.rs:770` **materialises a second axe** → repeat.

⛔⛔ **NOT FIXED, and deliberately.** `owned.take` on throw closes it and opens a
worse hole: a dropped `GroundItem` is room-scoped, so leaving the room destroys it
and **the count is the only survivor**. ⭐ **the count is not entitlement here — it
is the durable-save MIRROR of an instance, and the mint arm is its restore path.**
Closing it needs the body inventory that does not exist yet. ⇒ **"who owns a body
inventory?" is now the blocking unknown for everything downstream** — the
duplication, the mint arm, and `OwnedItems::equipped` all wait on it. It is
already open in the plan's own question list; it does not need a new decision row.

✔ **what DID land, because it needed no product decision:** `unequip_held` empties
a hand and touches nothing else, so the menu's **Stow** and **equip-swap** left the
object recording `ItemCustody::Held` by a body with an empty hand — **a third
state the enum does not have**, skipped by physics, by pickup and by the drawn
view, and unfindable by the throw. ⛔ **an authored axe with a `placement:` `SimId`
silently ceased to exist through the menu** — the exact loss `ItemCustody` exists
to prevent, and textbook *retract by REMOVING*. `return_released_items` re-derives
custody from the hand each tick and **RESETS** to `InWorld` at the holder's
position; it is matched by **spec id** rather than `Has<HeldItem>`, so equip-swap
is caught too, and it runs first in `CoreHeldItems` so it reads a settled hand.
⚠ behaviour change: a body that stows now leaves a `GroundItem` at its feet.

⛔⛔ **I RAN THE FALSIFIER THE WORKER COULD NOT, AND IT FAILED.** Removing
`return_released_items` from the production chain leaves
`an_item_stowed_from_the_menu_returns_to_the_world_and_can_be_taken_again`
**GREEN**, because the test builds its own `Update` chain that lists the system
by name. ⇒ **the test pins the FUNCTION's behaviour, not the WIRING** — the worker
reported the poison as reasoned-not-executed and was wrong about its outcome.
⭐ the fix is real and the gate is green; what is missing is any oracle that would
notice the system being unregistered. ⛔ do not answer that with a test that
asserts a system is registered — that is source/policy machinery. The honest
answer is a behavioural test built on the real plugin, and this crate's tests
hand-list their chains as an established pattern, so **changing that is its own
slice, not a patch to this one.**

⇒ **deletion gates named by the measurement**, none of them takeable yet: the
throw's mint arm · `OwnedItems::equipped` (self-described *"ONE FACT STORED
TWICE"*, and structurally wrong for a couch — **one equipped slot, four seats**)
plus the `owned:` parameter threaded through all four equip/unequip functions ·
the portal gun's despawn/respawn pair. ✔ **rollback untouched: schema 30 stands,
no re-baseline owed** — `ItemCustody` was already registered as `entity:item_custody`.
...
✔ **THE LEDGE TRANSITION IS CLOSED (2026-08-15), and it was one predicate.** The shadow's walk-off test compared the body's **centre** to `ground_span` while `surface_supports_body_at_rest` compares its **FOOTPRINT**, so `left_the_ground` captured a position a half-extent early — 11px on the shipped fighter — at which the real kernel still found the body standing and reprieved the walk-off with the very platform it was leaving. The fix is not a nudge: `integrate` now calls `ae::collision_semantics::spans_overlap_for_support`, the 1-D core extracted out of `perpendicular_overlap`, so the shadow asks the kernel's own question with the kernel's own `EDGE_OVERLAP_SLOP` and the centre test is DELETED. `Perceived::supporting_floor` carries the identical correction from 2026-07-31; this was the last centre-in-span ground test in the fighter's ledge reasoning.

⚠ **two limits still recorded, not hidden:** blast margins are probed at **zero** while the stage authors 120px (conservative by exactly that; it does not interact with the ledge case — the probe reprieved off a *platform*, not off the envelope) · cost is unmeasured, and the existing budget bench rolls no movement lines so it does not price the lens. Probe FREQUENCY is unchanged by the ledge fix (still ≤1 per modelled verb per decision); the capture point moves ~2 ticks later along the same line.
...
- ✔✔ **the gate portal's rollback waiver was a REAL DESYNC DEFECT, audited and
  fixed 2026-08-15** — and the shape generalises, so it is written up below rather
  than buried. ⭐ **the waiver was a TRUE SENTENCE ABOUT THE WRONG HALF OF A
  VALUE**: `GatePortalConfig` really was authored data, and the mutable phase was
  never on it.

  ⭐⭐ **the decisive asymmetry: the INPUT rewound and its INTEGRAL did not.** The
  switch lives in `AmbitionGameSave`, which **is** rollback-registered. The phase
  is that switch integrated over time, and was not — so a rewind restored the
  input and left the integrator ahead by the rollback depth, **permanently**.

  Evidence, all read from registration sites rather than comments:
  `tick_portal_phases_system` is registered into `app.sim_schedule()`, which the
  shipped host binds to `bevy_ggrs::GgrsSchedule` ⇒ it mutates on speculative
  frames. `Opening` lasts **0.640s ≈ 38 ticks** and is entered by an ordinary
  player interaction — not a loading frame, not a barrier. And the consumer is
  authoritative, not cosmetic: `world/rooms/systems.rs:219` **refuses a room
  crossing** unless the phase is `On`, so two peers promoting `Opening → On` on
  different frames disagree about whether the crossing happened.

  ✔ **fix, and it is a deletion:** `GatePortalConfig.phase` is **gone**; live
  phase moved to a `GatePortalPhases` resource registered as
  `resource.gate_portal_phases`. The registry is now genuinely authored, so its
  waiver text was **rewritten to answer the checker's real question** instead of
  being deleted. ⚠ the registry deliberately stays **unregistered**: its populate
  runs in `Update` behind a one-shot `installed` flag that is itself waived, so
  snapshotting it would let a rewind **erase authored portals nothing refills** —
  recorded at the registration site.

  ⚠ **schema 30 → 31**, so the wire-format baseline and the registered-name
  absence contracts both need re-running.

  ⭐ **why no instrument caught it:** the state was a **string-keyed `HashMap`
  resource**, which is invisible to entity-scoped rollback sweeps. A survey of
  every `Resource` holding `HashMap<String, _>` found exactly one other mutated
  per-frame in the sim — `ActorSteering` — and it is already covered by
  `declare_rollback_derived_resource`. The rest are authored catalogs or render
  indices.
