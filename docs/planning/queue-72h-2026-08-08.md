...
- ▢ **D33 THE DECOMPOSITION BRIEF (GPT 5.6, handed over by Jon 2026-08-08).**
  ...
  **▢ STILL OPEN — the next action is STEP 2**, flagged stale by a parallel
  session 2026-08-08 and verified here (the row said "step 1" after step 1 had
  landed — [[reference_a_jobs_verdict_is_not_its_first_targets]]'s failure mode,
  a ▢ on landed work).
  * **step 2**: move two pure-data components down — `PlayerBlinkCameraState`
    (`avatar/components/mod.rs:31`) and `ActorAnimOverride`
    (`features/ecs/actor_clusters.rs:39`);
  * **step 3**: invert `posed_body_for` (`character_runtime/presentation.rs:365`)
    so the monolith stops constructing `SpritePosedBody`.
  ...
  hoist — it **refuses** to price a nested module, correctly), and whether the
  runtime crate can own `SpritePosedBody` construction.
...
- ▢ **D38 GIVE THE PROCEDURAL SFX PATH A LOUDNESS TARGET. ⛔ AND JON'S PREMISE
  ABOUT THE MUSIC IS REFUTED — do not turn Sanic's score down.**
