...
- ▢ **D98 SEVEN CHARACTERS AUTHOR FACTS NOTHING EVER READS — and one of them is
  ...
  * **`special_patent_clerk`** — authors the eleven-move repertoire written this
  ...
  ⇒ **the unblock is the same one the goblin and the lab raider took**: author
  each character's body from the numbers it gets TODAY (the `combatant` fallback:
  4 HP, 155 px/s, 0.70 contact, 1 damage), carried across unchanged, then
  register. Making a quartermaster tougher than a lookout is a design decision
  and belongs where design decisions are visible — not smuggled in by a
  migration.
...
- ▢ **D96 SIX CONTENT DECISIONS ARE STACKED UP, AND EVERY ONE OF THEM BLOCKS A
  ...
  ⇥ ⭐ **and an eighth that is different in kind**: P3.25's bridge cannot be
  ...
  ⚠ **the header says six and there are eight**; the count is left wrong in the
