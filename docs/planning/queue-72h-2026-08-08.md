...
- ▢ **D9 COMPILE TELEMETRY: record enough to find non-obvious wins.** Jon,
...
- ▢ **D10 (= tracks K2b.1) migrate `headless` / `rl_sim` / `capture_scene` to
  ...
  ⚠ **the tracks staging is PARTLY STALE and the row should not be worked from it
  verbatim.** Measured 2026-08-08: `shell_host.rs:51` states *"`direct_entry` was
  deleted with the path it gated (K2b edit 3)"* — so part of K2b.2/K2b.3 has
  already landed. But `cli.rs:245,271,886` still carries a *different*
  `cli_direct_entry()`, and `headless.rs` + `rl_sim/mod.rs` still compose the
  simulation plugin directly, so K2b.1 itself is genuinely open. **Two things
  named `direct_entry`, one dead and one alive** — re-measure before touching
  either.
  ⛔ **sequencing**: `capture_scene` is the phone proxy and is in active use by
  the sprite and death-beat measurements. Do not migrate it while a capture-based
  measurement is in flight; do `headless`/`rl_sim` first and prove both paths
  agree before deleting the build-time fallback, which is what the staging asks
  for anyway.
