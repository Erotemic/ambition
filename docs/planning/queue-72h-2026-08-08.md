...
| **D60** | the portal gun has D51's bug | ⛔⛔ **DO NOT DISPATCH AS WRITTEN** — 187 refs; arbitrate the resolution |
| **D59** | `ambition_render` lib test cannot link | ⚠ theory **WEAKENED**; filed for Jon as hygiene, **not** as the fix |
| **D56** | the renderer binds a sheet by DISPLAY NAME | ⛔ **must land with D48**; deadlock proven symmetric |
...
- ▢ **D33 ✔✔ THE CARVE LANDED `a64bf22f8` (2026-08-09) — `ambition_character_sprites`
  ...
  **4.80 s / 2,212 lines = 2.170 ms/line** against the 2.557 median it had been
  ...
  ### ▢ WHAT REMAINS — a measurement, and it needs an EMPTY fleet

  ⛔ **`python3 scripts/compile_ratchet.py` exits 1** on the `UNPRICED` finding, so
  the repo-tooling ratchet job is red until `ambition_character_sprites` is
  actually measured. Saying *"the guess is accepted"* in a commit is the right
  disclosure and **does not change the exit code**.
  ⇒ run `python3 scripts/compile_collect.py` and re-freeze.
  ⚠ **NOT NOW**: that script's own docstring says *"one cargo build at a time, and
  its OWN target dir … this script never backgrounds a build"*, and records the
  symptom of ignoring it — **a warm no-op that should take under a second
  reporting 222 s**. With another agent building, the measurement would be
  garbage, which is worse than the placeholder. ⇒ **run it when nothing else is
  building.**
