...
- ▢ **D52 ⭐⭐ LIVE QUALITY APPLY CONVERGES ONE CLASS OF SPRITE AND SILENTLY
  ...
  * **`publish_under` aliases can pin the old image.** `demote_stale_realizations`
    only removes stale `sheets` whose token is in `declared`, and `publish_under`
    creates no declaration — so an undeclared alias keeps a clone of the old
    `CharacterSpriteAsset`, and a strong handle keeps the texture resident. Mary-O
    publishes under sheet target, character id AND display name.
