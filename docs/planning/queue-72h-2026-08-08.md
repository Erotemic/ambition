...
- ▢ **D23 SPLIT THE BOLT/HURTBOX FINDING: one half is a bug, one half is Jon's.**
  ...
  one needs Jon:
  * ⛔ **INTANGIBILITY IS A BUG, not a feel question.** An EMPTY
    `DamageableVolumes` means *this body cannot be hit*. A bolt currently hits it
    anyway. There is no taste in that — no reading of "how projectiles should
    connect" wants a bolt landing on something authored as intangible.
  * ▢ **PRECISION IS JON'S.** Making a bolt test the authored hurtbox rectangles
    instead of the coarse `CenteredAabb` retires `strict_intersects` for
    projectiles and changes how every shot connects. That is feel.
  ⭐ **so the intangibility half is takeable now**, and it is the half with a
  correctness argument. ⚠ probe it first — a body with an empty volume list, hit
  by a bolt — because *whether it is reachable in shipped content* decides
  whether this is live or latent, and today has produced both.
