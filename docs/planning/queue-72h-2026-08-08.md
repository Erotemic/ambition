...
- ▢ **D89 ⛔ THE PCA AUTHORS ITS WHOLE BODY AND THE DUEL STILL WILL NOT TAKE IT
  ...
  ⇥ ⇒ **the CAPABILITY assertions pass and only the ACTIVATION count is zero**,
  so the next probe belongs on the BRAIN'S DECISION rather than the body's kit:
  instrument what `smash_cfg_from_spec` receives for a character-first body and
  whether `can_shield` survives into the running driver.
