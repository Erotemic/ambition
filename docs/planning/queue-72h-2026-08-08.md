...
- ▢ **D34 WHY IS `ambition_platformer2d_runtime` SO EXPENSIVE TO COMPILE?**
  Dispatched 2026-08-08. **A bounded piece of D33, promoted ahead of the campaign
  because the measurement says the campaign may be aimed at the wrong crate.**
  ⚠ the row is written after dispatch rather than before — the charter asks for
  the reasoning in the spine first, and D33's own row already carried it, but
  this should have been its own row at dispatch time.
  * **the case**: the runtime is the most expensive first-party unit in **3 of 3**
    release builds, on ~14.7k lines against the monolith's ~112k — a per-line gap
    of 8–24x. The decomposition brief flagged it (*"a surprisingly expensive
    frontend phase despite being far smaller"*) and then targeted the monolith
    anyway. Its instinct and its target disagreed; the measurement backs the
    instinct.
  * ⭐ **it is also the ONE crate already singled out for mitigation** —
    `[profile.dev.package.ambition_platformer2d_runtime] opt-level = 0` — so
    somebody has already noticed it hurts, in dev only. Release has no profile
    table at all.
  * **the brief's own §"Also investigate the runtime frontend hotspot" is the
    task**, and it is cheap next to the campaign it gates.
  * ⛔ **two dead ends handed over so they are not re-walked**: a prior experiment
    removed ~2,130 lines of rollback registration and many generics for
    essentially no `cargo check` change, so *"rollback looks generic-heavy"* is
    already falsified; and my own `-Ztime-passes` on this crate is unusable for
    the frontend question (release AND warm-cache — `type_check_crate` read
    0.019 s for 14,747 lines, which is reuse, not speed).
  * ⚠ **scope fenced deliberately**: measurement and a recommendation. No carve,
    no module moves, no rollback surgery, and no new compile-analysis
    infrastructure — three readers and a collector already exist.
