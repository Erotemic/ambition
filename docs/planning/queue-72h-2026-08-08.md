...
- ▢ **D109 JON'S "MARY-O CAN ONLY HAVE 1 FIREBALL OUT" IS SATISFIED IN CODE AND
  ...
  **no test**. A one-character edit to that constant restores the behaviour Jon
  ...
  ⚠ ▢ **what is worth doing is the guard, not the fix** — a test that fires three
  times and asserts two live sparks, with the count read from the constant so it
  cannot drift from the rule. ⛔ and asserting the CONSTANT equals 2 would be
  worthless: it must count live shots, which is what the gate actually does.
...
- ▢ **D108 ⛔⛔ LANDING LAG IS A PLAYER-ONLY MECHANIC, IN TWO INDEPENDENT WAYS —
  ...
  **for exactly the reason this row is about** — one list is checked by the
  ...
  across the rebuild, decrement it in the actor tick, and pass
  `recoil_lock.max(landing_lag)` on the actor road too. ⛔ **carrying it alone
