...
- ◐ **D102 ⭐⭐⭐ THE LAST TWO ARCHETYPE ROWS — NOW ONE — 60 production references
  ...
  `combatant` key, `spec_for_brain` substitutes it for any unknown key, the
  content schema requires it as `FALLBACK_BRAIN_KEY`, and the ROLLBACK provoke
  road still resolves the archetype id a binding recorded. The first three are
