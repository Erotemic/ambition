...
- ▢ **D61 ⭐ 1-2's FLAGPOLE HAS NO FLAG BECAUSE THE MAP NEVER AUTHORS ONE — and
  the KNOB is missing too, which Jon did not report.** Jon's *"In mary-o 1-2 the
  flagpole doesn't have a flag."* Filed 2026-08-09. **His observation had NO
  ledger row at all** — found by grepping his file for lines the queue never
  picked up. ⇒ **fully diagnosed, exact coordinates below, the fix is pure LDtk
  authoring and zero Rust.**
