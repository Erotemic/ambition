...
- ▢ **D85 ⛔ THE MATCH SEAT IS THE LAST CONSTRUCTION PATH THAT ASKS TO BE
  ...
  * ▢ `IdentityKit` · ▢ the moveset · ▢ the motion-model switch · ▢ the physical
    baseline (health/mass)
  * ▢ then the stamp, and `RecharacterizeBody` leaves the seat.

- ▢ **D86 ⛔ ROBOT v3 OFF `HostCode` NEEDS A PRODUCT DECISION FIRST: does the
  protagonist keep its CHARGE?** Jon's second redirect P5, measured 2026-08-11.
