...
- ~~**D75 SMASH HAS NO KNOCKBACK GROWTH, AND JON IS RIGHT ABOUT IT.**~~ ✔✔
  **FIXED 2026-08-10.** The fix is neither of the two routes the row weighed.
  ...
  already resolves per victim. A volume that authors its own `kb_growth` still
  ...
  * `simple_melee` hardcodes `knockback: 120.0` and `kb_growth: 0.0`, with its
  ...
  * ⛔ and `scaled_knockback` returns `base` **immediately** when growth is zero.
  ...
  per-volume (`knockback`, `kb_growth`, `launch_dir`) and which already outranks
...
- ▢ **D78 ⛔⛔ A HOSTILE CHARACTER-FIRST CRAWLER DESYNCS UNDER ROLLBACK.**
  ...
  ⭐ **the discriminator is HOSTILITY, not the character road.** Ten other
  `npc_puppy_slug` placements are already character-first and rollback-clean —
  every one of them authors `disposition: Peaceful`. This placement does not, so
  it is the first HOSTILE character-first surface-walker: the crawler motion
  model plus a live brain plus contact damage. ⚠ it is NOT the patrol path
  (`EnemySpawn-104857` carries `brain: medium_striker`, no path) and not the
  positional brains — bisected to this one placement alone.
  ⇒ suspects in order: the `AdhesiveCrawler` motion model's state under a hostile
  brain, `dream_seed`/the psychedelic pass reaching a rollback-registered
  component, and `cling_breaks_on_hit` (this is the first slug that can be HIT).
  ...
  the level says Puppy Slug and the game spawns a humanoid raider.
