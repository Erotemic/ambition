...
- ✔ **D9 (original brief — the SCHEMA landed above; the COLLECTOR is still owed and is tracked as D11) COMPILE TELEMETRY: record enough to find non-obvious wins.** Jon,
...
- ▢ **D13 two rows ORPHANED between runs, carried forward from `queue-72h-2026-08-06.md`.**
  ...
  * **E1b — how many of the 30 `#[ignore]`d tests are hiding a red?** Counted
    ...
    `enemy_body_scale`, `dive_drill_reachability`. **Three have been surfaced
    this way before.** ⭐ **the cheap version is to RUN them, not read them** —
    an `#[ignore]` whose reason has expired is indistinguishable from one still
    earning it until it executes. ⚠ needs the cargo target dir to itself.
