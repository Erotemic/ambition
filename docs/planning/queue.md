...
| AP44 | The materializer asked the catalog again which sheet a character wears | ✅ DONE 2026-09-26 | Wrong: preparation folds a character's sheet (its definition's, else its catalog row's) and tuning (its row's) into `PreparedCharacterDefinition::{sheet, sheet_sizing}`, and the prepared cast holds a definition for every catalog row (`cast_with_catalog_rows`). The sprite materializer answered again: `sheet_for_declared_character` took the registered target, else the catalog row's; `character_variant_tuning` read the row's tuning AND keyed the quality tiers by the ROW's sheet. A definition that named sheet X over a row naming Y drew its base page from X and its reduced tiers' frame rects from Y. Fix: `sheet_for_prepared_character(authored, prepared)` reads the prepared sheet and tuning, and the tiers are keyed by the same prepared sheet; `character_variant_tuning`, `registered_sheet_target` and the catalog/registered-target parameters are deleted. The prepared cast is now REQUIRED by `materialize_demanded_character_sheets` (gated like the catalog), because an empty stand-in would resolve no sheet for anyone. The quality-tier fixture now carries the cast its catalog implies. Guard: `a_character_wears_the_sheet_and_tuning_its_preparation_resolved` (the goblin registered wearing another row's sheet; poisoned twice: ignoring the prepared sheet fails the sheet assertion, dropping the prepared tuning fails the tuning assertion). Validation: monolith lib 1245/1245, app_it 782/782, `external_consumer` `a_consumer_authors_the_sheet_its_own_character_renders_from` (it now resolves through the consumer's composed cast). Residue: `load_character_sprites_in` still DECLARES the sprite table from the catalog (`sheet_for_character_id_in`), and a named target that fails to load still falls back to the sheet named by the id. |
| AP45 | A fighter's moves are authored in Rust and shipped as RON | DONE 2026-09-26 (residue below) | Measured: 17 fighters read their move table from `assets/data/movesets/*.ron` through the pack (fast-iteration I2 step 5 is done), but every file's header says *"the Rust table is still the source … edit the Rust, re-export"*. The 17 `*_moveset.rs` tables (~12k lines) are the authoring road, `moveset_source_export` regenerates the files from them, and 126 behaviour tests guard the Rust copy rather than the files that ship. Two authorities for one table. Why the Rust survives: four fighters (director, officer, performer, medic) BORROW a polygon archetype's table under their own name (`archetype_moveset::under_own_name`) and replace specials, and the files can only hold the expanded copy, which drifts when the archetype is tuned. Plan, one commit each: (1) the move schema learns `borrows: (archetype, prefixes)`, resolved in `aggregate` after the merge (rename, then the file's own moves replace by id and its verbs rebind), with the borrower validated after resolution; (2) the four borrowers' files are rewritten in the borrow form, proven equal to today's tables by the parity test; (3) the 126 tests read the shipped table; (4) the 17 Rust tables, `TABLE_CHARACTERS` and the exporter are deleted, and the file headers say the file is the source. `player_robot`/`theorem_chain` stay compiled until their own migration (the robot lineage never reaches the pack seam). ✅ STEPS 1–2 DONE 2026-09-26: `EntityContracts::borrows: Option<MovesetBorrow { archetype, prefixes }>` (declared before `moveset`, so a file states what it borrows first); `MovesetContract::under_own_name` (the rename, now returning an error instead of panicking) and `overlaid_with` (a move with an existing id replaces it in place, a new move is appended in file order, a rebound verb drops the move it displaced when no verb still binds it — `replace_special`'s rule). The move schema validates borrowers after `aggregate` resolves them against the merged table, refuses a missing archetype, a stray prefix, a broken override and a borrow of a borrower, and names the borrower's file. `director`/`officer`/`performer`/`medic` are rewritten in the borrow form (7,976 → 3,370 lines), each proven by a generator to reproduce its table exactly, and by `every_content_move_table_is_the_table_it_used_to_compile_with`. `measure_authored_strike_extents.py` and `measure_hitbox_authoring_coverage.py` resolve borrows like the engine (the census output is byte-identical before and after). Guards: `a_borrower_plays_its_archetypes_timings_under_its_own_name`, `retiming_the_archetype_retimes_its_borrower`, `a_borrow_that_cannot_resolve_is_refused_and_names_its_file`, `a_chain_of_borrows_is_refused` (poisoned: skipping the overlay fails the first; dropping `in_source` fails the third). ✅ STEP 3 DONE 2026-09-26: the fighters' tests (143 call sites across the 17 files, `special_slots`, `aerial_authoring` and `two_fighters_author_a_grab`) read `authored_movesets::shipped(id)`, the table the pack carries, instead of the Rust function. Poisoned: deleting the director's `special_up` rebind from `director.ron` fails `the_director_recovers_by_teleporting_and_the_archetypes_rise_is_gone`. ✅ STEP 4 DONE 2026-09-26: the RON file is the only source of a fighter's moves. Deleted: the 17 Rust move tables (the `*_moveset.rs` files keep only their tests, compiled under `#[cfg(test)]`), `archetype_moveset.rs` (the borrow is content now), `special_slots.rs`, `moveset_source_export`, `TABLE_CHARACTERS` and the parity test that compared the two copies (−9,621 lines of Rust). `authored_movesets::tables()` reads the pack's lowered tables; `smash_in_the_host` reads the geyser's lift from the shipped move instead of restating it. The Rust tables' ~2,000 lines of design prose moved into the files: a note that names one move sits above that move, the rest under a `DESIGN NOTES` header (a script checked that no comment line was lost). The file headers say the file is the source. The maintainer-attribution floors and the reviews-page census now read the RON files (poisoned: renaming one `Jon:` in `performer.ron` fails the floor; blanking `Jon` in `alice.ron` fails "listed as REVIEWED but … no maintainer input"). A new fighter is now one RON file plus its pack line, and a variant of an archetype states only what differs. RESIDUE: `player_robot` and `theorem_chain` were still compiled Rust tables (the robot lineage never reached the pack seam); ✅ migrated by AP50. The fighters' test modules restate some tuning constants to build expected values; a test that needs a timing should read it from `shipped(id)`. |
| AP46 | Two answers to "how far away does a body notice a foe" | ✅ RULED 2026-09-26 (reported by ToothbrushAmbition) | A fighter brain's `aggro_radius` (policy data, jittered in `brain_builders.rs`) gates pursuit in `brain/smash/mod.rs` and `mode.rs`. Separately, `Perception::Sighted { viewport_half }` (`DEFAULT_VIEWPORT_HALF` 480×320, `features/ecs/perception`) clips what the body can see. A foe at 400 px can be seen and ignored, and a radius past 480 px names a foe the body cannot see. Plan: measure which one each brain reads, rule one authority for "noticed" (sight clips, the brain's radius is a preference inside it, or the reverse), delete the other road, and state it in the character definition. | Measured: not a duplicate authority, two layers with one question each. The brain tick fills the snapshot's `target_pos` / `target_alive` from the body's belief (`belief_from_nearest` / `believed_target` in `actors/update.rs`) for every brain whose `perception_requirement` is `TargetBelief` or `TacticalWorld`, which includes Smash and Fighter. The Smash brain's `distance_to_target` is computed from that `target_pos` (`smash/observation.rs`), so `aggro_radius` compares against the known foe only. Ruling: the senses answer "what does this body know" (AP57: `perception_of`, the seat and the session's one extent, plus memory); `aggro_radius` answers "inside what distance does this driver engage a foe it knows". A foe seen at 400 px and left alone is the preference working; a radius past the viewport keeps a chase toward a remembered foe. The radius cannot name an unknown foe, so nothing is deleted. Stated where an author sets it: `BrainProfile::aggro_radius`'s doc now says it engages a KNOWN foe and does not decide what the body knows (it said "notices", which was the ambiguity). |
| AP47 | `ActorDecisionSet::Observe` has an order nobody states | ✅ DONE 2026-09-26 | Reported by ToothbrushAmbition: adding any system to `ActorDecisionSet::Observe` turned `leaving_a_room_and_returning_rebuilds_what_entering_it_built` red. Measured: Observe was not the cause. One added system reorders about 440 of the sim schedule's 5,720 unordered conflicting pairs. The pair that mattered: the collision overlay (`rebuild_feature_ecs_world_overlay` and seven contributors — both lock-wall writers, the gnu_ton gate and back, Sanic's monitors, Mary-O's bricks and hidden blocks, sand) had no edge to its readers in `WorldPrep` (`integrate_sim_bodies`, `tick_actor_brains`, `update_body_mode`). When the executor put integration first, a body collided with LAST frame's overlay. On the first frame after a room commit that is the previous room's geometry: the returning `EnemySpawn-104857` was pushed out of `gate_stack_lower`'s `gate_alice_private_lock` (right edge x=976) and stood at 1018.1 instead of its authored 1008.0. The gnu_ton registration's comment already claimed the edge ("before the WorldPrep collision consumers"); nothing stated it. Fix: `FeatureWorldOverlayContributions` (shared schedule vocabulary). The actor kernel configures it once, after `FeatureWorldOverlaySet`, with both sets before `ActorDecisionSet::Targeting`, the first `WorldPrep` phase that reads the overlay. Every contributor joins the set and names no edge of its own. Guards: `collision_overlay_order::the_collision_overlay_is_complete_before_anything_reads_it` (app_it, shipped composition) refuses any unordered overlay pair that is not contributor–contributor, and checks by type that the exclusive `sync_authored_gated_lock_walls` is a member, since an exclusive system's conflicts carry no component ids. Poisoned: deleting the `.before(Targeting)` edge reports 21 pairs; taking the lock walls out of the set fails the membership check. `world_gating`'s unit test now asserts membership; poisoned, it reddens through the membership assertion. Validated: the peer's reproducer is green (the returned enemy stands at 1012.7, as in the plain order); app_it 784; the six touched crates; the sand room and the guard with `--features falling_sand`. RESIDUE → AP48. |
| AP48 | The collision overlay is room-scoped and does not know its room | ✅ DONE 2026-09-26 | AP47 ordered the overlay before its readers in `WorldPrep`, so a frame integrates against its own overlay. It did not make the overlay a statement about one room. On the commit frame, every system after `RoomTransitionSet::Apply` composed the NEW room's geometry with the overlay of the room just left, until the next rebuild; `CollisionWorld::solids()` cannot tell. Measured: returning from `gate_stack_lower` to `combat_calibration_lab`, the commit tick's overlay still held `gate_stack_lower`'s gate solid (880,656)..(976,736). The commit's own arrival diagnostic (`log_room_transition_landing`) probed that overlay too, beside the staged target geometry, so its `overlay` gap and overlap counts described the previous room. Fix: the commit retracts the overlay in the same transaction. `FeatureEcsWorldOverlay::retract_for_room_change` clears every field of both owners (the destructure has no `..`, so a new field must get a decision), and `RoomTransitionCombatReset::clear_carryover` calls it beside the projectile and gravity resets. Retract by resetting, not removing, the rule `retract_gated_lock_wall_verdicts` states. The overlay is a derived rollback resource rebuilt every tick, so no stamp is needed: an empty overlay states nothing about any room, which is true until the contributors rebuild it at the head of the next tick. Deleted: the overlay half of the landing diagnostic (`ground_gap_below_feet` probes the arrival room's geometry only, and the log line lost `overlay_blocks` and the overlay overlap count). Guard: `canonical_reconstitution::the_collision_overlay_leaves_with_the_room_it_describes` (app_it). It samples the commit tick, refuses a neighbour whose overlay has nothing the home room lacks (premise), and checks as a control that the tick after the rebuild gives back the home room's overlay unchanged. Poisoned: without the retraction call it fails at the commit-tick assertion and names the gate. The multi-writer ratchet saw the commit as an eleventh overlay writer; the adjudication now records it as a retraction, not a contribution. Validated: runtime and shared_tangle crates, app_it 788, pytest lane 1,581. RESIDUE → AP56. |
| AP56 | The overlay's contributors are unordered among themselves, and a rebuilt body lags one tick | OPEN (2026-09-26, from AP48) | Measured by AP47 and not fixed: (1) the overlay's contributors (`FeatureWorldOverlayContributions`) stay unordered among themselves, so block order in the overlay depends on the executor. Collision is expected to be order-free, but no test says so. (2) A rebuilt body stands still for its first live tick (vx=0 at tick 24), and `canonical_reconstitution`'s `POSITION_TOLERANCE` (2.0 px, "one frame of the fastest patrol") absorbs that one-tick lag rather than naming it. |
| AP57 | A brained body is built without its senses, and a system repairs it | ✅ DONE 2026-09-26 | Before: every brained actor was built with no `Perception` and no `PerceptionMemory`, and `ensure_perception` (`ActorDecisionSet::Prepare`) found such bodies each tick and attached `Perception::Sighted { viewport_half }` plus an empty memory: a construction repair. It also copied a session value into every body: `viewport_half` is the generation's one extent (`SessionMechanics.perception_extent`, or the App's in a direct composition), so each body stored a copy of it. When the session could not decide the extent (a shell session between generations) it inserted a third component, `SensesUndecided`, which the brain tick filtered on, because an absent `Perception` read as `Omniscient`. That is three components (two of them rollback rows) and a per-tick system to say two facts: what a body is, and how far the session lets bodies see. A rebuilt room's bodies spent the rest of their commit tick without senses, which `a_rebuilt_body_is_complete_before_anything_decides_for_it` had to hold by schedule order. Now: the brain tick derives the senses when a body decides. `perception::perception_of(seated, extent)` is the rule (a body seated in a match is `Omniscient`; any other is `Sighted` with the session's extent; `None` when the extent cannot be decided, and the tick skips the body, the outcome the marker gave). `SenseExtent` is the one read of the extent (the same `perception_extent_for` ranking). `PerceptionMemory` moved to `ambition_characters::perception` beside `WorldMemory`, and `Brain` `#[require]`s it, so a body is built with its memory on every spawn road (room, summon, encounter wave, direct minion), not only the ones a construction hook would see. Deleted: `ensure_perception`, the `Perception` component (it is a value now) and its snapshot codec, `SensesUndecided`, the rollback rows `actor.perception` and `actor.senses_undecided` (schema 233 → 234), the `Default` that made absence read as `Omniscient`, and the test that the marker is cleared in the same command. Guards: `perception::tests::a_session_that_cannot_decide_senses_takes_the_body_out_of_the_decision` (the direct-composition control, the shell subject, and a new seat arm) and `the_perception_extent_knob_is_inert_unset_and_reaches_every_body_when_set`, both now through `SenseExtent` and `perception_of`; `smash_in_the_host::a_seated_fighter_keeps_its_omniscient_senses` asks the same rule about every brained body in the real host; `a_replayed_room_is_built_whole::a_rebuilt_body_is_complete_before_anything_decides_for_it` now checks that rebuilt brained bodies arrive with their memory. Poisoned: deleting the seat branch of `perception_of` fails the unit seat arm and the host seat test (seat 0 decided with `Sighted 480`); deleting `Brain`'s `#[require]` fails the rebuilt-body arm (3,014 brained bodies with no memory) and `frozen_belief_does_not_resurrect`; reading a `None` extent as `Omniscient` fails the unit subject arm. Game expression: a spawn road, a demo or a new actor kind states nothing about senses; a brain brings its memory, and the seat is the one fact that changes how a body sees. Validated: actor_monolith lib 1,245, ambition_characters lib 448, runtime and rollback_ggrs, app_it 788, pytest lane 1,581 (the codec-shape baseline re-recorded for the moved codec). |
| AP58 | A game states its cast twice: catalog rows in a Rust string, and a Rust registration loop | ✅ DONE 2026-09-26 (steps 1-3 and the versus roster; see AP63-AP72) | Before: a demo's cast was said in two places, both code. Its catalog was RON pasted into a Rust string (`SMASH_CATALOG_RON`, `SANIC_CATALOG_RON`, Mary-O's, twintrack's, pocket's, the host's versus roster), registered through `CharacterCatalogFragment::from_ron`, which that type's own doc calls the legacy reader: a second parser beside the content compiler, so a pack could pass `compile()` and be assembled by a different reader. Then a Rust loop registered each character again, restating its name and sheet and adding what the row could not say. Smash's loop stated a weight column, `DEFAULT_TUNING` and an 11-verb `AbilitySet` for each of three fighters. Adding a fighter meant a row in a string, a tuple in a loop, a pack line and a move file. The catalog row can already say most of it: ability grants (`AbilitySet::compose`), `axis_tuning` (sparse; `Some(())` is `DEFAULT_TUNING`), `max_health`, the sheet (`manifest_target`), and preparation folds these into the definition. Plan: (1) the engine states a cast from a pack in one call; (2) each demo's catalog becomes a pack source and its loop is deleted; (3) Ambition's `authored/` files (36, 2,085 lines of per-creature Rust: locomotion 39 calls, health 38, inline brain profiles 20, contact damage 15, action sets 14) move to content, the same road the moves took in AP50-AP55. ✅ STEP 1 DONE 2026-09-26 (engine + Smash): `EmbeddedPack::cast(provider, default).register(app)` registers the catalog fragment from the COMPILED catalog (`from_prepared`, not `from_ron`) and every row as a character, with its move table and its platform-fighter facet's knockback weight; a pack with no `character_catalog` source is a refusal naming the pack. `AbilityGrant` gained six single-verb grants whose systems have landed (`Attack`, `DirectionalPrimary`, `Pogo`, `Shield`, `Dodge`, `LedgeGrab`), as its doc prescribes. Smash: `assets/data/character_catalog.ron` is a pack source; each row states its grants and `axis_tuning: Some(())`; the stand-ins' weights are their own facet files (`data/fighters/smash_duelist_{a,b}.ron`), beside George's, so weight has one content home. Deleted: `SMASH_CATALOG_RON`, the registration loop with its weight column, tuning and ability literal (about 110 lines); `install_smash_content`'s cast is now three lines. Game expression: a Smash fighter is a catalog row, a move file, an optional facet and their `pack.ron` lines, with no Rust. Guards: `smash_in_the_host::the_smash_demos_cast_is_prepared_from_its_pack_as_the_rust_registration_built_it` (shipped host, admission on; name, sheet, `DEFAULT_TUNING`, and the kit as the match applies it; poisoned: dropping `Pogo` from Robot v2's row fails the kit arm, dropping George's `axis_tuning` fails the feel arm) and `george_carries_the_knockback_weight_his_own_facet_authors` (now every fighter's weight is its own facet's; the old arm asserted the stand-ins stated none, which guarded the deleted per-id table); the facade test checks the no-catalog refusal. Validated: ambition_demo_smash 267, ambition_demo_smash_app 18 + 51, facade content test, app_it 789, pytest lane 1,581 (MODULES.md regenerated for the module doc). ✅ STEP 2, SANIC HALF: AP63. ✅ STEP 2, MARY-O HALF: AP64. NEXT (measured 2026-09-26): Sanic and Mary-O also state a sheet-scaled body (`with_sprite_authored_body(world_per_pixel)`, computed from a baked sheet and a standing height; Mary-O's three forms share one scale from the small form's sheet) and a per-form voice, and Mary-O registers four enemies with full Rust definitions (locomotion, contact damage, a named policy). Her catalog is a RON template with a tuning block substituted into it in Rust (`$CLASSIC_AXIS_TUNING`). The catalog row has no field for locomotion, contact damage, a named autonomous policy, a sheet-scaled body or a voice, so step 2 starts by adding that vocabulary to the row and folding it at preparation, like `max_health`. Then the row, not Rust, is where Ambition's `authored/` files migrate (step 3). ✅ STEP 3 DONE: AP66, AP67, AP69 and AP71 moved all 36 files of `authored/` into catalog rows (the directory is gone; every Ambition creature is its row), and AP72 moved the host's versus roster into a pack. Left, as their own rows: the RON-game builder API (`PlatformerExperienceAuthoring::characters` / `CharacterContent::Ron`) still reads catalog RON through `from_ron`, and the definition-level `voice` floor has no author left (AP73). |
| AP59 | A cache of the action scheme that nothing read | ✅ DONE 2026-09-26 | From the AP34 residue ("`ActorActionScheme` ... insert-if-missing"). Measured by a read-only walk and re-checked here: `ActorActionScheme` had no production reader. `reconcile_action_schemes` attached it to every `BodyAbilities` body at first sight and re-derived it on change; its own module doc called it an "OBSERVATION CACHE" for RL, possession and debug tooling, and none of those reads it. Both real consumers already derive the scheme from the body's live authorities with `derive_action_scheme`: the persona gate (`gate_body_control`) and the control prompt. The AP34 row's "through `reconcile_action_schemes`" was wrong: the prompt never read the cache. So this was a construction repair maintaining a second copy of a derived fact. Deleted: the component, the system, its five tests, the monolith `action_scheme` module and its rollback declaration `derived.actor_action_scheme` (schema 234 → 235). The plugin also scheduled `reconcile_effective_repertoire`, which stays, now in `repertoire::EffectiveRepertoirePlugin`, named for what it does. The current-state docs (input, mental model, abilities, expressibility checklist) now name the derived scheme; ADR 0025 is history and keeps the old name. No new witness: a stored copy is now unrepresentable, and the derivation's own tests (`ambition_characters::action_scheme`) and the gate and prompt tests read the one that is left. Also fixed here: `docs/planning/game/bosses.md` cited a bare `character_catalog.ron`, which became ambiguous when AP58 committed Smash's catalog (AP58's lane ran while that file was untracked, and the checker reads tracked files only), and the cited line had drifted from 857 to 905. Validated: actor_monolith lib 1,239 (6 fewer: the deleted module's tests), ambition_characters 448, runtime, app_it 789, pytest lane 1,581. |
| AP60 | A body is built without its orientation, and a system repairs it every tick | ✅ DONE 2026-09-26 (e23071c78) | From the AP34 residue ("all 242 rebuilt bodies lack `ActorRoll`+`SurfaceUpright` ... on the rebuild tick"). `ensure_actor_roll` scanned every `BodyKinematics` body without `ProjectileGameplay` each tick and inserted `ActorRoll` and `SurfaceUpright` on those that lacked them: a construction repair. Measured before choosing the home, over the 71 runtime rooms (the sand room excluded) at tick 12 with a scratch census: every non-projectile `BodyKinematics` body carries `ResolvedMotionFrame` (240 actors, 71 players, 11 bosses; no other group), and projectiles carry none, because the frame comes with the ancillary movement bundle that players, actors and bosses are built with. So `ResolvedMotionFrame` now `#[require]`s both: a body that has a movement frame can be reoriented, and it is built with the orientation it rights itself with. The same census after the change reads the same three groups, all with both components. Deleted: `ensure_actor_roll` and its schedule entry, and portal2d's `TODO(compat-remove)` block that re-exported it with `update_actor_roll`, `ActorRoll` and `portal_transform_velocity` (its one remaining caller, `ambition_projectiles::portal_transit`, now names the owning crate). ⚠ Outside the census: twintrack's relativity characters are `BodyKinematics` bodies with no movement frame, so they no longer get a roll. Nothing reorients them (no portal, no gravity zone), and a missing roll renders as an unrotated body, which is what a zero roll drew. Guard: `a_replayed_room_is_built_whole::a_rebuilt_body_is_complete_before_anything_decides_for_it` gained a roll arm (the replay rebuilds the Hall's brained bodies); poisoned by deleting the `require`: 3,014 bodies reached the decision pass with no `ActorRoll`. Validation: targeted crates (shared_tangle, portal2d, projectiles, monolith 1,239, twintrack) and `ambition_app --test app_it` 789 passed / 0 failed. The `ambition_content` lib test did not compile on HEAD: AP54 (823fa1a56) added `EntityContracts::takes` and `reload_tests.rs` did not name it; fixed here. Its one other failure (`every_row_a_pin_names_is_on_the_sheet_that_draws_it`) is a stale local generated sheet that predates a3e7f9d6c's art, not this change. |
| AP61 | Four tag passes attach a dormancy stance on first sight, and a hosted game cannot state its own | ✅ DONE 2026-09-26 (9140639f9) | From the AP34 residue (rebuilt bodies lack `DormancyPolicy` on the rebuild tick). The stance was attached per body, after spawn, by four first-sight passes: `declare_ambition_dormancy` (content), `tag_sanic_badniks`, and the dormancy halves of Mary-O's `tag_mary_o_ai_slop` and `tag_mary_o_snakes`. Each game used ONE radius (Ambition 2560, Sanic 4800, Mary-O 720, which the snake already took from the slop), and each pass restated a classification: Ambition's `stance_for` (only an `Enemy` sleeps; an encounter mob, a mount and a limb never do; the duel pair by feature id, though both are `Npc`s and needed nothing), and the demos' brain-key matches. ⭐ The obvious home, one global resource, is the defect ADR 0033 already recorded for death rules: Ambition hosts Mary-O and Sanic, the type is the key, and the last plugin built wins. So the scoped declaration became general: `ambition_combat::scoped_rules` holds `RulesScope` (was `DeathRulesScope`), `DeclaredRules<T>` (was `DeclaredDeathRules`) and `App::declare_rules` (was `declare_death_rules`), and the monolith's `session::governing_rules::GoverningRules<T>` resolves the active room's answer (was `GoverningDeathRules`). Death rules and the new `DormancyRule { hostile_wake_radius }` are its two users; the next per-game rule is one more `T`. The one authority: a game declares `DormancyRule` for its rooms, and `assess_dormancy` derives each body's radius every tick from that rule plus facts the body carries (`dormancy::wake_radius`: a free `Enemy` sleeps; boss, NPC, player, prop, encounter mob, mount and limb never do). Deleted: the `DormancyPolicy` component and its rollback row, all four passes, Ambition's `stance_for` and duel exception, `SNAKE_WAKE_RADIUS`, and the three death-specific scope names. `EncounterMob` is now rollback-registered (`encounter.mob`), because the rule reads its presence and the policy that latched it is gone (its presence-filter exemption is deleted). Schema v235 -> v236. What a game says now: one `declare_rules(scope, DormancyRule { .. })` beside its death rules, no system. Guards: `a_game_governs_only_its_own_rooms::each_game_sleeps_its_hostiles_at_its_own_distance_in_its_own_rooms` (composed host: untagged 2560, sanic 4800, mary_o 720, smash none); `content_dormancy` moves every brained body in four Ambition rooms far away for one tick and reads `Dormant` against Ambition's intent; Mary-O `every_authored_enemy_sleeps_when_she_is_far`; Sanic `every_authored_badnik_sleeps_under_sanics_rule`; monolith dormancy units for each non-sleeping class; `scoped_rules` units. Poisoned, each by its own assertion: an encounter mob allowed to sleep -> the monolith unit ("an encounter mob fell asleep"); Ambition's declaration removed -> `content_dormancy` (basement hostiles awake) and the composed-host test (untagged rooms read `None`); Mary-O's removed -> her test's `expect`; Sanic declaring for every room even when hosted -> the composed-host smash arm (4800 where `None` was due). Validation: combat 672, monolith 1,240, content 454, Mary-O 161, Sanic 89, Mary-O app 55, Sanic app 35, `ambition_app --test app_it` 790 / 0 failed, pytest lane 1,581, and the presence-filter, multi-writer and mutator ratchets. The first app_it run failed `every_presence_only_probe_is_named_with_its_reason`: `encounter.mob` was presence-only, so it is value-probed (its encounter id). |
| AP62 | Three passes opt bodies into portal transit on first sight, and two games reconcile one transit | ✅ DONE 2026-09-26 (341d2033f) | From the AP34 residue (rebuilt bodies lack `PortalBody`+`PortalPolicy` on the rebuild tick). The portal core transited only bodies tagged `PortalBody` with a `PortalPolicy`, and three passes tagged them after spawn: Ambition's `ensure_portal_bodies` (primary player: reorient+carry; boss: neither; other bodies: carry) and `ensure_projectile_portal_bodies` (carry), and Sanic's `enable_sanic_portal_bodies` (primary: reorient; all: carry). In a hosted Sanic room both games' passes ran, unordered, so which classification a body got depended on the executor. Measured before choosing, over 71 Ambition rooms at tick 60 (a scratch census) and standalone Sanic over 600 ticks: every `BodyKinematics` body is a mover or a projectile, and every one was tagged, so the population is every body. The policy was a function of identity in both games. ⭐ A second duplicate turned up beside it: Ambition's `reconcile_kernel_bodies_after_portal_transit` and Sanic's `reconcile_sanic_portal_transit` were the same function (`ae::movement::reconcile_transit` on every transited kernel body), and both ran in a hosted Sanic room. The content doc said the core "cannot reconcile kernel body state"; `ambition_portal2d` depends on the core crate that owns `reconcile_transit`, so it can. The one authority: `portal_transit` drives every body and derives how each takes part (every body carries its momentum; a body in the player population, `PlayerEntity`, reorients on a same-wall turn-around, because its facing follows its seat's input), and the core's `reconcile_transited_bodies` completes the transit, chained after it. Decided per the autonomous-decision rule, and recorded here rather than asked: a boss now carries its momentum through a portal (it did not). No test or doc stated why, a boss rarely fits an aperture, and a special case keyed on `BossConfig` is the identity mapping this slice removes; and "reorients" widens from the primary player to every player body, which is the same set in every shipped room and the correct one for a second couch seat. Deleted: `PortalBody`, `PortalPolicy`, their rollback rows (schema v237), the three passes, both reconcilers, and every fixture's hand-tag. What a game says now: nothing, since installing the portal plugin is the opt-in. Guard: `portal::tests::only_a_player_body_turns_around_through_a_same_wall_pair` (two untagged bodies; poisoned by letting every body reorient: "only a body in the player population turns around (player: false)", 1.0 vs -1.0). The existing transit tests (`portals_teleport_a_fitting_actor_and_skip_an_oversized_one`, the projectile tests, and the two app_it portal tests) now spawn untagged bodies, so each witnesses that no tag is needed. Validation: portal2d 71, content 455 (default and `portal`), Sanic 89, projectiles 46, monolith 1,240, Mary-O app 55, Sanic app 35, `ambition_app --test app_it` 790 / 0 failed, pytest lane 1,581, the presence-filter / multi-writer / mutator ratchets, and `examples/portal_tutorial` (its own workspace) checks. |
| AP63 | A catalog row cannot state a whole creature, so Sanic's cast is a Rust string plus two registration functions (AP58 step 2, Sanic half) | ✅ DONE 2026-09-26 (4835960ae) | Before: `SANIC_CATALOG_RON` (a catalog in a Rust string, read by the legacy `from_ron`), a loop that registered both forms again with a sheet, a sheet-scaled body (`sanic_world_per_pixel`, from a Rust height constant) and a voice, and `register_badnik_character`, which restated the badnik's sheet, a sheet-scaled body at three fifths of Sanic, its gait, momentum, contact damage, policy and health. The row could not say a sheet-scaled body, a gait, contact damage or a policy. The row vocabulary now can, and each field has one home: `posed_body: Some(OwnHeight | SameAs("id"))` (the sheet authors the body per pose, at the scale where the named row's idle body stands its `standing_height` tall; `SameAs` lets a character's forms share one scale, so their sizes differ only where their art does), `locomotion`, `contact_damage` and `autonomous_profile` (inline). Preparation folds the last three under a registered definition's own, like `max_health`. `EmbeddedPack::cast(..).register` asks the baked sheet for the scale once (`world_per_pixel_for_standing_height`, the one engine helper, beside `posed_body_geometry`) and builds the body with it; a scale that names a row outside the catalog, or a row with no sheet or height, is a refusal. The voice was a second home for a row's `fallback_dialogue` (the catalog answers every situation before a definition's voice), so it moved into the rows. Deleted: `SANIC_CATALOG_RON`, the registration loop, `register_badnik_character`, `sanic_world_per_pixel`, `world_per_pixel_for_height` (demo copy), `SANIC_STANDING_HEIGHT` and `BADNIK_STANDING_HEIGHT` (the rows state 48 and 28.8): 302 lines of Rust for a 174-line data file that also carries every former comment. What a game says now: a Sanic character, player form or enemy, is a row in `assets/data/character_catalog.ron` (plus an optional move file), and `install_sanic_content` states its cast in one call. Guard: `sanic_it::the_cast_is_its_pack::the_sanic_cast_is_prepared_from_its_pack_as_the_rust_registration_built_it` compares each prepared character with the values the deleted Rust stated (both forms stand 48 at one scale; the badnik stands 28.8 with its gait, momentum, contact damage, policy and health). Poisoned three ways, each caught by its own assertion: the super form on its own scale ("share one scale"), no contact-damage fold (the contact arm, `None`), and the badnik row without `posed_body` ("must be a body its sheet authors per pose"). NEXT: Mary-O (AP64): three forms `SameAs` her small form, four enemies as rows, and her `$CLASSIC_AXIS_TUNING` template substitution. Validation: characters 448, sprite_sheet 77, character_sprites 55, facade 10, content 455, Sanic 89, Smash 267, Mary-O app 55, Sanic app 36, Smash app 18 + 51, `ambition_app --test app_it` 790 / 0 failed, pytest lane 1,581. |
| AP64 | Mary-O's cast is a RON template with a feel spliced in three times, a registration loop and three enemy functions (AP58 step 2, Mary-O half) | ✅ DONE 2026-09-26 (0b5b89acb) | Before: `MARY_O_CATALOG_RON_TEMPLATE`, a catalog in a Rust string with `$CLASSIC_AXIS_TUNING` substituted into each of her three forms by `str::replace` (`MARY_O_CLASSIC_AXIS_TUNING`), read by the legacy `from_ron`; a loop that registered the forms again with a sheet, a record-only `with_canonical_height`, a sheet-scaled body and a voice; and `register_solid_snake_character`, `register_ai_slop_character` (through `register_mary_o_enemy_character`) and `register_snakes_on_a_plane_characters`, which restated each enemy's sheet, body scale, gait, contact damage, policy and health. The body scales had four derivations in Rust: `mary_o_world_per_pixel` (height 32 over her idle pixels), `snake_world_per_pixel` (as wide as `mary_o_body_width`, headless fallback 0.35), `ai_slop_world_per_pixel` (`AI_SLOP_BODY_WIDTH` 28 over its idle pixels) and `form_world_per_pixel`, which ignored its argument. One authority now: the pack's `data/character_catalog.ron`. The row vocabulary gained what Mary-O needed and nothing demo-specific: `PosedBodyScale::Width(w)` (a creature whose width is what a level cares about) and `AsWideAs("id")` (a width derived from another character, so it moves when that character does), beside AP63's `OwnHeight` and `SameAs`; and a catalog-level `axis_tuning_presets` map with a row field `axis_tuning_preset`, so the forms of one character name one feel instead of carrying copies. Presets are namespaced per provider at assembly like the other preset maps; the validator and the pack's content schema (new identity kind `axis_tuning_preset`, a local reference) refuse an unknown preset and a row that states both a preset and its own tuning. `CharacterCatalog::axis_tuning` resolves the preset, so preparation reads one answer. The scale resolver is one engine function, `ambition_character_sprites::posed_body_world_per_pixel(catalog, id)`, which follows `SameAs` chains (a cycle is a refusal) and replaces AP63's `world_per_pixel_for_standing_height` and the facade's private resolver. `PackCast::register` asks it once per row, and `EmbeddedPack::posed_body_world_per_pixel(id)` asks the same function for anything else that must agree (Mary-O's level tests, `ai_slop_half_size`, the enemy-quad test). Deleted: the template and its splice, `mary_o_catalog_ron`, the registration loop, the three enemy functions, `snake_world_per_pixel`, `snake_body_width`, `mary_o_body_width`, `ai_slop_world_per_pixel`, `AI_SLOP_BODY_WIDTH`, `form_world_per_pixel`, `form_height`, `MARY_O_STANDING_HEIGHT`, `SMALL_FORM_HEIGHT`, `GROWN_FORM_HEIGHT` (it reached only the record-only canonical height) and `with_canonical_height` on her forms. The remaining scale helpers in `powerups.rs` are test-only and ask the pack. Mary-O's `src/` is 722 lines shorter and 102 longer, for a 269-line data file that keeps every former comment that says why. The four stomp fixtures in `power_loop.rs` register the cast through the production call (`register_mary_o_cast`) instead of one enemy function. Decided (autonomous-decision-making): the headless fallback for every posed body is the resolver's 1.0 (the snake's was 0.35); `the_snake_is_sized_by_its_art_and_not_by_the_no_sheet_fallback` now pins that the 1.0 fallback does not fire. What a game says now: a Mary-O character, player form or enemy, is a row in `assets/data/character_catalog.ron` (plus an optional move file), and her forms share a feel by naming it. Guard: `mary_o_it::the_cast_is_its_pack::the_mary_o_cast_is_prepared_from_its_pack_as_the_rust_registration_built_it` compares each prepared character with the values the deleted Rust stated (small form stands 32; all forms one scale and one feel, jump 450, gravity 2250; snake as wide as she is; slop 28 wide; each enemy's gait, contact damage, policy and health; the planes' free flight). Poisoned: the snake row at `Width(28)` fails the snake-width arm; the fire form without its preset fails the feel arm (`mary_o_fire`, `None`); and `ambition_characters::…::registry::tests::a_shared_feel_is_the_providers_own_and_is_stated_once` fails (700 for 400) when assembly does not namespace the feels. Two guards spoke on the first lane run, and both were right. `test_hall_dialogue_ids_have_nodes` read pedestal ids from Ambition's catalog and from Rust only, so the plane swarms' hall conversations looked orphaned once their rows left Rust; it now also reads each demo pack's `character_catalog.ron`. The absence contract `the-catalog-axis-tuning-is-confined-to-one-file` refused the feel test's `axis_tuning(` call in `registry.rs`, so the test lives beside the accessor in `character_catalog/mod.rs`. Validation: characters 449 (486 with `content_pack`), sprite_sheet 77, character_sprites 55, facade 10, content 455, Sanic 89, Smash 267, Mary-O 161 + 11, Mary-O app 56, Sanic app 36, Smash app 18 + 51, `ambition_app --test app_it` 790 / 0 failed, pytest lane 1,579 passed with the two guards above red, then both files green after the fixes (192 passed). |
| AP65 | TwinTrack and Pocket state their casts as a Rust string plus a registration block (AP58 step 2, the last two demos) | ✅ DONE 2026-09-26 (5d8482d2a) | Before: `TWINTRACK_CATALOG_RON` and `POCKET_CATALOG_RON`, catalogs in Rust strings read by the legacy `from_ron`, each followed by `register_character` calls that restated every name and sheet. TwinTrack wrote its plaza flight feel out twice, once per participant, and gave Emmy her gait (540, float, free flight) and a stand-still policy in Rust; both demos gave each character a voice in Rust. One authority now: each demo's pack (`assets/pack.ron`, `assets/data/character_catalog.ron`, `src/pack.rs`, the facade's `content_pack` feature on). TwinTrack's rows name one `axis_tuning_presets` entry (`plaza_flight`, AP64's vocabulary), Emmy's row states her locomotion and policy, and the voices are the rows' `fallback_dialogue`. `install_twintrack_content` and `install_pocket_content` each register their cast with one `PACK.cast(..).register(app)`. Deleted: both Rust catalogs and both registration blocks (TwinTrack's `lib.rs` 154 lines out and 6 in, Pocket's 73 out and 8 in; the two data files are 105 and 38 lines, comments included). With AP58 step 1, AP63 and AP64, every demo's cast is now its pack; the remaining `from_ron` readers are the host's versus roster (`versus.rs`, which also generates each duelist's moves in Rust from `DuelistNumbers`), the RON-game builder road (`CharacterContent::Ron`, a public authoring API) and fixtures. What a game says now: a TwinTrack or Pocket character is a row, and two characters share a feel by naming it. Guards: `twintrack_it::the_twintrack_cast_is_prepared_from_its_pack_as_the_rust_registration_built_it` (sheets, one flight feel at `INVARIANT_SPEED` and `TARGET_SPEED`, Emmy's gait and stand-still policy, health). Poisoned: Emmy without the preset fails the feel arm (`None`); Emmy without her policy fails the policy arm. Pocket: `the_pocket_cast_registers_the_runner_with_its_sheet` pins that the runner is prepared with its sheet (the launcher's old blue box). ⚠ ITS POISON PASSED, and that is a finding, not a gap: skipping the runner in `PackCast::register` leaves it prepared, because preparation publishes every catalog row as a character (`cast_with_catalog_rows`). So the old comment in `install_pocket_content` ("a catalog row with no registration drew a plain blue box") described a road that no longer exists, and is gone; the cast's registration adds something only for a row with a body scale or moves. Also a hollow check caught before it landed: I first added Pocket to `registered_character_art_resolves`'s list of starting characters, because its comment said Pocket joins the shell-hosted composition. The poison run failed there and I read that as the arm working; the clean app_it run failed the same way. Pocket is in no shipped composition (only `gameplay_presentation_profiles.rs` adds its plugin), so the arm is removed and the comment now says so. Validation: Pocket 2, TwinTrack 24, TwinTrack app 25 + 1 ignored, `registered_character_art_resolves` 3, app_it 789 + the one hollow arm above (fixed and re-run), pytest lane 1,581, MODULES.md regenerated (two new `pack` modules). |
| AP66 | Fourteen Ambition creatures are a Rust file each that states only what their catalog row could say (AP58 step 3, first cut) | ✅ DONE 2026-09-26 (bc9c18889) | Before: `game/ambition_content/src/authored/` had 36 files, one per creature, each an `author` function on a `CharacterDefinition` listed in `AUTHORED_CAST`, and that list was what made a character buildable. Fourteen of them stated only a gait, health, and for three a policy, contact damage or one trait: `director`, `medic`, `npc_ai_slop`, `npc_alice`, `npc_bob`, `npc_kernel_guide`, `npc_ninja_shadow_oni_leader`, `npc_oiler`, `performer`, `pointed_polygon`, `pugnacious_polygon`, `special_patent_clerk`, `sandbag` (`as_practice_target`) and `npc_emmy_noether` (`preserving_mirror_symmetry`). Five shared one Rust constant, `HUMANOID_RUN_SPEED` in `hall_humanoids.rs`. Each creature was stated in two places, its row and its file, and the reasons for its numbers lived in the file. One authority now: the row. The row vocabulary gained `locomotion_preset` with a catalog-level `locomotion_presets` map (AP64's preset pattern: namespaced per provider at assembly, refused when unknown or stated beside `locomotion` by the validator and the pack schema, which gains identity kind `locomotion_preset`, resolved by `CharacterCatalog::locomotion`, which preparation reads), and the traits `practice_target` and `preserves_mirror_symmetry`, which preparation takes from either the row or a registered definition. Ambition's build rule is now stated once: a row that states how its body moves is a character the game builds (`buildable_only_cast` = the `authored/` files plus `rows_with_a_body`). The hall walk is `locomotion_presets.hall_walk`. Every reason the files gave moved into the rows as comments (Emmy's mirror-match note, the medic's and performer's speeds against their archetype, the kernel guide's ruling, the patent clerk's heavyweight). Deleted: the fourteen files and `hall_humanoids.rs` (618 lines) and their `AUTHORED_CAST` entries and orphaned comments; Emmy's two file tests (their claim is in the new test). 21 files remain in `authored/`, each needing a fact the row cannot state yet: a mount (shark, gnu, admiral, raider), death traits (mites, shark, infinite sandbag), a named policy (goblin, lab raider, via `with_autonomous_profile_named`), an action set, a weapon (officer, projectile polygon), abilities, a dream seed, a held item, a sprite-authored body with its own sheet (gnu, hands). Stale citations fixed: Emmy's row prose named the deleted file, `ambition_demo_smash/src/select.rs` named the clerk's, and `docs/recipes/adding-a-character.md` now says a character is mostly its row. MEASURED, not assumed: a temporary census printed every prepared Ambition character whole (`{:#?}` of `PreparedCharacterDefinition`, 133 characters, 182,967 lines) before and after. The two are byte-identical. The census was checked by moving the hall walk to 211, which changed exactly five characters (the five hall humanoids). What a game says now: an Ambition creature with a gait, health, a policy, contact damage or a trait is a row and nothing else; five humanoids share a walk by naming it. Guard: `character_catalog::tests::a_row_that_states_its_body_is_built_with_that_body` checks, for every row that states a body (at least 14), that it is prepared with its row's gait and health and, where the pack has its moves, that it was built and so wears them; plus the hall walk, the sandbag's trait, Emmy's trait and the admiral as the control. Poisoned four ways, each caught by its own assertion: rows not buildable ("`director` ... fights with no moves"), no `practice_target` fold, no mirror fold, the accessor ignoring the preset ("the hall walk", `None`). `a_shared_gait_is_refused_when_unknown_or_stated_twice` covers the validator. Three existing guards asked the deleted `author` functions (`authored_intrinsics`) what a character authors, and failed on the first lane run: `every_build_only_id_authors_something`, `the_sandbag_authors_the_dummy_its_archetype_row_used_to` and `every_migrated_character_has_an_autonomous_default_something_can_reach`. They now ask the PREPARED character (definition folded with row, `prepared_cast_app`), which is the authority; each was poisoned through its row and spoke (the sandbag without `practice_target`, the director without health: "authors NOTHING", the sandbag without a policy: stranded `["sandbag"]`). Validation: characters 487 (with `content_pack`), content 454 + 32, Mary-O 161, Sanic 89, Smash 267, Smash app 18 + 51, `ambition_app --test app_it` 790 / 0 failed, pytest lane 1,581, MODULES.md regenerated. |
| AP67 | Ten more Ambition creatures are Rust files, and nine of their rows name an action set the file overrules (AP58 step 3, second cut) | ✅ DONE 2026-09-26 | Before: `goblin`, `npc_lab_raider`, `npc_carl_stargan`, `npc_dividing_mite`, `npc_exploding_mite`, `npc_goblin_brute`, `npc_pirate_crew` (six pirates, one `match` on the id), `npc_salvage_guard`, `stochastic_parrot` and `sandbag_infinite` stated, in Rust, death traits, a named policy (`with_autonomous_profile_named`) and an inline `ActionSet`, which the row could not say. ⚠ AND THE ROWS DISAGREED WITH THEM: every row must name a `default_action_set`, and preparation takes a definition's own action set over it, so eleven rows named a preset that nothing used: `striker_swipe` on Carl, both mites, the brute and two pirates (the Rust swings were 0.30/30, 0.30/0.10/30, 0.22/26, a 2-damage 44-reach hammer, a 36-reach cutlass), `brute_lunge` on the two heavy pirates (the Rust was a swipe), `ranger_arrow` on the lookout and navigator (the Rust was a 500/1 bolt, not a 520/2 arrow, and a walk, not a strafe), and `peaceful` on the parrot (the Rust bit). The salvage guard's Rust set was `striker_swipe` exactly: a pure copy. Two authorities per creature, and the row's was dead. One authority now: the row. The row vocabulary gained `death_traits` (`CharacterDeathTraits` is now deserializable, every field defaulted, unknown fields refused) and `named_autonomous_profile` (a name in `autonomous_profiles`, namespaced at assembly like the presets, refused when unknown or stated beside an inline `autonomous_profile`, by the validator and by the pack schema), both folded at preparation under a registered definition's own. Each row now names the preset that holds its real action set: eight new presets (`carl_swipe`, `dividing_mite_swipe`, `exploding_mite_swipe`, `brute_hammer`, `pirate_cutlass`, `pirate_heavy_cutlass`, `pirate_bolt`, `parrot_bite`) and three shared pirate policies (`pirate_striker`, `pirate_brute`, `pirate_ranger`), each shape worn by two pirates. Deleted: the ten files (551 lines, 43 in `mod.rs`); `authored/` now holds 11 files for 11 reasons: mounts and mass (shark, gnu, admiral, raider), a sprite body with its own sheet (gnu, hands), abilities and a dream seed (puppy slug, companion dog, PCA), a held item (raider), and weapons (officer, projectile polygon). MEASURED: the whole-struct census of all 133 prepared characters is byte-identical before and after, so every swing, policy, death and gait is what the Rust stated. A finding on the way: with AP67 every buildable Ambition character states its own body (44 of 44), so `the_body_complete_cast_only_grows` (a floor of 19 plus a control that asserted the migration was NOT finished) is now `every_buildable_character_states_its_body`, with the vault keeper (a row with no body) as the control. What a game says now: an Ambition enemy that explodes, divides, names a shared policy or swings its own swing is a row; a crew of six in three shapes is six rows naming three policies and three presets. Guards: `a_row_that_states_its_body_is_built_with_that_body` now checks every body row's death traits, policy (inline or named) and action set against its row (29 rows), plus the lookout's bolt and the brute's hammer, which the old dead presets would change. Six guards that asked the deleted files now ask the prepared character (the goblin's named policy, the mites' death and health, the parrot's body, the body-complete census). Poisoned: no death fold (the mites, `npc_dividing_mite`), no named-policy fold (`goblin`, and the stranded eight: goblin, lab raider, six pirates), the lookout back on `ranger_arrow` ("shoots the bolt its crew file stated"). ⚠ The named-policy poison first did not compile (a guarded match arm left the match non-exhaustive) and printed nothing; it was redone. `a_named_policy_is_refused_when_unknown_or_stated_beside_its_own` covers the validator. Two more guards spoke on the first lane run, and both were right. `aerial_authoring::shipped_characters_state_their_flight_answer_explicitly` asked the deleted files for the authored three-state flight answer; it now reads the definition's gait, else the row's (poisoned: the parrot's row without `baseline_free_flight` fails "the parrot flies and must say so itself"). The absence contract `the-catalog-default-action-set-is-confined-to-one-file` refused my test's `build_default_action_set` call; the test lowers the row's own preset with `action_set_from_preset` instead (poisoned: preparation ignoring the row's preset fails "`director` fights with a different action set"). Process note: I started AP68's edit while this lane compiled; the lane was stopped, AP68 set aside and the lane rerun on AP67 alone. Validation: characters 488 (with `content_pack`), content 454 + 32, Mary-O 161, Sanic 89, Smash 267, Smash app 18 + 51, `ambition_app --test app_it` 790 / 0 failed, pytest lane 1,580 + the contract failure above, its file then 189 green. Commit 7431f4106. |
| AP68 | A scripted boss's beat runs on two clocks, so a phase change eats the next move's windup | ✅ DONE 2026-09-26 | Reported by ToothbrushAmbition from `fight_discovery` on GNU-ton: Enrage's first `orbit` rested 0.017 s of its authored 0.6, and on aggressor seeds it struck with no telegraph. Before: two clocks answered "where is the boss in its beat". The pattern's step timer (`BossPatternState::step_elapsed`) ran every tick from the moment `tick_boss_pattern` reset the cursor on a phase change. The body's committed move ran to completion on its own playback, and the trigger starts no move while one plays. So a strike committed in phase 1 kept playing while phase 2's first Telegraph step counted down: the pattern led the body by what was left of that strike, the new move's rest was eaten, and when the lead passed the telegraph the move struck with no windup. One clock now: the body's. In `advance_scripted`, a Telegraph step's clock does not run while the body plays a DIFFERENT move (`ctx.live_attack`, the projected `LiveBossAttack`), so the step starts when the body is free and every authored length is played. Nothing was deleted; the fix removes the lead, not a system. Scope, stated: a committed strike of the SAME profile as the next telegraph is not told apart from the telegraph's own move, so in that case the clock still runs during the old strike (not measured on the shipped patterns; the peer's rerun will show it if it happens). Guard: `pattern::tests::a_new_phase_waits_for_a_committed_strike_before_its_first_telegraph` (phase 2 = Telegraph `orbit` 0.8, Strike 0.4, Rest 0.6; the old strike stays committed for 1.0 s, longer than the telegraph; asserts the telegraph shows for 0.8 s and the rest lasts 0.6 s, each within one tick). Poisoned (the clock runs unconditionally, as before): "the first telegraph showed for 0 s of its authored 0.8", the peer's symptom. Validation: `ambition_boss_encounter` 198, `ambition_content --lib` 454, `ambition_app --test app_it` 790 / 0 failed, pytest lane 1,581. MEASURED after (ToothbrushAmbition, `fight_discovery`, 3 policies x 2 seeds, 54f6a502c against the pre-AP68 run), Enrage's first `orbit` (authored telegraph 0.8, rest 0.6): aggressor seeds 0 and 1, telegraph 0.00 -> 0.80 and rest 0.017 -> 0.600 (it no longer strikes with no warning); random seed 0, rest 0.017 -> 0.600; random seed 1 (phase entered between moves, the control) 0.82 / 0.600 before and after. The finding "orbit: no punish window follows it" is gone; orbit's median rest over 5 whole beats is 0.60. Residual, present before and after, filed as its own row: the pattern learns the phase one tick after the encounter. What a game says now: a boss pattern's authored telegraph and rest lengths are what the player sees, whatever the phase change interrupted. |
| AP69 | Seven more Ambition creatures are Rust files, because a row cannot say mount, mass, dream seed, held item or provoked policy (AP58 step 3, third cut) | ✅ DONE 2026-09-26 | Before: `npc_puppy_slug`, `npc_companion_dog`, `npc_burning_flying_shark`, `npc_giant_gnu`, `npc_giant_gnu_hands`, `npc_pirate_admiral` and `npc_pirate_raider` (two ids, one `id ==` branch) were files under `authored/`, because their facts had no row field: a mount (`with_mount`), a mass (`vitals.mass`), a dream seed, a held item, two single verbs (a slug that only walks, a dog that walks and hops), and a sprite body at a stated scale (`with_sprite_authored_body(1.3)`). Two more copies of facts stood beside them. (1) THE PROVOKED POLICY WAS A NAME RULE: `authored_intrinsics` gave every id that starts with `npc_pirate_` a provoked policy, and chose the heavy one by `id.contains("pirate_heavy")`, so the fact about a creature was a fact about its spelling. (2) THE GIANT'S SCALE WAS STATED TWICE: `authored::npc_giant_gnu::GNU_WORLD_PER_PIXEL` built the body and a private `GNU_WORLD_PER_PIXEL` in `bosses::gnu_ton::conductor` placed the back the player stands on, both 1.3, with nothing to keep them equal. And two rows named dead presets again: the shark's `peaceful_float` and the raiders' `striker_swipe` / `brute_lunge` were overruled by their files' bite and bolts. One authority now: the row. The row vocabulary gained `mass`, `mount` (`CharacterMount` was already deserializable), `dream_seed`, `held_item` and `provoked_profile` (a name in `autonomous_profiles`, namespaced at assembly and refused when unknown by the validator and the pack schema), all folded at preparation under a registered definition's own; `PosedBodyScale::WorldPerPixel` (the one scale that does not ask the art, so it holds without a baked sheet); and the grants `Walk` and `Jump`. Ambition's cast registration (`register_declared_cast`) now reads a row's posed body scale, as every pack's cast (`EmbeddedPack::cast`) already did. The fists name the giant's scale (`SameAs("npc_giant_gnu")`), not the number. The conductor reads the giant body's own `SpritePosedBody::world_per_pixel` for `back_platform`, which has three readers since the peer's reflex buck (the overlay block, `buck_throw` and the reflex's standing check). Each row names the preset that holds its real action set (new: `shark_bite`, `raider_gun_sword`, `heavy_raider_gun_sword`). Deleted: the seven files, the pirate name rule, the conductor's copy of the scale, and seven per-creature tests that asked `authored_intrinsics` (which no longer states these creatures) and pinned numbers the row now holds. `authored/` now holds 3 files: the officer's pistol, the polygon's charged cannon, and the cellular automaton's kit and provoked policy. The prepared-override docs that said "no catalog counterpart" for death traits, abilities, locomotion and contact damage (all folded now) now say what folds. MEASURED: the whole-struct census of all 133 prepared characters is byte-identical before and after (`cmp`), so every mount, mass, seed, item, verb, scale and policy is what the Rust stated. Guards: `a_row_that_states_its_body_is_built_with_that_body` now compares, for every prepared row, mass, mount, dream seed, held item, ability grants, contact damage, provoked policy and posed-body scale against the row (34 facts, floor 34), plus the fists' 1.3, the slug's one verb and Iron Mary's heavy boarder. `the_mounts_and_their_riders_are_what_their_rows_say` and `every_pirate_is_provoked_into_the_boarder_its_row_names` (every `npc_pirate_` row through preparation, registered, with Alice as the control) replace the seven. `a_provoked_policy_is_refused_when_unknown_and_namespaced_when_known` covers the validator and the namespacing. Poisoned, each with the assertion that spoke: no mass fold ("`npc_burning_flying_shark` weighs what its row says"), no mount fold ("`npc_burning_flying_shark` rides as its row says", and the riders' `[] != ["shark"]`), no dream-seed fold ("the slug's own psychedelic pass"), no held-item fold (the riders' `None != Some("gun_sword")`), no provoked fold ("`npc_pirate_admiral` when struck": `ambition::provoked_combatant` for `ambition::pirate_boarder`), and the registration ignoring the posed body ("`npc_giant_gnu` is built at its row's scale"). Validation: characters 489 (with `content_pack`), character sprites 55, platformer core 589, content 449, Mary-O 161, Sanic 89, Smash 267, `ambition_app --test app_it` 791 / 0 failed (`gnu_ton_fight` included), pytest lane 1,581, absence contracts 50 of 50, MODULES.md current. A peer merge (54f6a502c) had added two `back_platform` callers in the conductor; they take the same scale. What a game says now: a mount, a rider, a giant and its fists, a crawling slug with one verb, a gun-sword crew and whom they become when struck are rows; a boss's arena geometry reads the scale its body was built at. Commit 8da7fbe42. |
| AP70 | A boss decides its move before the tick's damage ends its phase, so a new phase opens with a one-tick ghost of the old phase's move | OPEN (2026-09-26, reported by ToothbrushAmbition after AP68) | Measured (`fight_discovery`, aggressor seed 0, GNU-ton, beats 2-4 of `target/fight_discovery_ap68/aggressor_seed0.json`), present before and after AP68: Phase 1 `demonstrate` is cut in its rest at about 13.08; Phase 2 opens with `stomp` telegraph 0.017, strike 0, at t = 13.083, then `fluxions` at 13.117. Content-visible: GNU-ton's conductor latches a new beat for that tick (it flips which fist leads) and the gnu's stomp pin shows for one frame. The peer's trace (reasoned from the registrations, NOT checked against the built schedule graph): the reader `ambition_boss_encounter::ecs::tick_boss_brains_system` builds `BossPatternContext.encounter_phase` from `BossEncounter.encounter.phase` in `Monolith::WorldPrep`; `trigger_boss_attack_moves` starts the move in `CombatSet::Trigger`; the writer `update_boss_encounters` (`ActorPhaseState::tick` -> `enter()`) runs in `ProgressionSet::BossAdvance`, in `Monolith::Progression`. So on the flip tick every read agrees with the start of the tick, and the phase ends from damage that lands after the decision. If that holds, an ordering edge does not remove the ghost. The peer's proposed fix: the phase change cancels a boss move still in its WINDUP at the write (on `PhaseChanged`), since the next tick's reset cancels it anyway; a committed strike keeps running (AP68). Next: ask the schedule for the order, then decide. |
| AP71 | The last three Ambition creatures are Rust files, because a row cannot say a weapon (AP58 step 3, last cut); `authored/` is gone | ✅ DONE 2026-09-26 | Before: `officer` (a pistol with a hand muzzle, a half-plane aim, its own round art and no recharge), `projectile_polygon` (a bolt with a straight flight, a five-tier charge and a cannon muzzle, plus a shot look) and the two cellular automatons (one `author` fn for both ids: a swipe and a glider, eight verbs from `AbilitySet::basic()` plus five, a provoked policy and a shot look) were files under `authored/`. The action-set preset vocabulary could say a shot's style, speed and damage and nothing else, and a row could not say a shot's look or verbs like blink or flight. And their rows named dead presets once more: the officer's `striker_swipe`, the polygon's `ranger_arrow` and both automatons' `striker_swipe` were overruled by the files. One authority now: the row and the preset it names. `RangedPreset` gains `Weapon(RangedActionSpec)`, the whole spec (flight, charge, muzzle, aim, visual, refire), beside the four short forms; the row gains `ranged_vfx` (the look of a shot whose own spec states none), folded at preparation; the grants gain `Interact`, `Reset`, `Blink`, `Flight` (fly + its toggle) and `Dash`. New presets: `officer_sidearm`, `polygon_cannon`, `automaton_swipe_and_glider`. The two automatons each state the kit (about 15 lines), where one Rust function served both ids: a row per character is the vocabulary's unit, and the census holds them equal today. With no file left, `AUTHORED_CAST`, `author_for`, `authored_ids` and the `authored` module are deleted; `buildable_only_cast` is the rows that state a body; and `authored_intrinsics`, which only attached the pack's move table now, is renamed `with_pack_moveset` for what it does (6 callers). Two older guards had gone hollow on the way, because they asked the Rust definition: `the_cast_that_states_its_own_verbs_only_grows` would have found no character, and `a_character_states_its_policy_in_one_place` passed over an empty set. They now ask the prepared character and the row (the second with a floor of 25 policy-stating rows, measured). MEASURED: the whole-struct census of all 133 prepared characters is byte-identical before and after (`cmp`), including the pistol's aim angle (`FRAC_PI_2` as a RON literal), the flight's `half_extent` and the charge ladder. Guards: `the_weapons_leave_the_body_where_they_are_drawn` (the officer's hand muzzle and round art, the polygon's charge and cannon muzzle, from the prepared kit; it replaces the two tests the files carried), the row guard's `ranged_vfx` comparison, and a pin on the automaton's verbs (blink, flight and its toggle, shield, dash, attack; no double jump, fast fall, dodge or ledge grab). Poisoned: a `Weapon` lowered to its short form ("his sidearm states a discharge"; ⚠ the row guard's action-set comparison did NOT speak, because it lowers the row through the same code preparation uses, so it is circular for the lowering), no `ranged_vfx` fold ("`imperfect_cellular_automaton`'s shots look as its row says"), `Flight` granting no toggle ("the automaton lost a capability its row grants"; the row guard's grant comparison is circular in the same way). Validation: characters 489 (with `content_pack`), platformer core 589, content 448 + 32 (with `aerial_authoring`), Mary-O 161, Sanic 89, Smash 267, `ambition_app --test app_it` 791 + 1 failed, then that file 12 / 12, pytest lane 1,580 + 1 failed, then that file 2 / 2, absence contracts 50 of 50, MODULES.md regenerated. Both failures were right. (1) `smash_roster_movesets::every_fighter_on_the_smash_grid_can_throw_a_punch` read each fighter's ROW preset, not its kit: the Officer's row had named `striker_swipe`, a swipe his file overruled, so the census counted a melee he never had. It now reads the prepared kit, which also shows that `player_robot_v3` was never unarmed (its row says `peaceful` and its lineage definition, which outranks the row, states a melee); the Officer joins the polygon as ranged by design. (2) `test_maintainer_authored_moves_keep_their_attribution` guarded Jon's "PCA needs to shoot a glider" in the deleted automaton file; the quote moved with the kit into the row, and the floor now counts `character_catalog.ron` (3). Checking the other deleted creature files for maintainer quotes found one AP69 had dropped: Jon's words on the admiral riding a shark (`npc_pirate_admiral.rs`, not in the guard's list), now restored in the admiral's row. Poisoned: the admiral's quote without `Jon:` fails "2 attribution(s), was 3". Process note: the restored quote is a RON comment edited while `app_it` compiled; it changes no behaviour. What a game says now: every Ambition creature is its row, including a gun that fires from the hand with a half-plane aim and a cannon whose charged shot grows through five looks. Adding an Ambition character is a row (and a move file if it fights with moves) and no Rust, unless it is offered on the select grid, which is still an entry in the Rust list `PLAYABLE_ROSTER`. Commit e2aea8bf5. |
| AP72 | The versus stage states its two duelists as a RON string and a Rust grammar (AP58, the host's versus roster) | ✅ DONE 2026-09-26 | Before: the versus stage's cast was said in two places, both code. `VERSUS_CATALOG_RON` was catalog RON pasted into a Rust string and read by `CharacterCatalogFragment::from_ron` (the legacy reader beside the content compiler), and `duelists()` registered each fighter again with a Rust definition: `duelist_moveset(numbers)` generated four moves from a `DuelistNumbers` struct, `duelist_hurtboxes(numbers)` generated a hurtbox timeline, and `with_sheet`, `with_abilities(VERSUS_FIGHTER_KIT)`, a `WithHealth` trait for health and mass, an explicit empty `ActionSet` and `with_voice` stated the rest. The row's preset was `authored_elsewhere`, a name that said the row was not the authority. One authority now: the stage's content pack (`game/ambition_app/assets/`: `pack.ron`, `data/character_catalog.ron`, `data/movesets/arena_duelist_{long,close}.ron`), registered by `register_versus_cast` through `EmbeddedPack::cast`, the road every demo's cast takes. The rows state health, mass, the grants (`[RunJump, Interact, Reset, Attack, FastFall, AirJump]`, exactly the stage's ceiling), the kit (`moves_only`), the hurtboxes and the lines; the move files hold the grammar exported from the Rust (one table per fighter, the numbers in each file's header). The row vocabulary gained `hurtboxes` (a `HurtboxDoc`, folded at preparation). `VERSUS_FIGHTER_KIT` stays as the stage's ceiling, a ruleset fact. FOUND on the way: the move-file document has `body`, `hurtboxes` and `presentation` slots that the moveset schema never read, so a value there was dropped with no error; the schema now refuses each by name ("a move file's `hurtboxes` is read by nothing"). Deleted: `VERSUS_CATALOG_RON`, `duelists()`, `duelist_moveset`, `duelist_hurtboxes`, `DuelistNumbers`, `LONG_GUARD`, `CLOSE_GUARD`, `WithHealth` (`versus_fighters.rs` 541 -> 208 lines); the stage's registration is one call, and `versus.rs`'s activation test uses the same call. MEASURED: a whole-host census (all 148 prepared characters of the shipped host, run twice first: deterministic) is byte-identical before and after except the two duelists' `voice`, which is now empty: their lines moved to their rows' `fallback_dialogue`, which the bark resolver reads before the definition's voice in every situation, so what they say is unchanged. Guards: `a_duelist_speaks_in_its_own_voice` (the bark resolver's line for each duelist at Hall, OnHit and Provoked), `what_the_duelists_author_is_exactly_the_stages_ceiling` (each row's grants through the ceiling equal the ceiling), and the two grammar tests, now reading the pack's tables. Poisoned: no hurtbox fold (`versus_stage::a_seated_fighter_is_damageable_through_its_authored_hurtbox`: "carries no authored hurtbox document"), the long guard's first line deleted (the voice test, `left: "I will meet you exactly where you were."`), a move file stating `hurtboxes` (the pack refuses to compile, naming the file and the slot), the close guard without `FastFall` (the ceiling test, `fast_fall: false`). Validation: characters 489 (with `content_pack`), content 448, `ambition_app --lib` 216, Smash app 18 + 51, `ambition_app --test app_it` 792 / 0 failed, pytest lane 1,581. What a game says now: a stage's fighters are two rows and two move files; a new duelist is a row and a table, with no Rust. |
| AP49 | A direct session is built without its construction authority | ✅ DONE 2026-09-26 | From `architecture-census.md` ("A SPAWN COPIED ACROSS THREE COMPOSITIONS", raised 2026-09-21). Before: `install_direct_session_root` stood a direct session's root up but not the `SessionMechanics` that every live rebuild (replay, reset, room transition) reads and declines without. The host's demo gate added a DEFAULT one on the next line (empty cast, empty sheets, no bosses), with a comment calling it "THE LINE EVERY DEMO APP NOW COPIES", and Mary-O and Sanic added none, so their direct sessions declined every reset. Two answers to "what does this session build from": the fingerprint's capture, and whatever the demo declared. Now one road. The helper captures the sheets, bosses and developer knobs from the same resources and at the same moment as the fingerprint, and `freeze_direct_session_mechanics` (`PreStartup`, after `PreparationBarrier`, the first point where the folded cast exists) completes the capture with the cast and installs it. A second `SessionMechanics` aborts as a composition error. Deleted: the host gate's line and its comment, and the `demo_fixture::SessionMechanics` re-export that existed only so a demo could write that line. Game expression: a direct demo writes nothing for its session to support replay, reset and transitions. Witness `a_direct_session_freezes_its_construction_authority_from_its_own_preparation` (provider): each arm compares against the authority it was frozen from with a non-default member (a staged character's health, a population cap, the shipped boss catalog), and `GenerationMechanics::for_live_session` accepts the result. Poisoned: no freeze fails the existence arm; a default authority fails the cast arm. ⚠ No demo-level behavioural witness: the smoke suite passes with and without the freeze (none of its tests rebuilds a room), and the Mary-O/Sanic unit tests do not run the fixed-tick sim. Validated: provider, host (smoke 8), runtime, Mary-O, Sanic, app_it, pytest. |
| AP50 | The robot lineage's moves are compiled while every other fighter's are content | ✅ DONE 2026-09-26 | AP45's residue. Before: `player_robot_v3`'s platform-fighter table and `player_robot_v2`'s theorem chain were Rust functions that `player_robot_lineage::definition_from` attached with `with_moveset`, and `authored_movesets::tables()` appended them by hand under names no character has (`player_robot`, `theorem_chain`). The lineage never passed `authored_intrinsics`, so a pack could not change the robot's moves, and a reload of an edited file could not reach it. A test (`the_robot_lineage_does_not_reach_the_seam_the_pack_applies_moves_at`) pinned that exclusion. Now: `assets/data/movesets/player_robot.ron` carries both tables (entities `player_robot_v3` and `player_robot_v2`), listed in `pack.ron` and in `pack.rs`'s static list. `definition_from` ends in `authored_intrinsics(id, definition, pack)`, the seam the declared cast passes, and `register` reads the App's selected pack once, as `register_declared_cast` does. All 107 comment lines of the Rust tables moved with the moves (one note per move above its move, the rest in the file's header; a script checked that none was lost), including the one maintainer-attributed line, and the attribution floor now reads the RON path. Deleted: `player_robot_moveset()`, `theorem_chain_moveset()`, their two timing constants and imports (the file keeps `player_robot_action_set` and its tests, which read `shipped(id)`), the hand-appended rows in `tables()`, and the exclusion test. ⭐ Found on the way: the reload test fixtures built "the shipped cast" from the catalog fragment and the declared cast and left out the lineage, so they staged a cast with no robot and refused any edit of its file. The cast now registers through one call, `character_catalog::register_cast` (fragment, lineage, declared cast), used by `ContentPlugin` and by the ten fixtures that repeated the calls by hand. Game expression: every move of every fighter this provider ships, the protagonist included, is now content; a variant of the robot is one RON entity. Guard: `moves_are_content::the_robot_lineage_wears_the_tables_its_file_carries` compares each `LINEAGE` member's built definition with the pack's table (v0 included, with no table). Poisoned: dropping the seam call fails it, plus the v2 theorem-chain test and the reload kit-subset test. Validated: ambition_content lib 453 and content_it 32, app_it 784, pytest lane 1,579. The census now sees the robot: `measure_authored_move_clock.py` reads 366 moves across 20 entities (347 before). |
| AP51 | Two fighters that play one table carry two copies of it | ✅ DONE 2026-09-26 | Before: `cellular_automaton.ron` carried the perfect and the imperfect automaton as two identical 2,227-line tables, so a retune of one left the other behind with no error. A borrow could not say "the same table": `MovesetBorrow` renames every move id to the borrower's name, because one id naming two DIFFERENT moves makes a trace, a cue table and a cancel window ambiguous. That reason does not apply to a borrower that changes nothing: the same id then names the same move. Now: `prefixes: []` means "wear the archetype's table as it is", and it is refused when the borrower also authors moves of its own (`resolve_borrow`). `imperfect_cellular_automaton.ron` is that borrow, in its own file like every other borrower, so the census scripts read it with the rule they already share (`measure_authored_strike_extents.borrowed`, which now keeps the ids for an empty prefix list). Deleted: the 2,290-line copy (`cellular_automaton.ron` 4,674 → 2,384 lines). Game expression: a second fighter that plays an existing table is an 18-line file that names it. The smash demo's two stand-in duelists, which share one Rust table, can now be said this way. Guards: `a_borrower_with_no_prefixes_wears_the_archetypes_table_unchanged`, and a new arm in `a_borrow_that_cannot_resolve_is_refused_and_names_its_file` (no prefixes plus own moves). Poisoned: skipping the refusal fails the arm; routing empty prefixes through the rename fails the witness with "carries none of the prefixes []". The reload test that drops one entity of a two-entity file now picks a victim no entity borrows; dropping an archetype is a compile refusal, which is a different rule. Validated: the census reports the same 366 moves and 384 volumes (only the labels change), ambition_characters (content_pack) 491, ambition_content lib 453 (1 red: `gnu_ton` choreography reads untracked published sheets that this checkout has not republished since a peer's merge; not a move table), app_it 784, pytest lane 1,580 (the reviews page lists the new file with its archetype). |
| AP52 | The Smash demo's stand-in duelists get their moves from compiled Rust | ✅ DONE 2026-09-26 | Before: `moveset::fighter_moveset()` (625 lines of composed Rust) was the table both stand-ins (`smash_duelist_a`, `smash_duelist_b`) wore, attached in `install_smash_content` with `with_moveset`. The demo's own content pack held only George's facet, so the second game in the workspace said its fighters' moves in code while Ambition's said them in data. Now: `assets/data/movesets/smash_duelist_a.ron` carries the table (exported from the Rust function, so equal by construction), and `smash_duelist_b.ron` wears it with the AP51 no-prefix borrow. `pack.ron` declares both, `smash_pack::MOVESETS` embeds them, and `smash_pack::shipped_moveset(id)` is the one reader: the registration and the tests use it, and a missing table panics with the two places to fix. The function's 152 comment lines moved with the moves (7 `Feel` variant docs stay with the enum, which George's compiled table still uses). Deleted: `fighter_moveset()`, its `CHARGE_POSE_AT_S` and `airborne_only` (moveset.rs 1,541 → 894 lines, all that remain are George's helpers and the tests). Game expression: a Smash fighter's moves are one RON file plus two lines in the pack, and a second fighter on the same table is an 18-line borrow. Guard: `smash_in_the_host::the_stand_in_duelists_fight_with_the_table_their_file_carries` (app_it) compares each duelist's prepared `authored_moveset` with the file and names the moves that differ. Poisoned: handing duelist b a table without `jab2` fails it with `the moves that differ: ["jab2"]`. Validated: ambition_demo_smash 267, ambition_demo_smash_app 18 + 51, app_it 785, pytest lane 1,580. RESIDUE → AP53: George's table (`george_booul_moveset.rs`, 1,338 lines) is still compiled, and it builds its grab from the facet's `capture` kit (`smash_pack::capture_kit`), a second authoring road for moves. His facet file records a 2026-08-19 review fence, "do NOT flatten George's sixteen composed moves into RON"; fast-iteration I2 (AP45) answered the same concern for all 17 Ambition fighters by moving the prose with the moves, and the decision doc puts content in RON, so AP53 migrates him, moves the grab into his move table and deletes the facet's `capture` road. |
| AP53 | George's moves are compiled, and his grab has two authoring roads | ✅ DONE 2026-09-26 | `george_booul_moveset()` composes 16 slots from `strike`, `impulse`, `on_hit`, `committed_tail` and `feel`, and takes its grab, pummel and throws from `SmashFighterFacet::capture` (`CaptureKitAuthoring`), which only George authors. Plan: export his table to a `moveset` file beside his facet in the character-authoring submodule (the review's ownership ruling: "Smash should select George; it should not own George's values"), carry the prose, move the capture reasoning to the grab moves, delete `capture` from the facet schema with `CaptureKitAuthoring`, and delete `george_booul_moveset()`, `feel`, `Feel`, `grounded_only` and `jab_string_continuations`. The excluded-middle law stays a test on the shipped table (`no_move_lives_between_the_pokes_and_the_commitments`). Rewrite the facet file's fence comment to state the new owner. ✅ STEP 1 DONE 2026-09-26 (the refusals first, so deleting the facet's validator loses none): the facet's `CaptureKitAuthoring::problems` was the only place that refused a grab reach with no area or a throw with no launch direction, and it ran for George alone; the 17 Ambition fighters' grabs, authored as plain moves, passed admission on `check_hydrates`, which asks only whether the params parse. Now `smash_capture::check_capture_attempt` and `check_capture_throw` are the capture techniques' param checks in `combat_schedule`, so the refusal covers every fighter at the preparation barrier. The facet's other refusals are already the move validator's (`EventOutOfRange` for a pummel impact or throw release past its move, `DuplicateMoveId`, `EmptyClipBinding`). Guards: `admission_check_tests` (unit; poisoned, both arms fail) and `authored_effects_are_admitted::the_shipped_support_table_refuses_a_grab_that_reaches_nothing` (app_it, with an admitted control; poisoned by restoring `check_hydrates`). The shipped corpus passes: `every_authored_effect_in_the_shipped_composition_is_admitted` stays green. Validated: entity_catalog 134, runtime, app_it 786, pytest 1,580. ✅ STEP 2 DONE 2026-09-26: George's table is content. `smash_moveset.ron` (entity `smash_george_booul`) sits beside his facet in the character-authoring submodule and the demo's pack declares it by path, as it does the facet. All 143 comment lines of the Rust table moved with the moves, and the facet's capture prose moved onto the grab, pummel and four throws it described; the stale notes ("the sixteen slots stay in Rust", "this is the last place that knows both numbers") were rewritten to name the test that now holds the law. The facet file's fence comment now names the new owner. Deleted: `george_booul_moveset()` and its constants, the helpers only it and the stand-ins used (`feel`, `Feel`, `grounded_only`, `jab_string_continuations`, the flurry constants), `smash_pack::capture_kit`, and from the engine `SmashFighterFacet::capture` with `CaptureKitAuthoring`, `GrabAuthoring`, `PummelAuthoring`, `ThrowAuthoring`, `into_repertoire` and their validator (smash_fighter/mod.rs 459 → 185 lines; george_booul_moveset.rs 1,338 → 721 and moveset.rs 1,541 → 666, both now test-only modules). The registration has no fighter arm: `definition.with_moveset(smash_pack::shipped_moveset(id))` for every fighter. Game expression: every fighter in the workspace's two fighting games now states its moves in a move file; the facet states only body and weight. The ascent tests read the lift from the shipped move's impulse (`frame_data`) instead of restating three constants. Guards: the app guard became `every_smash_demo_fighter_fights_with_the_table_its_file_carries` (George added; poisoned, it names `george_uthrow`); the excluded-middle law now reads the file (poisoned: moving `tilt_forward`'s startup to 0.12 in the RON fails `no_move_lives_between_the_pokes_and_the_commitments`). ⚠ One copy this created: `jab2` and `jab3` were authored once and pushed into both George's and the duelists' tables, and are now two copies. `the_jab_string_is_one_string_in_both_tables` keeps them equal (poisoned: `jab2` damage 9 in one file fails it) until AP54. Validated: ambition_characters (content_pack) 483, ambition_demo_smash 268, ambition_demo_smash_app, ambition_entity_catalog, app_it 786, pytest 1,580. Submodule commit `5ad34b0`. |
| AP54 | A move file cannot take single moves from another table | ✅ DONE 2026-09-26 | `borrows` takes a whole table. George's `smash_moveset.ron` and `smash_duelist_a.ron` both carry the same `jab2` and `jab3`, which Rust once authored once (`jab_string_continuations`); a drift test holds them equal. Plan: let an entity name moves to take from another entity's table by id (for example `takes: [(from: "smash_duelist_a", moves: ["jab2", "jab3"])]`), resolved in the move schema's `aggregate` beside `borrows`, with the same refusals (an unknown source, a missing id, a chain). Then delete George's copies and the drift test. The same shape would let a fighter share a grab or a taunt without borrowing a whole table. Done as planned. `EntityContracts::takes: Vec<MovesetTake { from, moves }>` (omitted from a file when empty). The move schema resolves a taker with the borrowers, after every authored table is merged: the borrow first if there is one, then the entity's own table, then each taken move appended under its own id, and the whole validated once (`resolve_derived`). Refused, naming the taker's file: a source that authors no table, a move the source does not have, a take from a taker (a chain), and an id the taker already authors. The last is also a `DuplicateMoveId` to the whole-table validator (a poison of the take's own check passed until the test asked for the take's words), so the take's check exists for its message, which names the take to change. George's `smash_moveset.ron` now `takes` `jab2` and `jab3` from `smash_duelist_a`; his copies and the drift test `the_jab_string_is_one_string_in_both_tables` are deleted, and his own jab keeps the notes the port had put on `jab2`. `measure_authored_strike_extents.read` refuses a `takes` line rather than count a table short (pytest witness, poisoned). Game expression: two fighters that share a move state it once, without sharing a whole table. Guards: `a_taker_plays_the_taken_move_as_its_owner_authors_it` (the owner's move unchanged, own moves first, no borrow of the rest, and a retune of the owner reaches the taker) and `a_take_that_cannot_resolve_is_refused_and_names_its_file`; poisoned (skipping the takes fails both). Validated: ambition_characters (content_pack) 485, entity catalog, ambition_content, ambition_demo_smash 267, smash app, app_it 786, pytest 1,581 (feature-gated count 449 -> 451). |
| AP55 | Mary-O and Sanic say their moves in Rust, and each game hand-rolls its pack | ✅ DONE 2026-09-26 | Before: `mary_o_moveset()` and `sanic_moveset()` (about 470 lines of composed Rust each) were attached with `with_moveset` to every form of each fighter, and neither demo had a content pack. The Smash demo had one, and `smash_pack.rs` spelled out the compile, the `OnceLock` cache, the look-up and the refusal wording by hand; a third and fourth game would copy it. Now the engine owns that once: `ambition_platformer2d::content::EmbeddedPack` (a `const fn new(manifest, sources)`, `prepared()` compiled once, `moveset(id)` that refuses a character the pack does not author and names the ones it does). A game states `pub static PACK: EmbeddedPack = EmbeddedPack::new(include_str!("../assets/pack.ron"), &[(path, include_str!(path))])` and registers with `.with_moveset(PACK.moveset(id))`. Mary-O and Sanic each gained `assets/pack.ron`, `assets/data/movesets/<fighter>.ron` (exported from the Rust function, all 55 and 58 comment lines carried) and a 16-line `src/pack.rs`; each form (Mary-O tall and fire, Super Sanic) is a no-prefix borrow in the same file. `smash_pack.rs` uses `EmbeddedPack` (130 → 80 lines) and its `shipped_moveset` is gone; callers read `smash_pack::PACK.moveset(id)`. Deleted: both Rust tables and their effect constants; the `smash_moveset` modules keep only their tests, now test-only. The two test comments that claimed a missing slot is a compile error (true only of `SmashRepertoire` in Rust) now name the schema refusal and the host ratchet. Game expression: every fighter in every game in the workspace states its moves as content; a new game's pack is a manifest, its files and one `static`. Guards: `content_sdk_tests::an_embedded_pack_answers_with_the_table_its_file_carries` (facade; the table and the refusal's words) and `smash_in_the_host::the_crossover_fighters_and_their_forms_fight_with_their_files_tables` (app_it, all five ids; poisoned: a fire form one throw short fails it naming `mary_o_dthrow`). Validated: facade content tests, Mary-O and Sanic crates and apps (353 across 12 binaries), ambition_demo_smash, app_it 787, pytest 1,581 (feature-gated count 451 -> 452, the facade test). |
