...
| AP46 | Two answers to "how far away does a body notice a foe" | OPEN (2026-09-26, reported by ToothbrushAmbition; not yet measured here) | A fighter brain's `aggro_radius` (policy data, jittered in `brain_builders.rs`) gates pursuit in `brain/smash/mod.rs` and `mode.rs`. Separately, `Perception::Sighted { viewport_half }` (`DEFAULT_VIEWPORT_HALF` 480×320, `features/ecs/perception`) clips what the body can see. A foe at 400 px can be seen and ignored, and a radius past 480 px names a foe the body cannot see. Plan: measure which one each brain reads, rule one authority for "noticed" (sight clips, the brain's radius is a preference inside it, or the reverse), delete the other road, and state it in the character definition. |
