...
> **Every exhaustive literal and every exhaustive destructure is a CALL SITE.**
> `cargo test -p` cannot see one crate over.

The smash lane added `when_refused` to `MoveGates`, verified the five crates it
EDITED (1,644 tests green), and pushed; `ambition_app`'s exhaustive literal at
`versus_fighters.rs:85` is one crate over and stopped compiling. Hours earlier I
had landed an unconditional `use` of a `#[cfg(feature)]` module in an ungated
file, so the crate built WITH the feature and not without it.

⭐ **In both cases the instrument existed and neither of us had run it**:
`scripts/check_no_warnings.py` — the workspace `cargo check --all-targets` at
DEFAULT features — which `--rust` AND `--rust-alone` both OMIT. ⇒ Run it after
any landing that touches a shared type or a feature gate; it is seconds against
a warm tree and it is the only job in the plan that sees either shape.

⚠ **And when it fires, the exhaustive literal is the HERO, not the obstacle.**
The tempting repair is `..Default::default()`, which is wrong twice: it turns a
compile error into a silent wrong answer, and it answers a question the failing
file often says in its own comments that it cannot see. Patch the literals.
...
⛔⛔ **AND THE SAME SHAPE CAUGHT ME AGAIN SIX HOURS LATER, IN THE FORM THAT HAS
NO SKIP-LIST TO READ PAST — 2026-09-04.** Having fixed the `--rust` footer that
morning, I pushed a feature commit (`abd5191f3`) having run `cargo test -p` on
each touched crate plus `check_no_warnings`, `check_doc_links`,
`check_planning_citations` and `check_severed_sentences` **by hand** — and
believed that was the gate. It was not: the pytest guard set was never in it, and
the commit landed **four red guards** — three rollback-ratchet arms and
`MODULES.md` currency. The sibling session found them by running
`./run_tests.sh --rust`, the lane I had spent the morning documenting.
⇒ **A HAND-ASSEMBLED LANE IS MORE DANGEROUS THAN A NAMED ONE, and that is the
transferable part.** A named lane has a documented skip-list; the whole fix that
morning was to make `--rust`'s skip-list complete. A lane you assemble yourself
by picking checkers deliberately has **no skip-list at all**, and choosing its
members on purpose is exactly what makes it feel sufficient. ⚠ Neither of the
four was a defect in the change: the rollback entry was a *deliberate* new
registration the ratchet exists to make deliberate (banked), and the module map
was a new module nobody had described (regenerated by its own writer). ⭐ The
ratchet firing on three arms at once is it WORKING — one live-tree check and two
poison arms — which is the arity read this file already records under
`-- --no-fail-fast`: check the arity of a failure before reading its content.
...
⛔⛔ **THE FAST LANES DROP THE ONLY CHECK THAT COMPILES AT DEFAULT FEATURES —
measured 2026-09-05, after I broke `origin/main` that way.**
`scripts/check_no_warnings.py` runs `cargo check --workspace --all-targets` with
NO `--features`, so it is the guard for "this crate no longer builds without its
optional feature". It is in the DEFAULT plan and the `--rust` banner names it in
the list it OMITS — `--rust` and `--rust-alone` both drop it.

⇒ **A session verifying with `--rust-alone` cannot see a default-features break
at all**, and a session checking one crate with `--features X` cannot either. I
did both: `cargo check -p ambition_game_shell --features basic_presentation`
passed every time while the crate failed to compile without the feature, and
`ambition_content` pulls it in transitively, so a peer's builds stopped.

⭐ **The invariant is not "run more tests".** Both of the regressions I shipped
this afternoon were a NEW EDGE crossing a boundary I never varied — a launcher
the shell's own tests do not compose, and a feature every check I ran turned on.
⇒ **When you add a consumer, ask which CONFIGURATION the new edge is absent in**,
and run that one. `cargo check -p <crate>` with no features is seconds.

- ▢ **D-MENU-CHANNEL — ONE UNGATED MENU CHANNEL, 25 READERS, AND AN ARBITRATION
  AUTHORITY THAT NONE OF THEM ASK. Found under Jon's title-screen bug and it is
  not title-screen specific.**
  ⛔⛔ MEASURED 2026-09-05: `populate_menu_control_frame_from_actions`
  (`actor_monolith/src/schedule/input_systems.rs:735`) builds `MenuControlFrame`
  from raw `ActionState` with **no `SeatInputContexts` parameter at all**, and
  **25 non-test readers** consume it — dialog, the shell launcher, the shell
  pause menu, load presentation, the map menu, the actor monolith. ⇒ Any two
  surfaces live at once BOTH act on the same press.
  ⭐ **The authority to settle it already exists and is correct.** Claims carry
  priorities (`LAUNCHER` 200 capturing, `PAUSE` 190, `GAMEPLAY` 100),
  `resolved()` cuts after the first capturing claim, and SEMANTIC actions ARE
  gated by it — `semantic.rs:314` refuses an action whose declared contexts the
  seat does not allow. The menu channel simply never asks.
  ⇒ **That is (A): two authorities for "who owns this press", and the correct one
  is unconsulted.** Jon's report — *"it also seems non deterministic which menu
  is chosen"* — is what an unarbitrated channel looks like from the outside.
  ⚠ **The title-screen case is already fixed and this row is NOT that.** The shell
  pause menu now yields to the launcher by asking about the launcher directly,
  which is a point fix between two named surfaces; it does nothing for the other
  23 readers.
  ⚠ **Sizing, honestly: this is shipped input for every menu in the game**, and a
...
- ✔ **D-TITLE-MENU — CLOSED 2026-09-05.** Jon: the title screen stacked the
  ...
  ("is AMBITION'S OWN session live"), and the module doc's *"the two menus
  partition every LIVE SESSION with no overlap"* is true and covers nothing on a
  title screen — the state with NO session. The apparent randomness is separate
  and also real: `populate_menu_control_frame_from_actions` builds
  `MenuControlFrame` from raw `ActionState` with NO `SeatInputContexts`, so the
  one menu channel is ungated by context while the priority table that would
  settle it (`LAUNCHER` 200 capturing > `PAUSE` 190) goes unconsulted.
  ⇒ **Both halves landed.** The shell menu yields to the launcher (`e8f45f582`),
  and settings became the launcher's second TAB carrying the SAME
  `ShellAudioControl` rows the pause menu uses (`e405c74b1`) — Jon's own design.
  ⛔ **And the fix removed a capability before restoring it**, which only an
  assembled-host test caught: making the menu yield also removed the only route
  to "how do I mute this" from the one screen that asks it. Start now reaches the
  settings tab (`ccbdff22b`); the shell crate's 79 tests were green throughout,
...
- ▢ **D-MARYO-SPRITE — Mary-O's sprite is mispositioned against her collision
  ...
  louder signal and may be the same cause. Next action: reproduce, then compare
  ...
  ✔ **Ruled OUT 2026-09-05 — `PlayerSpriteBaseline` and everything downstream of
  it, and this one is worth reading before the others because I nearly shipped a
  fix for it.** Her forms author `BodySource::SpriteAuthored`
  (`game/ambition_demo_mary_o/src/lib.rs:1302`), so her body carries
  `SpritePosedBody` and publishes `BodyPoseView::authored_render` — and
  `sync_visuals` takes the `authored_render` arm BEFORE the baseline arm and
  never reads the baseline for her. ⇒ No baseline value, guessed or stale,
  reaches her on screen.
  ⚠ A real staleness DOES exist there (a form swap in phase `FeatureInteraction`
  9 rebinds against the box `WorldPrep` 2 gave the body earlier that frame, and
  nothing re-seeds it) and I reproduced it — 32 vs 64, a permanent 2.0 ratio —
  and committed it as her fix. It was not: the branch is unreachable for her, and
  the bodies that DO read the baseline are the dev body-profile preview, where a
  ratio other than one is the intended mechanism. Reverted in `3f9f0558d`; the
  staleness is now pinned by a characterization test so the next reader does not
  repeat it.
  ⇒ **Where to look instead:** `authored_render` / `authored_offset`, published by
  `sync_sprite_posed_bodies`
  (`crates/ambition_character_sprites/src/posed_body.rs:171-192`). That pass owns
  her quad's SIZE and its PLACEMENT together, including the `stance_shift` term
  that reverses `resize_feet_planted`'s centre slide. A positioning error against
  her collider is a disagreement in that term or in `geometry.sprite_offset`.
  ⭐ And one measured lead found on the way: a posed body is
  size-without-placement for exactly ONE frame after projection (the marker lands
  in phase 3, the offset is published in phase 2), during which `sync_visuals`
  falls back to a feet anchor whose own comment records it disagreeing with the
  published offset by ~1px vertically and ~2.5px horizontally. That is a
  one-frame pop at spawn — probably not Jon's persistent report, but it is a real
  placement disagreement in exactly the code his report points at.

  ✔ **Ruled OUT 2026-09-05 — the SNAKE half, both roads, and one is now a
  guard.** His report names the snake separately (*"the size of the snake has
  seemed to vary depending on the global game state"*), so it was chased on its
  own terms:
  * **`snake_world_per_pixel`'s 0.35 fallback cannot vary at runtime.** It reads
    BAKED sheet metrics — which is why `posed_body_geometry` resolves in a unit
    test with no `AssetServer` — so the fallback is a build-time property, not a
    game-state one. Whether it fires is fixed for a given binary.
  * **No pose is missing metrics.** `sync_sprite_posed_bodies` resolves geometry
    PER ANIMATION and `continue`s on `None`, FREEZING the previous pose's box —
    which would look exactly like a size that varies with state. The snake plays
    five poses (Idle, Emerge, Peek, Retreat, ShellIdle) and had no test that they
    resolve, unlike Mary-O. All five do; the property is now pinned by
    `snake::tests::every_pose_the_snake_plays_resolves`, poison-verified.
  ⇒ So the snake's box is neither mis-scaled by the fallback nor frozen by a
  missing pose. Whatever varies is downstream of the box.

  ✔ **Ruled OUT — the silent scale fallbacks**, though they are real and now
  guarded (`0cc7be43f`). `snake_world_per_pixel` falls back to `0.35` where the
  art derives ~`0.182` (its own doc table says so — a snake about TWICE the right
  size), and `mary_o_world_per_pixel` falls back to `1.0`, roughly twenty times
  the real scale. Both fire only when the baked sheet does not resolve, and
  MEASURED: all four sheets resolve in this composition, so no fallback fires.
  ⚠ Lane: the crate's own test composition; a different binary bakes a different
  sheet table.

  ✔ **Ruled OUT — Mary-O is on the GOOD sizing road.** She calls
  `with_sprite_authored_body`, so `pose.authored_render` is published and the
  quad is the frame at the authored scale — *"the box and the quad are two
  readings of one number"*. She is NOT on the fallback ratio
  `standing_render * (base_size / standing_collision)`, which the code itself
  calls *"a guess about the art CORRECTED by how far the box has drifted from a
  baseline — two independent quantities reconciled by a ratio"*.

  ▢ **STILL OPEN, in the order I would chase them:**
  1. **The one-frame skew.** `sync_sprite_posed_bodies` derives BOTH the
     collision box and the render quad/offset from `posed_body_geometry(target,
     ANIM, wpp)` — per ANIMATION — in a sim system (`WorldPrepSet::BeforeIntegrate`),
     and the render path reads them through the pose view. If the anim changes
     between publish and read, box and quad are one frame apart, which is exactly
     a sprite sitting slightly off its box.
  2. **`CharacterAnimator::render_basis` is latched once and NEVER invalidated.**
     `ensure_render_basis` is a no-op after the first write. The proper road
     (`build_character_presentation_with_render_size`) seeds it with the LOGICAL
     size before trimming; the seeding in `apply_character_frame` is labelled by
     its own comment a *"compatibility fallback for specialized/legacy sprite
     construction"* and takes whatever `custom_size` currently holds — which
     after frame one is the TRIMMED size. A fresh animator (cast republication,
     form swap) seeded on that road gets a wrong basis for the body's lifetime.
  3. **The `authored_offset`-absent branch**, which is a KNOWN-disagreeing road:
     with no published offset the actor path recomputes a feet anchor from
     `feet_anchor_norm`, and that comment records it disagreeing with the
     published offset by *"~1 px vertically and ~2.5 px horizontally"* for v3.
     Worth confirming Mary-O publishes an offset in every form and pose.

  ⭐ **The instrument already exists and needs a window, so it is Jon's fastest
  path**: `game/ambition_app/src/dev/presentation_probe.rs` records `custom_size`,
  `transform_translation` and `anchor` beside `authored_render`, `authored_offset`
  and `base_size` — the quad-versus-box comparison, per frame.

  ⭐⭐ **LEAD 1 IS RULED OUT BY MEASUREMENT, AND IT SHARPENED THE REST.** Her
  geometry is identical across every pose she plays, so a one-frame POSE skew
  cannot move the sprite at all:

```text
  sheet             pose(any)   box            quad            offset
  mary_o_v2         Idle..Fall  21.33 x 32.00  60.95 x 73.14   ( 0.76, -19.81)
  mary_o_v2_tall    Idle..Fall  21.33 x 64.00  60.95 x 73.14   ( 0.76,  -4.57)
```

  ⇒ Poses do not differ; FORMS do — the box DOUBLES while the quad is unchanged
  and only the offset moves. So the skew that could matter is at a FORM CHANGE,
  not per pose.

  ⛔⛔ **THE STRONGEST REMAINING SUSPECT IS A SECOND AUTHORITY THE CODE ALREADY
  DOCUMENTS.** `PlayerSpriteBaseline` has TWO seed sites in
  `crates/ambition_render/src/rendering/actors/mod.rs`, and they disagree by
  construction — its own comment says so: *"Both seed `standing_collision`
  differently (this one from the default body constant, the assets-changed rebind
  from the live pose) and the render size is not linear in collision"*.
  - `:133` `player_collision = base_size.map(..).unwrap_or(DEFAULT_PLAYER_BODY_*)`
    — a GUESS when the live pose is not there yet;
  - `:941` `collision = pose.base_size` — always live.
  The baseline is then the ratio DENOMINATOR at `:359`
  (`pose.base_size / baseline.standing_collision`, quad = `standing_render *
  ratio`). ⇒ A baseline seeded from the default constant makes that ratio ≠ 1 and
  SCALES THE ART — which the same file says must never happen: *"Growing should
  swap which art is drawn and how big her box is; it should never scale the art.
  Binding against the body's real baseline makes the ratio 1 for every form."*
  ⚠ **The reach is narrower than it first looks, and saying so matters**: Mary-O
  normally takes the `authored_render` branch, which is tested BEFORE the ratio
  branch, so the guessed baseline only bites on frames where `authored_render` is
  absent (no resolved `SpritePosedBody` yet). That is a TIMING window, which is
  consistent with "varies with global game state" and with it being a regression
  rather than a constant error.
  ⇒ Next action: with the probe running, watch `base_size` and `authored_render`
  on the first frames after a bind and after a FORM CHANGE, and log which of the
  two seed sites ran — the `eprintln!` at `:155` exists for exactly that question.
  ⛔ **AND THE OBVIOUS FIX IS WRONG — evaluated and rejected 2026-09-05, recorded
  so nobody spends the afternoon I nearly did.** "Do not record a
  `PlayerSpriteBaseline` when the collision came from the DEFAULT guess" sounds
  like the clean (B) move: no baseline, no wrong ratio. But the no-baseline path
  is not benign — the same file warns that with `custom_size` unset the sprite
  *"renders at the atlas frame's native pixel size until a baseline arrives"*.
  ⇒ Withholding the baseline trades a mis-scaled sprite for an unscaled one,
  which is louder, not quieter.
  ⚠ Deferring the BIND instead is no better: the file binds unconditionally on
  purpose ("a worn player is ALWAYS drawn"), and a body whose `base_size` never
  arrives would never get a sheet at all.
  ⭐ **The author already anticipated this and INSTRUMENTED it rather than
  guessing** — the bind sites "report what they SEEDED", the draw path reports
  what is actually drawn, and the comment says the two need not agree. ⇒ This
...
- ▢ **RE-MEASURED 2026-09-04 EVENING at `ae6b6b4fd` — SIX findings now, and the
  ...
  ⛔⛔⛔ **THE BASELINE HOLDS ONE FACT TWICE, WITH DIFFERENT VALUES, AND FOUR OF
  THE SIX FINDINGS ARE THAT INCONSISTENCY — measured 2026-09-05.**
  `--adopt-wins` rebuilds the merged baseline from the CURRENT snapshot (so the
  per-crate `crates` table is refreshed) and then writes the held SCALARS back
  from the old one. Those scalars are derived from that same table. ⇒ The file
  now disagrees with itself, and the gate compares today against the stale copy:

```text
  crate                      crates[] table   held scalar    today    vs TABLE   vs HELD   budget
  ambition_geometry               592,091       540,227    600,530    +8,439    +60,303   +10,804
  ambition_platformer2d_core      588,780       537,395    597,219    +8,439    +59,824   +10,747
...
```
  ...
  `first_party_lines` 619,625 = the sum of `crates[].lines`, and
  `first_party_crates` 66 = `len(crates)`. `critical_path_crates` is genuinely
  primary (a max over depths, with no per-crate line twin), which is why the
  disclosure excludes it by name. ⇒ Two metrics can disagree with the table:
  `worst_edit_cost` and `watched_edit_cost[*]`. Nothing else.
  ⚠ The other two ratchet baselines in `dev/` (`doc_link`, `zone_name`) do not
  carry a table-plus-summary shape at all, so this is one file's defect and not a
  pattern to go hunting.
  ⭐ **The elegant fix is (A), not a re-freeze:** `worst_edit_cost` and
  `watched_edit_cost` are DERIVED facts — a max and a selection over `crates` —
  and storing them beside the table is storing a derivation. Derive them from the
  frozen table at comparison time and the file cannot disagree with itself.
  ⚠ Recorded rather than implemented: it changes what the GATE means, and this
  row's standing ruling reserves that for the carve owner. But it means the
  backbone's red is mostly an accounting artefact, not four regressions.

  ⛔⛔ **AND THE RATCHET GATES THE DEFAULT LANE, which changes what these
  findings ARE.** MEASURED 2026-09-05: `scripts/compile_ratchet.py` exits **1**
  with findings outstanding, its `Job` carries no allow-fail field (`Job` has
  only `name`/`argv`/`cwd`/`builds`), and it is in the DEFAULT plan. ⇒
  **`./run_tests.sh` is red today for this reason alone**, and anyone running the
  backbone sees a failure that has nothing to do with their change.
  ⚠ The standing note says the ratchet "is NOT in the `--rust` lane, so no union
  run touches it" — true, and easy to read as "it gates nothing". `--rust` and
  `--rust-alone` DROP it; the default lane RUNS it and FAILS on it. ⇒ These six
  are not a report somebody should get to; they are the backbone's colour.
  ⚠ Read the exit code without a pipe: `python3 scripts/compile_ratchet.py | tail`
  reports `$?` from `tail` and looks like a pass.

  ✔ **2026-09-05: the tool grew the verb this row's ruling needed, and one
  finding closed on its own merits.** The ruling below rejects `--update`
  because it "banks the regressions with the wins", and `--adopt-wins` banks no
  regression at all — so a finding that had been TRIAGED, ATTRIBUTED and ACCEPTED
  had no way to be recorded as such and stayed red. `--accept METRIC --reason
  TEXT` re-freezes exactly one named metric with its cause stored beside it, and
  leaves every other finding frozen.
  ⇒ `critical_path_crates: 14 -> 16` is now accepted with `368f1b551`'s
  attribution (the chain walked from cargo metadata; `ambition_abilities` and
  `ambition_held_items`, both D33 carve outputs, at positions 9-10). **7 -> 6
  findings, and the ruling below is untouched**: the re-freeze it forbids is
  still forbidden, and eight destinations are still on the placeholder.
  ⛔⛔ **AND THE RATCHET WAS POINTING AT THE WRONG COMMIT RANGE, which is why
  nobody could answer its question.** `--adopt-wins` builds from the CURRENT
  snapshot, so `commit` advanced for numbers it held back: `f507fcb91` changed
  ONLY that field (`11ef33c5b5a5` -> `c4a51a1b76a9`) while `critical_path_crates:
  14` stayed as measured at the older one. The report then claimed the newer
  commit and `--diff` offered a range in which **no `Cargo.toml` changed at all**,
  while the finding instructed the reader to "say which carve did it". ⇒ The
  baseline records `carried_from` now and the report says so.
...
```
  ...
  ⛔⛔ **AND ON 2026-09-05 EVENING THE DENOMINATOR MOVED, WHICH BREAKS ALL FOUR
  READINGS AT ONCE.** Every line above compares against the frozen **108,364**.
  The baseline file no longer holds that: `dev/compile_ratchet_baseline.json`
  records `largest_unit: {crate: ambition_platformer2d_actor_monolith, lines:
  100742}`, `recorded_at: 2026-09-05T17:07`, `dirty: true`. ⇒ The win was BANKED
  into a new floor, so "−8,207 against the frozen baseline" is no longer the right
  SHAPE of statement, not merely the wrong digits.

  ⭐ **Measured against the floor that actually exists** — using the tool's own
  `crate_lines`, not a re-implementation of it — the monolith is **101,130 lines
  across 236 files, `+388` ABOVE the frozen value.** Above baseline is the
  ...
  file, not a finding against anyone — but it means the only honest way to quote
  the floor is to read the file.
  ...
  ⛔⛔ **CORRECTION 2026-09-05: THOSE SIX ROWS CANNOT PRICE ANYTHING, AND THE
  REASON GIVEN BELOW IS THE WRONG ONE.** The gate does not still say eight
  because it "reads the frozen baseline, not the telemetry" — it says eight
  because the rows are the WRONG PHASE. `compile_ratchet.py` sets
  `WEIGHT_CACHE_CLASS = "rebuild"` and builds its weight table from rebuild-class
  rows ONLY; the same file records why, citing 68.1 s rebuild against 309.0 s
  cold on the monolith and noting that mixing them makes "every weight come out
  2-4x high". ⇒ A `--phase cold` run appends honest seconds that the ratchet
  then ignores, so a re-freeze would NOT retire the UNPRICED finding and the
  objection it was thought to answer still stands.
  ⇒ **What actually prices them is a REBUILD measurement** —
  `compile_collect.py --config release --phase first-party`, which is the phase
  that "applies temporary source edits to measure the rebuild Ambition itself
  pays".
  ⚠ **And that is why it has not been run here:** it EDITS SOURCE FILES in a tree
  shared with Jon and a peer session. It restores from saved bytes and refuses
  dirty probe files, but a concurrent edit to a probe file is exactly the race
  that has cost work in this tree before. It wants a quiet tree, which is also
  what a compile measurement needs to be valid at all.
  ⓘ **The superseded claim, kept because the data is real:** `python3 scripts/compile_collect.py --config release --phase cold`
...
- ▢ **(the standing analysis) THE COMPILE-COST RATCHET FAILS THE GATE, and its five messages are the
  ...
  * ⚠ **UNPRICED** `ambition_body_seed`, `ambition_held_items`,
    ...
    joined as their carves landed.
    ⭐⭐ **RE-MEASURED 2026-09-05, AND THERE ARE THREE DIFFERENT TRUE NUMBERS HERE.
    I first recorded this as "it is FOURTEEN and the other readings were wrong";
    that was itself wrong, and the reason is worth more than the count.**
    (`python3 scripts/unpriced_baseline_crates.py` reads the committed baseline
    and builds nothing.)

    | number | what it actually counts |
    | --- | --- |
    | **14** | crates in the frozen file's `crates` table with `seconds_source: estimated` |
    | **6** | the frozen file's own `unpriced_crates` list |
    | **8** | the DELTA the gate reports — the standing UNPRICED finding |

    ⛔ **THE 6-vs-14 GAP INSIDE ONE FILE IS DELIBERATE, NOT A DEFECT.**
    `compile_ratchet.py:1533` holds `unpriced_crates` at the FROZEN value while
    the per-crate table advances, with a comment saying why: copying it from the
    current snapshot "SILENTLY ACCEPTED eight crates newly priced at the
    population median — a placeholder the UNPRICED finding exists to keep loud".
    ⇒ The gap IS the mechanism that keeps the eight visible. A future reader
    diffing the two lists will think they have found a bug; they have found the
    guard working.

    ⚠ **What the raw 14 does add:** the placeholder population is WIDER than "D33
    destinations". `ambition_binding`, `ambition_boss_encounter`,
    `ambition_conversation`, `ambition_damage`, `ambition_mount` and
    `ambition_platformer2d_rollback_ggrs` are placeholder-priced and none is a
    carve destination ⇒ the prediction below, that the blast radius grows with the
    campaign, holds for anything that adds or re-shapes a crate, not only a carve.
    At 14 of 66, **21% of the baseline's `seconds` figures rest on a rate the tool
    itself says predicts cost at R²=0.12.** ⛔ Which is the shape to notice rather than
...
- ▢ **~47 GB ON THE SHARED VOLUME IS UNACCOUNTED FOR, AND THE NEW BINDMOUNT
  ...
  ⛔⛔ **AND IT REGREW 95 GB THE SAME DAY, WHICH IS THE ACTUAL FINDING.** A
  SECOND reclaim hours later on this box removed **95 G across 1,392 crate
  sessions**, 88% → **69% used, 153 GB free**. ⇒ The rate under agent activity is
  roughly a hundred gigabytes a DAY, so a one-off reclaim is a chore that comes
  back, not a fix. A session doing ad-hoc `cargo test -p` per crate mints a fresh
  incremental generation per feature/flag shape and reaps none — the three shapes
  used today (default, `portal_render`, `basic_presentation`) are three
  independent generations of the same crates.
  ⚠ **And the obvious structural fix is WRONG, which is why it is not done here.**
  A repo-wide `[build] incremental = false` would stop the growth and take the
  edit-rebuild loop away from the human: `run_tests.py`'s own comment says
  incremental "stays ON for a developer's own `cargo run`, which is where the
...
- ▢ **POST-CARVE: SWEEP THE DOCS FOR PROSE LOCATIONS, because the citation
  ...
  ✔ **SWEPT AGAIN 2026-09-05 FOR THE FILE-PATH SHAPE, AND IT IS CLEAN — with the
  bound stated, because the population is what decides whether a tool is worth
  building.** Matching a backticked SYMBOL, one of "lives in" / "stayed in" /
  "is in", and a backticked .rs path on one line, across every planning doc returns **THREE** claims. One is a false match on the
  keyword `None`; the other two verify —
  `collect_ecs_pickups` IS in `features/ecs/pickups.rs`, and
  `Platformer2dFeelTuningMonolith` IS in `crates/ambition_combat/src/feel.rs`.
  ⇒ **No tool is warranted for three**, and reading them cost less than writing
  the check would have.
  ⚠ **AND THAT IS ONE SHAPE, NOT THE CLASS.** The examples below are MODULE
  paths in prose (`actor_monolith::items::pickup`, "which stayed in the kernel"),
  which that pattern does not match at all; that population is larger and
  unmeasured. Saying so rather than reporting "prose locations are clean" is the
  difference between a bound and an over-claim.
...
```
  ...
  a backticked `a.b` there is usually one; code comments discuss FIELDS, and a
  field named `active` is written in prose exactly the way `quest.active` is.
  The same rule, same corpus shape, opposite signal-to-noise — **2 hits both
  real over 100 docs, 9 hits none real over 1,776 files.**
  ⭐⭐ **AND THIS PARAGRAPH REDDENED THE GUARD IT DESCRIBES, which is the
  cleanest demonstration of the rule anyone could ask for.** The first draft
  wrote the collision as an inline backticked example, and
  `no_planning_doc_names_a_condition_the_engine_does_not_publish` duly failed
  the union at `queue.md:3287` — a doc discussing a fabricated id IS a doc
  containing one. ⇒ The fix is the guard's own documented escape, and it made
  the sentence better rather than worse: **name the correct id in the same
  paragraph**, which here means saying that the collision is with
  `quest.active` instead of gesturing at it.
...
- ▢ **D33 — continue actor-monolith decomposition by coherent ownership.** Pick a
  ...
  ✔ **RE-MEASURED AT HEAD 2026-09-05, so the next selector does not start from a
  stale graph** (`measure_kernel_module_graph.py --edges 12`):

```text
  module            prod     all   top out-edges
  features        23,850  38,448   construction:30 world:12 avatar:10 character_runtime:10
  character_runtime 3,602  11,090   character_sprites:7 features:5
  world             3,280   5,804   features:8 construction:5
  avatar            2,971   7,264   body_mode:1 control:1
  construction      2,019   5,747   features:15 world:4

  top edges:  features -> construction 30 (and 15 back)
              rollback_registration -> features 28
              features -> world 12 (and 8 back)
```

  ⭐ The heaviest edge is the BIDIRECTIONAL `features ↔ construction` pair (30
  out, 15 back) — the classic carve shape, since a cycle cannot be cut by moving
  one side. ⚠ **And it sits in the fighter lane's surface**
  (`actor_monolith/src/{features,construction}/**`), so selecting it is that
  session's call, not this one's; measured here and handed over rather than
  taken.
  ⚠ `rollback_registration -> features` at 28 is one-directional and a different
  shape: a registrar naming 28 things in one module is a LIST, not a cycle.
