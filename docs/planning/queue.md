...
| AP47 | `ActorDecisionSet::Observe` has an order nobody states | OPEN (2026-09-26, reported by ToothbrushAmbition with a deterministic reproducer) | Adding any system to `ActorDecisionSet::Observe` beside `observe_actor_decision_inputs` flips `canonical_reconstitution` red: the set's result depends on an order the schedule does not declare, so an unrelated addition changes it. Plan: find the pair of systems whose order matters, state it with `.before/.after` or merge them into one observation, and keep the peer's reproducer as the guard. |
