...
"recovery still playing" means the move whose `RecoveryUse` spent the charge,
...
|---|---|---|
| **D183 — three demos hand-roll `DefaultPlugins`, and the engine has no offscreen face** | ✔ **ITS OWN TWO DELIVERABLES LANDED — verified at HEAD 2026-08-28**; what is left is the migration it explicitly deferred | ⭐ mary-o, sanic and twintrack each declare their own `RenderMode` and their own windowed builder; **the Smash demo has NONE, so its shell has never rendered anything** and `./run_game.sh smash` loops schedules with nothing to draw to. The engine's `install_windowed_foundation` already does this job with a `gpu: bool` and its own doc says a consumer re-deriving the disables is the leak. ⇒ add the third mode — **no window, REAL backend** — beside the two it has, give Smash a builder that uses it, and leave the other three duplicated with a note ✔✔ **THE THIRD MODE EXISTS AND SMASH USES IT.** `ambition_platformer2d::app::Display` has `Window`, `Offscreen` (*"a real backend with NO window: frames are rendered and can be read back"*) and `NoGpu`, with the runner consequence stated where the enum is rather than rediscovered by each caller. And `ambition_demo_smash_app::build_windowed_demo_app` goes through `install_windowed_foundation` — the demo has a face now, and it is the ENGINE's rather than a fourth hand-rolled `DefaultPlugins`. ✔✔ **AND THE DEFERRED SLICE LANDED 2026-08-29: mary-o, sanic and twintrack ALL go through `install_windowed_foundation` now** — no hand-rolled `DefaultPlugins` remains among the demos. Each keeps its `RenderMode` as public vocabulary and maps it to `Display` in one `From` impl. ⭐ **THE BLAST RADIUS WAS ONE SPECIFIC THING:** those builders also installed `ScheduleRunnerPlugin` on their offscreen arm, because disabling `winit` removes the app RUNNER and `run()` would otherwise do ONE update and return; the engine's `Display::Offscreen` is deliberately CALLER-STEPPED, so the runner moved to each `capture_*` binary — the only consumer that calls `run()`. Sanic also carried a private `desktop_asset_root()` that was the engine's function rewritten (same early-out, same canonicalized target, same fallback); it and its two tests now use the engine's. ⭐ Verified by OUTPUT, not compilation: mary-o and twintrack captures still draw real scenes, and sanic's is BYTE-IDENTICAL before and after. ⚠ **AND IT SURFACED D257** — the demo captures' frame does not track its subject as the target shrinks (correct at 1280x720, subject at the edge at the default 960x540, nothing but sky at 320x240). ⛔ I first filed that as "capture_sanic writes a blank image"; it was an artifact of a size I chose, and D257 carries the correction. Byte-identical at HEAD either way, so not a regression from this migration. |
| **D184 — the fighter brain has no evaluation path for a MOVEMENT change** | ◐ **RE-SCOPED HALF CONFIRMED BUILT — verified at HEAD 2026-08-28** | ⛔ THE "MISSING SCENARIO HALF" IS STALE — IT IS BUILT. `ladder_rig --scenarios` plays the fixtures through real CPU-vs-CPU bouts and reports time-to-elimination, stocks and damage per rung pair; RUN 2026-08-25: 5 of 9 fixtures play, and it NAMES what it skips per fixture ("cannot set up: velocity, body phase") rather than producing a positional fixture under a tactical name. ⭐ THE REAL REMAINING GAP IS TWO SMALLER, BETTER-SPECIFIED THINGS. (a) FOUR fixtures need state placement cannot make — juggle_escape (velocity, body phase), projectile_camper (projectiles), edgeguard_window (velocity), edgeguard_ledge_hang (ledge hang); each needs a staging verb, not a runner. (b) ⛔⛔ **RE-MEASURED 2026-08-29 AND THIS CLAIM IS STALE — THE VACUOUS ROWS MOVED, they did not go away.** `recovery_above` is now HEALTHY: all FOUR rung pairs engage at **131–160% damage** with 1:1 / 2:2 stocks. ⚠ The vacuous rows are now **three, across TWO OTHER fixtures**: `recovery_below` 3v1 and 6v5, and `recovery_right` 3v1 — each `0.0% : 0.0%` peak damage with **`0 : 0` STOCKS REMAINING** (⛔ that column is stocks LEFT, not stocks lost — I first read it the other way; it means BOTH fighters were fully eliminated) and *"BUT NEITHER LANDED A HIT"*. ⇒ **both sides lose EVERY stock in ~7 seconds while never taking a point of damage — they are dying to BLASTZONES, not to each other.** ⛔⛔ **I FIRST WROTE THAT THE REMAP SCALES THE OVERSHOOT UP. THAT IS WRONG, MEASURED:** the fixture stage is **800x600** and the real smash room is **640x480** (`STAGE_SIZE` in `ambition_demo_smash`), so the proportional map SHRINKS by 0.8 — a 40px overshoot becomes **32px**, not larger. ⭐⭐ **THE ACTUAL MECHANISM IS THAT THE FIXTURE ENCODES 'PAST THE BLASTZONE', NOT 'PAST THE LEDGE', AND THOSE ARE DIFFERENT PLACES.** The fightable platform spans **x 80..560 INSIDE a room of 0..640** — so *just past the ledge* is x slightly under 80 or over 560, still inside the room and recoverable, whereas the fixtures sit OUTSIDE THE ROOM (x<0, x>800, y>600 in their own frame) and map to still-outside. ⚠ And the fixture's premise SAYS SO: *"Self is past a blastzone … a stock lost there is not repaid by a punish"*. ⇒ **the fixture is doing exactly what it claims, and what it claims cannot rank rungs** — a bout that begins with a fighter already dead measures the respawn, not the ladder. ⚠ `recovery_above` escapes this because gravity returns you to the stage. ⭐⭐ **AND THE `recovery_left` / `recovery_right` ASYMMETRY NARROWS TO THE AGENT, BY ELIMINATION — every input to it is symmetric.** Placement: both map **32px outside opposite edges** at **`TransitVelocity::Zero`** (`place_at` routes through `transit_body`, so no velocity and no facing are set). Geometry: the platform spans **x 80..560 centred in a 0..640 room — 80px of margin on EACH side**, and both bodies start at the same height (y 240) above the same floor. ⇒ symmetric placement + symmetric geometry + **asymmetric outcome** (left reaches 110–161% over 38s–>60s; right dies at 0% in 7.6s) ⇒ **the asymmetry is in the AGENT — its facing or its directional recovery — not in the fixture or the rig.** ⚠ HYPOTHESIS, not measured: a body that recovers toward the stage from the left but away from it on the right is a SIGN ERROR in the recovery decision. ⭐ That is this row's own subject (*"no evaluation path for a MOVEMENT change"*), so the next probe is: place a body 32px off EACH edge and log the movement intent it chooses. ⛔⛔ **A MIRROR-SYMMETRY PROBE ON `survival_stick` WAS TRIED AND WITHDRAWN — AND THE REASON I FIRST GAVE WAS WRONG. READ THIS BEFORE REPEATING IT.** Mirroring position and velocity, the stick answers `{x: -0.0, y: 1.0}` on the right and `{x: 0.0, y: -1.0}` on its mirror. ⛔ I first excused that as *"`LocalAxes` is a local frame"* — **it is not an excuse here**: `AccelerationFrame::new` derives `side` from `down` alone and its own doc says under normal gravity *"every transform is the IDENTITY"*, so with `gravity_down=(0,1)` the local components ARE world components and the y-flip is real. ⭐⭐ **THE ACTUAL REASON IT IS BENIGN: IT IS AN ARBITRARY TIE-BREAK ON A SATURATED METRIC.** `survival_stick` evaluates BOTH perpendiculars (`let (a, b) = (-perpendicular, perpendicular)`) and scores each by `time_inside(..).min(needed)` — **capped at the hitstun still owed**. At centre height both deflections outlast `needed`, so they TIE, and the strict `if survive_a > survive_b` falls through to `b = +perpendicular`, whose world direction follows the launch sign. The code documents exactly this: *"at centre stage both deflections keep a body inside for so long that the argmax between them was noise"*. ⇒ **a mirror assertion at centre stage asserts symmetry where the design says noise.** ✔✔ **THE PROBE WAS THEN RUN CORRECTLY AND `survival_stick` IS CLEARED.** Placed 40px above the floor — where the two deflections genuinely differ — a body launched RIGHT and its mirror launched LEFT both deflect AWAY from the floor. Kept as `near_the_floor_the_deflection_does_not_depend_on_the_launch_sign`. ⛔⛔ **AND IT CARRIES THE TRAP THAT DEFEATED TWO EARLIER DRAFTS: `phase_remaining` MUST BE SET.** The survival term is `time_inside(..).min(needed)` where `needed` IS `phase_remaining`, so at its DEFAULT OF 0 both deflections score zero, the argmax ties, and the tie-break answers instead of the ranking — the test then 'fails' while measuring nothing. ⇒ **D184's left/right asymmetry is NOT in the DI/survival path.** ✔ **NOR IN CLASSIFICATION:** `a_body_past_either_ledge_is_classified_the_same` — a body 40px and 120px past the LEFT ledge and past the RIGHT ledge `classify` identically. ⇒ **two layers are now excluded by test, and the asymmetry lies below them**: in the OPTION GENERATION / SELECTION for a Recovery situation, in the movement that executes it, or in the bout setup rather than the brain at all. ⛔ **THE PROBE THAT WOULD SETTLE IT NEEDS A HARNESS THAT DOES NOT EXIST:** `decide` is private and takes `FighterState` + `BrainSnapshot` + `ActorControlFrame`, so unlike `classify` and `survival_stick` there is no way to ask *"what movement does a Recovery view choose"* from a `WorldView` alone. ⇒ building that seam is the next step, and it is the row's own subject. ⇒ the decision is whether these fixtures want PAST THE LEDGE (recoverable, rankable) or PAST THE BLASTZONE (what they say, and unrankable by this rig). ⚠ The remap was itself added to fix the PREVIOUS version of this — its comment records *"pasting them put every recovery quadrant far outside any platform, where the blastzone took it instantly"* — so it converted an absolute error into a proportional one rather than removing it. ⇒ the fix is a JUDGEMENT the brain lane owns, and it is not about scaling: it is whether a *recovery* fixture should start you PAST THE LEDGE (inside the room, off the platform — x just under 80 or over 560 on the real stage) instead of past the blastzone. ⛔ Do not re-scale the remap; the remap is not the defect. ⭐⭐ **AND THEY SHARE A SIGNATURE THE OLD NOTE COULD NOT SEE: every one ends at ~7.1–7.6s** (7.3 / 7.1 / 7.6) while every ENGAGING row runs to 38s–>60s. ⇒ this is not "a fixture that cannot rank rungs" but **a bout that TERMINATES EARLY with nothing having happened**, and the ~7s constant is the lead — diagnose what ends a scenario bout at seven seconds, not what is special about a direction. ⛔ The old reading blamed ABOVE; above is the one direction gravity returns you to the stage, which is why it now looks best of the four. ~~`recovery_above` MEASURES NOTHING: all four rung pairs end 3:3 stocks at 0.0% —~~ "both survive BUT NEITHER LANDED A HIT". A fixture that produces no engagement cannot rank rungs, so its four rows are vacuous evidence and must not be read as "the ladder is flat". Diagnose the fixture before trusting any row of it ⭐ `crates/ambition_combat/src/brain/fighter/scenarios.rs` carries the fixtures and asserts its own playable count — *"only {playable} of {} scenarios can be played"* — and `rollout.rs` names `ladder_rig --scenarios` as a thing that stays green. So the row's *"MISSING SCENARIO HALF IS STALE"* note is itself confirmed rather than merely asserted. ▢ its two named remainders (fixtures needing state placement cannot make, and the movement-evaluation path) are what is left. |
...
| **D199 — projectiles resolve body damage BEFORE walls, and neither is swept** | ▢ **STILL REAL — re-verified at HEAD 2026-08-28**, and the only one of this review's five that is | ⭐ GPT review 2026-08-23, ⚠ NOT independently verified by me. `projectile/systems.rs` orders: move to endpoint → portal transit → victim loop (overlap, parry, damage, despawn) → feature overlap → `resolve_world_collision()`. So a shot whose new endpoint lands on a victim BEHIND blocking geometry damages them before anything asks whether a wall stopped the travel. ⇒ both tests are ENDPOINT overlap, not swept, so fast shots tunnel thin walls and thin hurt volumes; and the victim loop `break`s on the first qualifying row, so which victim is hit is query order rather than geometric first contact. ⛔ **do not fix by swapping the two blocks** — that trades one wrong answer for the opposite one when the body genuinely came first. The shape is: propose the motion SEGMENT, collect portal/world/body/breakable contacts along it, take the earliest time-of-impact with an explicit deterministic tie-break, resolve, continue only if policy says bounce or transit. ⇒ land the cheap regression NOW (projectile, wall, victim immediately behind it, victim takes no damage) and schedule the swept transaction when speed or content pressure warrants it ⚠ **CONFIRMED, not inherited**: in `projectile/systems.rs` the victim loop still `break`s on its first qualifying row around line 640-690 and `resolve_world_collision` is called at line 754 — so a shot whose endpoint lands on a victim BEHIND a wall damages them before anything asks whether the wall stopped the travel, and neither test is swept. ⛔ the row's own warning stands: swapping the two blocks trades one wrong answer for its opposite. ⭐ `ambition_platformer2d_core::cast::aabb_path_contacts` is the swept primitive the thrown-item road adopted on 2026-08-28 and is the same shape this needs, but the wall-vs-body ORDERING half is a restructure across portal/world/body/breakable contacts, not a call swap. ✔✔✔ **A SECOND DEFECT IS FIXED 2026-08-29 — THE WALL-VS-BODY ORDERING, and it is the first BEHAVIOURAL proof this row ever had** (its confirmation was a source reading). The cheap regression the row asks for now exists: `a_shot_does_not_damage_a_victim_standing_behind_a_wall` — projectile, solid wall, victim immediately behind — and before the fix it took the victim **4 HP → 3 straight through the block**. ⭐ **FIXED THE WAY THIS ROW PRESCRIBES, NOT BY SWAPPING THE BLOCKS:** the determinism fix already computes the motion segment (`leg_start`), so the ordering question is asked directly — `raycast_solids` from `leg_start` toward the victim, skip it only if a solid stands STRICTLY NEARER than the body. ⚠ **SOLIDS ONLY** (`include_one_way = false`): a fireball crosses a one-way from below by design, and treating one-ways as blockers would silently un-hit victims standing on ledges — the existing passthrough/bounce tests are what pin that choice. 40 projectile tests green, 1148 in the crate. ✔✔✔ **THE WALL TUNNELLING IS ALSO FIXED 2026-08-29.** `resolve_world_collision` is an ENDPOINT test, so a shot fast enough to end the tick past a thin wall never touched it — measured, 4000 px/s through an 8px wall left the body in flight. ⭐ Fixed WITHOUT changing that function: its eight test call sites pass an endpoint and test endpoint semantics correctly; what was wrong was handing it an endpoint that had already left the wall behind. The caller now raycasts the leg and, when the endpoint is not already touching a solid, PUTS THE SHOT BACK AT THE CROSSING — the endpoint test then sees what it would have seen at a slower speed, and bounce/expire/one-way passthrough are unchanged. The regression asserts it (1 surviving body before, 0 after). ⭐⭐ **AND THE SWEPT-VS-HURT-VOLUME HALF IS NOT REACHABLE BY CURRENT CONTENT — MEASURED 2026-08-29, which turns this row's own deferral condition (*"when speed or content pressure warrants it"*) into a NUMBER.** The fastest projectile authored is `HadoukenSuper` at **640 px/s = 10.67px per 60Hz tick** (Fireball 360 = 6.00px, Hadouken 520 = 8.67px). Tunnelling a **28px body needs >1680 px/s — 2.6x anything authored**; the **8px wall case IS reachable** at 10.67px/tick, which is exactly why the wall fix earned its keep and this one does not. ⇒ **re-open when a projectile exceeds ~1700 px/s or a hurt volume goes below ~11px.** ✔ **THE PRIMITIVE IT WILL NEED IS ALREADY ON DISK AND TESTED:** `CombatVolume::swept_aabb` (exact hull of the box at both ends of a leg) on a new `convex_hull` helper in `ambition_geometry`, with a test pinning the reason it is a HULL and not the cheap union of the two boxes — the union is right only for axis-aligned travel and invents hits on bodies a diagonal shot visually missed. ⛔ Its per-tick USE was reverted deliberately: a hull build per projectile per tick buys nothing content can produce. ⚠ **AND THE TEST THAT CHASED IT WAS REMOVED, NOT LEFT IGNORED** — it needed 10x authored speed, and above ~4000 px/s the shot EXPIRES in `game.tick` before the victim loop runs, a different outcome wearing the same symptom. Two drafts were checks that COULD NOT FAIL (one ended INSIDE the body; one ignored the fireball's own 12px half-extent so its endpoint box still reached the victim) — both caught by poisoning the fix. ~~WHAT REMAINS IS THE SWEPT-VS-HURT-VOLUME HALF, AND IT NOW HAS A FAILING TEST ON DISK:~~ `a_fast_shot_still_hits_a_victim_it_crosses` (`#[ignore]`d with its reason) — a 6000 px/s shot CROSSES a body in one tick and it takes NO damage. ⛔ The wall technique does NOT transfer: the victim test resolves against PUBLISHED DAMAGE VOLUMES and there is no swept-versus-volume test to call. ⛔⛔ And the shortcut is refused deliberately — sweeping the projectile AABB and testing the UNION of start and end boxes is exact only for an axis-aligned shot and OVER-approximates a diagonal one, so it would buy the fix by inventing false hits on bodies the shot visually missed, which is the exact trade this row warns against. ⇒ un-ignore that test with the swept transaction. ▢ ~~THE SWEPT HALF REMAINS AND IS NOW THE WHOLE OF THIS ROW — and the two halves are cleanly separated, MEASURED:** (a) the DAMAGE question is fixed AT ANY SPEED, because the raycast spans the whole leg and finds the wall regardless of velocity; (b) the TRAVEL question is still ENDPOINT — probed 2026-08-29 inside the regression: after ONE tick at **4000 px/s through an 8px wall, 1 projectile body REMAINS**, i.e. the shot passes through the wall while correctly failing to damage anyone behind it. ⛔ An earlier note of mine said a fast shot tunnels *"on a tick where the raycast finds nothing between the two points"* — WRONG, the raycast does find it; the tunnelling is in `resolve_world_collision`, which is the endpoint test the swept transaction replaces. ✔✔ **ONE OF THE THREE DEFECTS IS FIXED 2026-08-28 — the victim choice, which was the DETERMINISM half.** The loop `break`s on its first qualifying row and it iterated `&victims` directly: Bevy archetype order, which is not a promise and which a rollback resimulation does not reproduce — on damage, which is rollback-authoritative state. ⇒ ordered by geometry now: distance from where the shot began this leg, ties broken on the victim's own POSITION rather than its entity, because an entity index is not stable across a rewind and a position is (resimulation reproduces the same world). ⛔⛔ **AND IT WAS A LIVE GAMEPLAY BUG, not only a theoretical one** — with the ordering removed the test reports the FAR body taking a shot that passed through the near one, purely because it was spawned first. ⚠ the first version of that test placed the two bodies 40px apart and PASSED with the fix removed: at 50px/s a fireball covers 0.8px in a tick, so only one body was ever reachable and the loop had nothing to arbitrate. They are 8px apart now. ▢ **the two remaining halves are the sweep and the wall ordering**, and they are the restructure above. |
...
| **D257 — a demo capture inherits GAMEPLAY framing, so a small target loses its subject** | ▢ **FOUND and fully diagnosed 2026-08-29; ⛔⛔ FIRST FILED WRONG TWICE — as "capture_sanic writes a BLANK image" (an artifact of a size I chose) and then as a defect of "the demo captures" (it is SANIC-ONLY). Both corrections are below.** | ⭐ **MEASURED:** `capture_sanic` is correct at **1280x720**, puts the player at the extreme edge at its DEFAULT **960x540**, and shows **nothing but sky at 320x240** — reporting success at every size. ⛔ `capture_mary_o` and `capture_twintrack` render correctly at 320x240, and `mary_o` has the SAME `camera_ortho=0.5`, so it is neither the projection nor the pipeline. ⭐⭐ **MECHANISM, NAMED:** sanic declares `profiles::high_speed_full_bleed()` (`game/ambition_demo_sanic/src/provider.rs:61`) — *"soft velocity-aware framing … so the camera LEADS the runner instead of trailing it"*. The lead is a fixed world-space affordance; shrink the visible region and the subject leaves the frame. ⇒ **a capture inherits GAMEPLAY framing when it wants a PORTRAIT.** ⭐ **THE REPO ALREADY SOLVED THIS SHAPE:** `capture_scene` has `--fit-room` for exactly this reason — its own doc says the gameplay resolver is *"exactly right for gameplay and exactly wrong for a portrait of the room"*. The demo captures have no equivalent. ✔ **A GUARD LANDED 2026-08-29 (not a fix — the framing is a design call):** `capture_sanic` now checks the player against every active camera's viewport before shooting and prints `⛔ THE SUBJECT IS NOT IN FRAME` with the position; verified firing at 320x240 and silent at 1280x720. ⇒ the remaining fix is still a DESIGN call: give the demo captures a framing override like `--fit-room`, or document a native-size-only contract. ⚠ **A COORDINATE-SPACE TRAP THE GUARD'S MESSAGE NOW NAMES:** the reported position reads `(-3040,-297)`, which looks like a player who escaped a level spanning 0..6400 — but engine world space CENTRES the level on the origin, so 6400 wide spans -3200..3200 and -3040 IS the LDtk spawn at px=146. That nearly became a fourth wrong claim on this row. ⛔ Do NOT 'fix' the camera lead — it is deliberate and it is the whole feel of the speedway. |
| **D256 — RUNTIME EFFICIENCY CAMPAIGN (Jon, 2026-08-29)** | ◐ **MEASURED OUT. Read the one-screen summary at the top of [`engine/performance-and-iteration.md`](engine/performance-and-iteration.md) before touching this.** | ⭐⭐⭐ **THE ANSWER: SMASH IS NOT SLOW AND ITS SPIKES ARE NOT THE ENGINE.** On a quiet machine a 2-fighter match is a **4.31ms mean against the 16.67ms 60Hz budget** and **ZERO of 5,164 match frames exceeded it** (worst 10.56ms); the tail is CONTENTION — six busy loops take frames >8ms from **0.9% to 11.8% (13x)** while the median moves 6.8%. ⛔ Several "dropped frame" readings during this campaign were the measuring session's OWN builds. ⇒ **no fundable frame-time lever exists here, and that is a measurement, not a shrug** — what the work produced is a COST MODEL, an OWNERSHIP MAP and instruments that refuse to lie. ⭐⭐ **THE FRAME IS BROAD, NOT DEEP: ~630 systems at 2.9–15.6us each and NO hot system.** A 2-fighter Smash match is 4.5–5.0ms (`dev`) / 2.8–3.2ms (`profiling`); the whole gameplay simulation is **0.93ms across 17 phases**, largest `WorldPrep` at 0.22ms. ⛔⛔ **MOST OF THE STARTING PREMISES DID NOT SURVIVE — do not re-run them:** ⭐ **CORRECTED 2026-08-29 by sweeping ALL 72 ROOMS:** the sprite-heavy room is REAL — `mockingbird_arena` bursts to **295 visible sprites for ~1 second** — but it costs **+0.36ms of mean (4.9%)** at 1.40us/sprite, and the damage is in the TAIL (worst frame 11.66 → **17.30ms**, past the 60Hz budget), not throughput. The old "46–87 visible sprites" claim was a FOUR-ROOM SAMPLE of rooms AT REST and is retired;  removing FOUR whole experiences moved neither frame time NOR startup registration; presentation churn on real content is **55 of 2515 transforms**; `world_rendering=1` so Smash draws the world once; rollback skips saving entirely at `check_distance: 0`; and entity population failed to predict cost in **three separate probes**. ⭐ **THE ONE LEVER THAT WORKED:** `gameplay_allowed` hoisted from **83 evaluations per run to 1** — it worked because it retired a WHOLE CLASS at once, and gating a set off is calibrated to reclaim its systems' actual work (0.95ms shed against 0.93ms measured). ⛔⛔ **THE INSTRUMENT RULE THAT COST A RETRACTED FINDING:** `[census] phases` attributes WALL TIME between markers, so GPU blocking lands in whichever phase brackets it — raising a render target 320x240→1280x960 took `StateTransition` from 0.169 to **1.822ms**, a state phase scaling with PIXELS. Phase splits are valid ONLY from non-rendering runs, and `fragment_shader_invocations=0` does NOT make them safe. ▢ **WHAT IS ACTUALLY OPEN, in order:** (1) a real-hardware profile — `./scripts/profile_desktop.sh --smash` on a GPU machine, because every number here is from a GPU-less host; ⭐⭐ **THE SMASH FRAME IS NOW FULLY ATTRIBUTED (4.45ms):** 0.83ms marked gameplay sim, **0.21ms GGRS driver overhead** (cheap, as `check_distance: 0` predicts), **0.93ms `PreUpdate` OUTSIDE the driver — a FIFTH of the frame and the largest unexplained block**, 1.23ms `Update`, 0.51ms `PostUpdate`, 0.31ms `RunFixedMainLoop`. ⛔ `PreUpdate` is NOT "the simulation tick". ⭐⭐⭐ **A FIGHT COSTS PHYSICS, NOT COMBAT LOGIC** — interleaved within-run A/B (n=401): combat-active frames cost ~150-250us more, **58% of it `RunFixedMainLoop`** (the fixed-timestep/physics loop), while the whole gameplay sim moves **+0.003ms** and the `Combat` phase **+0.004ms**. ⭐⭐⭐ **AND SMASH'S FRAME SPIKES ARE NOT IN THE SIMULATION AT ALL** — 340 intervals quartile-split by worst frame: sim total +1.4% while frame max goes 4.64 → 8.28ms ⇒ making the sim cheaper is worth ~zero for RESPONSIVENESS (still correct for throughput). ⭐⭐⭐ **AND THE SPIKES ARE NOW ANSWERED — THEY ARE CONTENTION, NOT THE ENGINE:** frames >8ms become **8.6x more common** under CPU load (1.1%→9.5%) while the median moves 5%, and on an IDLE machine **ZERO of 15,747 frames exceeded the 16.67ms budget** (worst 13.66ms). ⛔ Earlier "dropped frame" readings were this session's own concurrent builds ⇒ **record MACHINE LOAD beside every frame number.** ⛔ The phase guard was FIXED: it warned on the CAMERA COUNT, but `NoWindow` omits the RenderApp, so it condemned the sound runs; it now tests for `RenderDevice`. (2) ✅ **CLOSED — the room is `hall_of_characters`**, and the cost is SIM not RENDER: `WorldPrep` **2.373ms** against 0.269ms in an ordinary room, scaling with BODIES (130 vs 1) and NOT with entities — two 4-body rooms differing 30% in live entities agree to 1us. ⇒ the engine's per-body sim price is **~16us**. ⛔ It funds no optimisation: it is a GALLERY room, and a Smash match's 8 bodies costs ~0.13ms;  (3) ✅ **CLOSED — `Update` (1.23ms over 497 systems) and `PreUpdate`'s non-driver 0.93ms are BREADTH, not hot spots:** 2.4us and 6.9us per system, both inside the measured 2.9–15.6us band ⇒ no group hides a millisecond, and the only lever on a frame this shape is retiring a WHOLE CLASS. ⭐⭐⭐ **THE FRAME'S OWNERSHIP IS MAPPED:** the SIM is OURS (545 systems, `ambition_platformer2d_actor_monolith` = **162 = 30% of the simulation**, a number the decomposition can be planned against) while **`PreUpdate` (137) and `PostUpdate` (169) are MOSTLY BEVY'S** — our code owns ~20 of 137 and ~5 of 169. ⇒ **optimisation effort should go where AUTHORSHIP is**, and the 31% of a fighter that lands in `PostUpdate` CANNOT be optimised by editing our code. ⭐ **COST MODEL:** ~4.5ms baseline · **~125-240us per FIGHTER** (2 reps; the SHARES are stable — ~1/3 lands in `PostUpdate` — the MAGNITUDE is not, and the only thing that scales with a player's choice) · ~16us per body · ~1.4us per visible sprite. ⛔ **A fighter is already LEAN — 8 entities and 1 sprite** (measured at go-live, zero variance), so "fewer entities/sprites" is not a lever; fighter entities are EXPENSIVE, not numerous. ✅ **INPUT LATENCY is correct by construction** — `bevy_ggrs` orders `RunGgrsSystems.after(InputSystems)`; ⛔ do not add a duplicate, and true device-to-sim latency cannot be measured headlessly (the latch needs device authority). ✅ All six original brief directions are CLOSED or PRICED. ⛔ **DO NOT fund runtime capability composition as a PERFORMANCE migration** — it is architecture and startup work, both were measured, and neither moved. Landed: 9 changes, only ONE with a measured speed claim, plus a softlock fix; 16 investigations recorded, 12 of them rejections. |
| **D189 — the demo shell's catalog is smaller than the app's, so the rigs can measure three fighters** | ◐ **ITS STATED PREMISE IS STALE — re-measured 2026-08-28** | ⭐ **CORRECTED: it is not two seating paths disagreeing.** Both use the same `smash_roster_at_levels`; what differs is the APP. `app_it`'s duel guard builds the FULL app (`build_visible_app`) and seats `npc_pirate_admiral` fine; the rigs build the demo shell (`build_demo_app`), whose catalog carries `smash_george_booul` and the two stand-in duelists and nothing else. A character it does not carry seats no fighter, so `ladder_rig` aborts on its noise-seed guard and `match_report` used to print headers over zeros. ✔ the report now names the ids it has and exits 2 instead. ⚠⚠ **THE CAP IS REAL AND UNCHANGED**: every CPU number on this run is George or a duelist, and D188's regression was found on a character the demo shell does not carry. ⇒ the row is whether the demo should carry more of the roster, or whether the rigs should compose the full app. ⭐⭐ **AND IT HAS NOW COST TWO FINDINGS IN ONE DAY, which settles that it is worth paying for.** Both lanes proved a change live in this shell, correctly, and both missed a regression that exists only in the full app: the respawn interval reads clean on George and costs `npc_pirate_admiral` two-thirds of its damage (D194), and D190's perception bound only bit characters this shell cannot seat. ⇒ ✔ PARTIAL 2026-08-23: `match_report` now prints its COMPOSITION and the three ids it carries on every run, so a proof carries its own scope and a reader who quotes the number at the shipped roster is making a claim the header already refused. ⛔ that is honesty, not a fix — the rigs still cannot measure the shipped grid. ⚠ the dep arithmetic decides the real answer and neither arm is free: `ambition_demo_smash_app` does not depend on `ambition_content` (which is where `AmbitionContentPlugin` registers the roster) and adding that edge drags dialogue, falling sand, items and presentation into a rig; composing `ambition_app` instead is the heavier edge and may be circular. ⇒ a third arm worth pricing: move the rigs INTO `ambition_app`. ✔✔ **PRICED 2026-08-29, AND IT IS ALREADY PAID FOR.** The dep arithmetic this row calls unfree is ALREADY SATISFIED by `game/ambition_app_tools`, which depends on **`ambition_app` AND `ambition_content` AND `ambition_demo_smash` all three** — so moving a rig there costs **ZERO new dependency edges**. ⭐ And the pattern is not hypothetical: `bin/smash_match_profile.rs` already composes `build_visible_app` + `smash_roster` and drives a LIVE MATCH in that crate. ⭐⭐ **THE GAIN IS MEASURED: 3 → 21.** That bin now prints `composition_can_seat=` from the runtime `SmashRoster`, and the full app reports **21 ids including `npc_pirate_admiral`** — the very character this row says the rigs cannot reach, and where D194's regression was found — against the demo shell's THREE. ⇒ the remaining work is porting `match_report`/`ladder_rig` into `ambition_app_tools`, NOT resolving a dependency question; that question is answered. ✔ **AND THE CAPABILITY IS PROVEN, 2026-08-29:** `smash_match_profile --character npc_pirate_admiral` now seats 2 fighters in the FULL app — the exact character this row says the rigs cannot reach — and a bogus id aborts naming the 21 carried ids instead of blaming the ceremony. ⚠ **BUT THAT BIN DOES NOT REPLACE `match_report`:** it profiles FRAMES, not per-move damage/hitstun tallies. ⭐ **THE PORT IS SIZED:** `match_report.rs` is **842 lines** — roughly 400 of pure PRINTING over a plain `Tally` struct (~30 count fields), ~200 of COLLECTION that reads engine components, and ~100 of app driving. **The collection depends on `ambition_platformer2d`, which BOTH crates already carry — not on the demo app** — so it is portable. ⇒ the clean shape is to lift `Tally` + collection + printing into `ambition_demo_smash` (the crate both sides depend on) and leave two thin bins: one composing the demo shell, one composing the full app. ⛔ Do it deliberately with its own tests; it is a real refactor and not a drive-by. ⛔⛔ **THAT LAST NOTE CONFLATED TWO DIFFERENT LISTS, AND THE ROW'S STATUS WAS AMBIGUOUS BECAUSE OF IT — MEASURED 2026-08-29.** `SMASH_ROSTER` is what the SELECT GRID NAMES; the demo shell's ASSEMBLED CATALOG is what it can actually SEAT, and `SmashRoster` is literally the first FILTERED TO the second at `Startup`. Read out of a built demo app, the shell carries **exactly THREE** — `smash_george_booul`, `smash_duelist_a`, `smash_duelist_b`. ⇒ **THE CAP IS REAL AND UNCHANGED; only the grid grew.** ✔ And `match_report`'s composition header now READS that resource instead of naming three constants beside it (as does its unseatable-character abort), so the claim cannot go stale again. ⚠ The superseded note follows: **THE DEMO SHELL'S CATALOG IS NOT THREE ROWS ANY MORE.** This says it *"carries `smash_george_booul` and the two stand-in duelists and nothing else"*. `SMASH_ROSTER` now names twenty-one — the automaton, the goblin, Noether, Stargan, the patent clerk, four polygons, the Author, the Actor, the Officer, the Medic and the rest — with the two stand-ins LAST and `roster_for` intersecting the list against what the composition can actually seat. ⇒ re-measure the rig gap against that before treating it as a catalog-size problem; the shape of the answer may have changed with the roster. |
