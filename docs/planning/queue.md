...
| **D200 — three branches hold D192's respawn interval unmerged, and the note that holds them has decayed** | ✔ **ANSWERED 2026-08-25 by review: verdict RE-DO on current main** | ⭐ all three branches are now pushed (`respawn-interval-holding`, `d194-and-respawn-verified`, `attrib-beat-only`) and are **REFERENCE ONLY, not merge candidates** — salvage from them the intended behaviour, the useful tests, the measurements and the KO-attribution lesson; do NOT mechanically port schema-73 registrations or old staging shapes. ⇒ D194 closed, D192 re-staffed for reimplementation; delete the three branches once the replacement lands and passes the repaired-mirror guard with the interval enabled |
| **D201 — D192's respawn beat should be a DEATH INTERLUDE (ADR 0033), not a body left lying in the blast zone** | ▢ unstaffed — **MECHANICS, small and principled**; D192 landed without it deliberately | ⭐ a `PendingRespawn` body is never moved, so a ring-out leaves it outside the stage for the whole beat. That single fact caused TWO separate defects during D192: the blast zone knocks it out again every tick and spends every remaining stock (fixed with `Without<PendingRespawn>` on the spend), and the repertoire tests' positional `offstage` proxy counted 220 respawn-wait ticks as recovery opportunity. ⇒ the repo ALREADY has the concept: `OutOfPlay` + `DeathInterlude` (ADR 0033), *"the open window between a participant's death and its consequence"*, whose whole job is keeping the world's hands off the body while it is open — and `damage_apply`, `actors/update` and `world/rooms/systems` already respect it. ⚠ adopting it changes behaviour in those three consumers, which is why it is its own row. ⭐ Jon 2026-08-25 saw the adjacent symptom: *"in smash when you are respawning, currently if I make the character jump they raise up on the platform"* |
