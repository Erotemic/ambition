...
| AP6 | Wrong-owner body state | ✅ DONE 2026-09-24 | Three wrong owners, three commits. (1) `BodyLifetime` (W012) → `BodyRestartLatch` (the replay latch, `body.restart_latch`) + `BodyLifeStats` (`time_alive`, `resets`; unregistered diagnostic, advanced by `track_body_life_stats` from the latch just before `announce_body_restarts` consumes it; waived in `rollback_coverage`); `max_speed` deleted. (2) `BodyComboTrace` (W011) left the kernel: no cluster member, no kernel parameter (`op_clusters` deleted; ops go only to `FrameEvents::operations`), no rollback row; the app's `ComboTrace` (`app/combo_trace.rs`) is optional HUD state fed from `PlayerBodyFrameOutput`, skipped on replayed ticks, restarted by a `BodyRestarted` observer. Cluster populations did not move: every remaining member ships in `AncillaryMovementBundle`. (3) `BodyMelee::ranged_cooldown` (W022) → `RangedRefire` (`actor.ranged_refire`), spawned beside `BodyMelee` at every site (seed bundle, player bundle, boss bundle), ticked by `tick_ranged_refire`; `weapon_ready`, `try_fire`, the moveset spend and the prompt read it. Schema v208/v210/v211. Witnesses: `a_restart_is_counted_once_and_restarts_the_clock`, `the_readout_ages_out_and_keeps_its_reset_mark`, `a_recharging_weapon_refuses_the_firing_move_and_acceptance_spends_it` (re-pointed). | `BodyMelee::cooldown`/`pending_axis`/`swing` (AP11/AP12 rows) are this lane's next BodyMelee items. |
| AP7 | Explicit fallback policy | OPEN | `default_provoked_policy()` (W026) and `UNDESCRIBED_BODY_RESPAWN` are ruleset choices the engine makes when composition is silent. `default_player_action_set` seeds `PlayerSimulationBundle::from_scratch` with the robot's kit and `ChargesProjectiles`, which the one production spawn overwrites in the same command batch (construction repair, unobservable today). | Move the choice into preparation as an explicit decision the engine consumes. `peaceful()` for an unknown id is an engine safety law and stays. Census 2026-09-24 (from AP12), `CombatTuning`/`ActorConfig::sprite_character_id` behind `WornCharacter`: every road that inserts `WornCharacter` writes the config id from the same character (NPC road `npc_character_id`, seats, migrated enemies and encounter mobs via their definition), so where both exist they agree. The `.or_else(sprite_character_id)` arms (moveset strike attribution, presentation source, hit barks) are LIVE for one road: `spawn_runtime_minion_into` builds through `new_character_in` (config id set) and inserts no `WornCharacter`, so a runtime minion's identity exists only on the config. (`demand_actor_character_sheets`'s doc says an `EnemySpawn` wears nothing; stale: `spawn_enemy_with_faction_into` inserts `WornCharacter`.) Making `WornCharacter` the one authority therefore starts by having the minion road wear its character. The only runtime `WornCharacter` writers are player bodies (Sanic's super form, Mary-O's power-ups). Latent split: the actor render index (`sim_view/view_index.rs`) reads the CONFIG copy, so the first actor that changes persona at runtime draws its old sheet; retire the config field when that road appears or move the index onto `WornCharacter` first. |
| AP8 | Bounded C07 audit | OPEN | Per-site classification of optional-authority reads: optional capability (keep), required authority absent (queue), global fallback (suspicious). Mechanical/session authorities first. Walked 2026-09-24, `ControlledSubject` (the largest session-owned member, 13 optional sites): it is a derived projection of `DrivingParticipant`, resolved at the head of the input stage (`resolve_controlled_subject`), so `None` is not a construction lag; it means nobody holds the primary seat. Presentation readers (`sim_view` facts/prompts/proximity) show nothing on `None`: keep. `sandbox_reset`, `held_items` and the Sanic/Mary-O rules also treat `None` as "no subject": keep. THREE sim readers substitute the primary player instead: the room-transition trigger (`world/rooms/systems.rs`, `.or_else(primary_q.single())`), `controlled_frame_down` (`control/queries.rs`), and the portal input adapter's `primary_fallback`. That turns "nobody is driving" into "the home body is driving", which is a policy the composition should state (a seatless harness, a match with no local seat), not one the engine should infer. Needs a census of which shipped compositions reach `None` with a primary body before choosing between refusing and declaring. The checkpoint baselines (`OccurrenceBaseline`/`CustodyBaseline`, 5 sites each): both restore roads in `session/checkpoint.rs` pin `unwrap_or_default()` (an EMPTY ledger) when the baseline is absent, which would restore "nothing ever occurred". Inert today, not a fail-open: the pinned value's only reader, `restore_occurrence_baseline`, is installed by `LifecycleCheckpointHorizonPlugin` together with both resources, so the default is pinned only where nothing applies it. Items already pin `Option` (`minted.zip(owned)`); pinning the other two the same way would make the absence explicit instead of relying on that co-installation. `MovingPlatformSet` in `CollisionWorld` (absent ⇒ no platforms): keep; the simulation foundation (`sim_core_resources`) always initializes it and `body_integration` requires it, so only minimal test apps reach the `None` arm. | Population and method are C07's (`consolidation-plan.md` §7); record findings here. |
...
| AP15 | Morph Ball entitlement | OPEN (Jon, 2026-09-24) | `PlayerSimulationBundle` spawns every home player with `BodyModeCapabilities::full()` (crouch + morph + climb) before identity or experience matter, so Mary-O (authored `RunJump`) can morph. `BodyModeCapabilities` is a second capability authority beside `AbilityBase` → `BodyAbilities`; `AbilitySet`/`AbilityGrant` have no morph; it is declared rollback-derived but no production projector exists (its one construction is the hardcoded `full()`). Morph entry is a double-tap read directly by `update_body_mode`; presentation is a bespoke sibling sprite with documented visibility/order battles. | Product rule: the Ambition protagonist gets Morph Ball in the Ambition game only — not Mary-O, not Sanic, not V3 seated in Versus — decided by game/content composition, never an engine id check. Split ENTITLEMENT (effective `BodyAbilities`, fed by intrinsic `AbilityBase` + experience/equipment/temporary grants + restrictions) from EXECUTION (body-mode transition, collision-safe), STATE (`BodyModeState`) and PRESENTATION. Morph Ball is the second real customer the capability-progression plan waited for (after falling-sand's temporary Swim): build the smallest contribution/projection that serves both, not a GAS framework. Do not move it into `ambition_abilities` (wielded family) or move it for SCC effect (AP14: `body_mode` is outside the 10-cycle). Delete `BodyModeCapabilities` or reduce it to what is truly structural once crouch/climb are classified. Witnesses via real composition roads: Mary-O in Mary-O cannot morph; the protagonist in Ambition can; the protagonist in an experience that grants nothing cannot; one collision-safe entry/exit. Audit possession, prompts, and whether the morph sprite's ownership can become one read-model projection. |
...
| AP11 | Per-tick mirrors in canonical rows | OPEN | `ActiveRoomMetadata` ← `RoomSet::active_metadata()` (`rooms/systems.rs:37`); `RoomMusicRequest` ← that metadata (audio only); ~~`SwitchOn`~~ ✅ deleted 2026-09-24 (schema v212; it was also spawned `false` until the first mirror, a construction repair — readers take the save by the activation id, which is the key the activation writes); `BossAttackState` ← `MovePlayback` (KEEP as a snapshotted row: the brain reads last tick's projection before the projector runs, so it cannot be derived; its second writer, `tick_commanded_moves`, now clears the brain's INTENT as documented — a lured boss used to keep starting attacks); `MovesetMelee`/`MovesetRanged` ← the moveset's verbs; `BodyCombat::armor` ← `MovePlayback::armor_now` (KEEP: a projection written by `project_move_defense_windows` before its one reader, `apply_body_hit_reaction`, and restored consistent with `MovePlayback`); `GravityField` ← the primary body's resolved frame. | Per row: delete and read the authority, or declare it derived (re-projected on restore, as `SimDt`). A row read before its projector runs needs the re-projection, not deletion. Measured 2026-09-24: `MovesetMelee`/`MovesetRanged` are a projection of `ActorMoveset`'s verbs kept by `reconcile_moveset_routing_markers` (Changed) plus spawn seeds; deleting them needs `ActorMoveset` readable from `ambition_characters` (`emit_brain_action_messages`), i.e. the component moves down beside `MovesetContract` (entity_catalog) and its `#[require(BodyStaleMoves)]` must be restated. `GravityField` (canonical resource) mirrors the primary body's derived `ResolvedMotionFrame`; `GravityCtx` falls back to it when `GravityZones` is absent and the host/content portal transit read it — audit their order against `resolve_active_gravity` before declaring it derived. Audit 2026-09-24: the zone-aware sim readers (`GravityCtx`, `sync_sprite_posed_bodies`) use `GravityZones` and fall to the field only when zones are absent (tests, headless); the camera-continuity reader is presentation. The finding is `apply_portal_carried_momentum` (content transit adapter): it takes the run axis for EVERY transiting body from the primary body's field, so an actor crossing under a different zone splits its carried run on the wrong axis. Its own `ResolvedMotionFrame` is the authority, but that frame was resolved at the pre-transit position, so the fix has to decide which side of the transit the axis belongs to. |
| AP12 | Dead or constant rollback state | OPEN, first batch done 2026-09-24 | ~~`LocalPlayer`~~, the ~~`PlayerSlot` component~~ (the type stays as a value; slot identity is the `SimId` the identity bundle mints) and ~~`EncounterRegistry::ids`~~ (a write-only index; the resource is now the populate latch) are deleted, schema v214. `RegimePolicy`, `FactionRelations` and `FriendlyFire` never change in production. `CombatTuning::sprite_character_id` duplicates `ActorConfig::sprite_character_id`, and `ActorConfig.tuning.weight` goes stale once a persona change rewrites `CombatTuning::weight`. | Delete dead rows and fields. `BodyMelee` fields (`cooldown`, `pending_axis`, and AP11's `swing`) moved to CalculexAmbition with AP6.3. A never-changing resource is either a declared ruleset constant or goes. Verdicts 2026-09-24: `ActorConfig.tuning.weight` is construction input only (runtime reads `CombatTuning::weight`, and a persona swap moves weight through `PhysicalBaseline`/displacement), so not a live second truth. The `FriendlyFire` RESOURCE is a designed world-authored baseline that `project_combat_rules` folds declared match rules over (`resolved_combat_tuning.rs`, `versus.rs` tests pin the seam); no production writer yet, KEEP. `FactionRelations` is the same shape: every `set_hostile` outside `Default` is a test, and `preparing_a_candidate_does_not_reset_the_live_sessions_faction_relations` makes it non-default on purpose to pin candidate isolation; KEEP. `RegimePolicy` is ADR 0010's clock-permission seam (non-Solo regimes are designed, not yet selected by a host); KEEP. `CombatTuning::sprite_character_id` duplicating `ActorConfig::sprite_character_id` sits behind `WornCharacter` in a fallback chain: needs a census of bodies with no `WornCharacter` before deciding, so it is AP7's. |
| AP13 | Presentation state in the rollback kernel | ✅ DONE 2026-09-24 | Two deletions: `PlayerBlinkCameraState::blink_camera_to` (written by the room commit and the blink, never read; the other two fields are live) and `EncounterDef::placement_id` (always `config.id`, the same value as the sibling `Encounter::id`; the debug HUD and tests read `Encounter`). The rest stay canonical, for reasons that are not presentation: `BodyAnimFacts` and `BossDeathAnimation` are armed by sim events and ticked on the sim clock, so an unrestored timer is re-ticked and re-armed by the resim (`BodyPoseClock`, the gameplay pose, does not read them); `ActorRenderSize`/`ActorSpriteOffset` are re-projected by `sync_sprite_posed_bodies` through `&mut`, so an entity a rewind re-creates without them is never posed again; `PickupArt` is construction data a re-created pickup must carry; `EncounterMusicRequest`'s priority tier is a claim latch written by one-shots, and an unrewound claim is the stale-track bug `music.rs` documents; `BodyCombat::training_dummy` is the one copy of `practice_target` (AC6.2 removed the other). | — |
