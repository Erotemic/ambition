...
| **D257 — a `capture_*` frame LOSES ITS SUBJECT as the target shrinks** | ▢ **FOUND 2026-08-29; ⛔⛔ FIRST FILED WRONG AS "capture_sanic writes a BLANK image" AND CORRECTED THE SAME HOUR — the original claim was an artifact of a size I chose** | ⭐ **WHAT IS ACTUALLY TRUE, measured across three targets on `capture_sanic`:** at **1280x720** the shot is correct (Sanic, the rings, the cyan speedway, a spike platform); at the tool's DEFAULT **960x540** the level renders but the player sits at the extreme bottom-left EDGE; at **320x240** the frame contains NOTHING BUT SKY. ⇒ **the framed region does not track the subject as the target size changes**, so a smaller capture silently drops the thing it was taken of. ⛔ It reports success at every size. ⚠ `capture_scene` (the app-side tool) does NOT share this — it renders `sanic_sandbox` correctly at 320x240 — so this is the demo captures' camera framing, not the engine's. ⭐⭐ **AND THE CORRECTION IS THE LESSON:** three eliminations were run against the WRONG premise before the size was varied — not timing (identical bytes at `--warmup 1..600`), not the route, not the spawn (`PlayerStart` at (146,626) sits on ground among 1279 solid collision cells, with no `DamageVolume` within 400px), and a `lethal blow` log line that looked like an instant death is a one-time EXPLANATORY info message from the mount system. ⛔ Vary the INPUT YOU CHOSE before investigating the subject. |
