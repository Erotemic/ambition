...
5. **THE P0 FLAKY `workspace` JOB IS STILL OPEN — AND IT IS NO LONGER
   ...
   *"only 48 frames had two seated bodies"* — **a fourth name in a fourth file,
   ...
   gate. The next probe is to loop it and get a RATE. The standing instruction is
   unchanged and now has a precedent: record the test name, the assertion, **the
   SHA and the machine.**
