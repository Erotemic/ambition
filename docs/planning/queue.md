...
## ✅ RESOLVED BY A PEER: the `app_it` red is green at `c98e88ae1`

`two_cpus_in_the_shipped_composition_damage_each_other` passes again —
`-p ambition_app --test app_it` is **653 passed, 0 failed** on 2026-09-14 after
merging `8c102308a` *"Un-ship three throw values measured by an instrument that
cannot see its own blind spot"*. It had been stable-red across four runs, so this
is a fix rather than the flake it never looked like. The account below is kept
because the way it was attributed is the reusable part.

## ⛔ (RESOLVED — see above) MAIN WAS RED AT `158b442be`: `two_cpus_in_the_shipped_composition_damage_each_other`
