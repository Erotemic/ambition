...
| **D187 — the fighter brain presses once, so every chain and flurry is human-only** | COORDINATOR lane | ⭐ the jab chain and the held flurry landed 2026-08-23 and a human uses both every match; a CPU throws **zero** `jab2`. A chain needs a second undirected press inside ~0.14s on a jab that CONNECTED, and the brain emits one press per decision cadence and never sets `melee_held` at all — the same gap the smash charge had before `charge_hold_ticks`. ⛔ this is a MECHANISM the brain lacks (a follow-up press keyed on a landed hit), not a score to re-price. ⚠ the brain also cannot see that its own attack connected; that fact has to reach perception first |
