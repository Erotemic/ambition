...
| AP48 | The collision overlay is room-scoped and does not know its room | OPEN (2026-09-26, found by AP47) | AP47 ordered the overlay before its readers in `WorldPrep`, so a frame integrates against its own overlay. It did not make the overlay a statement about one room. On the commit frame, every system after `RoomTransitionSet::Apply` (combat, projectiles, and the commit's own arrival ground probe, `commit.rs:1309`) composes the NEW room's geometry with the overlay of the room just left, until the next rebuild. `CollisionWorld::solids()` cannot tell. Plan: stamp the overlay with the room it was built for and let the commit retract it in the same transaction (the retraction rule `retract_gated_lock_wall_verdicts` already states for one field), or have `solids()` refuse an overlay stamped for another room. Also measured and not fixed: (1) the overlay's contributors stay unordered among themselves, so block order in the overlay depends on the executor; collision is expected to be order-free, but no test says so; (2) a rebuilt body stands still for its first live tick (vx=0 at tick 24), and `canonical_reconstitution`'s `POSITION_TOLERANCE` (2.0 px, "one frame of the fastest patrol") absorbs that one-tick lag rather than naming it. |
