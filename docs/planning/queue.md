...
|---|---|---|---|---|
| AP0 | Driven provocation leaves a stale `Brain` | OPEN, blocker (review of `3b5885382`) | `provoke_actor_in_place` installs the provoked `Brain` only when the body has no `DrivingParticipant`, but still flips disposition and `BrainBinding.source`. `DrivingParticipant` only suppresses execution (`actors/update.rs:472`, the one production brain tick), and release only removes the driver (`control/authority.rs`); nothing rebuilds the brain from the binding. So a body provoked while possessed resumes its PEACEFUL brain under a hostile disposition. | Insert the provoked brain unconditionally; rewrite `provoking_a_player_driven_body_changes_its_mood_and_not_its_driver` to assert the driven body carries the provoked mind and keeps its driver. |
...
| AP4b | Wave mobs write permanent death flags | OPEN, measured | Wave mobs keep their character's `RespawnPolicy` (default `DeadStaysDead`), so each death writes `enemy_encounter:<spec>:w<n>:<k>_dead` (`actor_hit.rs:693`). The only reader of that flag is placement construction (`construction/mod.rs:287`), and a wave mob is not a placement, so the flag is never read: not the summon poison, but unbounded junk in the save. | A non-placement body persists no death: set the wave road's policy to `OnRoomReenter`, as the summon road already does. |
...
| AP6 | Wrong-owner body state | OPEN (2 of 3 done) | ✅ `BodyLifetime` (W012) split: `BodyRestartLatch` (the replay latch, rollback row `body.restart_latch`) owns the announcement; `BodyLifeStats` (`time_alive`, `resets`) is an unregistered diagnostic advanced by `track_body_life_stats` from the latch just before `announce_body_restarts` consumes it, and waived in `rollback_coverage`; `max_speed` deleted. Schema v207. Witness: `a_restart_is_counted_once_and_restarts_the_clock`. ✅ `BodyComboTrace` (W011) left the kernel: no cluster member, no kernel parameter (`op_clusters` is gone; ops go only to `FrameEvents::operations`), no rollback row (v210). The app's `ComboTrace` (`app/combo_trace.rs`) is optional HUD state fed from `PlayerBodyFrameOutput` after the step, skipped on replayed ticks, restarted by a `BodyRestarted` observer. Cluster query populations did not move: every remaining member ships in the same `AncillaryMovementBundle`. Witness: `the_readout_ages_out_and_keeps_its_reset_mark`. `BodyMelee::ranged_cooldown` (W022): the live ranged refire floor, owned by the melee type. | Give ranged cadence its own owner. |
...
| AP10 | Semantic actor-monolith SCC decomposition | CONTINUOUS (Jon, 2026-09-24) | The largest textual SCC is 10 modules — `abilities avatar character_runtime construction control features items projectile session world` — plus `assets ↔ character_sprites` (measured 2026-09-24 at `2c95a83b9`). `shrine` left through a real ownership move (checkpoint restore → session lifecycle); `avatar` and `character_runtime` later joined, so the count is not monotone. Single-edge cut scores (`abilities→features` 10→6, `control→abilities`, `avatar→control`, `character_runtime→avatar`, `features→projectile`) are CLUES, not objectives: `abilities→features` is the puppy-slug gun asking the actor domain to spawn a minion, and inverting it only for the graph is rejected ([`actor-monolith-decomposition.md`](engine/actor-monolith-decomposition.md), [`actor-monolith-work-frontier.md`](engine/actor-monolith-work-frontier.md)). | Every AP item asks: does the dependency this seam touches exist because the consumer owns the knowledge, or because an authority/adapter is misplaced? When the owner is clear, move state + behavior + installation/lifetime together; delete the obsolete edge (no callbacks, no compat re-exports). After each coherent migration run `python3 scripts/measure_kernel_module_graph.py --scc --cuts --edges 80` and record what fell out here. Target: a small residual body-integration kernel whose remaining cycles are genuinely inseparable simulation, not a number. Queue new decomposition rows as cleanup reveals misplaced responsibilities. |
| AP11 | Morph Ball entitlement | OPEN (Jon, 2026-09-24) | `PlayerSimulationBundle` spawns every home player with `BodyModeCapabilities::full()` (crouch + morph + climb) before identity or experience matter, so Mary-O (authored `RunJump`) can morph. `BodyModeCapabilities` is a second capability authority beside `AbilityBase` → `BodyAbilities`; `AbilitySet`/`AbilityGrant` have no morph; it is declared rollback-derived but no production projector exists (its one construction is the hardcoded `full()`). Morph entry is a double-tap read directly by `update_body_mode`; presentation is a bespoke sibling sprite with documented visibility/order battles. | Product rule: the Ambition protagonist gets Morph Ball in the Ambition game only — not Mary-O, not Sanic, not V3 seated in Versus — decided by game/content composition, never an engine id check. Split ENTITLEMENT (effective `BodyAbilities`, fed by intrinsic `AbilityBase` + experience/equipment/temporary grants + restrictions) from EXECUTION (body-mode transition, collision-safe), STATE (`BodyModeState`) and PRESENTATION. Morph Ball is the second real customer the capability-progression plan waited for (after falling-sand's temporary Swim): build the smallest contribution/projection that serves both, not a GAS framework. Do not move it into `ambition_abilities` (wielded family) or move it for SCC effect (AP10: `body_mode` is outside the 10-cycle). Delete `BodyModeCapabilities` or reduce it to what is truly structural once crouch/climb are classified. Witnesses via real composition roads: Mary-O in Mary-O cannot morph; the protagonist in Ambition can; the protagonist in an experience that grants nothing cannot; one collision-safe entry/exit. Audit possession, prompts, and whether the morph sprite's ownership can become one read-model projection. |

| AP10 | Control state stored twice | OPEN, class 1 | `ScriptedControl` is present exactly when `ControlHolds` is non-empty (both rollback rows; `ambition_characters/src/control.rs:500-541`, its own doc says "derived"). `TemporaryControl` is `ControlClaims::effective()` recomputed every tick by `project_control_claims` and stored as a canonical row. | Readers ask the holds/claims (or a query filter derived from them); delete `ScriptedControl`; make `TemporaryControl` a declared-derived row or delete it in favour of `effective()`. Check readers that run before the projection. |
| AP11 | Per-tick mirrors in canonical rows | OPEN | `ActiveRoomMetadata` ← `RoomSet::active_metadata()` (`rooms/systems.rs:37`); `RoomMusicRequest` ← that metadata (audio only); `SwitchOn` ← the save's switch map (`save_sync.rs:15`); `BossAttackState` ← `MovePlayback` (sole writer is a projection, read by the brain before it runs); `MovesetMelee`/`MovesetRanged` ← the moveset's verbs; `BodyCombat::armor` ← `MovePlayback::armor_now`; `BodyMelee::swing` ← `MovePlayback`; `GravityField` ← the primary body's resolved frame. | Per row: delete and read the authority, or declare it derived (re-projected on restore, as `SimDt`). A row read before its projector runs needs the re-projection, not deletion. |
| AP12 | Dead or constant rollback state | OPEN | `LocalPlayer` has no production reader and is a PER-PEER fact inside synchronized state. The `PlayerSlot` component's only reader (`sort_players_by_slot`) has no caller. `BodyMelee::cooldown` is never armed in production (`begin` is test-only) yet feeds brains a constant 0; `BodyMelee::pending_axis` is never written. `RegimePolicy`, `FactionRelations` and `FriendlyFire` never change in production. `EncounterRegistry::ids` has no reader. `CombatTuning::sprite_character_id` duplicates `ActorConfig::sprite_character_id`, and `ActorConfig.tuning.weight` goes stale once a persona change rewrites `CombatTuning::weight`. | Delete dead rows and fields. Decide whether the melee cooldown floor should exist before deleting it. A never-changing resource is either a declared ruleset constant or goes. |
| AP13 | Presentation state in the rollback kernel | OPEN, class 7 | `BodyAnimFacts` (its doc says presentation-only), `ActorRenderSize`/`ActorSpriteOffset`, `EncounterMusicRequest` (audio), `EncounterDef` (debug HUD; `placement_id` duplicates `Encounter.id`), `PickupArt`, `PlayerBlinkCameraState` (three dead fields), `BossDeathAnimation`, `BodyCombat::training_dummy` (health-bar colour). | Move out of the canonical set where a restore does not need them; keep a marker where the pose read model must survive a restore (the registration comments argue this for `PlayerVisual`/`PosedBody`/`RoomVisual`). |
