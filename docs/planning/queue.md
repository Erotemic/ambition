...
| **D200 — three branches hold D192's respawn interval unmerged, and the note that holds them has decayed** | ▢ unstaffed — **REVIEW judgement, not mechanics**: keep, rebase, re-do or drop | ⭐ raised 2026-08-25. `respawn_delay_ticks` exists NOWHERE in main, so `respawn-interval-holding` (3 commits) is the ONLY copy of D192's mechanism; `d194-and-respawn-verified` (6) and `attrib-beat-only` (4) are **LOCAL-ONLY, unpushed, one disk**. ⛔ D194's row still reads `unstaffed` although `3a2100d86` landed a real grab-lock fix on main (124 lines in capture/systems.rs), so the ledger cannot answer its own merge precondition. ⚠ the block is now MECHANICAL too: `git merge-tree` exits 1 with **12 conflicts** across the rollback registry, snapshot impls and all three schema baselines, worsening daily. ⇒ full question, evidence and the four candidate verdicts: [`review-respawn-interval-hold.md`](review-respawn-interval-hold.md) |
