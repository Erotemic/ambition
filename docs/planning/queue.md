...
| **D257 — `capture_sanic` writes a BLANK image and reports success** | ▢ **FOUND 2026-08-29 while migrating D183's last demo; PRE-EXISTING, not a regression — byte-identical at HEAD before the migration** | ⛔ The tool exits 0, prints `capture: wrote …`, and produces a 9.7K PNG that is nothing but the 'skybridge' parallax gradient — no level, no character. **This is exactly the failure every capture file in the repo warns about in comments** (*"a capture reports success with no file written" / "reports success on a transparent PNG"*), live in a shipped tool. ⭐ **ALREADY LOCALIZED — do not re-derive:** (a) NOT timing — identical bytes at `--warmup 1, 5, 20, 90, 600`, so no amount of warmup changes it; (b) NOT the engine or the capture pipeline — `capture_scene sanic_sandbox player out.png 320x240` renders a full scene (characters, platforms, spikes) through the same offscreen path; (c) NOT the route — it boots `SANIC_GAMEPLAY_ROUTE`, the same shape as `capture_mary_o`, whose capture DOES draw its level; (d) the room DOES load (`world-event room-loaded sanic_speedway`) and the player DOES bind (`sprite-bind worn character 'sanic' collision=30x48`). ⭐⭐ **THE LEAD: a `lethal blow: damage=1` fires at frame ~1**, so the suspicion is that the player spawns into a blast zone / void in `sanic_speedway` and the camera follows it out of the level — which would make this a CONTENT defect in the demo's own world, not a tool defect. ⇒ next step is to check `sanic_speedway`'s spawn point against its geometry; ⛔ do NOT start by editing the capture binary. |
