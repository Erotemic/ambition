...
| **P1** | **"DOMAIN SUPPORTED" MUST MEAN "THIS PARTICIPANT CAN APPLY THIS TRANSITION COMPLETELY"**, not "this schema id is on the allow-list". MEASURED 2026-09-12: three transitions pass today and leave the pack and the cast disagreeing — the moveset section removed, the section emptied, and (the one no allow-list row can see) an ENTITY dropped from the candidate, whose old moveset is re-published under the new generation. Reachable by deleting one entity from a file. |
