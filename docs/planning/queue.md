...
| **D256 — RUNTIME EFFICIENCY CAMPAIGN (Jon, 24h goal armed 2026-08-29T02:37Z)** | ▢ **OPEN — plan and leads in [`engine/performance-and-iteration.md`](engine/performance-and-iteration.md)** | ⭐⭐ Jon: *"make this game run faster, more efficiently, and elegantly"*, on evidence, and BOTH deliverables are required — measurements preserved as HISTORY and landed work with BEFORE/AFTER numbers; a run that only builds instruments has failed, and so has one that only optimizes and preserves nothing. ⛔⛔ **READ THE 2026-08-28 BASELINE BEFORE BUILDING ANY INSTRUMENT** — the phase/schedule census, the Tracy observer effect (~9x on frame time) and six ranked directions are ALREADY MEASURED and written down; re-deriving them is the main way to waste this run. **The leads, in leverage order, each already quantified:** (1) `gameplay_allowed` is evaluated **87x/frame** across **89 `.run_if` sites in 11 files** (76 of them in four: `items/pickup/mod.rs` 29, `runtime/combat_schedule.rs` 26, `content/portal/plugin.rs` 13, `runtime/player_schedule.rs` 8) — all four attach to `app.sim_schedule()`, and ⭐ THE MECHANISM ALREADY EXISTS: `configure_platformer2d_simulation_phases` already puts the OTHER condition (`simulation_authorized`) on `GameplaySimulationRoot` with one `configure_sets`. ⛔ not every system in those tuples wants the gate — `pickup/mod.rs` already marks the shrine-resume system as deliberately ungated, and the exceptions are the whole risk. (2) a sandbox run that never entered Sanic/Smash/Mary-O still ticked their systems **1802 times each** — per-experience set gating plus a documented single-experience composition, ⛔ NOT deletion, the broad launcher host must keep working. (3) per-frame rebuilds whose inputs change on events (`rebuild_control_prompt` 31.8us, `rebuild_feature_view_index`, `rebuild_attack_vfx_views`, `sync_ecs_actors_with_save`) — find the COMPLETE authoritative input set first; a missed invalidation path is a correctness bug. (4) dev instrumentation is a **top-five per-frame cost in an OPTIMIZED build** (`record_actor_oob_frame_system` 40.5us + `record_frame_system` 37.9us outweigh `tick_actor_brains`; `poll_world_source_changes` 34.5us AND a blocking `fs::metadata` on the main thread, 3.9ms max) — a budget question, not a bug. (5) `dev/ambition_dev_measurements/` had compile-cost series but **NO runtime series**, so no commit could be compared to any other; the gap is normalized JSONL that survives the raw trace being deleted, with a comparability key strict enough that lavapipe never groups with a hardware GPU nor Tracy with an unprofiled run. ⛔ **PRESERVE EXPRESSIVENESS** — portals, multiple local views, rich VFX, systemic simulation, extensible characters, rollback and dev tooling all STAY; the objective is that a capability pays PROPORTIONALLY (absent = nothing, dormant = little, active = attributable), and deleting a capability is not an optimization. ⛔ **NO CARGO-CULT PERF WORK** — do not replace Bevy scheduling, write a custom sprite renderer, merge systems to shrink a count, add unsafe, or lower visual quality to fix an architectural inefficiency. ⚠ be cheap with the machine: 6-CPU VM, no GPU, shared target dir ⇒ ONE cargo invocation at a time and never `--workspace --tests`. ⭐ a REJECTED HYPOTHESIS is a deliverable — write it down so the next session does not re-run the dead end. |
