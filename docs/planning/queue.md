...
⚠ **THIS FILE IS 3,318 LINES AGAINST THE 908 THE C10 CLEANUP LEFT ON
...
⛔⛤ **NOT COMPLETE. THIS BANNER SAID "COMPLETE FOR ITS CURRENT SCOPE" FOR A DAY
AND THE REVIEW OF 2026-09-17 FOUND A P0 UNDERNEATH IT.** Three open roads have
their decision outside the campaign — `Q122`, `Q128`, and netcode's `N2` for the
unchecksummed float rows' timeline half — and the fourth, the GGRS carrier order
inside the component checksum, is ordinary engineering with nothing in front of
it. ⇒ **That road is the work here; a pass over the rest of the TABLE is not.** ⛔⛤ **What has
...
**Current state (2026-09-17): FOURTEEN CLOSED, FOUR OPEN, EIGHTEEN LIVE —
...
TRUSTING THIS SENTENCE:** the table below holds sixteen roads, twelve closed and
four open, and the seventeenth and eighteenth are the two prose receipts beneath
it, both closed. 12 + 2 = fourteen, and all four open roads are in the table.
...
⛔⛤ **THREE OF THE FOUR OPEN ROADS ARE BLOCKED OUTSIDE THIS CAMPAIGN. THE
FOURTH IS NOT, AND IT IS THE ONE TO PICK UP.** Two want a maintainer decision
before anyone starts — the absolute `SimTick` (`Q128`, netcode) and the snapshot
schema fingerprint hashing English prose (`Q122`) — and the 25 unchecksummed
float rows are a DIFFERENT KIND of road: those carry no host-local id at all,
they are simply never compared between peers, so no projection can fix them and
only netcode's `N2` can observe them. ⇒ **The fourth — the GGRS carrier order
inside the component checksum — wants no ruling and no P2P session.** It is
reproduced, measured and held by a witness; see the last row of the table.
...
| **the GGRS carrier ORDER inside the component checksum** | ⛔⛔ **OPEN, NOT BLOCKED, AND THE ONLY ROAD HERE THAT IS NEITHER — FOUND 2026-09-17 BY THE GPT ARCHITECTURE REVIEW AND REPRODUCED THE SAME DAY.** `ComponentChecksumPlugin` hashes `RollbackOrdered.order(rollback_id)` together with the value projection before XORing carriers, so the checksum two peers compare contains an App-lifetime INSERTION INDEX. `RollbackOrdered` keeps every index it ever handed out, deleted entities included, is itself snapshotted, and nothing rebases it: `install_rebased_sync_test_session` resets `RollbackFrameCount`, `ConfirmedFrameCount`, the input authority and `Time<GgrsTime>`, and deliberately retains the snapshot infrastructure. ⭐ **MEASURED on the two-host fixture:** the same 22 canonical identities, orders `0..21` against `74..95` — a CONSTANT offset of 74, the rollback entities Sanic and Mary-O registered and retired, so relative construction order survives and only the base moves — and **59 of the 146 real `ChecksumPart`s disagree**, 81 agreeing and non-zero so the comparison is not vacuous. ⛔⛤ **AND THIS IS WHY THE VALUE CENSUS SAID CLEAN**: the probe folds `count` and a wrapping sum of the per-value projection and ignores WHICH ENTITY carried each value, so it measures GGRS's projection and not GGRS's checksum — the instrument fix of `decdb036b` was necessary and is not sufficient. ⇒ Held by `two_local_histories_compute_the_same_ggrs_component_checksums` (`shell_host_lifecycle`), `#[ignore]`d rather than inverted because an inverted witness has to be deleted by whoever fixes this. ⚠ The repair is a SESSION-RELATIVE carrier ordering established where a synchronised session declares frame zero, and it must NOT be *"sort by `RollbackId`"* — that is the Bevy `Entity` one layer down. `SimId` is the peer-stable key, and the fixture measures that every live carrier has one in both hosts. `RollbackOrdered::push` is private upstream, so the road is either a small `bevy_ggrs` primitive or an Ambition-side reconstruction at that edge |
