...
| AP12 | Dead or constant rollback state | OPEN: one decision left (2026-09-24) | ✅ Deleted as dead: `LocalPlayer`, the `PlayerSlot` component and `EncounterRegistry::ids` (v214); `EncounterMusicRequest::last_applied` (W021; a presentation mirror in gameplay state, no reader; no schema change); `AttackSpec::damage_kind`/`can_pogo`/`damage_override` and `with_held_melee` (W010, v220); `BodyOffense` and the F3 "slash damage" knob that edited it (W009, v222; its `With<>` in the victim filter was population-neutral); `BodyMelee::pending_axis` (v223; its one writer `BodyMelee::begin` has no production caller). ✅ Kept, with reasons: `ActorConfig.tuning.weight` (construction input only); `FriendlyFire` and `FactionRelations` (designed baselines that match rules fold over; tests pin the seams); `RegimePolicy` (ADR 0010's clock seam). `CombatTuning::sprite_character_id` went with AP7. `BodyMelee::swing` is AP11's mirror question (a per-tick projection of `MovePlayback`). | ⛔ DECIDE: `BodyMelee::cooldown` is constant 0 in production. Only `begin` arms it (tests only), so every brain gate on `attack_cooldown_remaining <= 0.0` (`state_machine/mod.rs` MeleeBrute/bite/lined-up arms, `smash/action.rs`) is always open, and `BrainProfile::attack_cooldown_mult` is authored and read by nothing (AP3). Either the move road arms the floor from the authored profile (a behavior change: AI swings get slower), or the floor, the snapshot field and the gates are deleted. | <!-- cite-ok: the deleted fields this row closed -->
...
| AP18 | Control gate was player-only | ✅ DONE 2026-09-24 (one residue recorded) | `gate_worn_player_control` filtered `(With<PlayerEntity>, With<WornCharacter>)`, so a body driven by possession or a second seat kept a raw frame: no technique edge (content grants techniques to the `ControlledSubject`), and no stripping of unowned verbs (guard, grab, attack). Renamed `gate_body_control`; the filter is gone and every body is gated by its own scheme. Census before the change (a probe beside the gate, app_it + mary_o/sanic/smash/twintrack, ~1.7M non-player body-ticks): autonomous bodies changed 0 times, and only DRIVEN bodies differed — twintrack's lab twin (a stray `attack_axis`) and a possessed boss. Witness: `a_driven_actor_outside_the_player_population_is_gated_by_its_own_scheme` (red with the `PlayerEntity` filter restored). | ⛔ A POSSESSED BOSS'S REPERTOIRE IS INVISIBLE TO ITS SCHEME: `possessed_attack_choice` maps raw melee/special presses through `BossBehaviorProfile::possessed_verbs` + `BossCapability`, while the boss's `ActionSet` (`special: None`) and profile-keyed moveset give the scheme no Attack/Special slot. The boss tick consumes the seat before the gate, so behaviour is unchanged (`possessed_boss_commands_its_authored_specials…` green), but the gate now strips verbs the boss does own, and the `ControlPrompt` for a possessed boss shows neither. The fix is to declare the boss's possessed verbs in its scheme at construction (e.g. Attack/Special technique slots), then have the boss tick read the routed edges. |
| AP19 | Unauthored bodies are given a capability default | OPEN: ⛔ DECIDE (Jon) | The generic constructor confers nothing (`ActorBody::from_abilities`; `from_kit`, `locomotion_abilities`, `default_fighting_kit` and `CombatKit` are deleted). ✅ 2026-09-24: `new_peaceful_npc_in`'s fallthrough (no character, no body blueprint, or an unprepared id) is built with `AbilitySet::NONE`; it used to take the actor default, so the anonymous cut-rope victory NPC could jump, double-jump, attack and interact. Witness `an_anonymous_npc_is_built_with_no_abilities` (poison: the default reds it). What is left is one engine-owned ruleset default, `ActorBody::default_actor_abilities()` (move, jump, variable jump, double jump, attack, plus `basic()`'s interact/crouch/climb), applied at two production sites. (1) `new_character_in`, for a character whose blueprint states no abilities: measured 2026-09-24 on the sandbox composition, **42 of the 48** blueprinted characters (every `npc_*`, plus goblin, medic, sandbag, the polygons, director and others). Only robot, player_robot_v2/v3, npc_puppy_slug and the two cellular automata author a set. (2) The boss seed (`actor_spawn/mod.rs`), which gives every boss the default plus flight. | Options for (1): (a) the content provider declares its actor default, and preparation resolves `abilities` to authored-or-declared, so `CharacterBodyBlueprint::abilities` is not `Option` and `ambition_body_seed` holds no default; (b) author a set on each of the 42 characters and make an absent set a content error. (a) is mechanical and keeps behaviour; (b) is content work. (2) wants the boss authoring's own answer; check whether anything reads a boss's `jump`/`double_jump`/`attack` before changing it. |
