...
| Item | State |
| --- | --- |
| P0 clip-identity resolution (`ClipBinding` → sheet row) | ✔ **DONE for every ACTOR** — `first_bound_row` → `CharacterSheetSpec::clip_slot` → `CharacterAnimator::request_clip`, and `ActorAnimFrame` carries the active `MovePlayback`'s chain so the renderer resolves it against the sheet it is about to draw. `CharacterAnim` is untouched at 56 variants. ✔ **and the local PLAYER path too** — the second sub-tuple had room for a fifteenth member, so a human-driven fighter and a CPU one on the same character draw the SAME animation for the same move |
...
| P2 generic fighter-state rows | ◑ — **air_dodge / tumble / knockdown / getup DONE**, on both roads, through the same clip seam a move uses: `body_state_clip(&BodyMotionFacts)` returns the authored chain and the move wins when one is playing. Facts are READ, never inferred. ▢ **tech / tech_roll / getup_roll are NOT distinguished, deliberately**: the sim publishes ONE `getup_invulnerable` flag, so telling them apart means the subsystem that knows which `MovementOp` ran must publish the fact first |
| P3 **Noether's new art** | ✔✔ **SHIPPED 2026-08-11.** Jon: *"if noether gameplay doesn't exist, make it."* Written (geometry derived from the rig's bone chain, not eyeballed), target renders 123 rows with all nine signature clips, `publish noether` installed the 7.0MB sheet over the 165KB one from 08-08, source metadata says `npc_noether`, and the game-side identity waiver is DELETED — the scan passes on its own merits |
