...
| §2 ONE resolved seat kit | ◑ — `action_set` and `combat_kit` now ride the seat beside `identity_kit`/`moveset`, and `realize_seat` inserts them rather than re-deriving. ⭐ ONE expensive call was never the property that mattered; ONE ANSWER is. ▢ `ranged_execution` is authored on the character now (§4) but the SEAT does not yet carry the resolved one |
...
| §4 D86: **KEEP the robot's charge**; rename `HostCharge` → `ChargedProjectile` | ✔ **DONE** — and the rename was the smaller half. `ranged_execution` is an authored field on `CharacterDefinition` (default `MovesetVerb`), carried through preparation, and the persona derive's Authored arm READS it instead of returning `MovesetVerb` unconditionally. Robot v3 authors `ChargedProjectile`, so §5 can delete `HostCode` without deleting the Hadouken |
| §5 Robot v3 authors its canonical `ActionSet`; delete `HostCode` | ◑ — **the robot is OFF `HostCode`**: it authors `player_robot_action_set()`, `ActionSet::gated_by(abilities)` is the general progression filter, and its catalog row says `Authored`. ▢ the two `HostCode` variants survive: `PlayableKitSource::HostCode` has **3 demo adopters** (smash, pocket, the host smoke test) and `PreparedKit::HostCode` is still the fallback for any character authoring no action set — deleting THAT is the unauthored-floor bridge, which is §8's job and larger |
