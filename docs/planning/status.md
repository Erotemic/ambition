...
| workspace cargo receipt | ✔ **7,290 passed / 0 failed, exit 0, at `4b9e684d9`** (2026-09-05, `cargo test --workspace`) — supersedes 7,264 at `ea28cc7b9`. ⛔⛔ **AND THE TREE MOVED UNDER IT, WHICH IS WHY THE DRIFT IS ANALYSED RATHER THAN ASSERTED AWAY.** Two commits by another agent landed IN THIS WORKING TREE mid-run — I merged nothing and committed nothing. MEASURED: `git diff --name-only 4b9e684d9..HEAD` is `.llm_resource_tally/*` (accounting JSON) and a `tools/ambition_sprite2d_renderer` SUBMODULE POINTER bump (`aba1c1eb9` → `978a4ba6f`); **ZERO `.rs`/`.toml`**. `cargo test --workspace` compiles neither, so the Rust receipt stands for `4b9e684d9`. ⚠ The renderer pointer move WAS checked separately, because a Python lane reads it: `check_gauntlet_props_are_rendered.py` is green at the new pin and the submodule checkout is clean. ⛔ **A RECEIPT IN A SHARED TREE MUST RECORD HEAD BEFORE AND AFTER.** Equality is not available here — other agents commit into it — so the honest form is to diff the range and say what class of file moved. An earlier run this hour was VOID for the same reason with my own hand on it: I started it in the background and then ran three other `cargo test` invocations and committed edits while it ran, which produced a phantom failure that did not reproduce (the binary re-ran 567/0). | `cargo test --workspace`, HEAD diffed before and after |
