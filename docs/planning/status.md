...
| the workspace FEATURE UNION | ◐ **50/53 jobs at `bf748dd5a`** (2026-09-05, `[lane: exhaustive]`, 2,850 s) — the DEFAULT-features job is **7,143 run / 7,143 passed / 33 skipped**, and the three failures are each named rather than summarised: **(1)** the gate's own stated feature-gated count, moved by two tests I added, fixed in `0e789168d`; **(2)** the compile-cost ratchet, red by design during D33 and awaiting a carve owner; **(3) ⛔ a REAL defect the default plan cannot see** — 9 damage/hurtbox tests in `ambition_platformer2d_actor_monolith` **pass at default features and panic under the feature union** (`1083 passed; 9 failed`). Same test, same run: `PASS` at 4341/7143 in the nextest job, `FAILED` in the union job; both modules are plain `#[cfg(test)]`, so this is behaviour, not a skipped test. ⇒ **This is the run that justifies the union's hour.** | ⛔ **CAUSE NOT YET KNOWN — a first diagnosis was RETRACTED the same evening.** I read `7b0418faf` folding three `MessageWriter` params into `StrikeOutcomeWriters` and adding a FOURTH (`ParriedBodyHit`), saw the failing tests hand-list exactly the three that existed, and called it evidence-backed. ⇒ It cannot be: the new field carries no `#[cfg]`, so it is demanded in EVERY build, and **a mechanism that is always active cannot explain a feature-conditional failure**. A second candidate (`BodyHitResolved` under `causal`) died too — it is an `Option<MessageWriter>`, which yields `None` rather than panicking. ⇒ ⭐ **When a failure is CONDITIONAL, the condition is the fact with the most information in it**; a candidate must explain the ASYMMETRY, not just the breakage. ✔ What survives: the hand-listed registration fragility is real, just not what flipped these red. ⚠ The panic text is swallowed, so only a local `--all-features` reproduction has it. Owned by the fighter lane; reproduction pending here. ⚠ **NOT exact-HEAD**: HEAD has moved since `bf748dd5a`; a fresh union is running. ⓘ Supersedes the 49/49 / 7,243 at `40d078509` (2026-09-04), which was also not exact-HEAD. |
