...
| Content ownership | **PARTIAL.** Dialogue cast voiceprints now register from `ambition_content` through an open dialogue catalog, and the pirate gun-sword renderer is replaced by a generic wielded-item visual catalog populated by content. Closed item, asset/art, projectile, input-technique, and additional named-render paths remain. | `game/ambition_content/src/dialogue/voiceprints.rs`; `game/ambition_content/src/items/wielded_visuals.rs`; reusable catalogs in `ambition_dialog` and `ambition_render` | Each remaining eviction ends in provider-owned registration/catalog/presentation structure; no relocated closed table. |
