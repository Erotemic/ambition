...
|---|---|---|
| Portal camera continuity | **PATCHED; execution verification pending (2026-07-21 review).** The bisect and instrumentation were right that `BodyPoseView` was one frame stale, but the cause was narrower than the journal concluded: on the frame-stepped host both `FeatureViewSync` and `PresentedPoseSet` live in `Update`, and the new presented-pose plugin never ordered its consumer set after the sim read-model producer. Bevy could therefore resample the pre-transit view before the same-frame rebuild while portal continuity already saw post-transit authoritative state. The patch adds the missing `FeatureViewSync -> PresentedPoseSet` edge only when the sim shares `Update`, plus a schedule-graph regression test. | Run `cargo test -p ambition_app --test app_it -- portal_translation_camera_continuity` and the full `./run_tests.sh` gate in a Rust-equipped checkout. The review sandbox had no `cargo`, so this row deliberately does not claim green execution. |
...
| Super Mary-O acceptance | **LEVEL-1 GATE CLOSED (2026-07-21).** Every item on the acceptance list is done: pickups/equip, grown form, ranged powerup, bricks, crony stomp, flag sequence, clock, tally, cyclic restart, a five-slot declared HUD, lives that a death actually spends, the secret pipe + underground coin vault, the brainless sliding shell, title/results cards, and a deterministic scripted run that plays the whole thing through the real control seam. | Gate closed. The demo's next want is additional levels — see [`demos/super-mary-o.md`](demos/super-mary-o.md). |
