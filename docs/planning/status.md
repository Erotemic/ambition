...
| Explicit provenance + planned construction | **DONE (2026-07-22).** Milestone B. One content-free planner (`ambition_platformer_primitives::construction`) carries an authored ground item, a provider-staged actor, and a summoned minion through validation → plan → commit, with a canonical dump and planned-vs-committed roster parity. `SpawnOrigin` is a snapshot-registered component, so reconstruction reads provenance instead of splitting a `SimId` on `/` — the tree's only such parse is deleted. Three silent skips became preflight failures (an unresolvable held item, a grudge naming nobody, two summons colliding on one authored id). Deliberately ONE family per origin kind; the rest of the room still takes family-specific loops. | Closed. Next: that doc's Phase 4 — activation/reset/transition/hot-reload/reconstruction become variations of one construction transaction. |
