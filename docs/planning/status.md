...
| Atomic active-room restoration | **DONE + CLOSED OUT (2026-07-16).** `restore` stages a differing snapshot room through the canonical construction (`RoomStaging`) with every preflightable refusal before mutation. The GPT-5.6 closeout landed on top: room-content staging is part of construction (`RoomContentStagingRegistry` — the duel/crony `RoomLoaded` consumers became registered pure stagers; `RoomLoaded` is a pure notification), a room-backed restore refuses rather than bare-spawn a hollow roster member, snapshots are session-bound (`SimSnapshot.session`; `SessionMismatch` is the first preflight), and the projectile family is the first blob-rebuildable dynamic family (`declare_dynamic_anchor` + `heal_projectile_owners`) — a rollback window may span a projectile's whole life. Exit oracles: `portal_lab` CLEAN and `a_staged_restore_rebuilds_the_duel_roster_completely`. See [`engine/netcode.md`](engine/netcode.md) N3.2b + closeout. | — closed; honest boundaries: non-anchored dead `spawned(..)` entities (minions, lowering-spawned children under staging), same-room content-staged deaths, room-presence mismatch, `PlayerProjectileState` debt. |
