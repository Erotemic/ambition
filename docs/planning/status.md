...
|---|---|---|
| Actor monolith decomposition | **OPEN 2026-08-07.** `crates/ambition_platformer2d_actor_monolith/src/` is now a >100k-line compile unit, and the movement-only API consumer still inherits unrelated capabilities through it. The July no-further-carve ruling is retired. Work proceeds by subtracting coherent ownership domains little by little, using `.agent` dependency/ECS inventories plus source confirmation and compile/dependency measurements. | [`engine/actor-monolith-decomposition.md`](engine/actor-monolith-decomposition.md): drain independent capabilities and presentation/authoring/session glue until the residue is honestly actor/body/control simulation; demonstrate material dependency-closure and compile-iteration improvement. |
