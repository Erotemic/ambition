...
| workspace cargo receipt | ✔ **7,291 passed / 0 failed, exit 0, at `f9c283dcb`** (2026-09-05, `cargo test --workspace`) — covers the portal-compositing work (per-pane relation, Shift+F8 ordering report, host candidate publishing, the `bevy_size_to_world` inverse). ⛔⛔ **THE TREE MOVED UNDER IT AGAIN, AND THAT IS NOW THE EXPECTED CASE, NOT AN INCIDENT.** Other agents commit into this working tree; HEAD equality before and after is NOT an available standard. ⇒ The receipt records HEAD both sides and DIFFS the range: `.llm_resource_tally/*` plus a `tools/ambition_sprite2d_renderer` pointer bump, **ZERO `.rs`/`.toml`**, which `cargo test --workspace` does not compile. ⚠ The renderer pointer got its own check because a PYTHON lane reads that submodule — `check_gauntlet_props_are_rendered.py` green at the new pin. ⓘ Supersedes 7,290 at `4b9e684d9` and 7,264 at `ea28cc7b9`. | `cargo test --workspace`, HEAD diffed before and after |
