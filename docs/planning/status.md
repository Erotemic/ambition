...
| the workspace FEATURE UNION | ◐ **50/53 jobs at `bf748dd5a`** (2026-09-05, `[lane: exhaustive]`, 2,850 s) — the DEFAULT-features job is **7,143 run / 7,143 passed / 33 skipped**, and the three failures are each named rather than summarised: **(1)** the gate's own stated feature-gated count, moved by two tests I added, fixed in `0e789168d`; **(2)** the compile-cost ratchet, red by design during D33 and awaiting a carve owner; **(3) ⛔ a REAL defect the default plan cannot see** — 9 damage/hurtbox tests in `ambition_platformer2d_actor_monolith` **pass at default features and panic under the feature union** (`1083 passed; 9 failed`). Same test, same run: `PASS` at 4341/7143 in the nextest job, `FAILED` in the union job; both modules are plain `#[cfg(test)]`, so this is behaviour, not a skipped test. ⇒ **This is the run that justifies the union's hour.** | ⭐ Mechanism read off the diff: `7b0418faf` folded three loose `MessageWriter` params into `StrikeOutcomeWriters` and added a FOURTH (`ParriedBodyHit`); the failing tests build their app by HAND-LISTING exactly the three that existed, so the new writer has no registration and panics in `Main::run_main`. ⇒ **A hand-listed set of dependencies is a POPULATION, and adding to the SOURCE does not update the LISTS** — and the compiler cannot help, because an unregistered message is a runtime panic. ⚠ **UNEXPLAINED and the more important half: WHY the default build is green.** If a feature is what makes that writer live, the default suite cannot protect the seam at all. Owned by the fighter lane, relayed with the evidence. ⚠ **NOT exact-HEAD**: HEAD has moved since `bf748dd5a`; a fresh union is running. ⓘ Supersedes the 49/49 / 7,243 at `40d078509` (2026-09-04), which was also not exact-HEAD. |
