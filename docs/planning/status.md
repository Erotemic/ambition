...
| Provider lifecycle | **OPEN — coherent extraction.** Substantive preparation/activation code lives in `crates/ambition/src/provider.rs`, and providers repeat the same typed prepared-session protocol. | `crates/ambition/src/provider.rs`; provider modules under `game/ambition_content` and the demo crates | A dedicated provider crate owns one preparation, exact activation, session-build, and cleanup protocol; the `ambition` facade returns to re-exports. |
