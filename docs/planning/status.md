...
| Super Mary-O acceptance | **LEVEL-1 GATE PARTIAL — REOPENED 2026-07-21.** It was declared closed the same day; that was wrong on two counts. (1) The acceptance contract requires the scripted headless run to use "a powerup through the real pickup/equipment path". It never collects one — the only pickups it takes are coins, which are currency, not equipment. (2) It was described as playing "the whole thing through the real control seam"; it drives `ControlFrame` for most of its ~900 frames but SETS her position at three beats (pipe, vault exit, flag), so no test proves the level is traversable spawn-to-pole. Shipped features are real and unaffected: pickups/equip, grown form, ranged powerup, bricks, crony stomp, flag sequence, clock, tally, cyclic restart, the five-slot declared HUD, the secret pipe + coin vault, the sliding shell, title/results cards. Lives accounting was separately BROKEN until 2026-07-21 (a replay's own body reset read as a death; winning drained the run) — fixed and regression-tested. So was the replay itself: in the standalone binary `RoomReplayRequested` had no consumer at all, so neither the flag cycle nor the timeout rebuilt anything — fixed the same day by moving the seam into `ambition_runtime` (tracks §2.5), which is what makes the run's replay clause assertable. | Two clauses: a run that traverses under its own input, and one that takes a powerup through the shared pickup/equipment path. See [`demos/super-mary-o.md`](demos/super-mary-o.md). |
