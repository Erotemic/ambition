...
| feature-gated tests | **810 hidden behind features across 29 crates** (2026-09-04 late; the figure moved 793 → 798 → 800 → 803 → 810 in one day, so RUN the tool rather than quoting this) | `scripts/feature_gated_tests.py` |
...
| the workspace FEATURE UNION | ◐ **50/53 jobs at `bf748dd5a`** (2026-09-05, `[lane: exhaustive]`, 2,850 s) — the DEFAULT-features job is **7,143 run / 7,143 passed / 33 skipped**, and the three failures are each named rather than summarised: **(1)** the gate's own stated feature-gated count, moved by two tests I added, fixed in `0e789168d`; **(2)** the compile-cost ratchet, red by design during D33 and awaiting a carve owner; **(3) ⛔ a REAL defect the default plan cannot see** — 9 damage/hurtbox tests in `ambition_platformer2d_actor_monolith` **pass at default features and panic under the feature union** (`1083 passed; 9 failed`). Same test, same run: `PASS` at 4341/7143 in the nextest job, `FAILED` in the union job; both modules are plain `#[cfg(test)]`, so this is behaviour, not a skipped test. ⇒ **This is the run that justifies the union's hour.** | ✔ **CAUSE MEASURED**: `Parameter StrikeOutcomeWriters<'_>::parried failed validation: Message not initialized` — `7b0418faf` added a FOURTH writer to a struct whose users hand-list their registrations, so `apply_hitbox_damage` demands `ParriedBodyHit` in an app that never registered it. ⇒ **A hand-listed set of dependencies is a POPULATION, and adding to the SOURCE does not update the LISTS.** ⛔⛔ **AND THIS RECEIPT IS NOT A SINGLE-HEAD RECEIPT.** The run took ~5 h and I merged `origin/main` into the tree repeatedly while it ran; the parry/counter commits landed at 00:43 against a run whose HEAD was 00:18. So its nextest job tested a tree WITHOUT them and its union job tested one WITH them — which produced a fake "passes at default, panics under the union" asymmetry that made me RETRACT the correct diagnosis. ⇒ *"Do not edit while the suite runs"* was never only about my own edits: a mid-run `git merge` is the same act and is the one you forget. ⚠ **NOT exact-HEAD**: HEAD has moved since `bf748dd5a`; a fresh union is running. ⓘ Supersedes the 49/49 / 7,243 at `40d078509` (2026-09-04), which was also not exact-HEAD. |
