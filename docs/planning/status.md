...
| Fighter-brain L3 rollouts | **REDESIGNED 2026-07-30** — [`engine/fighter-brain.md`](engine/fighter-brain.md) §12. Rollouts run on a pure shadow model whose only world input is `Perceived` (the private-field type is the enforcement, so the rollout physically cannot hold an unperceived fact), under an exact `rollout_k × (1 + rollout_depth)` step budget that is profile DATA — no wall clock anywhere in the decision path — against a deterministic predicted opponent (modal habit when the model holds a real read, inertia otherwise; no RNG in L3). The scratch-world/GGRS problem is deleted rather than solved: nothing persists across a decision, so there is nothing to snapshot. | Implementation, now unblocked and opus-executable per §12.7: FB6a–FB6d are pure-module work today (FB6a's `max_damage`/`max_knockback` derivation also closes §9's recorded "nothing prices power" gap); FB6e's ladder gate `l3_earns_its_depth` and the resimulation determinism check wait on FB4's owed decision rig. |
