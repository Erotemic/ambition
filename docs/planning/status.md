...
| feature-gated tests | **416 hidden behind OPT-IN features across 27 crates** (re-derived 2026-09-06; ⛔⛔ **A SECOND INSTRUMENT CORRECTION, same shape as the first**: the scanner knew `default` and transitive-through-`default` as roads to "this feature is on", and missed a THIRD — a crate dev-depending on ITSELF with features (`ambition_content = { path = ".", features = ["portal"] }`), which turns the feature on for `cargo test -p <crate>` while leaving downstream consumers untouched. ⭐ VERIFIED AGAINST CARGO: `cargo test -p ambition_content -- --list` sums **351** bare and **367** with `--all-features`, and the corrected scanner now says `351 of 367` exactly where it said `297 of 367`. ⚠ MEASURED: exactly ONE crate in the workspace uses that pattern, so the blast radius is bounded — and the movement 447 → 469 → 416 is instrument, not tests appearing or vanishing. ⭐ Found only because a test I had just added RAN in an invocation the survey said was hiding it; the disagreement between two instruments is what caught it, not diligence. Prior correction, kept because the lesson is the same one twice: ⛔ THE DROP FROM 812/29 IS AN INSTRUMENT CORRECTION, NOT TESTS APPEARING — the scanner counted any `#[cfg(feature = "…")]` without asking whether the feature is in `default`, so it reported `ambition_platformer2d_ldtk` as *51 of 53 run bare* while cargo answers **53 of 53**; a gate whose feature is on by default hides nothing from a default run) ⚠ the consumer was the expensive half: the ledger test told whoever moved the number that their test *"does NOT run in the default gate plan"*, which was FALSE for a default-on gate — measured two ways, the two tests that moved the count run under `cargo test --workspace` and cargo lists them in a bare `cargo test -p`; ⇒ "behind a cfg(feature)" and "absent from a default run" are DIFFERENT CLAIMS and the survey now makes the second one | `scripts/feature_gated_tests.py` |
