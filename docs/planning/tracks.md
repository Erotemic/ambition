...
| Decomposition D-B | same | **🔴 REOPENED (audit 2026-07-10)** — Series 1 landed the executable gate (`crates/ambition_runtime/tests/module_size.rs`: line limit + bidirectional reasoned waiver list, poison-tested), regenerated the stale `MODULES.md`, and corrected 42→44. STILL REOPENED because nine modules remain waived debt (snapshot.rs 3744, moveset.rs 3022, …) | split the nine waived modules down; D-B re-closes when the waiver list empties, not when it is merely gated |
...
| Netcode ladder | [engine/netcode.md](engine/netcode.md) | N0.1/N0.2 LANDED. **N0.3 LANDED** (determinism scan now reaches `game/ambition_content` + demo rules; caught 2 real `falling_sand` bugs). **N0.4 LANDED** (required rooms hard-fail — a load failure or silent room-fallback panics, no vacuous skip; coverage ledger reacts to added debt). **N3.2 substrate LANDED + re-audit findings closed** (2026-07-10, Series 2 + re-audit): identity invariant with a full snapshot `roster` (catches a collision across disjoint components; `take` enforces uniqueness), active-room ownership invariant + `CrossRoomBoundary` refusal + active room folded into the hash, reconciliation-first stale accounting (H4), `DecodeFailed` transactional for standalone codecs (decode-preflight before mutation; cursor/resolved a named residual), self-measured reliable `lossless()` (census-gated; message channels claimed; per-type `SIM_RESOURCE_EXCLUSIONS`), and an `UnsupportedDynamicReconstruction` refusal (ONE reconstruction refusal, not a general window bound) — each with a red-before/green-after poison test. N0.3's manifest scan un-vacuumed. **`portal_lab` is a transition-spanning window, NOT a leak** (`central_hub_complex` authors `NpcSpawn-0017`; restore refuses the cross-room window). | REMAINING (N3.2): full atomic room-context restore (→ portal_lab CLEAN); per-spawner reconstruction recipes; `SnapshotResolve::resolve` → `Result` (resolved-codec decode failure); boss hands should mint `SimId::spawned(..)` not `placement:`. Then N3.3 bevy_ggrs spike |
