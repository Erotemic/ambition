...
| Decomposition D-B | same | **🔴 REOPENED (audit 2026-07-10)** — Series 1 landed the executable gate (`engine.module-size` policy in `tests/ambition_workspace_policy`, config `policies/module_size.toml`: code-line limit 1500 + bidirectional reasoned waiver list, poison-tested; migrated 2026-07-10 from the retired `crates/ambition_runtime/tests/module_size.rs`), regenerated the stale `MODULES.md`, and corrected 42→44. STILL REOPENED because **THREE modules remain waived debt** (status corrected 2026-07-11, was overstated as "nine … moveset.rs 3022": moveset.rs is 1536 total / under the code-line limit and is NOT waived): `snapshot.rs` (3684), `portal_presentation/view_cones.rs` (2206), `menu/kaleidoscope_app.rs` (1814). | split the three waived modules; D-B re-closes when the waiver list empties. **`snapshot.rs`: a pre-solved, opus-executable 4-way split (mod.rs core / registry.rs / restore.rs / codecs.rs, each <1500; the shared-`bc`-alias gotcha noted) is in [decomposition.md §D-B](engine/decomposition.md) — run fresh, delete the waiver.** `view_cones.rs` needs a presentation seam; `kaleidoscope_app.rs` is a declarative Lunex tree (arguably a permanent justified waiver). |
...
- **opus** — **D-B status corrected (Series 3 pass 1).** The D-B row claimed "nine
  ...
  `kaleidoscope_app.rs` 1814), and the gate is GREEN; `moveset.rs` is 1536 total /
  under the code-line limit and is NOT waived. A false status that would have steered
  a phantom "split nine modules" effort. Row rewritten to the measured three, with
  each waiver's real split path named. No code change — a status correction.
