...
4. ~~**CC3 — the fuzz-oracle delta** (§6.1).~~ 🟡 **LARGELY DONE 2026-07-10.**
   Invariants 1 (now carve-aware, via `world_with_portal_carves` — the transit
   exemption falls out of the geometry), 3, 4 (folded + cataloged), and 6
   (one-way fall-through) are live, with the §6.2 minimum payload
   `(seed, room, tick, invariant #, GeoId)` and every shipped room in the matrix.
   **The measurement is the headline: 72 rooms × 3 seeds × 300 ticks = 64,800
   frames, 15 violations, NONE a collision bug.** Remaining: invariant 2
   (straddle-outside-carve) and invariant 5 (one Class-B remap per frame — needs
   the Class-B writers to emit a countable event). Both want the same portal
   read-model row, which would also retire the one known false positive (a portal
   transit tripping the legacy teleport probe). [opus]
...
| Collision doctrine | [engine/collision-and-ccd.md](engine/collision-and-ccd.md) | CC1 + CC2 + CC5 + CC6 LANDED; **CC3 largely EXECUTED 2026-07-10** — invariants 1 (carve-aware), 3, 4, 6 live + the §6.2 minimum payload + every shipped room. **64,800 stepped frames, 15 violations, none of them a collision bug** (14 open-edge OOB = authoring; 1 portal-transit teleport = explicitly legal). Zero embeds, zero one-way fall-throughs, zero suspect clips | CC3 remainder: invariant 2 (straddle-outside-carve) + invariant 5 (Class-B remap counter — needs new machinery); both want the same portal read-model row, which also retires the teleport false positive. Then CC4 (profile first), CC7 P3a |
...
| All bosses render the generic sheet | **MEASURED 2026-07-10; two of three causes RULED OUT and pinned.** `upgrade_boss_sprites` draws the generic body exactly when `boss_sprites[boss_key]` misses (`boss_key` = the lowercased BEHAVIOR ID). (1) every `dedicated_boss_sheets()` key resolves a real catalog path, and (2) every boss with authored art is registered under its behavior id, not its `sprite_target` — both now gate-enforced by `game/ambition_app/tests/boss_sheet_wiring.rs`, so the disproven `sprite_target` dispatch stays disproven. The `boss_sprites.len()` logging the diagnosis asked for is now PERMANENT (`[boss_sprites] N dedicated sheet(s) loaded: …` + a FAILED list). What remains can only be (3) the image not yet loaded on the frame the boss spawns (benign — the system retries), or a render-side ordering problem. **Next: a windowed RUN, which OV1 blocks.** | [opus] |
