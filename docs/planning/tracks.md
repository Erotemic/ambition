...
| Decomposition D-B | same | **🔴 REOPENED (audit 2026-07-10)** — Series 1 landed the executable gate (`engine.module-size` policy in `tests/ambition_workspace_policy`, config `policies/module_size.toml`: code-line limit 1500 + bidirectional reasoned waiver list, poison-tested; migrated 2026-07-10 from the retired `crates/ambition_runtime/tests/module_size.rs`), regenerated the stale `MODULES.md`, and corrected 42→44. STILL REOPENED because the over-limit list is not yet empty. **`snapshot.rs` (3684) SPLIT 2026-07-11** → `snapshot/{mod,registry,restore,codecs}.rs` (1155/718/471/1379, all <1500), 46 tests + rl_sim `desync_canary` green, waiver deleted. The gate counts TOTAL lines vs the 1500 limit (no separate "code-line" count), so the remaining over-limit set is: `combat/moveset.rs` (1536, **UNWAIVED — the gate fails on it**, a pre-existing miss the earlier "under the code-line limit" note masked), `portal_presentation/view_cones.rs` (2206, waived), `menu/kaleidoscope_app.rs` (1814, waived). | **`moveset.rs` (1536) is the live blocker — split it (preferred, shrink the debt) or waive with a reason; the gate is RED until then.** `view_cones.rs` needs a presentation seam; `kaleidoscope_app.rs` is a declarative Lunex tree (arguably a permanent justified waiver). D-B re-closes when the over-limit list empties. |
