...
| Netcode ladder | [engine/netcode.md](engine/netcode.md) | N0.1/N0.2 LANDED. **N0.3 LANDED** (determinism scan now reaches `game/ambition_content` + demo rules; caught 2 real `falling_sand` bugs). **N0.4 LANDED** (required rooms hard-fail — a load failure or silent room-fallback panics, no vacuous skip; coverage ledger reacts to added debt). **N3.2 substrate LANDED** (2026-07-10, Series 2): identity invariant (restore refuses a non-unique `SimId`), active-room ownership invariant + `CrossRoomBoundary` refusal, reconciliation-first stale accounting (H4), `DecodeFailed` on codec failure, positive `lossless()` with named `SIM_RESOURCE_EXCLUSIONS`, and an `UnsupportedDynamicBirth` window boundary — each with a red-before/green-after poison test. **`portal_lab` is a transition-spanning window, NOT a leak** (`central_hub_complex` authors `NpcSpawn-0017`; restore refuses the cross-room window). | REMAINING (N3.2): full atomic room-context restore (→ portal_lab CLEAN); per-spawner reconstruction recipes; boss hands should mint `SimId::spawned(..)` not `placement:`. Then N3.3 bevy_ggrs spike |
