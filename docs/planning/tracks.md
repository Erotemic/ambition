...
- 2026-07-09 — fable F9.2 IR-consolidation ARC CLOSED (family 6 = hazards + guard deletion). `convert_damage_volume` now LIFTS a legacy inline `motion: KinematicPath` to a synthesized room-level `KinematicPath` (`{iid}__inline_motion`) referenced by `path_id`, so every hazard emits a normal placement — behavior-preserving (`HazardRuntime::new_with_paths` resolves the path_id back to motion; the two F7 contract tests were rewritten to pin the lift instead of the old legacy-Vec-only assertion). Deleted `RoomSpec.hazards`/`RoomEmission.hazards`/the `hazard` helper; render's hazard visual reads `spec.placements`; and the `lowered_hazard_ids` dedup + `for hazard in &room.hazards` fallback — THE DUAL-EMIT GUARD — are removed (HashSet import dropped). RESULT: zero typed per-family Vecs on `RoomSpec`, zero typed spawn loops, `placements` is the SOLE authored-entity channel — the two-channel split-brain the F1.1 ruling flagged is dissolved. A future authored family now adds ONE `PlacementSchema` variant + one lowering interpreter, no second channel to sync. Verified: `ambition_app --features rl_sim` exit 0 (real hazard rooms), content portal 101, actors lib 748, ldtk_map 24, world 25, entity_catalog 16.

- 2026-07-09 — fable F9.1 demo content ADVANCED (umbrella authoring oracle). `ambition_demo_sanic` moved from an empty content plugin to authoring a real momentum SHOWCASE ROOM — `sanic_speedway`: a wide room with a long solid floor and a rideable Sonic LOOP built as an `ae::SurfaceChain` (interior-rideable winding), constructed entirely through the `ambition` umbrella surface (`ambition::engine_core`, `ambition::world::rooms::RoomSpec`, `ambition::prelude`). A headless test composes the room and runs the engine's own chain validator, so a missing umbrella re-export (an engine-for-other-games leak) would fail to compile HERE — the oracle held, nothing was missing. RULING: the FEEL half of demo content (momentum-body tuning to a Sanic identity, a playable demo binary/app shell, character art) is a fundamentally interactive build requiring playtesting and cannot be responsibly completed headlessly (per Jon's don't-pause-for-feel + always-visual directives); the umbrella authoring oracle is advanced+verified, the movement-feel + binary remain the interactive build. `ambition_demo_smb1` untouched (same shape for its 1-1 geometry). demo_sanic: 2 tests green.
