...
| Decomposition D-B | same | **🔴 REOPENED (audit 2026-07-10)** — Series 1 landed the executable gate (`engine.module-size` policy in `tests/ambition_workspace_policy`, config `policies/module_size.toml`: code-line limit 1500 + bidirectional reasoned waiver list, poison-tested; migrated 2026-07-10 from the retired `crates/ambition_runtime/tests/module_size.rs`), regenerated the stale `MODULES.md`, and corrected 42→44. STILL REOPENED, but the over-limit list is shrinking. **`snapshot.rs` (3684) SPLIT 2026-07-11** → `snapshot/{mod,registry,restore,codecs}.rs` (1155/718/471/1379), 46 tests + rl_sim `desync_canary` green, waiver deleted. **`moveset.rs` (1536) SPLIT 2026-07-11** → `moveset/{mod,prefabs}.rs` (862/691) — the builders (`simple_melee`/`build_actor_moveset`/…) went to `prefabs.rs`, runtime playback stayed in `mod.rs`; 102 combat tests green; it was never waived (a pre-existing gate RED the "under the code-line limit" note had masked — the gate counts TOTAL lines). The over-limit set is now the TWO remaining waivers: `portal_presentation/view_cones.rs` (2206), `menu/kaleidoscope_app.rs` (1814). **Gate GREEN (28 policy checks).** | Empty the last two waivers: `view_cones.rs` needs a presentation seam; `kaleidoscope_app.rs` is a declarative Lunex tree (arguably a permanent justified waiver — if so, keep it waived and D-B closes with a one-entry justified list). D-B re-closes when the over-limit list is empty or justified. |
