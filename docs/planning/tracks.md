...
- **SR-6 / TwinTrack classroom spectacle candidate:** make the 35%/55%/75% c clock race visibly readable in the plaza and replace the flat isometric graph with a real perspective 3D 2+1D exhibit: helical worldlines, one-own-second beads, recorded light paths, a wire past light cone, and distinct laboratory/observer simultaneity planes. This is presentation-only and does not make TwinTrack a 3D game. Local compile and high-school classroom-feel validation remain the candidate gate.
...
- **SR-7 / TwinTrack spacetime-minimap candidate:** composite the real perspective 2+1D spacetime scene over live play as a default-on, toggleable minimap using a private physical camera viewport; keep the lab/optical game cameras authoritative and remove solid plaza wall blocks so every clock orbit is spatially reachable and visible. No new simulation authority or rollback state. Local compile and classroom-feel validation remain the candidate gate.
- **SR-8 / TwinTrack open-follow composition candidate:** recenter the laboratory and 35%/55%/75%-of-light clock race into the opening composition, keep the plaza collision-free, and author an effectively unbounded `CameraClampMode::None` follow zone plus a matching deep zero-gravity blast margin so the controlled body can leave the old room rectangle while the ordinary smooth camera continues following. No new engine-global camera policy or rollback state. Local compile and classroom-feel validation remain the candidate gate.
- **SR-9 / TwinTrack dual-observer split-screen direction:** next major gameplay push after SR-8. Two controlled participants share one authoritative simulation while each screen presents that participant's own observer-relative view/reference frame. Use this to make disagreements about simultaneity, visible positions, Doppler shift, clocks, and causal knowledge into direct multiplayer interaction rather than a teacher-panel readout. Not implemented in SR-8.
