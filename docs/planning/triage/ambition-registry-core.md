...
| `MovePrefabRegistry` | `crates/ambition_combat/src/moveset/prefab_registry.rs:21` | `String` | `register` | ⛔ ROW WAS STALE — already `Result<(), String>` and REFUSES a duplicate, with the reason stated in the error: a second builder "would silently change what every existing row expands to" (re-read 2026-09-05) |
...
| `PreparedCharacterRegistry` | `crates/ambition_characters/src/prepared.rs:1320` | `ambition_entity_catalog::CharacterId` | `insert_prepared` | ⛔ ROW WAS STALE — replacement is a RULING, argued in place and measured by making it refuse (4 hot-reload tests fell); the PRODUCTION door `register_character` refuses `DuplicateId`. Two roads, both right (re-read 2026-09-05) |
...
### ⛔⛔ THE VERDICT COLUMN IS UNRELIABLE: THREE OF THREE ROWS RE-READ TODAY WERE STALE

Checked 2026-09-05, one at a time, and every one had already been fixed in source
while the table still described the old shape:

| row | table said | source says |
|---|---|---|
| `RoomContentStagingRegistry` | `register` | already Result + transactional refusal, sealed |
| `PreparedCharacterRegistry` | `silent` | replacement is a RULING, argued in place and measured |
| `MovePrefabRegistry` | `silent` | `Result<(), String>`, refuses duplicates with a reason |

⭐ **The column duplicates a fact that lives in the source** — a function's return
type and its refusal behaviour — and a hand-kept copy of a signature goes stale
the first time somebody improves the signature. That is this page's own thesis
applied to this page: one authority per fact, and the authority for "what does
`register` return" is `register`.

⇒ Read the row as a POINTER, not a verdict. Anything acted on should be re-read
first; three for three is not a sample that supports trusting the rest.

✔ **BUT ONLY THAT COLUMN — MEASURED, because "the rest deserves a re-derive" is
the easy over-generalisation and it is wrong here.** Checked every row
mechanically the same day:

```text
  rows in the table                                    31
  rows whose KEY column says `String`                  23   (the headline holds)
  cited file does NOT define the named struct           0   (citations sound)
  KEY column vs the source map's key type          25 agree / 1 / 5 undetermined
```

The single disagreement is the TABLE being more precise than the source:
`ConstructionRegistry` is listed as `RecipeId (+RelationKind)` where the map keys
on `RecipeId` alone. The five undetermined are shapes the check could not read (no
`HashMap`/`BTreeMap` in the struct body), not shapes it found wrong.

✔ **AND THE VERDICT IS DERIVED NOW** — `scripts/registry_register_returns.py`
reads each row's register fn out of its cited file and prints what it returns, so
the column stops being a copy somebody has to remember to update:

```text
  registry rows                                31
  refusing (Result / Option / bool)            16
  silent (returns `()`)                         4   all four are stated rulings
  no register fn at all                        11
```

The four silent ones are `EncounterRegistry` (an id→live-entity INDEX),
`FrontendAudioRegistry`, `ParamSchemaRegistry` (`classify` not expressible — the
value is a fn pointer) and `PreparedCharacterRegistry` (replacement argued in
place, measured). ⇒ Every remaining "silent" is a decision on the record, not an
oversight.

⚠ It REPORTS and never gates: a return type is the SHAPE of the answer, not
whether the shape is right. A `Result` that is always `Ok`, a refusal by panic,
or a deliberate replacement all read the same from a signature.

⇒ **Trust the identity and the citations; re-read the verdict.** The difference
is which facts are DERIVED FROM A SIGNATURE that somebody later improved -- the
key type rarely changes, the refusal behaviour is exactly what a fix changes.
