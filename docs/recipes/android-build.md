...
```text
target/android/ambition_actors_android/
```
...
```text
target/android/apks/ambition_actors-debug-arm64-v8a.apk
```

The build script copies `crates/ambition_actors/assets/` into the generated
...
`libambition_actors.so`. Do not point `AndroidManifest.xml` directly at
...
Android builds package the checked-out `crates/ambition_actors/assets/` tree into
...
Regenerate or copy the sprite PNGs into `crates/ambition_actors/assets/sprites/`
...
ls -lh target/android/ambition_actors_android/app/src/main/jniLibs/arm64-v8a/libambition_actors.so \
      target/android/apks/ambition_actors-debug-arm64-v8a.apk
...
`static_sfx_bank` Cargo feature when `crates/ambition_actors/assets/audio/sfx.bank`
...
Android builds still package `crates/ambition_actors/assets/` into APK assets,
...
```text
crates/ambition_actors/assets/fonts/bundled/
```
