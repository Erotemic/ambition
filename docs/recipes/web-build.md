...
- **WebServedAssets** (`./build_for_web.sh --served`) bundles
  ...
  `crates/ambition_actors/src/audio/bank_asset.rs`.
...
Run after any change that touches the app web entry, sandbox web features, or asset profile code. Pairs with the desktop `cargo check -p ambition_app` / `cargo check -p ambition_actors` smoke checks.
