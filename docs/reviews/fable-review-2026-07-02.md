...
inversions). **§A1 slice 3 STARTED** (E27–E29): slice-3a landed (bosses are full
victim-side bodies — the vuln trio + `apply_hitbox_damage` `Option` dropped), the
motion+float **parity net** is in place (E28), and the driver fold is precisely
re-scoped (E29) with the elegant end-state (boss = flight-enabled actor archetype,
NOT a bespoke integrate-arm adapter) + its 5 concrete blockers + a bold sequencing.
Other independent open items: the **features/mod.rs hub**. All work is committed
...
2. **Brain-context divergence.** Actors tick via `build_enemy_brain_snapshot` +
   generic `brain.tick`. The boss uses a bespoke `tick_boss_pattern(cfg, state,
   BossPatternContext, frame, &mut BossAttackState)` — richer context
   (encounter_phase / front_wall_clearance / world_size) AND an attack-state
   side-effect the universal `Brain::tick` doesn't produce. Unifying means either
   teaching the universal tick to emit attack-state or carrying `BossPatternContext`
   fields on `BrainSnapshot`.
...
## Next (in order) — A1 slice 3 driver fold (3b→3g, see E29 for the blocker-aware plan) / D4.2 platforms/physics extract / D4.3 LDtk converter extensibility (crux) / D3 blocked on `actors|props`
