...
```text
crates/ambition_actors/src/persistence/save.rs
  game/save flags, player/world progression state

crates/ambition_actors/src/persistence/settings/
...
The older `crates/ambition_actors/src/settings/` path is retired. Do not add new code or docs that point there.
...
```bash
cargo test -p ambition_actors settings
