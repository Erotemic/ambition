...
- **Quality variants** (`sprites_0_5x/`, `sprites_0_25x/`, `sprites_potato/`)
  are generated by `./regen_visual_quality_variants.sh`. `./regen_sprites.sh`
  ends by running its sprite half, and `regen_assets.sh` runs both halves —
  ...
  ```bash
  ./regen_visual_quality_variants.sh --target patent_clerk     # one character, all tiers
  ./regen_visual_quality_variants.sh --target 'pirate_*' --tier 0_5x
  ./regen_visual_quality_variants.sh --force                   # ignore freshness
  AMBITION_QUALITY_VARIANTS=0 ./regen_sprites.sh               # skip it entirely
  ```
