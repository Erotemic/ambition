...
pub fn duelist_moveset(numbers: DuelistNumbers) -> MovesetContract {
    ...
    for (verb, id, params, grounded_only) in [
        ...
        spec.gates = MoveGates {
            ...
            // ⛔ NONE, which is the behaviour these duelists already had: a
            // refused press produces silence. The field arrived with the smash
            // lane's action-variants rung and this exhaustive literal REFUSED to
            // compile without it -- the destructure doing exactly its job, since
            // a `..Default::default()` here would have silently answered a
            // question this file cannot see.
            //
            // ⚠ Naming a fallback for the duel arena is a content decision and
            // belongs to whoever authors these moves; `None` changes nothing.
            when_refused: None,
        };
