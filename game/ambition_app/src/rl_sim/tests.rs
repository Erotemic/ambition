...
fn sim_constructs_and_returns_initial_observation() {
    let mut sim = Platformer2dSimHarness::new_with_timestep(TimestepMode::fixed_60hz()).expect("sim builds");
...
fn fixed_timestep_produces_deterministic_trajectory() {
    ...
    let mut sim_a = Platformer2dSimHarness::new_with_timestep(TimestepMode::fixed_60hz()).unwrap();
    let mut sim_b = Platformer2dSimHarness::new_with_timestep(TimestepMode::fixed_60hz()).unwrap();
...
fn timestep_setter_round_trips() {
    let mut sim = Platformer2dSimHarness::new().unwrap();
...
fn sim_can_start_in_a_specific_room_via_options() {
    ...
    let mut default_sim = Platformer2dSimHarness::new().expect("default builds");
