...
fn the_shipped_app_decides_a_mechanical_edit_before_the_timeline_advances() {
    ...
    // 2026-09-13.** All five live editor roads were migrated onto this chain out
    // of the sim schedule: movement tuning, the ability set, the developer body
    // profile, portal tuning, and the player-stats editor half. A regression that moved one BACK would leave the chain non-empty
    ...
    assert!(
        proposers.len() >= 5 && publishers.len() >= 5,
        "the mechanical-edit chain has {} proposer(s) and {} publisher(s); three \
         five live editor roads were migrated onto it and a road that went back \
         to `app.sim_schedule()` writes inside the rollback window again",
        proposers.len(),
        publishers.len()
    );
