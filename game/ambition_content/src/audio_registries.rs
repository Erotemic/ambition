...
/// `ron::from_str(MUSIC_REGISTRY_RON)` — a second reader of a file the pack
/// already reads. `MusicRegistry::validate()` did run, but only later and only
/// inside `AudioCatalogFragment::new`, which needs an App; and nothing anywhere
/// asked whether the OGG each track points at exists. The `music_registry`
/// schema declares every track's audio file as a requirement, so a strict
/// asset check reports a track whose file was never rendered.
