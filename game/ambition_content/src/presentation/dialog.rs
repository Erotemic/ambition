...
#[derive(Component)]
struct AmbitionDialogBody;

#[derive(Component)]
struct AmbitionDialogFooter;
    ...
    fn for_viewport(viewport: Vec2) -> Self {
        if viewport.y < 480.0 {
            ...
            Self {
                ...
                panel_gap: 5.0,
                portrait_width: 56.0,
                portrait_height: 62.0,
                speaker_font_size: 23.0,
                body_font_size: 20.0,
...
mod tests {
    ...
    fn mobile_layout_preserves_readable_text_and_limits_visible_choices() {
        ...
        assert_eq!(landscape.option_capacity, 1);
    ...
    fn ambition_dialog_is_horizontally_centered_in_the_upper_safe_band() {
        ...
        let mut bodies = app
            .world_mut()
            .query_filtered::<&Node, With<AmbitionDialogBody>>();
        let body = bodies.single(app.world()).expect("one dialogue body");
        assert_eq!(body.flex_shrink, 0.0);

        let mut footers = app
            .world_mut()
            .query_filtered::<&Node, With<AmbitionDialogFooter>>();
        let footer = footers.single(app.world()).expect("one dialogue footer");
        assert_eq!(footer.flex_shrink, 0.0);

        let mut choice_viewports = app
            .world_mut()
            .query_filtered::<Entity, With<AmbitionDialogChoiceViewport>>();
        assert_eq!(
            choice_viewports.iter(app.world()).count(),
            0,
            "non-branching dialogue does not create an empty choice viewport",
        );
    ...
    fn long_choice_lists_render_only_the_selected_window_with_absolute_slots() {
        ...
        let mut choice_viewports = app
            .world_mut()
            .query_filtered::<&Node, With<AmbitionDialogChoiceViewport>>();
        let choice_viewport = choice_viewports
            .single(app.world())
            .expect("one choice viewport");
        assert_eq!(
            choice_viewport.flex_shrink, 0.0,
            "the bounded options region retains its measured height",
        );

        let mut choices = app.world_mut().query::<(&DialogChoiceSlot, &Node)>();
        let mut slots = choices
            .iter(app.world())
            .map(|(slot, node)| {
                assert_eq!(node.flex_shrink, 0.0, "choice rows keep their text height");
                slot.index
            })
            .collect::<Vec<_>>();
        slots.sort_unstable();
