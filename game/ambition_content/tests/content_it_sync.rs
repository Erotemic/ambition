...
const AGGREGATE: &str = include_str!("content_it.rs");
...
fn every_integration_source_is_included_in_the_aggregate() {
    ...
    let on_disk: BTreeSet<String> = std::fs::read_dir(&dir)
        ...
        .filter(|name| name != "content_it")
        .collect();

    let missing: Vec<_> = on_disk.difference(&declared).collect();
    assert!(
        missing.is_empty(),
        "these tests/*.rs files are NOT in content_it.rs and therefore never run: {missing:?}\n\
         add `mod <name>;` to tests/content_it.rs"
    );
    let orphaned: Vec<_> = declared.difference(&on_disk).collect();
    assert!(
        orphaned.is_empty(),
        "content_it.rs declares modules with no source file: {orphaned:?}"
    );
}
