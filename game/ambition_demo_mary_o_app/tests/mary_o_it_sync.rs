...
const AGGREGATE: &str = include_str!("mary_o_it.rs");
...
fn every_integration_source_is_included_in_the_aggregate() {
    ...
    let on_disk: BTreeSet<String> = std::fs::read_dir(&dir)
        ...
        .filter(|name| name != "mary_o_it")
        .collect();

    let missing: Vec<_> = on_disk.difference(&declared).collect();
    assert!(
        missing.is_empty(),
        "these tests/*.rs files are NOT in mary_o_it.rs and therefore never run: {missing:?}\n\
         add `mod <name>;` to tests/mary_o_it.rs"
    );
    let orphaned: Vec<_> = declared.difference(&on_disk).collect();
    assert!(
        orphaned.is_empty(),
        "mary_o_it.rs declares modules with no source file: {orphaned:?}"
    );
}
