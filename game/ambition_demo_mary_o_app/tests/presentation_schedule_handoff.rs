...
//! real provider, route, session and 4:3 presentation profile.
...
fn mary_o_app(display: ae::Vec2) -> App {
...
fn fixed_tick_mary_o_keeps_one_layout_across_a_resize() {
    ...
    // A 4:3 profile on a 20:9 display really must pillarbox, or the fixture is
    // not exercising the interesting path at all.
    let gameplay = app
        .world()
        .resource::<ResolvedGameplayPresentation>()
        .gameplay_rect
        .size();
    assert_eq!(gameplay, ae::Vec2::new(1440.0, 1080.0));

    // ONE update after the resize. This is the whole test: a stale snapshot
    // survives exactly one frame, so a single update is what distinguishes a
    // real handoff from a one-frame-old mixture.
    ...
    assert_eq!(
        app.world()
            ...
            .gameplay_rect
            .size(),
        ae::Vec2::new(1600.0, 1200.0),
        "1600x1200 is 4:3 exactly, so the gameplay rect fills the display",
    );
}
