...
//! compile here.
//!
//! This crate holds three LDtk acts and the Sanic rules. The first act's loop
//! route is grafted onto its loaded room in [`sanic_speedway`]. The second and
//! third acts author their loops in LDtk. The windowed shell loads the shared
//! Ambition art tree through the public facade.
...
/// Act 3: the long night course.
pub const DARKNESS_ROOM_ID: &str = "sanic_darkness";

/// Act 3's level file.
pub const DARKNESS_WORLD_JSON: &str = include_str!("../assets/worlds/sanic_darkness.ldtk");
...
pub fn sanic_highway() -> RoomSpec {
    let project = ambition_platformer2d::ldtk_map::LdtkProject::from_json_str(HIGHWAY_WORLD_JSON)
        .expect(
        "sanic_highway.ldtk parses (regen: game/ambition_demo_sanic/tools/author_highway_ldtk.py)",
    );
    let room_set = project
        .to_room_set_with_entry(
            HIGHWAY_ROOM_ID,
...
/// Load the dark third act from its LDtk source.
pub fn sanic_darkness() -> RoomSpec {
    let project = ambition_platformer2d::ldtk_map::LdtkProject::from_json_str(DARKNESS_WORLD_JSON)
        .expect("sanic_darkness.ldtk parses (regen: game/ambition_demo_sanic/tools/author_darkness_ldtk.py)");
    let room_set = project
        .to_room_set_with_entry(
            DARKNESS_ROOM_ID,
            &ambition_platformer2d::ldtk_map::LdtkVocabulary::engine(),
        )
        .unwrap_or_else(|errors| panic!("sanic_darkness.ldtk converts to rooms: {errors:?}"));
    let mut room = room_set
        .rooms
        .into_iter()
...
/// the speedway leads to the highway, then to the dark act and back. The trip is the
