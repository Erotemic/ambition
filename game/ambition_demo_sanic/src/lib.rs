...
impl Plugin for SanicRulesPlugin {
    fn build(&self, app: &mut App) {
        ...
        // ⛔⛔ AND THE ADMITTED REPLAY, which the monitor re-arm READS. A world
        // that does not register a message a system reads fails parameter
        // validation and the system is DROPPED -- so without this, a thin
        // rules-only harness silently loses the re-arm and the demo's own tests
        // panic on an unnamed system. Registered here rather than in each
        // fixture for the same reason the line above is: `add_message` is
        // idempotent, and a harness should not have to know what its content
        // systems read.
