...
fn activate_player(app: &mut App) -> ambition_platformer2d::engine_core::BodyKinematics {
    ...
    panic!("the fresh load transaction must prepare and activate Sanic within the test budget");
}
...
fn the_demos_own_rules_run_because_its_room_claims_its_mode() {
    ...
    // The act clock is the SIM clock, so bullet-time and pause would slow it exactly as they
