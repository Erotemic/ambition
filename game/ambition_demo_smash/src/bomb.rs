...
/// `DamageBoxEffect::knockback` actually is — the same band as the shockwave
/// (1.3), the beam (1.1) and the boss blast (1.6).
///
/// ⭐ ABOVE THE BOSS'S, deliberately: a bomb you can see, hear and walk away from
/// for four seconds should hit harder than a hazard that arrives unannounced.
        ...
        effects.write(ambition_platformer2d::vfx::EffectRequest {
            ...
            effect: ambition_platformer2d::vfx::Effect::DamageBox(
                ambition_platformer2d::vfx::DamageBoxEffect {
                    ...
                    // `blast_radius * 2.4` = 134.4. `spawn_damage_box` says it in
                    // words — "DamageBox knockback is authored as a feel
                    // multiplier (values such as 1.0 or 1.6), not an engine-unit
                    // melee launch speed" — and every other author in the
                    // workspace obeys it: the shockwave 1.3, the beam 1.1, the
                    // boss 1.6, the exploder 1.6.
                    //
                    // ⇒ A 134× launch. Jon, 2026-09-05: *"does the projectile
                    // polygon bomb just straight up kill the other player?"* It
                    // did, at any percent, and it was never a balance number —
                    // it was a units error that `f32` cannot catch.
                    //
                    // ⚠ DERIVING IT FROM THE RADIUS WAS THE ROAD IN. A blast's
                    // reach and its launch are different facts; multiplying one
                    // by a constant to get the other reads like tuning and is a
                    // unit change. Stated as its own number now.
