...
//! `mod`-includes every integration source, collapsing 2 Bevy-linking test
...
const AGGREGATE: &str = include_str!("smash_it.rs");
...
fn every_integration_source_is_included_in_the_aggregate() {
    ...
    let on_disk: BTreeSet<String> = std::fs::read_dir(&dir)
        ...
        .filter(|name| name != "smash_it")
        .collect();

    let missing: Vec<_> = on_disk.difference(&declared).collect();
    assert!(
        missing.is_empty(),
        "these tests/*.rs files are NOT in smash_it.rs and therefore never run: {missing:?}\n\
         add `mod <name>;` to tests/smash_it.rs"
    );
    let orphaned: Vec<_> = declared.difference(&on_disk).collect();
    assert!(
        orphaned.is_empty(),
        "smash_it.rs declares modules with no source file: {orphaned:?}"
    );
}
