...
def main() -> int:
    ...
    rows = sorted(targets, key=lambda c: (files[c], len(dependents[c])))
    print(f"{'crate':32s} {'manifests':>9s} {'files':>6s}   cheapest first, by FILES")
    for crate in rows:
        print(f"  {crate:30s} {len(dependents[crate]):9d} {files[crate]:6d}")
    print(
        "\n⚠ Sorted by FILES, not by manifests. The two disagree: "
        f"the crate with the fewest manifests is `{min(targets, key=lambda c: len(dependents[c]))}` "
        f"and the one with the fewest files is `{rows[0]}`."
    )
    return 0
