...
ADJUDICATED: dict[str, str] = {
    ...
    "SwitchActivationQueue": (
        "OPEN — ONE PRODUCER, ONE CONSUMER, AND **NOTHING ORDERS THEM**. Measured "
        "per system 2026-09-18: `apply_switch_effects` "
        "(`features/ecs/effect_bus.rs`) PUSHES, inside "
        "`Platformer2dSimulationPhaseMonolith::GameplayEffects`; "
        "`drain_switch_activations` (`ambition_encounter/src/switches.rs`) "
        "`std::mem::take`s the whole queue, registered `.in_set("
        "SwitchActivationDrained)` — and that set is never `configure_sets`'d into "
        "any simulation phase. Two consumers order `.after` the drain "
        "(`encounter_features/src/lib.rs`, `features/ecs/encounter_rewards.rs`); "
        "NOTHING orders the drain against the producer. ⇒ Whether an activation is "
        "drained on the tick it was pushed or the next one is decided by executor "
        "order, which is stable per build and arbitrary — so a behavioural test "
        "passes either way, and `ResolvedSwitchActivations` (cleared at the head of "
        "the drain) reaches its readers a tick late or not, unpredictably across "
        "builds. ⚠ This is NOT the ordering gap `switches.rs` already records at "
        "line 437: that one is two writers of the SAVE's switch family "
        "(`drain_switch_activations` vs `content/src/falling_sand_sim.rs`), routed "
        "to `world-facts-observations-and-memory.md`. This is the queue's own "
        "producer/consumer edge, and it is not recorded anywhere yet. The third "
        "writer is `SESSION_SCOPE_RESET`."
    ),
