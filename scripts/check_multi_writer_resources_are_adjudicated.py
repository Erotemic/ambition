...
ADJUDICATED: dict[str, str] = {
    ...
    "FallingSandRoomState": (
        "CORRECT — ONE ROOM-CHANGE BOUNDARY PLUS ONE LATCH OWNER, read "
        "field by field rather than system by system, because the file-granular "
        "count says nothing about which of its three fields a writer touches. "
        ...
        "sets `seeded_boundaries` once and is guarded by it. The room's swim "
        "is a keyed ability contribution (`lend_room_swim`), not a field here. "
        "⇒ The only field two "
    ...
    "BaseGravity": (
        "CORRECT, AND THE SIXTH WRITER IS GONE — Q137 RULED AND THE DEAD PLATE "
        "WAS DELETED 2026-09-19. Six files, seven production sites. ONE developer road, and it is an "
        ...
        "baseline behind it.\n"
        "    ⭐ THE SIXTH WRITER WAS A DUPLICATED MECHANIC THAT COULD NOT FIRE, "
        "AND IT IS DELETED — Q137, 2026-09-19. The gravity flip-switch system "
        "computed the same negation as the encounter road's FlipGravity arm, "
        "and its only registration in the workspace was an add_systems call "
        "inside its own test module. It was not free: the component was "
        "rollback-registered TWICE so it sat inside the schema fingerprint two "
        "peers compare, the sim-view facts crate rebuilt a view over its "
        "always-empty query every tick, the renderer drew it, and the "
        "rollback exit oracle named it by string. Gravity SWITCHING stays and "
        "the unreachable overlap-plate vertical went, so the component, the "
...
SESSION_WORLD_ADJUDICATED: dict[str, str] = {
    "EncounterMusicRequest": (
        ...
        "`base_track` (`ambition_encounter_features/src/systems.rs`); ONE "
        "wrote only a last-applied track mirror (`actor_monolith/src/music/intent.rs`), "
