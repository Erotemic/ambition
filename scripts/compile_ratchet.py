...
def report(current: dict, frozen: dict) -> None:
    ...
    # ⛔⛔ A PARTIALLY ADOPTED BASELINE HOLDS NUMBERS FROM TWO COMMITS, and saying
    # only the newer one sends a reader hunting in the wrong range. `--adopt-wins`
    # banks improvements and leaves every regression at its older, tighter value;
    # the line above is the ADOPTING commit, which is the provenance of the
    # adopted numbers ONLY.
    # ⛔⛔⛔ A BASELINE THAT DISAGREES WITH ITSELF, DISCLOSED RATHER THAN JUDGED.
    # `worst_edit_cost` and `watched_edit_cost` are DERIVED from the `crates`
    # table -- a max and a selection over it. `--adopt-wins` refreshes the table
    # from the CURRENT snapshot and writes the held scalars back from the OLD
    # one, so the file ends up holding one fact twice with different values.
    #
    # Measured 2026-09-05: `ambition_geometry` reads 540,227 in the held scalar
    # and 592,091 in the table of the same file. The findings compare today
    # against the STALE copy, which turns +8,439 of real movement (inside a
    # +10,804 budget) into a reported +60,303 regression -- and the same three
    # crates are then counted twice more in seconds.
    #
    # ⚠ This only SAYS SO. Changing which number the findings compare against is
    # a change to what the gate MEANS, and that belongs to whoever owns the carve
    # programme -- not to the run that noticed.
    for label, entry in _derived_scalars(frozen):
        crate = entry.get("crate")
        table = (frozen.get("crates") or {}).get(crate, {}).get("edit_cost_lines")
        stored = entry.get("lines")
        if crate and table is not None and stored is not None and table != stored:
            print(
                f"  ⚠ baseline disagrees with itself: {label} holds "
                f"{stored:,} lines for {crate}, its own `crates` table says "
                f"{table:,}. Findings below compare against the STORED value, so "
                f"{table - stored:+,} of that gap predates this baseline."
            )
...
def _derived_scalars(frozen: dict):
    """`(label, entry)` for every stored scalar that is DERIVED from `crates`.

    ⭐ Named in one place because the disclosure above and any future repair must
    agree on which numbers are derivations. A scalar that is genuinely primary --
    `critical_path_crates` is a max over depths with no per-crate line count --
    does not belong here.
    """
    out = []
    worst = frozen.get("worst_edit_cost")
    if isinstance(worst, dict):
        out.append(("worst_edit_cost", worst))
    for name, entry in (frozen.get("watched_edit_cost") or {}).items():
        if isinstance(entry, dict):
            out.append((f"watched_edit_cost[{name}]", {**entry, "crate": name}))
    return out
...
#: The scalar metrics `--accept` can re-freeze one at a time.
#:
#: ⭐ A LIST, NOT "anything in the baseline". A baseline key is not automatically
#: a thing a human can reason about accepting -- `unit_weights` and
#: `unpriced_crates` are inputs to the pricing, and accepting them at "today's
#: value" is the laundering `--adopt-wins` already had to be taught not to do.
ACCEPTABLE_METRICS = {
    "critical_path_crates",
    "largest_unit",
    "worst_edit_cost",
...
def accept_regressions(
    current: dict, frozen: dict, metrics: list[str], reason: str
) -> tuple[dict, list[str], list[str]]:
    """Re-freeze exactly the NAMED metrics at today's value, with a reason.

    ⭐ THE MIDDLE VERB THE RATCHET WAS MISSING. `--update` accepts everything and
    `--adopt-wins` accepts only improvements; neither can express "this one
    regression is a deliberate landing", which is precisely what the regression
    message asks the reader to say.

    Returns `(baseline, accepted, unknown)`.
    """
    unknown = [m for m in metrics if m not in ACCEPTABLE_METRICS]
    if unknown:
    ...
    for metric in metrics:
        before, after = frozen.get(metric), current.get(metric)
        merged[metric] = after
        accepted.append(f"{metric}: {_brief(before)} -> {_brief(after)}")
    # ⚠ The provenance rule this file already learned: a baseline holding numbers
    # from two commits records both. An accept moves SOME numbers to today, so
    # the rest are still carried from wherever they were.
    merged.setdefault("carried_from", frozen.get("commit"))
    # ⛔⛔ DO NOT STAMP THE PROVENANCE HERE, AND THE FIRST VERSION DID.
    # `freeze()` builds `{**envelope("graph"), **current, ...}`, and `envelope`
    # supplies `commit` as a real sha or the string "unknown". A key set here
    # OVERRIDES that -- and `snapshot()` carries no `commit`, so
    # `current.get("commit")` is `None`. The baseline was written with
    # `commit: null`, destroying the very provenance field this command exists
    # beside, and it was pushed.
    #
    # ⭐ `adopt_wins` never had the bug because it never set the key: it lets the
    # envelope stamp. The fix is to do the same, not to compute a better value --
    # the authority for "which commit is this" already exists and is one call
    # away.
    #
    # ⚠ And my own test asserted the broken behaviour (`merged["commit"] ==
    # "newersha"`), so it read as verified. A test written from the
    # implementation cannot witness the implementation being wrong.
    # ⭐ The reason is stored PER METRIC, because the next reader's question is
    # "why is this number what it is", not "what was the last thing anyone
    # accepted".
...
def _brief(value) -> str:
    """A metric's value in one short phrase, whether it is a scalar or a dict."""
    if isinstance(value, dict):
        for key in ("lines", "seconds", "crates"):
            if key in value:
                return f"{value.get('crate', '?')} {value[key]:,}"
        return str(value)
...
def adopt_wins(current: dict, frozen: dict) -> tuple[dict, list[str], list[str]]:
    ...
    # the new commit -- 538 insertions, 411 deletions -- while HOLDING the scalar
    # regressions at their older values, `critical_path_crates: 14` among them.
    # ⇒ The file genuinely mixes two commits, which is the point: the report then
    # said "baseline frozen at c4a51a1b76a9" and `--diff` said "since
    # c4a51a1b76a9", sending a reader to hunt a change in a range where NO
    # manifest changed at all, while the PATH finding instructed them to "say
    # which carve did it" about a carve that cannot be in that range.
    #
    # ⚠ My first note here said that commit "changed ONLY `commit`". It did not;
    # I had grepped the diff for two field names and read the filtered view as
    # the whole. The conclusion survived because the SCALARS were held, which is
    # what `carried_from` describes -- but the sentence was false, and a filtered
    # grep reported as a complete diff is the same mistake this file's own
    # findings keep turning up.
    #
...
def freeze(current: dict) -> None:
    ...
    # ⛔⛔ A BASELINE WITH NO PROVENANCE CANNOT BE REASONED ABOUT AT ALL, and one
    # was written and pushed on 2026-09-05. `envelope()` supplies a real sha (or
    # the string "unknown"), but the `**current` spread OVERRIDES it, so any
    # caller that sets `commit` from a snapshot -- which carries none -- writes
    # `null` over it silently.
    #
    # ⭐ Guarded HERE rather than in the caller that did it, because every road
    # (`--update`, `--adopt-wins`, `--accept`) lands in this function: one place
    # to state "a baseline names the commit it came from", instead of three
    # places to remember it.
    if not frozen.get("commit"):
        raise SystemExit(
            "⛔ refusing to freeze a baseline with no `commit`: every finding it "
            "produces would name a range nobody can look in. `envelope()` "
            "supplies one, so this means a caller overrode it with a null."
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--accept", metavar="METRIC", action="append", default=[],
                        help="accept ONE named regression at today's value, with "
                             "--reason. Everything else stays frozen. Repeatable.")
    parser.add_argument("--reason", metavar="TEXT",
                        help="why the --accept'ed regression is a deliberate "
                             "landing; recorded in the baseline beside it")
    ...
    if args.accept:
        # ⛔⛔ THE TOOL ASKED A QUESTION IT GAVE NOBODY A WAY TO ANSWER. Its own
        # regression message says "if this is a deliberate landing, say so and
        # re-freeze" -- and the only re-freezes were `--update`, which banks
        # EVERY regression including the ones nobody looked at, and
        # `--adopt-wins`, which banks none of them. So a finding that was
        # triaged, attributed and accepted stayed red for a week, and the gate
        # taught its readers that red is the normal colour.
        #
        # ⭐ `critical_path_crates: 14 -> 16` is the worked example: `368f1b551`
        # derived the chain from cargo metadata and named the two D33 carve
        # outputs sitting at positions 9 and 10. Nothing was wrong; there was no
        # way to say so about ONE number.
        if not args.reason:
            print(
                "⛔ --accept needs --reason. Accepting a regression without one "
                "produces a number nobody can defend when it moves again, which "
                "is the failure --adopt-wins exists to avoid.",
                file=sys.stderr,
            )
            return 2
        merged, accepted, unknown = accept_regressions(
            current, frozen, args.accept, args.reason
        )
        if unknown:
            print(
                f"⛔ not a metric this baseline carries: {', '.join(unknown)}.\n"
                f"   known: {', '.join(sorted(ACCEPTABLE_METRICS))}",
                file=sys.stderr,
            )
            return 2
        freeze(merged)
        print("  accepted at today's value, with the reason recorded:")
        for line in accepted:
            print(f"    {line}")
        print(f"    reason: {args.reason}")
        print(
            "\n  ⚠ Every OTHER finding stays frozen and red. Accepting one "
            "number is not\n     accepting the run."
        )
        return 0
