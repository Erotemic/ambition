...
def test_the_linker_and_codegen_threads_are_build_tooling():
    for comm in ["ld.mold", "lto cgu.00", "opt cgu.15", "rustc", "cargo", "clang"]:
        assert bucket(comm) == "build tooling", (
            f"{comm!r} is a compile thread; counting it as the game hides a build "
...
def test_the_games_own_threads_are_still_the_game():
    ...
    Without this, widening `build tooling` until it matched everything would
    pass the test above.
    """
