...
def test_every_reading_names_a_mechanism_and_a_date():
    ...
    clothes. Each must name WHICH of the five reasons applies."""
    reasons = ("SCRATCH", "LATCH", "NOT ROLLBACK STATE", "CACHED QUERY", "PER-INSTANCE")
    for name, reading in guard.ADJUDICATED.items():
        ...
        assert any(r in reading for r in reasons), (
            f"{name}'s reading names none of the five mechanisms: {reading[:80]}"
        )
