...
def _render_sky(profile_name: str, spec: LayerSpec) -> Image.Image:
    ...
    if style.star_count > 0:
        _draw_stars(
            draw, rng, spec.width, spec.height, style.star_count, style.star_color
        )
    ...
    if style.silhouette_kind in {"cave", "sea"}:
        for _ in range(8):
            ...
            if style.silhouette_kind == "cave":
                draw.polygon(
                    [(x, 0), (x + w // 2, y), (x + w, 0)], fill=style.far_colors[1]
                )
            else:
...
def _draw_mid_detail(
    draw: ImageDraw.ImageDraw, rng: random.Random, spec: LayerSpec, style: LayerStyle
) -> None:
    ...
    if kind in {"spires", "ruins"}:
        for i in range(11):
            ...
            draw.rounded_rectangle(
                (x, y0, x + w, spec.height), radius=3, fill=style.mid_detail
            )
            if rng.random() < 0.55:
        ...
        for i in range(7):
            ...
            draw.rounded_rectangle(
                (x, y0, x + w, spec.height), radius=8, fill=style.mid_detail
            )
            draw.line((x + w // 2, y0, x + w // 2, y0 - 40), fill=style.accent, width=4)
            if rng.random() < 0.7:
                draw.rectangle(
                    (x + 5, y0 + 12, x + w - 5, y0 + 16), fill=style.mid_glow
                )
    elif kind == "palms":
        for _ in range(9):
            ...
            draw.line(
                (x, base_y, x + rng.randrange(-10, 10), top_y),
        ...
        for _ in range(6):
            ...
            draw.rounded_rectangle(
                (x0, y0, x0 + w, y0 + 10), radius=4, fill=style.mid_detail
            )
            draw.line(
                (x0 + 10, y0 + 10, x0 + 10, spec.height), fill=style.mid_detail, width=6
            )
            draw.line(
                (x0 + w - 10, y0 + 10, x0 + w - 10, spec.height),
                fill=style.mid_detail,
                width=6,
            )
    elif kind == "shards":
    ...
    if kind in {"branches", "cables"}:
        ...
        for _ in range(18):
            ...
            for step in range(1, 8):
                point = (
                    x + int(math.sin((step / 2.0) + x * 0.02) * 18),
                    base_y - step * rng.randrange(12, 20),
                )
        ...
        for _ in range(12):
            ...
            draw.line(
                (x0, y0, x1, y1), fill=style.near_line, width=rng.randrange(5, 11)
            )
