...
def _identity_profile(func: F | None = None, *args: Any, **kwargs: Any):
...
# Modern line_profiler usage is environment-gated: run normal code with no
# profiling overhead, then opt in with ``LINE_PROFILE=1 python ...``.  Older
# versions of line_profiler also expose ``profile`` as a decorator, so we only
# import it when the env var is truthy and otherwise keep annotations as no-ops.
_LINE_PROFILE = str(os.environ.get("LINE_PROFILE", "")).strip().lower()
if _LINE_PROFILE and _LINE_PROFILE not in {"0", "false", "no", "off"}:  # pragma: no cover - optional dev path.
    try:
        from line_profiler import profile as profile  # type: ignore
    except Exception:  # noqa: BLE001 - optional developer dependency.
