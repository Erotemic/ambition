...
THEMES: tuple[Theme, ...] = (
    Theme("hub", (16, 21, 54), (28, 36, 84), (7, 11, 24), (22, 28, 68), (110, 98, 188), (196, 218, 255), "moon", "station_night", 72),
    Theme("lab", (8, 30, 42), (12, 56, 68), (5, 18, 24), (14, 45, 56), (56, 168, 166), (148, 244, 228), "stars", "industrial_reactor", 34),
    Theme("basement", (34, 18, 18), (56, 32, 26), (14, 10, 10), (54, 30, 20), (168, 98, 62), (255, 176, 110), "embers", "ruined_furnace", 0),
    Theme("cove", (8, 34, 56), (18, 86, 94), (5, 26, 30), (18, 52, 58), (72, 150, 124), (194, 248, 228), "moon", "lagoon", 40),
    Theme("skybridge", (86, 124, 176), (132, 172, 208), (68, 92, 136), (94, 120, 160), (206, 216, 226), (255, 244, 218), "sun", "floating_bridge", 0),
    Theme("boss", (36, 8, 24), (76, 18, 34), (14, 5, 11), (64, 18, 30), (204, 56, 80), (255, 124, 78), "eclipse", "arena_spires", 18),
    Theme("water", (2, 24, 44), (7, 62, 80), (2, 14, 28), (8, 48, 66), (40, 132, 132), (128, 232, 236), "caustics", "underwater_ruins", 0),
    Theme("forest", (16, 24, 28), (42, 58, 46), (16, 24, 18), (30, 48, 32), (90, 126, 78), (222, 236, 196), "moon", "ninja_dojo_forest", 18),
    Theme("cave", (12, 14, 26), (22, 26, 40), (5, 7, 16), (22, 24, 36), (92, 86, 116), (186, 166, 224), "none", "crystal_cave", 0),
)
...
def _poly(draw: ImageDraw.ImageDraw, pts: Iterable[tuple[float, float]], fill: RGBA) -> None:
    draw.polygon([(int(x), int(y)) for x, y in pts], fill=fill)


def _line(draw: ImageDraw.ImageDraw, pts: Iterable[tuple[float, float]], fill: RGBA, width: int) -> None:
...
def _periodic_band(
    ...
    for i in range(steps + 1):
        x = SIZE * (i / steps)
        t = x / SIZE
        y = base_y + math.sin(t * math.tau + phase1) * amp1 + math.sin(t * math.tau * 2.0 + phase2) * amp2
        top.append((x, y))
...
def _blurred_alpha_composite(base: Image.Image, overlay: Image.Image, blur: float) -> None:
    if blur > 0:
...
def _draw_stars(draw: ImageDraw.ImageDraw, rng: random.Random, count: int, color: RGBA) -> None:
    for _ in range(count):
        ...
        if r == 2 and rng.random() < 0.55:
            draw.line([(x - 4, y), (x + 4, y)], fill=(c[0], c[1], c[2], a // 2), width=1)
            draw.line([(x, y - 4), (x, y + 4)], fill=(c[0], c[1], c[2], a // 2), width=1)


def _draw_moon(base: Image.Image, center: tuple[int, int], radius: int, tint: RGB) -> None:
    glow, gdraw = _new_overlay()
    x, y = center
    gdraw.ellipse([x - radius - 26, y - radius - 26, x + radius + 26, y + radius + 26], fill=_rgba(tint, 34))
    _blurred_alpha_composite(base, glow, 16)
    moon, mdraw = _new_overlay()
    mdraw.ellipse([x - radius, y - radius, x + radius, y + radius], fill=_rgba((240, 242, 248), 238))
    mdraw.ellipse([x - radius + 12, y - radius - 2, x + radius + 10, y + radius + 2], fill=(0, 0, 0, 128))
    base.alpha_composite(moon)


def _draw_sun(base: Image.Image, center: tuple[int, int], radius: int, tint: RGB) -> None:
    overlay, draw = _new_overlay()
    x, y = center
    for grow, alpha in ((54, 18), (30, 34), (12, 60)):
        draw.ellipse([x - radius - grow, y - radius - grow, x + radius + grow, y + radius + grow], fill=_rgba(tint, alpha))
    draw.ellipse([x - radius, y - radius, x + radius, y + radius], fill=_rgba((255, 242, 202), 230))
...
def _draw_eclipse(base: Image.Image, center: tuple[int, int], radius: int) -> None:
    ...
    for grow, alpha in ((22, 26), (10, 56), (4, 96)):
        draw.ellipse([x - radius - grow, y - radius - grow, x + radius + grow, y + radius + grow], outline=(255, 108, 72, alpha), width=4)
    draw.ellipse([x - radius + 5, y - radius + 4, x + radius + 5, y + radius + 4], fill=(10, 7, 12, 232))
...
def _draw_caustics(draw: ImageDraw.ImageDraw, rng: random.Random, color: RGBA) -> None:
    for y in (90, 132, 190, 250, 322):
        for _ in range(3):
            ...
            _line(draw, [(x0, y), (x0 + 120, y + rng.randint(-12, 12)), (x0 + 220, y + rng.randint(-8, 8))], color, rng.randint(2, 4))


def _draw_embers(draw: ImageDraw.ImageDraw, rng: random.Random, color: RGBA, count: int = 28) -> None:
    for _ in range(count):
        x = rng.randint(16, SIZE - 16)
        y = rng.randint(40, int(SIZE * 0.78))
        r = rng.randint(1, 3)
        draw.ellipse([x - r, y - r, x + r, y + r], fill=(color[0], color[1], color[2], _clamp(color[3] + rng.randint(-18, 18))))
...
def _draw_sky(theme: Theme) -> Image.Image:
    ...
    for y in range(SIZE):
        t = y / (SIZE - 1)
        c = _mix(theme.sky_top, theme.sky_mid, min(1.0, t / 0.42)) if t < 0.42 else _mix(theme.sky_mid, theme.sky_bottom, (t - 0.42) / 0.58)
        ...
        for x in range(SIZE):
            ...
            px[x, y] = (_clamp(c[0] + shade), _clamp(c[1] + shade), _clamp(c[2] + shade), 255)

    haze, hdraw = _new_overlay()
    _periodic_band(hdraw, _rgba(_mix(theme.sky_mid, theme.glow, 0.4), 30), 420, 44, 12, 18, 0.4, 1.7)
    _periodic_band(hdraw, _rgba(_mix(theme.sky_mid, theme.glow, 0.18), 20), 510, 32, 14, 10, 2.4, 0.8)
    ...
    if theme.celestial == "moon":
        _draw_moon(image, (136 if theme.key == "hub" else 574, 114 if theme.key == "hub" else 136), 36 if theme.key == "hub" else 28, theme.glow)
        ...
        for _ in range(36):
            ...
            odraw.ellipse([x - r, y - r, x + r, y + r], fill=_rgba(theme.glow, rng.randint(20, 46)))
    ...
    if theme.key in {"hub", "lab", "skybridge"}:
        _periodic_band(hdraw, _rgba(_mix(theme.glow, theme.sky_mid, 0.65), 28), 446, 22, 8, 6, 1.4, 2.2)
    if theme.key == "water":
        _periodic_band(hdraw, _rgba(_mix(theme.glow, theme.sky_mid, 0.55), 36), 438, 30, 10, 14, 0.8, 1.8)
    ...
    if theme.key == "forest":
        ...
        for x in (92, 188, 304, 470, 610, 704):
            ...
            _poly(cdraw, [(x - 18, 0), (x + 22, 0), (x + 92, SIZE), (x + 36, SIZE)], _rgba(theme.glow, 18))
...
def _add_scaffold(draw: ImageDraw.ImageDraw, color: RGBA, accent: RGBA) -> None:
    for side in (0, 1):
        ...
        draw.arc([x - 84, 50, x + 84, 236], 96 if side == 0 else 264, 264 if side == 0 else 96, fill=color, width=8)
...
def _add_ring_station(draw: ImageDraw.ImageDraw, color: RGBA, accent: RGBA) -> None:
    ...
    draw.rounded_rectangle([cx - 18, cy - 84, cx + 18, cy + 84], radius=6, fill=_scale_alpha(accent, 0.7))
    draw.rounded_rectangle([cx - 84, cy - 18, cx + 84, cy + 18], radius=6, fill=_scale_alpha(color, 0.8))
    ...
    while x < SIZE - 70:
        ...
        if rng.random() < 0.7:
            for yy in range(468 - h + 8, 466, 12):
                draw.rectangle([x + 4, yy, x + w - 4, yy + 3], fill=_scale_alpha(accent, 0.45))
...
def _add_reactor_towers(draw: ImageDraw.ImageDraw, color: RGBA, accent: RGBA) -> None:
    ...
    for cx, base_y, w, h in towers:
        draw.rounded_rectangle([cx - w // 2, base_y - h, cx + w // 2, base_y], radius=10, fill=color)
        draw.rounded_rectangle([cx - w // 2 + 12, base_y - h + 26, cx + w // 2 - 12, base_y - h + 44], radius=4, fill=_scale_alpha(accent, 0.7))
        draw.rectangle([cx - 6, base_y - h - 30, cx + 6, base_y - h + 4], fill=_scale_alpha(accent, 0.8))
    for y in (142, 210, 296):
        _line(draw, [(0, y), (178, y + 22), (328, y + 8)], _scale_alpha(color, 0.8), 7)
        _line(draw, [(430, y + 18), (600, y), (768, y + 12)], _scale_alpha(color, 0.8), 7)


def _add_hanging_cables(draw: ImageDraw.ImageDraw, color: RGBA, accent: RGBA, alpha_scale: float = 1.0) -> None:
    rng = random.Random(4242)
    for x in (58, 112, 656, 716):
        depth = rng.randint(180, 360)
        _line(draw, [(x, 0), (x + rng.randint(-18, 18), depth * 0.55), (x + rng.randint(-28, 28), depth)], _scale_alpha(color, alpha_scale), 6)
        draw.rounded_rectangle([x - 14, depth - 10, x + 14, depth + 10], radius=4, fill=_scale_alpha(accent, 0.8 * alpha_scale))
...
def _add_furnace_arches(draw: ImageDraw.ImageDraw, color: RGBA, accent: RGBA) -> None:
    for cx, w, h in ((168, 156, 220), (390, 212, 264), (630, 128, 188)):
        draw.arc([cx - w // 2, 246, cx + w // 2, 246 + h], 180, 360, fill=color, width=10)
        draw.arc([cx - w // 2 + 18, 264, cx + w // 2 - 18, 246 + h - 18], 180, 360, fill=_scale_alpha(accent, 0.75), width=5)
...
def _add_pipes(draw: ImageDraw.ImageDraw, color: RGBA, accent: RGBA) -> None:
    ...
    for x in (84, 690):
        _line(draw, [(x, 0), (x, 216), (x + (12 if x < 200 else -12), 404), (x, 650)], accent, 8)


def _add_sea_cliffs(draw: ImageDraw.ImageDraw, color: RGBA, accent: RGBA) -> None:
    _poly(draw, [(0, 432), (88, 382), (152, 398), (206, 350), (240, 358), (282, 454), (0, 544)], color)
    _poly(draw, [(768, 418), (654, 370), (588, 392), (520, 356), (468, 366), (432, 456), (768, 560)], color)
...
def _add_reeds(draw: ImageDraw.ImageDraw, color: RGBA, accent: RGBA, alpha_scale: float = 1.0) -> None:
    ...
    for x_region in ((18, 148), (618, 748)):
        for _ in range(18):
            ...
            _line(draw, [(x, SIZE), (x + bend * 0.35, SIZE - h * 0.6), (x + bend, SIZE - h)], _scale_alpha(color, alpha_scale), rng.randint(2, 4))
            if rng.random() < 0.48:
                draw.ellipse([x + bend - 4, SIZE - h - 10, x + bend + 5, SIZE - h + 8], fill=_scale_alpha(accent, 0.7 * alpha_scale))
...
def _add_bridge_pylons(draw: ImageDraw.ImageDraw, color: RGBA, accent: RGBA) -> None:
    ...
    for cx, base_y, w, h in pylons:
        draw.polygon([(cx - w, base_y), (cx + w, base_y), (cx + w // 3, base_y - h), (cx - w // 3, base_y - h)], fill=color)
        draw.rectangle([cx - 6, base_y - h - 26, cx + 6, base_y - h + 10], fill=_scale_alpha(accent, 0.85))
    _line(draw, [(90, 328), (220, 352), (384, 320), (542, 350), (676, 334)], _scale_alpha(accent, 0.85), 8)
...
def _add_monoliths(draw: ImageDraw.ImageDraw, color: RGBA, accent: RGBA) -> None:
    ...
    for x1, y1, x2, y2 in ((164, 338, 366, 286), (414, 270, 604, 356), (366, 286, 610, 216)):
        _line(draw, [(x1, y1), ((x1 + x2) // 2, (y1 + y2) // 2 - 18), (x2, y2)], _scale_alpha(accent, 0.65), 4)
...
def _add_ruins_and_kelp(draw: ImageDraw.ImageDraw, color: RGBA, accent: RGBA, glow: RGBA) -> None:
    for x, w, h in ((248, 62, 168), (398, 54, 204), (548, 44, 150)):
        draw.rectangle([x - w // 2, 472 - h, x + w // 2, 472], fill=color)
        draw.rectangle([x - w // 2 + 10, 472 - h + 24, x + w // 2 - 10, 472 - h + 44], fill=_scale_alpha(accent, 0.6))
    ...
    for x_region in ((28, 174), (586, 736)):
        for _ in range(10):
            ...
            for px, py in pts[1:-1:2]:
                draw.ellipse([px - 9, py - 4, px + 9, py + 4], fill=_scale_alpha(accent, 0.55))
    for x, y, r in ((150, 212, 8), (182, 246, 5), (622, 182, 6), (652, 222, 9)):
        draw.ellipse([x - r, y - r, x + r, y + r], outline=_scale_alpha(glow, 0.65), width=2)
...
def _add_stalactites(draw: ImageDraw.ImageDraw, color: RGBA) -> None:
    for x, w, h in ((66, 26, 120), (142, 18, 84), (604, 24, 118), (690, 18, 74), (386, 36, 142)):
        _poly(draw, [(x - w, 0), (x + w, 0), (x, h)], color)



def _add_forest_canopy(draw: ImageDraw.ImageDraw, color: RGBA, accent: RGBA, glow: RGBA, alpha_scale: float = 1.0) -> None:
    # Dense overhead foliage with readable negative space in the middle.
    _periodic_band(draw, _scale_alpha(color, 1.05 * alpha_scale), -8, 124, 26, 18, 0.4, 1.8)
    _periodic_band(draw, _scale_alpha(accent, 0.58 * alpha_scale), 68, 54, 18, 12, 2.2, 0.7)
    for cx, cy, rx, ry in (
        (80, 72, 96, 52),
        (196, 48, 88, 44),
        (566, 54, 92, 46),
        ...
        draw.ellipse([cx - rx, cy - ry, cx + rx, cy + ry], fill=_scale_alpha(color, 0.74 * alpha_scale))
    for x in (124, 226, 550, 662):
        _poly(draw, [(x - 12, 0), (x + 18, 0), (x + 74, 260), (x + 22, 260)], _scale_alpha(glow, 0.10 * alpha_scale))
...
def _add_tree_trunk(
    ...
    *,
    ...
    pts = [(left_top, top), (right_top, top), (right_bottom, bottom), (left_bottom, bottom)]
    ...
    for _ in range(max(4, width // 8)):
        ...
        _line(draw, [(xx, y0), (xx + rng.randint(-4, 5), y1)], _scale_alpha(highlight, 0.45 * alpha_scale), max(1, width // 12))
    if bottom - top > 140:
        knot_x = x + rng.randint(-width // 5, width // 5)
        knot_y = rng.randint(top + 70, bottom - 70)
        draw.ellipse([knot_x - 6, knot_y - 10, knot_x + 6, knot_y + 10], fill=_scale_alpha(shadow, 0.9 * alpha_scale))
    # Root flare.
    for s in (-1, 1):
        _line(
            draw,
            [(x + lean * 0.8, bottom - 10), (x + lean * 0.8 + s * (width // 2 + 16), bottom + 8)],
            _scale_alpha(shadow, 0.72 * alpha_scale),
            max(2, width // 7),
        )


def _add_leaf_cluster(draw: ImageDraw.ImageDraw, cx: int, cy: int, leaf_dark: RGBA, leaf_mid: RGBA, scale: float = 1.0) -> None:
    for dx, dy, rx, ry, fill in (
        ...
        draw.ellipse([cx + dx - rx * scale, cy + dy - ry * scale, cx + dx + rx * scale, cy + dy + ry * scale], fill=fill)


def _add_bamboo_grove(draw: ImageDraw.ImageDraw, color: RGBA, accent: RGBA, alpha_scale: float = 1.0, foreground: bool = False) -> None:
    ...
    for lo, hi in regions:
        for _ in range(count):
            ...
            _line(draw, [(x, y0), (x + lean * 0.35, (y0 + y1) * 0.5), (x + lean, y1)], stalk, w)
            for y in range(int(y1) + 42, int(y0), rng.randint(46, 66)):
                _line(draw, [(x + lean * 0.18 - w, y), (x + lean * 0.18 + w, y)], _scale_alpha(accent, 0.5 * alpha_scale), max(1, w // 3))
            if rng.random() < 0.45:
                leaf_y = rng.randint(y1 + 40, min(460, int(y0) - 80))
                leaf_x = x + int(lean * 0.28)
                for side in (-1, 1):
                    _line(draw, [(leaf_x, leaf_y), (leaf_x + side * rng.randint(22, 52), leaf_y + rng.randint(-18, 18))], _scale_alpha(accent, 0.42 * alpha_scale), max(1, w // 3))
...
def _add_forest_midstory(draw: ImageDraw.ImageDraw, alpha_scale: float = 1.0) -> None:
    ...
    for args in (
        ...
        _add_tree_trunk(draw, args[0], args[1], args[2], args[3], trunk, shadow, highlight, lean=args[4], alpha_scale=1.0)
    for cx, cy in ((116, 162), (224, 136), (260, 188), (554, 150), (650, 170), (688, 126)):
        _add_leaf_cluster(draw, cx, cy, leaf_dark, leaf_mid, 1.0)
    # Sparse bamboo only on the edges so the scene still reads as a forest.
    _add_bamboo_grove(draw, (26, 72, 40, _clamp(128 * alpha_scale)), (72, 126, 72, _clamp(112 * alpha_scale)), alpha_scale, foreground=False)


def _add_torii_and_dojo(draw: ImageDraw.ImageDraw, color: RGBA, accent: RGBA, glow: RGBA, alpha_scale: float = 1.0) -> None:
    ...
    draw.polygon([(306, 768), (462, 768), (430, 618), (392, 524), (360, 618)], fill=(86, 74, 52, _clamp(94 * alpha_scale)))
    draw.line([(306, 768), (392, 524), (462, 768)], fill=(124, 112, 86, _clamp(70 * alpha_scale)), width=3)
    # Torii gate.
    ...
    draw.rounded_rectangle([214, 280, 566, 304], radius=8, fill=(142, 78, 44, _clamp(196 * alpha_scale)))
    ...
    draw.polygon([(286, 454), (392, 384), (498, 454), (468, 474), (316, 474)], fill=roof)
    draw.line([(286, 454), (392, 384), (498, 454)], fill=(82, 100, 58, _clamp(148 * alpha_scale)), width=5)
    ...
    for x in (160, 624):
        draw.rectangle([x - 10, 484, x + 10, 554], fill=(88, 92, 80, _clamp(140 * alpha_scale)))
        draw.polygon([(x - 26, 484), (x + 26, 484), (x + 14, 460), (x - 14, 460)], fill=(108, 112, 96, _clamp(148 * alpha_scale)))
        draw.rectangle([x - 12, 462, x + 12, 478], fill=_scale_alpha(glow, 0.38 * alpha_scale))
    for sx, sy in ((350, 650), (372, 618), (394, 586), (408, 556)):
        draw.ellipse([sx - 14, sy - 6, sx + 14, sy + 6], fill=(104, 96, 80, _clamp(90 * alpha_scale)))


def _add_forest_floor(draw: ImageDraw.ImageDraw, color: RGBA, accent: RGBA, alpha_scale: float = 1.0) -> None:
    _periodic_band(draw, _scale_alpha(color, 0.88 * alpha_scale), 560, 110, 18, 14, 0.5, 1.6)
    _periodic_band(draw, _scale_alpha(accent, 0.42 * alpha_scale), 534, 42, 14, 10, 1.8, 0.2)
    for x in (92, 176, 592, 676):
        _line(draw, [(x, 588), (x + 40, 548), (x + 108, 562)], _scale_alpha(accent, 0.28 * alpha_scale), 3)


def _add_forest_dojo(draw: ImageDraw.ImageDraw, theme: Theme, layer_key: str, alpha_scale: float) -> None:
    _add_forest_canopy(draw, (16, 40, 22, _clamp(120 * alpha_scale)), (44, 82, 44, _clamp(108 * alpha_scale)), (208, 226, 176, _clamp(78 * alpha_scale)), alpha_scale)
    if layer_key == "far_backplate":
        _add_forest_floor(draw, (18, 36, 22, _clamp(120 * alpha_scale)), (52, 86, 50, _clamp(98 * alpha_scale)), alpha_scale)
        ...
        for x, w, lean in ((98, 24, -8), (174, 30, 6), (598, 26, -4), (676, 22, 4)):
            _add_tree_trunk(draw, x, 180, 622, w, distant_trunk, distant_shadow, distant_highlight, lean=lean, alpha_scale=1.0, seed_offset=5)
        for cx, cy in ((104, 188), (164, 162), (596, 174), (668, 152)):
            _add_leaf_cluster(draw, cx, cy, (24, 56, 30, _clamp(84 * alpha_scale)), (54, 88, 48, _clamp(74 * alpha_scale)), 0.92)
    elif layer_key == "near_background":
        _add_forest_floor(draw, (26, 52, 28, _clamp(154 * alpha_scale)), (70, 108, 62, _clamp(118 * alpha_scale)), alpha_scale)
        _add_forest_midstory(draw, alpha_scale)
        _add_torii_and_dojo(draw, (40, 60, 38, _clamp(174 * alpha_scale)), (90, 126, 78, _clamp(128 * alpha_scale)), (220, 236, 196, _clamp(88 * alpha_scale)), alpha_scale)
    else:
        ...
        _add_tree_trunk(draw, 42, -10, 790, 82, trunk, shadow, highlight, lean=18, alpha_scale=1.0, seed_offset=20)
        _add_tree_trunk(draw, 148, 40, 790, 62, trunk, shadow, highlight, lean=-10, alpha_scale=1.0, seed_offset=21)
        _add_tree_trunk(draw, 638, 10, 790, 76, trunk, shadow, highlight, lean=-18, alpha_scale=1.0, seed_offset=22)
        _add_tree_trunk(draw, 732, -20, 790, 64, trunk, shadow, highlight, lean=10, alpha_scale=1.0, seed_offset=23)
        leaf_dark = (18, 48, 26, _clamp(146 * alpha_scale))
        leaf_mid = (52, 92, 50, _clamp(114 * alpha_scale))
        for cx, cy, sc in ((58, 116, 1.3), (134, 172, 1.1), (684, 114, 1.35), (624, 182, 1.1)):
            _add_leaf_cluster(draw, cx, cy, leaf_dark, leaf_mid, sc)
        _line(draw, [(0, 128), (180, 170), (344, 132)], _scale_alpha(shadow, 0.75), 10)
        _line(draw, [(768, 118), (604, 162), (446, 140)], _scale_alpha(shadow, 0.75), 10)
        _add_bamboo_grove(draw, (32, 82, 42, _clamp(132 * alpha_scale)), (78, 132, 74, _clamp(114 * alpha_scale)), alpha_scale, foreground=True)
        for x, y in ((214, 216), (548, 182), (646, 330), (146, 382), (332, 252)):
            draw.ellipse([x - 3, y - 3, x + 3, y + 3], fill=(226, 238, 172, _clamp(126 * alpha_scale)))

def _add_theme_landmark(draw: ImageDraw.ImageDraw, theme: Theme, layer_key: str, alpha_scale: float) -> None:
    ...
    if theme.key == "hub":
        ...
        if layer_key != "foreground_atmosphere":
            _add_ring_station(draw, _scale_alpha(base, 1.05), _scale_alpha(accent, 1.05))
        ...
        _add_reeds(draw, _scale_alpha(base, 0.9), _scale_alpha(accent, 0.9), alpha_scale)
    elif theme.key == "skybridge":
        _add_cloud_decks(draw, base, glow)
        if layer_key != "foreground_atmosphere":
            _add_bridge_pylons(draw, _scale_alpha(base, 1.05), _scale_alpha(accent, 0.85))
    elif theme.key == "boss":
...
def _draw_far_backplate(theme: Theme) -> Image.Image:
    ...
    for _ in range(12):
        ...
        r = rng.randint(18, 56)
        draw.ellipse([x - r, y - r, x + r, y + r], fill=_rgba(theme.glow, rng.randint(4, 10)))
...
def _draw_foreground_atmosphere(theme: Theme) -> Image.Image:
    ...
    for _ in range(8):
        ...
        _line(draw, [(0, y), (SIZE * 0.35, y + rng.randint(-34, 34)), (SIZE, y + rng.randint(-22, 22))], _rgba(theme.glow, rng.randint(10, 22)), rng.randint(5, 9))
    for _ in range(18):
        x = rng.randint(0, SIZE)
        y = rng.randint(24, SIZE - 24)
        r = rng.randint(3, 10)
        draw.ellipse([x - r, y - r, x + r, y + r], fill=_rgba(theme.glow, rng.randint(6, 14)))
    if theme.key == "water":
        for _ in range(12):
            x = rng.randint(80, SIZE - 80)
            y = rng.randint(80, SIZE - 160)
            r = rng.randint(4, 10)
            draw.ellipse([x - r, y - r, x + r, y + r], outline=_rgba(theme.glow, 26), width=2)
