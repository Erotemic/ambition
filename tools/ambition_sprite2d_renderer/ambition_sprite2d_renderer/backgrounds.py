...
def soft_layer(size: Tuple[int, int], blur: float = 3.0) -> Tuple[Image.Image, ImageDraw.ImageDraw]:
...
def draw_mist_band(d: ImageDraw.ImageDraw, y: float, color: Color, s: float, amp: float = 10.0) -> None:
    pts: List[Point] = []
    for i in range(0, 17):
        x = i * 32.0
        wave = ((i % 4) - 1.5) * amp
...
def hub_foreground(d: ImageDraw.ImageDraw, s: float) -> None:
    ...
    blue = rgba("#1B2945", 56)
    d.polygon(poly_scaled([(0, 0), (512, 0), (512, 34), (380, 25), (250, 42), (80, 28), (0, 48)], s), fill=shadow)
    for x in (42, 438):
        d.line(poly_scaled([(x, 0), (x + 22, 126)], s), fill=brace, width=max(2, int(7 * s)))
        d.line(poly_scaled([(x + 35, 0), (x - 14, 142)], s), fill=rgba("#0E0B22", 70), width=max(2, int(4 * s)))
    d.line(poly_scaled([(0, 430), (112, 404), (188, 424)], s), fill=blue, width=max(2, int(9 * s)))
    d.line(poly_scaled([(342, 424), (430, 404), (512, 418)], s), fill=blue, width=max(2, int(9 * s)))
    # A small broken sign silhouette near a corner; not centered enough to read as interactive.
    d.rounded_rectangle((398*s, 72*s, 500*s, 104*s), radius=4*s, fill=rgba("#111428", 58))
    d.line(poly_scaled([(386, 38), (412, 82)], s), fill=rgba("#101226", 64), width=max(1, int(3 * s)))
...
def lab_foreground(d: ImageDraw.ImageDraw, s: float) -> None:
    ...
    d.line(poly_scaled([(0, 486), (512, 474)], s), fill=rgba("#1DD8D2", 25), width=max(1, int(8*s)))
...
def basement_foreground(d: ImageDraw.ImageDraw, s: float) -> None:
    ...
    d.polygon(poly_scaled([(0, 0), (76, 0), (60, 92), (88, 162), (70, 250), (92, 340), (64, 512), (0, 512)], s), fill=stone)
    d.polygon(poly_scaled([(432, 132), (512, 110), (512, 512), (460, 512), (446, 382), (470, 300)], s), fill=rgba("#120D15", 88))
    # Cracked arch silhouette near the top.
    d.arc((70*s, -98*s, 442*s, 238*s), start=188, end=350, fill=stone2, width=max(3, int(14*s)))
    # Bottom foreground slabs.
    d.polygon(poly_scaled([(0, 472), (118, 452), (204, 482), (512, 458), (512, 512), (0, 512)], s), fill=rgba("#161016", 92))
    for p in [((44, 370), (58, 382)), ((470, 248), (484, 258)), ((130, 456), (146, 461))]:
...
def cove_foreground(d: ImageDraw.ImageDraw, s: float) -> None:
    ...
    for base_x in list(range(8, 118, 17)) + list(range(404, 508, 18)):
        ...
        d.line(poly_scaled([(base_x, 512), (base_x + lean, 512 - h)], s), fill=reed, width=max(1, int(3*s)))
        d.polygon(poly_scaled([(base_x + lean, 512 - h), (base_x + lean + 8, 512 - h + 18), (base_x + lean - 3, 512 - h + 13)], s), fill=reed)
    # Top palm fronds entering from edges.
    d.polygon(poly_scaled([(0, 0), (126, 0), (60, 26), (158, 44), (42, 46), (112, 76), (0, 60)], s), fill=palm)
    d.polygon(poly_scaled([(512, 0), (392, 0), (458, 30), (358, 46), (470, 50), (404, 78), (512, 62)], s), fill=palm)
...
def skybridge_foreground(d: ImageDraw.ImageDraw, s: float) -> None:
    ...
    for y in (82, 164, 360, 430):
        d.line(poly_scaled([(30, y), (220, y - 24), (404, y - 12)], s), fill=streak, width=max(1, int(3*s)))
    d.line(poly_scaled([(0, 170), (512, 88)], s), fill=cable, width=max(1, int(2*s)))
    d.line(poly_scaled([(0, 316), (512, 250)], s), fill=rgba("#23365C", 38), width=max(1, int(2*s)))
...
def boss_foreground(d: ImageDraw.ImageDraw, s: float) -> None:
    ...
    for x in range(0, 512, 42):
        h = 18 + (x % 5) * 8
        d.polygon(poly_scaled([(x, 512), (x + 20, 512 - h), (x + 40, 512)], s), fill=dark)
    for x in range(22, 512, 62):
        h = 15 + (x % 4) * 7
        d.polygon(poly_scaled([(x, 0), (x + 16, h), (x + 34, 0)], s), fill=rgba("#090006", 70))
    for pts in [[(450, 82), (500, 112), (476, 160)], [(14, 382), (72, 356), (52, 426)], [(414, 436), (492, 420), (458, 486)]]:
...
def water_foreground(d: ImageDraw.ImageDraw, s: float) -> None:
    ...
    for base_x in (18, 42, 72, 454, 484, 506):
        ...
        pts = [(base_x, 512), (base_x + lean, 420), (base_x - lean * 0.2, 330), (base_x + lean * 0.5, 250)]
        d.line(poly_scaled(pts, s), fill=kelp, width=max(2, int(7*s)), joint="curve")
        d.ellipse(bbox((base_x + lean)*s, 408*s, 16*s, 38*s), fill=rgba("#08302D", 56))
    draw_mist_band(d, 108, caustic, s, amp=8.0)
    draw_mist_band(d, 210, rgba("#B7FFF2", 24), s, amp=6.0)
    d.rectangle((0, 0, 512*s, 512*s), fill=haze)
    for x, y, r in [(110, 430, 5), (126, 392, 3), (388, 356, 4), (404, 314, 3), (456, 214, 5)]:
        d.ellipse(bbox(x*s, y*s, r*s, r*s), outline=bubble, width=max(1, int(1*s)))
...
def cave_foreground(d: ImageDraw.ImageDraw, s: float) -> None:
    ...
    d.polygon(poly_scaled([(0, 0), (512, 0), (512, 28), (432, 24), (402, 92), (372, 24), (304, 32), (284, 128), (250, 36), (156, 26), (134, 102), (108, 26), (0, 40)], s), fill=rock)
    # Side contours and corner darkness.
    d.polygon(poly_scaled([(0, 50), (38, 64), (26, 192), (52, 318), (28, 512), (0, 512)], s), fill=rock2)
    d.polygon(poly_scaled([(512, 42), (472, 72), (488, 210), (456, 346), (484, 512), (512, 512)], s), fill=rock2)
    ...
    for x, h in [(118, 38), (286, 50), (405, 34)]:
        d.line(poly_scaled([(x, 72), (x - 3, 72 + h)], s), fill=drip, width=max(1, int(2*s)))
...
BACKGROUND_LAYER_SPECS: List[BackgroundLayerSpec] = [
    BackgroundLayerSpec("hub_foreground", "hub_foreground.png", "hub", "foreground", 1.08, "dark violet structural edge silhouettes"),
    BackgroundLayerSpec("lab_foreground", "lab_foreground.png", "lab", "foreground", 1.10, "cables, pipes, dim glass and teal instrument lines"),
    BackgroundLayerSpec("basement_foreground", "basement_foreground.png", "basement", "foreground", 1.12, "broken pillars, cracked arch silhouettes, dusty slabs"),
    BackgroundLayerSpec("cove_foreground", "cove_foreground.png", "cove", "foreground", 1.06, "reeds, rigging, palm edges and low sea mist"),
    BackgroundLayerSpec("skybridge_foreground", "skybridge_foreground.png", "skybridge", "foreground", 1.04, "wispy bright haze, wind streaks and thin bridge cables"),
    BackgroundLayerSpec("boss_foreground", "boss_foreground.png", "boss", "foreground", 1.05, "very sparse spikes, red shards and embers at extreme edges"),
    BackgroundLayerSpec("water_foreground", "water_foreground.png", "water", "foreground", 1.08, "side kelp, bubble trails, shimmer bands and blue-green haze"),
    BackgroundLayerSpec("cave_foreground", "cave_foreground.png", "cave", "foreground", 1.10, "stalactites, rock contours, drips and corner darkness"),
]


def render_background_layer(spec: BackgroundLayerSpec, supersample: int = 2) -> Image.Image:
    try:
...
def build_background_contact_sheet(tiles: List[Tuple[BackgroundLayerSpec, Image.Image]]) -> Image.Image:
    cols = 4
    label_h = 28
    cell = 154
