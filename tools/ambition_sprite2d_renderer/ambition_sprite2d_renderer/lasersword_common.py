...
def _project_anchor(local: Point, angle_deg: float, with_barrels: bool, frame_size: Tuple[int, int], offset_px: Point) -> Point:
...
def frame_anchors(*, angle_deg: float, with_barrels: bool, frame_size: Tuple[int, int] = (FRAME_PX, FRAME_PX), offset_px: Point = (0.0, 0.0)) -> WeaponAnchors:
    ...
    return WeaponAnchors(grip=grip, pommel=pommel, guard=guard, muzzle=muzzle, tip=tip, forward=forward, angle_deg=angle_deg)
...
def draw_blade_layer(
    ...
    *,
    ...
    mid_layer = mid_layer.filter(ImageFilter.GaussianBlur(radius=max(2, int(SUPER * 0.4))))
    ...
    inner_layer = inner_layer.filter(ImageFilter.GaussianBlur(radius=max(2, int(SUPER * 0.6))))
    ...
    spine_layer = spine_layer.filter(ImageFilter.GaussianBlur(radius=max(1, int(SUPER * 0.3))))
    ...
    if slash_streak > 0.01:
        ...
        streak = streak.filter(ImageFilter.GaussianBlur(radius=max(1, int(SUPER * 0.45))))
    ...
    if pulse_position is not None and pulse_intensity > 0.01:
        ...
        if blade_y0 - 1.0 <= dot_y <= blade_y1 + 1.0:
            ...
            for r_mult, a_mult in ((1.6, 0.4), (1.0, 0.85), (0.55, 1.0)):
                pr = head_r * r_mult
                pd.ellipse(
                    (cx - s(pr), cy + s(dot_y) - s(pr),
                     cx + s(pr), cy + s(dot_y) + s(pr)),
                    fill=with_alpha(BLADE_CORE, int(245 * intensity * a_mult)),
                )
        pulse_layer = pulse_layer.filter(ImageFilter.GaussianBlur(radius=max(1, int(SUPER * 0.4))))
    ...
    if tip_flare > 0.001:
        ...
        tip_layer = tip_layer.filter(ImageFilter.GaussianBlur(radius=max(1, int(SUPER * 0.4))))
...
def draw_hilt_layer(*, crystal_pulse: float = 1.0, with_receiver: bool = True) -> Image.Image:
    ...
    d.ellipse(
        (cx - s(pe_r * 0.4), cy + s(pe_cy) - s(pe_r * 0.4), cx + s(pe_r * 0.4), cy + s(pe_cy) + s(pe_r * 0.4)),
        fill=EMITTER_CORE,
    )
    ...
    for i in range(5):
        ...
        d.line(
            [(cx + s(-bw * 0.45), cy + s(y + 0.65)), (cx + s(bw * 0.45), cy + s(y + 0.65))],
            fill=GUN_DARK,
    ...
    for rx in (-gh * 0.78, -gh * 0.42, gh * 0.42, gh * 0.78):
        d.ellipse(
            (cx + s(rx) - s(1.05), cy + s(GUARD_Y0 + 1.6) - s(1.05),
             cx + s(rx) + s(1.05), cy + s(GUARD_Y0 + 1.6) + s(1.05)),
            fill=BRASS,
            outline=GUN_DARK,
        )
        d.ellipse(
            (cx + s(rx) - s(0.45), cy + s(GUARD_Y0 + 1.6) - s(0.45),
             cx + s(rx) + s(0.45), cy + s(GUARD_Y0 + 1.6) + s(0.45)),
            fill=BRASS_HI,
        )
    ...
    if with_receiver:
        ...
        d.line(
            [(cx + s(-RECEIVER_HALFW * 0.85), cy + s(RECEIVER_Y0 + RECEIVER_THICK * 0.65)),
             (cx + s(RECEIVER_HALFW * 0.85), cy + s(RECEIVER_Y0 + RECEIVER_THICK * 0.65))],
            fill=BRASS,
            width=max(1, int(SUPER * 0.65)),
        )
        ...
        for cyx, cyy in (
            ...
            d.ellipse(
                (cx + s(cyx) - s(0.7), cy + s(cyy) - s(0.7),
                 cx + s(cyx) + s(0.7), cy + s(cyy) + s(0.7)),
                fill=EMITTER_HOT,
                outline=GUN_DARK,
            )
        # Vent slits on the bottom row.
        for vx in (-RECEIVER_HALFW * 0.62, -RECEIVER_HALFW * 0.18,
                   RECEIVER_HALFW * 0.18, RECEIVER_HALFW * 0.62):
            d.line(
    ...
    for off, alpha in ((1.6, int(180 * crystal_pulse)), (1.0, int(220 * crystal_pulse)), (0.4, int(255 * min(1.0, crystal_pulse)))):
        ed.ellipse(
            (cx + s(-em_half - off), cy + s(em_y - 0.8 * off / 1.6) - s(0.6),
             cx + s(em_half + off), cy + s(em_y - 0.8 * off / 1.6) + s(1.4)),
            fill=with_alpha(EMITTER_HOT, alpha),
        )
    emitter_layer = emitter_layer.filter(ImageFilter.GaussianBlur(radius=max(2, int(SUPER * 0.45))))
...
def draw_gun_cluster_layer(*, charge: float = 0.0, flash: float = 0.0) -> Image.Image:
    ...
    d.ellipse(
        (cx + s(RECEIVER_HALFW * 0.78) - s(1.1), cy + s(RECEIVER_Y0 + 2.2) - s(1.1),
         cx + s(RECEIVER_HALFW * 0.78) + s(1.1), cy + s(RECEIVER_Y0 + 2.2) + s(1.1)),
        fill=BRASS,
        outline=GUN_DARK,
    )
    ...
    barrel_positions = [
        (CLUSTER_OFFSET_X - CLUSTER_BARREL_GAP * 0.35, RECEIVER_Y1 + CLUSTER_OFFSET_Y - 0.6),  # top
        (CLUSTER_OFFSET_X - CLUSTER_BARREL_GAP * 0.05, RECEIVER_Y1 + CLUSTER_OFFSET_Y + CLUSTER_BARREL_GAP * 0.70),  # middle
        (CLUSTER_OFFSET_X + CLUSTER_BARREL_GAP * 0.55, RECEIVER_Y1 + CLUSTER_OFFSET_Y - 0.1),  # outer-top
    ]
    ...
    for (bx, by) in barrel_positions:
        ...
        if charge > 0.01:
            ...
            for r, a in ((4.0, 60), (2.4, 130), (1.3, 215)):
                gd_.ellipse(
                    (cx + s(bx) - s(r), cy + s(b_y1 - 0.3) - s(r),
                     cx + s(bx) + s(r), cy + s(b_y1 - 0.3) + s(r)),
                    fill=with_alpha(EMITTER_HOT, int(a * charge)),
                )
            glow_layer = glow_layer.filter(ImageFilter.GaussianBlur(radius=max(2, int(SUPER * 0.45))))
        ...
        if flash > 0.01:
            ...
            fd.ellipse(
                (fcx - s(2.0 * flash), fcy - s(2.0 * flash),
                 fcx + s(2.0 * flash), fcy + s(2.0 * flash)),
                fill=with_alpha(BLADE_CORE, int(245 * flash)),
            )
            flash_layer = flash_layer.filter(ImageFilter.GaussianBlur(radius=max(2, int(SUPER * 0.5))))
    ...
    drum_r = 3.8
    d.ellipse(
        (cx + s(drum_cx - drum_r), cy + s(drum_cy - drum_r * 0.55),
         cx + s(drum_cx + drum_r), cy + s(drum_cy + drum_r * 0.55)),
        fill=GUN_PLATE,
        outline=GUN_DARK,
    )
    d.ellipse(
        (cx + s(drum_cx - drum_r * 0.45), cy + s(drum_cy - drum_r * 0.25),
         cx + s(drum_cx + drum_r * 0.45), cy + s(drum_cy + drum_r * 0.25)),
        fill=BRASS,
        outline=GUN_DARK,
    )
    ...
    for offset_x in (-STINGER_OFFSET_X, STINGER_OFFSET_X):
        ...
        d.rectangle(
            (cx + s(offset_x - 1.4), cy + s(st_y0 - 1.0),
             cx + s(offset_x + 1.4), cy + s(st_y0 + 1.0)),
            fill=GUN_PLATE,
            outline=GUN_DARK,
        )
        ...
        d.ellipse(
            (cx + s(offset_x - STINGER_TIP_BULB), cy + s(st_y1 - STINGER_TIP_BULB),
             cx + s(offset_x + STINGER_TIP_BULB), cy + s(st_y1 + STINGER_TIP_BULB)),
            fill=BRASS,
            outline=GUN_DARK,
        )
        d.ellipse(
            (cx + s(offset_x - STINGER_TIP_BULB * 0.45), cy + s(st_y1 - STINGER_TIP_BULB * 0.45),
             cx + s(offset_x + STINGER_TIP_BULB * 0.45), cy + s(st_y1 + STINGER_TIP_BULB * 0.45)),
            fill=BRASS_HI,
        )
...
def draw_weapon(
    *,
    ...
    rotated = composed.rotate(angle_deg, expand=False, resample=Image.Resampling.BICUBIC)
