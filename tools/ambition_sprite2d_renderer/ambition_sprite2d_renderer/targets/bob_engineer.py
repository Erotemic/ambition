...
class BobEngineerGenerator:
    ...
    def pose_for_animation(self, animation: str, frame_index: int, frame_count: int) -> BobPose:
    ...
    def _render_three_quarter(self, base: Image.Image, cx: float, feet_y: float, spec: BobSpec, pal: Dict[str, Color], S: float, pose: BobPose) -> None:
    ...
    def _tq_draw_legs(self, base: Image.Image, cx: float, pants_top_y: float, boot_top_y: float, feet_y: float, spec: BobSpec, pal: Dict[str, Color], S: float, pose: BobPose) -> None:
        ...
        for sign, dx in zip((-1, 1), (-1, 1)):
            ...
            d.line([(hip[0], hip[1] + 1.0 * S), (ankle[0], ankle[1] - 1.0 * S)], fill=pal["pants_shadow"], width=max(1, int(0.7 * S)))
            # Boot — a chunky rounded rectangle with a darker sole.
            boot_c = (ankle[0], (ankle[1] + feet_y) * 0.5)
            d.rounded_rectangle(
                (boot_c[0] - spec.boot_w * 0.55 * S, ankle[1] - 0.5 * S, boot_c[0] + spec.boot_w * 0.55 * S, feet_y),
                radius=2.0 * S, fill=pal["boot"], outline=outline, width=max(1, int(1.0 * S)),
            )
            # Boot sole as a thinner band at the very bottom.
            d.rectangle(
                (boot_c[0] - spec.boot_w * 0.55 * S, feet_y - 1.6 * S, boot_c[0] + spec.boot_w * 0.55 * S, feet_y),
                fill=pal["boot_sole"], outline=outline, width=max(1, int(0.7 * S)),
            )
            # Laces — two short horizontal ticks across the boot face.
            for ly in (3.0, 1.2):
                d.line([(boot_c[0] - 2.4 * S, feet_y - ly * S), (boot_c[0] + 2.4 * S, feet_y - ly * S)], fill=pal["leather"], width=max(1, int(0.6 * S)))

    def _tq_draw_arms_back(self, base: Image.Image, cx: float, shoulder_y: float, spec: BobSpec, pal: Dict[str, Color], S: float, pose: BobPose) -> None:
        ...
        # Hand peek.
        d.ellipse(_bbox((ex, ey + 3.0 * S), 3.4 * S, 3.0 * S), fill=pal["skin"], outline=outline, width=max(1, int(0.8 * S)))

    def _tq_draw_vest(self, base: Image.Image, cx: float, vest_top_y: float, waist_y: float, spec: BobSpec, pal: Dict[str, Color], S: float, pose: BobPose) -> None:
        d = ImageDraw.Draw(base)
        outline = pal["outline"]
        ...
        for sign in (-1, 1):
            ...
            d.rectangle(
                (stripe_cx - 4.0 * S, stripe_y, stripe_cx + 4.0 * S, stripe_y + 2.4 * S),
                fill=pal["hi_vis"], outline=outline, width=max(1, int(0.6 * S)),
            )
        # Vest lapel highlight (camera-right side).
        d.line(
            [(cx + 3.0 * S, vest_top_y + 1.0 * S), (cx + spec.waist_w * 0.32 * S, waist_y - 1.0 * S)],
            fill=pal["vest_light"],
            width=max(1, int(0.9 * S)),
        )
        # Chest patch pocket on the camera-right panel.
        d.rounded_rectangle(
            (cx + 6.0 * S, vest_top_y + spec.vest_h * 0.18 * S, cx + 13.0 * S, vest_top_y + spec.vest_h * 0.42 * S),
            radius=1.2 * S, fill=pal["vest_dark"], outline=outline, width=max(1, int(0.6 * S)),
        )
        # Tee crew-neck band (small dark arc at the throat).
        d.arc(
            (cx - 6.0 * S, vest_top_y - 3.0 * S, cx + 8.0 * S, vest_top_y + 3.0 * S),
            start=10, end=170, fill=pal["tee_dark"], width=max(1, int(1.0 * S)),
        )

    def _tq_draw_belt(self, base: Image.Image, cx: float, waist_y: float, spec: BobSpec, pal: Dict[str, Color], S: float, pose: BobPose) -> None:
        d = ImageDraw.Draw(base)
        outline = pal["outline"]
        # Belt strap.
        d.rounded_rectangle(
            (cx - spec.waist_w * 0.52 * S, waist_y - 0.5 * S, cx + spec.waist_w * 0.52 * S, waist_y + 3.5 * S),
            radius=1.4 * S, fill=pal["leather"], outline=outline, width=max(1, int(0.9 * S)),
        )
        # Belt buckle (steel).
        d.rectangle(
            (cx - 2.6 * S, waist_y - 0.3 * S, cx + 2.6 * S, waist_y + 3.3 * S),
            fill=pal["steel"], outline=outline, width=max(1, int(0.7 * S)),
        )
        # Three hanging tools across the belt: key ring (camera-right),
        # wrench (center), small hammer (camera-left).
        self._draw_belt_keyring(d, cx + spec.waist_w * 0.30 * S, waist_y + 3.5 * S, S, pal)
        self._draw_belt_wrench(d, cx + 0.0 * S, waist_y + 3.5 * S, S, pal)
        self._draw_belt_hammer(d, cx - spec.waist_w * 0.30 * S, waist_y + 3.5 * S, S, pal)

    def _draw_belt_keyring(self, d: ImageDraw.ImageDraw, x: float, y: float, S: float, pal: Dict[str, Color]) -> None:
        outline = pal["outline"]
        # A small carabiner ring with two short keys dangling.
        ring_c = (x, y + 2.5 * S)
        d.ellipse(_bbox(ring_c, 2.4 * S, 2.4 * S), outline=outline, width=max(1, int(1.0 * S)))
        for dx in (-1.6, 1.6):
            kx = ring_c[0] + dx * S
            d.line([(kx, ring_c[1] + 2.4 * S), (kx, ring_c[1] + 8.0 * S)], fill=pal["brass"], width=max(1, int(1.1 * S)))
            d.line([(kx, ring_c[1] + 7.0 * S), (kx + 0.9 * S, ring_c[1] + 7.0 * S)], fill=pal["brass_dark"], width=max(1, int(0.7 * S)))

    def _draw_belt_wrench(self, d: ImageDraw.ImageDraw, x: float, y: float, S: float, pal: Dict[str, Color]) -> None:
        outline = pal["outline"]
        # Loop attaching wrench to belt.
        d.ellipse(_bbox((x, y + 1.5 * S), 1.6 * S, 1.6 * S), outline=outline, width=max(1, int(0.8 * S)))
        # Wrench body — long shaft + open jaw at the bottom.
        d.line([(x, y + 2.5 * S), (x - 0.4 * S, y + 9.0 * S)], fill=pal["steel"], width=max(1, int(1.8 * S)))
        d.line([(x, y + 2.5 * S), (x - 0.4 * S, y + 9.0 * S)], fill=pal["steel_dark"], width=max(1, int(0.9 * S)))
        ...
        d.polygon([
            ...
            (x - 0.4 * S - 1.2 * S, y + 11.5 * S),
        ], fill=pal["steel"], outline=outline)
        d.rectangle(
            (x - 0.4 * S - 0.6 * S, y + 10.5 * S, x - 0.4 * S + 0.6 * S, y + 11.5 * S),
            fill=pal["outline"],
        )

    def _draw_belt_hammer(self, d: ImageDraw.ImageDraw, x: float, y: float, S: float, pal: Dict[str, Color]) -> None:
        outline = pal["outline"]
        # Hammer hanging upside-down (head down, handle clipped to belt).
        d.ellipse(_bbox((x, y + 1.5 * S), 1.4 * S, 1.4 * S), outline=outline, width=max(1, int(0.8 * S)))
        # Handle.
        d.line([(x, y + 2.4 * S), (x + 0.4 * S, y + 8.0 * S)], fill=pal["leather"], width=max(1, int(1.4 * S)))
        d.line([(x, y + 2.4 * S), (x + 0.4 * S, y + 8.0 * S)], fill=pal["leather_dark"], width=max(1, int(0.7 * S)))
        # Head — steel rectangle perpendicular to the handle.
        head_c = (x + 0.4 * S, y + 8.5 * S)
        d.rectangle(
            (head_c[0] - 3.2 * S, head_c[1] - 0.5 * S, head_c[0] + 1.8 * S, head_c[1] + 2.5 * S),
            fill=pal["steel"], outline=outline, width=max(1, int(0.8 * S)),
        )
        ...
        ], fill=pal["steel_dark"], outline=outline)

    def _tq_draw_arms_front(self, base: Image.Image, cx: float, shoulder_y: float, spec: BobSpec, pal: Dict[str, Color], S: float, pose: BobPose) -> None:
        d = ImageDraw.Draw(base)
        outline = pal["outline"]
        ...
        d.ellipse(_bbox(hand_c, 3.8 * S, 3.4 * S), fill=pal["skin"], outline=outline, width=max(1, int(0.8 * S)))
        if pose.hold_keys:
            self._draw_held_keyring(d, hand_c, S, pal)

    def _draw_held_keyring(self, d: ImageDraw.ImageDraw, hand: Point, S: float, pal: Dict[str, Color]) -> None:
        outline = pal["outline"]
        # Larger key ring than the belt one — Bob's primary prop.
        ring_c = (hand[0] + 5.0 * S, hand[1] + 0.0 * S)
        d.ellipse(_bbox(ring_c, 3.4 * S, 3.4 * S), outline=outline, width=max(1, int(1.2 * S)))
        # Three keys hanging.
        for i, ddx in enumerate((-2.5, 0.0, 2.5)):
            ...
            for ty in (7.0, 9.5):
                d.line([(ring_c[0] + ddx * S, ring_c[1] + ty * S), (ring_c[0] + (ddx + 1.4) * S, ring_c[1] + ty * S)], fill=pal["brass_dark"], width=max(1, int(0.9 * S)))
    ...
    def _tq_draw_head(self, base: Image.Image, c: Point, spec: BobSpec, pal: Dict[str, Color], S: float, pose: BobPose) -> None:
        ...
        # Nose.
        d.line(
            [(c[0] + 3.5 * S, c[1] + 1.5 * S), (c[0] + 5.0 * S, c[1] + 3.5 * S), (c[0] + 3.5 * S, c[1] + 4.5 * S)],
    ...
    def _draw_eyes_three_quarter(self, d: ImageDraw.ImageDraw, c: Point, spec: BobSpec, pal: Dict[str, Color], S: float, pose: BobPose) -> None:
        ...
        if pose.blink:
            d.line([(near[0] - 2.0 * S, near[1]), (near[0] + 2.0 * S, near[1])], fill=outline, width=max(1, int(1.1 * S)))
            d.line([(far[0] - 1.4 * S, far[1]), (far[0] + 1.4 * S, far[1])], fill=outline, width=max(1, int(0.9 * S)))
            return
        d.ellipse(_bbox(near, 3.4 * S, 1.6 * S), fill=pal["white"], outline=outline, width=max(1, int(0.9 * S)))
        d.ellipse(_bbox((near[0] + 0.4 * S, near[1]), 1.2 * S, 1.8 * S), fill=outline)
        d.ellipse(_bbox(far, 2.6 * S, 1.4 * S), fill=pal["white"], outline=outline, width=max(1, int(0.8 * S)))
    ...
    def _render_side(self, base: Image.Image, cx: float, feet_y: float, spec: BobSpec, pal: Dict[str, Color], S: float, pose: BobPose) -> None:
        d = ImageDraw.Draw(base)
        outline = pal["outline"]
        ...
        for side, sgn in (("far", -step), ("near", +step)):
            ...
            # Boot.
            d.rounded_rectangle(
                (ankle_x - spec.boot_w * 0.55 * S, ankle_y - 0.5 * S, ankle_x + spec.boot_w * 0.95 * S, ankle_y + spec.boot_h * S),
                radius=2.0 * S, fill=pal["boot"], outline=outline, width=max(1, int(1.0 * S)),
            )
            d.rectangle(
                (ankle_x - spec.boot_w * 0.55 * S, ankle_y + spec.boot_h * S - 1.6 * S, ankle_x + spec.boot_w * 0.95 * S, ankle_y + spec.boot_h * S),
                fill=pal["boot_sole"], outline=outline, width=max(1, int(0.7 * S)),
            )
        ...
        ], fill=pal["tee"], outline=outline)
        # Hi-vis stripe across the side of the vest at chest level.
        stripe_y = shoulder_y + spec.vest_h * 0.50 * S
        d.rectangle(
            (cx - spec.shoulder_w * 0.10 * S, stripe_y, cx + spec.shoulder_w * 0.32 * S, stripe_y + 2.4 * S),
            fill=pal["hi_vis"], outline=outline, width=max(1, int(0.6 * S)),
        )
        # Belt.
        d.rounded_rectangle(
            (cx - spec.waist_w * 0.22 * S, waist_y - 0.5 * S, cx + spec.waist_w * 0.40 * S, waist_y + 3.5 * S),
            radius=1.2 * S, fill=pal["leather"], outline=outline, width=max(1, int(0.9 * S)),
        )
        d.rectangle(
            (cx + spec.waist_w * 0.06 * S, waist_y - 0.3 * S, cx + spec.waist_w * 0.14 * S, waist_y + 3.3 * S),
            fill=pal["steel"], outline=outline, width=max(1, int(0.7 * S)),
        )
        # One hanging tool (the wrench) — keep it simple in profile.
        self._draw_belt_wrench(d, cx + spec.waist_w * 0.20 * S, waist_y + 3.5 * S, S, pal)
        ...
        d.line([far_shoulder, far_hand], fill=pal["tee_dark"], width=max(1, int(spec.arm_w * S)))
        ...
        d.line([near_shoulder, near_hand], fill=pal["tee"], width=max(1, int(spec.arm_w * S)))
    ...
    def _render_front(self, base: Image.Image, cx: float, feet_y: float, spec: BobSpec, pal: Dict[str, Color], S: float, pose: BobPose) -> None:
        d = ImageDraw.Draw(base)
        outline = pal["outline"]
        boot_top_y = feet_y - spec.boot_h * S
        pants_top_y = boot_top_y - spec.leg_h * S
        ...
        for sgn in (-1, 1):
            ...
            d.polygon(leg, fill=pal["pants"], outline=outline)
            d.rounded_rectangle(
                (ankle[0] - spec.boot_w * 0.55 * S, ankle[1] - 0.5 * S, ankle[0] + spec.boot_w * 0.55 * S, feet_y),
                radius=2.0 * S, fill=pal["boot"], outline=outline, width=max(1, int(1.0 * S)),
            )
            d.rectangle(
                (ankle[0] - spec.boot_w * 0.55 * S, feet_y - 1.6 * S, ankle[0] + spec.boot_w * 0.55 * S, feet_y),
                fill=pal["boot_sole"], outline=outline, width=max(1, int(0.7 * S)),
            )
        ...
        for sgn in (-1, 1):
            ...
            stripe_y = vest_top_y + spec.vest_h * 0.40 * S
            d.rectangle(
                (cx + sgn * spec.shoulder_w * 0.30 * S - 4.0 * S, stripe_y, cx + sgn * spec.shoulder_w * 0.30 * S + 4.0 * S, stripe_y + 2.4 * S),
                fill=pal["hi_vis"], outline=outline, width=max(1, int(0.6 * S)),
            )
        # Tee crew-neck band.
        d.arc(
        ...
        self._draw_belt_keyring(d, cx + spec.waist_w * 0.32 * S, waist_y + 3.5 * S, S, pal)
        self._draw_belt_hammer(d, cx - spec.waist_w * 0.32 * S, waist_y + 3.5 * S, S, pal)
        ...
        for sgn in (-1, 1):
            ...
            d.polygon(sleeve, fill=pal["tee"], outline=outline)
            d.ellipse(_bbox((ex, ey + 3.0 * S), 3.6 * S, 3.0 * S), fill=pal["skin"], outline=outline, width=max(1, int(0.8 * S)))
    ...
    def _front_draw_head(self, base: Image.Image, c: Point, spec: BobSpec, pal: Dict[str, Color], S: float, pose: BobPose) -> None:
        ...
        for sgn in (-1, 1):
            ex = c[0] + sgn * spec.head_w * 0.18 * S
            if pose.blink:
                d.line([(ex - 2.0 * S, eye_y), (ex + 2.0 * S, eye_y)], fill=outline, width=max(1, int(1.1 * S)))
            else:
                d.ellipse(_bbox((ex, eye_y), 2.8 * S, 1.6 * S), fill=pal["white"], outline=outline, width=max(1, int(0.9 * S)))
        ...
        if pose.talk_open > 0.2:
            d.ellipse(_bbox((c[0], mouth_y), 4.0 * S, (1.0 + pose.talk_open * 1.6) * S), fill=outline)
        else:
            d.arc(
                (c[0] - 3.5 * S, mouth_y - 1.5 * S, c[0] + 3.5 * S, mouth_y + 2.5 * S),
                start=10, end=170, fill=outline, width=max(1, int(1.0 * S)),
            )
