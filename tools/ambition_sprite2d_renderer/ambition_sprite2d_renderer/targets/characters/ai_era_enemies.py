...
def _rounded_rect(draw: ImageDraw.ImageDraw, xy: Tuple[float, float, float, float], fill: RGBA, outline: RGBA = OUTLINE, width: float = 1.0, radius: float = 4.0) -> None:
    x1, y1, x2, y2 = xy
    draw.rounded_rectangle((_s(x1), _s(y1), _s(x2), _s(y2)), radius=_s(radius), fill=fill, outline=outline, width=max(1, _s(width)) if outline is not None else 0)
...
def _ribbon(draw: ImageDraw.ImageDraw, pts: Sequence[Point], width: float, fill: RGBA, outline: RGBA = OUTLINE, highlight: RGBA | None = None) -> None:
...
def _capsule(draw: ImageDraw.ImageDraw, a: Point, b: Point, width: float, fill: RGBA, outline: RGBA = OUTLINE, highlight: RGBA | None = None) -> None:
...
def _triangle(draw: ImageDraw.ImageDraw, center: Point, size: float, fill: RGBA, deg: float = 0.0, outline: RGBA = OUTLINE) -> None:
    pts: List[Point] = []
    for i in range(3):
...
def _star(draw: ImageDraw.ImageDraw, center: Point, r1: float, r2: float, points: int, fill: RGBA, outline: RGBA = OUTLINE) -> None:
    pts: List[Point] = []
...
def _emotion_eye(draw: ImageDraw.ImageDraw, x: float, y: float, rx: float, ry: float, iris: RGBA, pupil_shift: float = 0.0, blink: float = 0.0, outline: RGBA = OUTLINE) -> None:
    ry2 = max(0.6, ry * (1.0 - blink))
    _ellipse(draw, x, y, rx, ry2, WHITE, outline, 0.8)
    if blink < 0.92:
        _circle(draw, (x + pupil_shift, y + 0.2), min(rx, ry2) * 0.55, iris, outline, 0.7)
        _circle(draw, (x + pupil_shift * 1.18, y + 0.3), min(rx, ry2) * 0.24, BLACK, None, 0)
        _circle(draw, (x + pupil_shift * 0.8 - 0.7, y - 0.7), max(0.5, min(rx, ry2) * 0.12), (255, 255, 255, 180), None, 0)


def _draw_hand(draw: ImageDraw.ImageDraw, wrist: Point, ang: float, spread: float, scale: float, palm: RGBA, finger: RGBA | None = None, outline: RGBA = OUTLINE) -> Point:
    finger = finger or palm
    length1 = 9.0 * scale
    length2 = 7.0 * scale
    hand = (wrist[0] + math.cos(math.radians(ang)) * length1, wrist[1] + math.sin(math.radians(ang)) * length1)
    ...
    for i, off in enumerate([-34, -12, 10, 30]):
        ...
        tip = (hand[0] + math.cos(math.radians(a)) * length2, hand[1] + math.sin(math.radians(a)) * length2)
        _capsule(draw, hand, tip, 1.9 * scale, finger, outline, None)
    thumb_a = ang + 115
    thumb = (hand[0] + math.cos(math.radians(thumb_a)) * (length2 * 0.75), hand[1] + math.sin(math.radians(thumb_a)) * (length2 * 0.75))
    _capsule(draw, hand, thumb, 1.8 * scale, finger, outline, None)
    return hand


def _noise_squiggles(draw: ImageDraw.ImageDraw, pts: Sequence[Point], color: RGBA, width: float = 0.8) -> None:
...
def _draw_puppy_head(draw: ImageDraw.ImageDraw, cx: float, cy: float, scale: float, tilt: float, eye_pop: float, palette: Tuple[RGBA, RGBA, RGBA]) -> None:
    ...
    _poly(hd, [(cx - 6.5 * scale, cy - 3.0 * scale), (cx - 11.0 * scale, cy - 8.6 * scale), (cx - 4.5 * scale, cy - 7.0 * scale)], fur_hi, OUTLINE, 0.8)
    _poly(hd, [(cx + 6.3 * scale, cy - 3.2 * scale), (cx + 10.8 * scale, cy - 8.0 * scale), (cx + 4.3 * scale, cy - 7.3 * scale)], fur_hi, OUTLINE, 0.8)
    _ellipse(hd, cx + 5.4 * scale, cy + 1.5 * scale, 4.6 * scale, 3.7 * scale, muzzle, OUTLINE, 0.7)
    _emotion_eye(hd, cx - 1.2 * scale, cy - 0.9 * scale, 2.0 * scale, 2.4 * scale, CYAN, pupil_shift=eye_pop * 0.4)
    _emotion_eye(hd, cx + 2.4 * scale, cy - 0.7 * scale, 2.0 * scale, 2.4 * scale, MAGENTA, pupil_shift=eye_pop * 0.6)
    _circle(hd, (cx + 8.2 * scale, cy + 1.6 * scale), 0.95 * scale, BLACK, None, 0)
    _line(hd, [(cx + 7.8 * scale, cy + 4.0 * scale), (cx + 10.2 * scale, cy + 3.2 * scale)], BLACK, 0.8)
    for whisk in (-1, 1):
        _line(hd, [(cx + 9.5 * scale, cy + 2.2 * scale), (cx + 12.5 * scale, cy + 1.0 * whisk + 2.0 * scale)], fur_hi, 0.7)
...
def _render_puppy_slug_v2(anim: str, frame_idx: int, nframes: int) -> Image.Image:
    ...
    for i in range(10):
        ...
        y = 95 + math.sin(tt * math.pi * 1.5 + frame_idx * 0.7) * (3.0 + pose["stretch"] * 2.0) + pose["bob"] + tt * pose["dead"] * 9.0
    ...
    for i, (x, y) in enumerate(dorsal_pts[2:8]):
        _circle(draw, (x, y - 4.2 - math.sin(i + frame_idx) * 1.1), 2.0, ridge, OUTLINE, 0.6)
        _emotion_eye(draw, x - 3.2, y - 3.6, 1.5, 1.9, GREEN, pupil_shift=pose["eyes"] * 1.4)
        _emotion_eye(draw, x + 0.8, y - 2.8, 1.6, 2.0, MAGENTA, pupil_shift=pose["eyes"] * 1.2)
    head_y = 90 + pose["bob"] + pose["coil"] * -3.0 + pose["dead"] * 10.0
    head_x = 110 + pose["x"]
    _draw_puppy_head(draw, head_x, head_y, 1.15 + pose["coil"] * 0.08, pose["coil"] * 5.0 - pose["dead"] * 18.0, pose["eyes"] * 5.0, (body, body_hi, (238, 226, 182, 255)))
    mouth_y = head_y + 7.5
    _line(draw, [(head_x + 11.3, mouth_y), (head_x + 15.0, mouth_y + pose["mouth"] * 8.0)], BLACK, 1.1)
    for j in range(3):
        _circle(draw, (58 + j * 18, 104 + pose["bob"] + math.sin(frame_idx * 0.7 + j) * 2), 1.7, ridge, None, 0)
    if anim == "gaze":
        for ox in (-8, 3, 14):
            _star(draw, (head_x + ox, head_y - 8), 3.0 + pose["eyes"] * 4.0, 1.5, 6, (255, 244, 150, 110), None)
    if anim == "lunge":
        _line(draw, [(120, 95), (138 + pose["impact"] * 10, 90)], (255, 236, 180, 180), 2.2)
    if anim == "howl":
        for i in range(3):
            a = 0.2 + i * 0.55
            pts = _bezier((123, 86), (130 + i * 4, 78 - i * 6), (142 + i * 10, 74 - i * 3), (152 + i * 10, 80 - i * 2), 10)
...
def _pose_synth(anim: str, frame_idx: int, nframes: int) -> Dict[str, float]:
    ...
    cyc = math.tau * frame_idx / max(1, nframes)
    s = math.sin(cyc)
    pose = {"x": 0.0, "y": 0.0, "tilt": 0.0, "glitch": 0.0, "smile": 0.0, "reveal": 0.0, "hurt": 0.0, "dead": 0.0}
    if anim == "idle":
        pose["y"] = s * 1.2
        pose["tilt"] = s * 1.5
    elif anim == "drift":
...
def _render_synthetic_friend(anim: str, frame_idx: int, nframes: int) -> Image.Image:
    ...
    for ox, oy, tilt in offs:
        ...
        _rounded_rect(qd, (cx - 20 + ox, cy - 30 + oy, cx + 17 + ox, cy + 32 + oy), (228, 236, 246, 255), (190, 200, 216, 255), 1.0, 5.0)
        _ellipse(qd, cx + ox - 1, cy + oy - 6, 10, 12, (240, 212, 190, 255), OUTLINE, 0.7)
        _emotion_eye(qd, cx - 4 + ox, cy - 8 + oy, 2.4, 3.0, CYAN, 0.0, blink=0.0)
        _emotion_eye(qd, cx + 3 + ox, cy - 8 + oy, 2.4, 3.0, GREEN, 0.2, blink=0.0)
        _line(qd, [(cx - 5 + ox, cy + 2 + oy), (cx + 5 + ox, cy + 3 + oy)], (160, 104, 110, 255), 0.9)
    ...
    _rounded_rect(md, (cx - 22, cy - 33, cx + 22, cy + 37), (244, 246, 252, 255), (208, 214, 226, 255), 1.0, 6.0)
    ...
    _emotion_eye(md, cx - 4.0, cy - 12.0, 2.8, 3.5, CYAN, pupil_shift=pose["glitch"] * 1.6, blink=blink)
    _emotion_eye(md, cx + 4.2, cy - 11.4, 2.1, 4.1, GREEN, pupil_shift=-pose["glitch"] * 1.0, blink=0.0)
    _line(md, [(cx - 2.0, cy - 5.0), (cx + 2.0, cy - 3.0)], (190, 142, 126, 255), 0.8)
    mouth_curve = 0.0 + pose["smile"] * 4.2 - pose["hurt"] * 2.5
    _line(md, [(cx - 5.0, cy + 3.0), (cx, cy + 4.5 + mouth_curve * 0.4), (cx + 5.5, cy + 3.0 + mouth_curve)], (146, 92, 104, 255), 0.8)
    ...
    # Shoulder/bust drape.
    _poly(md, [(cx - 18, cy + 14), (cx - 8, cy + 5), (cx + 5, cy + 7), (cx + 18, cy + 18), (cx + 16, cy + 31), (cx - 14, cy + 30)], (164, 188, 224, 255), OUTLINE, 0.8)
    ...
    if pose["glitch"]:
        for ox, col in [(-6, CYAN), (7, MAGENTA)]:
            _rounded_rect(md, (cx - 18 + ox, cy + 20, cx + 8 + ox, cy + 25), (col[0], col[1], col[2], 120), None, 0, 1.4)
    ...
    for i, (ox, oy) in enumerate([(-22, -18), (25, -12), (-24, 10), (20, 16)]):
        _circle(draw, (cx + ox, cy + oy + math.sin(frame_idx * 0.7 + i) * 1.2), 1.5 + (i % 2) * 0.4, (255, 255, 255, 150), None, 0)
    return _downsample(img)
...
def _pose_saint(anim: str, frame_idx: int, nframes: int) -> Dict[str, float]:
    ...
    cyc = math.tau * frame_idx / max(1, nframes)
    s = math.sin(cyc)
    pose = {"bob": 0.0, "tilt": 0.0, "halo": 0.0, "grasp": 0.0, "bless": 0.0, "judge": 0.0, "hurt": 0.0, "dead": 0.0}
    if anim == "idle":
        pose["bob"] = s * 1.3
        pose["halo"] = s * 4.0
    elif anim == "hover":
...
def _render_hand_saint(anim: str, frame_idx: int, nframes: int) -> Image.Image:
    ...
    robe_pts = [
        (cx - 18, cy + 12), (cx - 28, cy + 40), (cx - 18, cy + 56), (cx, cy + 62),
        (cx + 18, cy + 56), (cx + 30, cy + 38), (cx + 18, cy + 12), (cx + 6, cy + 4), (cx - 6, cy + 4),
    ]
    _poly(rd, robe_pts, (118, 94, 154, 255), OUTLINE, 0.9)
    _poly(rd, [(cx - 12, cy + 12), (cx - 6, cy + 55), (cx + 4, cy + 58), (cx - 2, cy + 8)], (152, 132, 190, 180), None, 0)
    # Palm torso.
    palm_c = (cx, cy - 4)
    _circle(rd, palm_c, 12, (244, 226, 198, 255), OUTLINE, 0.8)
    for a in (-50, -22, 0, 22, 48):
        root = (palm_c[0] + math.cos(math.radians(a - 90)) * 6.0, palm_c[1] - 7.0 + math.sin(math.radians(a - 90)) * 3.0)
    ...
    _emotion_eye(rd, palm_c[0], palm_c[1] + 1.0, 3.4, 4.1, CYAN, blink=pose["dead"] * 0.95)
    _circle(rd, (palm_c[0], palm_c[1] + 8.5), 1.5 + pose["judge"] * 0.8, YELLOW, OUTLINE, 0.6)
    # Arms/hands.
    left_wrist = (cx - 14, cy + 18)
    right_wrist = (cx + 14, cy + 18)
    _capsule(rd, (cx - 8, cy + 10), left_wrist, 4.0, (208, 188, 166, 255), OUTLINE, None)
    _capsule(rd, (cx + 8, cy + 10), right_wrist, 4.0, (208, 188, 166, 255), OUTLINE, None)
    _draw_hand(rd, left_wrist, 145 - pose["grasp"] * 45 + pose["bless"] * 10, spread=8 + pose["grasp"] * 10, scale=0.9, palm=(244, 226, 198, 255))
    _draw_hand(rd, right_wrist, 35 + pose["grasp"] * 45 - pose["bless"] * 10, spread=8 + pose["grasp"] * 10, scale=0.9, palm=(244, 226, 198, 255))
    # Halo of smaller hands.
    for i in range(6):
        ang = pose["halo"] + i * 60.0
        hx = cx + math.cos(math.radians(ang)) * 22.0
        hy = cy - 24 + math.sin(math.radians(ang)) * 11.0
        _draw_hand(rd, (hx, hy), ang + 90, spread=3.0, scale=0.42, palm=(230, 214, 186, 255), finger=(244, 226, 198, 255), outline=(150, 132, 164, 255))
    if pose["judge"] > 0.0:
        for i in range(3):
            _star(rd, (cx, cy - 34 - i * 6), 3.8 + i * 1.0 + pose["judge"] * 4.0, 1.6 + i * 0.4, 8, (255, 236, 160, int(140 - i * 20)), None)
    if pose["bless"] > 0.0:
        for side in (-1, 1):
...
def _pose_spaghetti(anim: str, frame_idx: int, nframes: int) -> Dict[str, float]:
    ...
    cyc = math.tau * frame_idx / max(1, nframes)
    s = math.sin(cyc)
    pose = {"x": 0.0, "bob": 0.0, "noodle": s, "mouth": 0.0, "lash": 0.0, "unravel": 0.0, "hurt": 0.0, "dead": 0.0}
    if anim == "idle":
        pose["bob"] = s * 1.0
    elif anim == "slither":
        pose["x"] = s * 2.5
...
def _render_spaghetti(anim: str, frame_idx: int, nframes: int) -> Image.Image:
    ...
    # Body mound.
    body_paths = [
        _bezier((cx - 28, cy + 8), (cx - 14, cy - 22 - pose["unravel"] * 10), (cx + 12, cy - 24), (cx + 28, cy + 2), 18),
    ...
    for i in range(6):
        off = -24 + i * 10
        pts = _bezier((cx + off, cy + 8 + abs(off) * 0.05), (cx + off * 0.4, cy - 8 - abs(off) * 0.12), (cx + off * 0.2, cy + 22), (cx + off * 0.8, cy + 28 + math.sin(i + frame_idx) * 2.0), 10)
    ...
    for off in (-10, 12):
        ...
        _emotion_eye(draw, cx + off, cy - 4.2, 2.0, 2.6, CYAN if off < 0 else MAGENTA, blink=pose["dead"] * 0.9)
    # Mouth / maw.
    mouth_open = pose["mouth"] * 8.0 + pose["unravel"] * 5.0
    _ellipse(draw, cx + 2, cy + 10, 10, 4.0 + mouth_open, (84, 44, 44, 255), OUTLINE, 0.7)
    for i in range(5):
        tooth_x = cx - 6 + i * 4.0
        _triangle(draw, (tooth_x, cy + 8 - (i % 2) * 0.6), 1.2, WHITE, 180)
        _triangle(draw, (tooth_x + 0.6, cy + 12 + mouth_open * 0.45 + (i % 2) * 0.5), 1.0, WHITE, 0)
    # Fork appendages.
    for side in (-1, 1):
        arm = _bezier((cx + side * 18, cy + 10), (cx + side * 28, cy + 6 - pose["lash"] * 6), (cx + side * 34, cy - 8 + side * pose["lash"] * 3), (cx + side * (42 + pose["lash"] * 16), cy - 4 - pose["lash"] * 10), 12)
        _ribbon(draw, arm, 3.6, noodle_lo, OUTLINE, noodle_hi)
        base = arm[-1]
        for prong in (-4, 0, 4):
            _line(draw, [base, (base[0] + side * 4, base[1] - 7 + prong * 0.4)], (196, 210, 230, 255), 1.2)
    # Sauce drips.
    for xoff in (-14, -2, 9):
...
def _pose_liar(anim: str, frame_idx: int, nframes: int) -> Dict[str, float]:
    ...
    cyc = math.tau * frame_idx / max(1, nframes)
    s = math.sin(cyc)
    pose = {"x": 0.0, "bob": 0.0, "tilt": 0.0, "arm": 0.0, "confirm": 0.0, "misdirect": 0.0, "hurt": 0.0, "dead": 0.0}
    if anim == "idle":
        pose["bob"] = s * 1.2
        pose["tilt"] = s * 1.2
    elif anim == "roll":
...
def _render_helpful_liar(anim: str, frame_idx: int, nframes: int) -> Image.Image:
    ...
    _capsule(bd, (cx - 12, cy + 50), (cx + 12, cy + 50), 4.0, (150, 160, 184, 255), OUTLINE, None)
    for side in (-1, 1):
        _circle(bd, (cx + side * 9.5, cy + 54), 4.2, (104, 114, 132, 255), OUTLINE, 0.8)
        _circle(bd, (cx + side * 9.5, cy + 54), 1.5, (210, 218, 232, 255), None, 0)
    # Main screen.
    _rounded_rect(bd, (cx - 24, cy - 22, cx + 24, cy + 18), (194, 230, 240, 255), (118, 138, 164, 255), 1.0, 5.0)
    _rounded_rect(bd, (cx - 19, cy - 17, cx + 19, cy + 13), (74, 128, 164, 255), (64, 92, 120, 255), 0.9, 4.0)
    # Face speech bubble.
    bubble = [(cx - 14, cy - 9), (cx + 11, cy - 9), (cx + 11, cy + 4), (cx + 3, cy + 4), (cx - 1, cy + 9), (cx - 2, cy + 4), (cx - 14, cy + 4)]
    ...
    _line(bd, [(cx - 8, cy + 2), (cx - 2, cy + 4), (cx + 5, cy + 2)], (80, 116, 124, 255), 0.8)
    # UI badges.
    _circle(bd, (cx + 18, cy - 15), 3.0, GREEN, OUTLINE, 0.6)
    _line(bd, [(cx + 16.5, cy - 15), (cx + 18, cy - 13.2), (cx + 20.5, cy - 17.3)], WHITE, 0.8)
    ...
    _capsule(bd, (cx - 20, cy - 2), (left_wrist[0] - 8, left_wrist[1] + 6), 3.2, (214, 224, 236, 255), OUTLINE, None)
    _capsule(bd, (cx + 20, cy + 0), (right_wrist[0] + 8, right_wrist[1] + 4), 3.2, (214, 224, 236, 255), OUTLINE, None)
    _triangle(bd, (left_wrist[0] - 10, left_wrist[1] + 6), 5.0, WHITE, -20)
    _triangle(bd, (right_wrist[0] + 10, right_wrist[1] + 4), 5.0, WHITE, 160)
    if pose["confirm"] > 0:
        _rounded_rect(bd, (cx - 16, cy - 34, cx + 16, cy - 24), (236, 246, 252, 220), (182, 198, 216, 255), 0.8, 2.0)
        _line(bd, [(cx - 12, cy - 29), (cx - 8, cy - 25), (cx - 2, cy - 31)], GREEN, 1.0)
...
def _draw_card(draw: ImageDraw.ImageDraw, x: float, y: float, w: float, h: float, tilt: float, base: RGBA, accent: RGBA, *, bad_face: bool = False, pseudo_text: bool = True, play: bool = False) -> None:
    ...
    # headline strip
    _rounded_rect(ld, (x + 2, y + 2, x + w - 2, y + 2 + header_h), (248, 250, 252, 230), None, 0, 1.6)
    for i in range(2):
        line_y = y + 3.3 + i * 2.2
        _rounded_rect(ld, (x + 4, line_y, x + w - 4 - i * 2, line_y + 1.1), (232, 236, 244, 170), None, 0, 0.6)
    if play:
        _rounded_rect(ld, (x + 3, media_y, x + w - 3, media_y2), accent, None, 0, 1.8)
        _triangle(ld, (x + w * 0.54, media_y + media_h * 0.54), min(w, h) * 0.15, WHITE, 90)
    elif bad_face:
        _rounded_rect(ld, (x + 3, media_y, x + w - 3, media_y2), accent, None, 0, 1.8)
        # uncanny face.
        _ellipse(ld, x + w * 0.52, media_y + media_h * 0.55, max(2.5, w * 0.18), max(2.5, media_h * 0.22), (246, 220, 200, 255), OUTLINE, 0.5)
        _emotion_eye(ld, x + w * 0.45, media_y + media_h * 0.50, 1.5, 2.0, CYAN)
        _emotion_eye(ld, x + w * 0.56, media_y + media_h * 0.48, 1.2, 2.4, MAGENTA)
        _line(ld, [(x + w * 0.44, media_y + media_h * 0.64), (x + w * 0.58, media_y + media_h * 0.66)], RED, 0.6)
        ...
        if pseudo_text:
            for i in range(3):
                ...
                if ly + 1.1 <= media_y2 - 1.0:
                    _rounded_rect(ld, (x + 5, ly, x + w - 5 - (i % 2) * 4, ly + 1.1), (250, 250, 252, 165), None, 0, 0.6)
...
def _render_ai_slop(anim: str, frame_idx: int, nframes: int) -> Image.Image:
    ...
    blob = [
        (cx - 26, cy - 24), (cx - 36, cy - 6), (cx - 34, cy + 18), (cx - 24, cy + 34),
        (cx - 5, cy + 42), (cx + 18, cy + 38), (cx + 32, cy + 18), (cx + 34, cy - 8),
        (cx + 26, cy - 28), (cx + 6, cy - 38), (cx - 12, cy - 36),
    ]
    _poly(bd, blob, (118, 84, 164, 255), OUTLINE, 1.1)
    _poly(bd, [(cx - 18, cy - 20), (cx - 22, cy + 24), (cx - 4, cy + 30), (cx + 6, cy - 24)], (154, 112, 198, 190), None, 0)

    # Sticky cards / thumbnails mounted to the body.
    cards = [
        (cx - 28, cy - 20, 18, 22, -12 + pose["tilt"], (242, 244, 248, 255), (118, 146, 214, 255), False, False, True),
        (cx - 4, cy - 30, 24, 18, 8 - pose["tilt"], (246, 246, 250, 255), (238, 158, 184, 255), True, False, False),
        (cx + 12, cy - 8, 18, 24, 16 + pose["burst"] * 8, (248, 248, 252, 255), (128, 210, 188, 255), False, True, False),
        (cx - 12, cy + 6, 28, 20, -8 + pose["rep"] * 10, (246, 246, 250, 255), (238, 196, 96, 255), False, True, False),
        (cx + 8, cy + 18, 20, 16, 14 - pose["tilt"], (248, 248, 252, 255), (156, 120, 220, 255), True, False, False),
    ]
    for x, y, w, h, tilt, base, accent, bad_face, pseudo_text, play in cards:
        _draw_card(bd, x, y, w, h, tilt, base, accent, bad_face=bad_face, pseudo_text=pseudo_text, play=play)

    # Main face / clickbait mouth.
    _ellipse(bd, cx - 2, cy + 4, 15, 11, (242, 210, 188, 210), (200, 146, 148, 220), 0.7)
    _emotion_eye(bd, cx - 7, cy - 1, 2.6, 3.2, CYAN, pupil_shift=pose["hurt"] * 1.2)
    _emotion_eye(bd, cx + 1.8, cy - 2.0, 2.0, 4.0, MAGENTA, pupil_shift=-pose["hurt"] * 1.0)
    _circle(bd, (cx + 10.5, cy + 0.5), 1.6, YELLOW, OUTLINE, 0.5)
    _circle(bd, (cx + 12.8, cy + 9.0), 1.0, RED, OUTLINE, 0.5)
    grin_w = 15 + pose["ad"] * 4.0
    _ellipse(bd, cx - 1, cy + 11, grin_w, 5.0 + pose["burst"] * 3.8, (76, 34, 58, 255), OUTLINE, 0.8)
    for i in range(8):
        tx = cx - 12 + i * 3.5
        _triangle(bd, (tx, cy + 8.5), 1.2 + (i % 2) * 0.3, WHITE, 180)
        _triangle(bd, (tx + 0.4, cy + 13.5 + pose["burst"] * 1.8), 1.0 + ((i + 1) % 2) * 0.2, WHITE, 0)

    # Slime drips / glitched subtitle bars.
    for off in (-18, -9, 3, 14):
        length = 5 + (off % 3) * 2 + pose["burst"] * 5.0
        _ribbon(bd, [(cx + off, cy + 18), (cx + off + math.sin(frame_idx + off) * 1.1, cy + 18 + length)], 1.5, (112, 234, 122, 220), (72, 146, 86, 220))
    for i in range(5):
        y = cy - 35 + i * 2.8
        _rounded_rect(bd, (cx + 16, y, cx + 28 + (i % 2) * 5, y + 1.8), (250, 250, 252, 110), None, 0, 0.8)
    ...
    left_arm_pts = _bezier((cx - 20, cy + 10), (cx - 34, cy + 6 + pose["arm"] * 2), (cx - 44, cy + 18 - pose["arm"] * 4), (cx - 50, cy + 8 - pose["arm"] * 6), 12)
    right_arm_pts = _bezier((cx + 18, cy + 6), (cx + 30, cy - 4), (cx + 42, cy + 4 + pose["arm"] * 5), (cx + 50, cy - 8 + pose["arm"] * 7), 12)
    _ribbon(bd, left_arm_pts, 4.0, (240, 222, 198, 255), OUTLINE, None)
    _ribbon(bd, right_arm_pts, 4.0, (240, 222, 198, 255), OUTLINE, None)
    _draw_hand(bd, left_arm_pts[-1], 180 - pose["arm"] * 10, spread=12 + pose["rep"] * 8, scale=0.75, palm=(244, 226, 200, 255))
    _draw_hand(bd, right_arm_pts[-1], 0 + pose["arm"] * 10, spread=12 + pose["rep"] * 8, scale=0.75, palm=(244, 226, 200, 255))
    # Extra wrong thumb.
    _capsule(bd, (cx + 40, cy + 12), (cx + 48, cy + 20 + pose["rep"] * 6), 3.0, (244, 226, 200, 220), OUTLINE, None)
    _draw_hand(bd, (cx + 48, cy + 20 + pose["rep"] * 6), 35, spread=6, scale=0.45, palm=(244, 226, 200, 220))
    # Cursor quills.
    for i in range(4):
    ...
    if pose["rep"] > 0.0:
        for i in range(3):
            ...
            dy = -22 - i * 4
            alpha = int(170 - i * 36)
            _draw_card(bd, cx + dx, cy + dy, 16, 12, 8 + i * 6, (248, 248, 252, alpha), (120, 186, 228, alpha), bad_face=(i % 2 == 0), pseudo_text=True, play=(i == 1))
    ...
    if pose["burst"] > 0.0:
        for i in range(5):
            ...
            p2 = (cx + math.cos(math.radians(ang)) * r2, cy - 4 + math.sin(math.radians(ang)) * r2)
            _capsule(bd, p1, p2, 1.8, (255, 236, 146, 180), None, None)
    if pose["ad"] > 0.0:
        _rounded_rect(bd, (cx - 22, cy - 46, cx + 22, cy - 36), (255, 244, 196, 210), (226, 180, 84, 240), 0.8, 2.0)
...
def _pose_agent(anim: str, frame_idx: int, nframes: int) -> Dict[str, float]:
    ...
    cyc = math.tau * frame_idx / max(1, nframes)
    s = math.sin(cyc)
    pose = {"bob": 0.0, "orbit": frame_idx * (360.0 / max(1, nframes)), "scan": 0.0, "deploy": 0.0, "converge": 0.0, "hurt": 0.0, "dead": 0.0}
    if anim == "idle":
        pose["bob"] = s * 1.2
    elif anim == "orbit":
...
def _render_agent_swarm(anim: str, frame_idx: int, nframes: int) -> Image.Image:
    ...
    _emotion_eye(draw, cx, cy + 1, 4.4, 5.4, CYAN, pupil_shift=(pose["scan"] - pose["hurt"]) * 2.0, blink=pose["dead"] * 0.9)
    ...
    for idx, (base_ang, rad, kind) in enumerate(drones):
        ...
        if kind == "lens":
            ...
            _capsule(draw, (dx - 2.0, dy + 2.0), (dx - 6.0, dy + 8.0), 1.5, (120, 136, 170, 255), OUTLINE, None)
            _capsule(draw, (dx + 2.0, dy + 2.0), (dx + 6.0, dy + 8.0), 1.5, (120, 136, 170, 255), OUTLINE, None)
        elif kind == "scan":
            for i in range(3):
                _line(draw, [(dx - 6, dy + 4 + i * 2), (dx + 6, dy + 4 + i * 2)], CYAN, 0.6)
        elif kind == "bolt":
            _triangle(draw, (dx, dy + 1), 3.3, YELLOW, 180)
        else:
            bubble = [(dx - 5, dy - 2), (dx + 3, dy - 2), (dx + 3, dy + 3), (dx, dy + 3), (dx - 2, dy + 6), (dx - 2, dy + 3), (dx - 5, dy + 3)]
            _poly(draw, bubble, WHITE, (170, 180, 204, 255), 0.6)
    if pose["scan"] > 0.0:
        for i in range(3):
            _ellipse(draw, cx, cy + 4, 10 + i * 7 + pose["scan"] * 6, 8 + i * 4 + pose["scan"] * 4, (0, 0, 0, 0), (96, 236, 240, int(140 - i * 30)), 0.8)
    if pose["deploy"] > 0.0:
        for side in (-1, 1):
...
def _render_target(target: str, frame_fn: Callable[[str, int, int], Image.Image], out_dir: str | Path, *, label_width: int = 124) -> List[Path]:
...
def _entry(target: str, frame_fn: Callable[[str, int, int], Image.Image], label_width: int = 124) -> Dict[str, object]:
    return {
        "render": lambda out_dir, **opts: _render_target(target, frame_fn, out_dir, label_width=label_width),
        "sheet_files": [
...
LOCAL_ACTOR_METADATA_BY_TARGET = {'agent_swarm': {'actor': {'character_id': 'npc_agent_swarm', 'display_name': 'Agent Swarm'},
                 'body': {'body_plan': 'PropActor',
                          'body_kind': 'PropLike',
                          'mass_class': 'Light',
                 ...
                 'sockets': {'center': {'source': 'explicit.profile.floating_prop',
                                        'point': {'x': 80.0, 'y': 80.0}},
                             'projectile_origin': {'source': 'explicit.profile.floating_prop',
                                                   'point': {'x': 92.0, 'y': 72.0}},
                             'speech_bubble': {'source': 'explicit.profile.floating_prop',
                                               'point': {'x': 80.0, 'y': 24.0}}},
                 'animation_bindings': {'default': {'animation': 'idle', 'events': []},
                                        ...
                                        'action.special.deploy': {'animation': 'deploy',
                                                                  'events': [{'t': 0.5,
                                                                              'event': 'summon_minion',
                                                                              'source': 'explicit.profile.ai_era'}]}}},
 'ai_slop': {'actor': {'character_id': 'npc_ai_slop', 'display_name': 'AI Slop'},
             'body': {'body_plan': 'Crawler',
                      'body_kind': 'Wide',
                      'mass_class': 'Heavy',
             ...
             'sockets': {'mouth': {'source': 'explicit.profile.ai_era',
                                   'point': {'x': 104.0, 'y': 74.0}},
                         'center': {'source': 'explicit.profile.ai_era',
                                    'point': {'x': 80.0, 'y': 80.0}}},
             'animation_bindings': {'default': {'animation': 'idle', 'events': []},
                                    'locomotion.crawl': {'animation': 'crawl', 'events': []},
                                    'action.melee.primary': {'animation': 'lunge',
                                                             'events': [{'t': 0.35,
                                                                        ...
                                                                        {'t': 0.55,
                                                                         ...
                                                                         'source': 'explicit.profile.ai_era'}]},
                                    'action.ranged.adblast': {'animation': 'adblast',
                                                              'events': [{'t': 0.5,
                                                                          'event': 'projectile_release',
                                                                          'source': 'explicit.profile.ai_slop'}]}}},
 'hand_saint': {'actor': {'character_id': 'npc_hand_saint', 'display_name': 'Hand Saint'},
                'body': {'body_plan': 'PropActor',
                         'body_kind': 'PropLike',
                         'mass_class': 'Light',
                ...
                'sockets': {'center': {'source': 'explicit.profile.floating_prop',
                                       'point': {'x': 80.0, 'y': 80.0}},
                            'projectile_origin': {'source': 'explicit.profile.hand_saint',
                                                  'point': {'x': 80.0, 'y': 58.0}},
                            'speech_bubble': {'source': 'explicit.profile.floating_prop',
                                              'point': {'x': 80.0, 'y': 24.0}},
                            'palm': {'source': 'explicit.profile.hand_saint',
                                     'point': {'x': 80.0, 'y': 78.0}}},
                'animation_bindings': {'default': {'animation': 'idle', 'events': []},
                                       'locomotion.hover': {'animation': 'hover', 'events': []},
                                       'action.ranged.primary': {'animation': 'shoot',
                                                                 'events': [{'t': 0.5,
                                                                             'event': 'projectile_release',
                                                                             'source': 'explicit.profile.floating_prop'}]},
                                       'action.special.bless': {'animation': 'bless',
                                                                'events': [{'t': 0.45,
                                                                            'event': 'blessing_emit',
                                                                            'source': 'explicit.profile.hand_saint'}]}}},
 'helpful_liar': {'actor': {'character_id': 'npc_helpful_liar', 'display_name': 'Helpful Liar'},
                  'body': {'body_plan': 'PropActor',
                           'body_kind': 'PropLike',
                           'mass_class': 'Light',
                  ...
                  'animation_bindings': {'default': {'animation': 'idle', 'events': []},
                                         ...
                                         'action.ranged.primary': {'animation': 'shoot',
                                                                   'events': [{'t': 0.5,
                                                                               'event': 'projectile_release',
                                                                               'source': 'explicit.profile.floating_prop'}]}}},
 'puppy_slug_variant2': {'actor': {'character_id': 'npc_puppy_slug_variant2',
                         ...
                         'sockets': {'mouth': {'source': 'explicit.profile.ai_era',
                                     ...
                                     'center': {'source': 'explicit.profile.ai_era',
                                                'point': {'x': 80.0, 'y': 80.0}}},
                         'animation_bindings': {'default': {'animation': 'idle', 'events': []},
                                                'locomotion.crawl': {'animation': 'crawl',
                                                                     'events': []},
                                                'action.melee.primary': {'animation': 'lunge',
                                                                         'events': [{'t': 0.35,
                                                                                     'event': 'hitbox_active_start',
                                                                                    ...
                                                                                    {'t': 0.55,
                                                                                     'event': 'hitbox_active_end',
                                                                                     'source': 'explicit.profile.ai_era'}]}}},
 'spaghetti_event': {'actor': {'character_id': 'npc_spaghetti_event',
                     ...
                     'sockets': {'mouth': {'source': 'explicit.profile.ai_era',
                                 ...
                                 'center': {'source': 'explicit.profile.ai_era',
                                            'point': {'x': 80.0, 'y': 80.0}},
                                 'noodle_tip': {'source': 'explicit.profile.spaghetti_event',
                                            ...
                                            'action.melee.primary': {'animation': 'lunge',
                                                                     'events': [{'t': 0.35,
                                                                                ...
                                                                                {'t': 0.55,
                                                                                 'event': 'hitbox_active_end',
                                                                                 'source': 'explicit.profile.ai_era'}]}}},
 'synthetic_friend': {'actor': {'character_id': 'npc_synthetic_friend',
                      ...
                      'sockets': {'center': {'source': 'explicit.profile.floating_prop',
                                  ...
                                  'speech_bubble': {'source': 'explicit.profile.floating_prop',
                                                    'point': {'x': 80.0, 'y': 24.0}}},
                      'animation_bindings': {'default': {'animation': 'idle', 'events': []},
                                             'locomotion.hover': {'animation': 'hover',
                                                                  'events': []},
                                             'action.ranged.primary': {'animation': 'shoot',
                                                                       'events': [{'t': 0.5,
                                                                                   'event': 'projectile_release',
