...
ACTOR_METADATA = {'actor': {'character_id': 'npc_creator', 'display_name': 'Creator', 'actor_id': 'creator'},
 ...
 'missing_information': ['Creator story-presence state is intentionally authored outside the '
                         'renderer sidecar.']}
...
def _draw_creator(anim: str, frame_idx: int, nframes: int) -> Image.Image:
    ...
    for leg_dx, foot_y, coat_slit in [
        ...
        draw.rectangle(_box(base_x + leg_dx - 4.2, 120 + bob, base_x + leg_dx + 4.0, foot_y - 8.0), fill=_rgba("#2d314f"), outline=shadow)
        draw.rectangle(_box(base_x + leg_dx - 4.7, foot_y - 11.0, base_x + leg_dx + 5.2, foot_y), fill=_rgba("#4f5565"), outline=shadow)
        draw.line(_box(base_x + coat_slit, 105 + bob, base_x + coat_slit + leg_dx * 0.18, 128 + bob), fill=shadow, width=_s(0.6))
    ...
    draw.polygon([_pt(72.0, 112.0 + bob), _pt(79.0, 160.0 + bob), _pt(86.0, 112.0 + bob)], fill=lining)

    # High collar and lapels.
    collar_left = [(67.0, 77.0 + bob), (77.0, 63.0 + bob), (84.0, 80.0 + bob), (77.0, 93.0 + bob)]
    collar_right = [(92.0, 77.0 + bob), (82.0, 63.0 + bob), (76.0, 80.0 + bob), (82.0, 94.0 + bob)]
    ...
    mantle_poly = [(58.0, 77.0 + bob), (71.0, 68.0 + bob - sway * 0.2), (93.0, 68.0 + bob), (104.0, 77.0 + bob), (100.0, 88.0 + bob), (75.0, 90.0 + bob), (62.0, 86.0 + bob)]
    draw.polygon([_pt(*p) for p in mantle_poly], fill=mantle, outline=shadow)
    epaulet = [(56.0, 78.0 + bob), (45.0, 88.0 + bob), (52.0, 100.0 + bob), (70.0, 90.0 + bob)]
    ...
    draw.line((_s(l0[0]), _s(l0[1]), _s(l1[0]), _s(l1[1])), fill=coat_mid, width=_s(3.3))
    draw.line((_s(l1[0]), _s(l1[1]), _s(l2[0]), _s(l2[1])), fill=coat_light, width=_s(2.9))
    draw.line((_s(r0[0]), _s(r0[1]), _s(r1[0]), _s(r1[1])), fill=coat_mid, width=_s(3.1))
    draw.line((_s(r1[0]), _s(r1[1]), _s(r2[0]), _s(r2[1])), fill=coat_light, width=_s(2.7))
    # Hands.
    draw.ellipse(_box(l2[0] - 3.5, l2[1] - 3.5, l2[0] + 3.5, l2[1] + 3.5), fill=skin, outline=shadow)
    draw.ellipse(_box(r2[0] - 3.3, r2[1] - 3.3, r2[0] + 3.3, r2[1] + 3.3), fill=skin, outline=shadow)

    # Right hand held codex / tablet.
    codex = [(r2[0] - 7.0, r2[1] - 8.0), (r2[0] + 3.0, r2[1] - 10.0), (r2[0] + 7.0, r2[1] + 4.0), (r2[0] - 4.0, r2[1] + 6.0)]
    draw.polygon([_pt(*p) for p in codex], fill=_rgba("#d8dce8"), outline=shadow)
    draw.line(_box(r2[0] - 2.0, r2[1] - 7.0, r2[0] + 2.0, r2[1] + 4.0), fill=_rgba("#a59bcf"), width=_s(0.55))

    # Head.
    draw.ellipse(_box(66.0, head_y - 1.0, 96.0, head_y + 30.0), fill=skin, outline=shadow)
    # Hair.
    ...
    draw.arc(_box(77.5, head_y + 15.0, 85.5, head_y + 23.0 + speak_open * 2.8), 20, 160, fill=_rgba("#7e3f56"), width=_s(0.55))
    draw.line(_box(81.0, head_y + 11.0, 80.0, head_y + 16.0), fill=_rgba("#b98f86"), width=_s(0.35))
    ...
    outer_r = 23.0
    inner_r = 18.0
    hd.ellipse(_box(halo_cx - outer_r, halo_cy - outer_r, halo_cx + outer_r, halo_cy + outer_r), outline=_rgba("#6fdfff", 180), width=_s(1.0))
    hd.ellipse(_box(halo_cx - inner_r, halo_cy - inner_r, halo_cx + inner_r, halo_cy + inner_r), outline=_rgba("#cc9cff", 160), width=_s(0.7))
    for i in range(6):
        ...
        shard = [(x2, y2), (x2 + math.cos(ang + 0.45) * 3.5, y2 + math.sin(ang + 0.45) * 3.5), (x2 + math.cos(ang - 0.45) * 3.5, y2 + math.sin(ang - 0.45) * 3.5)]
    ...
    _glow_polygon(img, [(sigil_cx, sigil_cy - 6.0), (sigil_cx + 6.0, sigil_cy), (sigil_cx, sigil_cy + 6.0), (sigil_cx - 6.0, sigil_cy)], _rgba("#b88cff", 80), blur=3.5)
    draw.polygon([_pt(sigil_cx, sigil_cy - 4.0), _pt(sigil_cx + 4.0, sigil_cy), _pt(sigil_cx, sigil_cy + 4.0), _pt(sigil_cx - 4.0, sigil_cy)], fill=glow_cyan, outline=_rgba("#effdff"))
    draw.ellipse(_box(sigil_cx - 2.1, sigil_cy - 2.1, sigil_cx + 2.1, sigil_cy + 2.1), fill=glow_violet)
    ...
    sash = [(90.0, 100.0 + bob), (101.0, 103.0 + bob), (94.0, 149.0 + bob), (84.0, 145.0 + bob)]
    draw.polygon([_pt(*p) for p in sash], fill=_rgba("#7e4a92"), outline=shadow)
    draw.line(_box(92.0, 105.0 + bob, 88.0, 145.0 + bob), fill=cloth_light, width=_s(0.42))
