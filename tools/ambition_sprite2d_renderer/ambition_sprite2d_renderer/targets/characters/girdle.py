...
ACTOR_METADATA = {'actor': {'character_id': 'npc_girdle', 'display_name': 'Girdle'},
...
def poly(draw: ImageDraw.ImageDraw, pts: Sequence[Point], fill: RGBA, outline: RGBA = OUTLINE, width: float = 1.0) -> None:
...
def line(draw: ImageDraw.ImageDraw, pts: Sequence[Point], fill: RGBA, width: float = 1.0) -> None:
...
def circle(draw: ImageDraw.ImageDraw, c: Point, r: float, fill: RGBA, outline: RGBA = OUTLINE, width: float = 1.0) -> None:
...
def draw_paper(draw: ImageDraw.ImageDraw, center: Point, angle: float, w: float = 22.0, h: float = 28.0, with_title: bool = False) -> None:
    hw = w / 2
    hh = h / 2
    pts = []
    ...
    line(draw, [(center[0] - 7, center[1] - 6), (center[0] + 5, center[1] - 6)], INK, 0.5)
    line(draw, [(center[0] - 7, center[1] - 1), (center[0] + 7, center[1])], INK, 0.45)
    line(draw, [(center[0] - 7, center[1] + 4), (center[0] + 6, center[1] + 5)], INK, 0.45)
    if with_title:
        line(draw, [(center[0] - 7, center[1] - 10), (center[0] + 2, center[1] - 10)], RED, 0.55)
...
class GirdleRenderer:
    def render_frame(self, anim: str, frame_idx: int, nframes: int) -> Image.Image:
        ...
        ellipse(draw, root[0], root[1] + 4, 42, 8, SHADOW, outline=(0, 0, 0, 0), width=0)
    ...
    def _draw_legs(self, draw: ImageDraw.ImageDraw, P, pose: Pose, yaw: float) -> None:
        ...
        poly(draw, [(back_foot[0] - 7, back_foot[1] - 3), (back_foot[0] + 14, back_foot[1] - 3), (back_foot[0] + 18, back_foot[1] + 4), (back_foot[0] - 5, back_foot[1] + 5)], SUIT_DARK, OUTLINE, 0.8)
        # Front leg
        front_hip = P(front_x, hip_y)
        front_knee = P(front_x + 4 + pose.front_leg * 0.16, knee_y)
        front_foot = P(front_x + 8 + pose.front_leg * 0.12, foot_y - pose.front_foot_lift)
        line(draw, [front_hip, front_knee, front_foot], SUIT, 6.8)
        line(draw, [front_hip, front_knee, front_foot], OUTLINE, 1.4)
        poly(draw, [(front_foot[0] - 7, front_foot[1] - 3), (front_foot[0] + 15, front_foot[1] - 3), (front_foot[0] + 20, front_foot[1] + 4), (front_foot[0] - 5, front_foot[1] + 5)], SUIT, OUTLINE, 0.9)

    def _draw_torso(self, draw: ImageDraw.ImageDraw, P, pose: Pose, yaw: float) -> Point:
        if yaw < 0.35:
            torso = [P(-28, -116), P(28, -116), P(34, -60), P(20, -42), P(-20, -42), P(-34, -60)]
            poly(draw, torso, SUIT, OUTLINE, 1.2)
            # lapels
            poly(draw, [P(-7, -112), P(0, -88), P(-14, -62), P(-24, -94)], SUIT_LIGHT, OUTLINE, 0.8)
            poly(draw, [P(7, -112), P(0, -88), P(14, -62), P(24, -94)], SUIT_DARK, OUTLINE, 0.8)
            poly(draw, [P(-10, -116), P(10, -116), P(8, -72), P(-8, -72)], SHIRT, OUTLINE, 0.8)
            ...
            torso = [P(-20, -116), P(18, -118), P(28, -100), P(30, -58), P(18, -42), P(-10, -42), P(-24, -62), P(-24, -98)]
            poly(draw, torso, SUIT, OUTLINE, 1.2)
            poly(draw, [P(-4, -114), P(10, -108), P(18, -86), P(8, -60), P(-2, -78)], SUIT_DARK, OUTLINE, 0.8)
            poly(draw, [P(-8, -114), P(6, -112), P(4, -74), P(-8, -72)], SHIRT, OUTLINE, 0.7)
    ...
    def _draw_arms(self, draw: ImageDraw.ImageDraw, P, pose: Pose, yaw: float, chest: Point) -> None:
        if yaw < 0.35:
            left_sh = P(-26, -106)
            right_sh = P(26, -106)
            # Back/left arm holding manuscript loosely.
            left_el = P(-32 + pose.back_arm_angle * 0.08, -72 + pose.back_arm_angle * 0.10)
            left_hand = P(-30 + pose.back_arm_angle * 0.10, -36 + pose.paper_raise * 0.4)
            ...
            if pose.manuscript > 0.1 or pose.paper_raise > -0.5:
                draw_paper(draw, (left_hand[0] - 4, left_hand[1] + 2), -10 + pose.paper_raise * 0.2, 18, 24, with_title=True)

            right_el = P(28 + pose.front_arm_angle * 0.08 + pose.front_arm_reach * 0.10, -72 + pose.front_arm_angle * 0.12)
            right_hand = P(28 + pose.front_arm_angle * 0.10 + pose.front_arm_reach, -38 + pose.front_arm_reach * 0.04)
            ...
            if pose.front_arm_reach > 4.0:
                line(draw, [(right_hand[0] + 2, right_hand[1]), (right_hand[0] + 18, right_hand[1] - 2)], SKIN, 1.2)
                line(draw, [(right_hand[0] + 2, right_hand[1]), (right_hand[0] + 18, right_hand[1] - 2)], OUTLINE, 0.4)
        else:
            back_sh = P(-12, -106)
            front_sh = P(18, -106)
            back_el = P(-22 + pose.back_arm_angle * 0.06, -74 + pose.back_arm_angle * 0.10)
            back_hand = P(-18 + pose.back_arm_angle * 0.08 + pose.back_arm_reach, -40 + pose.paper_raise * 0.3)
            ...
            if pose.manuscript > 0.15:
                draw_paper(draw, (back_hand[0] - 1, back_hand[1] + 1), -14 + pose.paper_raise * 0.2, 18, 24, with_title=True)

            front_el = P(30 + pose.front_arm_angle * 0.08 + pose.front_arm_reach * 0.22, -78 + pose.front_arm_angle * 0.12)
            front_hand = P(44 + pose.front_arm_angle * 0.12 + pose.front_arm_reach, -52 + pose.front_arm_reach * 0.16)
    ...
    def _draw_head_front(self, img: Image.Image, draw: ImageDraw.ImageDraw, c: Point, pose: Pose) -> None:
        ...
        ellipse(draw, cx + 22, cy + 2, 5.5, 8.5, SKIN_SHADE, OUTLINE, 0.7)
        # Hair cap and sides
        poly(draw, [(cx - 20, cy - 30), (cx + 20, cy - 30), (cx + 22, cy - 4), (cx + 14, cy - 24), (cx - 14, cy - 24), (cx - 22, cy - 4)], HAIR, OUTLINE, 0.8)
        # Face
        face = [(cx - 17, cy - 20), (cx + 17, cy - 20), (cx + 19, cy + 8), (cx + 10, cy + 24), (cx, cy + 30), (cx - 10, cy + 24), (cx - 19, cy + 8)]
        poly(draw, face, SKIN, OUTLINE, 1.0)
        # Forehead highlight
        line(draw, [(cx - 8, cy - 16), (cx + 8, cy - 16)], SKIN_LIGHT, 0.8)
        # Hairline: high and receding
        line(draw, [(cx - 12, cy - 18), (cx - 8, cy - 26), (cx + 8, cy - 26), (cx + 12, cy - 18)], OUTLINE, 1.0)
        ...
        if pose.blink:
            ...
            ellipse(draw, cx - 10, cy + 0, 2.3, 2.0 + pose.eye_widen, SHIRT, OUTLINE, 0.5)
            ellipse(draw, cx + 10, cy + 0, 2.3, 2.0 + pose.eye_widen, SHIRT, OUTLINE, 0.5)
        ...
        line(draw, [(cx - 7, cy + 17), (cx, cy + 19 + pose.mouth * 6), (cx + 7, cy + 17)], OUTLINE, 0.8)
    ...
    def _draw_head_three_quarter(self, img: Image.Image, draw: ImageDraw.ImageDraw, c: Point, pose: Pose) -> None:
        ...
        # Hair back
        poly(draw, [(cx - 18, cy - 28), (cx + 14, cy - 30), (cx + 20, cy - 20), (cx + 18, cy - 3), (cx - 4, cy - 8), (cx - 18, cy - 2)], HAIR, OUTLINE, 0.8)
        face = [(cx - 15, cy - 18), (cx + 11, cy - 21), (cx + 20, cy - 10), (cx + 25, cy + 3), (cx + 21, cy + 11), (cx + 9, cy + 24), (cx - 8, cy + 26), (cx - 17, cy + 14)]
        ...
        if pose.blink:
            ...
            ellipse(draw, cx - 8, cy + 1, 1.6, 1.4 + pose.eye_widen * 0.6, SHIRT, OUTLINE, 0.4)
        ...
        # Nose and mouth
        poly(draw, [(cx + 8, cy + 2), (cx + 25, cy + 4), (cx + 11, cy + 10)], SKIN, OUTLINE, 0.6)
        line(draw, [(cx + 1, cy + 17), (cx + 10, cy + 18 + pose.mouth * 6), (cx + 17, cy + 16)], OUTLINE, 0.8)
    ...
    def _draw_head_side(self, img: Image.Image, draw: ImageDraw.ImageDraw, c: Point, pose: Pose) -> None:
        ...
        # Hair mass
        poly(draw, [(cx - 16, cy - 30), (cx + 7, cy - 30), (cx + 18, cy - 22), (cx + 18, cy - 8), (cx + 6, cy - 10), (cx - 12, cy - 6), (cx - 18, cy - 12)], HAIR, OUTLINE, 0.8)
        ...
        poly(draw, [(cx + 8, cy + 2), (cx + 28, cy + 4), (cx + 12, cy + 10)], SKIN, OUTLINE, 0.6)
        line(draw, [(cx + 1, cy + 17), (cx + 12, cy + 18 + pose.mouth * 5), (cx + 18, cy + 15)], OUTLINE, 0.8)
...
def main(argv: List[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Render the Girdle character spritesheet.")
