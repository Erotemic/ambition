...
def _draw_kanabo(draw: ImageDraw.ImageDraw, hand: Point, angle: float, front: bool = True) -> None:
    ...
    club = [
        tr(74, -22), tr(102, -28), tr(117, -17), tr(121, 2), tr(116, 21),
        tr(97, 33), tr(74, 30), tr(84, 8), tr(70, -4),
    ]
    _poly(draw, club, STEEL_DARK, OUTLINE, 1.8)
    _poly(draw, [tr(78, -18), tr(98, -22), tr(111, -14), tr(115, 2), tr(110, 17), tr(96, 25), tr(79, 23), tr(87, 6), tr(74, -4)], STEEL, OUTLINE, 1.0)
    spike_pts = [(86, -23), (102, -22), (111, -10), (114, 5), (109, 18), (96, 27), (84, 28)]
    for sx, sy in spike_pts:
        _poly(draw, [tr(sx - 2, sy), tr(sx + 2, sy), tr(sx, sy - 8)], SPIKE, OUTLINE, 0.7)
    if front:
...
def _draw_head(draw: ImageDraw.ImageDraw, p, pose: Pose) -> None:
    P = p
    # hood / cowl back mass
    hood = [P(-27, -148), P(-18, -164), P(18, -164), P(31, -148), P(28, -121), P(15, -101), P(-15, -102), P(-29, -120)]
    _poly(draw, hood, CLOTH_DARK, OUTLINE, 1.8)
    hood_inner = [P(-19, -148), P(-11, -158), P(10, -158), P(20, -148), P(18, -125), P(8, -111), P(-7, -111), P(-18, -125)]
    _poly(draw, hood_inner, CLOTH, OUTLINE, 1.0)

    # mask / face wrap
    face = [P(-18, -142), P(-5, -150), P(11, -149), P(20, -139), P(17, -119), P(9, -108), P(-6, -108), P(-18, -118)]
    ...
    if pose.x_eyes:
        ...
        slit_y = -124
        _poly(draw, [P(-13, slit_y), P(-2, slit_y - 3), P(9, slit_y - 2), P(14, slit_y + 1), P(8, slit_y + 4), P(-4, slit_y + 4), P(-14, slit_y + 1)], EYE, OUTLINE, 0.8)
        _line(draw, [P(-10, slit_y + 2), P(10, slit_y + 2)], EYE_HI, 0.8)
        # angry eyelid angle
        _line(draw, [P(-15, slit_y - 2 - pose.eye_narrow * 3.0), P(-3, slit_y - 5)], OUTLINE, 1.0)
        _line(draw, [P(3, slit_y - 5), P(15, slit_y - 2 - pose.eye_narrow * 3.0)], OUTLINE, 1.0)
...
def _draw_torso(draw: ImageDraw.ImageDraw, p, pose: Pose) -> None:
    ...
    shadow = [P(-40, -80), P(41, -82), P(52 + pose.skirt_sway * 0.15, -14), P(0, 9), P(-51 + pose.skirt_sway * 0.15, -14)]
    _poly(draw, shadow, SMOKE, None, 0)

    shoulders = [P(-47, -103), P(-28, -122), P(30, -122), P(50, -103), P(40, -76), P(-41, -76)]
    ...
    chest = [P(-31, -104), P(-19, -116), P(18, -116), P(31, -104), P(28, -74), P(0, -62), P(-28, -74)]
...
def _draw_limbs(draw: ImageDraw.ImageDraw, p, pose: Pose) -> Tuple[Point, Point]:
    ...
    _line(draw, [(left_hand[0] - 6, left_hand[1] - 4), (left_hand[0] + 5, left_hand[1] - 4)], BRASS, 0.9)

    right_shoulder = P(43, -95)
    right_elbow = P(56 + pose.right_arm * 0.05, -64 + pose.right_arm * 0.15 + pose.club_lift * 0.2)
    right_hand = P(40 + pose.right_arm * 0.22, -41 + pose.right_arm * 0.22 + pose.club_lift)
    ...
    _line(draw, [(right_hand[0] - 6, right_hand[1] - 4), (right_hand[0] + 5, right_hand[1] - 4)], BRASS, 0.9)
...
def _render_frame(anim: str, frame_idx: int, nframes: int) -> Image.Image:
    ...
    root = (WORK_FRAME_SIZE[0] * 0.48 + pose.root_x + pose.dead_t * 7.0, WORK_FRAME_SIZE[1] * 0.68 + pose.root_y + pose.bob)
    tilt = pose.lean
    ...
    if anim == "slash":
        hand = P(40 + pose.right_arm * 0.22, -41 + pose.right_arm * 0.22 + pose.club_lift)
    ...
    if anim == "slash" and pose.impact > 0.45:
        for i, (dx, dy) in enumerate([(-50, 8), (-34, 13), (35, 9), (53, 12)]):
            ...
            c = P(dx + jitter, dy)
            _poly(draw, [(c[0] - 2.5, c[1] - 1.5), (c[0] + 3.0, c[1]), (c[0], c[1] + 3.0)], DUST, (80, 66, 50, 130), 0.5)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Render the standalone ninja-heavy spritesheet.")
