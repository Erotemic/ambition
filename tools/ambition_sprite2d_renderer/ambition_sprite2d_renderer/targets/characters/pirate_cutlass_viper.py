...
ACTOR_METADATA = {'actor': {'character_id': 'npc_pirate_cutlass_viper', 'display_name': 'Pirate Cutlass Viper'},
 'body': {'body_plan': 'HumanoidBiped',
          'body_kind': 'Standard',
          'mass_class': 'Medium',
          'traits': ['story', 'humanoid', 'enemy', 'combatant', 'pirate', 'cutlass', 'duelist'],
 ...
 'sockets': {'head': {'source': 'explicit.profile.humanoid', 'point': {'x': 64.0, 'y': 24.0}},
             ...
             'poison_edge': {'source': 'explicit.profile.pirate',
...
class SidePose:
    def __init__(self, side: int, openness: float, frame_idx: int, nframes: int, walkish: bool = False) -> None:
        cyc = math.tau * frame_idx / max(1, nframes)
        s = math.sin(cyc)
        c = math.cos(cyc)
        self.side = side
...
def _draw_cutlass(draw: ImageDraw.ImageDraw, hand: Point, angle: float, front: bool = True) -> None:
    ...
    blade = [
        tr(21, -3), tr(40, -10), tr(62, -13), tr(83, -11), tr(90, -6),
        tr(88, 2), tr(80, 9), tr(60, 14), tr(38, 16), tr(23, 12), tr(28, 6), tr(20, 2),
    ]
...
def _draw_boot_front(draw: ImageDraw.ImageDraw, p: Point, toe_dir: float) -> None:
    shaft = [(p[0] - 7, p[1] - 19), (p[0] + 7, p[1] - 19), (p[0] + 8, p[1] - 2), (p[0] - 8, p[1] - 2)]
    _poly(draw, shaft, BOOT, OUTLINE, 1.0)
    foot = [(p[0] - 9, p[1] - 3), (p[0] + 9 + toe_dir * 2.2, p[1] - 4), (p[0] + 15 + toe_dir * 3.0, p[1] + 4), (p[0] - 8, p[1] + 6)]
    _poly(draw, foot, BOOT_HI, OUTLINE, 1.0)
    _line(draw, [(p[0] - 5, p[1] - 15), (p[0] + 5, p[1] - 14)], GOLD, 0.8)


def _draw_boot_side(draw: ImageDraw.ImageDraw, x: float, y: float, side: int, scale: float = 1.0, lifted: float = 0.0) -> None:
    y -= lifted
    shaft = [
        (x - 5 * scale, y - 20 * scale), (x + 4 * scale, y - 20 * scale),
        (x + 5 * scale, y - 4 * scale), (x - 6 * scale, y - 4 * scale),
    ]
    _poly(draw, shaft, BOOT, OUTLINE, 1.0)
    foot = [
        (x - 5 * scale, y - 5 * scale), (x + 10 * side * scale, y - 5 * scale),
        (x + 17 * side * scale, y + 1 * scale), (x + 10 * side * scale, y + 6 * scale),
        (x - 6 * scale, y + 5 * scale),
    ]
    _poly(draw, foot, BOOT_HI, OUTLINE, 1.0)
    _line(draw, [(x - 3 * scale, y - 15 * scale), (x + 3 * scale, y - 15 * scale)], GOLD, 0.8)


def _draw_front_head(draw: ImageDraw.ImageDraw, P, pose: FrontPose) -> None:
    tail = [P(18, -128), P(42 + pose.hair_swing * 0.35, -118), P(34 + pose.hair_swing * 0.55, -96), P(16, -101)]
    _poly(draw, tail, HAIR, OUTLINE, 1.0)
    tail2 = [P(12, -112), P(30 + pose.hair_swing * 0.28, -94), P(18 + pose.hair_swing * 0.48, -72), P(7, -85)]
    _poly(draw, tail2, HAIR_HI, OUTLINE, 0.8)

    head = [P(-18, -137), P(-8, -150), P(10, -151), P(21, -139), P(19, -116), P(10, -101), P(-6, -101), P(-19, -114)]
    _poly(draw, head, SKIN, OUTLINE, 1.2)
    brim = [P(-33, -145), P(-12, -161), P(10, -165), P(32, -154), P(27, -138), P(10, -135), P(-7, -136), P(-28, -136)]
    _poly(draw, brim, HAT, OUTLINE, 1.5)
    _poly(draw, [P(-31, -145), P(-19, -158), P(-6, -149), P(-10, -138)], HAT_HI, OUTLINE, 0.8)
    _poly(draw, [P(13, -160), P(32, -154), P(23, -140), P(8, -142)], HAT_HI, OUTLINE, 0.8)
    ...
    if pose.x_eyes:
        ...
        if pose.blink:
            ...
            _ellipse(draw, P(-8, eye_y + 2)[0], P(-8, eye_y + 2)[1], 4, 3, (248, 244, 234, 255), OUTLINE, 0.7)
            _ellipse(draw, P(8, eye_y + 2)[0], P(8, eye_y + 2)[1], 4, 3, (248, 244, 234, 255), OUTLINE, 0.7)
        ...
        if pose.mouth > 0.18:
            _ellipse(draw, P(0, mouth_y)[0], P(0, mouth_y)[1], 6, 3.0 + pose.mouth * 2.0, (82, 34, 40, 255), OUTLINE, 0.8)
        else:
            _line(draw, [P(-6, mouth_y), P(0, mouth_y + 2), P(7, mouth_y)], OUTLINE, 0.9)
...
def _draw_front_torso(draw: ImageDraw.ImageDraw, P, pose: FrontPose) -> None:
    blouse = [P(-29, -102), P(-16, -118), P(19, -118), P(31, -103), P(26, -73), P(0, -61), P(-25, -74)]
    _poly(draw, blouse, BLOUSE, OUTLINE, 1.5)
    _ellipse(draw, P(-33, -96)[0], P(-33, -96)[1], 11, 11, BLOUSE, OUTLINE, 1.0)
    _ellipse(draw, P(33, -96)[0], P(33, -96)[1], 11, 11, BLOUSE, OUTLINE, 1.0)

    bodice = [P(-26, -103), P(-12, -115), P(14, -115), P(28, -102), P(26, -75), P(13, -63), P(0, -59), P(-15, -64), P(-26, -74)]
    _poly(draw, bodice, BODICE, OUTLINE, 1.3)
    left_bust = [P(-22, -101), P(-8, -110), P(-1, -98), P(-5, -81), P(-16, -68), P(-24, -77)]
    ...
    _line(draw, [P(-18, -80), P(-8, -72), P(0, -69), P(9, -72), P(19, -80)], (98, 52, 66, 255), 1.0)
    ...
    _poly(draw, [P(-9, -62), P(-34, -58), P(-43 + sway * 0.18, -20), P(-22, -9), P(-4, -28)], BODICE, OUTLINE, 1.0)
    _poly(draw, [P(8, -62), P(32, -58), P(42 + sway * 0.18, -20), P(22, -9), P(5, -28)], BODICE, OUTLINE, 1.0)


def _draw_front_limbs(draw: ImageDraw.ImageDraw, P, pose: FrontPose) -> Tuple[Point, Point]:
    left_hip = P(-14, -47)
    right_hip = P(16, -47)
    ...
    right_elbow = P(42 + pose.right_arm * 0.08, -67 + pose.right_arm * 0.16 + pose.blade_lift * 0.18)
    right_hand = P(26 + pose.right_arm * 0.24, -44 + pose.right_arm * 0.23 + pose.blade_lift)
...
def _render_front(anim: str, frame_idx: int, nframes: int) -> Image.Image:
    ...
    root = (WORK_FRAME_SIZE[0] * 0.47 + pose.root_x + pose.dead_t * 8.0, WORK_FRAME_SIZE[1] * 0.68 + pose.root_y + pose.bob)
    tilt = pose.lean
    ...
    if anim == "slash":
        hand = P(26 + pose.right_arm * 0.24, -44 + pose.right_arm * 0.23 + pose.blade_lift)
...
def _render_side(turn_side: int, openness: float, frame_idx: int, nframes: int, *, walkish: bool = False) -> Image.Image:
    ...
    side = pose.side
    root = (WORK_FRAME_SIZE[0] * 0.47 + pose.root_x, WORK_FRAME_SIZE[1] * 0.68 + pose.root_y + pose.bob)
    tilt = pose.lean
    ...
    back_panel = [P(-12 * side, -106), P(-1 * side, -116), P(8 * side, -111), P(5 * side, -70), P(-10 * side, -65)]
    _poly(draw, back_panel, BLOUSE, OUTLINE, 1.0)
    bodice = [
        P(-10 * side, -105), P(7 * side, -113), P(17 * side, -102),
        P(18 * side, -83), P(13 * side, -68), P(2 * side, -59),
        P(-10 * side, -65), P(-12 * side, -86),
    ]
    _poly(draw, bodice, BODICE, OUTLINE, 1.2)
    # profile bust contour
    bust = [P(3 * side, -108), P(16 * side, -98), P(20 * side, -86), P(14 * side, -71), P(4 * side, -66), P(-1 * side, -82)]
    _poly(draw, bust, BODICE_HI, OUTLINE, 0.9)
    _line(draw, [P(-1 * side, -90), P(10 * side, -79), P(16 * side, -72)], (98, 52, 66, 255), 1.0)

    sleeve_back = [P(-16 * side, -104), P(-9 * side, -112), P(0 * side, -108), P(-1 * side, -91), P(-14 * side, -90)]
    sleeve_front = [P(4 * side, -108), P(16 * side, -113), P(23 * side, -103), P(22 * side, -88), P(8 * side, -90)]
    _poly(draw, sleeve_back, BLOUSE, OUTLINE, 0.8)
    _poly(draw, sleeve_front, BLOUSE, OUTLINE, 0.8)

    belt = [P(-12 * side, -67), P(15 * side, -66), P(17 * side, -54), P(-10 * side, -54)]
    ...
    front_elbow = P(20 * side + pose.front_arm * 0.13 * side, -70 + pose.front_arm * 0.12 + pose.sword_lift * 0.3)
    front_hand = P(16 * side + pose.front_arm * 0.18 * side, -46 + pose.front_arm * 0.18 + pose.sword_lift)
    ...
    tail = [P(-7 * side, -127), P(-22 * side - pose.hair_swing * 0.2 * side, -118), P(-28 * side - pose.hair_swing * 0.35 * side, -96), P(-12 * side, -98)]
    tail2 = [P(-4 * side, -114), P(-18 * side - pose.hair_swing * 0.20 * side, -97), P(-20 * side - pose.hair_swing * 0.28 * side, -76), P(-8 * side, -85)]
    _poly(draw, tail, HAIR, OUTLINE, 1.0)
    _poly(draw, tail2, HAIR_HI, OUTLINE, 0.8)

    head = [
        P(-12 * side, -139), P(-2 * side, -151), P(10 * side, -150), P(18 * side, -141),
        P(20 * side, -130), P(15 * side, -121), P(18 * side, -114), P(13 * side, -106),
        P(2 * side, -101), P(-10 * side, -106), P(-16 * side, -118), P(-16 * side, -131),
    ]
    _poly(draw, head, SKIN, OUTLINE, 1.2)
    ...
    band = [P(-14 * side, -139), P(14 * side, -138), P(16 * side, -131), P(-15 * side, -131)]
    _poly(draw, band, BANDANA, OUTLINE, 0.8)
    ...
    if pose.mouth > 0.16:
        _ellipse(draw, P(8 * side, mouth_y)[0], P(8 * side, mouth_y)[1], 4.8, 2.8 + pose.mouth * 1.8, (82, 34, 40, 255), OUTLINE, 0.8)
    else:
...
def _render_frame(anim: str, frame_idx: int, nframes: int) -> Image.Image:
    if anim in {"turn_right", "profile_right", "turn_left", "profile_left"}:
        if anim == "turn_right":
            return _render_side(1, _ease(frame_idx / max(1, nframes - 1)), frame_idx, nframes)
        if anim == "turn_left":
            return _render_side(-1, _ease(frame_idx / max(1, nframes - 1)), frame_idx, nframes)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Render the Cutlass Viper pirate spritesheet with dedicated side/profile and turning animations.")
