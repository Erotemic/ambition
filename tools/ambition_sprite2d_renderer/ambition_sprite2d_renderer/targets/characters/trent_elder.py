...
class TrentElderGenerator:
    ...
    def pose_for_animation(self, animation: str, frame_index: int, frame_count: int) -> TrentPose:
        p = TrentPose()
    ...
    def _draw_robe(self, base: Image.Image, cx: float, shoulder_y: float, hem_y: float, spec: TrentSpec, pal: Dict[str, Color], S: float, pose: TrentPose) -> None:
        ...
        for sign in (-1, 1):
            ...
            ], fill=pal["robe_dark"], width=max(1, int(1.6 * S)))
        ...
        ], fill=pal["robe_light"], width=max(1, int(1.0 * S)))
        # Hem trim — a gold band along the floor edge of the robe.
        d.rounded_rectangle(
            (cx - spec.robe_hem_w * 0.5 * S, hem_y - 5.0 * S, cx + spec.robe_hem_w * 0.5 * S, hem_y - 1.0 * S),
            radius=2.0 * S,
            fill=pal["gold"],
            outline=outline,
        ...
        for stud_x in (-12.0, 0.0, 12.0):
            d.ellipse(_bbox((cx + stud_x * S, hem_y - 3.0 * S), 1.6 * S, 1.6 * S), fill=pal["gold_dark"], outline=outline, width=max(1, int(0.6 * S)))
        # Yoke band across the shoulders — separate from the placket so
        # the chain of office has a structured surface to sit on.
        d.rounded_rectangle(
            (cx - spec.robe_top_w * 0.50 * S, top_y - 1.0 * S, cx + spec.robe_top_w * 0.50 * S, top_y + 6.0 * S),
            radius=2.0 * S,
            fill=pal["robe_dark"],
            outline=outline,
    ...
    def _draw_arms(self, base: Image.Image, cx: float, shoulder_y: float, spec: TrentSpec, pal: Dict[str, Color], S: float, pose: TrentPose) -> None:
        d = ImageDraw.Draw(base)
        outline = pal["outline"]
        ...
        far_cuff = (cx - spec.shoulder_w * 0.36 * S, shoulder_y + spec.arm_len * S + 2.0 * S)
        far_sleeve = [
        ...
        d.rounded_rectangle(
            (far_cuff[0] - spec.cuff_w * 0.6 * S, far_cuff[1] - 1.5 * S, far_cuff[0] + spec.cuff_w * 0.6 * S, far_cuff[1] + 1.5 * S),
            radius=1.4 * S,
            fill=pal["gold_dark"],
            outline=outline,
        ...
        d.ellipse(_bbox((far_cuff[0], far_cuff[1] + 3.5 * S), 3.5 * S, 3.0 * S), fill=pal["skin"], outline=outline, width=max(1, int(0.7 * S)))
        ...
        d.rounded_rectangle(
            (near_cuff[0] - spec.cuff_w * 0.65 * S, near_cuff[1] - 1.5 * S, near_cuff[0] + spec.cuff_w * 0.65 * S, near_cuff[1] + 1.8 * S),
            radius=1.5 * S,
            fill=pal["gold"],
            outline=outline,
        ...
        d.ellipse(_bbox(hand_c, 4.0 * S, 3.4 * S), fill=pal["skin"], outline=outline, width=max(1, int(0.8 * S)))
        # The balance scales hanging from the hand (optional via pose).
        if pose.hold_scales:
            self._draw_scales(base, hand_c, pal, S)

    def _draw_scales(self, base: Image.Image, hand: Point, pal: Dict[str, Color], S: float) -> None:
        d = ImageDraw.Draw(base)
        outline = pal["outline"]
        ...
        # Finial.
        d.ellipse(_bbox(pillar_top, 2.0 * S, 2.0 * S), fill=pal["gold"], outline=outline, width=max(1, int(0.6 * S)))
        ...
        for end in (beam_a, beam_b):
            ...
            d.arc(
                (end[0] - 4.0 * S, pan_anchor[1] - 0.5 * S, end[0] + 4.0 * S, pan_anchor[1] + 5.0 * S),
                start=0,
                end=180,
                fill=outline,
                width=max(1, int(1.0 * S)),
            ...
            d.line(
                [(end[0] - 4.0 * S, pan_anchor[1] + 0.5 * S), (end[0] + 4.0 * S, pan_anchor[1] + 0.5 * S)],
                fill=pal["gold_dark"],
                width=max(1, int(0.8 * S)),
            )

    def _draw_head(self, base: Image.Image, c: Point, spec: TrentSpec, pal: Dict[str, Color], S: float, pose: TrentPose) -> None:
        d = ImageDraw.Draw(base)
        outline = pal["outline"]
        ...
        d.pieslice(
            _bbox((c[0] - 0.5 * S, c[1] + spec.head_h * 0.04 * S), (spec.head_w + spec.fringe_w * 2.0) * S, spec.head_h * 0.46 * S),
            start=180,
            end=360,
            fill=pal["beard"],
            outline=outline,
        ...
        d.ellipse(
            _bbox((c[0] + spec.head_w * 0.42 * S, c[1] + spec.head_h * 0.10 * S), 3.6 * S, 3.0 * S),
            fill=pal["beard"],
            outline=outline,
            width=max(1, int(0.8 * S)),
        )
        ...
        if pose.blink:
            d.line([(c[0] - spec.head_w * 0.20 * S, eye_y), (c[0] - spec.head_w * 0.06 * S, eye_y)],
                   fill=outline, width=max(1, int(1.1 * S)))
            d.line([(c[0] + spec.head_w * 0.06 * S, eye_y), (c[0] + spec.head_w * 0.22 * S, eye_y)],
                   fill=outline, width=max(1, int(1.1 * S)))
        else:
            for ex in (-spec.head_w * 0.14, spec.head_w * 0.14):
                d.ellipse(_bbox((c[0] + ex * S, eye_y), 2.4 * S, 1.6 * S), fill=pal["white"], outline=outline, width=max(1, int(0.7 * S)))
                d.ellipse(_bbox((c[0] + (ex + 0.2) * S, eye_y), 1.2 * S, 1.4 * S), fill=outline)
        ...
        d.polygon([
            ...
            (chin[0] - 0.5 * S, chin[1] + spec.beard_h * 0.95 * S),
        ], fill=pal["beard_shadow"], outline=None)
        ...
        for sign in (-1, 1):
            for i, (dx, dy_start, dy_end) in enumerate((
                ...
                start = (chin[0] + sign * dx * 0.6 * S, chin[1] + spec.beard_h * dy_start * S)
                end = (chin[0] + sign * (dx + 1.2) * 0.6 * S, chin[1] + spec.beard_h * dy_end * S)
                d.line([start, end], fill=pal["beard_shine"], width=max(1, int(0.7 * S)))
    ...
    def _draw_chain(self, base: Image.Image, cx: float, shoulder_y: float, spec: TrentSpec, pal: Dict[str, Color], S: float, pose: TrentPose) -> None:
        d = ImageDraw.Draw(base)
        outline = pal["outline"]
        ...
        for i in range(n_links):
            ...
            y = chain_top + y_off
            d.ellipse(_bbox((x, y), 1.6 * S, 1.6 * S), fill=pal["gold"], outline=outline, width=max(1, int(0.5 * S)))
        # Pendant medallion at the bottom of the curve.
        med_c = (cx, chain_dip + 2.0 * S)
        d.ellipse(_bbox(med_c, 4.4 * S, 4.4 * S), fill=pal["gold_dark"], outline=outline, width=max(1, int(0.9 * S)))
        ...
        d.line([(med_c[0], med_c[1] - 1.4 * S), (med_c[0], med_c[1] + 1.4 * S)], fill=pal["outline"], width=max(1, int(0.7 * S)))
        d.ellipse(_bbox((med_c[0] - 1.4 * S, med_c[1] + 0.4 * S), 0.8 * S, 0.8 * S), fill=pal["outline"])
        d.ellipse(_bbox((med_c[0] + 1.4 * S, med_c[1] + 0.4 * S), 0.8 * S, 0.8 * S), fill=pal["outline"])
