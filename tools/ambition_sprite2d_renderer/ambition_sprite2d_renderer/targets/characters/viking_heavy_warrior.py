...
ACTOR_METADATA = {'actor': {'character_id': 'npc_viking_heavy_warrior', 'display_name': 'Viking Heavy Warrior'},
...
def _draw_double_axe(draw: ImageDraw.ImageDraw, hand_a: Point, hand_b: Point, angle: float, length: float) -> Point:
    mid = ((hand_a[0] + hand_b[0]) / 2.0, (hand_a[1] + hand_b[1]) / 2.0)
    tail = (mid[0] - length * 0.46 * math.cos(math.radians(angle)), mid[1] - length * 0.46 * math.sin(math.radians(angle)))
    tip = (mid[0] + length * 0.56 * math.cos(math.radians(angle)), mid[1] + length * 0.56 * math.sin(math.radians(angle)))
    ...
    for anchor, sign in [(tip, 1), (tail, -1)]:
        ...
        p1 = (hx + 6 * math.cos(math.radians(forward)), hy + 6 * math.sin(math.radians(forward)))
        left = (hx + 10 * math.cos(math.radians(forward + 90)), hy + 10 * math.sin(math.radians(forward + 90)))
        right = (hx + 10 * math.cos(math.radians(forward - 90)), hy + 10 * math.sin(math.radians(forward - 90)))
        outer1 = (hx + 36 * math.cos(math.radians(forward + 34)), hy + 36 * math.sin(math.radians(forward + 34)))
        outer2 = (hx + 36 * math.cos(math.radians(forward - 34)), hy + 36 * math.sin(math.radians(forward - 34)))
...
def _render_frame(anim: str, idx: int, n: int) -> Image.Image:
    ...
    tunic_back = [P(-48, -180), P(44, -180), P(56, -52), P(16, -4), P(-26, -4), P(-60, -58)]
    _poly(draw, tunic_back, TUNIC_SHADE, OUTLINE, 1.0)

    torso = [P(-60, -274), P(14, -282), P(62, -246), P(74, -180), P(58, -124), P(2, -102), P(-50, -124), P(-74, -190)]
    _poly(draw, torso, TUNIC, OUTLINE, 1.2)
    belly = [P(-38, -212), P(28, -214), P(50, -176), P(40, -130), P(0, -108), P(-40, -126), P(-50, -174)]
    _poly(draw, belly, FUR, OUTLINE, 0.8)
    fur = [P(-50, -258), P(14, -276), P(58, -250), P(74, -220), P(46, -202), P(6, -212), P(-36, -198), P(-64, -222)]
    ...
    hair_back = [H(-28, -8), H(-24, -28), H(18, -28), H(28, -10), H(28, 18), H(12, 30), H(-12, 24), H(-28, 12)]
    _poly(draw, hair_back, HAIR_SHADE, OUTLINE, 0.7)
    helmet = [H(-26, -8), H(-20, -34), H(20, -34), H(28, -10), H(24, 18), H(10, 26), H(-12, 26), H(-28, 10)]
    _poly(draw, helmet, STEEL, OUTLINE, 0.9)
    nose_guard = [H(2, -10), H(10, -2), H(6, 18), H(-2, 12)]
    _poly(draw, nose_guard, STEEL_SHADE, OUTLINE, 0.35)
    face = [H(-22, -2), H(-16, -20), H(12, -20), H(22, -2), H(18, 16), H(8, 24), H(-8, 24), H(-22, 12)]
    ...
    beard = [H(-18, 14), H(-2, 28 + pose.beard * 0.1), H(16, 22 + pose.beard * 0.08), H(22, 44 + pose.beard * 0.1), H(6, 64 + pose.beard * 0.1), H(-10, 58 + pose.beard * 0.1), H(-22, 34)]
    ...
    if pose.x_eye:
        ...
        _line(draw, [H(-12, 1), H(-4, 1)], OUTLINE, 0.7); _line(draw, [H(6, 1), H(14, 1)], OUTLINE, 0.7)
    else:
    ...
    if pose.mouth > 0.03:
        _ellipse(draw, H(2, 20)[0], H(2, 20)[1], 6.0, 3.2 + pose.mouth * 12.0, MOUTH, OUTLINE, 0.35)
        if pose.mouth > 0.14:
    ...
    tunic_front = [P(-52, -176), P(46, -176), P(56, -48), P(18, 0), P(-28, -2), P(-62, -54)]
    ...
    axe_tip = _draw_double_axe(draw, near_hand, far_hand, pose.weapon_angle, 122 + pose.weapon_len)
    for hand in [near_hand, far_hand]:
        _circle(draw, hand, 4.6, SKIN if hand == near_hand else SKIN_SHADE, OUTLINE, 0.3)
        _line(draw, [(hand[0] - 5, hand[1] - 2), (hand[0] + 5, hand[1] + 2)], LEATHER_DARK, 0.45)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Render the standalone opera-style heavy Viking warrior sprite sheet.")
