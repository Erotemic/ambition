...
def draw_shrine(d: ImageDraw.ImageDraw, s: float) -> None:
    ...
    def rect(x0, y0, x1, y1, fill):
        d.rectangle(
            (x0 * s, y0 * s, x1 * s, y1 * s), fill=fill, outline=outline, width=w
        )
    ...
    d.polygon(
        scaled([(33, 121), (55, 121), (50, 46), (38, 46)], s),
        fill=rgba(_STONE_MID),
        outline=outline,
        width=w,
    )
    d.line(scaled([(38, 46), (33, 121)], s), fill=rgba(_STONE_LIGHT), width=w)
    d.polygon(
        scaled([(38, 47), (50, 47), (44, 28)], s),
        fill=rgba(_STONE_LIGHT),
        outline=outline,
        width=w,
    )
    ...
    rx, ry = 44, 102
    d.line(
        scaled([(rx - 6, ry), (rx + 6, ry)], s),
        fill=with_alpha(accent, 220),
        width=max(1, int(2.4 * s)),
    )
    d.line(
        scaled([(rx, ry - 6), (rx, ry + 6)], s),
        fill=with_alpha(accent, 220),
        width=max(1, int(2.4 * s)),
    )
    ...
    d.polygon(
        scaled([(44, 60), (53, 74), (44, 90), (35, 74)], s),
        fill=with_alpha(accent, 205),
        outline=outline,
        width=w,
    )
    d.polygon(
        scaled([(44, 65), (49, 74), (44, 84), (39, 74)], s), fill=rgba("#FFFFFF", 210)
    )
    d.line(
        scaled([(44, 60), (44, 90)], s),
        fill=rgba("#FFFFFF", 175),
        width=max(1, int(1.4 * s)),
    )
...
def render_shrine(
    size: Tuple[int, int] = (88, 160), supersample: int = 4
) -> Image.Image:
    s = max(1, int(supersample))
    img = Image.new("RGBA", (size[0] * s, size[1] * s), (0, 0, 0, 0))
...
def write_shrine_prop(
    out_dir: str | Path, *, size: Tuple[int, int] = (88, 160)
) -> Path:
