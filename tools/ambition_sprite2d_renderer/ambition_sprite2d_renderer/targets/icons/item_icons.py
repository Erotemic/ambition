...
# Drawn on a logical canvas sized to the in-game footprint so the runtime can
# scale them to the collision box without distortion.

# Stone + healing-glow palette for the heal/save shrine. The accent matches the
# old ECS placeholder tint srgba(0.55,0.95,0.85) so the swap reads as continuous.
SHRINE_ACCENT = "#8CF2D9"
_STONE_DARK = "#2A3142"
_STONE_MID = "#3C465C"
_STONE_LIGHT = "#586480"


def draw_shrine(d: ImageDraw.ImageDraw, s: float) -> None:
    """A heal / save shrine: a tapered stone obelisk on a stepped base, a glowing
    healing crystal set into the shaft, a heal-cross rune, and an apex spark --
    a "rest here" landmark. Logical canvas is 88x160 (11:20 = the in-game 44x80
    footprint); base sits at the bottom (the floor), apex near the top. `s` is the
    supersample scale."""
    accent = rgba(SHRINE_ACCENT)
    outline = rgba("#05070D")
    w = max(1, int(2 * s))

    def rect(x0, y0, x1, y1, fill):
        d.rectangle((x0 * s, y0 * s, x1 * s, y1 * s), fill=fill, outline=outline, width=w)

    # Ambient aura behind the shrine + a ground-contact shadow.
    for r, a in [(48, 22), (36, 30), (26, 40)]:
        d.ellipse(bbox(44 * s, 78 * s, r * s, r * 1.5 * s), fill=with_alpha(accent, a))
    d.ellipse(bbox(44 * s, 153 * s, 62 * s, 12 * s), fill=(0, 0, 0, 70))

    # Stepped stone base (widest at the bottom).
    rect(14, 142, 74, 157, rgba(_STONE_DARK))
    rect(20, 130, 68, 144, rgba(_STONE_MID))
    rect(26, 119, 62, 132, rgba(_STONE_LIGHT))

    # Tapered obelisk shaft with a lit left edge, and a pyramidal cap.
    d.polygon(scaled([(33, 121), (55, 121), (50, 46), (38, 46)], s), fill=rgba(_STONE_MID), outline=outline, width=w)
    d.line(scaled([(38, 46), (33, 121)], s), fill=rgba(_STONE_LIGHT), width=w)
    d.polygon(scaled([(38, 47), (50, 47), (44, 28)], s), fill=rgba(_STONE_LIGHT), outline=outline, width=w)

    # Heal-cross rune glowing on the shaft below the crystal.
    rx, ry = 44, 102
    d.line(scaled([(rx - 6, ry), (rx + 6, ry)], s), fill=with_alpha(accent, 220), width=max(1, int(2.4 * s)))
    d.line(scaled([(rx, ry - 6), (rx, ry + 6)], s), fill=with_alpha(accent, 220), width=max(1, int(2.4 * s)))

    # Healing crystal set into the shaft, with a glow halo and a white core.
    for r, a in [(20, 70), (14, 120)]:
        d.ellipse(bbox(44 * s, 74 * s, r * s, r * s), fill=with_alpha(accent, a))
    d.polygon(scaled([(44, 60), (53, 74), (44, 90), (35, 74)], s), fill=with_alpha(accent, 205), outline=outline, width=w)
    d.polygon(scaled([(44, 65), (49, 74), (44, 84), (39, 74)], s), fill=rgba("#FFFFFF", 210))
    d.line(scaled([(44, 60), (44, 90)], s), fill=rgba("#FFFFFF", 175), width=max(1, int(1.4 * s)))

    # Apex spark crowning the cap.
    for r, a in [(9, 95), (4, 180)]:
        d.ellipse(bbox(44 * s, 25 * s, r * s, r * s), fill=with_alpha(accent, a))
    d.ellipse(bbox(44 * s, 25 * s, 3 * s, 3 * s), fill=rgba("#FFFFFF", 235))


def render_shrine(size: Tuple[int, int] = (88, 160), supersample: int = 4) -> Image.Image:
    s = max(1, int(supersample))
    img = Image.new("RGBA", (size[0] * s, size[1] * s), (0, 0, 0, 0))
    draw_shrine(ImageDraw.Draw(img), float(s))
    return img.resize(size, RESAMPLING.LANCZOS)


def write_shrine_prop(out_dir: str | Path, *, size: Tuple[int, int] = (88, 160)) -> Path:
    """Render the heal/save shrine world prop into ``out_dir`` (the sandbox
    ``sprites/props/`` dir) as ``shrine.png``. Consumed at runtime by
    ``shrine::sync_shrine_visual`` -- the obelisk that replaced the flat
    placeholder rectangle. Tall 11:20 aspect == the in-game 44x80 footprint, so
    the runtime ``custom_size`` scales it without distortion."""
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    path = out_dir / "shrine.png"
    render_shrine(size).save(path)
    return path
