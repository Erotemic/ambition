...
DEFAULT_SCENE = HERE / "mockingbird_boss_scene.yaml"
OUTPUT_FILES = [
...
class Scene:
    def __init__(self, data):
        ...
        self.animations = data.get("animations", {"hover": {"frames": 1, "duration_ms": 100}})
...
class Renderer:
    def __init__(self, scene: Scene, frame_size=None, aa_scale=None, origin=None):
        ...
        if frame_size is None:
            frame_size = tuple(scene.render_cfg.get("work_size", scene.meta.get("work_size", [900, 640])))
        self.frame_size = tuple(frame_size)
        self.aa = int(aa_scale if aa_scale is not None else scene.render_cfg.get("supersample", 2))
        self.origin = tuple(origin if origin is not None else scene.meta.get("origin", [self.frame_size[0]/2, self.frame_size[1]/2]))
    ...
    def update_bounds(self, node_id, pts):
        ...
        if old is None:
            ...
            self.bounds_by_id[node_id] = (min(old[0], box[0]), min(old[1], box[1]), max(old[2], box[2]), max(old[3], box[3]))
    ...
    def draw_polygon(self, draw, pts, fill, outline=None, width=1):
        ...
        if outline is not None:
            draw.line([self.P(p) for p in pts + [pts[0]]], fill=outline, width=max(1, self.S(width)), joint="curve")

    def draw_line(self, draw, pts, fill, width=1):
        if not pts:
            return
        draw.line([self.P(p) for p in pts], fill=fill, width=max(1, self.S(width)), joint="curve")
    ...
    def draw_special(self, draw, node, prim, M, anim_name, phase):
        ...
        if typ == "flame":
            ...
            bend = float(prim.get("bend", -8)); segs = int(prim.get("segments", 7))
            centers = []
            for i in range(segs):
                t = i / max(1, segs - 1)
                centers.append((lerp(start[0], end[0], t), lerp(start[1], end[1], t) + math.sin(t * math.pi) * bend))
            ...
            start = prim.get("start", [0,0]); end = prim.get("end", [20,0])
            count = int(prim.get("count", 6)); direction = prim.get("direction", "down")
            length = float(prim.get("length", 7)); width = float(prim.get("tooth_width", 3.2))
            lean = float(prim.get("lean", -1.2)); sign = 1 if direction == "down" else -1
            for i in range(count):
                t = i / max(1, count - 1)
                x = lerp(start[0], end[0], t); y = lerp(start[1], end[1], t)
                tri = [(x, y), (x + lean, y + sign * length), (x + width, y + sign * length * 0.78)]
                w = [mat_apply(M, p) for p in tri]
                self.draw_polygon(draw, w, self.scene.color(prim.get("fill", "tooth")), None)
                pts_for_bounds += w
        elif typ == "teeth":
            # compatibility: draw old combined teeth as two rows
            for row in [
                {"start": prim.get("upper_start", [0,0]), "end": prim.get("upper_end", [20,0]), "count": prim.get("count", 6), "direction": "down", "length": 7, "tooth_width": 3.2, "lean": -2.4, "fill": "tooth"},
                {"start": prim.get("lower_start", [0,10]), "end": prim.get("lower_end", [20,10]), "count": max(1, int(prim.get("count", 6)) - 2), "direction": "up", "length": 6.4, "tooth_width": 3.0, "lean": -2.0, "fill": "tooth"},
            ]:
                pts_for_bounds += self.draw_special(draw, node, {"type": "teeth_row", **row}, M, anim_name, phase)
        elif typ == "wing":
            ...
            main_wing = [(2, 0), (-16, -10), (-46, -15), (-78, -9), (-70, -1), (-38, 5), (-10, 9)]
            ...
            if prim.get("cannon", True):
                # pylon
                pylon = [(-24, 4), (-38, 9)]
                self.draw_line(draw, [mat_apply(M, pylon[0]), mat_apply(M, pylon[1])], self.scene.color("steel2"), 2)
                pts_for_bounds += [mat_apply(M, p) for p in pylon]
                # cannon body and muzzle pod
                for box, fill in [((-84, 9, -42, 15), "dark2"), ((-44, 8, -30, 16), "steel2"), ((-94, 8, -82, 16), "steel")]:
                    pts = ellipse_points(box, 20) if (box[2] - box[0]) <= 16 else rect_points(box)
                    w = [mat_apply(M, p) for p in pts]
                    self.draw_polygon(draw, w, self.scene.color(fill), outline if fill != "steel" else None, 1)
                    pts_for_bounds += w
                # muzzle barrel
                barrel = [(-102, 11), (-84, 11)]
                self.draw_line(draw, [mat_apply(M, barrel[0]), mat_apply(M, barrel[1])], self.scene.color("outline"), 2)
                pts_for_bounds += [mat_apply(M, p) for p in barrel]
        elif typ == "leg":
            hip = tuple(prim.get("hip", [0,0])); knee = tuple(prim.get("knee", [0,20])); foot = tuple(prim.get("foot", [0,40]))
            ...
            for tip in [(foot[0]-7, foot[1]+2), (foot[0]-8, foot[1]-2), (foot[0]+2, foot[1]+4)]:
                self.draw_line(draw, [mat_apply(M, foot), mat_apply(M, tip)], self.scene.color("outline"), 2)
            pts_for_bounds += w
        elif typ == "claw_arm":
            anchor = tuple(prim.get("anchor", [0,0])); elbow = tuple(prim.get("elbow", [-10,10])); wrist = tuple(prim.get("wrist", [-20,20]))
            ...
            for tip in [(wrist[0]-9, wrist[1]-spread), (wrist[0]-12, wrist[1]), (wrist[0]-9, wrist[1]+spread)]:
                self.draw_line(draw, [mat_apply(M, wrist), mat_apply(M, tip)], self.scene.color("outline"), 2)
            pts_for_bounds += w
        elif typ == "rotor":
            ...
            LOG.warning("unsupported special primitive type=%s node=%s", typ, node.get("id"))
    ...
    def draw_shape(self, draw, node, M, anim_name, phase):
        ...
        typ = prim.get("type")
        pts_for_bounds = []
        fill = self.scene.color(prim.get("fill")) if prim.get("fill") is not None else None
        outline = self.scene.color(prim.get("outline")) if prim.get("outline") is not None else None
    ...
    def render_node(self, draw, node, parent_M, anim_name, phase):
        ...
        if node.get("kind") == "shape":
            ...
            for child in sorted(children, key=lambda n: (int(n.get("z_order", 0)), str(n.get("id", "")))):
    ...
    def render(self, anim_name="hover", frame_index=0, nframes=1, debug=False):
        ...
        LOG.debug("render frame anim=%s frame=%d/%d size=%s aa=%s", anim_name, frame_index, nframes, self.frame_size, self.aa)
        self.render_node(draw, self.scene.root, [[1,0,0],[0,1,0],[0,0,1]], anim_name, phase)
...
def render_work(scene, anim="hover", frame=0, nframes=1, debug=False):
    work = tuple(scene.render_cfg.get("work_size", scene.meta.get("work_size", [900,640])))
    origin = tuple(scene.meta.get("origin", [work[0]/2, work[1]/2]))
    return Renderer(scene, work, aa_scale=scene.render_cfg.get("supersample", 2), origin=origin).render(anim, frame, nframes, debug=debug)
...
def build_sheet(scene, frame_size=(256,256), labeled=False):
    ...
    for r, (name, info) in enumerate(rows):
        nframes = int(info.get("frames",1)); dur = int(info.get("duration_ms",100))
        LOG.info("render row %s (%d frames)", name, nframes)
        if labeled:
            d.rounded_rectangle((8, r*fh+10, label_w-10, r*fh+fh-10), radius=8, fill=(32,38,48,220))
            d.text((16, r*fh+16), name, fill=(244,246,250,255), font=font(16))
            d.text((16, r*fh+38), f"{nframes}f @ {dur}ms", fill=(176,184,196,255), font=font(12))
        rects = []
        ...
        meta_rows.append({"row_index": r, "animation": name, "frames": nframes, "duration_ms": dur, "rects": rects})
...
def make_parts_debug(scene, bg_color=None):
    ...
    for node in walk(scene.root):
        ...
        sub["root"]["transform"] = {"x": 0, "y": 0, "rotation": 0, "scale_x": 1, "scale_y": 1}
        simg = render_work(Scene(sub), "hover", 1, 6)
        entries.append((node["id"], crop_alpha(simg, 8)))
    cols=3; cell_w=260; cell_h=190
    rows=math.ceil(len(entries)/cols)
    bg=Image.new("RGBA",(cols*cell_w,rows*cell_h), tuple(bg_color or scene.background_rgba()))
    d=ImageDraw.Draw(bg,"RGBA")
    for i,(name,img) in enumerate(entries):
        col,row=i%cols,i//cols; x0=col*cell_w; y0=row*cell_h
        d.rounded_rectangle((x0+8,y0+8,x0+cell_w-8,y0+cell_h-8), radius=10, fill=(247,249,252,255), outline=(190,198,208,255), width=1)
        ...
        obj=img.resize((max(1,int(img.width*scale)), max(1,int(img.height*scale))), Image.Resampling.LANCZOS)
        bg.alpha_composite(obj,(x0+(cell_w-obj.width)//2, y0+52+(max_h-obj.height)//2))
    return bg


def render_outputs(scene_path=DEFAULT_SCENE, outdir=None, frame_size=None, quick=False, force=False):
    with Timer(f"load scene {scene_path}"):
        scene = Scene.load(scene_path)
    outdir = Path(outdir) if outdir else HERE / "generated" / TARGET_NAME
    ...
    with Timer("parts debug"):
        ...
        dbg.save(outdir / f"{TARGET_NAME}_parts_debug.png"); outputs.append(outdir / f"{TARGET_NAME}_parts_debug.png")
    if not quick:
        with Timer("spritesheet"):
            sheet, rows = build_sheet(scene, frame_size, labeled=False)
            sheet.save(outdir / f"{TARGET_NAME}_spritesheet.png"); outputs.append(outdir / f"{TARGET_NAME}_spritesheet.png")
    ...
    lines += ["", "Notes:", "- This generator is nested scene-graph / vector-geometry driven.", "- Shape and group transforms are applied before rasterization.", "- The global post-composite outline is disabled by default; shapes own their outlines locally.", ""]
    (outdir / "sources_and_inspirations.md").write_text("\n".join(lines)); outputs.append(outdir / "sources_and_inspirations.md")
    return outputs


def install_outputs(render_dir=None, install_dir=None):
    src_dir = Path(render_dir) if render_dir else HERE / "generated" / TARGET_NAME
    repo_root = HERE.parents[1]
    dst_dir = Path(install_dir) if install_dir else repo_root / "crates" / "ambition_sandbox" / "assets" / "sprites" / TARGET_NAME
...
def cli(argv=None):
    p=argparse.ArgumentParser(description="Render/install nested scene-graph mockingbird boss sprites")
    ...
    q=sub.add_parser("install"); q.add_argument("--render-dir", default=None); q.add_argument("--install-dir", default=None)
    q=sub.add_parser("render-publish")
    q.add_argument("--scene", "--rig", default=str(DEFAULT_SCENE)); q.add_argument("--outdir", default=None); q.add_argument("--install-dir", default=None)
    q.add_argument("--frame-width", type=int, default=None); q.add_argument("--frame-height", type=int, default=None)
    q.add_argument("--quick", action="store_true"); q.add_argument("--force", action="store_true")
    args=p.parse_args(argv)
    logging.basicConfig(level=getattr(logging, args.log_level), format="[%(levelname)s] %(message)s")
    if args.cmd in {"render","preview"}:
        fs=None if args.frame_width is None else (args.frame_width, args.frame_height or args.frame_width)
        outs=render_outputs(args.scene, args.outdir, fs, args.quick, args.force)
        print("Rendered:"); [print(f"  {o}") for o in outs]
    elif args.cmd=="install":
        outs=install_outputs(args.render_dir,args.install_dir); print("Installed:"); [print(f"  {o}") for o in outs]
    elif args.cmd=="render-publish":
        fs=None if args.frame_width is None else (args.frame_width, args.frame_height or args.frame_width)
        render_outputs(args.scene,args.outdir,fs,args.quick,args.force)
        outs=install_outputs(args.outdir,args.install_dir); print("Installed:"); [print(f"  {o}") for o in outs]
    return 0
