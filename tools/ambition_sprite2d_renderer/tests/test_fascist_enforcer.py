...
def test_fascist_enforcer_canonical_smoke():
    job = CharacterJob.load(CONFIGS / 'fascist_enforcer.yaml')
    image = render_canonical(job)
    assert image.size == (job.render.single_width, job.render.single_height)


def test_fascist_enforcer_sheet_smoke(tmp_path: Path):
    job = CharacterJob.load(CONFIGS / 'fascist_enforcer.yaml')
    ...
    image_path, manifest_path = write_spritesheet(
        job, tmp_path / 'fascist_enforcer.png', tmp_path / 'fascist_enforcer.yaml'
    )
    ...
    red_pixels = sum(1 for r, g, b, a in pixels if a > 0 and r >= 140 and g <= 90 and b <= 90)
    dark_pixels = sum(1 for r, g, b, a in pixels if a > 0 and r <= 70 and g <= 75 and b <= 85)
