...
def parse_color(value: str):
    return (
        ...
        else ImageColor.getcolor(value, "RGBA")
    )
...
def rounded_rect(
    draw: ImageDraw.ImageDraw, box, radius: int, fill, outline, width: int = 1
):
    try:
        ...
        for row, anim in enumerate(animations):
            ...
            if label_w:
                ...
                rounded_rect(
                    ...
                    radius=max(6, int(chip_h * 0.25)),
                    fill=(232, 235, 241, 220),
                    outline=(70, 76, 88, 230),
                    width=1,
                )
                draw.text(
                    (
                        chip_x0 + (chip_w - tw) / 2 - bbox[0],
                        chip_y0 + (chip_h - th) / 2 - bbox[1],
                    ),
                    label,
                    font=font,
                    fill=(20, 22, 28, 255),
                ...
                draw.line(
                    [(label_w - 1, row_y + 6), (label_w - 1, row_y + row_h - 6)],
                    fill=(128, 132, 144, 210),
                    width=max(1, int(fh * 0.01)),
                )
            frame_count = anim_defs[anim]["frames"]
