...
def _fake_render_requests(
    adapter, job, requests, mode="frames", blender_executable=None
):
    for req in requests:
...
def test_goblin_sheet_smoke(tmp_path: Path, monkeypatch):
    monkeypatch.setattr(
        "gen3d_blender_lab.sheet.render_requests", _fake_render_requests
    )
    job = CharacterJob(
        target="goblin",
        seed=1,
        archetype="shadow_scout",
        held_item="spear",
        animations=["idle"],
    )
...
def test_robot_sheet_smoke(tmp_path: Path, monkeypatch):
    ...
    job = CharacterJob(
        target="robot", seed=7, archetype="scout_bot", animations=["idle"]
    )
