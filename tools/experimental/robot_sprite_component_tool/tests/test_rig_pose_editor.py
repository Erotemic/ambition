...
def test_pose_overrides_apply_frame_specific_art_and_zorder(tmp_path):
    ...
    asm = rig.RobotAssembler(
        atlas, job.render, pose_overrides=rig.load_pose_overrides(pp)
    )
    ...
    assert (
        abs(front_hand["angle"] - 17) > 0
    )  # includes hand-follow contribution but is frame-specific
...
def test_pose_editor_headless_preview_and_instance_report(tmp_path):
    ...
    img, manifest = editor.build_preview(
        paths.job,
        editor.load_yaml(paths.metadata),
        editor.load_yaml(paths.pose_overrides),
        animations=["run"],
    )
...
def test_relevant_animation_filter_finds_run_for_run_torso():
    paths = editor.resolve_job_paths(JOB, META, SLICES, POSE)
    anims = editor.relevant_animations(
        paths.job,
        editor.load_yaml(paths.metadata),
        editor.load_yaml(paths.pose_overrides),
        "torso_lean_forward",
...
def test_sparse_pose_keyframes_interpolate_between_first_and_last(tmp_path):
    ...
    front_arm = [c for c in manifest["pose"]["components"] if c["role"] == "front_arm"][
        0
    ]
...
def test_manifest_exposes_rig_constraints_and_snap_errors():
    ...
    atlas = rig.ComponentAtlas(META, SLICES)
    asm = rig.RobotAssembler(
        atlas, job.render, pose_overrides=rig.load_pose_overrides(POSE)
    )
    _img, manifest = asm.render_frame("run", 0)
    ...
    assert by_role["front_arm"]["child"] == {
        "sprite": "arm_capsule_vertical",
        "anchor": "shoulder",
    }
