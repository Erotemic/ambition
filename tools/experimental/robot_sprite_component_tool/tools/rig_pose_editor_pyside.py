...
def render_preview(paths: core.EditorPaths, output: Path, *, animations: Optional[List[str]] = None, debug: bool = False, background: str = "black") -> None:
    meta = core.load_yaml(paths.metadata)
    pose = core.load_yaml(paths.pose_overrides)
    img, _manifest = core.build_preview(paths.job, meta, pose, animations=animations, debug=debug, bg=background)
...
def pillow_to_qimage(img: Image.Image):
    ...
    qimg = QtGui.QImage(data, img.width, img.height, img.width * 4, QtGui.QImage.Format.Format_RGBA8888)
...
if _QtWidgets is not None:
    class PreviewGraphicsView(_QtWidgets.QGraphicsView):
        ...
        * image refreshes can preserve the current zoom/pan state instead of
        ...
        def __init__(self, parent=None):
            ...
            self.setRenderHints(_QtGui.QPainter.RenderHint.Antialiasing | _QtGui.QPainter.RenderHint.SmoothPixmapTransform)
            self.setDragMode(_QtWidgets.QGraphicsView.DragMode.ScrollHandDrag)
            self.setTransformationAnchor(_QtWidgets.QGraphicsView.ViewportAnchor.AnchorUnderMouse)
            self.setResizeAnchor(_QtWidgets.QGraphicsView.ViewportAnchor.AnchorViewCenter)
            self.setMouseTracking(True)

        def set_pillow_image(self, img: Image.Image, *, fit: bool = False, preserve_view: bool = True):
            ...
            if fit:
                self.resetTransform()
                self.fitInView(self._scene.sceneRect(), _QtCore.Qt.AspectRatioMode.KeepAspectRatio)
                self._has_user_zoom = False
            elif preserve_view:
        ...
        def mouseReleaseEvent(self, event):  # type: ignore[override]
            if self._scene_dragging and event.button() == _QtCore.Qt.MouseButton.LeftButton:
                x, y = self._scene_point_from_event(event)
    ...
    class AnchorGraphicsView(PreviewGraphicsView):
        ...
        def mouseReleaseEvent(self, event):  # type: ignore[override]
            if self._dragging_anchor and event.button() == _QtCore.Qt.MouseButton.LeftButton:
                x, y = self._scene_xy(event)
                if self._in_bounds(x, y):
...
class RigPoseEditorQt:
    ...
    def __init__(self, paths: core.EditorPaths, *, zoom: int = 6, background: str = "checker"):
    ...
    def _build_ui(self) -> None:
        ...
        self.live_sheet_preview.setToolTip("When off, edits update only the action preview until Render now is pressed.")
        ...
        self.fast_drag_preview.setToolTip("During drag, skip full-sheet/tree refresh and update only the action-preview canvas.")
        ...
        self._dock_widgets = []
        self.animation_dock = self._add_dock("Animation / Rig", self.left_panel, QtCore.Qt.DockWidgetArea.LeftDockWidgetArea)
        self.component_dock = self._add_dock("Components / Anchors", self.center_panel, QtCore.Qt.DockWidgetArea.RightDockWidgetArea)
    ...
    def _make_animation_panel(self):
        ...
        self.pose_frame = QtWidgets.QSpinBox(); self.pose_frame.setRange(0, 999)
        self.pose_role = QtWidgets.QLineEdit()
        self.pose_art = QtWidgets.QComboBox(); self.pose_art.setEditable(True); self.pose_art.addItems(self.sprite_names)
        self.pose_angle = QtWidgets.QDoubleSpinBox(); self.pose_angle.setRange(-360, 360); self.pose_angle.setDecimals(2)
        self.pose_scale = QtWidgets.QDoubleSpinBox(); self.pose_scale.setRange(0.05, 3.0); self.pose_scale.setDecimals(3); self.pose_scale.setSingleStep(0.025)
        self.pose_scale.setToolTip("Baseline part scale multiplier. Arms/legs may still endpoint-solve to a slightly different effective scale so their wrist/ground joint lands on target.")
        self.pose_dx = QtWidgets.QDoubleSpinBox(); self.pose_dx.setRange(-300, 300); self.pose_dx.setDecimals(2)
        self.pose_dy = QtWidgets.QDoubleSpinBox(); self.pose_dy.setRange(-300, 300); self.pose_dy.setDecimals(2)
        grid.addWidget(QtWidgets.QLabel("anim"), 0, 0); grid.addWidget(self.pose_anim, 0, 1)
        grid.addWidget(QtWidgets.QLabel("frame"), 0, 2); grid.addWidget(self.pose_frame, 0, 3)
        grid.addWidget(QtWidgets.QLabel("role"), 1, 0); grid.addWidget(self.pose_role, 1, 1)
        grid.addWidget(QtWidgets.QLabel("art"), 1, 2); grid.addWidget(self.pose_art, 1, 3)
        grid.addWidget(QtWidgets.QLabel("angle"), 2, 0); grid.addWidget(self.pose_angle, 2, 1)
        grid.addWidget(QtWidgets.QLabel("scale"), 2, 2); grid.addWidget(self.pose_scale, 2, 3)
        grid.addWidget(QtWidgets.QLabel("delta x"), 3, 0); grid.addWidget(self.pose_dx, 3, 1)
        grid.addWidget(QtWidgets.QLabel("delta y"), 3, 2); grid.addWidget(self.pose_dy, 3, 3)
        ...
        self.connection_table.setHorizontalHeaderLabels(["role", "joint", "child anchor", "parent socket", "snap px", "visible"])
        self.connection_table.setSelectionBehavior(QtWidgets.QAbstractItemView.SelectionBehavior.SelectRows)
        self.connection_table.setEditTriggers(QtWidgets.QAbstractItemView.EditTrigger.NoEditTriggers)
        ...
        self.anim_frame_count = QtWidgets.QSpinBox(); self.anim_frame_count.setRange(1, 128)
        self.anim_duration_ms = QtWidgets.QSpinBox(); self.anim_duration_ms.setRange(10, 1000)
        self.add_frame_button = QtWidgets.QPushButton("Add frame")
        self.remove_frame_button = QtWidgets.QPushButton("Remove last")
        tgrid.addWidget(QtWidgets.QLabel("frames"), 0, 0); tgrid.addWidget(self.anim_frame_count, 0, 1)
        tgrid.addWidget(QtWidgets.QLabel("ms/frame"), 0, 2); tgrid.addWidget(self.anim_duration_ms, 0, 3)
        ...
        zbuttons.addWidget(self.z_up); zbuttons.addWidget(self.z_down); zbuttons.addWidget(self.z_reset)
        zbuttons.addWidget(self.z_show_all); zbuttons.addWidget(self.z_only_selected); zbuttons.addStretch(1)
    ...
    def _make_component_panel(self):
        ...
        self.anchor_x = QtWidgets.QDoubleSpinBox(); self.anchor_x.setRange(-1000, 1000); self.anchor_x.setDecimals(2); self.anchor_x.setSingleStep(0.25)
        self.anchor_y = QtWidgets.QDoubleSpinBox(); self.anchor_y.setRange(-1000, 1000); self.anchor_y.setDecimals(2); self.anchor_y.setSingleStep(0.25)
        ...
        xy.addWidget(QtWidgets.QLabel("x"), 0, 0); xy.addWidget(self.anchor_x, 0, 1)
        xy.addWidget(QtWidgets.QLabel("y"), 0, 2); xy.addWidget(self.anchor_y, 0, 3)
        xy.addWidget(self.snap_anchor_pixels, 1, 0, 1, 4)
        xy.addWidget(self.apply_anchor, 2, 0); xy.addWidget(self.pivot_follow, 2, 1, 1, 3)
        xy.addWidget(self.anchor_zoom_out, 3, 0); xy.addWidget(self.anchor_zoom_fit, 3, 1, 1, 2); xy.addWidget(self.anchor_zoom_in, 3, 3)
        alayout.addLayout(xy)
        hint = QtWidgets.QLabel("Direct manipulation: mouse wheel = zoom, hand-drag = pan, left click/drag = move selected anchor.")
    ...
    def _make_preview_panel(self):
        ...
        preview_layout.addWidget(QtWidgets.QLabel("Live spritesheet preview (current unsaved state)"))
        ...
        anim_layout.addWidget(QtWidgets.QLabel("Animated action preview — drag parts to move endpoints/offsets; Shift-drag rotates"))
    ...
    def _connect_signals(self) -> None:
        ...
        self.preview_relevant.stateChanged.connect(lambda _=None: self.refresh_preview())
        self.preview_debug.stateChanged.connect(lambda _=None: self.refresh_preview())
        self.preview_fit.stateChanged.connect(lambda _=None: self.refresh_preview(force=True))
        self.live_sheet_preview.stateChanged.connect(lambda _=None: self.refresh_preview(force=True))
        self.show_bones.stateChanged.connect(lambda _=None: self.render_animation_frame())
        self.preview_bg.currentTextChanged.connect(lambda _=None: self.refresh_preview())
        ...
        self.sprite_filter.textChanged.connect(lambda _=None: self.populate_sprite_list())
        ...
        self.anchor_zoom_out.clicked.connect(lambda: self.component_view.zoom_by(1.0 / 1.15))
        self.anchor_zoom_fit.clicked.connect(lambda: self.component_view.fit_scene())
        self.apply_pose_button.clicked.connect(lambda _=None: self.apply_pose_edit(refresh=True, changed_fields=None))
        ...
        self.pose_frame.valueChanged.connect(lambda _=None: self.on_pose_frame_changed())
        ...
        self.connection_table.itemDoubleClicked.connect(lambda _=None: self.select_connection_child_part())
        self.select_child_part_button.clicked.connect(self.select_connection_child_part)
        self.select_parent_part_button.clicked.connect(self.select_connection_parent_part)
        self.select_child_anchor_button.clicked.connect(self.select_connection_child_anchor)
        self.select_parent_anchor_button.clicked.connect(self.select_connection_parent_anchor)
        self.pose_art.currentTextChanged.connect(lambda _=None: self.defer_pose_edit({"sprite"}))
        self.pose_angle.valueChanged.connect(lambda _=None: self.defer_pose_edit({"angle"}))
        self.pose_scale.valueChanged.connect(lambda _=None: self.defer_pose_edit({"scale"}))
        self.pose_dx.valueChanged.connect(lambda _=None: self.defer_pose_edit({"delta"}))
        self.pose_dy.valueChanged.connect(lambda _=None: self.defer_pose_edit({"delta"}))
        self.anim_frame_count.valueChanged.connect(lambda _=None: self.apply_anim_settings())
        self.anim_duration_ms.valueChanged.connect(lambda _=None: self.apply_anim_settings())
        ...
        self.window.shortcut_save = self.QtGui.QShortcut(self.QtGui.QKeySequence("Ctrl+S"), self.window)
        self.window.shortcut_save.activated.connect(self.save)
        self.window.shortcut_refresh = self.QtGui.QShortcut(self.QtGui.QKeySequence("Ctrl+R"), self.window)
        self.window.shortcut_refresh.activated.connect(lambda: self.refresh_preview(force=True))

    # Data / selection --------------------------------------------------
    def populate_sprite_list(self) -> None:
        filt = self.sprite_filter.text().strip().lower() if hasattr(self, "sprite_filter") else ""
        ...
        matches = self.sprite_list.findItems(current, self.QtCore.Qt.MatchFlag.MatchExactly)
        if matches:
    ...
    def populate_animation_tree(self) -> None:
        ...
        for anim, adata in (self.current_manifest.get("animations") or {}).items():
            anim_item = self.QtWidgets.QTreeWidgetItem([anim, f"{adata.get('duration_ms')}ms"])
            anim_item.setData(0, self.QtCore.Qt.ItemDataRole.UserRole, {"kind": "anim", "animation": anim})
            ...
            for frame in adata.get("frames", []):
                ...
                frame_item.setData(0, self.QtCore.Qt.ItemDataRole.UserRole, {"kind": "frame", "animation": anim, "frame": idx})
                ...
                for comp in (frame.get("pose") or {}).get("components", []):
                    ...
                    if conn:
                        ...
                        constraint = ""
                    part_item = self.QtWidgets.QTreeWidgetItem([role, str(comp.get("sprite", "")), constraint])
                    part_item.setData(0, self.QtCore.Qt.ItemDataRole.UserRole, {"kind": "part", "animation": anim, "frame": idx, "role": role})
                    frame_item.addChild(part_item)
            if anim == self.pose_anim.text() if hasattr(self, "pose_anim") else anim == "run":
    ...
    def select_sprite(self, name: str) -> None:
        ...
        matches = self.sprite_list.findItems(name, self.QtCore.Qt.MatchFlag.MatchExactly)
        if matches:
            self.sprite_list.blockSignals(True)
            self.sprite_list.setCurrentItem(matches[0])
            self.sprite_list.blockSignals(False)
    ...
    def populate_anchor_list(self) -> None:
        ...
        if self.selected_anchor not in self.anchor_names():
            self.selected_anchor = "pivot"
        matches = self.anchor_list.findItems(self.selected_anchor, self.QtCore.Qt.MatchFlag.MatchExactly)
        if matches:
    ...
    def update_anchor_fields(self) -> None:
        ...
        self.anchor_x.blockSignals(True); self.anchor_y.blockSignals(True)
        self.anchor_x.setValue(pt[0]); self.anchor_y.setValue(pt[1])
        self.anchor_x.blockSignals(False); self.anchor_y.blockSignals(False)

    def _normalize_anchor_point(self, x: float, y: float) -> Point:
        if getattr(self, "snap_anchor_pixels", None) is not None and self.snap_anchor_pixels.isChecked():
            x, y = round(x), round(y)
        return (float(x), float(y))

    def _apply_anchor_point(self, pt: Point, *, refresh_tree: bool = False, refresh_preview: bool = False, final: bool = False) -> None:
    ...
    def on_component_click(self, x: float, y: float) -> None:
        ...
        self.status.showMessage(f"Placed {self.selected_sprite}.{self.selected_anchor} at ({pt[0]:.2f}, {pt[1]:.2f})")

    def on_component_drag(self, x: float, y: float, final: bool) -> None:
        pt = self._normalize_anchor_point(x, y)
        self._apply_anchor_point(pt, refresh_tree=final, refresh_preview=final, final=final)
        suffix = " committed" if final else ""
        self.status.showMessage(f"Dragging {self.selected_sprite}.{self.selected_anchor} -> ({pt[0]:.2f}, {pt[1]:.2f}){suffix}")

    def on_component_hover(self, x: float, y: float) -> None:
        self.status.showMessage(f"Anchor canvas {self.selected_sprite}: ({x:.2f}, {y:.2f})")
    ...
    def nudge_anchor(self, dx: float, dy: float) -> None:
        ...
        self._apply_anchor_point(pt, refresh_tree=True, refresh_preview=True, final=True)
        self.status.showMessage(f"Nudged {self.selected_sprite}.{self.selected_anchor} -> ({pt[0]:.2f}, {pt[1]:.2f})")

    def apply_anchor_xy(self) -> None:
        pt = self._normalize_anchor_point(float(self.anchor_x.value()), float(self.anchor_y.value()))
        self._apply_anchor_point(pt, refresh_tree=True, refresh_preview=True, final=True)
    ...
    def pivot_follows_selected(self) -> None:
        ...
        self.populate_anchor_list()
        self.selected_anchor = keep_anchor
        matches = self.anchor_list.findItems(keep_anchor, self.QtCore.Qt.MatchFlag.MatchExactly)
        if matches:
            self.anchor_list.blockSignals(True)
            self.anchor_list.setCurrentItem(matches[0])
            self.anchor_list.blockSignals(False)
        ...
        self.draw_component()
        self.refresh_preview()
        self.status.showMessage(f"Pivot now follows {self.selected_sprite}.{keep_anchor}; anchor selection preserved")

    # Pose editing ------------------------------------------------------
    def current_frame_override(self) -> Dict[str, Any]:
        return self.pose_model.frame(self.pose_anim.text(), int(self.pose_frame.value()))
    ...
    def current_effective_override(self) -> Dict[str, Any]:
        return self.rig.interpolated_frame_overrides(self.pose_model.clean_for_save(), self.pose_anim.text(), int(self.pose_frame.value()))

    def scale_field_for_role(self, role: str) -> Optional[str]:
        return getattr(core, "ROLE_TO_SCALE_FIELD", {}).get(role)

    def scale_value_for_role(self, role: str, comp: Mapping[str, Any], fr: Mapping[str, Any]) -> float:
        ...
        anim_defaults = ((self.pose_model.clean_for_save().get("animations") or {}).get(self.pose_anim.text()) or {}).get("defaults") or {}
    ...
    def update_key_status(self) -> None:
        if self.has_current_frame_keyframe():
            ...
            self.key_status.setText("keyframe: inherited/interpolated" if eff else "keyframe: procedural")
    ...
    def load_pose_fields_from_model(self, role: str, anim: str, idx: int, comp: Mapping[str, Any]) -> None:
        fr = self.rig.interpolated_frame_overrides(self.pose_model.clean_for_save(), anim, idx)
        ...
        self.pose_art.blockSignals(True); self.pose_angle.blockSignals(True); self.pose_scale.blockSignals(True); self.pose_dx.blockSignals(True); self.pose_dy.blockSignals(True)
        if sprite_field:
            self.pose_art.setCurrentText(str(fr.get(sprite_field, comp.get("sprite", ""))))
        if angle_field:
        ...
        if delta_field:
            ...
            self.pose_dx.setValue(dx); self.pose_dy.setValue(dy)
        self.pose_art.blockSignals(False); self.pose_angle.blockSignals(False); self.pose_scale.blockSignals(False); self.pose_dx.blockSignals(False); self.pose_dy.blockSignals(False)
    ...
    def apply_pose_edit(self, *, refresh: bool = True, changed_fields: Optional[set[str]] = None) -> None:
        if self.selected_instance is None:
            return
        ...
        if sprite_field and self.pose_art.currentText() and (write_all or "sprite" in changed_fields):
    ...
    def update_anim_fields(self) -> None:
        info = self.rig.animation_info(self.pose_anim.text(), self.pose_model.clean_for_save())
        self.anim_frame_count.blockSignals(True); self.anim_duration_ms.blockSignals(True)
        self.anim_frame_count.setValue(int(info["frames"]))
        self.anim_duration_ms.setValue(int(info["duration_ms"]))
        self.anim_frame_count.blockSignals(False); self.anim_duration_ms.blockSignals(False)
        self.sync_timeline()
        self.populate_z_order()
        self.update_connection_table()
    ...
    def step_frame(self, delta: int) -> None:
        self.set_current_frame(int(self.pose_frame.value()) + int(delta), refresh_sheet=True)
    ...
    def populate_z_order(self) -> None:
        ...
        order = effective.get("z_order") or self.current_frame_override().get("z_order") or list(core.DEFAULT_Z_ORDER)
        ...
        for role in order:
            ...
            checked = str(role) not in hidden and (visible is None or str(role) in visible)
            item.setCheckState(self.QtCore.Qt.CheckState.Checked if checked else self.QtCore.Qt.CheckState.Unchecked)
    ...
    def navigate_connected_part(self) -> None:
        ...
        for comp in (self.find_frame_components(anim, idx) or []):
            ...
            if conn.get("sprite") == self.selected_sprite and conn.get("anchor") == anchor:
                ...
                if role and self.select_rig_role(role, anim=anim, idx=idx, comp=comp, sync_tree=True, refresh=True):
                    self.status.showMessage(f"Navigated {self.selected_sprite}.{anchor} -> {role} ({comp.get('sprite')})")
                    return
        self.status.showMessage(f"No component in {anim}[{idx}] is connected to {self.selected_sprite}.{anchor}")

    # Connection / constraint browser -------------------------------------
    def current_connection_components(self) -> List[Dict[str, Any]]:
        return list(self.find_frame_components(self.pose_anim.text(), int(self.pose_frame.value())) or [])
    ...
    def selected_connection_component(self) -> Optional[Dict[str, Any]]:
        ...
        return self.find_component(self.pose_anim.text(), int(self.pose_frame.value()), str(role))
        ...
        for comp in comps:
            ...
            for col, value in enumerate(values):
                ...
                if col == 4 and snap is not None and float(snap) > 0.5:
                    cell.setToolTip("Non-zero snap error means the child anchor did not land on the parent socket.")
    ...
    def _select_tree_part(self, role: str, *, emit: bool = True) -> bool:
        ...
        for item in self._iter_tree_items():
            ...
            try:
                ...
                frame_ok = False
            if data.get("kind") == "part" and data.get("animation") == anim and frame_ok and data.get("role") == role:
    ...
    def _component_for_role(self, anim: str, idx: int, role: str, fallback: Optional[Mapping[str, Any]] = None) -> Dict[str, Any]:
        comp = self.find_component(anim, idx, role)
        if comp:
            return dict(comp)
    ...
    def select_rig_role(self, role: str, *, anim: Optional[str] = None, idx: Optional[int] = None, comp: Optional[Mapping[str, Any]] = None, sync_tree: bool = True, refresh: bool = True) -> bool:
        ...
        if not role:
            return False
        anim = anim or self.pose_anim.text() or (self.job.animations[0] if self.job.animations else "run")
        if idx is None:
        ...
        if not tree_ok and sync_tree:
            self.status.showMessage(f"Selected {role} from viewport; tree has no current-frame item, so using rig-node selection")
        return True
    ...
    def select_connection_parent_part(self) -> None:
        ...
        if role and role != "root" and self.select_rig_role(role, comp=self.find_component(self.pose_anim.text(), int(self.pose_frame.value()), role), sync_tree=True, refresh=True):
            return
        self.status.showMessage(f"Parent {role or '<none>'} is not a selectable rendered part in this frame")
    ...
    def select_connection_parent_anchor(self) -> None:
        ...
        if sprite in self.sprites:
            ...
            self.status.showMessage(f"Parent socket {conn.get('role')}.{anchor} has no editable art sprite")
    ...
    def select_frame_tree_item(self, anim: str, idx: int) -> bool:
        for i in range(self.tree.topLevelItemCount()):
            ...
            for j in range(anim_item.childCount()):
                ...
                if fdata.get("kind") == "frame" and int(fdata.get("frame", -1)) == int(idx):
        ...
        for anim, adata in (self.current_manifest.get("animations") or {}).items():
            for frame in adata.get("frames", []):
                ...
                if fx <= x < fx + fw and fy <= y < fy + fh:
                    ...
                    self.status.showMessage(f"Selected {anim} frame {idx} from spritesheet preview")
                    return
        self.status.showMessage(f"No rendered frame at spritesheet click ({x:.1f}, {y:.1f})")
    ...
    def on_action_preview_drag_start(self, x: float, y: float) -> None:
        ...
        if not comp:
            return
        role = str(comp.get("role") or "")
        if not role or not self.select_rig_role(role, anim=self.pose_anim.text(), idx=int(getattr(self, "_play_index", self.pose_frame.value())), comp=comp, sync_tree=True, refresh=False):
            return
        ...
        # translation.
        handle = comp.get("endpoint_anchor_world") or comp.get("endpoint") or comp.get("target") or comp.get("parent_target")
        center = comp.get("parent_target") or comp.get("target") or handle
        if not center:
            b = comp.get("bounds") or [x, y, x, y]
            center = [(float(b[0]) + float(b[2])) / 2.0, (float(b[1]) + float(b[3])) / 2.0]
        if not handle:
            handle = center
        ...
        self._direct_drag = {
            ...
            "start_mouse_angle": math.degrees(math.atan2(float(y) - float(center[1]), float(x) - float(center[0]))),
        }
        self.status.showMessage(f"Grabbed {role}; drag uses bone constraints, Shift-drag rotates")
    ...
    def on_action_preview_drag_move(self, x: float, y: float, final: bool) -> None:
        ...
        if shift:
            ...
            if not field:
                self.status.showMessage(f"{role} has no editable drag offset yet; Shift-drag can still rotate it")
                return
            if role in {"front_arm", "back_arm", "front_leg", "back_leg"}:
                hx, hy = drag["start_handle"]
                px, py = drag["parent_target"]
                desired = (hx + mdx, hy + mdy)
                local = self._world_to_torso_local_delta((desired[0] - px, desired[1] - py))
                self._pose_spin_set_delta_silent(local[0], local[1])
            else:
                self._pose_spin_set_delta_silent(float(drag["start_dx"]) + mdx, float(drag["start_dy"]) + mdy)
    ...
    def on_action_preview_click(self, x: float, y: float) -> None:
        ...
        if comp:
            role = str(comp.get("role"))
            self.select_rig_role(role, anim=self.pose_anim.text(), idx=int(getattr(self, "_play_index", self.pose_frame.value())), comp=comp, sync_tree=True, refresh=True)
    ...
    def preview_animations(self) -> List[str]:
        if self.preview_relevant.isChecked() and self.selected_sprite:
            return core.relevant_animations(self.paths.job, self.metadata, self.pose_model.clean_for_save(), self.selected_sprite)
    ...
    def _timer_refresh_preview(self) -> None:
        if self.live_sheet_preview.isChecked():
            ...
            self.render_animation_frame()
            self.status.showMessage("Live full-sheet rendering is paused; action preview updated")
    ...
    def refresh_preview(self, force: bool = False) -> None:
        ...
        try:
            if self.live_sheet_preview.isChecked() or force:
                highlight = dict(self.selected_instance) if self.selected_instance else None
                img, manifest = core.build_preview(
                    self.paths.job,
                    self.metadata,
                    self.pose_model.clean_for_save(),
                ...
                self.current_manifest = manifest
                self.preview_view.set_pillow_image(img, fit=self.preview_fit.isChecked())
                self.update_connection_table()
                message = f"Preview {img.width}x{img.height}"
            else:
                message = "Action preview only; full sheet is paused"
            self.render_animation_frame()
            self.status.showMessage(f"{message}; metadata={'dirty' if self.dirty_meta else 'clean'}, pose={'dirty' if self.dirty_pose else 'clean'}")
        except Exception as ex:
    ...
    def _begin_fast_drag_renderer(self) -> None:
        ...
        asm = self.rig.RobotAssembler(atlas, job.render, pose_overrides=self.pose_model.clean_for_save())
    ...
    def _draw_bone_overlay(self, img: Image.Image, frame_manifest: Mapping[str, Any]) -> Image.Image:
        ...
        for comp in comps:
            if not comp.get("visible", True):
                continue
            role = str(comp.get("role") or "")
            p0 = comp.get("parent_target") or comp.get("target") or comp.get("child_anchor_world")
            p1 = comp.get("endpoint_anchor_world") or comp.get("endpoint") or comp.get("child_anchor_world") or comp.get("target")
            if not p0 or not p1:
                continue
            ...
            draw.ellipse((x0-r0, y0-r0, x0+r0, y0+r0), fill=(255, 80, 80, 235), outline=(255,255,255,220), width=1)
            draw.ellipse((x1-r1, y1-r1, x1+r1, y1+r1), fill=(80, 255, 120, 235), outline=(255,255,255,220), width=1)
        return out

    def _pose_spin_set_delta_silent(self, dx: float, dy: float) -> None:
        self.pose_dx.blockSignals(True); self.pose_dy.blockSignals(True)
        self.pose_dx.setValue(float(dx)); self.pose_dy.setValue(float(dy))
        self.pose_dx.blockSignals(False); self.pose_dy.blockSignals(False)
    ...
    def _component_by_role_from_animation_manifest(self, role: str) -> Optional[Dict[str, Any]]:
        comps = (((getattr(self, "current_animation_manifest", {}) or {}).get("pose") or {}).get("components") or [])
        for comp in comps:
            if comp.get("role") == role:
                return comp
        return None
    ...
    def render_animation_frame(self) -> None:
        anim = self.pose_anim.text() or (self.job.animations[0] if self.job.animations else "run")
        ...
        try:
            if self._fast_drag_renderer is not None:
                ...
                img, frame_manifest = asm.render_frame(anim, idx, debug_parts=self.preview_debug.isChecked())
                ...
                try:
                    ...
                    atlas = self.rig.ComponentAtlas(job.metadata, job.slices)
                    asm = self.rig.RobotAssembler(atlas, job.render, pose_overrides=self.rig.load_pose_overrides(job.pose_overrides))
                    img, frame_manifest = asm.render_frame(anim, idx, debug_parts=self.preview_debug.isChecked())
                    self.current_animation_manifest = frame_manifest
                finally:
            ...
            d.text((10, 10), f"animated preview failed: {ex}", fill=(255, 255, 255, 255))
    ...
    def save(self) -> None:
        ...
        if self.dirty_pose:
            ...
            self.dirty_pose = False
        self.status.showMessage(f"Saved {self.paths.metadata.name} and {self.paths.pose_overrides.name}")


def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="PySide6 editor for robot sprite anchors, logical part instances, poses, z-order, and live previews.")
    ...
    p.add_argument("--background", choices=["checker", "black", "white"], default="checker")
    p.add_argument("--render-preview", type=Path, default=None, help="Headless render of current preview and exit")
    p.add_argument("--anchor-report", type=Path, default=None, help="Headless JSON report of rendered part instances and exit")
    p.add_argument("--animations", nargs="*", default=None, help="Animations to render in headless preview mode")
    p.add_argument("--debug", action="store_true", help="Use solid-color debug render in headless preview mode")
    return p
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.render_preview:
        render_preview(paths, args.render_preview.resolve(), animations=args.animations, debug=args.debug, background="black")
