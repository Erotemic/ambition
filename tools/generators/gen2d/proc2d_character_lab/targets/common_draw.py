...
def draw_capsule(draw: ImageDraw.ImageDraw, a: Point, b: Point, radius: float, fill: Color, outline: Color, outline_w: float = 1.0) -> None:
    ...
    for c in (a, b):
        x, y = c
        draw.ellipse((x - radius - outline_w, y - radius - outline_w, x + radius + outline_w, y + radius + outline_w), fill=outline)
        draw.ellipse((x - radius, y - radius, x + radius, y + radius), fill=fill)


def draw_rotated_rounded_rect(base: Image.Image, center: Point, size: Point, angle: float, radius: float, fill: Color, outline: Color | None = None, outline_w: float = 0.0) -> None:
    ...
    box = (pad + outline_w, pad + outline_w, pad + outline_w + size[0], pad + outline_w + size[1])
...
def draw_rotated_ellipse(base: Image.Image, center: Point, size: Point, angle: float, fill: Color, outline: Color | None = None, outline_w: float = 0.0) -> None:
    w, h = max(2, int(math.ceil(size[0] + outline_w * 4))), max(2, int(math.ceil(size[1] + outline_w * 4)))
    pad = int(max(w, h) * 0.35 + abs(outline_w) + 4)
    layer = Image.new("RGBA", (w + pad * 2, h + pad * 2), (0, 0, 0, 0))
    d = ImageDraw.Draw(layer)
