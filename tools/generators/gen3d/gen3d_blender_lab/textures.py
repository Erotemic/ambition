...
def _make_base_canvas(size: int, c0: Tuple[int, int, int], c1: Tuple[int, int, int]) -> Image.Image:
...
def _draw_robot_texture(size: int, base_hex: str, shadow_hex: str, accent_hex: str, out: Path, dark: bool = False) -> Path:
    ...
    edge = _rgb01_to_u8(_mix(shadow, (0.03, 0.03, 0.04), 0.52 if not dark else 0.34)) + (185,)
    blocks = [
        (0.06, 0.08, 0.44, 0.44),
        (0.52, 0.12, 0.90, 0.38),
        (0.14, 0.56, 0.54, 0.88),
    ...
    draw.rounded_rectangle((size * 0.68, size * 0.18, size * 0.84, size * 0.30), radius=size * 0.02, fill=(255, 255, 255, 170))
    draw.line((size * 0.10, size * 0.22, size * 0.90, size * 0.22), fill=(255, 255, 255, 90), width=max(1, size // 64))
    draw.line((size * 0.16, size * 0.54, size * 0.86, size * 0.54), fill=(255, 255, 255, 70), width=max(1, size // 72))
    ...
    img = Image.fromarray(arr.astype(np.uint8), mode='RGB').filter(ImageFilter.GaussianBlur(radius=max(0.25, size / 360)))
    draw = ImageDraw.Draw(img, 'RGBA')
...
def _draw_goblin_skin_texture(size: int, base_hex: str, shadow_hex: str, eye_hex: str, out: Path) -> Path:
    base = _hex_to_rgb01(base_hex)
    shadow = _hex_to_rgb01(shadow_hex)
    eye = _hex_to_rgb01(eye_hex)
    img = _make_base_canvas(size, _rgb01_to_u8(_mix(base, shadow, 0.10)), _rgb01_to_u8(_mix(base, shadow, 0.30)))
    draw = ImageDraw.Draw(img, 'RGBA')
    ...
    arr = np.clip(arr, 0, 255)
    img = Image.fromarray(arr.astype(np.uint8), mode='RGB').filter(ImageFilter.GaussianBlur(radius=max(0.4, size / 300)))
    out.parent.mkdir(parents=True, exist_ok=True)
    img.save(out)
    return out


def _draw_goblin_cloth_texture(size: int, base_hex: str, shadow_hex: str, accent_hex: str, out: Path) -> Path:
    base = _hex_to_rgb01(base_hex)
    shadow = _hex_to_rgb01(shadow_hex)
    accent = _hex_to_rgb01(accent_hex)
    img = _make_base_canvas(size, _rgb01_to_u8(_mix(base, shadow, 0.06)), _rgb01_to_u8(_mix(base, shadow, 0.30)))
    draw = ImageDraw.Draw(img, 'RGBA')
    ...
    arr = np.clip(arr, 0, 255)
    img = Image.fromarray(arr.astype(np.uint8), mode='RGB').filter(ImageFilter.GaussianBlur(radius=max(0.20, size / 420)))
    out.parent.mkdir(parents=True, exist_ok=True)
    img.save(out)
    return out


def generate_texture_pack(out_dir: Path, spec: Dict[str, object], target: str, size: int = 128) -> Dict[str, str]:
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    result: Dict[str, str] = {}
    if target == 'robot':
        result['primary'] = str(_draw_robot_texture(size, str(spec['primary_color']), str(spec['primary_shadow']), str(spec['accent_color']), out_dir / 'robot_primary.png', dark=False))
        result['dark'] = str(_draw_robot_texture(size, str(spec['dark_color']), '#050509', str(spec['accent2_color']), out_dir / 'robot_dark.png', dark=True))
        result['metal'] = str(_draw_robot_texture(size, str(spec['metal_color']), '#77808D', str(spec['accent_color']), out_dir / 'robot_metal.png', dark=False))
    elif target == 'goblin':
        result['skin'] = str(_draw_goblin_skin_texture(size, str(spec['skin_color']), str(spec['skin_shadow']), str(spec['eye_color']), out_dir / 'goblin_skin.png'))
        result['cloth'] = str(_draw_goblin_cloth_texture(size, str(spec['cloth_color']), str(spec['cloth_shadow']), str(spec['accent_color']), out_dir / 'goblin_cloth.png'))
        result['accent'] = str(_draw_robot_texture(size, str(spec['accent_color']), str(spec['accent2_color']), str(spec['eye_color']), out_dir / 'goblin_accent.png', dark=False))
        result['metal'] = str(_draw_robot_texture(size, str(spec['metal_color']), '#7C74A2', str(spec['accent2_color']), out_dir / 'goblin_metal.png', dark=False))
    else:
