...
def _draw_robot_texture(size: int, base_hex: str, shadow_hex: str, accent_hex: str, out: Path, dark: bool = False) -> Path:
    ...
    img = Image.fromarray(arr.astype(np.uint8), mode='RGB').filter(ImageFilter.GaussianBlur(radius=max(0.5, size / 280)))
    draw = ImageDraw.Draw(img, 'RGBA')
...
def _draw_goblin_cloth_texture(size: int, base_hex: str, shadow_hex: str, accent_hex: str, out: Path) -> Path:
    ...
    arr = np.clip(arr, 0, 255)
    img = Image.fromarray(arr.astype(np.uint8), mode='RGB').filter(ImageFilter.GaussianBlur(radius=max(0.5, size / 300)))
    out.parent.mkdir(parents=True, exist_ok=True)
    img.save(out)
    return out
